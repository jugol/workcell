...
const ADMIN_EMAIL =
  process.env.WORKCELL_RELEASE_SMOKE_EMAIL ??
  process.env.SMOKE_ADMIN_EMAIL ??
  "smoke-admin@workcell.local";
const ADMIN_PASSWORD =
  process.env.WORKCELL_RELEASE_SMOKE_PASSWORD ??
  process.env.SMOKE_ADMIN_PASSWORD ??
  "workcell-smoke-password";
...
test.describe("Docker authenticated onboarding smoke", () => {
  test("logs in, completes onboarding, and triggers the first Orchestrator run", async ({
    page,
  }) => {
    await signIn(page);
    ...
    await expect(page.locator('input[placeholder="Orchestrator"]')).toHaveValue(AGENT_NAME);
    await page.getByRole("button", { name: "Next" }).click();
