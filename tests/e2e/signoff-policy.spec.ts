...
async function setupCompany(boardRequest: APIRequestContext): Promise<TestContext> {
  ...
  if (health.deploymentMode !== "local_trusted") {
    throw new Error(
      `Signoff e2e tests require local_trusted deployment mode, ` +
        ...
        `Set WORKCELL_DEPLOYMENT_MODE=local_trusted or use the webServer config.`,
    );
  }
  ...
  const approver = await createAgent("Approver", "lead", "Lead");
...
async function createIssueWithPolicy(ctx: TestContext, title: string, stages?: unknown[]) {
  ...
  // Proof-gated Done (WC-3): an issue cannot transition to `done` — including the
  // final approval stage of a signoff policy — without a proof bundle. The executor
  // produces one during execution (WC-4); attach it up front so the approval chain
  // can complete. Without this, the final approver's `done` PATCH returns 409.
  ...
  expect(proofRes.ok(), `attach proof → ${proofRes.status()}: ${await proofRes.text()}`).toBe(true);
