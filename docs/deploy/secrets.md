...
Workcell encrypts secrets at rest using a local master key. Agent environment variables that contain sensitive values (API keys, tokens) are stored as encrypted secret references.

## Custody Boundaries

Workcell protects secret values up to the moment they are handed to an agent
or workload:
...
- Transport: values are decrypted server-side and injected into the agent
  ...
  immediately before the call. Workcell does not return decrypted values to
  the board UI.
...
Once a value reaches the consuming process, Workcell can no longer guarantee
...
At runtime, Workcell resolves the selected secret server-side and injects the
...
matches an agent env key, the project value wins before Workcell injects its
own `WORKCELL_*` runtime variables.
...
```
~/.workcell/instances/default/secrets/master.key
```

This key is auto-created during onboarding. The key never leaves your machine.
Workcell best-effort enforces `0600` permissions when it creates or loads the
key file. `workcell doctor` and the provider health API warn when the file is
...
|----------|-------------|
| `WORKCELL_SECRETS_MASTER_KEY` | 32-byte key as base64, hex, or raw string |
| `WORKCELL_SECRETS_MASTER_KEY_FILE` | Custom key file path |
| `WORKCELL_SECRETS_STRICT_MODE` | Set to `true` to enforce secret refs |
...
configuration or `WORKCELL_SECRETS_STRICT_MODE=false`.

## External References

Provider-owned secrets can be linked without copying values into Workcell by
using `managedMode: "external_reference"` plus a provider `externalRef`.
Workcell stores metadata and a non-sensitive fingerprint, never the value.
...
For hosted Workcell Cloud on AWS, see the AWS Secrets Manager operational
...
identity, not in Workcell company secrets**. Allowed credential sources are
workload identity attached to the Workcell server (instance profile, IRSA, ECS
...
can override) the deployment-level `WORKCELL_SECRETS_AWS_*` env. Bootstrap
...
Workcell `external_reference` secrets. This is a metadata-only link: Workcell
...
4. Select an AWS vault, search the remote inventory, and load more pages as
   needed.
5. Check the rows to import, review/edit the Workcell name and key, then
   submit.
...
cursors. Do not expect Workcell to crawl a whole account in the background;
load pages deliberately and retry throttled requests with backoff.

Remote import exposes AWS secret metadata visible to the Workcell runtime
...
- Keep managed create/rotate/delete permissions scoped to the Workcell
...
dedicated Workcell runtime roles per environment/account, AWS vaults pointed at
...
- `local_encrypted`: back up the local master key file and the Workcell
  ...
  key bytes.
- `aws_secrets_manager`: back up Workcell's database for vault metadata
  ...
  configured prefix; restore by pointing the same Workcell company at the
...
- `gcp_secret_manager` and `vault`: while these are coming soon, only the
  draft vault config exists in Workcell. Database backups capture it. There
  is nothing to restore on the provider side until runtime support lands.

### AWS Provider Bootstrap Boundary

The AWS Secrets Manager provider cannot bootstrap itself from Workcell
...
For Workcell Cloud, provision the server runtime IAM role/workload identity,
KMS key, deployment prefix, and non-secret `WORKCELL_SECRETS_AWS_*` environment
...
Workcell secrets. Bootstrap material belongs in infrastructure IAM/workload
...
```sh
pnpm workcell secrets migrate-inline-env --company-id <company-id>
...
Use the CLI command for normal operations because it goes through the Workcell
...
```sh
pnpm workcell secrets declarations --company-id <company-id> --kind secret
```
...
the value custodian; Workcell stores metadata and provider version references,
