...
Workcell supports two runtime modes with different security profiles. Reachability is configured separately with `bind`.
...
```sh
pnpm workcell allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, Workcell emits a one-time claim URL at startup:
...
```sh
WORKCELL_DEPLOYMENT_MODE=authenticated WORKCELL_BIND=lan pnpm workcell run
```
