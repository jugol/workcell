...
summary: Deploy Workcell to AWS using ECS Fargate, RDS Postgres, and EFS
---

Deploy Workcell to AWS with ECS Fargate (compute), RDS Postgres 17 (database), and EFS (persistent storage). This guide uses the AWS CLI and produces a single-task ECS service behind an ALB with HTTPS.
...
- The Workcell repo cloned locally
...
export WORKCELL_DOMAIN=workcell.example.com   # your domain
...
aws ecr create-repository \
  --repository-name workcell-server \
...
# Build
docker build -t workcell-server .

# Tag and push
docker tag workcell-server:latest \
  $AWS_ACCOUNT_ID.dkr.ecr.$AWS_REGION.amazonaws.com/workcell-server:latest
...
ALB_SG=$(aws ec2 create-security-group \
  --group-name workcell-alb \
  --description "Workcell ALB" \
  --vpc-id $VPC_ID \
...
ECS_SG=$(aws ec2 create-security-group \
  --group-name workcell-ecs \
  --description "Workcell ECS tasks" \
  --vpc-id $VPC_ID \
  --query 'GroupId' --output text)
...
RDS_SG=$(aws ec2 create-security-group \
  --group-name workcell-rds \
  --description "Workcell RDS" \
  --vpc-id $VPC_ID \
  --query 'GroupId' --output text)
...
EFS_SG=$(aws ec2 create-security-group \
  --group-name workcell-efs \
  --description "Workcell EFS" \
  --vpc-id $VPC_ID \
  --query 'GroupId' --output text)
...
aws rds create-db-subnet-group \
  --db-subnet-group-name workcell-db-subnet \
  --db-subnet-group-description "Workcell RDS subnets" \
  --subnet-ids $SUBNET_1 $SUBNET_2

aws rds create-db-instance \
  --db-instance-identifier workcell-db \
  --db-instance-class db.t4g.micro \
  --engine postgres \
  --engine-version 17 \
  --master-username workcell \
...
DATABASE_URL="postgresql://workcell:${DB_PASSWORD}@${RDS_ENDPOINT}:5432/workcell"
```
...
EFS_ID=$(aws efs create-file-system \
  ...
  --encrypted \
  --tags Key=Name,Value=workcell-data \
...
aws secretsmanager create-secret \
  --name workcell/database-url \
  --secret-string "$DATABASE_URL"

aws secretsmanager create-secret \
  --name workcell/anthropic-api-key \
  --secret-string "YOUR_ANTHROPIC_KEY"

aws secretsmanager create-secret \
  --name workcell/better-auth-secret \
  --secret-string "$AUTH_SECRET"

aws secretsmanager create-secret \
  --name workcell/openai-api-key \
  --secret-string "YOUR_OPENAI_KEY"

aws secretsmanager create-secret \
  --name workcell/github-token \
...
# Task execution role
aws iam create-role \
  --role-name workcell-ecs-execution \
...
aws iam put-role-policy \
  ...
  --policy-document '{
    ...
    "Statement": [{
      ...
      "Resource": "arn:aws:secretsmanager:'$AWS_REGION':'$AWS_ACCOUNT_ID':secret:workcell/*"
    }]
  }'

# Task role (application — add permissions as needed)
aws iam create-role \
  --role-name workcell-ecs-task \
  --assume-role-policy-document '{
    "Version": "2012-10-17",
    "Statement": [{
      "Effect": "Allow",
...
```bash
aws ecs create-cluster --cluster-name workcell

aws logs create-log-group --log-group-name /ecs/workcell
```
...
sed -e "s|<ACCOUNT_ID>|$AWS_ACCOUNT_ID|g" \
    ...
    -e "s|<EFS_ID>|$EFS_ID|g" \
    -e "s|<DOMAIN>|$WORKCELL_DOMAIN|g" \
    docker/ecs-task-definition.json > /tmp/workcell-task-def.json

aws ecs register-task-definition \
  --cli-input-json file:///tmp/workcell-task-def.json
```
...
- Create a CNAME or ALIAS record for `$WORKCELL_DOMAIN` -> `$ALB_DNS`
...
aws ecs create-service \
  --cluster workcell \
  --service-name workcell-server \
  --task-definition workcell-server \
  --desired-count 1 \
  --launch-type FARGATE \
...
aws ecs describe-services \
  --cluster workcell \
  --services workcell-server \
  --query 'services[0].{desired:desiredCount,running:runningCount,status:status}'

# Check task health
aws ecs list-tasks --cluster workcell --service-name workcell-server
TASK_ARN=$(aws ecs list-tasks --cluster workcell --service-name workcell-server --query 'taskArns[0]' --output text)
aws ecs describe-tasks --cluster workcell --tasks $TASK_ARN \
  --query 'tasks[0].{status:lastStatus,health:healthStatus}'

# Check logs
aws logs tail /ecs/workcell --since 10m --follow

# Hit the health endpoint
curl -sf https://$WORKCELL_DOMAIN/api/health
```
...
aws ecs update-service \
  --cluster workcell \
  --service workcell-server \
  --task-definition workcell-server:<PREVIOUS_REVISION>
```
...
```bash
aws rds stop-db-instance --db-instance-identifier workcell-db
aws rds start-db-instance --db-instance-identifier workcell-db
```
...
aws ecs update-service --cluster workcell --service workcell-server --desired-count 0
aws ecs delete-service --cluster workcell --service workcell-server --force
aws ecs delete-cluster --cluster workcell
...
aws rds delete-db-instance \
  --db-instance-identifier workcell-db \
  --final-db-snapshot-identifier workcell-db-final
aws rds wait db-instance-deleted --db-instance-identifier workcell-db
aws rds delete-db-subnet-group --db-subnet-group-name workcell-db-subnet
...
for s in database-url anthropic-api-key better-auth-secret openai-api-key github-token; do
  aws secretsmanager delete-secret --secret-id workcell/$s --force-delete-without-recovery
done
...
# 7. ECR
aws ecr delete-repository --repository-name workcell-server --force

# 8. IAM roles
aws iam delete-role-policy --role-name workcell-ecs-execution --policy-name SecretsAccess
aws iam detach-role-policy --role-name workcell-ecs-execution \
  --policy-arn arn:aws:iam::aws:policy/service-role/AmazonECSTaskExecutionRolePolicy
aws iam delete-role --role-name workcell-ecs-execution
aws iam delete-role --role-name workcell-ecs-task

# 9. Log group
aws logs delete-log-group --log-group-name /ecs/workcell
```
