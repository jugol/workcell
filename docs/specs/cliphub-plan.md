# ClipHub: Marketplace for Workcell Team Configurations
...
> The "app store" for whole-company AI teams — pre-built Workcell configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Workcell-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Workcell company (agents, projects, governance) |
...
A complete Workcell company configuration:
...
Single-agent configurations designed to plug into a Workcell org:
...
Portable skill files that any Workcell agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Workcell's skill loading system
...
interface Listing {
  ...
  workcellVersionMin: string;     // Minimum Workcell version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Workcell company
...
| `POST` | `/api/listings/:id/install` | Install to Workcell company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Workcell company (or create new)
  → ClipHub API calls Workcell API to:
  ...
  → Redirect to Workcell dashboard with new team running
```
...
### 5.3 Creator: Export from Workcell → Publish

```
Running Workcell company → "Export as Blueprint" (CLI or UI)
  → Workcell exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Workcell brand blue/purple)
...
| Install flow | One-click deploy to Workcell company |
...
{
  "targetCompanyId": "uuid",         // Existing Workcell company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `workcell-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Workcell
- **Backend**: Node.js API (or extend Workcell server)
- **Database**: Postgres (can share Workcell's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Workcell auth (or OAuth2)

### 9.2 Integration with Workcell

ClipHub can be:
- **Option A**: A separate app that calls Workcell's API to install blueprints
- **Option B**: A built-in section of the Workcell UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Workcell UI and API.
...
- [ ] Install flow: blueprint → Workcell company
...
- [ ] "Export from Workcell" CLI command
