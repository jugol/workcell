...
The Workcell CLI handles instance setup, diagnostics, and control-plane operations.
...
| Flag | Description |
|------|-------------|
| `--data-dir <path>` | Local Workcell data root (isolates from `~/.workcell`) |
...
```sh
# Set defaults
pnpm workcell context set --api-base http://localhost:3100 --company-id <id>
...
Secret operations are available under `workcell secrets`:
...
Context is stored at `~/.workcell/context.json`.
