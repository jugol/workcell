...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only Workcell.
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.workcell.yaml`
...
- Workcell must not require extra top-level fields for skill validity
- Workcell-specific extensions must live under `metadata.workcell` or `metadata.sources`
...
For Workcell, the preferred fidelity extension is:
...
- issue/task Workcell-only metadata
...
- Workcell tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested Workcell shape:
...
Additional rules for Workcell exporters:
...
- omit empty and default-valued Workcell fields when possible
...
- Workcell exporters should emit `.workcell.yaml` as a sidecar by default
...
Workcell can map this spec to its runtime model like this:
...
- Workcell extension:
  - `.workcell.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other Workcell-specific fidelity

Inline Workcell-only metadata that must live inside a shared markdown file should use:

- `metadata.workcell`

That keeps the base format broader than Workcell.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only Workcell.

## 20. Cutover

Workcell should cut over to this markdown-first package model as the primary portability format.

`workcell.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For Workcell, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
