...
Workcell is a monorepo with four main layers.
...
│
├── skills/                      # Agent skills
│   └── workcell/               # Core Workcell skill (heartbeat protocol)
│
├── cli/                         # CLI client
...
3. **Agent process** — Adapter spawns the agent (e.g. Claude Code CLI) with Workcell env vars and a prompt
4. **Agent work** — The agent calls Workcell's REST API to check assignments, checkout tasks, do work, and update status
...
Adapters are the bridge between Workcell and agent runtimes. Each adapter is a package with three modules:
...
- **CLI module** — terminal formatter for `workcell run --watch`

Built-in adapters: `claude_local`, `codex_local`, `process`, `http`. You can create custom adapters for any runtime.

## Key Design Decisions

- **Control plane, not execution plane** — Workcell orchestrates agents; it doesn't run them
