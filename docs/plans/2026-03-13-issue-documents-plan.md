...
Add first-class **documents** to Workcell as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by Workcell workflow/docs.
...
Update the Workcell agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/workcell/SKILL.md`
