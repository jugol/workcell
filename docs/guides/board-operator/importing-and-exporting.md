...
Workcell companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .workcell.yaml     # Adapter config, env inputs, routines
```
...
- **.workcell.yaml** holds Workcell-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
```sh
workcell company export <company-id> --out ./my-export
```
...
workcell company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
workcell company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
workcell company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.workcell.yaml`
...
```sh
# From a local folder
workcell company import ./my-export

# From a GitHub URL
workcell company import https://github.com/org/repo

# From a GitHub subfolder
workcell company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
workcell company import org/repo
workcell company import org/repo/companies/acme
```
...
If `--target` is not specified, Workcell infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
```sh
workcell company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
workcell company import org/company-templates/engineering-team \
  --target new \
...
```sh
workcell company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
workcell company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
workcell company import ./package \
  --target new \
  --yes \
  --json
```
...
Workcell supports several GitHub URL formats:
