...
Workcell `external_reference` secrets without copying plaintext into
Workcell.
...
`ListSecrets` only and import stores a Workcell external reference plus
...
remote metadata, or copies secret plaintext into Workcell.
...
- `conflict`: the suggested Workcell `name` or `key` is already in use.
...
Workcell-managed AWS secret through a broad runtime role.
...
Each row may include an optional Workcell `description` entered during review;
...
the Workcell runtime role can list the AWS secret but lacks
