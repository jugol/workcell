...
To link a provider-owned secret without copying the value into Workcell, create
...
```json
{
  ...
  "externalRef": "arn:aws:secretsmanager:us-east-1:123456789012:secret:workcell/prod/stripe",
  "providerVersionRef": "version-id-or-label"
}
```

Workcell stores the provider reference and a non-sensitive fingerprint only.
...
credentials must not be stored in Workcell `company_secrets`.

The equivalent CLI check is:

```sh
pnpm workcell secrets doctor --company-id {companyId}
```
...
Remote import links existing AWS Secrets Manager entries into Workcell as
`external_reference` secrets. Import stores provider reference metadata only; it
does not copy the remote secret plaintext into Workcell.
...
- `duplicate`: a Workcell secret already links the same canonical provider
...
`provider_guardrail`. AWS refs under Workcell's own managed namespace are
blocked as external references; use the Workcell-managed secret flow for those
resources instead.
...
Workcell `name`, `key`, optional Workcell `description`,
`providerVersionRef`, and sanitized `providerMetadata`. Blank descriptions are
stored as `null`; AWS provider descriptions are not copied into Workcell
...
- `imported`: Workcell created an active `external_reference` secret and one
...
real value into the agent process environment. Workcell's custody guarantees
...
```sh
pnpm workcell secrets declarations --company-id {companyId}
```
