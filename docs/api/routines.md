...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-Workcell-Signature` + `X-Workcell-Timestamp` header pair matching the trigger's signing mode.
