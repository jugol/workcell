...
Build a custom adapter to connect Workcell to any agent runtime.
...
|---|---|---|
| Source | Inside `workcell-fork` | Separate npm package |
| Distribution | Ships with Workcell | Independent npm publish |
...
For most cases, **build an external adapter plugin**. It's cleaner, independently versioned, and doesn't require modifying Workcell's source. See [External Adapters](/adapters/external-adapters) for the full guide.
...
1. Read config using safe helpers (`asString`, `asNumber`, etc.) from `@workcell/adapter-utils/server-utils`
2. Build environment with `buildWorkcellEnv(agent)` plus context vars
...
|--------|--------|---------|
| `runChildProcess(cmd, opts)` | `@workcell/adapter-utils/server-utils` | Spawn with timeout, grace, streaming |
| `buildWorkcellEnv(agent)` | `@workcell/adapter-utils/server-utils` | Inject `WORKCELL_*` env vars |
| `renderTemplate(tpl, data)` | `@workcell/adapter-utils/server-utils` | `{{variable}}` substitution |
| `asString(v)` | `@workcell/adapter-utils` | Safe config value extraction |
| `asNumber(v)` | `@workcell/adapter-utils` | Safe number extraction |
...
For built-in adapters registered in Workcell's source:
...
`format-event.ts` — pretty-prints stdout for `workcell run --watch` using `picocolors`.
...
With these flags set, the Workcell UI will automatically show the instructions bundle editor, skills management tab, and working directory field for agents using this adapter — no Workcell source changes required.
...
Make Workcell skills discoverable to your agent runtime without writing to the agent's working directory:
