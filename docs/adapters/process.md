...
- Running a Python script that calls the Workcell API
...
1. Workcell spawns the configured command as a child process
2. Standard Workcell environment variables are injected (`WORKCELL_AGENT_ID`, `WORKCELL_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the Workcell API and perform work.
