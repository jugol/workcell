...
│  Workcell Server  │◄────────────────────────────────│  uiParserCache    │
│  (in-memory)      │                                 └──────────────────┘
└────────┬─────────┘
         │ serves JS to browser
         ▼
┌──────────────────┐   fetch() + eval   ┌──────────────────┐
│  Workcell UI     │─────────────────────→│  parseStdoutLine │
...
### 1. `workcell.adapterUiParser` — contract version
...
The Workcell host checks this field. If the major version is unsupported, the host logs a warning and falls back to the generic parser instead of executing potentially incompatible code.
