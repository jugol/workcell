...
summary: Build, package, and distribute adapters as plugins without modifying Workcell source
---

Workcell supports external adapter plugins that can be installed from npm packages or local directories. External adapters work exactly like built-in adapters — they execute agents, parse output, and render transcripts — but they live in their own package and don't require changes to Workcell's source code.

## Built-in vs External

| | Built-in | External |
|---|---|---|
| Source location | Inside `workcell-fork/packages/adapters/` | Separate npm package or local directory |
...
| Distribution | Ships with Workcell | Published to npm or linked via `file:` |
| Updates | Requires Workcell release | Independent versioning |
...
```json
{
  ...
  "dependencies": {
    "@workcell/adapter-utils": "^2026.325.0",
    "picocolors": "^1.1.0"
  },
  "devDependencies": {
    "@types/node": "^22.0.0",
...
| `workcell.adapterUiParser` | Contract version for the UI parser (`"1.0.0"`) |
...
```ts
import type { ServerAdapterModule } from "@workcell/adapter-utils";
...
export async function execute(
  ...
  // 2. Build environment with Workcell vars injected
  const env = buildWorkcellEnv(agent);
...
#### Available Helpers from `@workcell/adapter-utils`

| Helper | Purpose |
|--------|---------|
| `runChildProcess(command, opts)` | Spawn a child process with timeout, grace period, and streaming callbacks |
| `buildWorkcellEnv(agent)` | Inject `WORKCELL_*` environment variables |
...
```sh
# Via the Workcell UI
# Settings → Adapters → Install from npm → "my-workcell-adapter"

# Or via API
curl -X POST http://localhost:3102/api/adapters \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"packageName": "my-workcell-adapter"}'
```
...
Local adapters are symlinked into Workcell's adapter directory. Changes to the source are picked up on server restart.

### Via adapter-plugins.json

For development, you can also edit `~/.workcell/adapter-plugins.json` directly:

```json
[
  {
    "packageName": "my-workcell-adapter",
...
```ts
import type { AdapterSessionCodec } from "@workcell/adapter-utils";
...
Other Workcell users can then install your adapter by package name from the UI or API.
