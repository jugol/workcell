---
name: workcell-create-plugin
description: >
  Create and develop external Workcell plugins with the CLI-first workflow.
  Use when scaffolding a new plugin, working on a local plugin against a running
  Workcell instance, or updating plugin authoring docs. Covers `workcell
  plugin init`, the local install loop via `workcell plugin install <path>`,
  worker/UI rebuild and reload semantics, and the required success checklist.
---

# Create and develop a Workcell plugin

Use this skill when the task is to create, scaffold, or iterate on a Workcell plugin against a local Workcell instance.

## 1. Default: build the plugin OUTSIDE Workcell core
...
- Scaffold the plugin into a directory outside the Workcell checkout (e.g. `~/dev/workcell-plugins/<name>`).
- Install it into the running Workcell instance by local absolute path.
- Edit code in the external package; let Workcell pick up rebuilt output.

Only edit Workcell core itself when the user asks to surface a plugin as a bundled example (`server/src/routes/plugins.ts`, in-repo example lists, docs).
...
Use `workcell plugin init`. Do not invoke the scaffold package node entrypoint by hand unless the CLI command is unavailable in the environment.

```bash
workcell plugin init @acme/my-plugin --output ~/dev/workcell-plugins
```
...
- `--sdk-path <path>` — snapshot the local SDK from a Workcell checkout into `.workcell-sdk/` (useful when developing against an unreleased SDK).

On success the command prints the exact next commands (`cd`, `pnpm install`, `pnpm dev`, `workcell plugin install <abs-path>`). Run them in order.

If `workcell` is not on PATH in your environment, fall back to:

```bash
pnpm --filter @workcell/create-workcell-plugin build
...
workcell plugin install /absolute/path/to/my-plugin
```

Notes:

- `workcell plugin install` auto-detects local paths (absolute, `./`, `../`, `~`, or an existing relative folder) and forwards `isLocalPath: true` to the server. Pass `--local` to force local mode if the heuristic is ambiguous.
...
workcell plugin inspect <plugin-key>
```
...
- `package.json` — `workcellPlugin` block points at `dist/manifest.js`, `dist/worker.js`, `dist/ui/`
...
If you changed Workcell SDK/host/plugin runtime code in addition to the plugin, also run the relevant Workcell workspace checks.
...
- **Commands run** — the exact `workcell plugin init`, `pnpm install`, `pnpm dev`, `workcell plugin install <path>` invocations (and any verification commands).
- **Install status** — output of `workcell plugin list` / `plugin inspect` (plugin key, version, status). Note if `status` is anything other than `ready` and include `lastError`.
...
## 8. When NOT to edit Workcell core
