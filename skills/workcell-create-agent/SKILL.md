...
description: >
  Create new agents in Workcell with governance-aware hiring. Use when you need
  to inspect adapter configuration options, compare existing agent configs,
  draft a new agent prompt/config, and submit a hire request.
---

# Workcell Create Agent Skill
...
```sh
curl -sS "$WORKCELL_API_URL/api/agents/me" \
  -H "Authorization: Bearer $WORKCELL_API_KEY"
```

### 2. Discover adapter configuration for this Workcell instance

```sh
curl -sS "$WORKCELL_API_URL/llms/agent-configuration.txt" \
  -H "Authorization: Bearer $WORKCELL_API_KEY"

# Then the specific adapter you plan to use, e.g. claude_local:
curl -sS "$WORKCELL_API_URL/llms/agent-configuration/claude_local.txt" \
  -H "Authorization: Bearer $WORKCELL_API_KEY"
```
...
```sh
curl -sS "$WORKCELL_API_URL/api/companies/$WORKCELL_COMPANY_ID/agent-configurations" \
  -H "Authorization: Bearer $WORKCELL_API_KEY"
```
...
`skills/workcell-create-agent/references/agent-instruction-templates.md`

Generic fallback for no-template hires:
`skills/workcell-create-agent/references/baseline-role-guide.md`
...
```sh
curl -sS "$WORKCELL_API_URL/llms/agent-icons.txt" \
  -H "Authorization: Bearer $WORKCELL_API_KEY"
```
...
- for coding or execution agents, include the Workcell execution contract: start actionable work in the same heartbeat; do not stop at a plan unless planning was requested; leave durable progress with a clear next action; use child issues for long or parallel delegated work instead of polling; mark blocked work with owner/action; respect budget, pause/cancel, approval gates, and company boundaries
...
`skills/workcell-create-agent/references/draft-review-checklist.md`

### 8. Submit hire request

```sh
curl -sS -X POST "$WORKCELL_API_URL/api/companies/$WORKCELL_COMPANY_ID/agent-hires" \
  -H "Authorization: Bearer $WORKCELL_API_KEY" \
  ...
  -d '{
    ...
    "title": "Engineer",
    "icon": "code",
    "reportsTo": "<orchestrator-agent-id>",
    "capabilities": "Implements features, fixes bugs, owns technical execution",
...
- when the board approves, you will be woken with `WORKCELL_APPROVAL_ID`; read linked issues and close/comment follow-up

```sh
curl -sS "$WORKCELL_API_URL/api/approvals/<approval-id>" \
  -H "Authorization: Bearer $WORKCELL_API_KEY"

curl -sS -X POST "$WORKCELL_API_URL/api/approvals/<approval-id>/comments" \
  -H "Authorization: Bearer $WORKCELL_API_KEY" \
  -H "Content-Type: application/json" \
...
```sh
curl -sS -X POST "$WORKCELL_API_URL/api/issues/<issue-id>/approvals" \
  -H "Authorization: Bearer $WORKCELL_API_KEY" \
  -H "Content-Type: application/json" \
...
```sh
curl -sS "$WORKCELL_API_URL/api/approvals/$WORKCELL_APPROVAL_ID" \
  -H "Authorization: Bearer $WORKCELL_API_KEY"

curl -sS "$WORKCELL_API_URL/api/approvals/$WORKCELL_APPROVAL_ID/issues" \
  -H "Authorization: Bearer $WORKCELL_API_KEY"
```
...
- Template index and how to apply a template: `skills/workcell-create-agent/references/agent-instruction-templates.md`
- Individual role templates: `skills/workcell-create-agent/references/agents/`
- Generic baseline role guide (no-template fallback): `skills/workcell-create-agent/references/baseline-role-guide.md`
- Pre-submit draft-review checklist: `skills/workcell-create-agent/references/draft-review-checklist.md`
- Endpoint payload shapes and full examples: `skills/workcell-create-agent/references/api-reference.md`
