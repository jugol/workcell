...
When you wake up, follow the Workcell skill. It contains the full heartbeat procedure.
...
- On a greenfield project with no convention to follow yet, use the project-root `AGENTS.md` layout if present; otherwise scaffold a conventional tree (`src/`, `tests/`, `docs/`, config at the root) and record the layout you chose at the project root so other agents converge on it instead of sprawling
