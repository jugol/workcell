...
test("collectInternalDependencyProblems flags missing internal versions", () => {
  const manifest = {
    dependencies: {
      "@workcell/plugin-sdk": "2026.425.0-canary.5",
      e2b: "^2.19.0",
    },
  };
  ...
  assert.deepEqual(
    ...
    ["dependencies requires @workcell/plugin-sdk@2026.425.0-canary.5, but npm does not expose that version"],
  );
});
...
test("collectInternalDependencyProblems accepts version-specific manifests when the root document is stale", () => {
  ...
  const packageManifestsByKey = new Map([
    [
      createManifestLookupKey("@workcell/plugin-sdk", "2026.425.0-canary.5"),
      { name: "@workcell/plugin-sdk", version: "2026.425.0-canary.5" },
    ],
  ]);
...
test("collectInternalDependencyProblems ignores peer dependency range specifiers", () => {
  const manifest = {
    peerDependencies: {
      "@workcell/server": "^2026.430.0-canary.0",
    },
  };
...
test("collectInternalDependencyProblems reports unfetched transitive dependency metadata neutrally", () => {
  const manifest = {
    optionalDependencies: {
      "@workcell/browser": "2026.430.0-canary.0",
    },
  };
  ...
  assert.deepEqual(
    collectInternalDependencyProblems(manifest, new Map()),
    [
      "optionalDependencies requires @workcell/browser@2026.430.0-canary.0, but npm publication metadata was not fetched for that dependency",
    ],
  );
});
...
test("verifyPackageRegistryState tolerates a stale root versions map when dist-tags and direct manifests are correct", () => {
  ...
  const packageManifestsByKey = new Map([
    [
      createManifestLookupKey("@workcell/ui", "2026.430.0-canary.0"),
      {
        name: "@workcell/ui",
        version: "2026.430.0-canary.0",
        dependencies: {
          "@workcell/shared": "2026.430.0-canary.0",
        },
      },
    ],
    [
      createManifestLookupKey("@workcell/shared", "2026.430.0-canary.0"),
      {
        name: "@workcell/shared",
        version: "2026.430.0-canary.0",
      },
    ],
  ]);
  ...
  assert.deepEqual(
    verifyPackageRegistryState({
      packageName: "@workcell/ui",
      packageDoc: packageDocsByName.get("@workcell/ui"),
      packageDocsByName,
...
test("verifyPackageRegistryState fails when canary latest is left in place by default", () => {
    ...
    [
      "@workcell/plugin-e2b",
      {
        ...
        versions: {
          ...
          "2026.427.0-canary.3": {
            dependencies: {
              "@workcell/plugin-sdk": "2026.427.0-canary.3",
            },
          },
        },
      },
  ...
  assert.deepEqual(
    verifyPackageRegistryState({
      packageName: "@workcell/plugin-e2b",
      packageDoc: packageDocsByName.get("@workcell/plugin-e2b"),
      packageDocsByName,
      channel: "canary",
      distTag: "canary",
    ...
    [
      "@workcell/plugin-e2b: latest dist-tag still resolves to canary 2026.425.0-canary.5; if that state is intentional, rerun the verification script directly with --allow-canary-latest",
      "@workcell/plugin-e2b@2026.425.0-canary.5 via latest: dependencies requires @workcell/plugin-sdk@2026.425.0-canary.5, but npm does not expose that version",
    ],
  );
});
...
test("verifyPackageRegistryState allows intentional canary latest but still checks dependencies", () => {
    ...
    [
      "workcell",
      {
        ...
        versions: {
          "2026.427.0-canary.3": {
            dependencies: {
              "@workcell/server": "2026.427.0-canary.3",
            },
          },
        },
      },
  ...
  assert.deepEqual(
    verifyPackageRegistryState({
      packageName: "workcell",
      packageDoc: packageDocsByName.get("workcell"),
      packageDocsByName,
      channel: "canary",
      distTag: "canary",
      targetVersion: "2026.427.0-canary.3",
...
test("verifyPackageRegistryState still fails when the dist-tag is stale", () => {
  ...
  assert.deepEqual(
    ...
    ["@workcell/ui: dist-tag canary resolves to 2026.429.0-canary.2, expected 2026.430.0-canary.0"],
  );
});
...
test("verifyPackageRegistryState ignores internal peer dependency ranges", () => {
  ...
  assert.deepEqual(
    verifyPackageRegistryState({
      packageName: "@workcell/plugin-sdk",
      packageDoc: packageDocsByName.get("@workcell/plugin-sdk"),
      packageDocsByName,
      channel: "canary",
      distTag: "canary",
      targetVersion: "2026.430.0-canary.0",
...
test("fetchRegistryJson times out hung requests", async () => {
  ...
  try {
    await assert.rejects(
      fetchRegistryJson(new URL("https://registry.npmjs.org/@workcell%2Fui"), { timeoutMs: 1 }),
      /timed out/,
    );
  } finally {
