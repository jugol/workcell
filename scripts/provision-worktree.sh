...
base_cwd="${WORKCELL_WORKSPACE_BASE_CWD:?WORKCELL_WORKSPACE_BASE_CWD is required}"
worktree_cwd="${WORKCELL_WORKSPACE_CWD:?WORKCELL_WORKSPACE_CWD is required}"
workcell_home="${WORKCELL_HOME:-$HOME/.workcell}"
workcell_instance_id="${WORKCELL_INSTANCE_ID:-default}"
workcell_dir="$worktree_cwd/.workcell"
worktree_config_path="$workcell_dir/config.json"
worktree_env_path="$workcell_dir/.env"
worktree_name="${WORKCELL_WORKSPACE_BRANCH:-$(basename "$worktree_cwd")}"
...
source_config_path="${WORKCELL_CONFIG:-}"
if [[ -z "$source_config_path" && ( -e "$base_cwd/.workcell/config.json" || -L "$base_cwd/.workcell/config.json" ) ]]; then
  source_config_path="$base_cwd/.workcell/config.json"
fi
if [[ -z "$source_config_path" ]]; then
  source_config_path="$workcell_home/instances/$workcell_instance_id/config.json"
fi
source_env_path="$(dirname "$source_config_path")/.env"

mkdir -p "$workcell_dir"
...
run_isolated_worktree_init() {
  ...
  if command -v pnpm >/dev/null 2>&1 && pnpm workcell --help >/dev/null 2>&1; then
    (
      cd "$worktree_cwd"
      pnpm workcell worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    )
    return 0
  fi

  if command -v workcell >/dev/null 2>&1; then
    (
      cd "$worktree_cwd"
      workcell worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    )
    return 0
  fi
...
write_fallback_worktree_config() {
  ...
  WORKCELL_DIR="$workcell_dir" \
  SOURCE_CONFIG_PATH="$source_config_path" \
  SOURCE_ENV_PATH="$source_env_path" \
  WORKCELL_WORKTREES_DIR="${WORKCELL_WORKTREES_DIR:-}" \
  node <<'EOF'
...
async function main() {
  ...
  const workcellDir = process.env.WORKCELL_DIR;
  const sourceConfigPath = process.env.SOURCE_CONFIG_PATH;
  const sourceEnvPath = process.env.SOURCE_ENV_PATH;
  const worktreeHome = path.resolve(expandHomePrefix(nonEmpty(process.env.WORKCELL_WORKTREES_DIR) ?? "~/.workcell-worktrees"));
  const instanceId = sanitizeInstanceId(worktreeName);
  const instanceRoot = path.resolve(worktreeHome, "instances", instanceId);
  const configPath = path.resolve(workcellDir, "config.json");
  const envPath = path.resolve(workcellDir, ".env");
  ...
  const targetConfig = {
    ...
    storage: {
      ...
      s3: {
        bucket: sourceConfig?.storage?.s3?.bucket ?? "workcell",
  ...
  const inlineMasterKey = nonEmpty(sourceEnvEntries.WORKCELL_SECRETS_MASTER_KEY);
  if (inlineMasterKey) {
    ...
    const sourceKeyFilePath = nonEmpty(sourceEnvEntries.WORKCELL_SECRETS_MASTER_KEY_FILE)
      ? resolveRuntimeLikePath(sourceEnvEntries.WORKCELL_SECRETS_MASTER_KEY_FILE, sourceConfigPath)
  ...
  const envLines = [
    "WORKCELL_HOME=" + JSON.stringify(worktreeHome),
    "WORKCELL_INSTANCE_ID=" + JSON.stringify(instanceId),
    "WORKCELL_CONFIG=" + JSON.stringify(configPath),
    "WORKCELL_CONTEXT=" + JSON.stringify(path.resolve(worktreeHome, "context.json")),
    "WORKCELL_IN_WORKTREE=true",
    "WORKCELL_WORKTREE_NAME=" + JSON.stringify(worktreeName),
  ];

  const agentJwtSecret = nonEmpty(sourceEnvEntries.WORKCELL_AGENT_JWT_SECRET);
  if (agentJwtSecret) {
    envLines.push("WORKCELL_AGENT_JWT_SECRET=" + JSON.stringify(agentJwtSecret));
  }
...
if [[ -e "$worktree_config_path" && -e "$worktree_env_path" ]]; then
  echo "Reusing existing isolated Workcell worktree config at $worktree_config_path" >&2
else
  if workcell_command_available; then
    run_isolated_worktree_init
  else
    echo "workcell CLI not available in this workspace; writing isolated fallback config without DB seeding." >&2
    write_fallback_worktree_config
  fi
fi
...
if [[ -f "$worktree_cwd/package.json" && -f "$worktree_cwd/pnpm-lock.yaml" ]]; then
  ...
  if [[ "$needs_install" -eq 1 ]]; then
    backup_suffix=".workcell-backup-${BASHPID:-$$}"
    moved_symlink_paths=()
