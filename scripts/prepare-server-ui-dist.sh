...
# This keeps @workcell/server publish artifacts self-contained for static UI serving.
# When WORKCELL_RELEASE_REUSE_UI_DIST=1 and ui/dist already exists, reuse that
...
case "${WORKCELL_RELEASE_REUSE_UI_DIST:-}" in
  1|true|TRUE|yes|YES)
...
if [ "$should_reuse_existing_ui_dist" = true ] && [ -f "$UI_DIST/index.html" ]; then
  echo "  -> Reusing existing @workcell/ui dist output"
else
  echo "  -> Building @workcell/ui..."
  pnpm --dir "$REPO_ROOT" --filter @workcell/ui build
fi
