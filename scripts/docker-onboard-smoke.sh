...
IMAGE_NAME="${IMAGE_NAME:-workcell-onboard-smoke}"
HOST_PORT="${HOST_PORT:-3131}"
WORKCELLAI_VERSION="${WORKCELLAI_VERSION:-latest}"
...
WORKCELL_DEPLOYMENT_MODE="${WORKCELL_DEPLOYMENT_MODE:-authenticated}"
WORKCELL_DEPLOYMENT_EXPOSURE="${WORKCELL_DEPLOYMENT_EXPOSURE:-private}"
WORKCELL_PUBLIC_URL="${WORKCELL_PUBLIC_URL:-http://localhost:${HOST_PORT}}"
SMOKE_AUTO_BOOTSTRAP="${SMOKE_AUTO_BOOTSTRAP:-true}"
SMOKE_ADMIN_NAME="${SMOKE_ADMIN_NAME:-Smoke Admin}"
SMOKE_ADMIN_EMAIL="${SMOKE_ADMIN_EMAIL:-smoke-admin@workcell.local}"
SMOKE_ADMIN_PASSWORD="${SMOKE_ADMIN_PASSWORD:-workcell-smoke-password}"
...
write_metadata_file() {
  ...
  mkdir -p "$(dirname "$SMOKE_METADATA_FILE")"
  {
    ...
    printf 'SMOKE_WORKCELLAI_VERSION=%q\n' "$WORKCELLAI_VERSION"
  } >"$SMOKE_METADATA_FILE"
}
...
generate_bootstrap_invite_url() {
  ...
  if bootstrap_output="$(
    docker exec \
      -e WORKCELL_DEPLOYMENT_MODE="$WORKCELL_DEPLOYMENT_MODE" \
      -e WORKCELL_DEPLOYMENT_EXPOSURE="$WORKCELL_DEPLOYMENT_EXPOSURE" \
      -e WORKCELL_PUBLIC_URL="$WORKCELL_PUBLIC_URL" \
      -e WORKCELL_HOME="/workcell" \
      "$CONTAINER_NAME" bash -lc \
      'timeout 20s npx --yes "workcell@${WORKCELLAI_VERSION}" auth bootstrap-admin --data-dir "$WORKCELL_HOME" --base-url "$WORKCELL_PUBLIC_URL"' \
      2>&1
  )"; then
    bootstrap_status=0
  else
  ...
  if [[ $bootstrap_status -ne 0 && $bootstrap_status -ne 124 ]]; then
    echo "Smoke bootstrap failed: could not run bootstrap-admin inside container" >&2
    printf '%s\n' "$bootstrap_output" >&2
    return 1
  fi
  ...
  if [[ -z "$invite_url" ]]; then
    echo "Smoke bootstrap failed: bootstrap-admin did not print an invite URL" >&2
    printf '%s\n' "$bootstrap_output" >&2
    return 1
  fi

  if [[ $bootstrap_status -eq 124 ]]; then
    echo "    Smoke bootstrap: bootstrap-admin timed out after printing invite URL; continuing" >&2
  fi
...
sign_up_or_sign_in() {
  ...
  signup_status="$(post_json_with_cookies \
    "$WORKCELL_PUBLIC_URL/api/auth/sign-up/email" \
  ...
  signin_status="$(post_json_with_cookies \
    "$WORKCELL_PUBLIC_URL/api/auth/sign-in/email" \
...
auto_bootstrap_authenticated_smoke() {
  local health_url="$WORKCELL_PUBLIC_URL/api/health"
  local health_json
  ...
  if [[ "$health_json" == *'"bootstrapStatus":"ready"'* ]]; then
    ...
    echo "    Smoke bootstrap: generated bootstrap invite via auth bootstrap-admin"

    local invite_token="${invite_url##*/}"
    local accept_response="$TMP_DIR/accept.json"
    local accept_status
    accept_status="$(post_json_with_cookies \
      "$WORKCELL_PUBLIC_URL/api/invites/$invite_token/accept" \
      '{"requestType":"human"}' \
      "$accept_response")"
  ...
  local session_json
  session_json="$(get_with_cookies "$WORKCELL_PUBLIC_URL/api/auth/get-session")"
  ...
  companies_json="$(get_with_cookies "$WORKCELL_PUBLIC_URL/api/companies")"
...
docker build \
  --build-arg WORKCELLAI_VERSION="$WORKCELLAI_VERSION" \
...
echo "    Public URL: $WORKCELL_PUBLIC_URL"
echo "    Smoke auto-bootstrap: $SMOKE_AUTO_BOOTSTRAP"
echo "    Detached mode: $SMOKE_DETACH"
echo "    Data dir: $DATA_DIR"
echo "    Deployment: $WORKCELL_DEPLOYMENT_MODE/$WORKCELL_DEPLOYMENT_EXPOSURE"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/workcell-onboard-smoke.XXXXXX")"
COOKIE_JAR="$TMP_DIR/cookies.txt"

if ! wait_for_http "$WORKCELL_PUBLIC_URL/api/health" 90 1; then
  echo "Smoke bootstrap failed: server did not become ready at $WORKCELL_PUBLIC_URL/api/health" >&2
  exit 1
fi

if [[ "$SMOKE_AUTO_BOOTSTRAP" == "true" && "$WORKCELL_DEPLOYMENT_MODE" == "authenticated" ]]; then
  auto_bootstrap_authenticated_smoke
fi
...
if [[ "$SMOKE_DETACH" == "true" ]]; then
  ...
  echo "    Smoke base URL: $WORKCELL_PUBLIC_URL"
  echo "    Smoke admin credentials: $SMOKE_ADMIN_EMAIL / $SMOKE_ADMIN_PASSWORD"
