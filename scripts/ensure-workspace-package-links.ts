...
async function ensureWorkspaceLinksCurrent(workspaceDir: string) {
  ...
  console.log(`[workcell] detected stale workspace package links for ${workspaceDir}; relinking dependencies...`);
  for (const mismatch of mismatches) {
    console.log(
      `[workcell]   ${mismatch.packageName}: ${mismatch.actualPath ?? "missing"} -> ${mismatch.expectedPath}`,
    );
  }
