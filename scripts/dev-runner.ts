...
if (worktreeEnvBootstrap.missingEnv) {
  console.error(
    `[workcell] linked git worktree at ${repoRoot} is missing ${path.relative(repoRoot, worktreeEnvBootstrap.envPath)}. Run \`workcell worktree init\` in this worktree before \`pnpm dev\`.`,
  );
  process.exit(1);
}
...
const devServerStatusFilePath = path.join(repoRoot, ".workcell", "dev-server-status.json");
const devServerRestartRequestFilePath = path.join(repoRoot, ".workcell", "dev-server-restart-request.json");
const devServerStatusToken = mode === "dev" ? randomUUID() : null;
const devServerStatusTokenHeader = "x-workcell-dev-server-status-token";
...
const ignoredRelativePaths = new Set([
  ".workcell/dev-server-restart-request.json",
  ".workcell/dev-server-status.json",
]);
...
for (let index = 0; index < cliArgs.length; index += 1) {
  ...
  if (arg === "--bind") {
    ...
    if (!value || value.startsWith("--") || !BIND_MODES.includes(value as BindMode)) {
      console.error(`[workcell] invalid --bind value. Use one of: ${BIND_MODES.join(", ")}`);
      process.exit(1);
    }
  ...
  if (arg === "--bind-host") {
    ...
    if (!value || value.startsWith("--")) {
      console.error("[workcell] --bind-host requires a value");
      process.exit(1);
    }
    bindHost = value;
    index += 1;
...
if (bindMode === "custom" && !bindHost) {
  console.error("[workcell] --bind custom requires --bind-host <host>");
  process.exit(1);
}

const env: NodeJS.ProcessEnv = {
  ...process.env,
  WORKCELL_UI_DEV_MIDDLEWARE: "true",
};

if (mode === "dev") {
  env.WORKCELL_DEV_SERVER_STATUS_FILE = devServerStatusFilePath;
  env.WORKCELL_DEV_SERVER_STATUS_TOKEN = devServerStatusToken ?? "";
  env.WORKCELL_MIGRATION_AUTO_APPLY ??= "true";
}

if (mode === "watch") {
  delete env.WORKCELL_DEV_SERVER_STATUS_TOKEN;
  env.WORKCELL_MIGRATION_PROMPT ??= "never";
  env.WORKCELL_MIGRATION_AUTO_APPLY ??= "true";
}
...
if (tailscaleAuth || bindMode) {
  ...
  if (tailscaleAuth) {
    console.log("[workcell] note: --tailscale-auth/--authenticated-private are legacy aliases for --bind lan");
  }
  env.WORKCELL_BIND = effectiveBind;
  if (bindHost) {
    env.WORKCELL_BIND_HOST = bindHost;
  } else {
    delete env.WORKCELL_BIND_HOST;
  }
  if (effectiveBind === "loopback" && !tailscaleAuth) {
    delete env.WORKCELL_DEPLOYMENT_MODE;
    delete env.WORKCELL_DEPLOYMENT_EXPOSURE;
    delete env.WORKCELL_AUTH_BASE_URL_MODE;
    console.log("[workcell] dev mode: local_trusted (bind=loopback)");
  } else {
    env.WORKCELL_DEPLOYMENT_MODE = "authenticated";
    env.WORKCELL_DEPLOYMENT_EXPOSURE = "private";
    env.WORKCELL_AUTH_BASE_URL_MODE = "auto";
    console.log(
      `[workcell] dev mode: authenticated/private (bind=${effectiveBind}${bindHost ? `:${bindHost}` : ""})`,
    );
  }
} else {
  delete env.WORKCELL_BIND;
  delete env.WORKCELL_BIND_HOST;
  delete env.WORKCELL_DEPLOYMENT_MODE;
  delete env.WORKCELL_DEPLOYMENT_EXPOSURE;
  delete env.WORKCELL_AUTH_BASE_URL_MODE;
  console.log("[workcell] dev mode: local_trusted (default)");
}
...
if (existingRunner) {
  console.log(
    `[workcell] ${devService.serviceName} already running (pid ${existingRunner.pid}${typeof existingRunner.metadata?.childPid === "number" ? `, child ${existingRunner.metadata.childPid}` : ""})`,
  );
  process.exit(0);
}
...
async function getMigrationStatusPayload() {
  const status = await runPnpm(
    ["--filter", "@workcell/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[workcell] Command failed with code ${status.code}: pnpm --filter @workcell/db exec tsx src/migration-status.ts --json\n`,
    );
  ...
  try {
    ...
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        "[workcell] migration-status returned invalid JSON payload\n",
    );
...
async function maybePreflightMigrations(options: { interactive?: boolean; autoApply?: boolean; exitOnDecline?: boolean } = {}) {
  ...
  const autoApply = options.autoApply ?? env.WORKCELL_MIGRATION_AUTO_APPLY === "true";
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[workcell] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). Refusing to start watch mode against a stale schema.\n`,
      );
      process.exit(1);
    }
    return;
...
async function buildPluginSdk() {
  console.log("[workcell] building plugin sdk...");
  const result = await runPnpm(
    ["--filter", "@workcell/plugin-sdk", "build"],
    { stdio: "inherit" },
  );
  if (result.signal) {
  ...
  if (result.code !== 0) {
    console.error("[workcell] plugin sdk build failed");
...
async function startServerChild() {
  ...
  child = spawn(
    pnpmBin,
    ["--filter", "@workcell/server", serverScript, ...forwardedArgs],
