#!/usr/bin/env node
/**
 * Screenshot utility for Workcell UI.
 *
 * Reads the board token from ~/.workcell/auth.json and injects it as a
...
const rawUrl = args[0];
const outPath = args[1] || "/tmp/workcell-screenshot.png";
...
function loadBoardToken() {
  const authPath = path.resolve(os.homedir(), ".workcell/auth.json");
  try {
...
if (!cred) {
  console.error("No board token found in ~/.workcell/auth.json");
  process.exit(1);
}
...
(async () => {
  ...
  try {
    ...
    // Scope the auth header to the Workcell origin only
