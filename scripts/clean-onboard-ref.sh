...
Environment overrides:
  ...
  PC_DATA=/tmp/data           Workcell data dir to use
  WORKCELL_HOST=127.0.0.1    Host passed to the onboarded server
  WORKCELL_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/workcell-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/workcell-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export WORKCELL_HOST="${WORKCELL_HOST:-127.0.0.1}"
export WORKCELL_PORT="${WORKCELL_PORT:-3100}"
export WORKCELL_OPEN_ON_LISTEN="${WORKCELL_OPEN_ON_LISTEN:-false}"
...
echo "WORKCELL_HOST: $WORKCELL_HOST"
echo "WORKCELL_PORT: $WORKCELL_PORT"
...
env \
  ...
  HOST="$WORKCELL_HOST" \
  PORT="$WORKCELL_PORT" \
  WORKCELL_OPEN_ON_LISTEN="$WORKCELL_OPEN_ON_LISTEN" \
  pnpm workcell onboard --yes --data-dir "$PC_DATA"
