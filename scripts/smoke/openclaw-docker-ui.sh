...
WORKCELL_HOST_PORT="${WORKCELL_HOST_PORT:-3100}"
WORKCELL_HOST_FROM_CONTAINER="${WORKCELL_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.workcell-openclaw.override.yml"
...
detect_workcell_base_url() {
  ...
  for candidate in "$WORKCELL_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${WORKCELL_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${WORKCELL_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
workcell_base_url="$(detect_workcell_base_url || true)"
...
Workcell URL for OpenClaw container:
EOF
  if [[ -n "$workcell_base_url" ]]; then
    cat <<EOF
  $workcell_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${WORKCELL_HOST_PORT}
  (Do not use http://127.0.0.1:${WORKCELL_HOST_PORT} inside the container.)
  If Workcell rejects the host, run on host machine:
    pnpm workcell allowed-hostname host.docker.internal
  Then restart Workcell and re-run this script.
EOF
  fi
  cat <<EOF
