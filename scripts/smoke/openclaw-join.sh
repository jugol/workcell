...
COMPANY_ID="${COMPANY_ID:-${WORKCELL_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-workcell-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-workcell-openclaw-smoke}"
...
Provide one of:
  WORKCELL_AUTH_HEADER=\"Bearer <board-token>\"
  WORKCELL_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "Workcell OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.workcell.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
