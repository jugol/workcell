...
const DEFAULT_QUERY = "\"Co-Authored-By: Workcell <noreply@workcell.ing>\"";
const DEFAULT_CACHE_FILE = path.resolve("data/workcell-commit-metrics-cache.json");
...
const WORKCELL_EMAIL = "noreply@workcell.ing";
const WORKCELL_NAME = "workcell";
...
function printHelp() {
  console.log(`Usage: tsx scripts/workcell-commit-metrics.ts [options]
...
function normalizeContributor(input: {
  ...
  if ((email && email === WORKCELL_EMAIL) || (displayName && displayName.toLowerCase() === WORKCELL_NAME)) {
    return null;
  }
...
function printSummary(summary: Summary) {
  console.log("Workcell commit metrics");
    ...
    while (true) {
      const response = await fetch(`${this.apiBase}${pathname}`, {
        headers: {
          ...
          "User-Agent": "workcell-commit-metrics",
