...
const nonServerProjects = [
  ...
  "@workcell/adapter-codex-local",
  "@workcell/adapter-opencode-local",
  "@workcell/plugin-sdk",
  "@workcell/ui",
  "workcell",
];
...
const generalWorkspacesAProjects = ["@workcell/ui", "workcell"];
...
function runVitest(args, label) {
  ...
  const env = {
    ...process.env,
    WORKCELL_HOME: path.join(testRoot, "h"),
    WORKCELL_INSTANCE_ID: `vt-${process.pid}-${invocationIndex}`,
    TMPDIR: path.join(testRoot, "t"),
  };
  mkdirSync(env.WORKCELL_HOME, { recursive: true });
  mkdirSync(env.TMPDIR, { recursive: true });
  const result = spawnSync("pnpm", ["exec", "vitest", "run", ...args], {
    cwd: repoRoot,
    env,
    stdio: "inherit",
    // On Windows `pnpm` resolves to pnpm.cmd, which spawnSync cannot launch without
    // a shell (it fails with ENOENT). Linux/CI keeps the no-shell behavior. The args
    // are flags + repo-relative paths with no spaces, so shell quoting is a non-issue.
...
function runGeneralGroup(routeTests, groupName) {
  if (groupName === generalServerGroupName) {
    ...
    runVitest(
      ["--project", "@workcell/server", ...excludeRouteArgs],
