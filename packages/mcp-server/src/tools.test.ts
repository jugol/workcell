...
describe("workcell MCP tools", () => {
  ...
  it("design_attach with html POSTs a self-contained data-url 시안 to the issue's design-artifacts, authoritative by default", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      mockJsonResponse({ id: "wp-1", type: "design", reviewState: "none", isPrimary: true }),
    );
    vi.stubGlobal("fetch", fetchMock);
    ...
    const [url, init] = fetchMock.mock.calls[0] as [string, RequestInit];
    expect(String(url)).toBe("http://localhost:3100/api/issues/PAP-1135/design-artifacts");
    expect(init.method).toBe("POST");
    const body = JSON.parse(String(init.body));
    // The data URL MUST carry charset=utf-8 so Korean/UTF-8 mockups render.
    expect(body.url.startsWith("data:text/html;charset=utf-8,")).toBe(true);
    // The HTML round-trips through encodeURIComponent.
    ...
    // The provenance run id flows as the write header.
    expect((init.headers as Record<string, string>)["X-Workcell-Run-Id"]).toBe(
      "33333333-3333-3333-3333-333333333333",
    );
  ...
  it("design_attach with makeAuthoritative:false sends isPrimary false", async () => {
    const fetchMock = vi.fn().mockResolvedValue(mockJsonResponse({ id: "wp-3" }));
    vi.stubGlobal("fetch", fetchMock);
  ...
  it("design_attach honours an explicit design type", async () => {
    const fetchMock = vi.fn().mockResolvedValue(mockJsonResponse({ id: "wp-4" }));
    vi.stubGlobal("fetch", fetchMock);
