...
export function createToolDefinitions(client: WorkcellApiClient): ToolDefinition[] {
  return [
    ...
    makeTool(
      "design_attach",
      "Attach a design 시안 (mockup) you generated as the issue's source-of-truth design (D22). Provide `html` to attach a self-contained HTML mockup you generated (preferred — it is embedded as a preview, no hosting needed), or `url` for an externally hosted design. By default this becomes the issue's authoritative source-of-truth design (makeAuthoritative=true). Use this when assigned a UI/screen/design issue: generate the 시안, then call design_submit_for_review to open the board's review gate. Returns the created design work product (id + reviewState).",
      designAttachSchema,
      async ({ issueId, title, html, url, type, summary, makeAuthoritative }) => {
        const resolvedUrl = html !== undefined ? htmlToDataUrl(html) : url;
        if (!resolvedUrl) {
