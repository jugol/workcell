...
export type { IssueWorkMode, IssueWorkOwnerKind };
...
// WC-195/197: design-first gate opt-out for an issue. null on the issue means
// "inherit the company default" (companies.requireDesignFirst). { required:false }
// exempts the issue from the gate; setByKind records who set it ("auto" = an AI
// agent such as the orchestrator/planner judging the work non-screen).
export interface IssueDesignRequirement {
  required: boolean;
  reason: string | null;
...
export interface Issue {
  ...
  // WC-195: design-first gate opt-out. Optional + nullable so older API
  // responses without it are tolerated; null = inherit the company default.
  designRequirement?: IssueDesignRequirement | null;
  // WC-23: WorkOwner kind. "single" is the default and preserves existing
  // single-assignee semantics; "pair" reserves the value for future
  // PairGroup orchestration. Optional on this Issue type so the field is
  // tolerated across older API responses that don't yet carry it.
  workOwnerKind?: IssueWorkOwnerKind;
  // WC-24: when workOwnerKind="pair", the active PairGroup id. Null for
  // single-owner issues and for pair issues whose group hasn't been
  // instantiated yet (created lazily by the future orchestration slice).
  ...
  // WC-9: populated by the issue list endpoint so a Kanban card can render a
  // proof-status chip without fetching each issue's work products. Optional /
  // undefined elsewhere — consumers must treat `undefined` as "unknown", not
  // "absent" (only `false` is "explicitly no proof attached").
  hasProof?: boolean;
  // WC-11: total cost cents accumulated against this issue, populated by the
  // list endpoint. Tri-state semantics like hasProof: undefined = field not
  // carried on this issue (legacy/detail paths); 0 = verified no cost events
  // yet; positive = sum of cost_events.cost_cents for the issue. The UI usage
  // chip renders only on `> 0`.
