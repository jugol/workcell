...
export const draftIssueFromPromptSchema = z
  .object({
...
export type DraftIssueFromPrompt = z.infer<typeof draftIssueFromPromptSchema>;

// WC-184 (CP0 "Grill mode"): the grill endpoint takes the same prompt as the
// draft endpoint and returns clarifying questions instead of creating an issue.
export const draftGrillFromPromptSchema = z
  .object({
    prompt: z.string().trim().min(1).max(20000),
    projectId: z.string().uuid().optional().nullable(),
  })
...
export type DraftGrillFromPrompt = z.infer<typeof draftGrillFromPromptSchema>;
