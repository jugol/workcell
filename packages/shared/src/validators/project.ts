...
// Move a local workspace's source folder to a new absolute path (and update its
// cwd). Absolute-path / no-clobber / active-run checks live in the service.
export const relocateProjectWorkspaceSchema = z.object({
  targetPath: z.string().trim().min(1),
  force: z.boolean().optional().default(false),
});

export type RelocateProjectWorkspace = z.infer<typeof relocateProjectWorkspaceSchema>;
