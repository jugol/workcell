...
import { agentDeliberationConfigSchema } from "./agent-deliberation.js";
...
export const createAgentSchema = z.object({
  ...
  // WC-205 (deliberation mode, slice 2): accept + persist the per-agent
  // dual-brain consensus config on create/update. The engine that consumes it
  // (server/src/services/agent-deliberation.ts) is not wired into the live
  // execution path yet (slice 3). null clears it; absent leaves it untouched.
  deliberation: agentDeliberationConfigSchema.nullable().optional(),
});
...
export const updateAgentSchema = createAgentSchema
  ...
  .extend({
    ...
    // WC-205: explicit so the per-agent deliberation config is accepted +
    // persisted through the PATCH /agents/:id update path. null clears it.
    deliberation: agentDeliberationConfigSchema.nullable().optional(),
  });
...
export const testAdapterEnvironmentSchema = z.object({
   ...
   * test runs against the local Workcell host. When provided and the
