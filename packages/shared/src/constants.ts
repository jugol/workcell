...
// Workcell's recommended starting team for a development project: an
// Orchestrator that routes work, an Engineer that builds, a Designer that owns
// the design system (the project's source of truth), and a QA that owns the
// proof-gated Done verdict. The product suggests — never forces — this shape:
// onboarding seeds the Orchestrator and the Agents page surfaces the missing
// roles as one-click hires.
export const RECOMMENDED_TEAM_ROLES = [
  "orchestrator",
  "engineer",
  "designer",
  "qa",
...
export const ISSUE_ORIGIN_KINDS = [
  ...
  "planner_draft_request",
  // WC-13 (D19 follow-up sweep): an issue produced from a "Follow-up issues"
  // bullet on a parent's compound-checklist. Each line in section 5 becomes
  // a child of the parent with this origin kind so the discovery trail
  // ("which follow-ups did this completion spawn?") stays queryable.
  "compound_followup",
  // WC-19 (D19 LLM auto-fill): a planning child spawned to ask a Planner-capable
  // agent to fill in sections 1–4 of the parent's compound-checklist (what
  // changed / learnings / prevention / failed approaches). Section 5 stays
  // human-driven. The child is short-lived planning work; once the parent's
  // checklist is filled, the child closes.
  "compound_checklist_autofill",
  // Design-first dispatch gate: a child issue auto-created when an execution
  // run was held because the parent (screen work) has no approved
  // source-of-truth 시안. Routed to a designer; the designer attaches the
  // design to the PARENT issue and requests board review. The child itself is
  // design-exempt (it IS the design work).
  "design_request",
] as const;

// WC-23 (P2 §3 first slice): WorkOwner indirection. "single" preserves the
// existing single-assignee semantics (one agent owns the issue). "pair"
// reserves the value for PairGroup orchestration (multiple rounds, stop
// policy, per-round diffs) added in a later slice. New issues default to
// "single" — the field is opt-in.
export const ISSUE_WORK_OWNER_KINDS = ["single", "pair"] as const;
export type IssueWorkOwnerKind = (typeof ISSUE_WORK_OWNER_KINDS)[number];

// WC-24 (P2 §3 second slice): PairGroup status lifecycle. "active" — round
// work is ongoing. "completed" — natural exit (agreed solution, success
// criteria reached). "aborted" — failure exit (max rounds without
// convergence, executor abort, user cancel). Future PairTurn-level slices
// extend with more granular reasons.
export const PAIR_GROUP_STATUSES = ["active", "completed", "aborted"] as const;
export type PairGroupStatus = (typeof PAIR_GROUP_STATUSES)[number];

// Default cap from the spec ("pair=최대 10라운드"). Individual groups can
// override via stopPolicy.maxRounds.
export const PAIR_GROUP_DEFAULT_MAX_ROUNDS = 10;
...
export const SECRET_MANAGED_MODES = ["workcell_managed", "external_reference"] as const;
...
export const HEARTBEAT_INVOCATION_SOURCES = [
  ...
  "on_demand",
  "automation",
  // A live pair-collaboration turn promoted to a real heartbeat run record
  // (pair-turn-real-invoke.ts). The DB column is free text; this union keeps
  // typed consumers in sync.
  "pair_round",
] as const;
...
export const PLUGIN_CAPABILITIES = [
  ...
  "http.outbound",
  "secrets.read-ref",
  // WC-65: call out to a company-scoped outbound MCP server (resolved + gated
  // by the capability registry) via ctx.mcpClients.
  "mcp.client",
 ...
 * Workcell UI where plugin components can be rendered.
 *
