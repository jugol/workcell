...
// codex emits this when the resumed thread has filled the model's context
// window ("Codex ran out of room in the model's context window. Start a new
// thread or clear earlier history before retrying."). Retrying the SAME thread
// just hits it again — the only recovery is a fresh thread, so the run must
// clear its resume session. Distinct from transient-upstream (no backoff window
// would help) and from unknown-session (the thread exists, it's just full).
export function isCodexContextWindowExhaustedError(input: {
  stdout?: string | null;
  stderr?: string | null;
  errorMessage?: string | null;
}): boolean {
  const haystack = buildCodexErrorHaystack(input);
  return /ran out of room in the model'?s context window|context window|out of room/i.test(haystack);
}
