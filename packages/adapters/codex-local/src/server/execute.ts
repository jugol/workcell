...
import {
  ...
  isCodexContextWindowExhaustedError,
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  // Terminal-result cleanup (claude-local parity): codex emits a final
  // `turn.completed` / `turn.failed` NDJSON event, but the CLI process can
  // linger afterwards (e.g. an MCP server child that never closes its pipes).
  // Without this, a finished run hangs in "running" forever and trips the
  // silent-run watchdog. Codex gets a longer default grace than claude (it
  // writes session rollout files on shutdown).
  const terminalResultCleanupGraceMs = Math.max(
    0,
    asNumber(config.terminalResultCleanupGraceMs, 15_000),
  );
  ...
  // workcellTaskMarkdown is the cross-adapter task channel — pair turns,
  // deliberation, and grill all deliver their round/task instructions on this
  // context key. claude-local already folds it into its prompt; codex must do
  // the same (claude parity) or those flows silently degrade into a plain
  // heartbeat. Included on the resume-delta path too, matching claude-local.
  const taskContextNote = asString(context.workcellTaskMarkdown, "").trim();
  // WC-DSR (designer visual self-review): absolute PNG paths the heartbeat put on
  // the wake context. codex attaches them to the initial prompt as image input
  // via `codex exec --image <file>` (built in buildCodexExecArgs below) so the
  // designer SEES the rendered screenshot of its own 시안. Empty/absent → no
  // images attached (byte-identical invocation).
  const designReviewImagePaths = asStringArray(context.designReviewImagePaths).filter(
    (p) => p.trim().length > 0,
  );
  ...
  const runAttempt = async (resumeSessionId: string | null) => {
    const execArgs = buildCodexExecArgs(
      forceSaferInvocation ? { ...config, fastMode: false } : config,
      {
        ...
        imagePaths: designReviewImagePaths,
      },
    );
    const args = execArgs.args;
    const commandNotesWithFastMode = [
      ...commandNotes,
      ...(execArgs.fastModeIgnoredReason == null ? [] : [execArgs.fastModeIgnoredReason]),
      ...(designReviewImagePaths.length > 0
        ? [
            `Attached ${designReviewImagePaths.length} rendered 시안 screenshot(s) as image input via --image: ${designReviewImagePaths.join(", ")}`,
          ]
        : []),
    ];
    if (onMeta) {
    ...
    const proc = await runAdapterExecutionTargetProcess(runId, runtimeExecutionTarget, command, args, {
      ...
      terminalResultCleanup: {
        ...
        hasTerminalResult: ({ stdout }) =>
          stdout.includes('"type":"turn.completed"') || stdout.includes('"type":"turn.failed"'),
      },
    });
  ...
  const toResult = (
    ...
    // A turn codex itself declared complete (with no turn.failed error) is a
    // SUCCESS regardless of the process exit code: the terminal-result cleanup
    // SIGTERMs a lingering CLI after `turn.completed`, and on Windows that
    // surfaces as a non-zero exit code that must not fail the finished run.
    const turnCompletedCleanly =
      !parsedError && attempt.proc.stdout.includes('"type":"turn.completed"');
    const exitedCleanly = (attempt.proc.exitCode ?? 0) === 0 || turnCompletedCleanly;
    ...
    // Context-window exhaustion: the resumed thread is full. Retrying the same
    // thread loops forever — the only recovery is a fresh thread, so we MUST
    // clear the resume session even though codex emitted a valid sessionId
    // (thread.started fired before the failure). The issue continuation summary
    // is re-injected on the next wake, so a fresh thread keeps progress.
    ...
    return {
      exitCode: turnCompletedCleanly ? 0 : attempt.proc.exitCode,
      signal: attempt.proc.signal,
      timedOut: false,
      errorMessage: exitedCleanly ? null : fallbackErrorMessage,
      errorCode:
        contextExhausted
          ? "codex_context_exhausted"
          : transientUpstream
            ? "codex_transient_upstream"
            : null,
      errorFamily: contextExhausted
        ? "context_exhausted"
        : transientUpstream
          ? "transient_upstream"
          : null,
      ...
      // Drop the full thread on context exhaustion so the next wake starts fresh.
      clearSession: Boolean(
        contextExhausted || ((clearSessionOnMissingSession || forceFreshSession) && !resolvedSessionId),
      ),
    };
  };
