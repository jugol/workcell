...
describe("isCodexContextWindowExhaustedError", () => {
  it("detects the codex out-of-room message", () => {
    expect(
      isCodexContextWindowExhaustedError({
        errorMessage:
  ...
  it("detects it from the JSONL error/turn.failed stream on stdout", () => {
    const stdout = [
    ...
    expect(isCodexContextWindowExhaustedError({ stdout })).toBe(true);
  });

  it("does not fire on unrelated failures", () => {
    expect(isCodexContextWindowExhaustedError({ errorMessage: "model overloaded" })).toBe(false);
    expect(isCodexContextWindowExhaustedError({ stderr: "no rollout found for thread id abc" })).toBe(false);
  });
});
