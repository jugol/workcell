...
import {
  ...
  resolveWorkcellInstanceRootForAdapter,
  type WorkcellSkillEntry,
} from "@workcell/adapter-utils/server-utils";

type SkillEntry = WorkcellSkillEntry;
...
async function buildClaudePromptBundleKey(input: {
  ...
  hash.update("workcell-claude-prompt-bundle:v1\n");
...
export async function prepareClaudePromptBundle(input: {
  ...
  for (const entry of skills) {
    ...
    try {
      await ensureWorkcellSkillSymlink(entry.source, target);
    } catch (err) {
      await onLog(
        "stderr",
        `[workcell] Failed to materialize Claude skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
