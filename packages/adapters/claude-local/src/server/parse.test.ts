...
describe("isClaudeContextWindowExhaustedError", () => {
  it("detects Anthropic's prompt-too-long 400", () => {
    expect(
      isClaudeContextWindowExhaustedError({
        parsed: { is_error: true, result: "prompt is too long: 215000 tokens > 200000 maximum" },
      }),
    ).toBe(true);
  });

  it("detects the model_context_window_exceeded stop reason", () => {
    expect(
      isClaudeContextWindowExhaustedError({
        stderr: 'API Error: {"type":"error","error":{"type":"invalid_request_error"}} model_context_window_exceeded',
      }),
    ).toBe(true);
  });

  it("does not fire on unrelated failures", () => {
    expect(isClaudeContextWindowExhaustedError({ errorMessage: "overloaded_error" })).toBe(false);
    expect(isClaudeContextWindowExhaustedError({ parsed: { result: "no conversation found with session id x" } })).toBe(false);
  });
});
