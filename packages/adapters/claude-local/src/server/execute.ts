...
import type { AdapterExecutionContext, AdapterExecutionResult } from "@workcell/adapter-utils";
import type { RunProcessResult } from "@workcell/adapter-utils/server-utils";
// WC-67: serialize agent-declared MCP servers to a .mcp.json for the Claude CLI
// (feature-gated, local-target only).
import {
  buildMcpJsonConfig,
  isAdapterMcpInjectionEnabled,
  type McpServerSpec,
...
import { shellQuote } from "@workcell/adapter-utils/ssh";
import {
  parseClaudeStreamJson,
  describeClaudeFailure,
  detectClaudeLoginRequired,
  extractClaudeRetryNotBefore,
  isClaudeContextWindowExhaustedError,
  isClaudeMaxTurnsResult,
  isClaudeTransientUpstreamError,
  isClaudeUnknownSessionError,
} from "./parse.js";
import { prepareClaudeConfigSeed } from "./claude-config.js";
import { buildDesignReviewImageNote, readDesignReviewImagePaths } from "./design-review-prompt.js";
...
async function buildClaudeRuntimeConfig(input: ClaudeExecutionInput): Promise<ClaudeRuntimeConfig> {
  ...
  if (shapedWorkspaceEnv.workspaceHints.length > 0) {
    env.WORKCELL_WORKSPACES_JSON = JSON.stringify(shapedWorkspaceEnv.workspaceHints);
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const claudeSkillEntries = await readWorkcellRuntimeSkillEntries(config, __moduleDir);
  ...
  const preparedExecutionTargetRuntime = executionTargetIsRemote
    ? await (async () => {
        await onLog(
          "stdout",
          `[workcell] Syncing workspace and Claude runtime assets to ${describeAdapterExecutionTarget(executionTarget)}.\n`,
        );
  ...
  const effectivePromptBundleAddDir = executionTargetIsRemote
    ? preparedExecutionTargetRuntime?.assetDirs.skills ??
      path.posix.join(effectiveExecutionCwd, ".workcell-runtime", "claude", "skills")
    : promptBundle.addDir;
  ...
  // WC-67: when MCP injection is enabled (WORKCELL_ADAPTER_MCP_INJECTION, OFF by
  // default) and we're running locally, write the agent's declared MCP servers
  // (heartbeat put them on context.workcellMcpServers) to a .mcp.json inside the
  // Workcell-owned prompt bundle dir — NOT the user's repo cwd, so a project's
  // own .mcp.json is never clobbered — and point the CLI at it via --mcp-config.
  // Default runs (flag off / no servers) write nothing and add no flag, so the
  // invocation is byte-for-byte unchanged. Remote relay is a documented follow-up.
  let mcpConfigFilePath: string | null = null;
  if (!executionTargetIsRemote && isAdapterMcpInjectionEnabled(process.env)) {
    const specs = Array.isArray(context.workcellMcpServers)
      ? (context.workcellMcpServers as McpServerSpec[])
      : [];
    if (specs.length > 0) {
      const candidate = path.join(promptBundle.addDir, ".mcp.json");
      try {
        await fs.writeFile(candidate, JSON.stringify(buildMcpJsonConfig(specs), null, 2), "utf-8");
        mcpConfigFilePath = candidate;
      } catch (err) {
        await onLog(
          "stderr",
          `[workcell] Failed to write MCP config (.mcp.json); continuing without it: ${
  ...
  const remoteClaudeRuntimeRoot = executionTargetIsRemote
    ? preparedExecutionTargetRuntime?.runtimeRootDir ??
      path.posix.join(effectiveExecutionCwd, ".workcell-runtime", "claude")
    : null;
  ...
  if (remoteClaudeConfigDir && remoteClaudeConfigSeedDir) {
    ...
    await onLog(
      "stdout",
      `[workcell] Materializing Claude auth/config into ${remoteClaudeConfigDir}.\n`,
    );
  ...
  if (
    ...
    await onLog(
      "stdout",
      `[workcell] Claude session "${runtimeSessionId}" does not match the current remote execution identity and will not be resumed in "${effectiveExecutionCwd}". Starting a fresh remote session.\n`,
    );
  } else if (
    runtimeSessionId &&
    ...
    await onLog(
      "stdout",
      `[workcell] Claude session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${effectiveExecutionCwd}".\n`,
    );
  }
  if (runtimeSessionId && runtimePromptBundleKey.length > 0 && runtimePromptBundleKey !== promptBundle.bundleKey) {
    await onLog(
      "stdout",
      `[workcell] Claude session "${runtimeSessionId}" was saved for prompt bundle "${runtimePromptBundleKey}" and will not be resumed with "${promptBundle.bundleKey}".\n`,
    );
  }
  ...
  const taskContextNote = asString(context.workcellTaskMarkdown, "").trim();
  // WC-DSR (designer visual self-review): the heartbeat put absolute PNG paths on
  // the wake context (the rendered screenshot of the designer's own 시안). The
  // codex CLI takes images as a flag (--image); the claude CLI has no such flag,
  // but Claude Code's built-in Read tool renders a LOCAL image file when given
  // its path. So we name the absolute path(s) in the prompt and instruct the
  // agent to Read them. Empty/absent → no section (byte-identical prompt).
  const designReviewImagePaths = readDesignReviewImagePaths(context);
  const designReviewImageNote = buildDesignReviewImageNote(designReviewImagePaths);
  const prompt = joinPromptSections([
    renderedBootstrapPrompt,
    wakePrompt,
    sessionHandoffNote,
    taskContextNote,
    designReviewImageNote,
    renderedPrompt,
  ]);
  const promptMetrics = {
    ...
    designReviewImageChars: designReviewImageNote.length,
  ...
  const buildClaudeArgs = (
    ...
    // WC-67: load the generated MCP server config (only set when injection is
    // enabled + local + servers declared; otherwise null → no flag added).
  ...
  const toAdapterResult = (
    ...
    if (!parsed) {
      ...
      const transientRetryNotBefore = transientUpstream
        ...
        : null;
      // A hard context-overflow 400 can arrive with no terminal result JSON —
      // classify it here too so it clears the session and retries fresh.
      ...
      return {
        ...
        errorMessage: fallbackErrorMessage,
        errorCode,
        errorFamily: contextExhausted ? "context_exhausted" : transientUpstream ? "transient_upstream" : null,
        retryNotBefore: transientRetryNotBefore ? transientRetryNotBefore.toISOString() : null,
        errorMeta,
        resultJson: {
          stdout: proc.stdout,
          stderr: proc.stderr,
          ...(contextExhausted ? { errorFamily: "context_exhausted" } : {}),
        ...
        clearSession: Boolean(contextExhausted || opts.clearSessionOnMissingSession),
      };
    }
    ...
    const transientRetryNotBefore = transientUpstream
      ...
      : null;
    // Context-window exhaustion (mirror codex): the resumed conversation filled
    // the model's window. Retrying the same session loops; recovery is a fresh
    // session, so we MUST clear it even though Claude emitted a sessionId. The
    // issue continuation summary is re-injected on the next wake.
    const contextExhausted =
      failed &&
      !loginMeta.requiresLogin &&
      !clearSessionForMaxTurns &&
  ...
  try {
    ...
    if (
      ...
      await onLog(
        "stdout",
        `[workcell] Claude resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
      );
