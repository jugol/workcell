import type { UsageSummary } from "@workcell/adapter-utils";
import {
  asString,
  asNumber,
  parseObject,
...
// Anthropic surfaces a context-window overflow as a 400 invalid_request_error
// ("prompt is too long: N tokens > M maximum") and Claude 4.5+ can stop with
// stop_reason "model_context_window_exceeded". Claude Code normally auto-compacts
// long sessions so this rarely surfaces — but if compaction is off/fails, the
// resumed conversation hard-errors and (without this) the session is kept and
// the next wake hits the same overflow forever. Mirror codex: detect it so the
// adapter can clear the session and retry on a fresh one. Distinct from
// transient-upstream (no backoff helps) and unknown-session (the session exists,
// it's just full).
export function isClaudeContextWindowExhaustedError(input: {
  parsed?: Record<string, unknown> | null;
  stdout?: string | null;
  stderr?: string | null;
  errorMessage?: string | null;
  ...
  return /prompt is too long|model_context_window_exceeded|context (window|length) (exceeded|too long)|exceeds? .{0,40}context window|tokens?\s*>\s*\d+\s*maximum/i.test(
    haystack,
  );
}
