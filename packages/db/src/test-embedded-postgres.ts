...
const DEFAULT_WORKCELL_EMBEDDED_POSTGRES_PORT = 54329;

function getReservedTestPorts(): Set<number> {
  const configuredPorts = [
    DEFAULT_WORKCELL_EMBEDDED_POSTGRES_PORT,
    Number.parseInt(process.env.WORKCELL_EMBEDDED_POSTGRES_PORT ?? "", 10),
    ...String(process.env.WORKCELL_TEST_POSTGRES_RESERVED_PORTS ?? "")
      .split(",")
...
async function getAvailablePort(): Promise<number> {
  ...
  throw new Error(
    `Failed to allocate embedded Postgres test port outside reserved Workcell ports: ${[
      ...reservedPorts,
    ].join(", ")}`,
  );
}
...
async function probeEmbeddedPostgresSupport(): Promise<EmbeddedPostgresTestSupport> {
  ...
  try {
    const created = await createEmbeddedPostgresTestInstance(
      "workcell-embedded-postgres-probe-",
    );
...
export async function startEmbeddedPostgresTestDatabase(
  ...
  try {
    ...
    const connectionString = `postgres://workcell:workcell@127.0.0.1:${port}/workcell`;
