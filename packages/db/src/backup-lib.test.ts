...
async function createTempDatabase(): Promise<string> {
  const db = await startEmbeddedPostgresTestDatabase("workcell-db-backup-");
...
describe("createBufferedTextFileWriter", () => {
  it("preserves line boundaries across buffered flushes", async () => {
    const tempDir = createTempDir("workcell-buffered-writer-");
...
describeEmbeddedPostgres("runDatabaseBackup", () => {
  it(
    ...
    async () => {
      ...
      const restoreConnectionString = await createSiblingDatabase(
        ...
        "workcell_restore_target",
      );
      const backupDir = createTempDir("workcell-db-backup-output-");
      ...
      try {
        ...
        const result = await runDatabaseBackup({
          ...
          filenamePrefix: "workcell-test",
          backupEngine: "javascript",
        });

        expect(result.backupFile).toMatch(/workcell-test-.*\.sql\.gz$/);
  ...
  it(
    ...
    async () => {
      ...
      const restoreConnectionString = await createSiblingDatabase(
        sourceConnectionString,
        "workcell_full_logical_restore_target",
      );
      const backupDir = createTempDir("workcell-db-full-logical-backup-");
      const sourceSql = postgres(sourceConnectionString, { max: 1, onnotice: () => {} });
      const restoreSql = postgres(restoreConnectionString, { max: 1, onnotice: () => {} });
      ...
      try {
        await sourceSql.unsafe(`
          ...
          VALUES ('workcell-migration-history', 1770000000000);
        `);
        await sourceSql.unsafe(`
        ...
        const result = await runDatabaseBackup({
          ...
          backupDir,
          retention: { dailyDays: 7, weeklyWeeks: 4, monthlyMonths: 1 },
          filenamePrefix: "workcell-full-logical-test",
          backupEngine: "javascript",
        ...
        const migrationRows = await restoreSql.unsafe<{ hash: string }[]>(`
          ...
          WHERE "hash" = 'workcell-migration-history'
        `);
        expect(migrationRows).toEqual([{ hash: "workcell-migration-history" }]);
  ...
  it(
    ...
    async () => {
      ...
      const restoreConnectionString = await createSiblingDatabase(
        sourceConnectionString,
        "workcell_composite_fk_restore_target",
      );
      const backupDir = createTempDir("workcell-db-composite-fk-backup-");
      const sourceSql = postgres(sourceConnectionString, { max: 1, onnotice: () => {} });
      const restoreSql = postgres(restoreConnectionString, { max: 1, onnotice: () => {} });
      ...
      try {
        ...
        const result = await runDatabaseBackup({
          ...
          backupDir,
          retention: { dailyDays: 7, weeklyWeeks: 4, monthlyMonths: 1 },
          filenamePrefix: "workcell-composite-fk-test",
          backupEngine: "javascript",
        });
  ...
  it(
    ...
    async () => {
      ...
      const restoreSql = postgres(restoreConnectionString, { max: 1, onnotice: () => {} });
      const backupDir = createTempDir("workcell-db-restore-manual-");
      ...
      try {
        await fs.promises.writeFile(
          backupFile,
          [
            "-- Workcell database backup",
            "-- Created: 2026-04-06T00:00:00.000Z",
            "",
            "BEGIN;",
            "-- workcell statement breakpoint 69f6f3f1-42fd-46a6-bf17-d1d85f8f3900",
