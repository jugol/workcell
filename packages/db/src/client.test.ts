...
async function createTempDatabase(): Promise<string> {
  const db = await startEmbeddedPostgresTestDatabase("workcell-db-client-");
...
// Production-readiness Wave 2 (REL — DB client timeouts/pool): the postgres.js
// client must be created with explicit pool + timeout options instead of a bare
// `postgres(url)`. These are the options createDb passes through; assert the
// sane defaults and the WORKCELL_DB_* env overrides. Unit-only (no DB needed).
describe("buildPostgresClientOptions", () => {
  const ENV_KEYS = [
    "WORKCELL_DB_POOL_MAX",
    "WORKCELL_DB_CONNECT_TIMEOUT_SEC",
    "WORKCELL_DB_STATEMENT_TIMEOUT_MS",
    "WORKCELL_DB_IDLE_IN_TRANSACTION_TIMEOUT_MS",
  ] as const;
  ...
  it("applies sane defaults (pool 20, connect 10s, statement 30s, idle-in-txn 60s)", () => {
    for (const key of ENV_KEYS) delete process.env[key];
    const opts = buildPostgresClientOptions();
    expect(opts.max).toBe(20);
    expect(opts.connect_timeout).toBe(10);
    // statement_timeout / idle_in_transaction_session_timeout are Postgres GUCs in
    // MILLISECONDS and must ride under `connection`.
  ...
  it("honors WORKCELL_DB_* overrides", () => {
    process.env.WORKCELL_DB_POOL_MAX = "8";
    process.env.WORKCELL_DB_CONNECT_TIMEOUT_SEC = "5";
    process.env.WORKCELL_DB_STATEMENT_TIMEOUT_MS = "15000";
    process.env.WORKCELL_DB_IDLE_IN_TRANSACTION_TIMEOUT_MS = "45000";
    const opts = buildPostgresClientOptions();
    expect(opts.max).toBe(8);
    expect(opts.connect_timeout).toBe(5);
    expect(opts.connection.statement_timeout).toBe(15_000);
    expect(opts.connection.idle_in_transaction_session_timeout).toBe(45_000);
  });

  it("ignores invalid (non-positive / non-numeric) overrides and keeps defaults", () => {
    process.env.WORKCELL_DB_POOL_MAX = "0";
    process.env.WORKCELL_DB_CONNECT_TIMEOUT_SEC = "-3";
    process.env.WORKCELL_DB_STATEMENT_TIMEOUT_MS = "not-a-number";
    const opts = buildPostgresClientOptions();
    expect(opts.max).toBe(20);
    expect(opts.connect_timeout).toBe(10);
    expect(opts.connection.statement_timeout).toBe(30_000);
...
describeEmbeddedPostgres("applyPendingMigrations", () => {
  ...
  // WC-212 (production-readiness Wave 1, fix #5): migration 0113 adds a partial
  // index on heartbeat_runs(status) restricted to the active states so the
  // companyId-less 30s sweeps stop seq-scanning the table.
  it(
    "creates the heartbeat_runs active-status partial index (migration 0113)",
    async () => {
      const connectionString = await createTempDatabase();
      ...
      const verifySql = postgres(connectionString, { max: 1, onnotice: () => {} });
      try {
        const rows = await verifySql.unsafe<{ indexname: string; indexdef: string }[]>(
          `
            SELECT indexname, indexdef
            FROM pg_indexes
            WHERE schemaname = 'public'
              AND tablename = 'heartbeat_runs'
              AND indexname = 'heartbeat_runs_active_status_idx'
          `,
        );
        expect(rows).toHaveLength(1);
        // It must be a PARTIAL index restricted to the active states — a full
        // index would not give us the targeted, bounded scan we want.
