...
export const pairGroups = pgTable(
  "pair_groups",
  {
    ...
    // reasons, success criteria). requireConvergence defaults to TRUE (a
    // no_change sign-off completes the group); set it to false to opt out.
    ...
    // Dual-brain pivot: "agent_pair" = legacy two-agent pair; "dual_brain" =
    // ONE agent self-reviewing across two brains (owner === counterpart; the
    // per-brain adapter/model comes from agents.deliberation).
    kind: text("kind").notNull().default("agent_pair"),
    // Group status.
    ...
    // OUT via PATCH autoRunEnabled=false; maxRounds/stopPolicy/abort stay in
    // place as the spend safety net either way.
