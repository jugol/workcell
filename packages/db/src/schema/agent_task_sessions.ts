...
export const agentTaskSessions = pgTable(
  "agent_task_sessions",
  {
    ...
    // WC-175: SET NULL so deleting a heartbeat run nulls this pointer instead of
    // FK-blocking — the last remaining heartbeat_runs child in the agent-removal
    // path that could re-introduce a referencing row between the purge and the run
    // delete (the WC-171/174 race class). Lower-frequency writer, but closed for
    // completeness; last_run_id is an informational pointer, safe to null.
