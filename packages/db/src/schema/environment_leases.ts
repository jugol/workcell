...
export const environmentLeases = pgTable(
  ...
  (table) => ({
    ...
    // WC-165: at most one ACTIVE lease per (environment, provider sandbox) — prevents
    // two concurrent runs from both reusing/resuming the same sandbox lease.
    activeProviderLeaseUq: uniqueIndex("environment_leases_active_provider_lease_uq")
      .on(table.environmentId, table.providerLeaseId)
      .where(sql`status = 'active' AND provider_lease_id IS NOT NULL`),
  }),
);
