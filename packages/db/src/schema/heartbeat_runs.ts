...
export const heartbeatRuns = pgTable(
  ...
  (table) => ({
    ...
    // WC-212 (production-readiness Wave 1, fix #5): the 30s sweeps
    // (reapOrphanedRuns / resumeQueuedRuns / scanSilentActiveRuns) filter on a
    // bare status='running'/'queued' with NO companyId, but every other index
    // leads with companyId — so each tick seq-scans this append-only table. A
    // partial index keyed on status, restricted to just the active states,
    // keeps that hot path bounded to the (small) set of in-flight runs.
    activeStatusIdx: index("heartbeat_runs_active_status_idx")
      .on(table.status)
      .where(sql`status IN ('running', 'queued')`),
  }),
);
