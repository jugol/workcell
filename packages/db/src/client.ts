...
/**
 * Read a positive-integer override from an environment variable, falling back to
 * `fallback` when the variable is unset, empty, non-numeric, or non-positive.
 * Used to make the DB client pool/timeout knobs (`WORKCELL_DB_*`) operator-tunable
 * without changing code, while never letting a bad value silently disable a guard.
 */
...
// client previously ran with `postgres(url)` and zero options — pool max 10 and
// NO connect/statement/idle-transaction timeouts. A single hung query then pins a
// connection indefinitely and can exhaust the pool. These defaults bound every
// connection and size the pool deliberately; each is overridable via a
// `WORKCELL_DB_*` env var for operators who need to tune for their database.
//
//   - statement_timeout / idle_in_transaction_session_timeout are Postgres GUCs
//     expressed in MILLISECONDS and must be passed under `connection`.
//   - connect_timeout is in SECONDS (postgres.js top-level option).
//   - max is the connection-pool ceiling.
const DEFAULT_DB_STATEMENT_TIMEOUT_MS = 30_000;
const DEFAULT_DB_IDLE_IN_TRANSACTION_TIMEOUT_MS = 60_000;
const DEFAULT_DB_CONNECT_TIMEOUT_SEC = 10;
const DEFAULT_DB_POOL_MAX = 20;

export function buildPostgresClientOptions(): {
  max: number;
  connect_timeout: number;
  connection: { statement_timeout: number; idle_in_transaction_session_timeout: number };
} {
  return {
    max: readPositiveIntEnv("WORKCELL_DB_POOL_MAX", DEFAULT_DB_POOL_MAX),
    connect_timeout: readPositiveIntEnv("WORKCELL_DB_CONNECT_TIMEOUT_SEC", DEFAULT_DB_CONNECT_TIMEOUT_SEC),
    connection: {
      statement_timeout: readPositiveIntEnv(
        "WORKCELL_DB_STATEMENT_TIMEOUT_MS",
        DEFAULT_DB_STATEMENT_TIMEOUT_MS,
      ),
      idle_in_transaction_session_timeout: readPositiveIntEnv(
...
export function createDb(url: string) {
  const sql = postgres(url, buildPostgresClientOptions());
