...
import {
  ...
  resolveWorkcellEnvPathForConfig,
...
export type ResolvedDatabaseTarget =
  | {
      mode: "postgres";
      connectionString: string;
      source: "DATABASE_URL" | "workcell-env" | "config.database.connectionString";
      configPath: string;
      envPath: string;
    }
  | {
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".workcell", CONFIG_BASENAME);
...
function resolveWorkcellConfigPath(): string {
  if (process.env.WORKCELL_CONFIG?.trim()) {
    return path.resolve(process.env.WORKCELL_CONFIG.trim());
  }
  return findConfigFileFromAncestors(process.cwd()) ?? resolveWorkcellConfigPathForInstance();
}

function resolveWorkcellEnvPath(configPath: string): string {
  return resolveWorkcellEnvPathForConfig(configPath);
}
...
export function resolveDatabaseTarget(): ResolvedDatabaseTarget {
  ...
  const envPath = resolveWorkcellEnvPath(configPath);
