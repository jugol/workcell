...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://workcell:workcell@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "workcell");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://workcell:workcell@127.0.0.1:${preferredPort}/workcell`,
  ...
  if (runningPid) {
    ...
    const adminConnectionString = `postgres://workcell:workcell@127.0.0.1:${port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "workcell");
    return {
      connectionString: `postgres://workcell:workcell@127.0.0.1:${port}/workcell`,
  ...
  const adminConnectionString = `postgres://workcell:workcell@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "workcell");

  return {
    connectionString: `postgres://workcell:workcell@127.0.0.1:${selectedPort}/workcell`,
