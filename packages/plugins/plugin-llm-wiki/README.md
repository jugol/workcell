...
here rather than in Workcell core.
...
- opt-in company-scoped Workcell event ingestion controls for issues, comments, and documents; event ingestion is disabled by default and routes captured raw provenance into the default space only
- manual Workcell project/root issue distillation and bounded backfill actions with explicit work items, operation issues, source caps, and estimated cost recording
- Workcell-derived distillation (cursor windows, manual `distill-now`, backfill) always writes into the default wiki space in Phase 1; non-default spaces remain on manual / raw-file ingest until per-space Workcell ingestion profiles ship
- Workcell-derived distillation maintains `wiki/projects/<slug>/standup.md` as the executive current-state view for each represented project, alongside durable `wiki/projects/<slug>/index.md` knowledge pages
...
Workcell-derived text ingestion stays limited to issue titles/descriptions, issue comments, and issue documents.
...
The accepted policy lives in [doc/plans/2026-05-06-llm-wiki-workcell-asset-security-gate.md](../../../doc/plans/2026-05-06-llm-wiki-workcell-asset-security-gate.md).
...
From the Workcell repo root:

```bash
pnpm --filter @workcell/plugin-llm-wiki typecheck
pnpm --filter @workcell/plugin-llm-wiki test
pnpm --filter @workcell/plugin-llm-wiki build
```

## Alpha Verification

Run these commands from the Workcell repo root before handing off alpha plugin
changes:
...
- disabled and enabled Workcell event ingestion paths
...
- Host-level plugin install and live agent invocation still need Workcell
  server/runtime smoke coverage when preparing a release candidate.



## Install Into Workcell

```bash
curl -X POST http://127.0.0.1:3100/api/plugins/install \
  -H "Content-Type: application/json" \
  -d '{"packageName":"/Users/dotta/workcell/.workcell/worktrees/PAP-3179-design-a-llm-wiki-plugin/packages/plugins/plugin-llm-wiki","isLocalPath":true}'
```

## Build Options

- `pnpm build` uses esbuild presets from `@workcell/plugin-sdk/bundlers`.
...
The package-local `dist/` directory is ignored by git, but local Workcell
...
restart the Workcell dev server so the host imports the fresh bundle.
...
write the starter files. The plugin uses Workcell's local folder API for path
...
`CLAUDE.md` renamed to `AGENTS.md` and Workcell project overviews, standups,
