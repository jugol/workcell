...
import {
  bootstrapWikiRoot,
  bootstrapSpace,
  assembleWorkcellSourceBundle,
  archiveSpace,
  captureWikiSource,
  createSpace,
  createWorkcellDistillationRun,
  createWorkcellDistillationWorkItem,
  createOperationIssue,
  distillWorkcellProjectPage,
  ...
  listWorkcellIngestionCandidates,
  getWorkcellIngestionProfile,
  getOverview,
  listSpaces,
  handleWorkcellEventIngestion,
  ...
  readTemplate,
  readWikiPage,
  recordWorkcellDistillationOutcome,
  ...
  updateWorkcellIngestionProfile,
  updateSpace,
  writeTemplate,
  writeWikiPage,
} from "./wiki.js";
...
const WORKCELL_EVENT_INGESTION_EVENTS = [
  "issue.created",
  "issue.updated",
...
function buildManualDistillPrompt(input: { companyId: string; projectId?: string | null; rootIssueId?: string | null }) {
  ...
  return [
    ...
    "",
    "Prompt source: LLM Wiki plugin action `distill-workcell-now` (`packages/plugins/plugin-llm-wiki/src/worker.ts`).",
    `Required skill: use the installed \`${WORKCELL_DISTILL_SKILL_KEY}\` skill before changing wiki files.`,
    "",
    "Scope:",
    ...
    !input.projectId && !input.rootIssueId
      ? "- Do not hardcode a single project. Find non-plugin Workcell issues/comments/documents that changed in any project after the last processed cursor and are old enough for the stale/debounce threshold."
      : null,
    "",
    "Process:",
    "1. Read the wiki root AGENTS.md, wiki/index.md, and recent wiki/log.md entries.",
    "2. Assemble bounded Workcell source bundles for every eligible project or root issue, excluding LLM Wiki plugin-operation issues.",
    "3. Turn durable signal into project standups, wiki-insightful project pages, decisions, history, index, and log updates per the workcell-distill skill.",
...
const plugin = definePlugin({
  async setup(ctx) {
    ...
    for (const eventName of WORKCELL_EVENT_INGESTION_EVENTS) {
      ctx.events.on(eventName, async (event) => {
        const result = await handleWorkcellEventIngestion(ctx, event);
        if (result.status === "recorded") {
          ctx.logger.info("LLM Wiki recorded Workcell event for cursor discovery", {
    ...
    ctx.data.register("workcell-ingestion-profile", async (params) => {
      return getWorkcellIngestionProfile(ctx, {
        companyId: readCompanyIdFromParams(params),
        wikiId: stringField(params.wikiId),
        spaceSlug: stringField(params.spaceSlug),
      });
    ...
    ctx.data.register("workcell-ingestion-candidates", async (params) => {
      return listWorkcellIngestionCandidates(ctx, {
        companyId: readCompanyIdFromParams(params),
        wikiId: stringField(params.wikiId),
        spaceSlug: stringField(params.spaceSlug),
    ...
    ctx.actions.register("update-workcell-ingestion-profile", async (params) => {
      return updateWorkcellIngestionProfile(ctx, {
        companyId: readCompanyIdFromParams(params),
        wikiId: stringField(params.wikiId),
        spaceSlug: stringField(params.spaceSlug),
    ...
    ctx.actions.register("queue-workcell-ingestion-backfill", async (params) => {
      const companyId = readCompanyIdFromParams(params);
      ...
      for (const scope of scopes) {
        ...
        const workItem = await createWorkcellDistillationWorkItem(ctx, {
          companyId,
          wikiId,
          spaceSlug,
          kind: "backfill",
          ...
          metadata: { backfillStartAt, backfillEndAt, requestedFrom: "queue-workcell-ingestion-backfill" },
        });
        const operation = await createOperationIssue(ctx, {
          ...
          spaceSlug,
          operationType: "backfill",
          title: scope.rootIssueId ? "Backfill Workcell root issue wiki history" : "Backfill Workcell project wiki history",
          useCheapModelProfile: params.useCheapModelProfile === true,
          prompt: [
            "Backfill LLM Wiki distillation was queued from a per-space Workcell ingestion profile.",
    ...
    ctx.actions.register("assemble-workcell-source-bundle", async (params) => {
      return assembleWorkcellSourceBundle(ctx, {
        companyId: readCompanyIdFromParams(params),
        wikiId: stringField(params.wikiId),
        spaceSlug: stringField(params.spaceSlug),
    ...
    ctx.actions.register("create-workcell-distillation-run", async (params) => {
      return createWorkcellDistillationRun(ctx, {
        companyId: readCompanyIdFromParams(params),
        wikiId: stringField(params.wikiId),
        spaceSlug: stringField(params.spaceSlug),
        projectId: stringField(params.projectId),
    ...
    ctx.actions.register("record-workcell-distillation-outcome", async (params) => {
      ...
      return recordWorkcellDistillationOutcome(ctx, {
        companyId: readCompanyIdFromParams(params),
        wikiId: stringField(params.wikiId),
        spaceSlug: stringField(params.spaceSlug),
        runId,
    ...
    ctx.actions.register("distill-workcell-project-page", async (params) => {
      return distillWorkcellProjectPage(ctx, {
        companyId: readCompanyIdFromParams(params),
        wikiId: stringField(params.wikiId),
        spaceSlug: stringField(params.spaceSlug),
        projectId: stringField(params.projectId),
    ...
    ctx.actions.register("distill-workcell-now", async (params) => {
      const companyId = readCompanyIdFromParams(params);
      const spaceSlug = stringField(params.spaceSlug);
      const projectId = stringField(params.projectId);
      ...
      const workItem = await createWorkcellDistillationWorkItem(ctx, {
        ...
        metadata: { requestedFrom: "distill-workcell-now" },
      });
      const operation = await createOperationIssue(ctx, {
        companyId,
        wikiId: stringField(params.wikiId),
        ...
        operationType: "distill",
        title: rootIssueId
          ? "Distill Workcell root issue into wiki"
          : projectId
            ? "Distill Workcell project into wiki"
            : "Distill Workcell changes into wiki",
        useCheapModelProfile: params.useCheapModelProfile === true,
    ...
    ctx.actions.register("enable-workcell-distillation-active-projects", async (params) => {
    ...
    ctx.actions.register("backfill-workcell-distillation", async (params) => {
      const companyId = readCompanyIdFromParams(params);
      const spaceSlug = stringField(params.spaceSlug);
      const projectId = stringField(params.projectId);
      const rootIssueId = stringField(params.rootIssueId);
      ...
      const workItem = await createWorkcellDistillationWorkItem(ctx, {
        ...
        metadata: { backfillStartAt, backfillEndAt, requestedFrom: "backfill-workcell-distillation" },
      });
      const operation = await createOperationIssue(ctx, {
        companyId,
        wikiId: stringField(params.wikiId),
        spaceSlug,
        operationType: "backfill",
        title: rootIssueId ? "Backfill Workcell root issue wiki history" : "Backfill Workcell project wiki history",
        useCheapModelProfile: params.useCheapModelProfile === true,
        prompt: [
          "Backfill LLM Wiki distillation requested for a bounded Workcell source window.",
      ...
      const result = await distillWorkcellProjectPage(ctx, {
        companyId,
        wikiId: stringField(params.wikiId),
        spaceSlug,
        projectId,
    ...
    ctx.actions.register("create-workcell-distillation-work-item", async (params) => {
      ...
      return createWorkcellDistillationWorkItem(ctx, {
        companyId: readCompanyIdFromParams(params),
        wikiId: stringField(params.wikiId),
        spaceSlug: stringField(params.spaceSlug),
        kind,
