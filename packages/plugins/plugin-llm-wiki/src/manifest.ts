...
export const PLUGIN_ID = "workcell.plugin-llm-wiki";
...
export const WORKCELL_DISTILL_SKILL_KEY = "workcell-distill";
...
export const DEFAULT_MAX_WORKCELL_ISSUE_SOURCE_CHARS = 12000;
export const DEFAULT_MAX_WORKCELL_CURSOR_WINDOW_CHARS = 60000;
export const DEFAULT_MAX_WORKCELL_ROUTINE_RUN_CHARS = 120000;
export const DEFAULT_WORKCELL_COST_CENTS_PER_1K_CHARS = 1;
...
function canonicalSkillKey(skillKey: string) {
  return `plugin/workcell-plugin-llm-wiki/${skillKey}`;
}
...
const CURSOR_WINDOW_ROUTINE_DESCRIPTION = `Process bounded Workcell issue-history windows into the LLM Wiki.

Run procedure:
Target space: default (slug: default). Workcell-derived indexing currently writes only into the default space, so this routine never sweeps other spaces. Per-space Workcell ingestion profiles are a later phase; until they ship, treat any prompt to operate on a non-default space here as a bug and stop.
...
5. For ambiguous contradictions or major rewrites, leave the pages unchanged and create or comment a follow-up Workcell issue with the exact files and evidence.
...
const manifest: WorkcellPluginManifestV1 = {
  ...
  localFolders: [
    {
      ...
      description: "Company-scoped local folder that stores raw sources, wiki pages, Workcell project standups under wiki/projects/, AGENTS.md, IDEA.md, wiki/index.md, and wiki/log.md.",
      access: "readWrite",
      requiredDirectories: [
        "raw",
        "wiki",
  ...
  skills: [
    ...
    {
      skillKey: WORKCELL_DISTILL_SKILL_KEY,
      displayName: "Workcell Distill",
      slug: WORKCELL_DISTILL_SKILL_KEY,
      description: "Turn Workcell cursor-window, distill, or backfill source bundles into wiki-insightful project knowledge.",
      markdown: skillMarkdown(WORKCELL_DISTILL_SKILL_KEY)
    },
    {
