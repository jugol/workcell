...
import type { Agent, AgentSessionEvent, Issue, IssueComment, PluginContext, PluginEvent, PluginLocalFolderEntry, Project, ToolResult } from "@workcell/plugin-sdk";
import type { IssueDocument, PluginIssueOriginKind, PluginManagedRoutineResolution, PluginManagedSkillResolution } from "@workcell/plugin-sdk/types";
import {
  DEFAULT_MAX_SOURCE_BYTES,
  DEFAULT_MAX_WORKCELL_CURSOR_WINDOW_CHARS,
  DEFAULT_MAX_WORKCELL_ISSUE_SOURCE_CHARS,
  DEFAULT_MAX_WORKCELL_ROUTINE_RUN_CHARS,
  DEFAULT_WORKCELL_COST_CENTS_PER_1K_CHARS,
  PLUGIN_ID,
...
const MAX_WORKCELL_INGESTION_PROFILE_SOURCE_COUNT = 3;
const MAX_WORKCELL_DISTILLATION_FAN_OUT = 25;
const MAX_WORKCELL_PROFILE_SELECTED_PROJECTS = 25;
const MAX_WORKCELL_PROFILE_ROOT_ISSUES = 25;
...
export type WorkcellDistillationScope = "company" | "project" | "root_issue";
export type WorkcellDistillationWorkItemKind = "manual" | "retry" | "backfill" | "priority_override" | "review_patch";
...
export type WorkcellIngestionSourceScope =
...
export type WorkcellIngestionProfileV1 = {
  version: 1;
  enabled: boolean;
  sourceScopes: WorkcellIngestionSourceScope[];
...
export type WorkcellIngestionProfileEffectiveState =
  | "enabled"
  | "disabled"
  | "policy_blocked"
  | "pending_approval"
...
export type WorkcellIngestionProfileRead = {
  wikiId: string;
  space: Pick<WikiSpace, "id" | "slug" | "displayName" | "accessScope" | "status">;
  profile: WorkcellIngestionProfileV1;
  effectiveState: WorkcellIngestionProfileEffectiveState;
  policyBlocks: string[];
...
type WorkcellIngestionPolicyPurpose =
  | "profile_read"
  | "profile_update"
  | "candidate_search"
  | "queue"
...
type WorkcellIngestionPolicyDecision =
...
type WorkcellIngestionCandidatesInput = SpaceInput & {
  query?: string | null;
};
...
type WorkcellSourceBundleInput = {
  companyId: string;
  wikiId?: string | null;
  spaceSlug?: string | null;
  projectId?: string | null;
...
type WorkcellSourceBundle = {
  markdown: string;
  sourceRefs: WorkcellSourceRef[];
  sourceHash: string;
...
type WorkcellDistillationRunInput = WorkcellSourceBundleInput;

type WorkcellDistillationOutcomeInput = {
  companyId: string;
  wikiId?: string | null;
  spaceSlug?: string | null;
  runId: string;
...
type WorkcellDistillationWorkItemInput = {
  companyId: string;
  wikiId?: string | null;
  spaceSlug?: string | null;
  kind: WorkcellDistillationWorkItemKind;
  projectId?: string | null;
  rootIssueId?: string | null;
...
type WorkcellProjectPageDistillationInput = WorkcellSourceBundleInput & {
  autoApply?: boolean;
...
type WorkcellDistillationPatchOperation =
  | "standup_update"
  ...
  | "index_refresh"
  | "log_append";
type WorkcellDistillationPatch = {
  pagePath: string;
  operationType: WorkcellDistillationPatchOperation;
...
type WorkcellEventIngestResult =
...
async function requireWorkcellIngestionPolicy(
  ctx: PluginContext,
  input: { companyId: string; wikiId: string; spaceSlug?: string | null },
  purpose: WorkcellIngestionPolicyPurpose,
  ...
  const decision = evaluateWorkcellProfilePolicy({
    space,
    profile,
    purpose,
...
function assertWorkcellSourceScopePayload(input: { projectId?: string | null; rootIssueId?: string | null }) {
  if (input.projectId && input.rootIssueId) {
    throw new Error("Workcell source scope must specify either projectId or rootIssueId, not both.");
  }
}
...
function assertRequestedCharacterLimit(name: string, value: unknown, max: number) {
  ...
  if (Math.floor(value) > max) {
    throw new Error(`${name} exceeds the hard Workcell ingestion cap of ${max} characters.`);
  }
}
...
function normalizeCostRate(value: unknown): number {
  if (typeof value !== "number" || !Number.isFinite(value)) return DEFAULT_WORKCELL_COST_CENTS_PER_1K_CHARS;
  return Math.max(0, value);
}

type WorkcellDistillationLimits = {
  maxCharacters: number;
  maxCharactersPerSource: number;
...
type DistillationSourceProtectionResult = {
  body: string;
  warning: string | null;
  refPatch: Pick<WorkcellSourceRef, "redactionStatus" | "redactionReasons">;
};
...
function protectDistillationSourceBody(input: {
  ...
  return {
    body: [
      ...
      "- Review the original Workcell source directly if a human needs the unredacted material.",
    ].join("\n"),
...
async function resolveWorkcellDistillationLimits(
  ctx: PluginContext,
  input: Pick<WorkcellSourceBundleInput, "maxCharacters" | "maxCharactersPerSource" | "routineRun">,
): Promise<WorkcellDistillationLimits> {
  assertRequestedCharacterLimit("maxCharacters", input.maxCharacters, DEFAULT_MAX_WORKCELL_CURSOR_WINDOW_CHARS);
  assertRequestedCharacterLimit("maxCharactersPerSource", input.maxCharactersPerSource, DEFAULT_MAX_WORKCELL_ISSUE_SOURCE_CHARS);
  const config = await ctx.config.get() as Record<string, unknown>;
  const maxCharactersPerSource = Math.min(
    normalizeBundleLimit(input.maxCharactersPerSource, DEFAULT_MAX_WORKCELL_ISSUE_SOURCE_CHARS),
    normalizeBundleLimit(config.maxWorkcellIssueSourceCharacters, DEFAULT_MAX_WORKCELL_ISSUE_SOURCE_CHARS),
  );
  const cursorWindowCap = normalizeBundleLimit(
    config.maxWorkcellCursorWindowCharacters,
    DEFAULT_MAX_WORKCELL_CURSOR_WINDOW_CHARS,
  );
  const routineRunCap = normalizeBundleLimit(
    config.maxWorkcellRoutineRunCharacters,
    DEFAULT_MAX_WORKCELL_ROUTINE_RUN_CHARS,
  );
  ...
  return {
    ...
    costCentsPerThousandSourceCharacters: normalizeCostRate(config.workcellCostCentsPerThousandSourceCharacters),
  };
}

async function resolveWorkcellDistillationLimitsForSpace(
  ctx: PluginContext,
  input: Pick<WorkcellSourceBundleInput, "companyId" | "maxCharacters" | "maxCharactersPerSource" | "routineRun"> & { space: WikiSpace },
): Promise<WorkcellDistillationLimits> {
  const [base, profile] = await Promise.all([
    resolveWorkcellDistillationLimits(ctx, input),
...
function defaultWorkcellIngestionProfile(input: {
  space: Pick<WikiSpace, "slug">;
  legacySettings?: WikiEventIngestionSettings | null;
}): WorkcellIngestionProfileV1 {
  ...
  return {
    ...
    cursor: {
      maxWindowCharacters: DEFAULT_MAX_WORKCELL_CURSOR_WINDOW_CHARS,
      maxCharactersPerSource: DEFAULT_MAX_WORKCELL_ISSUE_SOURCE_CHARS,
...
function normalizeWorkcellIngestionSourceScope(value: unknown): WorkcellIngestionSourceScope | null {
  ...
  if (kind === "active_projects") {
    ...
    return {
      kind,
      limit: normalizeLimit(record.limit, 3, MAX_WORKCELL_PROFILE_SELECTED_PROJECTS),
      ...(statuses && statuses.length > 0 ? { statuses: [...new Set(statuses)] } : {}),
    };
  }
  if (kind === "selected_projects") {
    return { kind, projectIds: stringArray(record.projectIds).slice(0, MAX_WORKCELL_PROFILE_SELECTED_PROJECTS) };
  }
  if (kind === "root_issues") {
    return { kind, issueIds: stringArray(record.issueIds).slice(0, MAX_WORKCELL_PROFILE_ROOT_ISSUES) };
  }
  if (kind === "company_all") {
...
function normalizeWorkcellIngestionProfile(
  value: unknown,
  input: { space: Pick<WikiSpace, "slug">; legacySettings?: WikiEventIngestionSettings | null },
): WorkcellIngestionProfileV1 {
  const fallback = defaultWorkcellIngestionProfile(input);
  ...
  return {
    ...
    sourceScopes: Array.isArray(record.sourceScopes)
      ? record.sourceScopes.map(normalizeWorkcellIngestionSourceScope).filter((scope): scope is WorkcellIngestionSourceScope => Boolean(scope))
    ...
    cursor: {
      maxWindowCharacters: normalizeLimit(cursor.maxWindowCharacters, fallback.cursor.maxWindowCharacters, DEFAULT_MAX_WORKCELL_CURSOR_WINDOW_CHARS),
      maxCharactersPerSource: normalizeLimit(cursor.maxCharactersPerSource, fallback.cursor.maxCharactersPerSource, DEFAULT_MAX_WORKCELL_ISSUE_SOURCE_CHARS),
...
async function profileForSpace(ctx: PluginContext, companyId: string, space: WikiSpace): Promise<WorkcellIngestionProfileV1> {
  const legacySettings = space.slug === DEFAULT_SPACE_SLUG ? await getEventIngestionSettings(ctx, companyId) : null;
  return normalizeWorkcellIngestionProfile(space.settings.workcellIngestion, { space, legacySettings });
}
...
function evaluateWorkcellProfilePolicy(input: {
  space: WikiSpace;
  profile?: WorkcellIngestionProfileV1 | null;
  purpose: WorkcellIngestionPolicyPurpose;
  requireEnabledProfile?: boolean;
}): WorkcellIngestionPolicyDecision {
  ...
  if (space.status !== "active") {
    return {
      ...
      message: `Workcell ingestion policy denied ${purpose}: space "${space.slug}" is ${space.status}.`,
    };
  }
  if (space.accessScope !== "shared") {
    return {
      ...
      message: `Workcell ingestion policy denied ${purpose}: ${space.accessScope} spaces cannot ingest Workcell sources until host permissions are enforced.`,
    };
  }
  if (input.requireEnabledProfile && space.slug !== DEFAULT_SPACE_SLUG && !profile?.enabled) {
    return {
      ...
      message: `Workcell ingestion policy denied ${purpose}: Workcell ingestion is not enabled for space "${space.slug}".`,
    };
  }
  if (input.requireEnabledProfile && space.slug !== DEFAULT_SPACE_SLUG && profile?.enabled && profile.sourceScopes.length === 0) {
    return {
      ...
      message: `Workcell ingestion policy denied ${purpose}: space "${space.slug}" has no source scopes configured.`,
    };
  }
  return { allowed: true, space };
}

export async function getWorkcellIngestionProfile(
  ctx: PluginContext,
  input: { companyId: string; wikiId?: string | null; spaceSlug?: string | null },
): Promise<WorkcellIngestionProfileRead> {
  ...
  const profile = await profileForSpace(ctx, input.companyId, space);
  const policy = evaluateWorkcellProfilePolicy({ space, profile, purpose: "profile_read" });
  const historicalPageCount = await countWorkcellHistoricalPages(ctx, {
    companyId: input.companyId,
    wikiId,
    spaceId: space.id,
  });
  const overlapCount = await countWorkcellProfileOverlaps(ctx, {
    companyId: input.companyId,
    wikiId,
    space,
    profile,
  });
  const effectiveState: WorkcellIngestionProfileEffectiveState = !policy.allowed
    ? "policy_blocked"
...
async function countWorkcellHistoricalPages(ctx: PluginContext, input: { companyId: string; wikiId: string; spaceId: string }): Promise<number> {
...
function scopeIdentity(scope: WorkcellIngestionSourceScope): string[] {
...
async function countWorkcellProfileOverlaps(ctx: PluginContext, input: {
  companyId: string;
  wikiId: string;
  space: WikiSpace;
  profile: WorkcellIngestionProfileV1;
...
async function validateWorkcellIngestionProfile(ctx: PluginContext, input: {
  companyId: string;
  space: WikiSpace;
  profile: WorkcellIngestionProfileV1;
}) {
  const policy = evaluateWorkcellProfilePolicy({
    space: input.space,
  ...
  if (input.profile.enabled && input.profile.sourceScopes.length === 0) {
    throw new Error("Workcell ingestion profile must include at least one source scope before it can be enabled.");
  }
  if (input.profile.sourceScopes.length > MAX_WORKCELL_INGESTION_PROFILE_SOURCE_COUNT) {
    throw new Error(`Workcell ingestion profile sources exceed the hard cap of ${MAX_WORKCELL_INGESTION_PROFILE_SOURCE_COUNT}.`);
  }
  for (const scope of input.profile.sourceScopes) {
    ...
    if (scope.kind === "selected_projects") {
      if (scope.projectIds.length > MAX_WORKCELL_PROFILE_SELECTED_PROJECTS) {
        throw new Error(`selected_projects exceeds the hard cap of ${MAX_WORKCELL_PROFILE_SELECTED_PROJECTS}.`);
      }
    ...
    if (scope.kind === "root_issues") {
      if (scope.issueIds.length > MAX_WORKCELL_PROFILE_ROOT_ISSUES) {
        throw new Error(`root_issues exceeds the hard cap of ${MAX_WORKCELL_PROFILE_ROOT_ISSUES}.`);
      }
...
export async function updateWorkcellIngestionProfile(ctx: PluginContext, input: {
  companyId: string;
  wikiId?: string | null;
  spaceSlug?: string | null;
  profile: unknown;
  ...
  const profile = normalizeWorkcellIngestionProfile(input.profile, { space, legacySettings: space.slug === DEFAULT_SPACE_SLUG ? await getEventIngestionSettings(ctx, input.companyId) : null });
  await validateWorkcellIngestionProfile(ctx, { companyId: input.companyId, space, profile });
  await updateSpace(ctx, {
    companyId: input.companyId,
    wikiId,
    spaceSlug: space.slug,
    settings: { workcellIngestion: profile },
  });
  ...
  await ctx.activity.log({
    companyId: input.companyId,
    message: `Updated Workcell ingestion profile for ${space.displayName}`,
    entityType: "llm_wiki_space",
    entityId: space.id,
    metadata: {
      type: "plugin.llm_wiki.workcell_ingestion_profile_updated",
      wikiId,
      spaceSlug: space.slug,
  ...
  return getWorkcellIngestionProfile(ctx, { companyId: input.companyId, wikiId, spaceSlug: space.slug });
}

export async function listWorkcellIngestionCandidates(ctx: PluginContext, input: WorkcellIngestionCandidatesInput): Promise<{
  ...
  const wikiId = normalizeWikiId(input.wikiId);
  await requireWorkcellIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "candidate_search");
  ...
  await requireWorkcellIngestionPolicy(ctx, {
    companyId: input.companyId,
  ...
  if (sourceKeys.length > MAX_WORKCELL_INGESTION_PROFILE_SOURCE_COUNT) {
    throw new Error(`Workcell ingestion profile sources exceed the hard cap of ${MAX_WORKCELL_INGESTION_PROFILE_SOURCE_COUNT}.`);
  }
  ...
  const profile = normalizeWorkcellIngestionProfile(
    {
      ...defaultWorkcellIngestionProfile({ space: defaultSpace, legacySettings: next }),
      ...
      cursor: {
        ...defaultWorkcellIngestionProfile({ space: defaultSpace, legacySettings: next }).cursor,
...
function assertWikiPath(path: string, options: { allowMetadata?: boolean } = {}): string {
  ...
  if (
    ...
    !(options.allowMetadata && trimmed.startsWith(".workcell/"))
  ) {
...
function distillationCursorTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "workcell_distillation_cursors");
}

function distillationRunTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "workcell_distillation_runs");
}

function sourceSnapshotTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "workcell_source_snapshots");
}

function distillationWorkItemTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "workcell_distillation_work_items");
}

function pageBindingTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "workcell_page_bindings");
}
...
function operationPromptWithSpaceContext(input: OperationSpaceContext): string {
  const workcellDerived = input.operationType === "distill" || input.operationType === "backfill";
  return [
    ...
    workcellDerived
      ? "- Workcell-derived distill/backfill operations are default-space-only in Phase 1. Stop and comment if asked to write Workcell-derived pages into a non-default space."
...
export async function createOperationIssue(ctx: PluginContext, input: OperationInput) {
  ...
  const space = input.operationType === "distill" || input.operationType === "backfill"
    ? await requireWorkcellIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "queue", { requireEnabledProfile: true })
...
function workcellDistillationScope(input: { projectId?: string | null; rootIssueId?: string | null }): WorkcellDistillationScope {
  if (input.rootIssueId) return "root_issue";
  if (input.projectId) return "project";
  return "company";
}

function workcellCursorScopeMetadata(input: { projectId?: string | null; rootIssueId?: string | null }) {
  const sourceScope = workcellDistillationScope(input);
...
async function upsertWorkcellDistillationCursor(ctx: PluginContext, input: {
  companyId: string;
  wikiId: string;
  spaceId: string;
  projectId?: string | null;
  ...
  const scope = workcellCursorScopeMetadata(input);
  await ctx.db.execute(
    `INSERT INTO ${distillationCursorTable(ctx)} AS workcell_distillation_cursors
     ...
     VALUES ($1, $2, $3, $11, $4, $5, $6, $7, 'workcell_issue_history', $8::timestamptz, $9, $10::jsonb)
     ON CONFLICT (company_id, wiki_id, space_id, source_scope, scope_key, source_kind)
     DO UPDATE SET last_observed_at = GREATEST(
                       COALESCE(workcell_distillation_cursors.last_observed_at, EXCLUDED.last_observed_at),
                       COALESCE(EXCLUDED.last_observed_at, workcell_distillation_cursors.last_observed_at)
                     ),
                   pending_event_count = workcell_distillation_cursors.pending_event_count + EXCLUDED.pending_event_count,
                   metadata = workcell_distillation_cursors.metadata || EXCLUDED.metadata,
                   updated_at = now()`,
    [
      cursorId,
      input.companyId,
  ...
  const rows = await ctx.db.query<{ id: string }>(
    `SELECT id
      ...
      WHERE company_id = $1
        ...
        AND source_scope = $4
        AND scope_key = $5
        AND source_kind = 'workcell_issue_history'
      LIMIT 1`,
...
export async function enableActiveProjectDistillation(ctx: PluginContext, input: {
  ...
  const wikiId = normalizeWikiId(input.wikiId);
  const space = await requireWorkcellIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "candidate_search", { requireEnabledProfile: true });
  if (typeof input.limit === "number" && Number.isFinite(input.limit) && Math.floor(input.limit) > MAX_WORKCELL_DISTILLATION_FAN_OUT) {
    throw new Error(`Workcell ingestion fan-out exceeds the hard cap of ${MAX_WORKCELL_DISTILLATION_FAN_OUT} enabled profiles.`);
  }
  ...
  for (const project of activeProjects) {
    ...
    const cursorId = await upsertWorkcellDistillationCursor(ctx, {
      companyId: input.companyId,
      wikiId,
      spaceId: space.id,
...
function appendBoundedSection(input: {
  ...
  title: string;
  body: string;
  refs: WorkcellSourceRef[];
  ref: WorkcellSourceRef;
...
function sourceRefUpdatedAt(ref: WorkcellSourceRef): string | null {
  return ref.updatedAt ?? ref.createdAt ?? null;
}

function issueInBackfillWindow(issue: Issue, input: Pick<WorkcellSourceBundleInput, "backfillStartAt" | "backfillEndAt">): boolean {
...
async function listWorkcellBundleIssues(ctx: PluginContext, input: WorkcellSourceBundleInput): Promise<Issue[]> {
...
export async function assembleWorkcellSourceBundle(ctx: PluginContext, input: WorkcellSourceBundleInput): Promise<WorkcellSourceBundle> {
  const wikiId = normalizeWikiId(input.wikiId);
  assertWorkcellSourceScopePayload(input);
  const space = await requireWorkcellIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "execute", { requireEnabledProfile: true });
  const limits = await resolveWorkcellDistillationLimitsForSpace(ctx, { ...input, space });
  ...
  const issues = await listWorkcellBundleIssues(ctx, input);
  const scope = workcellCursorScopeMetadata(input);
  const sourceRefs: WorkcellSourceRef[] = [];
  const warnings: string[] = [];
  const lines = [
    `# Workcell source bundle`,
    "",
    "## Bundle Metadata",
    "",
...
export async function createWorkcellDistillationRun(ctx: PluginContext, input: WorkcellDistillationRunInput) {
  const wikiId = normalizeWikiId(input.wikiId);
  assertWorkcellSourceScopePayload(input);
  const space = await requireWorkcellIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "execute", { requireEnabledProfile: true });
  const scope = workcellCursorScopeMetadata(input);
  ...
  const bundle = await assembleWorkcellSourceBundle(ctx, input);
...
export async function recordWorkcellDistillationOutcome(ctx: PluginContext, input: WorkcellDistillationOutcomeInput) {
  const wikiId = normalizeWikiId(input.wikiId);
  const space = await requireWorkcellIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "execute", { requireEnabledProfile: true });
...
export async function createWorkcellDistillationWorkItem(ctx: PluginContext, input: WorkcellDistillationWorkItemInput) {
  const wikiId = normalizeWikiId(input.wikiId);
  assertWorkcellSourceScopePayload(input);
  const space = await requireWorkcellIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "queue", { requireEnabledProfile: true });
  ...
  await ctx.db.execute(
    `INSERT INTO ${distillationWorkItemTable(ctx)} AS workcell_distillation_work_items
     ...
     DO UPDATE SET priority = EXCLUDED.priority,
                   metadata = workcell_distillation_work_items.metadata || EXCLUDED.metadata,
                   updated_at = now()`,
    [
      itemId,
      input.companyId,
...
function sourceRefLabel(ref: WorkcellSourceRef): string {
  const issue = ref.issueIdentifier ? issueReference(ref.issueIdentifier) : (ref.title ?? "Workcell source");
  if (ref.kind === "document") return `${issue} document:${ref.documentKey ?? "unknown"}`;
  if (ref.kind === "comment") return `${issue} comment`;
  return issue;
}

function sourceRefMarkdown(ref: WorkcellSourceRef): string {
  const metadata = [
...
function issueSourceRef(issue: Issue): WorkcellSourceRef {
  return {
    kind: "issue",
    issueId: issue.id,
    issueIdentifier: issue.identifier ?? null,
...
function projectPageSlug(input: { project: Project | null; rootIssue: Issue | null }): string {
  return slugify(input.project?.name ?? input.rootIssue?.title ?? "workcell-project");
}
...
function hasDurableSignal(bundle: WorkcellSourceBundle, issues: Issue[]): boolean {
...
function standupPageContents(input: {
  ...
  issues: Issue[];
  bundle: WorkcellSourceBundle;
  pagePath: string;
  durablePagePath: string;
}): string {
  const currentAsOf = input.bundle.sourceWindowEnd ?? new Date().toISOString();
  const title = input.project?.name ?? input.rootIssue?.title ?? "Workcell Project";
  ...
  return [
    ...
    "",
    lead
      ? `The current center of gravity is **${issueConcept(lead)}** (${issueReferenceFor(lead)}). ${input.bundle.clipped ? "The source window was clipped, so treat this as a bounded readout rather than the full live state." : "This is a high-level readout of the meaningful Workcell work in the current source window."}`
...
function projectPageContents(input: {
  ...
  return [
    ...
    "## Overview",
    "",
    description ? excerpt(description, 700) : `This page synthesizes Workcell issue history into a stable project brief for ${title}.`,
    "",
    "## Current Direction",
    "",
    activeIssues.length
    ...
    input.bundle.clipped ? "\nThe source window was clipped, so verify Workcell before treating this as complete state." : null,
    "",
    "## Workstreams",
    "",
    ...(recentIssues.length
...
function decisionsPageContents(input: { project: Project | null; rootIssue: Issue | null; issues: Issue[]; bundle: WorkcellSourceBundle }): string {
  const title = input.project?.name ?? input.rootIssue?.title ?? "Workcell Project";
...
function historyPageContents(input: { project: Project | null; rootIssue: Issue | null; issues: Issue[]; bundle: WorkcellSourceBundle }): string {
  const title = input.project?.name ?? input.rootIssue?.title ?? "Workcell Project";
...
function appendProjectLogContents(current: string | null, input: { standupPath: string; pagePath: string; runId: string; sourceHash: string; status: string; warnings: string[] }): string {
  ...
  const warningLines = input.warnings.length
    ...
    : ["- warnings: none"];
  const entry = [
    `## [${new Date().toISOString().slice(0, 10)}] workcell-distill | ${input.status}`,
...
async function upsertPageBinding(ctx: PluginContext, input: {
  ...
  await ctx.db.execute(
    `INSERT INTO ${pageBindingTable(ctx)} AS workcell_page_bindings
     ...
     DO UPDATE SET last_applied_source_hash = EXCLUDED.last_applied_source_hash,
                   ...
                   metadata = workcell_page_bindings.metadata || EXCLUDED.metadata,
                   updated_at = now()`,
    [
      randomUUID(),
      input.companyId,
...
export function getDistillationAutoApplyRestriction(): DistillationAutoApplyRestriction {
  const rawMode = process.env.WORKCELL_DEPLOYMENT_MODE;
  const rawExposure = process.env.WORKCELL_DEPLOYMENT_EXPOSURE;
...
export async function distillWorkcellProjectPage(ctx: PluginContext, input: WorkcellProjectPageDistillationInput) {
  if (!input.projectId && !input.rootIssueId) {
  ...
  const run = await createWorkcellDistillationRun(ctx, input);
  ...
  if (!hasDurableSignal(bundle, issues)) {
    await recordWorkcellDistillationOutcome(ctx, {
      companyId: input.companyId,
      wikiId,
      spaceSlug: space.slug,
      runId: run.runId,
      ...
      warning: "Skipped low-signal Workcell source window.",
    });
    return {
      status: "skipped",
      reason: "low_signal",
      ...
      cursorId: run.cursorId,
      sourceHash: bundle.sourceHash,
      warnings: ["Skipped low-signal Workcell source window."],
      patches: [] as WorkcellDistillationPatch[],
    };
  }
  ...
  if (existingBinding?.last_applied_source_hash === bundle.sourceHash) {
    await recordWorkcellDistillationOutcome(ctx, {
      ...
      sourceHash: bundle.sourceHash,
      sourceWindowEnd: bundle.sourceWindowEnd,
      warning: "Skipped unchanged Workcell source hash.",
    });
    return {
      status: "skipped",
      ...
      cursorId: run.cursorId,
      sourceHash: bundle.sourceHash,
      warnings: ["Skipped unchanged Workcell source hash."],
      patches: [] as WorkcellDistillationPatch[],
    };
  }
  ...
  const title = project?.name ?? rootIssue?.title ?? "Workcell Project";
  ...
  const patches: WorkcellDistillationPatch[] = [
  ...
  for (const patch of patches) {
    await writeWikiPage(ctx, {
      ...
      summary: `Workcell distillation ${patch.operationType} from ${bundle.sourceHash}`,
...
function rawPathForWorkcellEvent(input: {
  ...
  return assertRawPath(`raw/workcell/${input.sourceKind}/${eventDate}-${slugify(identifier)}-${slugify(input.label)}-${contentHash(input.contents).slice(0, 8)}.md`);
}

function formatIssueEventSource(issue: Issue, event: PluginEvent, maxCharacters: number): string {
  return truncateEventSource([
    `# Workcell issue: ${sourceTitleForIssue(issue)}`,
    "",
    "## Provenance",
    "",
...
function formatCommentEventSource(issue: Issue, comment: IssueComment, event: PluginEvent, maxCharacters: number): string {
  return truncateEventSource([
    `# Workcell comment on ${sourceTitleForIssue(issue)}`,
    "",
    "## Provenance",
    "",
    `- Company ID: ${issue.companyId}`,
...
function formatDocumentEventSource(issue: Issue, document: IssueDocument, event: PluginEvent, maxCharacters: number): string {
  return truncateEventSource([
    `# Workcell document: ${document.title ?? document.key}`,
    "",
    "## Provenance",
    "",
    `- Company ID: ${issue.companyId}`,
...
async function recordWorkcellCursorObservation(ctx: PluginContext, input: {
  companyId: string;
  wikiId: string;
  space: WikiSpace;
  sourceKind: WikiEventIngestionSource;
  ...
  issue: Issue;
  event: PluginEvent;
}): Promise<Extract<WorkcellEventIngestResult, { status: "recorded" }>> {
  const cursorId = await upsertWorkcellDistillationCursor(ctx, {
    companyId: input.companyId,
    wikiId: input.wikiId,
    spaceId: input.space.id,
...
async function workcellProfileIncludesIssue(ctx: PluginContext, input: {
  companyId: string;
  issue: Issue;
  profile: WorkcellIngestionProfileV1;
}): Promise<boolean> {
...
async function routeWorkcellCursorObservation(ctx: PluginContext, input: {
  companyId: string;
  sourceKind: WikiEventIngestionSource;
  sourceId: string;
  issue: Issue;
  event: PluginEvent;
}): Promise<WorkcellEventIngestResult> {
  const { spaces } = await listSpaces(ctx, { companyId: input.companyId, wikiId: DEFAULT_WIKI_ID });
  const recorded: Array<Extract<WorkcellEventIngestResult, { status: "recorded" }>> = [];
  let eligibleProfileCount = 0;
  for (const space of spaces) {
    const profile = await profileForSpace(ctx, input.companyId, space);
    if (!profile.enabled) continue;
    const policy = evaluateWorkcellProfilePolicy({ space, profile, purpose: "event_routing", requireEnabledProfile: true });
    if (!policy.allowed) continue;
    if (!profile.sourceKinds[input.sourceKind]) continue;
    if (!(await workcellProfileIncludesIssue(ctx, { companyId: input.companyId, issue: input.issue, profile }))) continue;
    eligibleProfileCount += 1;
    if (eligibleProfileCount > MAX_WORKCELL_DISTILLATION_FAN_OUT) {
      throw new Error(`Workcell ingestion fan-out exceeds the hard cap of ${MAX_WORKCELL_DISTILLATION_FAN_OUT} enabled profiles.`);
    }
    if (await ctx.state.get(eventIngestionDedupKey(input.companyId, space.wikiId, space.id, input.sourceKind, input.sourceId))) {
      continue;
    }
    recorded.push(await recordWorkcellCursorObservation(ctx, {
      ...input,
      wikiId: space.wikiId,
      space,
    }));
...
export async function handleWorkcellEventIngestion(ctx: PluginContext, event: PluginEvent): Promise<WorkcellEventIngestResult> {
  ...
  if (event.eventType === "issue.created" || event.eventType === "issue.updated") {
    ...
    return routeWorkcellCursorObservation(ctx, {
      companyId,
      sourceKind: "issues",
      sourceId,
      issue,
