...
const EXPANDED_STORAGE_KEY = `workcell.plugin-llm-wiki:route-sidebar-expanded:v2:${COMPANY_ID}`;
...
describe("WikiRouteSidebar", () => {
  ...
  beforeEach(() => {
    ...
    (globalThis as BridgeGlobal).__workcellPluginBridge__ = {
      sdkUi: {
  ...
  afterEach(() => {
    ...
    container.remove();
    window.localStorage.clear();
    delete (globalThis as BridgeGlobal).__workcellPluginBridge__;
  });
  ...
  it("keeps sidebar tree selection scoped to sidebar navigation", () => {
    ...
    expect(navigatedTo).toEqual({
      ...
      options: { state: { workcellWikiSidebarTreePath: "wiki/concepts/sidebar-navigation.md" } },
    });
    ...
    hostLocation = {
      pathname: "/PAP/wiki/page/wiki/entities/workcell.md",
      search: "",
      hash: "",
    };
    act(() => {
...
describe("WikiPage", () => {
  ...
  it("renders structured Workcell source refs as text", () => {
    act(() => {
