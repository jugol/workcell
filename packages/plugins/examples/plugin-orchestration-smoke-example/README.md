...
curl -X POST http://127.0.0.1:3100/api/plugins/install \
  ...
  -d '{"packageName":"/absolute/path/to/workcell/packages/plugins/examples/plugin-orchestration-smoke-example","isLocalPath":true}'
```
...
```bash
curl -X POST http://127.0.0.1:3100/api/plugins/workcell.plugin-orchestration-smoke-example/api/issues/<issue-id>/smoke \
  -H "Content-Type: application/json" \
