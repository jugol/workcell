...
import type { Goal, Issue } from "@workcell/shared";
import {
  DEFAULT_CONFIG,
  JOB_KEYS,
  PLUGIN_ID,
...
async function registerToolHandlers(ctx: PluginContext): Promise<void> {
  ...
  ctx.tools.register(
    TOOL_NAMES.companySummary,
    {
      ...
      description: "Summarizes current company counts from the Workcell APIs.",
...
const plugin: WorkcellPlugin = definePlugin({
  async setup(ctx) {
    currentContext = ctx;
