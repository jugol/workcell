...
Example Workcell plugin that demonstrates:
...
```bash
pnpm --filter @workcell/plugin-file-browser-example build
pnpm workcell plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm workcell plugin uninstall workcell-file-browser-example --force
```
...
- Optional: use `workcell-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
