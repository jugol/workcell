...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".workcell-sdk");
...
/**
 * Generate a complete Workcell plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A Workcell plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "workcell-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["workcell", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@workcell/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@workcell/plugin-sdk": sdkDependency,
...
`,
    ...
    writeFile(
      ...
      `import { usePluginData, type PluginWidgetProps } from "@workcell/plugin-sdk/ui";
...
} from "@workcell/plugin-sdk/testing";
...
`,
    ...
    writeFile(
      path.join(outputDir, "src", "ui", "index.tsx"),
      `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@workcell/plugin-sdk/ui";
...
When this package is installed from a local path, Workcell watches that rebuilt
output and reloads the plugin worker. Local installs run trusted code from this
folder on your machine.

${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@workcell/plugin-sdk\` and \`@workcell/shared\` from a local Workcell checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.workcell-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}

## Install Into Workcell

\`\`\`bash
workcell plugin install ${shellQuote(toPosixPath(outputDir))}
\`\`\`
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-workcell-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <workcell-sdk-path>]");
    process.exit(1);
  }
