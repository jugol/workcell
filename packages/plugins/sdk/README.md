...
Official TypeScript SDK for Workcell plugin authors.

- **Worker SDK:** `@workcell/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@workcell/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@workcell/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@workcell/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@workcell/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@workcell/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@workcell/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, `useHostNavigation`, slot prop types |
| `@workcell/plugin-sdk/ui/hooks` | Hooks only |
| `@workcell/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@workcell/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@workcell/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@workcell/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@workcell/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@workcell/plugin-sdk/types` | Worker context and API types (advanced) |
...
- Plugin UI bundles run as same-origin JavaScript inside the main Workcell app. They can call ordinary Workcell HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the Workcell instance at runtime.
...
- The host ships a small shared React component kit through `@workcell/plugin-sdk/ui`. Use it for native Workcell controls; custom React and CSS are still supported.
...
```ts
import { definePlugin, runWorker } from "@workcell/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@workcell/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core Workcell information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@workcell/plugin-sdk`.
...
const child = await ctx.issues.create({
  ...
  originKind: "plugin:workcell.missions:feature",
...
```tsx
import { usePluginData, usePluginAction } from "@workcell/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@workcell/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@workcell/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@workcell/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@workcell/plugin-sdk/ui";
...
Routes Workcell-internal plugin links through the host router without a full document reload. Use `linkProps()` for anchors so the browser still gets a real `href` for copy-link, modifier-click, middle-click, and open-in-new-tab behavior.

```tsx
import { useHostNavigation } from "@workcell/plugin-sdk/ui";
...
Avoid raw same-origin `href`s or `window.location.assign()` for Workcell-internal navigation from plugin UI. Those bypass the host router and can reload the whole app. External links should keep normal anchors with `target="_blank"` and `rel="noopener noreferrer"` as appropriate.
...
```tsx
import { usePluginStream } from "@workcell/plugin-sdk/ui";
...
The host provides selected shared UI components through `@workcell/plugin-sdk/ui`.
...
used by Workcell issue comments and documents:

```tsx
import { MarkdownBlock, MarkdownEditor } from "@workcell/plugin-sdk/ui";
...
```tsx
<MarkdownBlock
  content={"See [[wiki/entities/workcell|Workcell]]."}
  enableWikiLinks
...
```tsx
import { FileTree, type FileTreeNode } from "@workcell/plugin-sdk/ui";
...
or configure work against Workcell entities. Both are controlled components and
load their options from the host for the provided company.

```tsx
import { AssigneePicker, ProjectPicker } from "@workcell/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@workcell/plugin-sdk/ui`.
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@workcell/plugin-sdk/ui";
...
```ts
import { createPluginBundlerPresets } from "@workcell/plugin-sdk/bundlers";
...
```bash
workcell-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@workcell/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__workcell__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__workcell__/events` streams `reload` SSE events on UI build changes
