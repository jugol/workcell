 ...
 *
 * These components are exported from `@workcell/plugin-sdk/ui` and are
 ...
 *
 * @see PLUGIN_SPEC.md §19.6 — Shared Components In `@workcell/plugin-sdk/ui`
 */
...
function createSdkUiComponent<TProps>(name: string): React.ComponentType<TProps> {
  return function WorkcellSdkUiComponent(props: TProps) {
...
/**
 * Renders Workcell's shared Markdown editor.
 *
 * @see PLUGIN_SPEC.md §19.6 — Shared Components
 */
...
/**
 * Renders Workcell's native issue list component for company-scoped plugin
...
/**
 * Renders Workcell's native managed routines list for plugin settings pages.
 */
