/**
 * Core types for the Workcell plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@workcell/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @workcell/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of Workcell object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the Workcell UI. */
  title?: string;
...
export interface PluginLocalFoldersClient {
  ...
  declarations(): import("@workcell/shared").PluginLocalFolderDeclaration[];
...
/**
 * `ctx.events` — subscribe to and emit Workcell domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core Workcell domain event or a plugin-namespaced event.
   *
...
/**
 * Result of an MCP tool call surfaced to a plugin (WC-65).
 */
export interface PluginMcpToolResult {
  /** Concatenated text content from the tool result. */
  text: string;
  /** True when the SERVER reported a tool-execution error (not a transport failure). */
  isError: boolean;
...
/**
 * A handle to one outbound MCP server, scoped to the plugin's invocation
 * company. Returned by `ctx.mcpClients.getClient(mcpKey)`.
 */
export interface PluginMcpClient {
  /** Invoke a tool on the MCP server. */
  callTool(toolName: string, args?: Record<string, unknown>): Promise<PluginMcpToolResult>;
  /** List the tools the MCP server exposes. */
  listTools(): Promise<Array<{ name: string; description?: string }>>;
}

/**
 * `ctx.mcpClients` — call out to company-scoped outbound MCP servers.
 *
 * Requires the `mcp.client` capability. The host resolves `mcpKey` through the
 * capability registry, enforces an ACTIVE company-wide assignment, and scopes
 * every call to the plugin's invocation company (the plugin cannot target
 * another tenant). Transport failures surface as a rejected promise; a tool
 * that returns an error result resolves with `isError: true`.
 *
 * @see PLUGIN_SPEC.md §15 — Capability Model
 */
export interface PluginMcpClientsClient {
  /** Acquire a handle to the MCP server registered under `mcpKey`. */
  getClient(mcpKey: string): PluginMcpClient;
}
 ...
 * This client resolves the reference through the Workcell secret provider
...
export interface PluginSecretsClient {
   ...
   * in the Workcell secret provider (e.g. `"MY_API_KEY"`).
   *
...
/**
 * `ctx.routines` — resolve and reconcile plugin-managed Workcell routines.
 *
 ...
 * ```
 *
 * @see usePluginStream in `@workcell/plugin-sdk/ui`
 */
...
export interface PluginContext {
  ...
  /** Call out to company-scoped outbound MCP servers. Requires `mcp.client`. */
  mcpClients: PluginMcpClientsClient;
