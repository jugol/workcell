...
function requireCapability(manifest: WorkcellPluginManifestV1, allowed: Set<PluginCapability>, capability: PluginCapability) {
 ...
 * can validate plugin behavior without spinning up the Workcell server runtime.
 */
export function createTestHarness(options: TestHarnessOptions): TestHarness {
  ...
  function isManagedAgent(agent: Agent, agentKey: string) {
    const marker = agent.metadata?.workcellManagedResource;
    return Boolean(
      marker
      ...
      getClient(mcpKey) {
        return {
          async callTool(toolName) {
            requireCapability(manifest, capabilitySet, "mcp.client");
            return {
              text: `[test-mcp ${mcpKey}.${toolName}]`,
              isError: false,
              content: [{ type: "text", text: `[test-mcp ${mcpKey}.${toolName}]` }],
            };
          },
          async listTools() {
            requireCapability(manifest, capabilitySet, "mcp.client");
