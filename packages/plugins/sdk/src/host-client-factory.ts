...
import type { PluginCapability } from "@workcell/shared";
...
export interface HostServices {
  ...
  /**
   * WC-65: outbound MCP clients. The host derives the company from the trusted
   * invocation scope and passes it here — the plugin never supplies it.
   */
  mcpClients: {
    callTool(params: {
      companyId: string;
    ...
    }): Promise<WorkerToHostMethods["mcp.callTool"][1]>;
    listTools(params: {
      companyId: string;
      mcpKey: string;
    }): Promise<WorkerToHostMethods["mcp.listTools"][1]>;
  };
...
const METHOD_CAPABILITY_MAP: Record<WorkerToHostMethodName, PluginCapability | null> = {
  ...
  // MCP (outbound) — WC-65
  "mcp.callTool": "mcp.client",
  "mcp.listTools": "mcp.client",
...
export function createHostClientHandlers(
  ...
  return {
    ...
    // MCP (outbound) — WC-65. The company is taken from the TRUSTED invocation
    // scope (never from params), so a plugin can't target another tenant's MCP
    // server. A scope-less invocation is refused (can't resolve a company).
    "mcp.callTool": gated("mcp.callTool", async (params, context) => {
      const companyId = readNonEmptyString(context?.invocationScope?.companyId);
      if (!companyId) {
        throw new InvocationScopeDeniedError(
          pluginId,
          "mcp.callTool",
          "an MCP tool call requires a company-scoped invocation",
        );
      }
      return services.mcpClients.callTool({
        companyId,
    ...
    "mcp.listTools": gated("mcp.listTools", async (params, context) => {
      const companyId = readNonEmptyString(context?.invocationScope?.companyId);
      if (!companyId) {
        throw new InvocationScopeDeniedError(
          pluginId,
          "mcp.listTools",
          "listing MCP tools requires a company-scoped invocation",
        );
      }
      return services.mcpClients.listTools({ companyId, mcpKey: params.mcpKey });
    }),
