 ...
 * channel.
 *
 * The Workcell plugin runtime uses JSON-RPC 2.0 over stdio to communicate
...
export type { PluginLauncherRenderContextSnapshot } from "@workcell/shared";
 ...
 * we use strings (UUIDs or monotonic counters) for all Workcell messages.
 */
...
> {
   ...
   */
  readonly workcellInvocation?: PluginInvocationContext;
  /** Opaque top-level invocation id echoed by worker→host requests. */
  readonly workcellInvocationId?: string;
}
...
> {
   ...
   * Worker→host notifications echo only `workcellInvocationId`.
   */
  readonly workcellInvocation?: PluginInvocationContext;
...
/**
 * Workcell plugin-specific error codes.
 *
...
export interface InitializeParams {
  ...
  instanceInfo: {
    /** UUID of this Workcell instance. */
    instanceId: string;
    /** Semver version of the running Workcell host. */
    hostVersion: string;
  };
  /** Host API version. */
  apiVersion: number;
...
export interface PluginPerformActionActorContext {
  /** Authenticated principal type resolved by the Workcell host. */
...
export interface WorkerToHostMethods {
  ...
  // MCP (outbound) — WC-65. companyId is NOT a param: the host derives it from
  // the trusted invocation scope, so a plugin can't target another tenant's
  // MCP server.
  "mcp.callTool": [
    params: { mcpKey: string; toolName: string; args?: Record<string, unknown> },
    result: { text: string; isError: boolean; content: unknown },
  ];
  "mcp.listTools": [
    params: { mcpKey: string },
