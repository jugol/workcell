...
import type { WorkcellPlugin } from "./define-plugin.js";
import type {
 ...
 * import { startWorkerRpcHost } from "@workcell/plugin-sdk";
 *
 * startWorkerRpcHost({ plugin });
 * ```
 *
...
export function startWorkerRpcHost(options: WorkerRpcHostOptions): WorkerRpcHost {
  ...
  let manifest: WorkcellPluginManifestV1 | null = null;
  ...
  const toolHandlers = new Map<string, {
    declaration: Pick<import("@workcell/shared").PluginToolDeclaration, "displayName" | "description" | "parametersSchema">;
    ...
    return new Promise<WorkerToHostMethods[M][1]>((resolve, reject) => {
      ...
      try {
        ...
        const request = {
          ...
          ...(activeInvocation ? { workcellInvocationId: activeInvocation.id } : {}),
        };
        sendMessage(request);
      } catch (err) {
  ...
  function buildContext(): PluginContext {
    return {
      ...
      // WC-65: outbound MCP clients. getClient returns a thin proxy whose
      // callTool/listTools are worker→host RPCs; the host resolves the server
      // + scopes to the invocation company.
      mcpClients: {
        getClient(mcpKey: string) {
          return {
            async callTool(toolName: string, args?: Record<string, unknown>) {
              return callHost("mcp.callTool", { mcpKey, toolName, args });
            },
            async listTools() {
              return callHost("mcp.listTools", { mcpKey });
            },
          };
        },
      },
  ...
  async function handleHostRequest(request: JsonRpcRequest): Promise<void> {
    ...
    try {
      ...
      const result = request.workcellInvocation
        ? await invocationContextStorage.run(request.workcellInvocation, invoke)
        : await invoke();
  ...
  function handleLine(line: string): void {
    ...
    if (isJsonRpcResponse(message)) {
      ...
      const runNotification = (fn: () => void | Promise<void>) => {
        if (notif.workcellInvocation) {
          return invocationContextStorage.run(notif.workcellInvocation, fn);
        }
        return fn();
      };
