...
This Worker is the operator-facing bridge used by `@workcell/plugin-cloudflare-sandbox`.

It exposes a small authenticated JSON API under `/api/workcell-sandbox/v1` and translates Workcell lease and command requests into Cloudflare Sandbox SDK calls.
...
- clean up timed-out sessions so Workcell does not inherit wedged background processes
...
After deploy, configure Workcell with:
