...
function buildHeaders(config: CloudflareDriverConfig, extra: BridgeClientHeaders = {}): Headers {
  ...
  if (extra.environmentId) headers.set("X-Workcell-Environment-Id", extra.environmentId);
  if (extra.runId) headers.set("X-Workcell-Run-Id", extra.runId);
  if (extra.issueId) headers.set("X-Workcell-Issue-Id", extra.issueId);
  return headers;
}
...
export function createCloudflareBridgeClient(options: BridgeClientOptions) {
  ...
  const apiPrefix = "/api/workcell-sandbox/v1";
