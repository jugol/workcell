...
async function resolveSandboxWorkingDirectory(sandbox: Sandbox): Promise<string> {
  const root = (await sandbox.getWorkDir())?.trim()
    ...
    || "/home/daytona";
  const remoteCwd = path.posix.join(root, "workcell-workspace");
...
async function executeOneShot(
  ...
  const stdinPath = params.stdin != null ? `/tmp/workcell-stdin-${randomUUID()}` : null;
  ...
  async onEnvironmentRealizeWorkspace(
    ...
    const remoteCwd =
      ...
      params.lease.metadata.remoteCwd.trim().length > 0
        ...
        : params.workspace.remotePath ?? params.workspace.localPath ?? "/workcell-workspace";
