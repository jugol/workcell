...
const DEFAULT_WORKDIR = "/workspace/workcell";
...
function resolveAuth(config: ModalDriverConfig): { tokenId: string; tokenSecret: string } | null {
  ...
  // environment config, which Workcell stores as company secrets.
...
function buildSandboxTags(input: {
  ...
  return {
    ...
    "workcell-company-id": input.companyId,
    "workcell-environment-id": input.environmentId,
    "workcell-reuse-lease": input.reuseLease ? "true" : "false",
    ...(input.runId ? { "workcell-run-id": input.runId } : {}),
  };
}
  ...
  async onEnvironmentProbe(
    ...
    try {
      ...
      try {
        ...
        const proc = await sandbox.exec(["sh", "-lc", "printf workcell-probe"]);
        const { stdout, exitCode } = await readProcessStreams(proc);
        if (exitCode !== 0 || stdout.trim() !== "workcell-probe") {
          return {
            ok: false,
  ...
  async onEnvironmentExecute(
    ...
    try {
      ...
      const stdinPath = params.stdin != null
        ? `/tmp/workcell-stdin-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`
        : null;
      try {
