...
describe("exe.dev sandbox provider plugin", () => {
  ...
  it("normalizes config and emits SSH guidance warnings", async () => {
    ...
    expect(result).toEqual({
      ok: true,
      warnings: [
        "The Workcell host must have SSH access to the created exe.dev VM, and its SSH key must be registered with exe.dev. The API token only covers provisioning.",
  ...
  it("acquires a lease by creating a VM and preparing the SSH workspace", async () => {
    fetchMock.mockResolvedValueOnce(
      new Response(JSON.stringify({
        vm_name: "workcell-env-run",
        ssh_dest: "workcell-env-run.exe.xyz",
        https_url: "https://workcell-env-run.exe.xyz",
        status: "running",
      }), { status: 200 }),
    );
    ...
    expect(lease).toMatchObject({
      providerLeaseId: "workcell-env-run",
      metadata: {
        provider: "exe-dev",
        vmName: "workcell-env-run",
        sshDest: "workcell-env-run.exe.xyz",
  ...
  it("surfaces exe.dev SSH onboarding guidance during lease acquisition", async () => {
    ...
    await expect(plugin.definition.onEnvironmentAcquireLease?.({
      ...
      "the Workcell host SSH key is not registered with exe.dev",
    );

    expect(String(fetchMock.mock.calls[1]?.[1]?.body ?? "")).toBe("rm --json 'workcell-env-run'");
  });
  ...
  it("returns exe.dev SSH onboarding guidance for command execution failures", async () => {
    ...
    expect(String(result?.stderr ?? "")).toContain("the Workcell host SSH key is not registered with exe.dev");
  ...
  it("probes by creating and then deleting a VM after SSH verification", async () => {
    fetchMock
      .mockResolvedValueOnce(
        new Response(JSON.stringify({
          vm_name: "workcell-probe",
          ssh_dest: "workcell-probe.exe.xyz",
          status: "running",
        }), { status: 200 }),
      )
    ...
    expect(result).toMatchObject({
      ok: true,
      summary: "Connected to exe.dev VM workcell-probe.",
      metadata: {
        provider: "exe-dev",
        vmName: "workcell-probe",
        sshDest: "workcell-probe.exe.xyz",
        shellCommand: "bash",
      },
    });
    expect(String(fetchMock.mock.calls[1]?.[1]?.body ?? "")).toBe("rm --json 'workcell-probe'");
  });
  ...
  it("returns onboarding guidance when probe hits exe.dev SSH registration", async () => {
    ...
    expect(String(result?.metadata?.error ?? "")).toContain("the Workcell host SSH key is not registered with exe.dev");
    expect(String(fetchMock.mock.calls[1]?.[1]?.body ?? "")).toBe("rm --json 'workcell-probe'");
  });
  ...
  it("realizes a workspace by mkdir-ing the remote cwd over SSH when VM metadata is present", async () => {
    ...
    const result = await plugin.definition.onEnvironmentRealizeWorkspace?.({
      ...
      lease: {
        ...
        metadata: {
          sshDest: "vm-1.exe.xyz",
          remoteCwd: "/srv/workcell/run-1",
        },
      },
      workspace: {
        localPath: "/local/workcell",
    ...
    expect(sshCommand).toContain("/srv/workcell/run-1");
    expect(result).toMatchObject({
      cwd: "/srv/workcell/run-1",
      metadata: {
        provider: "exe-dev",
        remoteCwd: "/srv/workcell/run-1",
      },
  ...
  it("falls back through workspace.remotePath then workspace.localPath when lease.metadata.remoteCwd is missing", async () => {
    ...
    const result = await plugin.definition.onEnvironmentRealizeWorkspace?.({
      ...
      workspace: {
        localPath: "/local/workcell",
        remotePath: "/srv/workcell/remote-fallback",
      },
    });

    expect(result?.cwd).toBe("/srv/workcell/remote-fallback");
  });
  ...
  it("skips ensureRemoteWorkspace and returns the resolved cwd when no VM metadata is available", async () => {
    ...
    expect(result?.cwd).toBe("/srv/workcell/no-vm");
  });
});
