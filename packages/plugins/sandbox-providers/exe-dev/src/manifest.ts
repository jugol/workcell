...
const PLUGIN_ID = "workcell.exe-dev-sandbox-provider";
...
const manifest: WorkcellPluginManifestV1 = {
  ...
  description:
    "Sandbox provider plugin that provisions exe.dev VMs as Workcell execution environments.",
  author: "Workcell",
  ...
  environmentDrivers: [
    {
      ...
      description:
        "Provisions exe.dev VMs through the HTTPS API, then runs commands over direct SSH for long-lived Workcell workloads.",
      configSchema: {
        type: "object",
        properties: {
          apiKey: {
            ...
            format: "secret-ref",
            description:
              "Environment-specific exe.dev API token. Needs `/exec` permission for at least `new`, `ls`, and `rm`. Paste a token or an existing Workcell secret reference; saved environments store pasted values as company secrets. Falls back to EXE_API_KEY if omitted.",
          },
          apiUrl: {
            type: "string",
            description:
          ...
          sshPrivateKey: {
            ...
            maxLength: 4096,
            description:
              "Optional exe.dev-registered SSH private key. Paste the private key or an existing Workcell secret reference; saved environments store pasted values as company secrets. If omitted, Workcell falls back to sshIdentityFile, then the host's default SSH agent/keychain.",
          },
          sshIdentityFile: {
            type: "string",
            description:
              "Optional absolute path to the SSH private key the Workcell host should use for VM access when sshPrivateKey is omitted. Leave both blank to rely on the host's default SSH agent/keychain.",
          },
          sshPort: {
            type: "number",
