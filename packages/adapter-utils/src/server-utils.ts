...
// Windows has no POSIX process group: process.kill(pid, ...) / child.kill() only
// terminate the single spawned process, orphaning its descendants (a coding-agent
// CLI's own subprocesses, an embedded-Postgres cluster's workers). `taskkill /T /F`
// terminates the whole tree. ALWAYS force (/F): Windows offers no SIGTERM
// equivalent for console processes — taskkill WITHOUT /F cannot signal them
// ("can only be terminated forcefully") and silently kills nothing, so a
// "graceful" phase would leave the whole tree alive past shutdown/cancel
// deadlines. Pre-tree-kill behavior (child.kill => TerminateProcess) was equally
// abrupt, so force-first preserves those semantics while covering descendants.
// Fire-and-forget to mirror the synchronous child.kill() call sites; if taskkill
// itself is unavailable (sanitized PATH), fall back to the direct handle kill so
// at least the parent dies.
function killProcessTreeWindows(child: ChildProcess) {
  const pid = child.pid;
...
function signalRunningProcess(
  ...
  if (process.platform === "win32") {
    // Never signal a raw pid the OS may have RECYCLED: a stale escalation timer
    // (e.g. the timeout SIGKILL arm firing after the child already exited) must
    // be a no-op, not a taskkill /T /F of an unrelated process tree. Our own
    // ChildProcess handle knows whether the process it spawned has exited.
    if (
    ...
    void signal; // no graceful tier on win32 — see killProcessTreeWindows
    killProcessTreeWindows(running.child);
    return;
  }
...
export const DEFAULT_STDIO_CLOSE_GRACE_MS = 10_000;
const TERMINAL_RESULT_SCAN_OVERLAP_CHARS = 64 * 1024;
const DEFAULT_WORKCELL_INSTANCE_ID = "default";
const PATH_SEGMENT_RE = /^[a-zA-Z0-9_-]+$/;
const SENSITIVE_ENV_KEY = /(key|token|secret|password|passwd|authorization|cookie)/i;
const REDACTED_LOG_VALUE = "***REDACTED***";
const WORKCELL_SKILL_ROOT_RELATIVE_CANDIDATES = [
  "../../skills",
  "../../../../../skills",
];
const MATERIALIZED_SKILL_SENTINEL = ".workcell-materialized-skill.json";
...
export function resolveWorkcellInstanceRootForAdapter(input: {
  homeDir?: string;
  instanceId?: string;
  env?: NodeJS.ProcessEnv;
} = {}): string {
  const env = input.env ?? process.env;
  const homeRaw = input.homeDir?.trim() || env.WORKCELL_HOME?.trim();
  const homeDir = path.resolve(homeRaw ? expandHomePrefix(homeRaw) : path.resolve(os.homedir(), ".workcell"));
  const instanceId = input.instanceId?.trim() || env.WORKCELL_INSTANCE_ID?.trim() || DEFAULT_WORKCELL_INSTANCE_ID;
  if (!PATH_SEGMENT_RE.test(instanceId)) throw new Error(`Invalid WORKCELL_INSTANCE_ID '${instanceId}'.`);
  return path.resolve(homeDir, "instances", instanceId);
}

export const DEFAULT_WORKCELL_AGENT_PROMPT_TEMPLATE = [
  "You are agent {{agent.id}} ({{agent.name}}). Continue your Workcell work.",
  "",
  "Execution contract:",
...
export interface WorkcellSkillEntry {
  key: string;
  runtimeName: string;
  source: string;
  required?: boolean;
...
export interface MaterializedWorkcellSkillCopyResult {
  copiedFiles: number;
...
interface PersistentSkillSnapshotOptions {
  adapterType: string;
  availableEntries: WorkcellSkillEntry[];
...
> {
  if (entry.required) {
    return {
      origin: "workcell_required",
      originLabel: "Required by Workcell",
      readOnly: false,
    };
  }
  return {
    origin: "company_managed",
    originLabel: "Managed by Workcell",
    readOnly: false,
  };
}
...
type WorkcellWakeExecutionPrincipal = {
  type: "agent" | "user" | null;
  agentId: string | null;
  userId: string | null;
};

type WorkcellWakeExecutionStage = {
  wakeRole: "reviewer" | "approver" | "executor" | null;
  stageId: string | null;
  stageType: string | null;
  currentParticipant: WorkcellWakeExecutionPrincipal | null;
  returnAssignee: WorkcellWakeExecutionPrincipal | null;
  reviewRequest: {
    instructions: string;
  } | null;
...
type WorkcellWakeContinuationSummary = {
  key: string | null;
  title: string | null;
  body: string;
  bodyTruncated: boolean;
...
type WorkcellWakeLivenessContinuation = {
  attempt: number | null;
...
type WorkcellWakeChildIssueSummary = {
  id: string | null;
  identifier: string | null;
  title: string | null;
  status: string | null;
...
type WorkcellWakeBlockerSummary = {
  id: string | null;
  identifier: string | null;
  title: string | null;
  status: string | null;
...
type WorkcellWakeTreeHoldSummary = {
  holdId: string | null;
...
type WorkcellWakePayload = {
  reason: string | null;
  issue: WorkcellWakeIssue | null;
  checkedOutByHarness: boolean;
  dependencyBlockedInteraction: boolean;
  treeHoldInteraction: boolean;
  activeTreeHold: WorkcellWakeTreeHoldSummary | null;
  unresolvedBlockerIssueIds: string[];
  unresolvedBlockerSummaries: WorkcellWakeBlockerSummary[];
  executionStage: WorkcellWakeExecutionStage | null;
  continuationSummary: WorkcellWakeContinuationSummary | null;
  livenessContinuation: WorkcellWakeLivenessContinuation | null;
  interactionKind: string | null;
  interactionStatus: string | null;
  childIssueSummaries: WorkcellWakeChildIssueSummary[];
  childIssueSummaryTruncated: boolean;
  commentIds: string[];
  latestCommentId: string | null;
  comments: WorkcellWakeComment[];
...
function normalizeWorkcellWakeIssue(value: unknown): WorkcellWakeIssue | null {
...
function normalizeWorkcellWakeComment(value: unknown): WorkcellWakeComment | null {
...
function normalizeWorkcellWakeContinuationSummary(value: unknown): WorkcellWakeContinuationSummary | null {
...
function normalizeWorkcellWakeLivenessContinuation(value: unknown): WorkcellWakeLivenessContinuation | null {
...
function normalizeWorkcellWakeChildIssueSummary(value: unknown): WorkcellWakeChildIssueSummary | null {
...
function normalizeWorkcellWakeBlockerSummary(value: unknown): WorkcellWakeBlockerSummary | null {
...
function normalizeWorkcellWakeTreeHoldSummary(value: unknown): WorkcellWakeTreeHoldSummary | null {
...
function normalizeWorkcellWakeExecutionPrincipal(value: unknown): WorkcellWakeExecutionPrincipal | null {
...
function normalizeWorkcellWakeExecutionStage(value: unknown): WorkcellWakeExecutionStage | null {
  ...
  const allowedActions = Array.isArray(stage.allowedActions)
    ...
    : [];
  const currentParticipant = normalizeWorkcellWakeExecutionPrincipal(stage.currentParticipant);
  const returnAssignee = normalizeWorkcellWakeExecutionPrincipal(stage.returnAssignee);
...
export function normalizeWorkcellWakePayload(value: unknown): WorkcellWakePayload | null {
  const payload = parseObject(value);
  const comments = Array.isArray(payload.comments)
    ? payload.comments
        .map((entry) => normalizeWorkcellWakeComment(entry))
        .filter((entry): entry is WorkcellWakeComment => Boolean(entry))
    : [];
  ...
  const commentIds = Array.isArray(payload.commentIds)
    ? payload.commentIds
        ...
        .map((entry) => entry.trim())
    : [];
  const executionStage = normalizeWorkcellWakeExecutionStage(payload.executionStage);
  const continuationSummary = normalizeWorkcellWakeContinuationSummary(payload.continuationSummary);
  const livenessContinuation = normalizeWorkcellWakeLivenessContinuation(payload.livenessContinuation);
  const childIssueSummaries = Array.isArray(payload.childIssueSummaries)
    ? payload.childIssueSummaries
        .map((entry) => normalizeWorkcellWakeChildIssueSummary(entry))
        .filter((entry): entry is WorkcellWakeChildIssueSummary => Boolean(entry))
    : [];
  ...
  const unresolvedBlockerSummaries = Array.isArray(payload.unresolvedBlockerSummaries)
    ? payload.unresolvedBlockerSummaries
        .map((entry) => normalizeWorkcellWakeBlockerSummary(entry))
        .filter((entry): entry is WorkcellWakeBlockerSummary => Boolean(entry))
    : [];

  const activeTreeHold = normalizeWorkcellWakeTreeHoldSummary(payload.activeTreeHold);
  if (comments.length === 0 && commentIds.length === 0 && childIssueSummaries.length === 0 && unresolvedBlockerIssueIds.length === 0 && unresolvedBlockerSummaries.length === 0 && !activeTreeHold && !executionStage && !continuationSummary && !livenessContinuation && !normalizeWorkcellWakeIssue(payload.issue)) {
    return null;
  }

  return {
    reason: asString(payload.reason, "").trim() || null,
    issue: normalizeWorkcellWakeIssue(payload.issue),
...
export function stringifyWorkcellWakePayload(value: unknown): string | null {
  const normalized = normalizeWorkcellWakePayload(value);
  if (!normalized) return null;
  return JSON.stringify(normalized);
}

export function readWorkcellIssueWorkModeFromContext(value: unknown): string | null {
  const context = parseObject(value);
  const issue = parseObject(context.workcellIssue);
  const direct = asString(issue.workMode, "").trim();
  if (direct) return direct;
  const wake = normalizeWorkcellWakePayload(context.workcellWake);
  return wake?.issue?.workMode ?? null;
}

export function renderWorkcellWakePrompt(
  value: unknown,
  ...
  const principalLabel = (principal: WorkcellWakeExecutionPrincipal | null) => {
  ...
  const lines = resumedSession
      ? [
        "## Workcell Resume Delta",
        "",
        "You are resuming an existing Workcell session.",
...
export function buildInvocationEnvForLogs(
  ...
  if (resolvedCommand) {
    merged[options.resolvedCommandEnvKey ?? "WORKCELL_RESOLVED_COMMAND"] = redactCommandTextForLogs(resolvedCommand);
  }

  return redactEnvForLogs(merged);
}

export function buildWorkcellEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const vars: Record<string, string> = {
    WORKCELL_AGENT_ID: agent.id,
    WORKCELL_COMPANY_ID: agent.companyId,
  };
  const runtimeHost = resolveHostForUrl(
    process.env.WORKCELL_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  const runtimePort = process.env.WORKCELL_LISTEN_PORT ?? process.env.PORT ?? "3100";
  const apiUrl =
    process.env.WORKCELL_RUNTIME_API_URL ??
    process.env.WORKCELL_API_URL ??
    `http://${runtimeHost}:${runtimePort}`;
  vars.WORKCELL_API_URL = apiUrl;
  return vars;
}

export function applyWorkcellWorkspaceEnv(
  env: Record<string, string>,
  input: {
  ...
  const mappings = [
    ["WORKCELL_WORKSPACE_CWD", input.workspaceCwd],
    ["WORKCELL_WORKSPACE_SOURCE", input.workspaceSource],
    ["WORKCELL_WORKSPACE_STRATEGY", input.workspaceStrategy],
    ["WORKCELL_WORKSPACE_ID", input.workspaceId],
    ["WORKCELL_WORKSPACE_REPO_URL", input.workspaceRepoUrl],
    ["WORKCELL_WORKSPACE_REPO_REF", input.workspaceRepoRef],
    ["WORKCELL_WORKSPACE_BRANCH", input.workspaceBranch],
    ["WORKCELL_WORKSPACE_WORKTREE_PATH", input.workspaceWorktreePath],
...
export function shapeWorkcellWorkspaceEnvForExecution(input: {
  workspaceCwd?: string | null;
  workspaceWorktreePath?: string | null;
  ...
  if (executionCwd === null) {
    ...
    console.warn(
      "[workcell] shapeWorkcellWorkspaceEnvForExecution called with executionCwd=null on a remote target; " +
...
export function refreshWorkcellWorkspaceEnvForExecution(input: {
  env: Record<string, string>;
  ...
  const shapedWorkspaceEnv = shapeWorkcellWorkspaceEnvForExecution({
  ...
  delete input.env.WORKCELL_WORKSPACE_CWD;
  delete input.env.WORKCELL_WORKSPACE_WORKTREE_PATH;
  delete input.env.WORKCELL_WORKSPACES_JSON;

  applyWorkcellWorkspaceEnv(input.env, {
  ...
  if (shapedWorkspaceEnv.workspaceHints.length > 0) {
    input.env.WORKCELL_WORKSPACES_JSON = JSON.stringify(shapedWorkspaceEnv.workspaceHints);
  }
...
export function sanitizeInheritedWorkcellEnv(baseEnv: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  const env: NodeJS.ProcessEnv = { ...baseEnv };
  for (const key of Object.keys(env)) {
    if (!key.startsWith("WORKCELL_")) continue;
    if (key === "WORKCELL_RUNTIME_API_URL") continue;
    if (key === "WORKCELL_LISTEN_HOST") continue;
    if (key === "WORKCELL_LISTEN_PORT") continue;
    delete env[key];
  }
  return env;
}
...
export async function resolveWorkcellSkillsDir(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<string | null> {
  const candidates = [
    ...WORKCELL_SKILL_ROOT_RELATIVE_CANDIDATES.map((relativePath) => path.resolve(moduleDir, relativePath)),
...
export async function listWorkcellSkillEntries(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<WorkcellSkillEntry[]> {
  const root = await resolveWorkcellSkillsDir(moduleDir, additionalCandidates);
  if (!root) return [];
  ...
  try {
    ...
    return Promise.all(dirs.map(async (entry) => {
      ...
      return {
        key: `workcell/workcell/${entry.name}`,
        runtimeName: entry.name,
        source: skillDir,
        required,
        requiredReason: required
          ? "Bundled Workcell skills are always available for local adapters."
          : null,
      };
    }));
  } catch {
...
export function buildPersistentSkillSnapshot(
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    warnings.push(`Desired skill "${desiredSkill}" is not available from the Workcell skills directory.`);
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      desired: true,
      ...
      sourcePath: null,
      targetPath: null,
      detail: "Workcell cannot find this skill in the local runtime skills directory.",
...
function normalizeConfiguredWorkcellRuntimeSkills(value: unknown): WorkcellSkillEntry[] {
  if (!Array.isArray(value)) return [];
  const out: WorkcellSkillEntry[] = [];
...
export async function readWorkcellRuntimeSkillEntries(
  config: Record<string, unknown>,
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<WorkcellSkillEntry[]> {
  const configuredEntries = normalizeConfiguredWorkcellRuntimeSkills(config.workcellRuntimeSkills);
  if (configuredEntries.length > 0) return configuredEntries;
  return listWorkcellSkillEntries(moduleDir, additionalCandidates);
}

export async function readWorkcellSkillMarkdown(
  moduleDir: string,
  skillKey: string,
): Promise<string | null> {
...
export function readWorkcellSkillSyncPreference(config: Record<string, unknown>): {
  explicit: boolean;
  desiredSkills: string[];
} {
  const raw = config.workcellSkillSync;
...
function canonicalizeDesiredWorkcellSkillReference(
  reference: string,
...
export function resolveWorkcellDesiredSkillNames(
  config: Record<string, unknown>,
  availableEntries: Array<{ key: string; runtimeName?: string | null; required?: boolean }>,
): string[] {
  const preference = readWorkcellSkillSyncPreference(config);
  ...
  const desiredSkills = preference.desiredSkills
    .map((reference) => canonicalizeDesiredWorkcellSkillReference(reference, availableEntries))
    .filter(Boolean);
  return Array.from(new Set([...requiredSkills, ...desiredSkills]));
}

export function writeWorkcellSkillSyncPreference(
  config: Record<string, unknown>,
  desiredSkills: string[],
): Record<string, unknown> {
  const next = { ...config };
  const raw = next.workcellSkillSync;
  const current =
  ...
  next.workcellSkillSync = current;
  return next;
}

export async function ensureWorkcellSkillSymlink(
  source: string,
  target: string,
  ...
  while (true) {
    try {
      ...
      if (Date.now() >= deadline) {
        throw new Error(`Timed out waiting for Workcell skill materialization lock at ${lockDir}`);
      }
...
export async function materializeWorkcellSkillCopy(
  source: string,
  target: string,
): Promise<MaterializedWorkcellSkillCopyResult> {
  ...
  if (!rootStat.isDirectory()) {
    throw new Error("Workcell skills must be directories.");
  }

  const result: MaterializedWorkcellSkillCopyResult = {
    copiedFiles: 0,
    skippedSymlinks: [],
  };
...
export async function runChildProcess(
  ...
  opts: {
    ...
    remoteExecution?: RemoteExecutionSpec | null;
    /**
     * How long to wait after the child process EXITS for its stdio pipes to
     * close before finalizing with the captured output anyway. `close` only
     * fires once every inherited pipe ends — a grandchild that inherited the
     * stdout/stderr handles (e.g. an MCP server the CLI spawned) holds them
     * open after the CLI is gone, and on win32 there is no process group to
     * sweep it with, so without this bound the run hangs "running" forever.
     * New output re-arms the timer, so a descendant that is still actively
     * streaming keeps the run alive.
     */
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    const rawMerged: NodeJS.ProcessEnv = {
      ...sanitizeInheritedWorkcellEnv(process.env),
      ...opts.env,
    };

    // Strip Claude Code nesting-guard env vars so spawned `claude` processes
    // don't refuse to start with "cannot be launched inside another session".
    // These vars leak in when the Workcell server itself is started from
    // within a Claude Code session (e.g. `npx workcell run` in a terminal
    ...
    void resolveSpawnTarget(command, args, opts.cwd, mergedEnv, {
      ...
      .then((target) => {
        ...
        let settled = false;
        let exitedResult: { code: number | null; signal: NodeJS.Signals | null } | null = null;
        let orphanedStdioCloseTimer: NodeJS.Timeout | null = null;
        const stdioCloseGraceMs = Math.max(0, opts.stdioCloseGraceMs ?? DEFAULT_STDIO_CLOSE_GRACE_MS);

        const clearOrphanedStdioCloseTimer = () => {
          if (orphanedStdioCloseTimer) clearTimeout(orphanedStdioCloseTimer);
          orphanedStdioCloseTimer = null;
        };

        // Debounced: every stdio data event after exit pushes the deadline
        // back, so only a SILENT held-open pipe (leaked grandchild handle)
        // trips it — a descendant that still streams output keeps the run
        // alive, matching the no-terminal-output cleanup semantics.
        const armOrphanedStdioCloseTimer = () => {
          if (!exitedResult || settled) return;
          const exited = exitedResult;
          clearOrphanedStdioCloseTimer();
          orphanedStdioCloseTimer = setTimeout(() => {
            orphanedStdioCloseTimer = null;
            finalize(exited.code, exited.signal);
          }, stdioCloseGraceMs);
        };
        ...
        child.on("error", (err: Error) => {
          ...
          // WC-151: a spawn failure is the #1 first-run stumble — usually the
          // local CLI (claude/codex) isn't installed or signed in. Lead with the
          // actual cause + a concrete next step instead of a bare "failed" plus a
          // wall of PATH. cwd/PATH stay at the end for debugging.
          const msg =
            errno === "ENOENT"
              ? `Command "${command}" was not found — make sure it is installed and on the PATH for this run. For a local CLI agent (Claude Code / Codex), install the CLI and run it once to sign in. (cwd: "${opts.cwd}"; PATH: ${pathValue})`
              : errno === "EACCES"
                ? `Command "${command}" is not executable (permission denied) — check the adapter command's file permissions. (cwd: "${opts.cwd}")`
