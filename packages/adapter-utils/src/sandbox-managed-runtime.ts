...
export async function prepareSandboxManagedRuntime(input: {
  ...
  const runtimeRootDir = path.posix.join(workspaceRemoteDir, ".workcell-runtime", input.adapterKey);
  const baselineSnapshot = await captureDirectorySnapshot(input.workspaceLocalDir, {
    exclude: [...new Set([".workcell-runtime", ...(input.preserveAbsentOnRestore ?? []), ...(input.workspaceExclude ?? [])])],
  });

  await withTempDir("workcell-sandbox-sync-", async (tempDir) => {
    ...
    const preservedNames = new Set([".workcell-runtime", ...(input.preserveAbsentOnRestore ?? [])]);
  ...
  return {
    ...
    restoreWorkspace: async () => {
      await withTempDir("workcell-sandbox-restore-", async (tempDir) => {
