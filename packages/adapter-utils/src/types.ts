...
// "transient_upstream" → backoff-and-retry the SAME session (remote hiccup).
// "context_exhausted" → the resumed thread filled the model's context window;
// retry must start a FRESH session (the adapter sets clearSession).
export type AdapterExecutionErrorFamily = "transient_upstream" | "context_exhausted";
...
export interface AdapterEnvironmentTestContext {
   ...
   *
   * If omitted (or `kind === "local"`), the adapter tests on the Workcell
...
export interface AdapterRuntimeCommandSpec {
  /**
   * The command Workcell should execute for this adapter in the current config.
   */
  command: string;
  /**
...
export interface ServerAdapterModule {
   ...
   * rather than reading config.workcellRuntimeSkills.
   */
