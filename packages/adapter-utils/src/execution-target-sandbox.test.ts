...
describe("sandbox adapter execution targets", () => {
  ...
  it("treats SSH targets as bridge-only", () => {
    ...
    expect(adapterExecutionTargetUsesWorkcellBridge(target)).toBe(true);
    expect(adapterExecutionTargetSessionIdentity(target)).toEqual({
      transport: "ssh",
      host: "ssh.example.test",
      port: 22,
  ...
  it("starts a localhost Workcell bridge for sandbox targets in bridge mode", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "workcell-execution-target-bridge-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".workcell-runtime", "codex");
    ...
    const apiServer = createServer((req, res) => {
      requests.push({
        ...
        runId: typeof req.headers["x-workcell-run-id"] === "string" ? req.headers["x-workcell-run-id"] : null,
      });
    ...
    const bridge = await startAdapterExecutionTargetWorkcellBridge({
      runId: "run-bridge",
      target,
      runtimeRootDir,
      adapterKey: "codex",
    ...
    try {
      ...
      expect(bridge?.env.WORKCELL_API_URL).toMatch(/^http:\/\/127\.0\.0\.1:\d+$/);
      expect(bridge?.env.WORKCELL_API_KEY).not.toBe("real-run-jwt");
      expect(bridge?.env.WORKCELL_API_BRIDGE_MODE).toBe("queue_v1");

      const response = await fetch(`${bridge!.env.WORKCELL_API_URL}/api/agents/me`, {
        headers: {
          authorization: `Bearer ${bridge!.env.WORKCELL_API_KEY}`,
  ...
  it("uses the effective adapter timeout when starting the sandbox callback bridge", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "workcell-execution-target-bridge-timeout-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".workcell-runtime", "codex");
    await mkdir(runtimeRootDir, { recursive: true });
  ...
  it("fails oversized host responses with a 502 before returning them to the sandbox client", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "workcell-execution-target-bridge-limit-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".workcell-runtime", "codex");
    await mkdir(runtimeRootDir, { recursive: true });
