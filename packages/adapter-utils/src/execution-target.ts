...
export interface AdapterExecutionTargetWorkcellBridgeHandle {
  env: Record<string, string>;
  stop(): Promise<void>;
}
...
function resolveDefaultWorkcellApiUrl(): string {
  const runtimeHost = resolveHostForUrl(
    process.env.WORKCELL_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  // 3100 matches the default Workcell dev server port when the runtime does not provide one.
...
function isBridgeDebugEnabled(env: NodeJS.ProcessEnv): boolean {
  const value = env.WORKCELL_BRIDGE_DEBUG?.trim().toLowerCase();
...
export function adapterExecutionTargetUsesWorkcellBridge(
  target: AdapterExecutionTarget | null | undefined,
): boolean {
  return target?.kind === "remote";
}
  ...
  if (detectCommand) {
    ...
    if (!recheck.timedOut && recheck.exitCode === 0) {
      if (input.onLog) {
        ...
        await input.onLog(
          "stderr",
          `[workcell] Install command ${reason} (${installCommand}) but ${detectCommand} is on PATH; continuing.\n`,
        );
      }
      return;
    }
 ...
 * the directory state inside the environment, not on the Workcell host.
 *
...
export function runtimeAssetDir(
  ...
  return prepared.assetDirs[key] ?? path.posix.join(fallbackRemoteCwd, ".workcell-runtime", key);
}
...
export async function startAdapterExecutionTargetWorkcellBridge(input: {
  runId: string;
  target: AdapterExecutionTarget | null | undefined;
...
}): Promise<AdapterExecutionTargetWorkcellBridgeHandle | null> {
  if (!adapterExecutionTargetUsesWorkcellBridge(input.target)) {
    return null;
  }
  ...
  if (hostApiToken.length === 0) {
    throw new Error("Sandbox bridge mode requires a host-side Workcell API token.");
  }

  const runtimeRootDir =
    input.runtimeRootDir?.trim().length
      ? input.runtimeRootDir.trim()
      : path.posix.join(target.remoteCwd, ".workcell-runtime", input.adapterKey);
  const bridgeRuntimeDir = path.posix.join(runtimeRootDir, "workcell-bridge");
  ...
  const hostApiUrl =
    ...
    process.env.WORKCELL_RUNTIME_API_URL?.trim() ||
    process.env.WORKCELL_API_URL?.trim() ||
    resolveDefaultWorkcellApiUrl();
  ...
  await onLog(
    "stdout",
    `[workcell] Starting sandbox callback bridge for ${input.adapterKey} in ${bridgeRuntimeDir}.\n`,
  );
  ...
  try {
    ...
    // WORKCELL_BRIDGE_DEBUG opts into verbose stdout logs of every bridge
    ...
    worker = await startSandboxCallbackBridgeWorker({
      ...
      handleRequest: async (request) => {
        ...
        if (bridgeDebugEnabled) {
          await onLog(
            "stdout",
            `[workcell] Bridge proxy ${method} ${request.path}${request.query ? `?${request.query}` : ""}\n`,
          );
        }
        ...
        headers.set("x-workcell-run-id", input.runId);
        ...
        if (bridgeDebugEnabled) {
          await onLog(
            "stdout",
            `[workcell] Bridge proxy response ${response.status} for ${method} ${request.path}${request.query ? `?${request.query}` : ""}\n`,
          );
        }
        return {
  ...
  return {
    env: {
      WORKCELL_API_URL: server.baseUrl,
      WORKCELL_API_KEY: bridgeToken,
