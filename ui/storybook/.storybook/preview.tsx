...
function installStorybookApiFixtures() {
  ...
  const currentWindow = window as typeof window & {
    __workcellStorybookFetchInstalled?: boolean;
  };
  if (currentWindow.__workcellStorybookFetchInstalled) return;

  const originalFetch = window.fetch.bind(window);
  currentWindow.__workcellStorybookFetchInstalled = true;
  ...
  window.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (adapterSchemaMatch) {
      ...
      const schemas = (window as typeof window & {
        __workcellStorybookAdapterSchemas?: Record<string, unknown>;
      }).__workcellStorybookAdapterSchemas;
...
const preview: Preview = {
  ...
  globalTypes: {
    theme: {
      description: "Workcell color mode",
      defaultValue: "dark",
      toolbar: {
        title: "Theme",
        icon: "mirror",
