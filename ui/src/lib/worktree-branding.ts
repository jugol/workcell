...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("workcell-worktree-enabled") !== "true") return null;

  const name = readMetaContent("workcell-worktree-name");
  const color = normalizeHexColor(readMetaContent("workcell-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("workcell-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
