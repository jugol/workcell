export const queryKeys = {
  ...
  dashboardSettlement: (companyId: string, start: string, end: string) =>
    ["dashboard", "settlement", companyId, start, end] as const,
  ...
  pairGroups: {
    // WC-189: active pair bindings for the agent list + org chart markers.
    list: (companyId: string, status: string = "active") =>
      ["pair-groups", companyId, status] as const,
    // Standing mutually-exclusive pairs (scope=standing) for the assignee
    // picker's merged pair option.
    standing: (companyId: string) => ["pair-groups", companyId, "standing"] as const,
  },
  skills: {
