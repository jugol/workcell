...
describe("buildAgentUpdatePatch", () => {
  ...
  // WC-205 (deliberation mode, slice 2) + WC-208 (per-brain adapter)
  it("emits deliberation (incl. each brain's adapter + model) into the patch only when the overlay carries it", () => {
    // Untouched: no deliberation key in the patch.
    expect(buildAgentUpdatePatch(makeAgent(), makeOverlay())).not.toHaveProperty("deliberation");
    ...
    const patch = buildAgentUpdatePatch(
      ...
      makeOverlay({
        deliberation: {
          enabled: true,
          // WC-208: cross-adapter brains — A on claude_local, B on codex_local.
          brainA: { adapter: "claude_local", model: "anthropic/claude-a" },
          brainB: { adapter: "codex_local", model: "openai/gpt-b" },
          maxRounds: 6,
        },
      }),
    );
    expect(patch.deliberation).toEqual({
      enabled: true,
      brainA: { adapter: "claude_local", model: "anthropic/claude-a" },
      brainB: { adapter: "codex_local", model: "openai/gpt-b" },
      maxRounds: 6,
  ...
  it("passes an explicit null deliberation through (clears the config)", () => {
    const patch = buildAgentUpdatePatch(makeAgent(), makeOverlay({ deliberation: null }));
    expect(patch).toHaveProperty("deliberation", null);
  });
