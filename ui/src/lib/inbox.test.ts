...
// A board/user-owned needs_decision attention → isBoardActionRequiredIssue=true.
...
// An agent-owned block → NOT a human 할 일 (isBoardActionRequiredIssue=false).
function agentAttention(): IssueBlockedInboxAttention {
  return boardAttention({
    reason: "blocked_chain_stalled",
    owner: { type: "agent", agentId: "agent-1", userId: null, label: "QA" },
  });
}
...
describe("inbox helpers", () => {
  ...
  it("counts the same inbox sources the badge uses", () => {
    const result = computeInboxBadgeData({
      ...
      mineIssues: [makeIssue("1", true, "done")],
      dismissedAlerts: new Set<string>(),
      dismissedAtByKey: new Map<string, number>(),
      currentUserId: "user-1",
    });

    // mineIssues FIELD still counts the done issue (1), but the badge `inbox`
    // EXCLUDES done (passive completion notice): 1 approval + 2 failed + 1 join
    // + 0 blocked + 0 design = 4. (A done issue stays in the inbox list.)
  ...
  it("badge counts a board-action-required issue but NOT a DONE issue (action-required policy)", () => {
    const doneOnly = computeInboxBadgeData({
      approvals: [],
      joinRequests: [],
      dashboard,
      heartbeatRuns: [],
      mineIssues: [makeIssue("d", true, "done")],
      dismissedAlerts: new Set<string>(),
      dismissedAtByKey: new Map<string, number>(),
      currentUserId: "user-1",
    });
    // The done issue is in the list (mineIssues=1) but does NOT ping the badge.
    expect(doneOnly.mineIssues).toBe(1);
    expect(doneOnly.inbox).toBe(0);

    const boardDecision = computeInboxBadgeData({
      approvals: [],
      joinRequests: [],
      dashboard,
      heartbeatRuns: [],
      // status is in_review (not "blocked") + already read — only the
      // board-owned needs_decision attention makes it a 할 일.
      mineIssues: [makeIssue("b", false, "in_review", boardAttention())],
      dismissedAlerts: new Set<string>(),
      dismissedAtByKey: new Map<string, number>(),
      currentUserId: "user-1",
    });
    // An issue awaiting a board decision pings the badge regardless of raw
    // status / read state.
    expect(boardDecision.mineIssues).toBe(1);
    expect(boardDecision.inbox).toBe(1);
  });
  ...
  it("excludes recovery/watchdog-origin issues from the badge (system artifacts, not board actions)", () => {
    // Both carry a board-owned needs_decision attention; the ONLY differentiator
    // is the watchdog's hidden origin kind, which must exclude it from the badge.
    const watchdog = makeIssue("wd", true, "blocked", boardAttention());
    (watchdog as Issue).originKind = "stale_active_run_evaluation";
    const realBlocked = makeIssue("real", true, "blocked", boardAttention());
    const result = computeInboxBadgeData({
      approvals: [],
      joinRequests: [],
      dashboard,
      heartbeatRuns: [],
      mineIssues: [watchdog, realBlocked],
      dismissedAlerts: new Set<string>(),
      dismissedAtByKey: new Map<string, number>(),
      currentUserId: "user-1",
    });
    // Only the real blocked issue pings the badge; the watchdog eval is hidden.
    expect(result.mineIssues).toBe(1);
    expect(result.inbox).toBe(1);
  });

  it("badge counts a board-action-required issue + an unread done (field only), not routine progress", () => {
    const result = computeInboxBadgeData({
      approvals: [],
      joinRequests: [],
      dashboard,
      heartbeatRuns: [],
      mineIssues: [
        // Read done never counts toward the field.
        makeIssue("1", false, "done"),
        // Unread but no board/user decision owed — routine progress, NOT a 할 일.
        makeIssue("2", true, "todo"),
        makeIssue("3", true, "in_progress"),
        makeIssue("4", true, "in_review"),
        // Unread done: passive completion notice → field only, not the badge.
        makeIssue("5", true, "done"),
        // A board decision is owed → badge action-required (attention-based).
        makeIssue("6", true, "blocked", boardAttention()),
      ],
      dismissedAlerts: new Set<string>(),
    ...
    // mineIssues FIELD = board-action #6 + unread-done #5 = 2.
    expect(result.mineIssues).toBe(2);
    // The badge counts only the board-action one (#6); the DONE one (#5) is a
    // passive completion notice excluded from the action-required badge.
    expect(result.inbox).toBe(1);
    expect(result.alerts).toBe(2);
  });

  it("counts design 시안 awaiting board review like approvals", () => {
    const result = computeInboxBadgeData({
      approvals: [],
      joinRequests: [],
      dashboard,
      ...
      dismissedAtByKey: new Map(),
      currentUserId: "user-1",
      pendingDesignReviewIssueIds: ["issue-a", "issue-b"],
    });

    expect(result.designReviews).toBe(2);
    expect(result.inbox).toBe(2);
  });

  it("counts an issue once when it is both unread-attention and design-review (bug 6 dedup)", () => {
    const result = computeInboxBadgeData({
      approvals: [],
      joinRequests: [],
      dashboard,
      heartbeatRuns: [],
      mineIssues: [
        // Board-action-required AND awaiting design review — must count once
        // (the design-review row wins; it is excluded from the blocked tally).
        makeIssue("dup", true, "blocked", boardAttention()),
        // Unread/done with no design review — counts toward the mineIssues field.
        makeIssue("mine-only", true, "done"),
      ],
      dismissedAlerts: new Set<string>(),
      dismissedAtByKey: new Map(),
      currentUserId: "user-1",
      pendingDesignReviewIssueIds: ["dup", "review-only"],
    });

    // "dup" is excluded from mineIssues (design review wins), leaving "mine-only".
    expect(result.mineIssues).toBe(1);
    // Two distinct design-review issues: "dup" and "review-only".
    expect(result.designReviews).toBe(2);
    // Badge = 2 design reviews only. "mine-only" is DONE → excluded from the
    // action-required badge (still in the list); "dup" already counts as a
    // design review, never double-counted.
    expect(result.inbox).toBe(2);
  });

  it("badge action-required issue tally equals the 할 일 list predicate (badge↔list parity)", () => {
    // The list (Inbox.tsx todoActionIssues) renders mineIssues.filter(isBoardActionRequiredIssue);
    // the badge must count the SAME set. Mix of owners/reasons/statuses/read-state.
    const mine = [
      makeIssue("read-in-review", false, "in_review", boardAttention()),                 // IN (read, not blocked)
      makeIssue("user-decision", true, "in_progress", boardAttention({
        reason: "pending_user_decision",
        owner: { type: "user", agentId: null, userId: "user-1", label: null },
      })),                                                                                 // IN
      makeIssue("no-attention", true, "blocked"),                                          // OUT (no attention)
      makeIssue("agent-owned", true, "blocked", agentAttention()),                         // OUT (agent owner)
      makeIssue("unread-done", true, "done"),                                              // OUT of action tally (field only)
    ];

    const listCount = mine.filter(isBoardActionRequiredIssue).length;
    ...
    // No approvals/joins/failed-runs/design-reviews here, so the badge number is
    // exactly the board-action-required issue tally — proving badge ⟺ list.
    expect(badge.inbox).toBe(listCount);
  });
  ...
  it("surfaces design reviews as first-class work items (bug 3)", () => {
    const items = getInboxWorkItems({
      issues: [],
      approvals: [],
      designReviews: [
        {
          issueId: "issue-design",
          title: "PAP-7 · Landing page redesign",
          count: 2,
    ...
    const item = items[0]!;
    expect(item.kind).toBe("design_review");
    if (item.kind === "design_review") {
      expect(item.designReview.issueId).toBe("issue-design");
      expect(item.designReview.count).toBe(2);
    }
    expect(getInboxWorkItemKey(item)).toBe("design-review:issue-design");
  });
  ...
  it("defaults the remembered inbox tab to the action-required todo tab and persists all", () => {
    localStorage.clear();
    expect(loadLastInboxTab()).toBe("todo");

    saveLastInboxTab("all");
    expect(loadLastInboxTab()).toBe("all");

    saveLastInboxTab("todo");
    expect(loadLastInboxTab()).toBe("todo");
  });
  ...
  it("normalizes invalid inbox filter storage back to safe defaults", () => {
    localStorage.setItem("workcell:inbox:filters:company-1", JSON.stringify({
  ...
  it("maps legacy new-tab storage to the todo tab", () => {
    localStorage.setItem("workcell:inbox:last-tab", "new");
    expect(loadLastInboxTab()).toBe("todo");
  });
  ...
  it("groups project sections by latest issue activity while preserving non-issue sections", () => {
    const workcellIssue = makeIssue("workcell", true);
    workcellIssue.projectId = "project-1";
    ...
    const items: InboxWorkItem[] = [
      { kind: "issue", timestamp: 9, issue: workcellIssue },
    ...
    expect(groupInboxWorkItems(items, "project", {
      projectById: new Map([
        ["project-1", { name: "Workcell App" }],
        ["project-2", { name: "Onboarding" }],
      ]),
    })).toEqual([
      { key: "project:project-1", label: "Workcell App", items: [items[0]] },
  ...
  it("returns empty collapsed inbox groups for missing or invalid storage", () => {
    ...
    localStorage.setItem("workcell:inbox:collapsed-groups:company-1", JSON.stringify({ nope: true }));
    expect(loadCollapsedInboxGroupKeys("company-1")).toEqual(new Set());
  });
...
describe("design-review board filter (badge ↔ list parity)", () => {
  type A = Parameters<typeof isPendingBoardDesignReview>[0] & { issueId?: string | null };
  const artifact = (overrides: Partial<A>): A => ({
  ...
  // The exact reduction the badge (useInboxBadge) performs via the shared
  // filter — must equal the rows the list (Inbox.tsx) renders from the same input.
  function pendingIssueIds(items: A[]): Set<string> {
    const ids = new Set<string>();
    for (const a of items) {
      if (isPendingBoardDesignReview(a) && a.issueId) ids.add(a.issueId);
    }
    return ids;
  }

  it("LOR-790 regression: a child's pending 시안 stays visible even when the parent holds an approved same-screen copy", () => {
    // A design_request child re-submitted a 시안 for a screen whose earlier copy
    // was approved on the parent. The server's design gate keeps the parent held
    // until the board decides on the child's pending copy — so the review is
    // live and MUST surface. (An earlier "screen approved elsewhere" heuristic
    // hid it as stale, leaving the board unaware while the system sat blocked.)
    const items: A[] = [
      artifact({ issueId: "child-790", reviewState: "needs_board_review" }),
      artifact({ issueId: "parent-789", reviewState: "approved" }),
    ];
    expect([...pendingIssueIds(items)]).toEqual(["child-790"]);
  });

  it("counts every pending primary review — badge and list share one predicate", () => {
    const items: A[] = [
      artifact({ issueId: "a" }),
      artifact({ issueId: "b" }),
      artifact({ issueId: "c", reviewState: "approved" }),
    ];
    const ids = pendingIssueIds(items);
    expect(ids).toEqual(new Set(["a", "b"]));
  });

  it("skips non-primary, non-pending, terminal-issue, and issueId-less artifacts", () => {
    expect(isPendingBoardDesignReview(artifact({}))).toBe(true);
    expect(isPendingBoardDesignReview(artifact({ isPrimary: false }))).toBe(false);
    expect(isPendingBoardDesignReview(artifact({ reviewState: "approved" }))).toBe(false);
    expect(isPendingBoardDesignReview(artifact({ reviewState: "changes_requested" }))).toBe(false);
    expect(isPendingBoardDesignReview(artifact({ issueStatus: "done" }))).toBe(false);
    expect(isPendingBoardDesignReview(artifact({ issueId: null }))).toBe(false);
  });
});
