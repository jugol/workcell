import type { Agent, AgentDeliberationConfig } from "@workcell/shared";
...
export interface AgentConfigOverlay {
  ...
  /**
   * WC-205 (deliberation mode, slice 2): full per-agent dual-brain consensus
   * config. `undefined` = untouched; a value (including `null`) is emitted into
   * the update patch as `deliberation` so the agent update path persists it.
   */
...
export function buildAgentUpdatePatch(agent: Agent, overlay: AgentConfigOverlay) {
  ...
  // WC-205: deliberation is a top-level agent column. The overlay only carries
  // the key once the user touches the section ("deliberation" in overlay), so a
  // plain `undefined` check distinguishes "untouched" from an explicit null.
  if ("deliberation" in overlay && overlay.deliberation !== undefined) {
    patch.deliberation = overlay.deliberation;
  }
