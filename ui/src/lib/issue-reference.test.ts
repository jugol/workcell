...
describe("issue-reference", () => {
  ...
  it("does not treat GitHub issue URLs as internal Workcell issue links", () => {
    expect(parseIssuePathIdFromPath("https://github.com/workcell/workcell/issues/1778")).toBeNull();
    expect(parseIssueReferenceFromHref("https://github.com/workcell/workcell/issues/1778")).toBeNull();
  });
  ...
  it("preserves absolute Workcell issue URLs so origin, port, and hash are not lost", () => {
