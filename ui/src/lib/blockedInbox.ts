...
// The inbox "할 일" (todo) allowlist: a blocked-inbox attention belongs to the
// human action queue ONLY when it is a standing decision the BOARD or USER must
// make. This is exactly the three `needs_decision` reasons — and ONLY when the
// computed owner is board/user (the server can map a needs_decision reason to an
// agent owner, e.g. missing_successful_run_disposition owned by an agent, which
// must NOT surface as a human 할 일). Repair/blocker/recovery/external reasons
// (in_review_without_action_path, invalid_review_participant, blocked_by_*,
// blocked_chain_stalled, open_recovery_issue, external_owner_action) live on the
// Blocked/Attention surface, never here.
//
// Independent of issue.status (the server computes attention for in_review /
// in_progress / todo, not just literal "blocked") and independent of
// isUnreadForMe (a pending decision is a standing action, not a one-time unread
// notification). Used as the SINGLE source of truth by BOTH the todo list
// (Inbox.tsx) and the inbox badge tally (inbox.ts) so they can never diverge.
export const BOARD_ACTIONABLE_BLOCKED_REASONS: ReadonlySet<IssueBlockedInboxReason> = new Set([
  "pending_board_decision",
  "pending_user_decision",
  "missing_successful_run_disposition",
]);

export function isBoardActionRequiredIssue(issue: Issue): boolean {
  const attention = issue.blockedInboxAttention;
  if (!attention) return false;
  if (attention.owner.type !== "board" && attention.owner.type !== "user") return false;
  return BOARD_ACTIONABLE_BLOCKED_REASONS.has(attention.reason);
}
