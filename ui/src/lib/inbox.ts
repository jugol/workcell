...
import { isBoardActionRequiredIssue } from "./blockedInbox";

export const RECENT_ISSUES_LIMIT = 100;
export const FAILED_RUN_STATUSES = new Set(["failed", "timed_out"]);
export const ACTIONABLE_APPROVAL_STATUSES = new Set(["pending", "revision_requested"]);
export const DISMISSED_KEY = "workcell:inbox:dismissed";
export const READ_ITEMS_KEY = "workcell:inbox:read-items";
export const INBOX_LAST_TAB_KEY = "workcell:inbox:last-tab";
export const INBOX_ISSUE_COLUMNS_KEY = "workcell:inbox:issue-columns";
export const INBOX_NESTING_KEY = "workcell:inbox:nesting";
export const INBOX_GROUP_BY_KEY = "workcell:inbox:group-by";
export const INBOX_FILTER_PREFERENCES_KEY_PREFIX = "workcell:inbox:filters";
export const INBOX_COLLAPSED_GROUPS_KEY_PREFIX = "workcell:inbox:collapsed-groups";
// "todo" (할 일) = action-required ONLY: exactly the items the badge counts
// (actionable approvals, design 시안 reviews, join requests, blocked issues you
// must decide, failed runs). It is the default tab so the board lands on
// "what do I have to act on right now" instead of a broad touched-issue list.
export type InboxTab = "todo" | "mine" | "recent" | "unread" | "blocked" | "all";
...
// A design 시안 awaiting board review, surfaced as an inbox work item so the
// badge count has a corresponding actionable row. Always tied to an issue
// (issueId is required) so clicking opens the issue where IssueDesignReviewPanel
// lets the board approve/request-changes — that is how the count is cleared.
export interface InboxDesignReview {
  issueId: string;
  title: string;
  /** Number of pending 시안 for this issue (one inbox row per issue). */
  count: number;
...
export interface InboxBadgeData {
  ...
  joinRequests: number;
  mineIssues: number;
  // Issues whose source-of-truth design (시안) awaits board review — a standing
  // human decision, counted like approvals until decided.
  designReviews: number;
  alerts: number;
}

// The inbox badge is an ATTENTION signal, not an activity feed. An unread
// touched issue only counts when it reached a state that needs the human:
// done (your issue finished — confirm it) or blocked (a decision is owed).
// Routine agent progress comments on running issues no longer ping the badge —
// that noise drowned out the few items (design approvals, finished issues)
// the board actually had to act on.
export const ATTENTION_UNREAD_ISSUE_STATUSES = new Set(["done", "blocked"]);

// System/recovery artifacts the board should never see in its inbox — they are
// the watchdog's own bookkeeping ("Review silent active run", stranded-issue
// recovery, productivity/liveness escalations), not anything a human presses a
// button on. They are hidden from BOTH the badge count and the inbox list, so a
// stuck recovery eval can't pin the badge open or clutter the list. (Real
// system-spawned WORK — compound follow-ups, design requests — is NOT hidden.)
export const INBOX_HIDDEN_ORIGIN_KINDS = new Set<string>([
...
export function isInboxHiddenIssue(issue: { originKind?: string | null }): boolean {
  return !!issue.originKind && INBOX_HIDDEN_ORIGIN_KINDS.has(issue.originKind);
}

// A design 시안 review is moot once its issue is terminal (done/cancelled/
// archived) — the work is finished or abandoned, so the board never needs to
// decide on its design. Such reviews must not sit in the "할 일" tab or drive the
// badge, even if a stale needs_board_review artifact survived on the issue.
export const DESIGN_REVIEW_TERMINAL_ISSUE_STATUSES = new Set<string>(["done", "cancelled", "archived"]);

export function isTerminalDesignReviewIssueStatus(status: string | null | undefined): boolean {
  return !!status && DESIGN_REVIEW_TERMINAL_ISSUE_STATUSES.has(status);
}

// The design-artifact fields the board-review filter reads. Kept structural so
// both the rendered inbox list (Inbox.tsx) and the badge counter (useInboxBadge)
// share ONE definition of "this 시안 is a pending board action" — before, the
// badge omitted the approved-elsewhere skip and over-counted (badge=3 vs list=1).
export interface PendingDesignReviewArtifact {
...
// The single source of truth for "this design artifact is a pending board
// review the inbox must surface AND the badge must count."
//
// Deliberately NO "screen already approved elsewhere" suppression. A pending
// primary 시안 on a design_request child is ALWAYS a live board action even when
// the parent carries an approved same-screen copy: the server's design gate
// unions the child's pending 시안 into the parent's gate, so the parent stays
// held until the board decides on the child (approve → the child completes and
// the parent resumes; request changes → the designer revises). An earlier
// heuristic hid such rows as "stale" and made a genuinely-required review
// invisible (LOR-790) while the system sat blocked waiting for it. The
// "To-do-with-no-button" problem that heuristic tried to paper over is fixed at
// the source instead: the child issue page now renders the approve /
// request-changes controls for its own pending 시안.
export function isPendingBoardDesignReview(artifact: PendingDesignReviewArtifact): boolean {
  // Only a primary (source-of-truth) 시안 awaiting board review is a board action.
  if (artifact.reviewState !== "needs_board_review") return false;
  if (!artifact.isPrimary) return false;
  // A terminal issue's design review is moot (done/cancelled/archived).
  if (isTerminalDesignReviewIssueStatus(artifact.issueStatus)) return false;
  // Only an issue-linked artifact can render as an inbox row / be counted.
  if (!artifact.issueId) return false;
  return true;
}
...
export function loadLastInboxTab(): InboxTab {
  try {
    ...
    if (raw === "new") return "todo";
    // Default to the action-required view.
    return "todo";
  } catch {
    return "todo";
  }
...
export function getApprovalsForTab(
  ...
  // todo + unread: only approvals that still need a decision.
  if (tab === "unread" || tab === "todo") {
...
export function getInboxWorkItems({
  ...
  designReviews?: InboxDesignReview[];
}): InboxWorkItem[] {
  return [
    ...issues.map((issue) => ({
      kind: "issue" as const,
    ...
    ...designReviews.map((designReview) => ({
      ...
      timestamp: normalizeTimestamp(designReview.updatedAt),
      designReview,
    })),
  ].sort((a, b) => {
...
export function getInboxWorkItemKey(item: InboxWorkItem): string {
  ...
  if (item.kind === "design_review") return `design-review:${item.designReview.issueId}`;
...
export function computeInboxBadgeData({
  ...
  dismissedAtByKey: ReadonlyMap<string, number>;
  currentUserId?: string | null;
  // Ids of issues with a design awaiting board review (needs_board_review).
  // Passed as the full set (not a count) so a design-review issue that is also
  // an unread-attention issue is only counted once (see dedup below, bug 6).
  pendingDesignReviewIssueIds?: Iterable<string>;
}): InboxBadgeData {
  const designReviewIssueIds = new Set(pendingDesignReviewIssueIds);
  ...
  // Dedup (bug 6): an issue that is both unread-attention (done/blocked) AND has
  // a 시안 awaiting board review would otherwise count twice for one issue.
  // The design-review row is the more actionable surface, so it wins — exclude
  // such issues from the mine-issue tally.
  //
  // Badge scope (user policy): the inbox BADGE pings only for things that need
  // the board to ACT — items you go in and press a button on (approvals,
  // design reviews, join requests, + an issue awaiting a board/user DECISION)
  // and FAILED runs (you may need to fix something). A DONE issue is a passive
  // "your issue finished" notification, not an action — it stays in the inbox
  // LIST but does NOT drive the badge. So the action-required tally and done are
  // split here.
  //
  // This MUST use the SAME isBoardActionRequiredIssue predicate the todo LIST
  // uses (Inbox.tsx todoActionIssues) so badge N ⟺ N rendered 할 일 rows. The
  // design-review dedup + hidden guard stay at THIS call site (the badge's
  // mineIssues is not pre-hidden-filtered, unlike the list's). Independent of
  // issue.status / isUnreadForMe — a standing board decision counts even after
  // it has been read.
  const blockedMineIssues = mineIssues.filter(
    (issue) =>
      isBoardActionRequiredIssue(issue) &&
      !designReviewIssueIds.has(issue.id) &&
      !isInboxHiddenIssue(issue),
  ).length;
  const doneMineIssues = mineIssues.filter(
    (issue) =>
      issue.isUnreadForMe &&
      issue.status === "done" &&
      !designReviewIssueIds.has(issue.id) &&
  ...
  // Kept as the full attention tally (done + blocked) for any list/consumer that
  // wants it; the badge sum below intentionally uses blocked-only.
  const visibleMineIssues = blockedMineIssues + doneMineIssues;
  const designReviews = designReviewIssueIds.size;
  ...
  return {
    // The inbox badge is an ACTION-REQUIRED signal (user policy): it counts only
    // items the board must act on — approvals, design reviews, join requests
    // (button-press decisions), a BLOCKED issue (go in and unblock), and FAILED
    // runs (may need a fix). A DONE issue is a passive completion notice and is
    // deliberately EXCLUDED from the badge (doneMineIssues), though it still
    // appears in the inbox list. Distinct from company-wide health `alerts`.
    inbox: actionableApprovals + visibleJoinRequests + failedRuns + blockedMineIssues + designReviews,
