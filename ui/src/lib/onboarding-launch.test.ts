...
describe("onboarding launch payloads", () => {
  ...
  it("uses the provided project name when given", () => {
    expect(buildOnboardingProjectPayload("goal-1", "  Acme Corp  ")).toEqual({
      name: "Acme Corp",
      status: "in_progress",
      goalIds: ["goal-1"],
    });
  ...
  it("falls back to the onboarding project name when no name is provided", () => {
    expect(buildOnboardingProjectPayload("goal-1", "   ")).toEqual({
      name: "Onboarding",
      status: "in_progress",
      goalIds: ["goal-1"],
    });
    expect(buildOnboardingProjectPayload(null, undefined)).toEqual({
      name: "Onboarding",
      status: "in_progress",
    });
  });
