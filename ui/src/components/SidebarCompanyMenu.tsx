    ...
    >
      ...
      {isEditing ? (
        <button
          ...
          aria-label={t("sidebarCompanyMenu.reorder", { defaultValue: "Reorder {{name}}", name: company.name })}
    ...
    <DropdownMenu open={open} onOpenChange={handleOpenChange}>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          aria-label={selectedCompany ? t("sidebarCompanyMenu.trigger.withCompany", { defaultValue: "Open {{name}} workspace switcher", name: selectedCompany.name }) : t("sidebarCompanyMenu.trigger.default", { defaultValue: "Open workspace switcher" })}
        >
          <span className="flex min-w-0 flex-1 items-center gap-2">
            {selectedCompany ? <WorkspaceIcon company={selectedCompany} /> : null}
            <span className="truncate text-sm font-bold text-foreground">
              {selectedCompany?.name ?? t("sidebarCompanyMenu.selectWorkspace", { defaultValue: "Select workspace" })}
            </span>
          </span>
      ...
      <DropdownMenuContent align="start" sideOffset={8} className="w-64 p-1">
        <div className="flex items-center justify-between gap-2 px-2 py-1.5">
          <DropdownMenuLabel className="p-0 text-[11px] font-semibold uppercase text-muted-foreground">
            {t("sidebarCompanyMenu.switchWorkspace", { defaultValue: "Switch workspace" })}
          </DropdownMenuLabel>
          <button
            type="button"
            onClick={(event) => {
          ...
          >
            {isEditingOrder ? t("sidebarCompanyMenu.editOrder.done", { defaultValue: "Done" }) : t("sidebarCompanyMenu.editOrder.edit", { defaultValue: "Edit" })}
          </button>
        </div>
        <div className="max-h-96 overflow-y-auto">
          ...
          {orderedCompanies.length === 0 ? (
            <DropdownMenuItem disabled>{t("sidebarCompanyMenu.noWorkspaces", { defaultValue: "No workspaces" })}</DropdownMenuItem>
          ) : null}
        </div>
        <DropdownMenuSeparator />
        <DropdownMenuItem
        ...
        >
          ...
          <span>{t("sidebarCompanyMenu.addCompany", { defaultValue: "Add team..." })}</span>
        </DropdownMenuItem>
        <DropdownMenuSeparator />
        <DropdownMenuItem asChild disabled={isEditingOrder}>
          ...
          >
            ...
            <span className="truncate">
              {selectedCompany ? t("sidebarCompanyMenu.invitePeople.withCompany", { defaultValue: "Invite people to {{name}}", name: selectedCompany.name }) : t("sidebarCompanyMenu.invitePeople.default", { defaultValue: "Invite people" })}
            </span>
          </Link>
        </DropdownMenuItem>
        <DropdownMenuItem asChild disabled={isEditingOrder}>
          ...
          >
            ...
            <span>{t("sidebarCompanyMenu.companySettings", { defaultValue: "Team settings" })}</span>
          </Link>
        </DropdownMenuItem>
        {session?.session ? (
          <>
            ...
            >
              ...
              <span>{signOutMutation.isPending ? t("sidebarCompanyMenu.signOut.pending", { defaultValue: "Signing out..." }) : t("sidebarCompanyMenu.signOut.label", { defaultValue: "Sign out" })}</span>
            </DropdownMenuItem>
          </>
        ) : null}
      </DropdownMenuContent>
