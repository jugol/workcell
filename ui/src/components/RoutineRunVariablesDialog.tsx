    ...
    <Dialog open={open} onOpenChange={(next) => !isPending && onOpenChange(next)}>
      <DialogContent className="flex h-[calc(100dvh-2rem)] max-h-[calc(100dvh-2rem)] max-w-xl flex-col gap-0 overflow-hidden p-0 sm:h-auto sm:max-h-[min(calc(100dvh-2rem),42rem)]">
        <DialogHeader className="shrink-0 border-b border-border/60 px-6 pb-4 pr-12 pt-6">
          ...
          <DialogDescription>
            {t("routineRunVariablesDialog.description", { defaultValue: "Choose the agent and optional project for this one run. Routine defaults are prefilled and won't be changed." })}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain px-6 py-4">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-1.5">
              <Label className="text-xs">{t("routineRunVariablesDialog.agentLabel", { defaultValue: "Agent *" })}</Label>
              <InlineEntitySelector
                value={selection.assigneeAgentId}
                options={assigneeOptions}
                recentOptionIds={recentAssigneeIds}
                placeholder={t("routineRunVariablesDialog.agentPlaceholder", { defaultValue: "Agent" })}
                noneLabel={t("routineRunVariablesDialog.agentNoneLabel", { defaultValue: "Select an agent" })}
                searchPlaceholder={t("routineRunVariablesDialog.agentSearchPlaceholder", { defaultValue: "Search agents..." })}
                emptyMessage={t("routineRunVariablesDialog.agentEmptyMessage", { defaultValue: "No agents found." })}
                disablePortal
                openOnFocus={false}
                ...
                renderTriggerValue={(option) =>
                  option ? (
                    ...
                    <span className="text-muted-foreground">{t("routineRunVariablesDialog.agentNoneLabel", { defaultValue: "Select an agent" })}</span>
                  )
                }
                renderOption={(option) => {
            ...
            <div className="space-y-1.5">
              <Label className="text-xs">{t("routineRunVariablesDialog.projectLabel", { defaultValue: "Project" })}</Label>
              <InlineEntitySelector
                value={selection.projectId}
                options={projectOptions}
                recentOptionIds={recentProjectIds}
                placeholder={t("routineRunVariablesDialog.projectPlaceholder", { defaultValue: "Project" })}
                noneLabel={t("routineRunVariablesDialog.projectNoneLabel", { defaultValue: "No project" })}
                searchPlaceholder={t("routineRunVariablesDialog.projectSearchPlaceholder", { defaultValue: "Search projects..." })}
                emptyMessage={t("routineRunVariablesDialog.projectEmptyMessage", { defaultValue: "No projects found." })}
                disablePortal
                openOnFocus={false}
                onChange={(projectId) => {
                ...
                renderTriggerValue={(option) =>
                  option && selectedProject ? (
                    ...
                    <span className="text-muted-foreground">{t("routineRunVariablesDialog.projectNoneLabel", { defaultValue: "No project" })}</span>
                  )
                }
                renderOption={(option) => {
                  if (!option.id) return <span className="truncate">{option.label}</span>;
              ...
              {isAutoWorkspaceBranchVariable(variable) ? (
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="true">{t("routineRunVariablesDialog.booleanTrue", { defaultValue: "True" })}</SelectItem>
                    <SelectItem value="false">{t("routineRunVariablesDialog.booleanFalse", { defaultValue: "False" })}</SelectItem>
                  </SelectContent>
                </Select>
                ...
                >
                  <SelectTrigger>
                    <SelectValue placeholder={t("routineRunVariablesDialog.chooseValuePlaceholder", { defaultValue: "Choose a value" })} />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="__unset__">{t("routineRunVariablesDialog.noValue", { defaultValue: "No value" })}</SelectItem>
        ...
        >
          {!selection.assigneeAgentId ? (
            ...
            <p className="mr-auto text-xs text-amber-600">
              {t("routineRunVariablesDialog.missingPrefix", { defaultValue: "Missing:" })} {missingRequired.join(", ")}
            </p>
          ) : workspaceSelectionEnabled && !workspaceConfigValid ? (
            <p className="mr-auto text-xs text-amber-600">
              {t("routineRunVariablesDialog.chooseExistingWorkspace", { defaultValue: "Choose an existing workspace before running." })}
            </p>
          ) : (
            <span className="mr-auto" />
          )}
          <Button variant="ghost" onClick={() => onOpenChange(false)} disabled={isPending}>
            {t("routineRunVariablesDialog.cancel", { defaultValue: "Cancel" })}
          </Button>
          <Button
            onClick={() => {
          ...
          >
            {isPending ? t("routineRunVariablesDialog.running", { defaultValue: "Running..." }) : t("routineRunVariablesDialog.runRoutine", { defaultValue: "Run routine" })}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
