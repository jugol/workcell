import type { FinanceByKind } from "@workcell/shared";
            ...
            >
              <div className="min-w-0">
                ...
                <div className="text-xs text-muted-foreground">
                  {t("financeKindCard.counts", {
                    defaultValue: "{{events}} · {{billers}}",
                    events: row.eventCount === 1
                      ? t("financeKindCard.eventCount_one", { defaultValue: "{{count}} event", count: row.eventCount })
                      : t("financeKindCard.eventCount_other", { defaultValue: "{{count}} events", count: row.eventCount }),
                    billers: row.billerCount === 1
                      ? t("financeKindCard.billerCount_one", { defaultValue: "{{count}} biller", count: row.billerCount })
                      : t("financeKindCard.billerCount_other", { defaultValue: "{{count}} billers", count: row.billerCount }),
                  })}
                </div>
              </div>
