...
export function FileTree({
  ...
  const resolvedAriaLabel = ariaLabel ?? t("fileTree.ariaLabel", { defaultValue: "Files" });
    ...
    return (
      <div aria-label={resolvedAriaLabel} role="tree" className="p-3">
        <div className="rounded-md border border-dashed border-border px-4 py-8 text-center">
          <div className="text-sm font-medium">{empty?.title ?? t("fileTree.empty.title", { defaultValue: "No files" })}</div>
          <div className="mt-1 text-xs text-muted-foreground">
            {empty?.description ?? t("fileTree.empty.description", { defaultValue: "Files will appear here when they are available." })}
          </div>
        </div>
      </div>
    );
          ...
          >
            ...
            {node.kind === "dir" && (
              <button
                ...
                aria-label={expanded
                  ? t("fileTree.collapse", { defaultValue: "Collapse {{name}}", name: node.name })
                  : t("fileTree.expand", { defaultValue: "Expand {{name}}", name: node.name })}
              >
                {expanded ? (
