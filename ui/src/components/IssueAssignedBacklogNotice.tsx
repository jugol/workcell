...
export function IssueAssignedBacklogNotice({
  ...
  const assigneeLabel = assigneeAgent?.name ?? t("issueAssignedBacklogNotice.assigneeFallback", { defaultValue: "the assignee" });
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1 space-y-1.5">
          <p className="leading-5">
            ...
            <span className="font-medium">{assigneeLabel}</span>{t("issueAssignedBacklogNotice.notWokenPrefix", { defaultValue: " will not be woken until status changes to " })}
            <code className="rounded bg-amber-100 px-1 py-0.5 text-[12px] dark:bg-amber-400/15">todo</code>{t("issueAssignedBacklogNotice.or", { defaultValue: " or " })}
            <code className="rounded bg-amber-100 px-1 py-0.5 text-[12px] dark:bg-amber-400/15">in_progress</code>.
          </p>
          {assigneeAgent ? (
            <p className="text-xs leading-5 text-amber-800 dark:text-amber-200">
              {t("issueAssignedBacklogNotice.commentsHint", {
