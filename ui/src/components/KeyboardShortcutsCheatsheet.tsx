    ...
    <>
                  ...
                  <div className="flex items-center gap-1">
                    {shortcut.keys.map((key, i) => (
                      <span key={key} className="flex items-center gap-1">
                        {i > 0 && <span className="text-xs text-muted-foreground">{t("keyboardShortcuts.then", { defaultValue: "then" })}</span>}
                        <KeyCap>{key}</KeyCap>
                      </span>
                    ))}
                  </div>
      ...
      <div className="border-t border-border px-5 py-3">
        <p className="text-xs text-muted-foreground">
          {t("keyboardShortcuts.pressBefore", { defaultValue: "Press" })} <KeyCap>Esc</KeyCap> {t("keyboardShortcuts.pressAfter", { defaultValue: "to close · Shortcuts are disabled in text fields" })}
        </p>
      </div>
    </>
  );
