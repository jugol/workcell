...
import type { Approval, Agent } from "@workcell/shared";
    ...
    <div className="rounded-xl border border-border/70 bg-card p-4 shadow-sm">
            ...
            <div className="min-w-0 flex-1 space-y-2">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs leading-5 text-muted-foreground">
                  {t("approvalCard.requestCreated", { defaultValue: "Approval request created {{time}}", time: timeAgo(approval.createdAt) })}
                </p>
              </div>
            </div>
          </div>
      ...
      {approval.decisionNote && (
        <div className="mt-4 rounded-lg border border-border/60 bg-muted/30 px-3.5 py-3 text-xs leading-5 text-muted-foreground">
          <span className="font-medium text-foreground">{t("approvalCard.decisionNote", { defaultValue: "Decision note." })}</span> {approval.decisionNote}
        </div>
      )}
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-border/60 pt-4">
            ...
            {showResolutionButtons && (
              <>
                <Button
                  ...
                  disabled={isPending}
                >
                  {pendingAction === "approve" ? t("approvalCard.approving", { defaultValue: "Approving..." }) : t("approvalCard.approve", { defaultValue: "Approve" })}
                </Button>
                <Button
                  variant="destructive"
                  size="sm"
                  ...
                  disabled={isPending}
                >
                  {pendingAction === "reject" ? t("approvalCard.rejecting", { defaultValue: "Rejecting..." }) : t("approvalCard.reject", { defaultValue: "Reject" })}
                </Button>
              </>
            )}
          </div>
          {(detailLink || onOpen) ? (
            detailLink ? (
              ...
              >
                {t("approvalCard.viewDetails", { defaultValue: "View details" })}
              </Link>
            ) : (
