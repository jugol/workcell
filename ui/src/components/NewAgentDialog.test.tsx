...
describe("NewAgentDialog", () => {
  ...
  beforeEach(() => {
    ...
    createCompanyInviteMock.mockResolvedValue({
      id: "invite-1",
      token: "agent-token",
      inviteUrl: "https://workcell.local/invite/agent-token",
      expiresAt: "2026-04-20T00:00:00.000Z",
      allowedJoinTypes: "agent",
      humanRole: null,
      onboardingTextUrl: "https://workcell.local/api/invites/agent-token/onboarding.txt",
      onboardingTextPath: "/api/invites/agent-token/onboarding.txt",
    });
    getInviteOnboardingMock.mockResolvedValue({
      onboarding: {
        connectivity: {
          connectionCandidates: ["https://workcell.local"],
          testResolutionEndpoint: {
            url: "https://workcell.local/api/invites/agent-token/test-resolution",
          },
        },
      },
    });
  ...
  it("generates an external agent onboarding prompt inside the add-agent modal", async () => {
    ...
    expect(clipboardWriteTextMock).toHaveBeenCalledWith(
      expect.stringContaining("You're invited to join a Workcell company as an agent."),
    );
  ...
  it("hides the Ask-the-Orchestrator option when the company has no orchestrator", async () => {
    listAgentsMock.mockResolvedValue([{ id: "agent-pm", role: "pm" }]);
    const root = createRoot(container);
    const queryClient = new QueryClient({
      defaultOptions: { queries: { retry: false } },
    });
    ...
    // The dead-end "ask the orchestrator" path is hidden — there is no
    // orchestrator to action the resulting issue. Direct creation leads instead.
    expect(container.textContent).not.toContain("Ask the Orchestrator to create a new agent");
    expect(container.textContent).toContain("There's no Orchestrator yet to delegate to");
    expect(container.textContent).toContain("Configure a runtime manually");
  ...
  // WC-185: pair must be discoverable from the add-agent entry point and route
  // into the existing pair-create path (new issue with the Pair toggle on),
  // rather than introducing a fake "pair agent type".
  it("offers a reachable Pair affordance that routes into pair setup", async () => {
    const root = createRoot(container);
    const queryClient = new QueryClient({
      defaultOptions: { queries: { retry: false } },
    });
    ...
    const pairButton = container.querySelector('[data-testid="new-agent-pair-option"]');
    expect(pairButton).not.toBeNull();
    expect(pairButton?.textContent).toContain("Set up a pair (two agents)");

    await act(async () => {
      pairButton!.dispatchEvent(new MouseEvent("click", { bubbles: true }));
    });

    expect(closeNewAgentMock).toHaveBeenCalledTimes(1);
    expect(openNewIssueMock).toHaveBeenCalledWith({ pairAtCreate: true });
  ...
  it("shows the Ask-the-Orchestrator option once an orchestrator exists", async () => {
    listAgentsMock.mockResolvedValue([{ id: "agent-orch", role: "orchestrator" }]);
    const root = createRoot(container);
    const queryClient = new QueryClient({
      defaultOptions: { queries: { retry: false } },
    });
    ...
    expect(container.textContent).toContain("Ask the Orchestrator to create a new agent");
    expect(container.textContent).not.toContain("There's no Orchestrator yet to delegate to");
