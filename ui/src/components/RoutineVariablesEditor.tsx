...
import { syncRoutineVariablesWithTemplate, type RoutineVariable } from "@workcell/shared";
  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen} className="overflow-hidden rounded-lg border border-border/70">
      <CollapsibleTrigger className="flex w-full items-center justify-between px-3 py-2 text-left">
        <div>
          <p className="text-sm font-medium">{t("routineVariables.variables", { defaultValue: "Variables" })}</p>
          <p className="text-xs text-muted-foreground">
            {t("routineVariables.detectedBefore", { defaultValue: "Detected from" })} `{"{{name}}"}` {t("routineVariables.detectedAfter", { defaultValue: "placeholders in the routine title and instructions." })}
          </p>
        </div>
          ...
          <div key={variable.name} className="p-4">
            <div className="mb-3 flex flex-wrap items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">
                {t("routineVariables.promptUser", { defaultValue: "Prompt the user for this value before each manual run." })}
              </span>
            </div>

            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-1.5">
                <Label className="text-xs">{t("routineVariables.label", { defaultValue: "Label" })}</Label>
                <Input
              ...
              <div className="space-y-1.5">
                <Label className="text-xs">{t("routineVariables.type", { defaultValue: "Type" })}</Label>
                <Select
                  value={variable.type}
              ...
              <div className="space-y-1.5 md:col-span-2">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <label className="flex items-center gap-2 text-xs text-muted-foreground">
                    ...
                    />
                    {t("routineVariables.required", { defaultValue: "Required" })}
                  </label>
                </div>
                ...
                {variable.type === "textarea" ? (
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="true">{t("routineVariables.true", { defaultValue: "True" })}</SelectItem>
                      <SelectItem value="false">{t("routineVariables.false", { defaultValue: "False" })}</SelectItem>
                    </SelectContent>
                  </Select>
  ...
  return (
    <>
      <div className="flex items-center justify-between gap-2 rounded-lg border border-dashed border-border/70 px-3 py-2 text-xs text-muted-foreground">
        <span>
          {t("routineVariables.hintBefore", { defaultValue: "Use" })} `{"{{variable_name}}"}` {t("routineVariables.hintAfter", { defaultValue: "placeholders in the instructions to prompt for inputs when the routine runs." })}
        </span>
        <button
          type="button"
      ...
      <Dialog open={helpOpen} onOpenChange={setHelpOpen}>
        <DialogContent className="sm:max-w-xl">
          <DialogHeader>
            <DialogTitle>{t("routineVariables.dialogTitle", { defaultValue: "Routine variables" })}</DialogTitle>
            <DialogDescription>
              {t("routineVariables.dialogDescription", { defaultValue: "How to prompt for inputs and which variables Workcell fills in automatically." })}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-5 text-sm">
            <section className="space-y-2">
              ...
              <p className="text-muted-foreground">
                {t("routineVariables.typeBefore", { defaultValue: "Type" })}{" "}
                <code className="rounded bg-muted px-1 py-0.5 font-mono text-xs text-foreground">
                  {"{{variable_name}}"}
                </code>{" "}
                {t("routineVariables.typeMiddle", { defaultValue: "anywhere in the title or instructions. Workcell detects each placeholder, lists it under" })} <span className="font-medium text-foreground">{t("routineVariables.variables", { defaultValue: "Variables" })}</span>{t("routineVariables.typeAfter", { defaultValue: ", and prompts for a value before each run." })}
              </p>
              <ul className="list-disc space-y-1 pl-5 text-muted-foreground">
                <li>{t("routineVariables.ruleNames", { defaultValue: "Names must start with a letter and may use letters, numbers, and underscores." })}</li>
                <li>{t("routineVariables.rulePick", { defaultValue: "Pick a type (text, textarea, number, boolean, select), default value, and whether it is required." })}</li>
                <li>{t("routineVariables.ruleReuse", { defaultValue: "The same name reused across the title and instructions is treated as one variable." })}</li>
              </ul>
            </section>
            ...
            <section className="space-y-2">
              ...
              <p className="text-muted-foreground">
                {t("routineVariables.builtinDescription", { defaultValue: "These are filled in automatically — no setup needed and they will not appear in the Variables list." })}
              </p>
              <div className="overflow-hidden rounded-lg border border-border/70">
                <table className="w-full text-left text-xs">
                  <thead className="bg-muted/40 text-muted-foreground">
                    <tr>
                      <th className="px-3 py-2 font-medium">{t("routineVariables.colPlaceholder", { defaultValue: "Placeholder" })}</th>
                      <th className="px-3 py-2 font-medium">{t("routineVariables.colExample", { defaultValue: "Example" })}</th>
                      <th className="px-3 py-2 font-medium">{t("routineVariables.colDescription", { defaultValue: "Description" })}</th>
                    </tr>
                  </thead>
