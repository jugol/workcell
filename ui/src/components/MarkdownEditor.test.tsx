...
import { buildProjectMentionHref, buildRoutineMentionHref, buildSkillMentionHref } from "@workcell/shared";
import {
...
describe("MarkdownEditor", () => {
  ...
  it("keeps mention queries active across spaces", () => {
    expect(findMentionMatch("Ping @Workcell App", "Ping @Workcell App".length)).toEqual({
      trigger: "mention",
      marker: "@",
      query: "Workcell App",
      atPos: 5,
      endPos: "Ping @Workcell App".length,
    });
  });
  ...
  async function openMentionMenuFor(
    ...
    range.setStart(textNode!, "@Wor".length);
    range.collapse(true);
    ...
    const option = Array.from(document.body.querySelectorAll('button[type="button"]'))
      .find((node) => node.textContent?.includes("Workcell App")) as HTMLButtonElement | undefined;
  ...
  it("accepts mention selection from a touch tap", async () => {
    ...
    expect(handleChange).toHaveBeenCalledWith(
      `[@Workcell App](${buildProjectMentionHref("project-123", "#336699")}) `,
    );
  ...
  it("marks the autocomplete portal as floating UI for modal pointer handling", async () => {
    ...
    const menu = option.closest("[data-workcell-floating-ui]");
    expect(menu).toBeTruthy();
