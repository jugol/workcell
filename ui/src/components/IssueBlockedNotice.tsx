...
function BlockerRecoveryIndicator({ action }: { action: IssueRecoveryAction }) {
  ...
  const label = t(`issueBlockedNotice.recoveryLabel.${state}`, { defaultValue: tone.label });
  return (
    <span
      ...
      role="status"
      aria-label={label}
      title={t("issueBlockedNotice.recoveryTitle", {
...
function SuccessfulRunRetryNowControl({
  ...
  const scheduleLabel = relative === "now"
    ? t("issueBlockedNotice.scheduleDueNow", { defaultValue: "due now" })
    : relative
      ? t("issueBlockedNotice.scheduleRelative", { defaultValue: "scheduled {{relative}}", relative })
      : t("issueBlockedNotice.scheduled", { defaultValue: "scheduled" });
    ...
    <div className="mt-2 rounded-md border border-amber-300/70 bg-background/80 p-2 dark:border-amber-500/40 dark:bg-background/40">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0 text-xs leading-5 text-amber-900 dark:text-amber-100">
          {t("issueBlockedNotice.correctiveWake", {
        ...
        >
          {retryNow.isPending ? (
            <span className="inline-flex items-center gap-1.5">
              ...
              {t("issueBlockedNotice.retrying", { defaultValue: "Retrying..." })}
            </span>
          ) : success ? (
            <span className="inline-flex items-center gap-1.5">
              <CheckCircle2 className="h-3.5 w-3.5" aria-hidden="true" />
              {retryNow.data?.outcome === "already_promoted"
                ? t("issueBlockedNotice.alreadyPromoted", { defaultValue: "Already promoted" })
                : t("issueBlockedNotice.promoted", { defaultValue: "Promoted" })}
            </span>
          ) : (
            <span className="inline-flex items-center gap-1.5">
              <RotateCcw className="h-3.5 w-3.5" aria-hidden="true" />
              {t("issueBlockedNotice.retryNow", { defaultValue: "Retry now" })}
            </span>
          )}
        </Button>
      </div>
...
export function IssueBlockedNotice({
  ...
  const blockerLabel = blockers.length === 1
    ? t("issueBlockedNotice.blockerLabelSingular", { defaultValue: "the linked issue" })
    : t("issueBlockedNotice.blockerLabelPlural", { defaultValue: "the linked issues" });
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 space-y-1.5">
          {showSuccessfulRunHandoff ? (
            <>
              <p className="font-medium leading-5">{t("issueBlockedNotice.needsNextStep", { defaultValue: "This issue still needs a next step." })}</p>
              <p className="leading-5">
                {t("issueBlockedNotice.runFinishedPrefix", { defaultValue: "A run finished successfully, but this issue is still open in" })}{" "}
                <code className="rounded bg-amber-100 px-1 py-0.5 text-[12px] dark:bg-amber-400/15">
                  in_progress
                </code>{" "}
                {t("issueBlockedNotice.runFinishedSuffix", { defaultValue: "with no clear owner for the next action." })}
              </p>
              <ul className="list-disc space-y-1 pl-5 text-xs leading-5 text-amber-900 dark:text-amber-100">
                <li>{t("issueBlockedNotice.optionMarkDone", { defaultValue: "Mark it done or cancelled." })}</li>
                <li>{t("issueBlockedNotice.optionReview", { defaultValue: "Send it for review or ask for input." })}</li>
                <li>{t("issueBlockedNotice.optionBlocked", { defaultValue: "Mark it blocked with a blocker owner." })}</li>
                <li>{t("issueBlockedNotice.optionDelegate", { defaultValue: "Delegate follow-up work or queue a continuation." })}</li>
              </ul>
              <div className="flex flex-wrap gap-1.5 text-xs">
                ...
                <span className="rounded-md border border-amber-300/70 bg-background/80 px-2 py-1 text-amber-900 dark:border-amber-500/40 dark:bg-background/40 dark:text-amber-100">
                  {t("issueBlockedNotice.correctiveWakeQueued", {
                    defaultValue: "Corrective wake queued for {{assignee}}",
                    assignee: agentName ?? t("issueBlockedNotice.assigneeFallback", { defaultValue: "the assignee" }),
                  })}
                </span>
              </div>
                ...
                {blockers.length > 0
                  ? isStalled
                    ? stalledLeafBlockers.length > 1
                      ? t("issueBlockedNotice.blockedStalledPlural", {
                      ...
                      : t("issueBlockedNotice.blockedStalledSingular", {
                    ...
                    : t("issueBlockedNotice.blockedUntilComplete", {
                        defaultValue: "Work on this issue is blocked by {{blockerLabel}} until {{completion}} complete. Comments still wake the assignee for questions or triage.",
                        blockerLabel,
                        completion: blockers.length === 1
                          ? t("issueBlockedNotice.completionSingular", { defaultValue: "it is" })
                          : t("issueBlockedNotice.completionPlural", { defaultValue: "they are" }),
                      })
                  : t("issueBlockedNotice.blockedMovedBack", {
              ...
              {showStalledRow ? (
                <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
                  <span className="text-xs font-medium text-amber-800 dark:text-amber-200">
                    {t("issueBlockedNotice.stalledInReview", { defaultValue: "Stalled in review" })}
                  </span>
                ...
                <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
                  <span className="text-xs font-medium text-amber-800 dark:text-amber-200">
                    {t("issueBlockedNotice.ultimatelyWaitingOn", { defaultValue: "Ultimately waiting on" })}
                  </span>
              ...
              {showParkedRow ? (
                ...
                >
                  <span className="inline-flex items-center gap-1 text-xs font-medium text-amber-800 dark:text-amber-200">
                    ...
                    {t("issueBlockedNotice.blockedByParkedWork", { defaultValue: "Blocked by parked work" })}
                  </span>
