  ...
  if (richEditorError) {
    return (
      <div
        ref={containerRef}
        className={cn(
          "relative workcell-mdxeditor-scope",
  ...
  return (
    ...
    >
      <MDXEditor
        ...
        className={cn("workcell-mdxeditor", !bordered && "workcell-mdxeditor--borderless")}
        contentEditableClassName={cn(
          "workcell-mdxeditor-content focus:outline-none [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5 [&_li]:list-item",
          contentClassName,
        )}
              ...
              >
                ...
                {option.kind === "project" && option.projectId && (
                  <span className="ml-auto text-[10px] uppercase tracking-wide text-muted-foreground">
                    {t("markdownEditor.mention.kind.project", { defaultValue: "Project" })}
                  </span>
                )}
                {option.kind === "user" && (
                  <span className="ml-auto text-[10px] uppercase tracking-wide text-muted-foreground">
                    {t("markdownEditor.mention.kind.user", { defaultValue: "User" })}
                  </span>
                )}
                {option.kind === "skill" && (
                  <span className="ml-auto text-[10px] uppercase tracking-wide text-muted-foreground">
                    {t("markdownEditor.mention.kind.skill", { defaultValue: "Skill" })}
                  </span>
                )}
                {option.kind === "routine" && (
                  <span className="ml-auto text-[10px] uppercase tracking-wide text-muted-foreground">
                    {t("markdownEditor.mention.kind.routine", { defaultValue: "Routine" })}
                  </span>
                )}
              </button>
            ))}
      ...
      {isDragOver && canDropFile && (
        ...
        >
          {t("markdownEditor.dropToUpload", {
            defaultValue: "Drop {{target}} to upload",
            target: onDropFile
              ? t("markdownEditor.dropTarget.file", { defaultValue: "file" })
              : t("markdownEditor.dropTarget.image", { defaultValue: "image" }),
          })}
        </div>
      )}
      {uploadError && (
