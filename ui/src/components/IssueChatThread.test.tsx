...
vi.mock("../hooks/useWorkcellIssueRuntime", () => ({
  useWorkcellIssueRuntime: () => ({}),
}));
...
describe("IssueChatThread", () => {
  ...
  it("renders the transcript directly from stable Workcell messages", () => {
    const root = createRoot(container);
  ...
  it("shows the live work bar above the composer only while a run is in flight", () => {
    const root = createRoot(container);
    const agentMap = new Map<string, Agent>([
      ["agent-1", { id: "agent-1", name: "Reviewer" } as Agent],
    ]);
    ...
    const bar = container.querySelector('[data-testid="issue-chat-live-work-bar"]');
    expect(bar).not.toBeNull();
    expect(bar?.textContent).toContain("Reviewer is working…");
    // Collapsed by default: the live tail (and its polling) only mounts on expand.
    expect(container.querySelector('[data-testid="live-run-tail"]')).toBeNull();
    // Docked inside the sticky composer dock, right above the composer.
    const dock = container.querySelector('[data-testid="issue-chat-composer-dock"]');
    ...
    expect(container.querySelector('[data-testid="issue-chat-live-work-bar"]')).toBeNull();
  ...
  it("explains the wake behavior in the composer placeholder when targeting an agent", () => {
    const root = createRoot(container);
    const agentMap = new Map<string, Agent>([
      ["agent-1", { id: "agent-1", name: "Reviewer" } as Agent],
    ]);
      ...
      root.render(
        <MemoryRouter>
          <IssueChatThread
            ...
            agentMap={agentMap}
            enableReassign
            reassignOptions={[{ id: "agent:agent-1", label: "Reviewer" }]}
            currentAssigneeValue="agent:agent-1"
            onAdd={async () => {}}
            enableLiveTranscriptPolling={false}
          />
    ...
    const editor = container.querySelector('textarea[aria-label="Issue chat editor"]');
    expect(editor?.getAttribute("placeholder")).toBe(
      "Message Reviewer — sending wakes the agent to handle your reply",
    );
    ...
    expect(
      container.querySelector('textarea[aria-label="Issue chat editor"]')?.getAttribute("placeholder"),
    ).toBe("Reply");
