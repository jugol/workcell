...
vi.mock("./InlineEntitySelector", async () => {
  ...
  return {
    ...
    >(function InlineEntitySelectorMock(
      { value, placeholder, options, onChange, renderTriggerValue, renderOption },
      ref,
    ) {
      return (
        <div>
          ...
          {(options ?? []).map((option) => (
            <button
              key={option.id}
              type="button"
              data-testid={`inline-option-${placeholder}-${option.id}`}
...
describe("NewIssueDialog", () => {
  ...
  it("allows editor autocomplete portal pointer events inside the modal", async () => {
    ...
    menu.setAttribute("data-workcell-floating-ui", "");
