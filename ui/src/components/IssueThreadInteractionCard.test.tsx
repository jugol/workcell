...
describe("IssueThreadInteractionCard", () => {
  ...
  it("shows the autonomy auto-approved badge when a confirmation was accepted by the system", () => {
    const host = renderCard({
      interaction: {
        ...acceptedRequestConfirmationInteraction,
    ...
    const badge = host.querySelector("[data-testid='interaction-auto-approved-badge']");
    expect(badge).toBeTruthy();
    expect(badge?.textContent).toContain("Auto-approved in autonomy mode");
    expect(host.textContent).toContain("System (autonomy mode)");
  });

  it("does not show the auto-approved badge when a person accepted the confirmation", () => {
    const host = renderCard({
      interaction: acceptedRequestConfirmationInteraction,
    });

    expect(host.querySelector("[data-testid='interaction-auto-approved-badge']")).toBeNull();
  });
  ...
  it("locks decision buttons and explains why while the source run is still running", () => {
    const host = renderCard({
      interaction: {
        ...pendingRequestConfirmationInteraction,
        sourceRunId: "11111111-1111-4111-8111-aaaaaaaaaaaa",
      },
      runningRunIds: new Set(["11111111-1111-4111-8111-aaaaaaaaaaaa"]),
      onAcceptInteraction: vi.fn(),
      onRejectInteraction: vi.fn(),
    });

    const approveButton = Array.from(host.querySelectorAll("button")).find((button) =>
      button.textContent?.includes("Approve plan"),
    );
    const declineButton = Array.from(host.querySelectorAll("button")).find((button) =>
      button.textContent?.includes("Request revisions"),
    ...
    const notice = host.querySelector("[data-testid='interaction-source-run-active']");
    expect(notice).toBeTruthy();
    expect(notice?.textContent).toContain(
      "The agent is still finishing this work — you can decide once it completes.",
    );
  });

  it("keeps decision buttons active once the source run is no longer running", () => {
    const host = renderCard({
      interaction: {
        ...pendingRequestConfirmationInteraction,
        sourceRunId: "11111111-1111-4111-8111-aaaaaaaaaaaa",
      },
      runningRunIds: new Set(["22222222-2222-4222-8222-bbbbbbbbbbbb"]),
      onAcceptInteraction: vi.fn(),
      onRejectInteraction: vi.fn(),
    });
    ...
    expect(declineButton?.hasAttribute("disabled")).toBe(false);
    expect(host.querySelector("[data-testid='interaction-source-run-active']")).toBeNull();
  });

  it("ignores running runs when the interaction has no source run", () => {
    const host = renderCard({
      interaction: pendingRequestConfirmationInteraction,
      runningRunIds: new Set(["22222222-2222-4222-8222-bbbbbbbbbbbb"]),
      onAcceptInteraction: vi.fn(),
