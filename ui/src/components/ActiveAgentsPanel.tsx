...
function RunCardRecoveryChip({ action }: { action: IssueRecoveryAction }) {
  ...
  return (
    <span
      ...
      title={t("activeAgentsPanel.recoveryChip.title", {
...
export function ActiveAgentsPanel({
  ...
  const { t } = useTranslation();
  const resolvedTitle = title ?? t("activeAgentsPanel.title", { defaultValue: "Agents" });
  const resolvedEmptyMessage =
    emptyMessage ?? t("activeAgentsPanel.emptyMessage", { defaultValue: "No recent agent runs." });
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            ...
            <div className="mt-2 flex items-center gap-2 text-[11px] text-muted-foreground">
              <span>{isActive
                ? t("activeAgentsPanel.runCard.liveNow", { defaultValue: "Live now" })
                : run.finishedAt
                  ? t("activeAgentsPanel.runCard.finished", { defaultValue: "Finished {{time}}", time: relativeTime(run.finishedAt) })
                  : t("activeAgentsPanel.runCard.started", { defaultValue: "Started {{time}}", time: relativeTime(run.createdAt) })}</span>
            </div>
          </div>
