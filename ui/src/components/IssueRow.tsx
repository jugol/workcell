...
import type { Issue, IssueRecoveryAction } from "@workcell/shared";
...
export function IssueRow({
  ...
  const productivityReviewIndicator = productivityReview ? (
    <span
      ...
      aria-label={t("issueRow.productivityReviewAria", { defaultValue: "Productivity review open" })}
    >
  ...
  const parkedBlockerIndicator = hasAssignedBacklogBlocker(issue.blockedBy) ? (
    <span
      ...
      title={t("issueRow.parkedBlockerTooltip", { defaultValue: "Blocked by parked work — at least one assigned blocker is in backlog and will not wake its assignee." })}
    >
      <Flag className="h-2.5 w-2.5" aria-hidden />
      {t("issueRow.parkedBlockerLabel", { defaultValue: "Blocked by parked work" })}
    </span>
  ) : null;
  const plannerDraftIndicator = issue.originKind === "planner_draft_request" ? (
    <span
      data-testid="issue-row-planner-draft"
      className={cn(
        "ml-1.5 inline-flex shrink-0 items-center rounded-full border border-violet-500/60 bg-violet-500/15 px-2 py-0.5 text-[10px] font-medium text-violet-700 dark:text-violet-300",
        selected ? "!border-muted-foreground !text-muted-foreground" : null,
      )}
      title={t("issueRow.plannerDraftTooltip", { defaultValue: "Planner draft — generated from a natural-language prompt." })}
    >
      {t("issueRow.plannerDraftLabel", { defaultValue: "Draft" })}
    </span>
  ) : null;
        ...
        <span className="inline-flex h-4 w-4 shrink-0 items-center justify-center self-center">
          {showUnreadDot ? (
            <button
              ...
              aria-label={t("issueRow.markAsReadAria", { defaultValue: "Mark as read" })}
            >
              <span
                className={cn(
            ...
            <button
              ...
              aria-label={t("issueRow.dismissFromInboxAria", { defaultValue: "Dismiss from inbox" })}
            >
