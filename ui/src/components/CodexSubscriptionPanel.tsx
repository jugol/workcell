import type { QuotaWindow } from "@workcell/shared";
...
function detailText(window: QuotaWindow): string | null {
  ...
  return t("codexSubscriptionPanel.resets", { defaultValue: "Resets {{time}}", time: formatted });
}
      ...
      <div className="flex items-start justify-between gap-3 border-b border-border pb-3">
        <div className="min-w-0">
          <div className="text-[11px] font-semibold uppercase tracking-[0.2em] text-muted-foreground">
            {t("codexSubscriptionPanel.heading", { defaultValue: "Codex subscription" })}
          </div>
          <div className="mt-1 text-sm text-muted-foreground">
            {t("codexSubscriptionPanel.subheading", { defaultValue: "Live Codex quota windows." })}
          </div>
        </div>
        {source ? (
      ...
      <div className="mt-4 space-y-5">
        <div className="space-y-3">
          <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">
            {t("codexSubscriptionPanel.accountWindows", { defaultValue: "Account windows" })}
          </div>
          <div className="space-y-3">
        ...
        {modelWindows.length > 0 ? (
          <div className="space-y-3">
            <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">
              {t("codexSubscriptionPanel.modelWindows", { defaultValue: "Model windows" })}
            </div>
            <div className="space-y-3">
    ...
    <div className="border border-border px-3.5 py-3">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="shrink-0 text-sm font-semibold tabular-nums text-foreground">
          {t("codexSubscriptionPanel.percentUsed", { defaultValue: "{{percent}}% used", percent: window.usedPercent })}
        </div>
      </div>
