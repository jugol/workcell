...
import { InboxActionHint } from "./InboxActionHint";
  ...
  if (error) {
    const message =
      error instanceof Error
        ? error.message
        : t("blockedInboxView.error.title", { defaultValue: "Couldn't load the Blocked tab." });
    return (
      <div
      ...
      >
        <div className="flex items-start gap-2">
          ...
          <div className="flex-1 space-y-1">
            <p className="text-sm font-medium">{t("blockedInboxView.error.title", { defaultValue: "Couldn't load the Blocked tab." })}</p>
            <p className="text-xs opacity-80">
              {t("blockedInboxView.error.body", { defaultValue: "Other Inbox tabs still work. {{message}}", message })}
            </p>
          </div>
          <Button
            type="button"
    ...
    return (
      ...
      >
        ...
        <div className="space-y-1">
          <p className="text-sm font-medium text-foreground">{t("blockedInboxView.empty.title", { defaultValue: "No work is stopped." })}</p>
          <p className="text-xs text-muted-foreground">
            {t("blockedInboxView.empty.description", { defaultValue: "Issues that need a decision, recovery, or external action will appear here." })}
          </p>
        </div>
      </div>
    );
  ...
  if (groups.length === 0) {
    return (
      <div className="space-y-3">
        ...
        >
          {t("blockedInboxView.noSearchResults", { defaultValue: "No stopped items match your search." })}
        </div>
      </div>
    );
  }
...
function BlockedInboxRow({
  ...
  const mobileMeta = (
    <span className="flex flex-wrap items-center gap-x-1.5 gap-y-1 text-xs text-muted-foreground">
      ...
      <span data-testid="blocked-row-reason-mobile">{row.reasonLabel}</span>
      {ownerName ? (
        <>
          <span aria-hidden="true">·</span>
          <span
  ...
  return (
    <IssueRow
      ...
      titleSuffix={
        <InboxActionHint attention={row.attention} className="ml-2 max-w-[14rem] align-middle" />
      }
