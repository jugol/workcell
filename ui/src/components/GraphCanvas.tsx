...
export interface GraphCanvasNode {
  ...
  tint?: string;
  /**
   * Optional sandboxed preview URL — when set, a scaled-down thumbnail of the
   * content renders above the label so the node SHOWS the screen (used by the
   * design flow wireframe to display each screen 시안, not just its name).
   */
      ...
      <div className="absolute inset-0" style={{ transform, transformOrigin: "0 0" }}>
        {placed.map(({ node, x, y }) => {
          ...
          return (
            ...
            >
              {node.previewUrl ? (
                // Scaled-down 시안 thumbnail. pointer-events-none so the node
                // button still receives the click; sandboxed for untrusted HTML.
                <div className="pointer-events-none mb-1 h-24 w-full overflow-hidden rounded border border-border/60 bg-white">
                  <iframe
                    sandbox=""
                    src={node.previewUrl}
                    title={node.label}
                    tabIndex={-1}
                    loading="lazy"
                    className="h-[300px] w-[390px] origin-top-left"
