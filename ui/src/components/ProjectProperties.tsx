...
import type { Project } from "@workcell/shared";
...
function ArchiveDangerZone({
  ...
  const action = isArchive
    ? t("projectProperties.archive", { defaultValue: "Archive" })
    : t("projectProperties.unarchive", { defaultValue: "Unarchive" });

  return (
    <div className="space-y-3 rounded-md border border-destructive/40 bg-destructive/5 px-4 py-4">
      <p className="text-sm text-muted-foreground">
        {isArchive
          ? t("projectProperties.archiveHint", { defaultValue: "Archive this project to hide it from the sidebar and project selectors." })
          : t("projectProperties.unarchiveHint", { defaultValue: "Unarchive this project to restore it in the sidebar and project selectors." })}
      </p>
      {archivePending ? (
        <Button size="sm" variant="destructive" disabled>
          <Loader2 className="h-3 w-3 animate-spin mr-1" />
          {isArchive ? t("projectProperties.archiving", { defaultValue: "Archiving..." }) : t("projectProperties.unarchiving", { defaultValue: "Unarchiving..." })}
        </Button>
      ) : confirming ? (
        <div className="flex items-center gap-2">
          <span className="text-sm text-destructive font-medium">
            {t("projectProperties.confirmAction", { defaultValue: "{{action}} “{{name}}”?", action, name: project.name })}
          </span>
          <Button
            size="sm"
            variant="destructive"
            ...
            <Popover open={goalOpen} onOpenChange={setGoalOpen}>
              <PopoverTrigger asChild>
                ...
                >
                  ...
                  {t("projectProperties.goal", { defaultValue: "Goal" })}
                </Button>
              </PopoverTrigger>
        ...
        >
          <div className="space-y-2">
            ...
            <p className="text-[11px] text-muted-foreground">
              {t("projectProperties.envHint", { defaultValue: "Applied to all runs for issues in this project. Project values override agent env on key conflicts." })}
            </p>
          </div>
        </PropertyRow>
      ...
      <div className="space-y-1 py-4">
        <div className="space-y-2">
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            ...
            <Tooltip>
              ...
              <TooltipContent side="top">
                {t("projectProperties.codebaseHelp", { defaultValue: "Repo identifies the source of truth. Local folder is the default place agents write code." })}
              </TooltipContent>
            </Tooltip>
          </div>
          <div className="space-y-2 rounded-md border border-border/70 p-3">
              ...
              <div className="flex items-center justify-between gap-2">
                <div className="min-w-0 space-y-1">
                  ...
                  {codebase.origin === "managed_checkout" && (
                    <div className="text-[11px] text-muted-foreground">{t("projectProperties.managedFolder", { defaultValue: "Workcell-managed folder." })}</div>
                  )}
                </div>
                <div className="flex items-center gap-1">
                  <Button
            ...
            {hasAdditionalLegacyWorkspaces && (
              <div className="text-[11px] text-muted-foreground">
                {t("projectProperties.legacyWorkspaces", { defaultValue: "Additional legacy workspace records exist on this project. Workcell is using the primary workspace as the codebase view." })}
              </div>
            )}
          ...
          {createWorkspace.isError && (
            <p className="text-xs text-destructive">{t("projectProperties.saveWorkspaceFailed", { defaultValue: "Failed to save workspace." })}</p>
          )}
          {removeWorkspace.isError && (
            <p className="text-xs text-destructive">{t("projectProperties.deleteWorkspaceFailed", { defaultValue: "Failed to delete workspace." })}</p>
          )}
          {updateWorkspace.isError && (
            <p className="text-xs text-destructive">{t("projectProperties.updateWorkspaceFailed", { defaultValue: "Failed to update workspace." })}</p>
          )}
        </div>
            ...
            <div className="py-1.5 space-y-2">
              <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                ...
                <Tooltip>
                  ...
                  <TooltipContent side="top">
                    {t("projectProperties.executionWorkspacesHelp", { defaultValue: "Project-owned defaults for isolated issue checkouts and execution workspace behavior." })}
                  </TooltipContent>
                </Tooltip>
              </div>
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="space-y-0.5">
                    <div className="flex items-center gap-2 text-sm font-medium">
                      <span>{t("projectProperties.enableIsolated", { defaultValue: "Enable isolated issue checkouts" })}</span>
                      <SaveIndicator state={fieldState("execution_workspace_enabled")} />
                    </div>
                    <div className="text-xs text-muted-foreground">
                      {t("projectProperties.enableIsolatedHint", { defaultValue: "Let issues choose between the project's primary checkout and an isolated execution workspace." })}
                    </div>
                  </div>
                  {onUpdate || onFieldUpdate ? (
                    <ToggleSwitch
                    ...
                    <span className="text-xs text-muted-foreground">
                      {executionWorkspacesEnabled ? t("projectProperties.enabled", { defaultValue: "Enabled" }) : t("projectProperties.disabled", { defaultValue: "Disabled" })}
                    </span>
                  )}
                </div>
                ...
                {executionWorkspacesEnabled ? (
                  <div className="space-y-3">
                    <div className="flex items-center justify-between gap-3">
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-2 text-sm">
                          <span>{t("projectProperties.newIssuesIsolated", { defaultValue: "New issues default to isolated checkout" })}</span>
                          <SaveIndicator state={fieldState("execution_workspace_default_mode")} />
                        </div>
                        <div className="text-[11px] text-muted-foreground">
                          {t("projectProperties.newIssuesIsolatedHint", { defaultValue: "If disabled, new issues stay on the project's primary checkout unless someone opts in." })}
                        </div>
                      </div>
                      <ToggleSwitch
                    ...
                    <div className="border-t border-border/60 pt-2">
                      ...
                      >
                        {executionWorkspaceAdvancedOpen
                          ? t("projectProperties.hideAdvanced", { defaultValue: "Hide advanced checkout settings" })
                          : t("projectProperties.showAdvanced", { defaultValue: "Show advanced checkout settings" })}
                      </button>
                    </div>
                    ...
                    {executionWorkspaceAdvancedOpen ? (
                      <div className="space-y-3">
                        <div className="text-xs text-muted-foreground">
                          {t("projectProperties.hostManaged", { defaultValue: "Host-managed implementation:" })} <span className="text-foreground">{t("projectProperties.gitWorktree", { defaultValue: "Git worktree" })}</span>
                        </div>
                        {environmentsEnabled ? (
                          <div>
                        ...
                        <div>
                          ...
                          <DraftInput
                            ...
                            immediate
                            className="w-full rounded border border-border bg-transparent px-2 py-1 text-xs font-mono outline-none"
                            placeholder=".workcell/worktrees"
                          />
                        </div>
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                        ...
                        <p className="text-[11px] text-muted-foreground">
                          {t("projectProperties.provisionTeardownHint", { defaultValue: "Provision runs inside the derived worktree before agent execution. Teardown is stored here for future cleanup flows." })}
                        </p>
                      </div>
                    ) : null}
                  </div>
