...
import type { CompanySecret, EnvBinding, SecretVersionSelector } from "@workcell/shared";
...
export function EnvVarEditor({
  ...
  async function sealRow(index: number) {
    ...
    const name = window.prompt(t("envVarEditor.secretNamePrompt", { defaultValue: "Secret name" }), suggested)?.trim();
    if (!name) return;
    ...
    try {
      ...
      setSealError(error instanceof Error ? error.message : t("envVarEditor.createSecretFailed", { defaultValue: "Failed to create secret" }));
    }
  }
  ...
  return (
    <div className="space-y-1.5">
        ...
        return (
          <div key={index} className="flex items-center gap-1.5">
            <input
              ...
              placeholder={t("envVarEditor.keyPlaceholder", { defaultValue: "KEY" })}
              value={row.key}
            ...
            >
              <option value="plain">{t("envVarEditor.sourcePlain", { defaultValue: "Plain" })}</option>
              <option value="secret">{t("envVarEditor.sourceSecret", { defaultValue: "Secret" })}</option>
            </select>
            {row.source === "secret" ? (
              <>
                <select
                ...
                >
                  <option value="">{t("envVarEditor.selectSecretOption", { defaultValue: "Select secret..." })}</option>
                  {row.secretId && !secrets.some((s) => s.id === row.secretId) ? (
                    <option value={row.secretId}>{t("envVarEditor.missingSecretOption", { defaultValue: "Missing ({{id}}…)", id: row.secretId.slice(0, 8) })}</option>
                  ) : null}
                  {secrets.map((secret) => (
                ...
                <select
                  ...
                  aria-label={t("envVarEditor.versionAria", { defaultValue: "Version" })}
                >
                  <option value="latest">{t("envVarEditor.versionLatest", { defaultValue: "latest" })}</option>
                  {(() => {
                ...
                <button
                  ...
                  title={t("envVarEditor.createSecretTooltip", { defaultValue: "Create secret from current plain value" })}
                >
                  {t("envVarEditor.newButton", { defaultValue: "New" })}
                </button>
              </>
            ) : (
              <>
                ...
                <button
                  ...
                  onClick={() => sealRow(index)}
                  disabled={!row.key.trim() || !row.plainValue}
                  title={t("envVarEditor.sealTooltip", { defaultValue: "Store value as secret and replace with reference" })}
                >
                  {t("envVarEditor.sealButton", { defaultValue: "Seal" })}
                </button>
              </>
            )}
            {!isTrailing ? (
      ...
      <p className="text-[11px] text-muted-foreground/60">
        {t("envVarEditor.helperText", {
          defaultValue: "Set KEY to the env var name the process expects, for example GH_TOKEN. Choose Secret to resolve a stored value at run start. WORKCELL_* variables are injected automatically.",
        })}
      </p>
    </div>
  );
