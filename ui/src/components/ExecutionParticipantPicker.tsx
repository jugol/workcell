...
export function ExecutionParticipantPicker({
  ...
  const participantLabel = (value: string) => {
    ...
    if (value.startsWith("user:")) return userLabel(value.slice("user:".length)) ?? t("executionParticipantPicker.user", { defaultValue: "User" });
    return value;
  };
  ...
  const displayLabel = stageType === "review"
    ? t("executionParticipantPicker.reviewers", { defaultValue: "Reviewers" })
    : t("executionParticipantPicker.approvers", { defaultValue: "Approvers" });
  const noneLabel = stageType === "review"
    ? t("executionParticipantPicker.noReviewers", { defaultValue: "No reviewers" })
    : t("executionParticipantPicker.noApprovers", { defaultValue: "No approvers" });
        ...
        <div className="max-h-48 overflow-y-auto overscroll-contain">
          ...
          {currentUserId && (
            ...
            >
              ...
              {t("executionParticipantPicker.assignToMe", { defaultValue: "Assign to me" })}
            </button>
          )}
