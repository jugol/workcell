      ...
      {preset === "custom" ? (
        <div className="space-y-1.5">
          ...
          <p className="text-xs text-muted-foreground">
            {t("scheduleEditor.fiveFields", { defaultValue: "Five fields: minute hour day-of-month month day-of-week" })}
          </p>
        </div>
      ) : (
        <div className="flex flex-wrap items-center gap-2">
          {preset !== "every_minute" && preset !== "every_hour" && (
            <>
              <span className="text-sm text-muted-foreground">{t("scheduleEditor.at", { defaultValue: "at" })}</span>
              <Select
                value={hour}
                onValueChange={(h) => {
                  setHour(h);
          ...
          {preset === "every_hour" && (
            <>
              <span className="text-sm text-muted-foreground">{t("scheduleEditor.atMinute", { defaultValue: "at minute" })}</span>
              <Select
                value={minute}
                onValueChange={(m) => {
                  setMinute(m);
          ...
          {preset === "weekly" && (
            <>
              <span className="text-sm text-muted-foreground">{t("scheduleEditor.on", { defaultValue: "on" })}</span>
          ...
          {preset === "monthly" && (
            <>
              <span className="text-sm text-muted-foreground">{t("scheduleEditor.onDay", { defaultValue: "on day" })}</span>
              <Select
                value={dayOfMonth}
                onValueChange={(dom) => {
                  setDayOfMonth(dom);
