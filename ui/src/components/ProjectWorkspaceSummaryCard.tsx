...
import type { ExecutionWorkspace, Issue } from "@workcell/shared";
...
function workspaceKindLabel(kind: ProjectWorkspaceSummary["kind"]) {
  return kind === "execution_workspace"
    ? t("projectWorkspaceSummaryCard.kind.execution", { defaultValue: "Execution workspace" })
    : t("projectWorkspaceSummaryCard.kind.project", { defaultValue: "Project workspace" });
}
          ...
          <div className="min-w-0 space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="inline-flex items-center rounded-full border border-border/70 bg-background px-2.5 py-1 text-xs text-muted-foreground">
                {t("projectWorkspaceSummaryCard.updated", { defaultValue: "Updated {{time}}", time: timeAgo(summary.lastUpdatedAt) })}
              </span>
              {summary.serviceCount > 0 ? (
                ...
                >
                  ...
                  />
                  {t("projectWorkspaceSummaryCard.services", { defaultValue: "{{running}}/{{total}} services", running: summary.runningServiceCount, total: summary.serviceCount })}
                </span>
              ) : null}
          ...
          >
            {summary.hasRuntimeConfig ? (
              ...
              >
                ...
                {hasRunningServices ? t("projectWorkspaceSummaryCard.stopServices", { defaultValue: "Stop services" }) : t("projectWorkspaceSummaryCard.startServices", { defaultValue: "Start services" })}
              </Button>
            ) : null}
            {summary.kind === "execution_workspace" && summary.executionWorkspaceId && summary.executionWorkspaceStatus ? (
              ...
              >
                {summary.executionWorkspaceStatus === "cleanup_failed" ? t("projectWorkspaceSummaryCard.retryClose", { defaultValue: "Retry close" }) : t("projectWorkspaceSummaryCard.closeWorkspace", { defaultValue: "Close workspace" })}
              </Button>
            ) : null}
          </div>
        </div>
          ...
          <div className="space-y-2 text-sm">
            {summary.branchName ? (
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{t("projectWorkspaceSummaryCard.branch", { defaultValue: "Branch" })}</div>
                  <div className="flex items-start gap-2">
                    <CopyText
            ...
            {summary.cwd ? (
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{t("projectWorkspaceSummaryCard.path", { defaultValue: "Path" })}</div>
                  <div className="flex items-start gap-2">
                    <CopyText
                      text={summary.cwd}
                      title={summary.cwd}
            ...
            {summary.primaryServiceUrl ? (
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0">
                  <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{t("projectWorkspaceSummaryCard.service", { defaultValue: "Service" })}</div>
                  <a
        ...
        {summary.issues.length > 0 ? (
          <div className="space-y-2">
            <div className="text-[11px] font-medium uppercase tracking-[0.14em] text-muted-foreground">
              {t("projectWorkspaceSummaryCard.linkedIssues", { defaultValue: "Linked issues" })}
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              {hiddenIssueCount > 0 ? (
                ...
                >
                  {t("projectWorkspaceSummaryCard.moreIssues", { defaultValue: "+{{count}} more", count: hiddenIssueCount })}
                </Link>
              ) : null}
            </div>
          </div>
