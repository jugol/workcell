...
import type { BudgetPolicySummary } from "@workcell/shared";
...
function windowLabel(windowKind: BudgetPolicySummary["windowKind"]) {
  return windowKind === "lifetime"
    ? t("budgetPolicyCard.lifetimeBudget", { defaultValue: "Lifetime budget" })
    : t("budgetPolicyCard.monthlyBudget", { defaultValue: "Monthly UTC budget" });
}
...
export function BudgetPolicyCard({
  ...
  const observedLimitLabel = summary.amount > 0
    ? t("budgetPolicyCard.percentOfLimit", { defaultValue: "{{percent}}% of limit", percent: summary.utilizationPercent })
    : t("budgetPolicyCard.noCapConfigured", { defaultValue: "No cap configured" });
  const softAlertLabel = (
    <>
      {t("budgetPolicyCard.softAlertAt", { defaultValue: "Soft alert at {{percent}}%", percent: summary.warnPercent })}{summary.paused && summary.pauseReason ? ` · ${summary.pauseReason} pause` : ""}
    </>
  );
  ...
  const observedBudgetGrid = isPlain ? (
    <div className="grid gap-6 sm:grid-cols-2">
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("budgetPolicyCard.observed", { defaultValue: "Observed" })}</div>
      ...
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("budgetPolicyCard.budget", { defaultValue: "Budget" })}</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">
          {summary.amount > 0 ? formatCents(summary.amount) : t("budgetPolicyCard.disabled", { defaultValue: "Disabled" })}
        </div>
        <div className="mt-1 text-xs text-muted-foreground">
          {softAlertLabel}
        </div>
  ...
  const progressSection = (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <span>{t("budgetPolicyCard.remaining", { defaultValue: "Remaining" })}</span>
        <span>{summary.amount > 0 ? formatCents(summary.remainingAmount) : t("budgetPolicyCard.unlimited", { defaultValue: "Unlimited" })}</span>
      </div>
    ...
    <div className="flex items-start gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-100">
      ...
      <div>
        {summary.scopeType === "project"
          ? t("budgetPolicyCard.pausedProject", { defaultValue: "Execution is paused for this project until the budget is raised or the incident is dismissed." })
          : t("budgetPolicyCard.pausedScope", { defaultValue: "Heartbeats are paused for this scope until the budget is raised or the incident is dismissed." })}
      </div>
    </div>
  ) : null;
    ...
    <div className={cn("flex flex-col gap-3 sm:flex-row sm:items-end", isPlain ? "" : "rounded-xl border border-border/70 bg-background/50 p-3")}>
      <div className="min-w-0 flex-1">
        <label className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">
          {t("budgetPolicyCard.budgetUsd", { defaultValue: "Budget (USD)" })}
        </label>
        <Input
          value={draftBudget}
      ...
      >
        {isSaving ? t("budgetPolicyCard.saving", { defaultValue: "Saving..." }) : summary.amount > 0 ? t("budgetPolicyCard.updateBudget", { defaultValue: "Update budget" }) : t("budgetPolicyCard.setBudget", { defaultValue: "Set budget" })}
      </Button>
    </div>
  ) : null;
  ...
  if (isPlain) {
    return (
      <div className="space-y-6">
        ...
        {saveSection}
        {parsedDraft === null ? (
          <p className="text-xs text-destructive">{t("budgetPolicyCard.invalidAmount", { defaultValue: "Enter a valid non-negative dollar amount." })}</p>
        ) : null}
      </div>
    );
  }
