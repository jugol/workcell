...
import type { CostByProviderModel, CostWindowSpendRow, QuotaWindow } from "@workcell/shared";
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <CardDescription className="text-xs mt-0.5">
              <span className="font-mono">{formatTokens(totalInputTokens)}</span> {t("providerQuotaCard.in", { defaultValue: "in" })}
              {" · "}
              <span className="font-mono">{formatTokens(totalOutputTokens)}</span> {t("providerQuotaCard.out", { defaultValue: "out" })}
      ...
      <CardContent className="px-4 pb-4 pt-3 space-y-4">
          ...
          <>
            ...
            <div className="space-y-2">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                {t("providerQuotaCard.rollingWindows", { defaultValue: "Rolling windows" })}
              </p>
        ...
        {totalSubRuns > 0 && (
          <>
            ...
            <div className="space-y-2">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                {t("providerQuotaCard.subscription", { defaultValue: "Subscription" })}
              </p>
              <p className="text-xs text-muted-foreground">
                <span className="font-mono text-foreground">{totalSubRuns}</span> {t("providerQuotaCard.runs", { defaultValue: "runs" })}
                {" · "}
                {totalSubTokens > 0 && (
                  <>
                    <span className="font-mono text-foreground">{formatTokens(totalSubTokens)}</span> {t("providerQuotaCard.total", { defaultValue: "total" })}
                    {" · "}
                  </>
                )}
                <span className="font-mono text-foreground">{formatTokens(totalSubInputTokens)}</span> {t("providerQuotaCard.in", { defaultValue: "in" })}
                {" · "}
                <span className="font-mono text-foreground">{formatTokens(totalSubOutputTokens)}</span> {t("providerQuotaCard.out", { defaultValue: "out" })}
              </p>
              {subSharePct > 0 && (
                <>
          ...
          <>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between gap-3">
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                  {t("providerQuotaCard.subscriptionQuota", { defaultValue: "Subscription quota" })}
                </p>
                <div className="flex items-center gap-1.5 shrink-0">
                  {/* WC-20: tri-state confidence chip — Exact (real-time fetch),
                      Synced (periodically refreshed), Estimated (derived from
                      local token counts). Colors reflect confidence: green for
                      verified, amber for cached, muted for derived. */}
                  {(() => {
                    const c = quotaConfidence(quotaSource);
                    const colorClass =
                      c === "exact"
                        ? "border-emerald-500/45 bg-emerald-50/60 text-emerald-700 dark:border-emerald-300/35 dark:bg-emerald-400/10 dark:text-emerald-300"
                        : c === "synced"
                          ? "border-amber-500/45 bg-amber-50/60 text-amber-700 dark:border-amber-300/35 dark:bg-amber-400/10 dark:text-amber-300"
                          : "border-border bg-muted/40 text-muted-foreground";
                    return (
                      <span
                        className={`rounded border px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-[0.14em] ${colorClass}`}
                        title={
                          c === "exact"
                            ? "Quota fetched in real-time from the provider."
                            : c === "synced"
                              ? "Quota periodically synced and cached."
                              : "Quota derived from local token usage — no provider sync."
                        }
                        aria-label={`Confidence: ${quotaConfidenceLabel(c)}`}
                      >
                        {quotaConfidenceLabel(c)}
                      </span>
                    );
                  })()}
                        ...
                        <div key={qw.label} className="space-y-1">
                          <div className="flex items-center justify-between gap-2 text-xs">
                            ...
                            {qw.valueLabel != null ? (
                              ...
                              <span className="font-medium tabular-nums">{t("providerQuotaCard.percentUsed", { defaultValue: "{{percent}}% used", percent: qw.usedPercent })}</span>
                            ) : null}
                          </div>
