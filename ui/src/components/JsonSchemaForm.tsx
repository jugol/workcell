  ...
  >
    ...
    >
      <SelectTrigger className="w-full">
        <SelectValue placeholder={t("jsonSchemaForm.selectAnOption", { defaultValue: "Select an option" })} />
      </SelectTrigger>
      <SelectContent>
        {options.map((option) => (
        ...
        <Textarea
          ...
          value={
            stringValue.length === 0
              ? ""
              : t("jsonSchemaForm.sensitiveHidden", {
    ...
    <FieldWrapper
      ...
      description={
        ...
        t("jsonSchemaForm.secretDescription", {
          defaultValue: "Pick an existing team secret, or paste a raw value (Workcell will store it as a secret on save).",
        })
      }
      required={isRequired}
      error={error}
    ...
    >
      <div className="space-y-2">
        <SecretBindingPicker
          ...
          label=""
          placeholder={t("jsonSchemaForm.selectExistingSecret", { defaultValue: "Select an existing secret" })}
          allowVersionSelector={false}
          emptyHint={t("jsonSchemaForm.noActiveSecrets", { defaultValue: "No active secrets yet. Create one or paste a raw value below." })}
          disabled={disabled}
        />
        {!isBoundToSecret ? (
          showRawInput ? (
            <div className="space-y-1">
              ...
              {!hasRawValue ? (
                <button
                  ...
                  disabled={disabled}
                >
                  {t("jsonSchemaForm.hideRawValueInput", { defaultValue: "Hide raw value input" })}
                </button>
              ) : null}
            </div>
          ) : (
            <button
              ...
              disabled={disabled}
            >
              {t("jsonSchemaForm.orPasteRawValue", { defaultValue: "Or paste a raw value" })}
            </button>
          )
        ) : null}
      </div>
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        >
          ...
          {isComplex
            ? t("jsonSchemaForm.addItem", { defaultValue: "Add item" })
            : t("jsonSchemaForm.add", { defaultValue: "Add" })}
        </Button>
      </div>
          ...
          >
            <div className="flex-1">
              <div className="mb-2 text-xs font-medium text-muted-foreground">
                {t("jsonSchemaForm.itemLabel", { defaultValue: "Item {{number}}", number: index + 1 })}
              </div>
              <FormField
            ...
            >
              ...
              <span className="sr-only">{t("jsonSchemaForm.removeItem", { defaultValue: "Remove item" })}</span>
            </Button>
          </div>
        ))}
        {items.length === 0 && (
          <div className="rounded-lg border border-dashed p-4 text-center text-xs text-muted-foreground">
            {t("jsonSchemaForm.noItemsAdded", { defaultValue: "No items added yet." })}
          </div>
        )}
      </div>
      {error && (
...
export function JsonSchemaForm({
  ...
  // Rules of Hooks: keep ALL hooks above the scalar early-return below — a
  // schema whose root type changes between renders would otherwise change the
  // hook count and crash ("Rendered more hooks than during the previous render").
  ...
  if (Object.keys(properties).length === 0) {
    return (
      ...
      >
        {t("jsonSchemaForm.noConfigOptions", { defaultValue: "No configuration options available." })}
      </div>
    );
  }
