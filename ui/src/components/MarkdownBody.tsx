...
function MarkdownIssueLink({
  ...
  return (
    <Link
      ...
      className="workcell-markdown-issue-ref"
      title={title}
...
function createWikiLinkNode(href: string, wikiLink: ParsedWikiLink): MarkdownAstNode {
  return {
    ...
    data: {
      hProperties: {
        "data-workcell-wiki-link": "true",
        "data-workcell-wiki-target": wikiLink.target,
      },
    },
...
function CodeBlock({
  ...
  return (
    <div className="workcell-markdown-codeblock" data-wrap-lines={wrapLines || undefined}>
      <pre
        {...preProps}
        ref={preRef}
        style={{
      ...
      <div
        className="workcell-markdown-codeblock-actions"
      ...
      >
        <button
          ...
          title={wrapLabel}
          className="workcell-markdown-codeblock-action workcell-markdown-codeblock-wrap"
          style={wrapLines
            ? {
        ...
        <button
          ...
          title={copyLabel}
          className="workcell-markdown-codeblock-action workcell-markdown-codeblock-copy"
        ...
        >
          ...
          <span className="workcell-markdown-codeblock-action-label">{copyLabel}</span>
        </button>
      </div>
    </div>
  );
...
function MermaidDiagramBlock({ source, darkMode }: { source: string; darkMode: boolean }) {
  ...
  useEffect(() => {
    ...
    loadMermaid()
      .then(async (mermaid) => {
        ...
        const rendered = await mermaid.render(`workcell-mermaid-${renderId}`, source);
        if (!active) return;
        setSvg(rendered.svg);
      })
      .catch((err) => {
  ...
  return (
    <div className="workcell-mermaid">
      {svg ? (
        <div dangerouslySetInnerHTML={{ __html: svg }} />
      ) : (
        <>
          <p className={cn("workcell-mermaid-status", error && "workcell-mermaid-status-error")}>
            {error ? `Unable to render Mermaid diagram: ${error}` : "Rendering Mermaid diagram..."}
          </p>
          <pre className="workcell-mermaid-source">
...
export function MarkdownBody({
  ...
  const components: Components = {
    ...
    table: ({ node: _node, style: tableStyle, children: tableChildren, ...tableProps }) => (
      <div className="workcell-markdown-table-scroll" role="region" aria-label="Scrollable table" tabIndex={0}>
    ...
    a: ({ node: _node, href, style: linkStyle, children: linkChildren, ...anchorProps }) => {
      ...
      const isWikiLink = dataProps["data-workcell-wiki-link"] === "true";
      ...
      if (parsed) {
        ...
        return (
          <a
            href={targetHref}
            className={cn(
              "workcell-mention-chip",
              `workcell-mention-chip--${parsed.kind}`,
              parsed.kind === "project" && "workcell-project-mention-chip",
            )}
  ...
  return (
    <div
      className={cn(
        "workcell-markdown prose prose-sm min-w-0 max-w-full break-words overflow-hidden",
