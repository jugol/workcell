...
import type { FeedbackDataSharingPreference, FeedbackVoteValue } from "@workcell/shared";
    ...
    <>
      <div className={cn(
        ...
        >
          ...
          {t("outputFeedback.helpful", { defaultValue: "Helpful" })}
        </Button>
        <Button
          type="button"
          size="sm"
        ...
        >
          ...
          {t("outputFeedback.needsWork", { defaultValue: "Needs work" })}
        </Button>
        {rightSlot ? <div className="ml-auto">{rightSlot}</div> : null}
      </div>
      {collectingDownvoteReason ? (
        <div className="mt-2 rounded-md border border-border/60 bg-accent/20 p-3">
          <div className="mb-2 text-sm font-medium">{t("outputFeedback.whatBetter", { defaultValue: "What could have been better?" })}</div>
          <Textarea
          ...
          <div className="mt-3 flex items-center justify-end gap-2">
            ...
            >
              {t("outputFeedback.dismiss", { defaultValue: "Dismiss" })}
            </Button>
            <Button
              type="button"
              size="sm"
            ...
            >
              {isSaving ? t("outputFeedback.saving", { defaultValue: "Saving..." }) : t("outputFeedback.saveNote", { defaultValue: "Save note" })}
            </Button>
          </div>
        </div>
      ) : null}
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("outputFeedback.dialog.title", { defaultValue: "Save your feedback sharing preference" })}</DialogTitle>
            <DialogDescription>
              {t("outputFeedback.dialog.description", { defaultValue: "Choose whether voted AI outputs can be shared with Workcell Labs. This answer becomes the default for future thumbs up and thumbs down votes." })}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-sm text-muted-foreground">
            <p>
              {t("outputFeedback.dialog.savedLocally", { defaultValue: "This vote is always saved locally." })}
            </p>
            <p>
              {t("outputFeedback.dialog.chooseBefore", { defaultValue: "Choose" })} <span className="font-medium text-foreground">{t("outputFeedback.alwaysAllow", { defaultValue: "Always allow" })}</span> {t("outputFeedback.dialog.chooseMiddle", { defaultValue: "to share this vote and future voted AI outputs. Choose" })}{" "}
              <span className="font-medium text-foreground">{t("outputFeedback.dontAllow", { defaultValue: "Don't allow" })}</span> {t("outputFeedback.dialog.chooseAfter", { defaultValue: "to keep this vote and future votes local." })}
            </p>
            <p>
              {t("outputFeedback.dialog.changeLater", { defaultValue: "You can change this later in Instance Settings > General." })}
            </p>
            {termsUrl ? (
              <a
                href={termsUrl}
              ...
              >
                {t("outputFeedback.dialog.terms", { defaultValue: "Read our terms of service" })}
              </a>
            ) : null}
          </div>
          <DialogFooter>
            ...
            >
              {isSaving ? t("outputFeedback.saving", { defaultValue: "Saving..." }) : t("outputFeedback.dontAllow", { defaultValue: "Don't allow" })}
            </Button>
            <Button
              type="button"
              disabled={!pendingVote || isSaving}
            ...
            >
              {isSaving ? t("outputFeedback.saving", { defaultValue: "Saving..." }) : t("outputFeedback.alwaysAllow", { defaultValue: "Always allow" })}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
