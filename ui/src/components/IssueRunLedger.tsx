...
import type { ActivityEvent, Issue, Agent } from "@workcell/shared";
...
const RETRY_PENDING_LIVENESS_COPY: LivenessCopy = {
  label: "Retry pending",
  tone: "border-cyan-500/30 bg-cyan-500/10 text-cyan-700 dark:text-cyan-300",
  description: "Workcell queued an automatic retry that has not started yet.",
};
...
function watchdogDecisionErrorMessage(error: unknown) {
  ...
  return error instanceof Error && error.message.trim().length > 0
    ? error.message
    : "Workcell could not record the watchdog decision.";
}
...
export function IssueRunLedgerContent({
  ...
  return (
    <section className="space-y-3" aria-label={t("issueRunLedger.sectionLabel", { defaultValue: "Issue run ledger" })}>
      <div className="flex items-center justify-between gap-2">
        <div className="min-w-0">
          <h3 className="text-sm font-medium text-muted-foreground">{t("issueRunLedger.title", { defaultValue: "Run ledger" })}</h3>
      ...
      {children.total > 0 ? (
        <div className="rounded-md border border-border/70 px-3 py-2">
          <div className="flex flex-wrap items-center gap-2 text-xs">
            <span className="font-medium text-foreground">{t("issueRunLedger.childWork", { defaultValue: "Child work" })}</span>
      ...
      {latestSilentRun?.outputSilence ? (
        ...
        >
          ...
          <p className="mt-1">
            ...
            {formatSilenceAge(latestSilentRun.outputSilence.silenceAgeMs) ?? t("issueRunLedger.watchdog.extendedPeriod", { defaultValue: "an extended period" })}.
            {latestSilentRun.outputSilence.evaluationIssueIdentifier ? (
              <>
                ...
                {" "}{t("issueRunLedger.forRecoveryContext", { defaultValue: "for recovery context." })}
              </>
            ) : null}
          </p>
          {onWatchdogDecision && canRecordWatchdogDecisions ? (
            <div className="mt-2 flex flex-wrap gap-1.5">
              ...
              >
                {t("issueRunLedger.watchdog.continue", { defaultValue: "Continue monitoring" })}
              </button>
              <button
                type="button"
                className="rounded-md border border-border bg-background/80 px-2 py-1 text-[11px] text-foreground hover:bg-background"
                ...
                disabled={pendingWatchdogDecision != null}
              >
                {t("issueRunLedger.watchdog.snooze", { defaultValue: "Snooze 1h" })}
              </button>
              <button
                type="button"
                className="rounded-md border border-border bg-background/80 px-2 py-1 text-[11px] text-foreground hover:bg-background"
                ...
                disabled={pendingWatchdogDecision != null}
              >
                {t("issueRunLedger.watchdog.falsePositive", { defaultValue: "Mark false positive" })}
              </button>
            </div>
          ) : null}
          {watchdogDecisionError ? (
      ...
      {feedItems.length === 0 ? (
        <div className="rounded-md border border-dashed border-border px-3 py-3 text-sm text-muted-foreground">
          {renderActivityEvent
            ? t("issueRunLedger.empty.withActivity", { defaultValue: "Runs and activity will appear here once this issue has history." })
            : t("issueRunLedger.empty.runsOnly", { defaultValue: "Historical runs without liveness metadata will appear here once linked to this issue." })}
        </div>
      ) : (
        <div className="space-y-1.5">
            ...
            return (
              ...
              >
                <div className="flex flex-wrap items-center gap-1.5">
                  ...
                  <span>{t("issueRunLedger.run.byAgent", { defaultValue: "by {{name}}", name: agentName })}</span>
                ...
                <div className="grid gap-2 text-xs text-muted-foreground sm:grid-cols-3">
                  <div className="min-w-0">
                    ...
                    {duration ?? t("issueRunLedger.run.unknown", { defaultValue: "unknown" })}
                  </div>
                  <div className="min-w-0">
                  ...
                  return (
                    <p className="min-w-0 break-words text-[11px] leading-5 text-amber-700 dark:text-amber-300">
                      {profile.requested === "cheap"
                        ? t("issueRunLedger.run.cheapProfileFallback", { defaultValue: "Cheap profile fell back to primary" })
          ...
          {feedItems.length > 20 ? (
            <div className="px-3 py-2 text-xs text-muted-foreground">
              {t("issueRunLedger.olderItemsNotShown", { defaultValue: "{{count}} older items not shown", count: feedItems.length - 20 })}
            </div>
          ) : null}
        </div>
      )}
