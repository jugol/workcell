...
export function IssueSiblingNavigation({ navigation, linkState }: IssueSiblingNavigationProps) {
  ...
  return (
    <nav
      aria-label={t("issueSiblingNavigation.navAriaLabel", { defaultValue: "Sub-issue navigation" })}
...
function SiblingLink({
  ...
  const label = direction === "previous"
    ? t("issueSiblingNavigation.previousLabel", { defaultValue: "Previous" })
    : t("issueSiblingNavigation.nextLabel", { defaultValue: "Next" });
  const ariaDirection = direction === "previous"
    ? t("issueSiblingNavigation.previousAriaDirection", { defaultValue: "Previous sub-issue" })
    : t("issueSiblingNavigation.nextAriaDirection", { defaultValue: "Next sub-issue" });
  ...
  return (
    <Link
      ...
      aria-label={t("issueSiblingNavigation.linkAriaLabel", {
        defaultValue: "{{ariaDirection}}: {{identifier}} - {{title}}",
        ariaDirection,
        identifier,
        title: issue.title,
      })}
