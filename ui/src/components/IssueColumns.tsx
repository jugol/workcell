  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-[300px] rounded-xl border-border/70 p-1.5 shadow-xl shadow-black/10">
        <DropdownMenuLabel className="px-2 pb-1 pt-1.5">
          <div className="space-y-1">
            <div className="text-[10px] font-semibold uppercase tracking-[0.22em] text-muted-foreground">
              {t("issueColumns.desktopIssueRows", { defaultValue: "Desktop issue rows" })}
            </div>
        ...
        >
          {t("issueColumns.resetDefaults", { defaultValue: "Reset defaults" })}
...
export function InboxIssueTrailingColumns({
  ...
  const userLabel = assigneeUserName ?? formatAssigneeUserLabel(issue.assigneeUserId, currentUserId) ?? t("issueColumns.user", { defaultValue: "User" });
    ...
    >
      {columns.map((column) => {
        ...
        if (column === "project") {
          ...
          return (
            <span key={column} className="min-w-0 truncate text-xs text-muted-foreground">
              {t("issueColumns.noProject", { defaultValue: "No project" })}
            </span>
          );
        }
              ...
              {workspaceId && onFilterWorkspace ? (
                <Tooltip>
                  ...
                  <TooltipContent side="top" sideOffset={6}>
                    {t("issueColumns.filterByWorkspace", { defaultValue: "Filter by workspace" })}
                  </TooltipContent>
                </Tooltip>
              ) : (
                workspaceName
          ...
          return (
            <span key={column} className="min-w-0 truncate text-xs text-muted-foreground" title={parentTitle ?? undefined}>
              {parentIdentifier ? (
                ...
                <span className="italic">{t("issueColumns.subIssue", { defaultValue: "Sub-issue" })}</span>
              )}
            </span>
          );
        }
