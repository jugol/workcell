...
import { AGENT_ROLE_LABELS, ISSUE_STATUSES, type Issue, type IssueStatus, type Project } from "@workcell/shared";
...
interface Agent {
  id: string;
  name: string;
  // WC-10: optional so callers that only thread id+name keep working. When
  // present, the list-view row shows the same owner-role chip the Kanban card
  // shows next to the assignee identity (WC-8 parity).
  role?: string | null;
}
...
interface IssuesListProps {
  ...
  isLoading?: boolean;
  error?: Error | null;
  // Refetch callback for the error banner. With placeholderData the list keeps
  // rendering the LAST successful page set when a refetch fails — without an
  // explicit "stale + retry" banner that silently reads as "issues missing".
  onRetry?: () => void;
  agents?: Agent[];
  ...
  return (
    <div className="relative w-48 sm:w-64 md:w-80">
      ...
      <Input
        ...
        aria-label={t("issues.searchAriaLabel", { defaultValue: "Search issues" })}
        ...
        <div className="min-w-0 flex-1 space-y-2">
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm">
            <span className="font-medium text-foreground">
              {t("issues.subIssueStrip.done", { defaultValue: "{{done}}/{{total}} done", done: summary.doneCount, total: summary.totalCount })}
            </span>
            <span className="text-muted-foreground">
              {t("issues.subIssueStrip.inProgress", { defaultValue: "{{count}} in progress", count: summary.inProgressCount })}
            </span>
            <span className="text-muted-foreground">
              {t("issues.subIssueStrip.blocked", { defaultValue: "{{count}} blocked", count: summary.blockedCount })}
            </span>
            {showCostSummary && (
              <>
                <span
                ...
                >
                  {t("issues.subIssueStrip.tokens", { defaultValue: "{{tokens}} tokens", tokens: formatTokens(totalTokens) })}
                </span>
                <span className="text-muted-foreground tabular-nums">
                  {t("issues.subIssueStrip.runtime", { defaultValue: "{{duration}} runtime", duration: formatDurationMs(costSummary.runtimeMs) })}
                </span>
              </>
            )}
          </div>
          <div
            role="progressbar"
            aria-label={t("issues.subIssueStrip.progressAria", { defaultValue: "Sub-issues completion progress" })}
            aria-valuemin={0}
          ...
          {target && targetIssue ? (
            <>
              <div className="text-xs font-medium text-muted-foreground">
                {target.kind === "next"
                  ? t("issues.subIssueStrip.nextUp", { defaultValue: "Next up" })
                  : t("issues.subIssueStrip.waitingOnBlockers", { defaultValue: "Waiting on blockers" })}
              </div>
              <Link
            ...
            <div className="text-sm font-medium text-foreground">{t("issues.subIssueStrip.noActive", { defaultValue: "No active sub-issues" })}</div>
          ) : summary.doneCount === summary.totalCount ? (
            <div className="text-sm font-medium text-foreground">{t("issues.subIssueStrip.allDone", { defaultValue: "All sub-issues done" })}</div>
          ) : (
            <div className="text-sm font-medium text-foreground">{t("issues.subIssueStrip.noActionable", { defaultValue: "No actionable sub-issues" })}</div>
          )}
        </div>
      </div>
    </div>
...
export function IssuesList({
  ...
  // WC-10: full-agent lookup so the row can read both name and role from a
  // single pass. Kept alongside agentName so existing call-sites that only
  // need the display name aren't disturbed.
  const agentForId = useCallback((id: string | null): Agent | null => {
    if (!id || !agents) return null;
    ...
    <div ref={rootRef} className="space-y-4">
        ...
        <div className="flex items-center gap-0.5 sm:gap-1 shrink-0">
          ...
          <div className="flex items-center border border-border rounded-md overflow-hidden mr-1">
            <button
              ...
              title={t("issues.listViewTooltip", { defaultValue: "List view" })}
            >
            ...
            <button
              ...
              title={t("issues.boardViewTooltip", { defaultValue: "Board view" })}
            >
          ...
          {viewState.viewMode === "list" && (
            <Button
              ...
              title={viewState.nestingEnabled
                ? t("issues.nestingDisableTooltip", { defaultValue: "Disable parent-child nesting" })
                : t("issues.nestingEnableTooltip", { defaultValue: "Enable parent-child nesting" })}
            >
            ...
            <>
              <Button
                ...
                title={boardCompactCards
                  ? t("issues.boardUseComfortableCards", { defaultValue: "Use comfortable cards" })
                  : t("issues.boardUseCompactCards", { defaultValue: "Use compact cards" })}
              >
              ...
              <Button
                ...
                title={boardCollapsedStatuses.length > 0
                  ? t("issues.boardExpandColdLanes", { defaultValue: "Expand cold lanes" })
                  : t("issues.boardCollapseColdLanes", { defaultValue: "Collapse cold lanes" })}
              >
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    ...
                    title={t("issues.cardsPerColumnTooltip", { defaultValue: "Cards per column" })}
                  >
                ...
                <PopoverContent align="end" className="w-40 p-0">
                  <div className="p-2 space-y-0.5">
                    {KANBAN_COLUMN_PAGE_SIZE_OPTIONS.map((pageSize) => (
                      ...
                      >
                        <span>{t("issues.perColumn", { defaultValue: "{{count}} per column", count: pageSize })}</span>
              ...
              <Button
                ...
                title={t("issues.resetBoardDensityTooltip", { defaultValue: "Reset board density" })}
              >
          ...
          <IssueColumnPicker
            ...
            title={t("issues.columnsPickerTooltip", { defaultValue: "Choose which issue columns stay visible" })}
            iconOnly
          />
      ...
      {error && (
        <div
          ...
          data-testid="issues-list-error-banner"
        >
          <p className="text-sm text-destructive">
            {t("issues.loadError", {
              defaultValue:
                "Could not refresh the issue list — what you see may be stale or incomplete.",
            })}
          ...
          {onRetry ? (
            <Button
              ...
              onClick={onRetry}
              data-testid="issues-list-error-retry"
            >
              {t("issues.retry", { defaultValue: "Retry" })}
            </Button>
          ) : null}
        </div>
      )}
      {!searchWithinLoadedIssues && normalizedIssueSearch.length > 0 && searchedIssues.length === ISSUE_SEARCH_RESULT_LIMIT && (
        <p className="text-xs text-muted-foreground">
          {t("issues.searchLimit", { defaultValue: "Showing up to {{count}} matches. Refine the search to narrow further.", count: ISSUE_SEARCH_RESULT_LIMIT })}
        </p>
      )}
      {boardColumnLimitReached && (
        <p className="text-xs text-muted-foreground">
          {t("issues.boardColumnLimit", { defaultValue: "Some board columns are showing up to {{count}} issues. Refine filters or search to reveal the rest.", count: ISSUE_BOARD_COLUMN_RESULT_LIMIT })}
        </p>
      )}
      {!isLoading && filtered.length === 0 && viewState.viewMode === "list" && (
        <EmptyState
          icon={CircleDot}
          message={t("issues.emptyState", { defaultValue: "No issues match the current filters or search." })}
                          ...
                          <>
                            ...
                            {issueBadge ? (
                              issueBadge === "Paused" ? (
                                <span
                                  ...
                                  aria-label={t("issues.pausedBadge", { defaultValue: "Paused" })}
                                  title={t("issues.pausedBadge", { defaultValue: "Paused" })}
                                >
                                  <CircleSlash2 className="h-3 w-3" />
                                  {t("issues.pausedBadge", { defaultValue: "Paused" })}
                                </span>
                              ) : (
                            ...
                            {isSuccessfulRunHandoffRequired(issue) ? (
                              <span
                                ...
                                aria-label={t("issues.needsNextStepAria", { defaultValue: "Needs next step" })}
                                title={t("issues.needsNextStepTooltip", { defaultValue: "This issue needs a next step" })}
                              >
                                <CircleDot className="h-3 w-3" />
                                {t("issues.needsNextStepLabel", { defaultValue: "Needs next step" })}
                              </span>
                            ) : null}
                          </>
                        )}
                              ...
                              assigneeContent={(
                                ...
                                >
                                  <PopoverTrigger asChild>
                                    ...
                                    >
                                      {issue.assigneeAgentId && agentForId(issue.assigneeAgentId) ? (() => {
                                        // WC-10: mirror the KanbanCard assignee block (WC-8/9 chips).
                                        // Chips are only shown for agent assignees because role + proof
                                        // are agent-execution concepts; user-assigned work follows a
                                        // different sign-off flow.
                                        const agent = agentForId(issue.assigneeAgentId)!;
                                        const roleKey = typeof agent.role === "string" ? agent.role : null;
                                        const roleLabel = roleKey
                                          ? (AGENT_ROLE_LABELS as Record<string, string>)[roleKey] ?? roleKey
                                          : null;
                                        return (
                                          <span className="flex min-w-0 items-center gap-1.5">
                                            ...
                                            {roleLabel ? (
                                              <Badge
                                                variant="outline"
                                                className="px-1.5 py-0 text-[10px] font-medium leading-tight shrink-0"
                                                title={`Owner role: ${roleLabel}`}
                                                aria-label={`Owner role: ${roleLabel}`}
                                              >
                                                {roleLabel}
                                              </Badge>
                                            ) : null}
                                            {issue.hasProof === true ? (
                                              <Badge
                                                variant="outline"
                                                className="px-1.5 py-0 text-[10px] font-medium leading-tight shrink-0 border-emerald-500/45 bg-emerald-50/60 text-emerald-700 dark:border-emerald-300/35 dark:bg-emerald-400/10 dark:text-emerald-300"
                                                title={t("issues.proofBadgeTooltip", { defaultValue: "Proof bundle attached — ready for sign-off" })}
                                                aria-label={t("issues.proofBadgeAria", { defaultValue: "Proof attached" })}
                                              >
                                                {t("issues.proofBadgeLabel", { defaultValue: "Proof" })}
                                              </Badge>
                                            ) : null}
                                            {typeof issue.totalCostCents === "number" && issue.totalCostCents > 0 ? (() => {
                                              // WC-11: same formatting rule as KanbanCard — sub-$100 keeps two
                                              // decimals; $100+ rounds to whole dollars so the chip stays compact.
                                              const label = issue.totalCostCents < 10_000
                                                ? `$${(issue.totalCostCents / 100).toFixed(2)}`
                                                : `$${Math.round(issue.totalCostCents / 100)}`;
                                              return (
                                                <Badge
                                                  variant="outline"
                                                  className="px-1.5 py-0 text-[10px] font-medium leading-tight shrink-0 font-mono"
                                                  title={`Cost accumulated: $${(issue.totalCostCents / 100).toFixed(2)}`}
                                            ...
                                            {/* WC-18: compound-origin chip — mirrors KanbanCard. Only the
                                                "compound_followup" origin gets a chip so the user can spot
                                                follow-ups spawned by Process follow-ups in a busy backlog.
                                                Other originKinds stay unchipped (would be too noisy). */}
                                            {issue.originKind === "compound_followup" ? (
                                              <Badge
                                                variant="outline"
                                                className="px-1.5 py-0 text-[10px] font-medium leading-tight shrink-0 border-indigo-500/45 bg-indigo-50/60 text-indigo-700 dark:border-indigo-300/35 dark:bg-indigo-400/10 dark:text-indigo-300"
                                                title={t("issues.compoundBadgeTooltip", { defaultValue: "Spawned from a compound-checklist follow-up" })}
                                                aria-label={t("issues.compoundBadgeAria", { defaultValue: "From compound checklist" })}
                                              >
                                                {t("issues.compoundBadgeLabel", { defaultValue: "Compound" })}
                                              </Badge>
                                            ) : null}
                                            {/* WC-34: pair-mode chip mirrors KanbanCard. */}
                                            {issue.workOwnerKind === "pair" ? (
                                              <Badge
                                                variant="outline"
                                                className="px-1.5 py-0 text-[10px] font-medium leading-tight shrink-0 border-violet-500/45 bg-violet-50/60 text-violet-700 dark:border-violet-300/35 dark:bg-violet-400/10 dark:text-violet-300"
                                                title={t("issues.pairBadgeTooltip", { defaultValue: "Pair collaboration mode (multi-agent rounds)" })}
                                                aria-label={t("issues.pairBadgeAria", { defaultValue: "Pair mode" })}
                                              >
                                                {t("issues.pairBadgeLabel", { defaultValue: "Pair" })}
                                              </Badge>
                                            ) : null}
                                          </span>
                                        );
                                        ...
                                        <span className="inline-flex items-center gap-1.5 text-xs text-muted-foreground">
                                          ...
                                          {t("issues.assigneePlaceholder", { defaultValue: "Assignee" })}
                                        </span>
                                      )}
                                    </button>
                                  </PopoverTrigger>
                                  ...
                                  >
                                    <input
                                      ...
                                      placeholder={t("issues.searchAssigneesPlaceholder", { defaultValue: "Search assignees..." })}
                                    ...
                                    <div className="max-h-48 overflow-y-auto overscroll-contain">
                                      ...
                                      >
                                        {t("issues.assigneeNone", { defaultValue: "No assignee" })}
                                      </button>
                                      {currentUserId && (
                                        <button
                                          className={cn(
                                        ...
                                        >
                                          ...
                                          <span>{t("issues.assigneeMe", { defaultValue: "Me" })}</span>
                                        </button>
                                      )}
                                      {(agents ?? [])
                                        .filter((agent) => {
