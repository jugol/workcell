  ...
  return (
    <DropdownMenu open={open} onOpenChange={setOpen}>
        ...
        >
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="text-sm font-medium truncate">
              {selectedCompany?.name ?? t("companySwitcher.selectCompany", { defaultValue: "Select team" })}
            </span>
          </div>
      ...
      <DropdownMenuContent align="start" className="w-[220px]">
        <DropdownMenuLabel>{t("companySwitcher.companies", { defaultValue: "Teams" })}</DropdownMenuLabel>
        <DropdownMenuSeparator />
        ...
        {sidebarCompanies.length === 0 && (
          <DropdownMenuItem disabled>{t("companySwitcher.noCompanies", { defaultValue: "No teams" })}</DropdownMenuItem>
        )}
        {archivedCompanies.length > 0 && (
          <>
            <DropdownMenuSeparator />
            <DropdownMenuLabel className="text-muted-foreground">
              {t("companySwitcher.archived", { defaultValue: "Archived" })}
            </DropdownMenuLabel>
        ...
        <DropdownMenuItem asChild>
          <Link to="/company/settings" className="no-underline text-inherit">
            ...
            {t("companySwitcher.companySettings", { defaultValue: "Team Settings" })}
          </Link>
        </DropdownMenuItem>
        <DropdownMenuItem asChild>
          <Link to="/companies" className="no-underline text-inherit">
            <Plus className="h-4 w-4 mr-2" />
            {t("companySwitcher.manageCompanies", { defaultValue: "Manage Teams" })}
          </Link>
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
