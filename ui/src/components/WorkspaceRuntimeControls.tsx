    ...
    <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:flex-wrap">
      {actions.map((action) => {
        ...
        const label = action === "run"
          ? t("workspaceRuntimeControls.action.run", { defaultValue: "Run" })
          : action === "start"
            ? t("workspaceRuntimeControls.action.start", { defaultValue: "Start" })
            : action === "stop"
              ? t("workspaceRuntimeControls.action.stop", { defaultValue: "Stop" })
              : t("workspaceRuntimeControls.action.restart", { defaultValue: "Restart" });
            ...
            <div key={item.key} className={cn("border border-border/80 bg-background px-3 py-3", square ? "rounded-none" : "rounded-xl")}>
              <div className="flex flex-col gap-3">
                ...
                <div className="space-y-1 text-xs text-muted-foreground">
                  ...
                  {item.port ? <div>{t("workspaceRuntimeControls.port", { defaultValue: "Port {{port}}", port: item.port })}</div> : null}
...
export function WorkspaceRuntimeControls({
  ...
  const { t } = useTranslation();
  const resolvedServiceEmptyMessageDefault =
    serviceEmptyMessage
    ?? t("workspaceRuntimeControls.servicesEmpty", { defaultValue: "No services are configured for this workspace." });
  const resolvedJobEmptyMessage =
    jobEmptyMessage
    ?? t("workspaceRuntimeControls.jobsEmpty", { defaultValue: "No one-shot jobs are configured for this workspace." });
  ...
  return (
    <div className={cn("space-y-4", className)}>
      <div className={cn("border border-border/70 bg-background p-3", square ? "rounded-none" : "rounded-xl")}>
        <div className="space-y-1">
          <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{t("workspaceRuntimeControls.heading", { defaultValue: "Workspace commands" })}</div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              ...
              {runningCount > 0
                ? t("workspaceRuntimeControls.servicesRunning", { defaultValue: "{{count}} services running", count: runningCount })
                : t("workspaceRuntimeControls.noServicesRunning", { defaultValue: "No services running" })}
            </span>
            <span className="text-xs text-muted-foreground">
              {resolvedSections.jobs.length > 0
                ...
                : t("workspaceRuntimeControls.commandsIndependent", { defaultValue: "Each command can be controlled independently." })}
            </span>
          </div>
      ...
      <CommandSection
        title={t("workspaceRuntimeControls.servicesTitle", { defaultValue: "Services" })}
        description={t("workspaceRuntimeControls.servicesDescription", { defaultValue: "Long-running commands that Workcell can supervise for this workspace." })}
      ...
      <CommandSection
        title={t("workspaceRuntimeControls.jobsTitle", { defaultValue: "Jobs" })}
        description={t("workspaceRuntimeControls.jobsDescription", { defaultValue: "One-shot commands that run now and exit when they finish." })}
      ...
      {resolvedSections.otherServices.length > 0 ? (
        <CommandSection
          title={t("workspaceRuntimeControls.untrackedTitle", { defaultValue: "Untracked services" })}
          description={t("workspaceRuntimeControls.untrackedDescription", { defaultValue: "Running services that no longer match the current workspace command config." })}
