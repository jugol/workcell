...
import type { IssueProductivityReview } from "@workcell/shared";
...
export function productivityReviewTriggerLabel(
  ...
  if (!trigger) return t("productivityReviewBadge.productivityReview", { defaultValue: "Productivity review" });
  return TRIGGER_LABELS[trigger] ?? t("productivityReviewBadge.productivityReview", { defaultValue: "Productivity review" });
}
    ...
    <Tooltip>
      ...
      <TooltipContent>
        <div className="space-y-1 text-xs">
          ...
          <div>
            <span className="text-muted-foreground">{t("productivityReviewBadge.trigger", { defaultValue: "Trigger:" })}</span> {label}
          </div>
          {typeof review.noCommentStreak === "number" && review.noCommentStreak > 0 ? (
            <div>
              <span className="text-muted-foreground">{t("productivityReviewBadge.noCommentStreak", { defaultValue: "No-comment streak:" })}</span>{" "}
              {t("productivityReviewBadge.runs", { defaultValue: "{{count}} runs", count: review.noCommentStreak })}
            </div>
          ) : null}
          <div>
            <span className="text-muted-foreground">{t("productivityReviewBadge.review", { defaultValue: "Review:" })}</span> {reviewIdentifier} ({statusLabel})
          </div>
        </div>
      </TooltipContent>
    </Tooltip>
