  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button variant={buttonVariant} size={iconOnly ? "icon" : "sm"} className={`text-xs ${iconOnly ? "relative h-8 w-8 shrink-0" : ""} ${activeFilterCount > 0 ? "text-blue-600 dark:text-blue-400" : ""}`} title={iconOnly ? (activeFilterCount > 0 ? t("issueFiltersPopover.trigger.withCount", { defaultValue: "Filters: {{count}}", count: activeFilterCount }) : t("issueFiltersPopover.trigger.label", { defaultValue: "Filter" })) : undefined}>
          <Filter className={iconOnly ? "h-3.5 w-3.5" : "h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1"} />
          {!iconOnly && <span className="hidden sm:inline">{activeFilterCount > 0 ? t("issueFiltersPopover.trigger.withCount", { defaultValue: "Filters: {{count}}", count: activeFilterCount }) : t("issueFiltersPopover.trigger.label", { defaultValue: "Filter" })}</span>}
      ...
      >
        <div className="space-y-3 p-3">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium">{t("issueFiltersPopover.heading", { defaultValue: "Filters" })}</span>
            {activeFilterCount > 0 ? (
              <button
                type="button"
              ...
              >
                {t("issueFiltersPopover.clear", { defaultValue: "Clear" })}
              </button>
            ) : null}
          </div>

          <div className="space-y-1.5">
            <span className="text-xs text-muted-foreground">{t("issueFiltersPopover.quickFilters", { defaultValue: "Quick filters" })}</span>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <div className="min-w-0 space-y-3">
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("issueFiltersPopover.section.status", { defaultValue: "Status" })}</span>
              ...
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("issueFiltersPopover.section.priority", { defaultValue: "Priority" })}</span>
                <div className="space-y-0.5">
            ...
            <div className="min-w-0 space-y-3">
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("issueFiltersPopover.section.assignee", { defaultValue: "Assignee" })}</span>
                <div className="max-h-32 space-y-0.5 overflow-y-auto">
                  <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                    ...
                    />
                    <span className="text-sm">{t("issueFiltersPopover.noAssignee", { defaultValue: "No assignee" })}</span>
                  </label>
                  {currentUserId ? (
                    <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                      <Checkbox
                      ...
                      <span className="text-sm">{t("issueFiltersPopover.me", { defaultValue: "Me" })}</span>
                    </label>
                  ) : null}
                  {(agents ?? []).map((agent) => (
              ...
              {creatorOptions.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("issueFiltersPopover.section.creator", { defaultValue: "Creator" })}</span>
                      ...
                      {selectedCreatorOptions.map((creator) => (
                        <Badge key={creator.id} variant="secondary" className="gap-1 pr-1">
                          ...
                          <button
                            ...
                            aria-label={t("issueFiltersPopover.removeCreator", { defaultValue: "Remove creator {{creator}}", creator: creator.label })}
                          >
                            <X className="h-3 w-3" />
                          </button>
                        </Badge>
                  ...
                  <div className="relative">
                    ...
                    <Input
                      ...
                      placeholder={t("issueFiltersPopover.searchCreatorsPlaceholder", { defaultValue: "Search creators..." })}
                  ...
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
                    {visibleCreatorOptions.length > 0 ? visibleCreatorOptions.map((creator) => {
                      ...
                      <div className="px-2 py-1 text-xs text-muted-foreground">{t("issueFiltersPopover.noCreatorsMatch", { defaultValue: "No creators match." })}</div>
                    )}
                  </div>
                </div>
              ) : null}
              ...
              {projects && projects.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("issueFiltersPopover.section.project", { defaultValue: "Project" })}</span>
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
            ...
            <div className="min-w-0 space-y-3">
              {labels && labels.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("issueFiltersPopover.section.labels", { defaultValue: "Labels" })}</span>
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
                    {labels.map((label) => (
              ...
              {workspaces && workspaces.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("issueFiltersPopover.section.workspace", { defaultValue: "Workspace" })}</span>
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
              ...
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("issueFiltersPopover.section.visibility", { defaultValue: "Visibility" })}</span>
                <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                  <Checkbox
                    checked={state.liveOnly}
                    onCheckedChange={(checked) => onChange({ liveOnly: checked === true })}
                  />
                  <span className="text-sm">{t("issueFiltersPopover.liveRunsOnly", { defaultValue: "Live runs only" })}</span>
                </label>
                {enableRoutineVisibilityFilter ? (
                  <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                    ...
                    />
                    <span className="text-sm">{t("issueFiltersPopover.hideRoutineRuns", { defaultValue: "Hide routine runs" })}</span>
                  </label>
                ) : null}
              </div>
            </div>
