...
function createExecutionWorkspace(overrides: Partial<ExecutionWorkspace> = {}): ExecutionWorkspace {
  return {
    ...
    status: "active",
    cwd: "/tmp/workcell/PAP-1",
    repoUrl: null,
    baseRef: "master",
    branchName: "pap-1-workspace",
    providerType: "git_worktree",
    providerRef: "/tmp/workcell/PAP-1",
...
function createProject(overrides: Partial<Project> = {}): Project {
  ...
  return {
    ...
    codebase: {
      ...
      defaultRef: "master",
      repoName: null,
      localFolder: "/tmp/workcell",
      managedFolder: "/tmp/workcell",
      effectiveLocalFolder: "/tmp/workcell",
      origin: "local_folder",
    },
...
describe("IssueProperties", () => {
  ...
  // WC-185: pair must be a first-class "who works this issue" choice in the
  // assignee picker, not only single agents/users.
  it("offers a reachable Pair option in the assignee picker that opens pair setup", async () => {
    const onSetupPair = vi.fn();
    const onUpdate = vi.fn();
    const root = renderProperties(container, {
      issue: createIssue(),
      childIssues: [],
    ...
    const pairOption = container.querySelector('[data-testid="assignee-pair-option"]');
    expect(pairOption).not.toBeNull();
    expect(pairOption?.textContent).toContain("Pair (two agents)");

    await act(async () => {
      pairOption!.dispatchEvent(new MouseEvent("click", { bubbles: true }));
    });

    // Selecting Pair routes to the issue-level pair setup — it must NOT stamp a
    // single assignee on the issue.
    expect(onSetupPair).toHaveBeenCalledTimes(1);
    expect(onUpdate).not.toHaveBeenCalled();

    act(() => root.unmount());
  });

  it("hides the assignee Pair option when the issue is already a pair", async () => {
    const root = renderProperties(container, {
      issue: createIssue({ workOwnerKind: "pair" }),
      childIssues: [],
      onUpdate: vi.fn(),
      onSetupPair: vi.fn(),
    });
    ...
    expect(container.querySelector('[data-testid="assignee-pair-option"]')).toBeNull();

    act(() => root.unmount());
  });

  it("omits the assignee Pair option when no pair-setup handler is provided", async () => {
    const root = renderProperties(container, {
      issue: createIssue(),
      childIssues: [],
      onUpdate: vi.fn(),
