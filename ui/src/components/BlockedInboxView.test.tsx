...
describe("BlockedInboxView", () => {
  ...
  it("surfaces the action directive next to the title and keeps the reason in its column", async () => {
    mockIssuesApi.list.mockResolvedValue([
      makeIssue(
        "issue-decision",
        "PAP-4",
    ...
    expect(rowText).toContain("Pending board decision"); // title
    // The action directive ("what to do") is now surfaced, right after the title.
    expect(rowText).toContain("Accept or reject");
    expect(rowText.indexOf("Accept or reject")).toBeGreaterThan(rowText.indexOf("Pending board decision"));
    expect(container.querySelector('[data-testid="inbox-action-hint"]')?.textContent).toContain("Accept or reject");
    // The reason ("why") stays in its dedicated desktop column.
