...
import type { BudgetIncident } from "@workcell/shared";
...
function incidentStateLabel(incident: BudgetIncident) {
  ...
  if (incident.approvalStatus === "revision_requested") return t("budgetIncidentCard.state.escalated", { defaultValue: "Escalated" });
  if (incident.approvalStatus === "pending") return t("budgetIncidentCard.state.pending", { defaultValue: "Pending approval" });
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <div className="text-[11px] uppercase tracking-[0.22em] text-red-200/80">
                {t("budgetIncidentCard.hardStop", { defaultValue: "{{scope}} hard stop", scope: incident.scopeType })}
              </div>
            ...
            <CardDescription className="mt-1 text-red-100/70">
              {t("budgetIncidentCard.spendingReached", { defaultValue: "Spending reached {{observed}} against a limit of {{limit}}.", observed: formatCents(incident.amountObserved), limit: formatCents(incident.amountLimit) })}
            </CardDescription>
          </div>
      ...
      <CardContent className="space-y-4 px-5 pb-5 pt-0">
        <div className="flex items-start gap-2 rounded-xl border border-red-400/20 bg-red-500/10 px-3 py-2 text-sm text-red-50/90">
          ...
          <div>
            {incident.scopeType === "project"
              ? t("budgetIncidentCard.pausedProject", { defaultValue: "Project execution is paused. New work in this project will not start until you resolve the budget incident." })
              : t("budgetIncidentCard.pausedScope", { defaultValue: "This scope is paused. New heartbeats will not start until you resolve the budget incident." })}
          </div>
        </div>

        <div className="rounded-xl border border-border/60 bg-background/60 p-3">
          <label className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">
            {t("budgetIncidentCard.newBudget", { defaultValue: "New budget (USD)" })}
          </label>
          <div className="mt-2 flex flex-col gap-3 sm:flex-row">
            ...
            >
              ...
              {isMutating ? t("budgetIncidentCard.applying", { defaultValue: "Applying..." }) : t("budgetIncidentCard.raiseAndResume", { defaultValue: "Raise budget & resume" })}
            </Button>
          </div>
          {parsed !== null && parsed <= incident.amountObserved ? (
            <p className="mt-2 text-xs text-red-200/80">
              {t("budgetIncidentCard.mustExceed", { defaultValue: "The new budget must exceed current observed spend." })}
            </p>
          ) : null}
        </div>
        ...
        <div className="flex justify-end">
          <Button variant="ghost" className="text-muted-foreground" disabled={isMutating} onClick={onKeepPaused}>
            {t("budgetIncidentCard.keepPaused", { defaultValue: "Keep paused" })}
          </Button>
        </div>
      </CardContent>
    </Card>
