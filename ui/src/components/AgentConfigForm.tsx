...
// WC-208: the adapter choices offered per deliberation brain. The empty value
// means "inherit the agent's own adapter" (stored as adapter: null).
const DELIBERATION_BRAIN_ADAPTER_OPTIONS = ["claude_local", "codex_local"] as const;
...
export function AgentConfigForm(props: AgentConfigFormProps) {
  ...
  // WC-205: per-brain model picker popovers for the deliberation section.
  const [deliberationBrainAOpen, setDeliberationBrainAOpen] = useState(false);
    ...
    <div className={cn("relative", cards && "space-y-6")}>
        ...
        <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            ...
            >
              ...
              >
                <option value="">{t("agentConfigForm.companyDefaultOption", { defaultValue: "Company default (Local)" })}</option>
      ...
      {!isCreate && (
        <div className={cn(!cards && "border-b border-border")} data-testid="deliberation-section">
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("agentConfigForm.deliberation.heading", { defaultValue: "Deliberation (내부 합의 · dual-brain)" })}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("agentConfigForm.deliberation.heading", { defaultValue: "Deliberation (내부 합의 · dual-brain)" })}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <ToggleField
              label={t("agentConfigForm.deliberation.enabledLabel", { defaultValue: "Deliberation 켜기" })}
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              {t("agentConfigForm.deliberation.hint", { defaultValue: "켜면 이 에이전트는 내부에서 두 brain(A·B)이 제안→검토→수정/Accept를 주고받아 합의된 결과만 내놓습니다. (역할 무관 — pair 모드)" })}
            </p>
            {currentDeliberation.enabled && (
              <div className="space-y-3" data-testid="deliberation-config">
                <div data-testid="deliberation-brain-a" className="space-y-2">
                  <div className="text-[11px] uppercase tracking-wide text-muted-foreground mb-1">
                    {t("agentConfigForm.deliberation.brainALabel", { defaultValue: "brain A 모델" })}
                  </div>
                  <DeliberationBrainAdapterSelect
                    value={currentDeliberation.brainA.adapter ?? ""}
                    onChange={(v) => setDeliberationBrainField("brainA", "adapter", v)}
                    testId="deliberation-brain-a-adapter"
                  />
                  <ModelDropdown
                    models={models}
                    value={currentDeliberation.brainA.model ?? ""}
                    onChange={(v) => setDeliberationBrainField("brainA", "model", v)}
                    open={deliberationBrainAOpen}
                    onOpenChange={setDeliberationBrainAOpen}
                    allowDefault
                    required={false}
                    groupByProvider={false}
                    creatable
                    defaultLabel={t("agentConfigForm.deliberation.inheritDefault", { defaultValue: "Default (에이전트 기본 모델)" })}
                  />
                </div>
                <div data-testid="deliberation-brain-b" className="space-y-2">
                  <div className="text-[11px] uppercase tracking-wide text-muted-foreground mb-1">
                    {t("agentConfigForm.deliberation.brainBLabel", { defaultValue: "brain B 모델" })}
                  </div>
                  <DeliberationBrainAdapterSelect
                ...
                <Field
                  label={t("agentConfigForm.deliberation.maxRoundsLabel", { defaultValue: "최대 라운드 (maxRounds)" })}
                  hint={t("agentConfigForm.deliberation.maxRoundsHint", { defaultValue: "A↔B가 Accept 없이 주고받을 수 있는 최대 횟수 (1–8)." })}
                >
                  <DraftNumberInput
                ...
                <p className="text-[11px] text-muted-foreground/80">
                  {t("agentConfigForm.deliberation.pairNote", { defaultValue: "참고: 이 dual-brain은 한 에이전트의 내부 합의입니다. 두 에이전트가 협업하는 기존 Pair 설정과는 별개입니다." })}
                </p>
              </div>
            )}
          </div>
...
// is "inherit the agent's own adapter" (stored as adapter: null); the explicit
// options are the local CLI adapters a brain can independently run on. A plain
// native <select> (same primitive as the Execution > Default environment field)
// keeps it simple + testable (a change event drives onChange).
function DeliberationBrainAdapterSelect({
  ...
  return (
    ...
    >
      <option value="">
        {t("agentConfigForm.deliberation.inheritAdapter", { defaultValue: "에이전트 기본 (어댑터 상속)" })}
      </option>
      {DELIBERATION_BRAIN_ADAPTER_OPTIONS.map((type) => (
        <option key={type} value={type}>
          {adapterLabels[type] ?? getAdapterLabel(type)}
        </option>
      ))}
    </select>
  );
