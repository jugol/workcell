...
export function ReportsToPicker({
  ...
  const resolvedDisabledEmptyLabel = disabledEmptyLabel ?? t("reportsToPicker.disabledEmpty", { defaultValue: "Reports to: N/A (Orchestrator)" });
  const resolvedChooseLabel = chooseLabel ?? t("reportsToPicker.choose", { defaultValue: "Reports to..." });
        ...
        >
          {unknownManager ? (
            <>
              ...
              <span className="min-w-0 truncate text-muted-foreground">{t("reportsToPicker.unknownManager", { defaultValue: "Unknown manager (stale ID)" })}</span>
            </>
          ) : current ? (
            <>
              ...
              >
                {terminatedManager
                  ? t("reportsToPicker.reportsToTerminated", { defaultValue: "Reports to {{name}} (terminated)", name: current.name })
                  : t("reportsToPicker.reportsTo", { defaultValue: "Reports to {{name}}", name: current.name })}
              </span>
            </>
          ) : (
            <>
      ...
      <PopoverContent className="w-48 p-1" align="start">
        ...
        {terminatedManager && (
          <div className="flex min-w-0 items-center gap-2 overflow-hidden px-2 py-1.5 text-xs text-muted-foreground border-b border-border mb-0.5">
            ...
            <span className="min-w-0 truncate">
              {t("reportsToPicker.currentTerminated", { defaultValue: "Current: {{name}} (terminated)", name: current.name })}
            </span>
          </div>
        )}
        {unknownManager && (
          <div className="px-2 py-1.5 text-xs text-muted-foreground border-b border-border mb-0.5">
            {t("reportsToPicker.savedMissing", { defaultValue: "Saved manager is missing from this company. Choose a new manager or clear." })}
          </div>
        )}
        {rows.map((a) => (
          <button
