...
type WorkcellSprite = (typeof WORKCELL_SPRITES)[number];
...
function spriteSize(sprite: WorkcellSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = WORKCELL_SPRITES[(baseCol + baseRow) % WORKCELL_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = WORKCELL_SPRITES[Math.floor(Math.random() * WORKCELL_SPRITES.length)]!;
