...
export function NewIssueDialog() {
  ...
  // A project is REQUIRED on a user-created top-level issue (the design system
  // anchors to it). When the company has exactly ONE active project and the
  // dialog opened without a project default, select it — there is nothing to
  // choose, and leaving it empty would silently block submit on a hidden rule.
  // With several projects the user still picks consciously.
  useEffect(() => {
    if (!newIssueOpen || isSubIssueMode || projectId) return;
    const activeProjects = orderedProjects.filter((project) => !project.archivedAt);
    if (activeProjects.length === 1) handleProjectChange(activeProjects[0]!.id);
  }, [newIssueOpen, isSubIssueMode, projectId, orderedProjects, handleProjectChange]);
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-t border-border shrink-0">
          ...
          <div className="flex items-center gap-3">
            <div className="min-h-5 text-right">
              ...
              ) : !isSubIssueMode && !projectId && titleHasText ? (
                <span className="text-xs text-muted-foreground" data-testid="new-issue-project-required">
                  {t("newIssueDialog.projectRequired", { defaultValue: "Select a project to create this issue." })}
                </span>
              ) : null}
            </div>
            <Button
              ...
              disabled={!titleHasText || (!isSubIssueMode && !projectId) || createIssue.isPending}
