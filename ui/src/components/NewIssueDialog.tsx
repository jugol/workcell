...
import type { IssueWorkMode } from "@workcell/shared";
...
const DRAFT_KEY = "workcell:issue-draft";
...
export function NewIssueDialog() {
  ...
  const [pairAtCreate, setPairAtCreate] = useState(false);
  ...
  const createIssue = useMutation({
    ...
    onSuccess: ({ issue, companyId, failures }) => {
      ...
      if (failures.length > 0) {
        ...
        pushToast({
          title: t("newIssueDialog.uploadWarningsTitle", { defaultValue: "Created {{issueRef}} with upload warnings", issueRef }),
          body: t("newIssueDialog.uploadWarningsBody", {
            defaultValue: "{{count}} staged {{noun}} could not be added.",
            count: failures.length,
            noun: failures.length === 1 ? "file" : "files",
          }),
          tone: "warn",
          action: prefix
            ? { label: t("newIssueDialog.openIssueAction", { defaultValue: "Open {{issueRef}}", issueRef }), href: `/${prefix}/issues/${issueRef}` }
            : undefined,
        });
      }
      const shouldOpenPairSetup = pairAtCreate;
      const pairIssuePrefix = (companies.find((company) => company.id === companyId)?.issuePrefix ?? "").trim();
      const pairIssueRef = issue.identifier ?? issue.id;
      clearDraft();
      reset();
      closeNewIssue();
      // WC-180: when the user marked the issue as a pair at creation, nudge them
      // to the issue with the pair setup panel opened to pick the two agents.
      if (shouldOpenPairSetup && pairIssuePrefix) {
        pushToast({
          title: t("newIssueDialog.pairToastTitle", { defaultValue: "Issue created — set up the pair" }),
          body: t("newIssueDialog.pairToastBody", { defaultValue: "Open the issue to pick the two agents that will work it together." }),
          tone: "success",
          action: {
            label: t("newIssueDialog.pairToastAction", { defaultValue: "Set up pair" }),
            href: `/${pairIssuePrefix}/issues/${pairIssueRef}?pair=1`,
          },
        });
      }
    },
  ...
  useEffect(() => {
    ...
    // WC-185: honor a pair-at-create intent passed by discoverability
    // affordances (e.g. the add-agent dialog routing into pair setup).
    setPairAtCreate(newIssueDefaults.pairAtCreate === true);
  ...
  const createIssueErrorMessage =
    createIssue.error instanceof Error ? createIssue.error.message : t("newIssueDialog.createErrorFallback", { defaultValue: "Failed to create issue. Try again." });
    ...
    >
      <DialogContent
        ...
        onPointerDownOutside={(event) => {
          ...
          if (target?.closest("[data-radix-popper-content-wrapper], [data-workcell-floating-ui]")) {
            event.preventDefault();
          }
        }}
      >
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-border shrink-0">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>{isSubIssueMode ? t("newIssueDialog.newSubIssue", { defaultValue: "New sub-issue" }) : t("newIssueDialog.newIssue", { defaultValue: "New issue" })}</span>
          </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
          ...
          <div className="px-4 pb-2">
            <div className="overflow-x-auto overscroll-x-contain">
              ...
              <span className="w-6 shrink-0 text-center">{t("newIssueDialog.forLabel", { defaultValue: "For" })}</span>
              <InlineEntitySelector
                ref={assigneeSelectorRef}
                value={assigneeValue}
                options={assigneeOptions}
                recentOptionIds={recentAssigneeOptionIds}
                placeholder={t("newIssueDialog.assigneePlaceholder", { defaultValue: "Assignee" })}
                disablePortal
                noneLabel={t("newIssueDialog.assigneeNoneLabel", { defaultValue: "No assignee" })}
                searchPlaceholder={t("newIssueDialog.assigneeSearchPlaceholder", { defaultValue: "Search assignees..." })}
                emptyMessage={t("newIssueDialog.assigneeEmptyMessage", { defaultValue: "No assignees found." })}
                onChange={(value) => {
                ...
                renderTriggerValue={(option) =>
                  option ? (
                    ...
                    <span className="text-muted-foreground">{t("newIssueDialog.assigneePlaceholder", { defaultValue: "Assignee" })}</span>
                  )
                }
                renderOption={(option) => {
              ...
              />
              <span>{t("newIssueDialog.inLabel", { defaultValue: "in" })}</span>
              <InlineEntitySelector
                ref={projectSelectorRef}
                value={projectId}
                options={projectOptions}
                recentOptionIds={recentProjectIds}
                placeholder={t("newIssueDialog.projectPlaceholder", { defaultValue: "Project" })}
                disablePortal
                noneLabel={t("newIssueDialog.projectNoneLabel", { defaultValue: "No project" })}
                searchPlaceholder={t("newIssueDialog.projectSearchPlaceholder", { defaultValue: "Search projects..." })}
                emptyMessage={t("newIssueDialog.projectEmptyMessage", { defaultValue: "No projects found." })}
                ...
                renderTriggerValue={(option) =>
                  option && currentProject ? (
                    ...
                    <span className="text-muted-foreground">{t("newIssueDialog.projectPlaceholder", { defaultValue: "Project" })}</span>
                  )
                }
                renderOption={(option) => {
                  if (!option.id) return <span className="truncate">{option.label}</span>;
            ...
            {showReviewerRow && (
              <div className="flex items-center gap-2 text-sm text-muted-foreground mt-1">
                ...
                options={assigneeOptions}
                recentOptionIds={recentAssigneeOptionIds}
                placeholder={t("newIssueDialog.reviewerPlaceholder", { defaultValue: "Reviewer" })}
                disablePortal
                noneLabel={t("newIssueDialog.reviewerNoneLabel", { defaultValue: "No reviewer" })}
                searchPlaceholder={t("newIssueDialog.reviewerSearchPlaceholder", { defaultValue: "Search reviewers..." })}
                emptyMessage={t("newIssueDialog.reviewerEmptyMessage", { defaultValue: "No reviewers found." })}
                ...
                renderTriggerValue={(option) =>
                  option ? (
                    ...
                    <span className="text-muted-foreground">{t("newIssueDialog.reviewerPlaceholder", { defaultValue: "Reviewer" })}</span>
                  )
                }
                renderOption={(option) => {
                  if (!option.id) return <span className="truncate">{option.label}</span>;
            ...
            {showApproverRow && (
              <div className="flex items-center gap-2 text-sm text-muted-foreground mt-1">
                ...
                options={assigneeOptions}
                recentOptionIds={recentAssigneeOptionIds}
                placeholder={t("newIssueDialog.approverPlaceholder", { defaultValue: "Approver" })}
                disablePortal
                noneLabel={t("newIssueDialog.approverNoneLabel", { defaultValue: "No approver" })}
                searchPlaceholder={t("newIssueDialog.approverSearchPlaceholder", { defaultValue: "Search approvers..." })}
                emptyMessage={t("newIssueDialog.approverEmptyMessage", { defaultValue: "No approvers found." })}
                ...
                renderTriggerValue={(option) =>
                  option ? (
                    ...
                    <span className="text-muted-foreground">{t("newIssueDialog.approverPlaceholder", { defaultValue: "Approver" })}</span>
                  )
                }
                renderOption={(option) => {
                  if (!option.id) return <span className="truncate">{option.label}</span>;
          ...
          {currentProject && currentProjectSupportsExecutionWorkspace && (
            ...
            <div className="space-y-1.5">
              ...
              <div className="text-[11px] text-muted-foreground">
                {t("newIssueDialog.executionWorkspaceDescription", { defaultValue: "Control whether this issue runs in the shared workspace, a new isolated workspace, or an existing one." })}
              </div>
              <select
              ...
              {executionWorkspaceMode === "reuse_existing" && selectedReusableExecutionWorkspace && (
                <div className="text-[11px] text-muted-foreground">
                  {t("newIssueDialog.reusingWorkspace", {
                    ...
                    source: selectedReusableExecutionWorkspace.branchName ?? selectedReusableExecutionWorkspace.cwd ?? t("newIssueDialog.existingExecutionWorkspaceFallback", { defaultValue: "existing execution workspace" }),
                  })}
                </div>
              )}
                ...
                <div className="space-y-1.5">
                  ...
                  >
                    {(["primary", ...(assigneeSupportsCheapLane ? (["cheap"] as const) : ([] as const)), "custom"] as const).map((lane) => (
                      ...
                      >
                        {lane === "primary"
                          ? t("newIssueDialog.lanePrimary", { defaultValue: "Primary" })
                          : lane === "cheap"
                            ? t("newIssueDialog.laneCheap", { defaultValue: "Cheap" })
                            : t("newIssueDialog.laneCustom", { defaultValue: "Custom" })}
                      </button>
                    ))}
                  </div>
                  {assigneeModelLane === "cheap" && (
                    <p className="text-[11px] text-muted-foreground">
                      {t("newIssueDialog.cheapLane.sends", { defaultValue: "Sends" })} <code>modelProfile: "cheap"</code>{" "}
                      {assigneeCheapProfile?.adapterConfig && typeof (assigneeCheapProfile.adapterConfig as Record<string, unknown>).model === "string"
                        ? <>{t("newIssueDialog.cheapLane.adapterDefault", { defaultValue: "· adapter default" })} <code>{String((assigneeCheapProfile.adapterConfig as Record<string, unknown>).model)}</code></>
                        : assigneeCheapProfile
                          ? <>{t("newIssueDialog.cheapLane.usesProfile", { defaultValue: "· uses the agent's configured cheap profile" })}</>
                          : <>{t("newIssueDialog.cheapLane.fallback", { defaultValue: "· falls back to the primary model if no cheap profile is configured" })}</>}
                    </p>
                  )}
                  {assigneeModelLane === "primary" && (
                    <p className="text-[11px] text-muted-foreground">{t("newIssueDialog.lanePrimaryHint", { defaultValue: "Runs on the agent's primary model." })}</p>
                  )}
                  {assigneeModelLane === "custom" && (
                    <p className="text-[11px] text-muted-foreground">{t("newIssueDialog.laneCustomHint", { defaultValue: "Override the model and effort for this issue only." })}</p>
                  )}
                </div>
                {assigneeModelLane === "custom" && (
                  <div className="space-y-1.5">
                    <div className="text-xs text-muted-foreground">{t("newIssueDialog.modelLabel", { defaultValue: "Model" })}</div>
                    <InlineEntitySelector
                      value={assigneeModelOverride}
                      options={modelOverrideOptions}
                      placeholder={t("newIssueDialog.defaultModelPlaceholder", { defaultValue: "Default model" })}
                      disablePortal
                      noneLabel={t("newIssueDialog.defaultModelPlaceholder", { defaultValue: "Default model" })}
                      searchPlaceholder={t("newIssueDialog.modelSearchPlaceholder", { defaultValue: "Search models..." })}
                      emptyMessage={t("newIssueDialog.modelEmptyMessage", { defaultValue: "No models found." })}
        ...
        <div className="flex items-center gap-1.5 px-4 py-2 border-t border-border flex-wrap shrink-0">
              ...
              >
                {currentPriority ? (
                  ...
                  <>
                    ...
                    {t("newIssueDialog.priority", { defaultValue: "Priority" })}
                  </>
                )}
              </button>
            </PopoverTrigger>
          ...
          {/* Pair toggle (WC-180): make pair collaboration discoverable right at
              issue creation. Lands on the issue with the pair panel opened. */}
          <button
            type="button"
            data-testid="new-issue-pair-toggle"
            aria-pressed={pairAtCreate}
            onClick={() => setPairAtCreate((v) => !v)}
            title={t("newIssueDialog.pairHint", { defaultValue: "Create as a pair — two agents work this issue together. Pick the two agents on the issue after creating." })}
            className={cn(
              "inline-flex items-center gap-1.5 rounded-md border px-2 py-1 text-xs transition-colors",
              pairAtCreate
          ...
          >
            ...
            {t("newIssueDialog.pair", { defaultValue: "Pair" })}
          </button>
        ...
        {assigneeValue && status === "backlog" ? (
          ...
          >
            ...
            <span className="leading-snug">
              {t("newIssueDialog.backlogNote.before", { defaultValue: "Assigning implies executable intent — leave status as" })} <span className="font-medium">{t("issueStatus.backlog", { defaultValue: "Backlog" })}</span> {t("newIssueDialog.backlogNote.middle", { defaultValue: "only to deliberately park this. The assignee will not be woken until status moves to" })} <span className="font-medium">{t("issueStatus.todo", { defaultValue: "Todo" })}</span> {t("newIssueDialog.backlogNote.or", { defaultValue: "or" })} <span className="font-medium">{t("issueStatus.in_progress", { defaultValue: "In Progress" })}</span>.
            </span>
          </div>
        ) : null}
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <span className="inline-flex items-center justify-center gap-1.5">
                ...
                <span>{createIssue.isPending ? t("newIssueDialog.creating", { defaultValue: "Creating..." }) : isSubIssueMode ? t("newIssueDialog.createSubIssue", { defaultValue: "Create Sub-Issue" }) : t("newIssueDialog.createIssue", { defaultValue: "Create Issue" })}</span>
              </span>
            </Button>
          </div>
        </div>
