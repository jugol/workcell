...
import type { CompanySecret, SecretVersionSelector } from "@workcell/shared";
...
export function SecretBindingPicker({
  ...
  const versionDisplay = (selector: SecretVersionSelector | undefined) => {
    if (selector === undefined || selector === VERSION_LATEST) return t("secretBindingPicker.latest", { defaultValue: "latest" });
    return `v${selector}`;
  };
    ...
    <div className={cn("space-y-1.5", className)}>
      ...
      <div className="flex items-center gap-1.5">
        <div className="relative flex-1">
          ...
          >
            <option value="">{secretsQuery.isPending ? t("secretBindingPicker.loading", { defaultValue: "Loading…" }) : placeholder}</option>
            {selectedMissing && value ? (
              <option value={value.secretId}>{t("secretBindingPicker.missingSecret", { defaultValue: "Missing secret ({{id}}…)", id: value.secretId.slice(0, 8) })}</option>
            ) : null}
      ...
      {selectedSecret ? (
        <p className={cn("text-[11px] text-muted-foreground", statusTone(selectedSecret.status))}>
          {selectedSecret.status !== "active" ? t("secretBindingPicker.statusPrefix", { defaultValue: "Status: {{status}}. ", status: selectedSecret.status }) : null}
          {t("secretBindingPicker.boundTo", { defaultValue: "Bound to {{version}} · {{key}}", version: versionDisplay(value?.version), key: selectedSecret.key })}
        </p>
      ) : selectedMissing ? (
        <p className="text-[11px] text-destructive flex items-center gap-1">
          <AlertCircle className="h-3 w-3" />
          {t("secretBindingPicker.noLongerAvailable", { defaultValue: "The previously selected secret is no longer available. Pick another or remove the binding." })}
        </p>
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("secretBindingPicker.createNew", { defaultValue: "Create new secret" })}</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <div>
              <label className="text-xs font-medium text-foreground/80" htmlFor="secret-name">{t("secretBindingPicker.name", { defaultValue: "Name" })}</label>
              <Input
                id="secret-name"
                value={createName}
            ...
            <div>
              <label className="text-xs font-medium text-foreground/80" htmlFor="secret-value">{t("secretBindingPicker.value", { defaultValue: "Value" })}</label>
              <Textarea
                id="secret-value"
                value={createValue}
              ...
              <p className="text-[11px] text-muted-foreground mt-1">
                {t("secretBindingPicker.valueNote", { defaultValue: "The value is stored once and never re-displayed. Rotate to replace." })}
              </p>
            </div>
            <div>
              <label className="text-xs font-medium text-foreground/80" htmlFor="secret-description">{t("secretBindingPicker.description", { defaultValue: "Description" })}</label>
              <Input
          ...
          <DialogFooter>
            <Button type="button" variant="outline" onClick={() => setCreateOpen(false)}>{t("secretBindingPicker.cancel", { defaultValue: "Cancel" })}</Button>
            <Button
              type="button"
