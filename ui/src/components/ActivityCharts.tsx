import type { DashboardRunActivityDay, HeartbeatRun } from "@workcell/shared";
...
const statusLabelDefaults: Record<string, string> = {
  todo: "To Do",
...
function chartStatusLabel(t: TFunction, status: string): string {
  return t(`activityCharts.status.${status}`, {
    defaultValue: statusLabelDefaults[status] ?? status,
  });
}
...
export function IssueStatusChart({ issues }: { issues: { status: string; createdAt: Date }[] }) {
  ...
  return (
    <div>
      ...
      <DateLabels days={days} />
      <ChartLegend items={statusOrder.map(s => ({ color: statusColors[s] ?? "#6b7280", label: chartStatusLabel(t, s) }))} />
    </div>
  );
}
