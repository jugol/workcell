...
import { priorityLabel } from "../lib/issue-labels";
...
export function PriorityIcon({ priority, onChange, className, showLabel }: PriorityIconProps) {
  ...
  if (!onChange) return showLabel ? <span className="inline-flex items-center gap-1.5">{icon}<span className="text-sm">{priorityLabel(priority, t)}</span></span> : icon;
