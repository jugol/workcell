...
export function RoutineHistoryTab({
  ...
  const restoreMutation = useMutation({
    ...
    onSuccess: async (data) => {
      ...
      pushToast({
        title: t("routineHistoryTab.toast.restored.title", {
          defaultValue: "Restored revision {{from}} as revision {{to}}",
          from: restoredFromNumber,
          to: newNumber,
        }),
        body: data.secretMaterials.length > 0
          ? t("routineHistoryTab.toast.restored.bodyWithSecrets", { defaultValue: "Trigger enabled state was restored from the snapshot. New webhook secrets are available in the banner above." })
          : t("routineHistoryTab.toast.restored.body", { defaultValue: "Trigger enabled state was restored from the snapshot." }),
        tone: "success",
      });
    ...
    onError: (error) => {
      pushToast({
        title: t("routineHistoryTab.toast.restoreFailed.title", { defaultValue: "Failed to restore revision" }),
        body: error instanceof Error ? error.message : t("routineHistoryTab.toast.restoreFailed.body", { defaultValue: "Workcell could not restore the revision." }),
        tone: "error",
      });
    },
  });
  ...
  if (revisionsQuery.error) {
    return (
      <div className="rounded-md border border-l-2 border-l-destructive border-border p-4 space-y-3">
        <div>
          <p className="text-sm font-medium">{t("routineHistoryTab.loadError.title", { defaultValue: "Could not load revisions" })}</p>
          <p className="text-xs text-muted-foreground">
            {revisionsQuery.error instanceof Error
              ? revisionsQuery.error.message
              : t("routineHistoryTab.loadError.unknown", { defaultValue: "Unknown error loading revisions." })}
          </p>
        </div>
        <Button size="sm" variant="outline" onClick={() => revisionsQuery.refetch()}>
          {t("routineHistoryTab.loadError.retry", { defaultValue: "Retry" })}
        </Button>
      </div>
    );
  }
      ...
      <div className="space-y-4 min-w-0">
        ...
        {!isEditDirty && onlyBootstrapRevision ? (
          <div className="space-y-2">
            <EmptyState
              icon={HistoryIcon}
              message={t("routineHistoryTab.empty.message", { defaultValue: "No edits yet" })}
            />
            <p className="text-center text-xs text-muted-foreground">
              {t("routineHistoryTab.empty.description", { defaultValue: "Revision 1 is the only history this routine has. Saving an edit creates the first additional revision." })}
            </p>
          </div>
        ) : (
          selectedRevision && (
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-1">
          <p className="text-sm font-medium text-amber-200">
            {t("routineHistoryTab.preview.viewing", { defaultValue: "Viewing revision {{number}} (read-only)", number: revisionNumber })}
          </p>
          <p className="text-xs text-muted-foreground">
            {t("routineHistoryTab.preview.restoreExplainer", {
              defaultValue: "Restoring this revision creates a new revision {{number}} with the same content. History stays append-only.",
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Button variant="outline" size="sm" onClick={onReturn} disabled={pending}>
            {t("routineHistoryTab.preview.returnToCurrent", { defaultValue: "Return to current" })}
          </Button>
          <Button size="sm" onClick={onRestore} disabled={pending}>
            <RotateCcw className="mr-1.5 h-3.5 w-3.5" />
            {t("routineHistoryTab.preview.restoreAsNew", { defaultValue: "Restore as new revision" })}
          </Button>
        </div>
      </div>
    </div>
...
function ConflictBanner({
  ...
  return (
    <div className="rounded-md border border-amber-500/30 bg-amber-500/5 px-4 py-3">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-1">
          <p className="text-sm font-medium text-amber-200">{t("routineHistoryTab.conflict.title", { defaultValue: "Unsaved routine edits" })}</p>
          <p className="text-xs text-muted-foreground">
            {t("routineHistoryTab.conflict.body", {
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Button variant="outline" size="sm" onClick={onDiscard}>
            {t("routineHistoryTab.conflict.discard", { defaultValue: "Discard changes" })}
          </Button>
          <Button size="sm" onClick={onSave}>
            {t("routineHistoryTab.conflict.save", { defaultValue: "Save and continue" })}
          </Button>
        </div>
      </div>
  ...
  return (
    <aside className="space-y-1">
      <header className="flex items-center justify-between pb-2">
        ...
        <span className="text-[11px] text-muted-foreground">{t("routineHistoryTab.revisionList.total", { defaultValue: "{{count}} total", count: totalRevisions })}</span>
      </header>
        ...
        return (
          ...
          >
            <div className="flex items-center gap-2 text-sm font-medium">
              <span>{t("routineHistoryTab.revisionList.rev", { defaultValue: "rev {{number}}", number: revision.revisionNumber })}</span>
              {isCurrent && (
                <span className="rounded-full border border-border px-1.5 text-[10px] uppercase tracking-[0.12em] text-muted-foreground">
                  {t("routineHistoryTab.revisionList.current", { defaultValue: "Current" })}
                </span>
              )}
              {revision.restoredFromRevisionId && (
                <span className="rounded-full border border-amber-500/40 bg-amber-500/10 px-1.5 text-[10px] uppercase tracking-[0.12em] text-amber-200">
                  {t("routineHistoryTab.revisionList.restored", { defaultValue: "Restored" })}
                </span>
              )}
            </div>
      ...
      {totalRevisions > revisions.length && !showOlder && (
        <Button variant="ghost" size="sm" className="w-full" onClick={onShowOlder}>
          {t("routineHistoryTab.revisionList.showOlder", { defaultValue: "Show {{count}} older…", count: totalRevisions - revisions.length })}
        </Button>
      )}
    </aside>
  );
...
function RevisionPreview({
  ...
  const restoreLabel = t("routineHistoryTab.preview.restoreThis", { defaultValue: "Restore this revision" });
  ...
  const fieldRows: Array<{ key: string; label: string; value: string; differs: boolean }> = [
    {
      key: "title",
      label: t("routineHistoryTab.field.title", { defaultValue: "Title" }),
      value: snapshot.title,
      differs: !!currentSnapshot && currentSnapshot.title !== snapshot.title,
    },
    {
      key: "priority",
      label: t("routineHistoryTab.field.priority", { defaultValue: "Priority" }),
      value: snapshot.priority,
      differs: !!currentSnapshot && currentSnapshot.priority !== snapshot.priority,
    },
    {
      key: "status",
      label: t("routineHistoryTab.field.status", { defaultValue: "Status" }),
      value: snapshot.status,
      differs: !!currentSnapshot && currentSnapshot.status !== snapshot.status,
    },
    {
      key: "assigneeAgentId",
      label: t("routineHistoryTab.field.defaultAgent", { defaultValue: "Default agent" }),
    ...
    {
      key: "projectId",
      label: t("routineHistoryTab.field.project", { defaultValue: "Project" }),
    ...
    },
    {
      ...
      label: t("routineHistoryTab.field.concurrency", { defaultValue: "Concurrency" }),
    ...
    <div className="space-y-4">
      <header className={`${cardWrapper} p-4 space-y-2`}>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="space-y-1 min-w-0">
            <p className="text-sm font-medium">{t("routineHistoryTab.revisionList.rev", { defaultValue: "rev {{number}}", number: revision.revisionNumber })}</p>
            <p className="text-xs text-muted-foreground truncate">
              {t("routineHistoryTab.preview.savedBy", {
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Button variant="outline" size="sm" onClick={onCompare}>
              ...
              {t("routineHistoryTab.preview.compareWithCurrent", { defaultValue: "Compare with current" })}
            </Button>
            <Button
              size="sm"
              onClick={onRestore}
      ...
      <div className={`${cardWrapper} p-3`}>
        <p className="pb-2 text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">
          {t("routineHistoryTab.structuredFields", { defaultValue: "Structured fields" })}
        </p>
              ...
              <p className="text-sm">
                ...
                {row.differs && (
                  <span className="ml-2 rounded-full border border-amber-500/40 bg-amber-500/10 px-1.5 text-[10px] uppercase tracking-[0.12em] text-amber-200">
                    {t("routineHistoryTab.differsFromCurrent", { defaultValue: "differs from current" })}
                  </span>
                )}
              </p>
            </div>
      ...
      <div className={`${cardWrapper} p-3 space-y-2`}>
        <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">
          {t("routineHistoryTab.description", { defaultValue: "Description" })}
        </p>
        <div className="rounded-md bg-background/40 p-3 text-sm leading-7">
          {snapshot.description ? (
            <MarkdownBody>{snapshot.description}</MarkdownBody>
          ) : (
            <span className="text-muted-foreground">{t("routineHistoryTab.noDescription", { defaultValue: "No description" })}</span>
          )}
        </div>
      </div>
      ...
      <div className={`${cardWrapper} p-3 space-y-2`}>
        <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">
          {t("routineHistoryTab.triggers", { defaultValue: "Triggers ({{count}})", count: triggers.length })}
        </p>
        {triggers.length === 0 ? (
          <p className="text-sm text-muted-foreground">{t("routineHistoryTab.noTriggers", { defaultValue: "No triggers in this revision." })}</p>
        ) : (
          <ul className="divide-y divide-border">
            {triggers.map((trigger) => (
              <li key={trigger.id} className="py-2 flex flex-wrap items-center gap-2 text-sm">
                ...
                >
                  {trigger.enabled
                    ? t("routineHistoryTab.trigger.enabled", { defaultValue: "enabled" })
                    : t("routineHistoryTab.trigger.disabled", { defaultValue: "disabled" })}
                </span>
              </li>
            ))}
          </ul>
        ...
        <p className="text-xs text-muted-foreground">
          {t("routineHistoryTab.webhookNote", { defaultValue: "Webhook secrets are not stored in revisions. If a restored webhook trigger needs re-creation, Workcell mints fresh secret material at restore time." })}
        </p>
      </div>
      ...
      {snapshot.variables.length > 0 && (
        <div className={`${cardWrapper} p-3 space-y-2`}>
          <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">
            {t("routineHistoryTab.variables", { defaultValue: "Variables ({{count}})", count: snapshot.variables.length })}
          </p>
          <ul className="divide-y divide-border">
            {snapshot.variables.map((variable) => (
              <li key={variable.name} className="py-2 flex items-center justify-between text-sm">
                ...
                <span className="text-xs text-muted-foreground">
                  {t("routineHistoryTab.variableDefault", { defaultValue: "default: {{value}}", value: formatVariableDefault(variable) })}
                </span>
              </li>
            ))}
          </ul>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{t("routineHistoryTab.confirm.title", { defaultValue: "Restore revision {{number}}?", number: target.revisionNumber })}</DialogTitle>
          <DialogDescription>
            {t("routineHistoryTab.confirm.description", {
              defaultValue: "This creates a new revision {{newNumber}} with the same content as revision {{targetNumber}}. Revisions {{targetNumber}}–{{rangeEnd}} stay in history and are not modified.",
              newNumber: newRevisionNumber,
              targetNumber: target.revisionNumber,
              rangeEnd: currentRevisionNumber,
            })}
          </DialogDescription>
        </DialogHeader>
        <ul className="space-y-2 text-sm">
          <li className="flex items-start gap-2">
            ...
            {t("routineHistoryTab.confirm.revertFields", { defaultValue: "Routine field values, variables, and schedule cron will revert." })}
          </li>
          {envDiffCounts.total > 0 && (
            <li className="flex items-start gap-2">
              <span className="mt-1 inline-block h-1.5 w-1.5 rounded-full bg-emerald-400" />
              {t("routineHistoryTab.confirm.revertSecrets", { defaultValue: "Routine secrets will revert: {{counts}}.", counts: formatEnvDiffCounts(envDiffCounts) })}
            </li>
          )}
          <li className="flex items-start gap-2">
            <span className="mt-1 inline-block h-1.5 w-1.5 rounded-full bg-emerald-400" />
            {t("routineHistoryTab.confirm.historyPreserved", { defaultValue: "Previous run history is preserved." })}
          </li>
          {recreatedWebhookLabels.map((label) => (
            <li key={label} className="flex items-start gap-2 text-amber-200">
              ...
              {t("routineHistoryTab.confirm.webhookRecreated", {
                defaultValue: "The webhook trigger {{label}} will be recreated with a new URL and secret. Workcell will show the secret once after restore — copy it before closing.",
                label,
              })}
            </li>
          ))}
        ...
        <div className="space-y-1.5">
          <Label htmlFor="restore-change-summary" className="text-xs">
            {t("routineHistoryTab.confirm.summaryLabel", { defaultValue: "Change summary (optional)" })}
          </Label>
          <Input
            id="restore-change-summary"
            value={changeSummary}
            placeholder={t("routineHistoryTab.confirm.summaryPlaceholder", { defaultValue: "Why are you restoring? Visible in history." })}
            onChange={(event) => onChangeSummaryChange(event.target.value)}
          />
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={() => onOpenChange(false)} disabled={pending}>
            {t("routineHistoryTab.confirm.cancel", { defaultValue: "Cancel" })}
          </Button>
          <Button onClick={onConfirm} disabled={pending}>
            <RotateCcw className="mr-1.5 h-3.5 w-3.5" />
            {pending
              ? t("routineHistoryTab.confirm.restoring", { defaultValue: "Restoring…" })
              : t("routineHistoryTab.confirm.restoreAsRevision", { defaultValue: "Restore as revision {{number}}", number: newRevisionNumber })}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="!max-w-[90%] w-full max-h-[85vh] overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle>{t("routineHistoryTab.diff.title", { defaultValue: "Compare routine revisions" })}</DialogTitle>
        </DialogHeader>
        <div className="flex flex-wrap items-center gap-3">
          <RevisionPicker
            label={t("routineHistoryTab.diff.old", { defaultValue: "Old" })}
            value={leftId}
            onChange={setLeftId}
            revisions={revisions}
            tone="red"
          />
          <RevisionPicker
            label={t("routineHistoryTab.diff.new", { defaultValue: "New" })}
            value={rightId}
            onChange={setRightId}
            revisions={revisions}
            tone="green"
        ...
        <div className="overflow-auto flex-1 space-y-4">
          <section className="space-y-2">
            <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">
              {t("routineHistoryTab.diff.fieldChanges", { defaultValue: "Field changes" })}
            </p>
            {fieldChanges.length === 0 ? (
              <p className="text-sm text-muted-foreground">{t("routineHistoryTab.diff.noFieldChanges", { defaultValue: "No structural field changes." })}</p>
            ) : (
              <table className="w-full text-sm border border-border rounded-md overflow-hidden">
                <thead>
                  <tr className="text-xs uppercase tracking-wide bg-muted/30 text-muted-foreground">
                    <th className="px-3 py-2 text-left">{t("routineHistoryTab.diff.colField", { defaultValue: "Field" })}</th>
                    <th className="px-3 py-2 text-left">{t("routineHistoryTab.diff.colOldValue", { defaultValue: "Old value" })}</th>
                    <th className="px-3 py-2 text-left">{t("routineHistoryTab.diff.colNewValue", { defaultValue: "New value" })}</th>
                  </tr>
                </thead>
                <tbody>
          ...
          <section className="space-y-2">
            <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">
              {t("routineHistoryTab.diff.descriptionDiff", { defaultValue: "Description diff" })}
            </p>
        ...
        <DialogFooter className="justify-between sm:justify-between">
          <Button variant="outline" onClick={() => onOpenChange(false)}>
            {t("routineHistoryTab.diff.close", { defaultValue: "Close" })}
          </Button>
          {leftIsHistorical && left && (
            <Button onClick={() => onRestore(left)}>
              <RotateCcw className="mr-1.5 h-3.5 w-3.5" />
              {t("routineHistoryTab.diff.restoreRev", { defaultValue: "Restore rev {{number}} as new revision", number: left.revisionNumber })}
            </Button>
          )}
        </DialogFooter>
      </DialogContent>
      ...
      >
        {revisions.map((revision) => (
          <option key={revision.id} value={revision.id}>
            {t("routineHistoryTab.picker.option", {
...
function DiffTable({ rows }: { rows: DiffRow[] }) {
  const { t } = useTranslation();
  if (rows.length === 0) {
    return <p className="text-sm text-muted-foreground">{t("routineHistoryTab.diff.noDescriptionEither", { defaultValue: "No description on either revision." })}</p>;
  }
  if (rows.every((row) => row.kind === "context")) {
    return <p className="text-sm text-muted-foreground">{t("routineHistoryTab.diff.identical", { defaultValue: "Descriptions are identical." })}</p>;
  }
  ...
  return (
    <div className="rounded-md border border-border text-xs font-mono leading-6 overflow-hidden">
      <div className="grid grid-cols-[56px_56px_24px_minmax(0,1fr)] border-b border-border/60 bg-muted/30 px-3 py-2 text-[11px] uppercase tracking-wide text-muted-foreground">
        <span>{t("routineHistoryTab.diff.colOld", { defaultValue: "Old" })}</span>
        <span>{t("routineHistoryTab.diff.colNew", { defaultValue: "New" })}</span>
        <span />
        <span>{t("routineHistoryTab.diff.colContent", { defaultValue: "Content" })}</span>
      </div>
      {rows.map((row, index) => (
        <div
