    ...
    <div className="mb-3 rounded-lg border border-border bg-muted/30 px-3 py-2">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          <div className="text-sm font-medium text-foreground">{t("issueMonitorActivityCard.scheduledTitle", { defaultValue: "Monitor scheduled" })}</div>
          <div className="text-xs text-muted-foreground">
            {t("issueMonitorActivityCard.nextCheck", {
              defaultValue: "Next check {{absolute}} ({{relative}})",
          ...
          {monitor.attemptCount > 0 ? (
            <div className="mt-1 text-xs text-muted-foreground">{t("issueMonitorActivityCard.attempt", { defaultValue: "Attempt {{count}}", count: monitor.attemptCount })}</div>
          ) : null}
        </div>
        {onCheckNow ? (
          <Button
          ...
          >
            {checkingNow
              ? t("issueMonitorActivityCard.checking", { defaultValue: "Checking..." })
              : t("issueMonitorActivityCard.checkNow", { defaultValue: "Check now" })}
          </Button>
        ) : null}
      </div>
    </div>
