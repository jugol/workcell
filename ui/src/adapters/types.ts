...
export type { TranscriptEntry, StdoutLineParser, CreateConfigValues } from "@workcell/adapter-utils";

export interface StatefulStdoutParser {
  parseLine: (line: string, ts: string) => import("@workcell/adapter-utils").TranscriptEntry[];
  reset: () => void;
}
...
export interface TranscriptParserSource {
  parseStdoutLine: (line: string, ts: string) => import("@workcell/adapter-utils").TranscriptEntry[];
