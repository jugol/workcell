...
import { Field, buildHelp } from "../components/agent-config-primitives";
...
export function RuntimeServicesJsonField({
  ...
  // Rules of Hooks: the experimental-flag early return must come AFTER the
  // hooks so the hook count never changes between renders.
  ...
  return (
    <Field label={t("agentConfig.runtimeServicesJsonLabel", { defaultValue: "Runtime services JSON" })} hint={help.runtimeServicesJson}>
      <textarea
...
export function PayloadTemplateJsonField({
  ...
  return (
    <Field label={t("agentConfig.payloadTemplateJsonLabel", { defaultValue: "Payload template JSON" })} hint={help.payloadTemplateJson}>
      <textarea
