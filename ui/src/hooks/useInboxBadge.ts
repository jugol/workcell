...
export function useDismissedInboxAlerts() {
  ...
  useEffect(() => {
    const handleStorage = (event: StorageEvent) => {
      if (event.key !== "workcell:inbox:dismissed") return;
...
export function useInboxBadge(companyId: string | null | undefined) {
  ...
  const { data: mineIssuesRaw = [] } = useQuery({
    ...
    // includeBlockedInboxAttention — the badge's 할 일 tally needs that field.
    queryKey: [...queryKeys.issues.listMineByMe(companyId!), "blocked-attn"],
    queryFn: () =>
      issuesApi.list(companyId!, {
        touchedByUserId: "me",
        inboxArchivedByUserId: "me",
        status: INBOX_ISSUE_STATUSES,
        // Required so issue.blockedInboxAttention is populated; the badge's
        // isBoardActionRequiredIssue tally is 0 for every issue without it.
  ...
  // Design 시안 awaiting board approval — a standing human decision (like
  // approvals), counted per issue regardless of read/touch state.
  const { data: designArtifacts } = useQuery({
    queryKey: ["design-artifacts", companyId, "inbox-badge"],
    queryFn: () => designArtifactsApi.listForCompany(companyId!),
    enabled: !!companyId,
  });
  const pendingDesignReviewIssueIds = useMemo(() => {
    // Shared with the rendered list (Inbox.tsx designReviews memo) via
    // isPendingBoardDesignReview so the badge count can never drift from the rows
    // the board actually sees.
    const items = designArtifacts?.items ?? [];
    const issueIds = new Set<string>();
    for (const artifact of items) {
      if (isPendingBoardDesignReview(artifact) && artifact.issueId) {
  ...
  return useMemo(
    ...
    [approvals, joinRequests, dashboard, heartbeatRuns, mineIssues, dismissedAlerts, dismissedAtByKey, currentUserId, pendingDesignReviewIssueIds],
  );
}
