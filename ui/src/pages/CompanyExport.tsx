...
/**
 * Filter .workcell.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterWorkcellYaml(yaml: string, checkedFiles: Set<string>): string {
...
const ROLE_LABELS: Record<string, string> = {
  orchestrator: "Orchestrator", lead: "Lead", pm: "PM", engineer: "Engineer",
  designer: "Designer", researcher: "Researcher", writer: "Writer", qa: "QA",
  security: "Security", devops: "DevOps", general: "General",
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://workcell.ing) package.");
  lines.push("");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("pnpm workcell company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [Workcell](https://workcell.ing) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [Workcell](https://workcell.ing) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function CompanyExport() {
  ...
  const downloadMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      pushToast({
        ...
        body: `${resultCheckedFiles.size} file${resultCheckedFiles.size === 1 ? "" : "s"} ${t("companyExport.toast.downloaded.exportedAs", { defaultValue: "exported as" })} ${result.rootPath}.zip`,
      });
    },
    onError: (err) => {
      pushToast({
  ...
  // Recompute .workcell.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .workcell.yaml
    const yamlPath = exportData.workcellExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterWorkcellYaml(exportData.files[yamlPath], checkedFiles);
    }
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Package} message={t("companyExport.empty.selectCompany", { defaultValue: "Select a team to export." })} />;
  }
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex flex-wrap items-center gap-4 text-sm">
            <span className="font-medium">
              {selectedCompany?.name ?? t("companyExport.bar.company", { defaultValue: "Team" })} {t("companyExport.bar.export", { defaultValue: "export" })}
            </span>
            <span className="text-muted-foreground">
              {selectedCount} / {totalFiles} file{totalFiles === 1 ? "" : "s"} {t("companyExport.bar.selected", { defaultValue: "selected" })}
            </span>
            {warnings.length > 0 && (
          ...
          >
            ...
            {downloadMutation.isPending
              ? t("companyExport.bar.building", { defaultValue: "Building export..." })
              : `${t("companyExport.bar.exportAction", { defaultValue: "Export" })} ${selectedCount} file${selectedCount === 1 ? "" : "s"}`}
          </Button>
        </div>
      </div>
          ...
          <div className="border-b border-border px-3 py-2 shrink-0">
            <div className="flex items-center gap-2 rounded-md border border-border px-2 py-1">
              ...
              <input
                ...
                placeholder={t("companyExport.searchPlaceholder", { defaultValue: "Search files..." })}
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {totalTaskChildren > visibleTaskChildren && !treeSearch && (
              <div className="px-4 py-2">
                ...
                >
                  {t("companyExport.showMoreIssues", { defaultValue: "Show more issues ({{visible}} of {{total}})", visible: visibleTaskChildren, total: totalTaskChildren })}
                </button>
              </div>
            )}
          </div>
