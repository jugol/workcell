  ...
  return (
    <Card>
      <CardHeader className="px-5 pt-5 pb-2">
        <CardTitle className="text-base">{t("costs.finance.ledgerTitle", { defaultValue: "Finance ledger" })}</CardTitle>
        <CardDescription>
          {t("costs.finance.ledgerDescription", { defaultValue: "Account-level charges that do not map to a single inference request." })}
        </CardDescription>
      </CardHeader>
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          label={t("costs.finance.debits.label", { defaultValue: "Debits" })}
          value={formatCents(debitCents)}
          subtitle={
            eventCount === 1
              ? t("costs.finance.debits.subtitle_one", {
                  defaultValue: "{{count}} total event in range",
                  count: eventCount,
                })
              : t("costs.finance.debits.subtitle_other", {
                  defaultValue: "{{count}} total events in range",
                  count: eventCount,
                })
          }
          icon={ArrowUpRight}
        />
        <MetricTile
          label={t("costs.finance.credits.label", { defaultValue: "Credits" })}
          value={formatCents(creditCents)}
          subtitle={t("costs.finance.credits.subtitle", { defaultValue: "Refunds, offsets, and credit returns" })}
          icon={ArrowDownLeft}
        />
        <MetricTile
          label={t("costs.finance.net.label", { defaultValue: "Net" })}
          value={formatCents(netCents)}
          subtitle={t("costs.finance.net.subtitle", { defaultValue: "Debit minus credit for the selected period" })}
          icon={ReceiptText}
        />
        <MetricTile
          label={t("costs.finance.estimated.label", { defaultValue: "Estimated" })}
          value={formatCents(estimatedDebitCents)}
          subtitle={t("costs.finance.estimated.subtitle", { defaultValue: "Estimated debits that are not yet invoice-authoritative" })}
          icon={Coins}
        />
      </CardContent>
    </Card>
...
export function Costs() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("costs.breadcrumb", { defaultValue: "Costs" }) }]);
  }, [setBreadcrumbs, t]);
    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>{t("costs.providers.allProviders", { defaultValue: "All providers" })}</span>
    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>{t("costs.billers.allBillers", { defaultValue: "All billers" })}</span>
            {billerKeys.length > 0 ? (
              <>
                <span className="font-mono text-xs text-muted-foreground">{formatTokens(allTokens)}</span>
                <span className="text-xs text-muted-foreground">{formatCents(allCents)}</span>
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={DollarSign} message={t("costs.emptyNoCompany", { defaultValue: "Select a company to view costs." })} />;
  }
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                <h1 className="text-3xl font-semibold tracking-tight">{t("costs.title", { defaultValue: "Costs" })}</h1>
          ...
          {preset === "custom" ? (
            <div className="flex flex-wrap items-center gap-2 border border-border p-3">
              ...
              />
              <span className="text-sm text-muted-foreground">{t("costs.dateRange.to", { defaultValue: "to" })}</span>
              <input
                type="date"
                value={customTo}
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              label={t("costs.metrics.inferenceSpend.label", { defaultValue: "Inference spend" })}
              value={formatCents(spendData?.summary.spendCents ?? 0)}
              subtitle={t("costs.metrics.inferenceSpend.subtitle", {
            ...
            />
            <MetricTile
              label={t("costs.metrics.budget.label", { defaultValue: "Budget" })}
              value={activeBudgetIncidents.length > 0 ? String(activeBudgetIncidents.length) : (
                spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                  ? `${spendData.summary.utilizationPercent}%`
                  : t("costs.metrics.budget.open", { defaultValue: "Open" })
              )}
              subtitle={
                activeBudgetIncidents.length > 0
                  ? t("costs.metrics.budget.pausedSubtitle", {
                  ...
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                    ? t("costs.metrics.budget.utilizationSubtitle", {
                    ...
                    : t("costs.metrics.budget.noCapSubtitle", { defaultValue: "No monthly cap configured" })
              }
              icon={Coins}
            />
            <MetricTile
              label={t("costs.metrics.financeNet.label", { defaultValue: "Finance net" })}
              value={formatCents(financeData?.summary.netCents ?? 0)}
              subtitle={t("costs.metrics.financeNet.subtitle", {
            ...
            />
            <MetricTile
              label={t("costs.metrics.financeEvents.label", { defaultValue: "Finance events" })}
              value={String(financeData?.summary.eventCount ?? 0)}
              subtitle={t("costs.metrics.financeEvents.subtitle", {
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        ...
        <TabsContent value="overview" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">{t("costs.selectDatePrompt", { defaultValue: "Select a start and end date to load data." })}</p>
          ) : showOverviewLoading ? (
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.3fr,1fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("costs.overview.inferenceLedger.title", { defaultValue: "Inference ledger" })}</CardTitle>
                    <CardDescription>
                      {t("costs.overview.inferenceLedger.description", { defaultValue: "Request-scoped inference spend for the selected period." })}
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4 px-5 pb-5 pt-2">
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      ...
                      <div className="border border-border px-4 py-3 text-right">
                        <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{t("costs.overview.usage", { defaultValue: "usage" })}</div>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {t("costs.overview.utilizationConsumed", {
              ...
              <div className="grid gap-4 xl:grid-cols-[1.25fr,0.95fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("costs.overview.byAgent.title", { defaultValue: "By agent" })}</CardTitle>
                    <CardDescription>{t("costs.overview.byAgent.description", { defaultValue: "What each agent consumed in the selected period." })}</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-2 px-5 pb-5 pt-2">
                    {(spendData?.byAgent.length ?? 0) === 0 ? (
                      <p className="text-sm text-muted-foreground">{t("costs.overview.byAgent.empty", { defaultValue: "No cost events yet." })}</p>
                    ) : (
                            ...
                            >
                              <div className="flex min-w-0 items-center gap-2">
                                ...
                                <Identity
                                  name={
                                    row.agentName ??
                                    t("costs.overview.byAgent.deletedAgent", { defaultValue: "Deleted agent" })
                                  }
                                  size="sm"
                                />
                              ...
                              <div className="text-right text-sm tabular-nums">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  {t("costs.overview.byAgent.inOut", {
                                ...
                                {(row.apiRunCount > 0 || row.subscriptionRunCount > 0) ? (
                                  <div className="text-xs text-muted-foreground">
                                    {t("costs.overview.byAgent.apiRuns", {
                                      defaultValue: "{{count}} api",
                                      count: row.apiRunCount,
                                    })}
                                    {" · "}
                                    {t("costs.overview.byAgent.subscriptionRuns", {
                                      defaultValue: "{{count}} subscription",
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("costs.overview.byProject.title", { defaultValue: "By project" })}</CardTitle>
                      <CardDescription>{t("costs.overview.byProject.description", { defaultValue: "Run costs attributed through project-linked issues." })}</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                      {(spendData?.byProject.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("costs.overview.byProject.empty", { defaultValue: "No project-attributed run costs yet." })}</p>
                      ) : (
                        spendData?.byProject.map((row, index) => (
                          ...
                          >
                            <span className="truncate">{row.projectName ?? row.projectId ?? t("costs.overview.byProject.unattributed", { defaultValue: "Unattributed" })}</span>
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage={t("costs.overview.financeTimelineEmpty", { defaultValue: "No finance events yet. Add account-level charges once biller invoices or credits land." })} />
                </div>
              </div>
            </>
          )}
            ...
            <>
              <Card className="border-border/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))]">
                <CardHeader className="px-5 pt-5 pb-3">
                  <CardTitle className="text-base">{t("costs.budgets.controlPlane.title", { defaultValue: "Budget control plane" })}</CardTitle>
                  <CardDescription>
                    {t("costs.budgets.controlPlane.description", { defaultValue: "Hard-stop spend limits for agents and projects. Provider subscription quota stays separate and appears under Providers." })}
                  </CardDescription>
                </CardHeader>
                <CardContent className="grid gap-3 px-5 pb-5 pt-0 md:grid-cols-4">
                  <MetricTile
                    label={t("costs.budgets.metrics.activeIncidents.label", { defaultValue: "Active incidents" })}
                    value={String(activeBudgetIncidents.length)}
                    subtitle={t("costs.budgets.metrics.activeIncidents.subtitle", { defaultValue: "Open soft or hard threshold crossings" })}
                    icon={ReceiptText}
                  />
                  <MetricTile
                    label={t("costs.budgets.metrics.pendingApprovals.label", { defaultValue: "Pending approvals" })}
                    value={String(budgetData?.pendingApprovalCount ?? 0)}
                    subtitle={t("costs.budgets.metrics.pendingApprovals.subtitle", { defaultValue: "Budget override approvals awaiting board action" })}
                    icon={ArrowUpRight}
                  />
                  <MetricTile
                    label={t("costs.budgets.metrics.pausedAgents.label", { defaultValue: "Paused agents" })}
                    value={String(budgetData?.pausedAgentCount ?? 0)}
                    subtitle={t("costs.budgets.metrics.pausedAgents.subtitle", { defaultValue: "Agent heartbeats blocked by budget" })}
                    icon={Coins}
                  />
                  <MetricTile
                    label={t("costs.budgets.metrics.pausedProjects.label", { defaultValue: "Paused projects" })}
                    value={String(budgetData?.pausedProjectCount ?? 0)}
                    subtitle={t("costs.budgets.metrics.pausedProjects.subtitle", { defaultValue: "Project execution blocked by budget" })}
                    icon={DollarSign}
                  />
                </CardContent>
              </Card>
              ...
              {activeBudgetIncidents.length > 0 ? (
                <div className="space-y-3">
                  <div>
                    <h2 className="text-lg font-semibold">{t("costs.budgets.activeIncidents.heading", { defaultValue: "Active incidents" })}</h2>
                    <p className="text-sm text-muted-foreground">
                      {t("costs.budgets.activeIncidents.description", { defaultValue: "Resolve hard stops here by raising the budget or explicitly keeping the scope paused." })}
                    </p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
                ...
                {(["company", "agent", "project"] as const).map((scopeType) => {
                  ...
                  return (
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold capitalize">{t("costs.budgets.scopeHeading", { defaultValue: "{{scope}} budgets", scope: scopeType })}</h2>
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ? t("costs.budgets.scopeDescription.company", { defaultValue: "Company-wide monthly policy." })
                            : scopeType === "agent"
                              ? t("costs.budgets.scopeDescription.agent", { defaultValue: "Recurring monthly spend policies for individual agents." })
                              : t("costs.budgets.scopeDescription.project", { defaultValue: "Lifetime spend policies for execution-bound projects." })}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
                ...
                {budgetPolicies.length === 0 ? (
                  <Card>
                    <CardContent className="px-5 py-8 text-sm text-muted-foreground">
                      {t("costs.budgets.noPolicies", { defaultValue: "No budget policies yet. Set agent and project budgets from their detail pages, or use the existing company monthly budget control." })}
                    </CardContent>
                  </Card>
                ) : null}
              </div>
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.2fr,0.95fr]">
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("costs.finance.byBiller.title", { defaultValue: "By biller" })}</CardTitle>
                      <CardDescription>{t("costs.finance.byBiller.description", { defaultValue: "Account-level financial events grouped by who charged or credited them." })}</CardDescription>
                    </CardHeader>
                    <CardContent className="grid gap-4 px-5 pb-5 pt-2 md:grid-cols-2">
                      {(financeData?.byBiller.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("costs.finance.byBiller.empty", { defaultValue: "No finance events yet." })}</p>
                      ) : (
