...
export function Search() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("search.breadcrumb", { defaultValue: "Search" }) }]);
  }, [setBreadcrumbs, t]);
      ...
      return {
        ...
        label: (
          <span className="flex items-center">
            {t(`search.scope.${value}`, { defaultValue: SCOPE_LABELS[value as CompanySearchScope] })}
      ...
      <div className="border-b border-border px-4 py-3 sm:px-6">
        ...
        <div className="relative">
          ...
          <Input
            ...
            aria-label={t("search.inputAriaLabel", { defaultValue: "Search query" })}
          ...
          {draftQuery.length > 0 ? (
            <button
              type="button"
              onClick={handleClear}
              aria-label={t("search.clearAriaLabel", { defaultValue: "Clear search" })}
      ...
      <div className="mx-auto flex w-full max-w-2xl flex-col gap-4 px-4 py-10 sm:px-6">
        <div>
          <h2 className="text-lg font-semibold">
            {t("search.initial.heading", { defaultValue: "Type to search company memory." })}
          </h2>
        ...
        <ul className="space-y-1 text-xs text-muted-foreground">
          <li>
            <span className="font-medium text-foreground">
              {t("search.initial.identifierLabel", { defaultValue: "Identifier lookup:" })}
            </span>{" "}
            {t("search.initial.identifierBefore", { defaultValue: "type" })}{" "}
            <code className="rounded bg-muted px-1 py-0.5 text-[11px]">PAP-123</code>{" "}
            {t("search.initial.identifierAfter", { defaultValue: "to jump straight to an issue." })}
          </li>
          <li>
            <span className="font-medium text-foreground">
              {t("search.initial.quotedLabel", { defaultValue: "Quoted phrases:" })}
            </span>{" "}
            {t("search.initial.quotedText", {
              defaultValue: "wrap a phrase in quotes to match the exact sequence.",
            })}
          </li>
          <li>
            <span className="font-medium text-foreground">
              {t("search.initial.commandKeyLabel", { defaultValue: "⌘K:" })}
            </span>{" "}
            {t("search.initial.commandKeyText", {
    ...
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <div className="text-base font-semibold">
          {t("search.error.heading", { defaultValue: "Couldn’t run that search" })}
        </div>
        <p className="text-sm text-muted-foreground">
          {status
            ? t("search.error.serverReturned", {
        ...
        <div className="flex flex-wrap items-center justify-center gap-2">
          <Button onClick={refetch} variant="default" size="sm">
            {t("search.error.retry", { defaultValue: "Retry" })}
          </Button>
    ...
    return (
      <div className="flex flex-col gap-2 px-2 py-3 sm:px-4">
        <div className="px-3 text-xs text-muted-foreground" data-testid="search-loading">
          {t("search.loading", { defaultValue: "Searching for “{{query}}”…", query: trimmedQuery })}
        </div>
    ...
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          {t("search.empty.body", {
            defaultValue:
              "We couldn’t find a match in {{scope}}. Try widening the scope or rephrasing your query.",
            scope: describeScope(scope).toLowerCase(),
          })}
        </p>
        <div className="flex flex-wrap items-center justify-center gap-2">
          {scope !== "all" ? (
            <Button onClick={showAllScope} size="sm" variant="outline">
              {t("search.empty.searchAllScopes", { defaultValue: "Search all scopes" })}
            </Button>
          ) : null}
          <Button onClick={openNewIssue} size="sm" variant="default">
            <Plus className="mr-1.5 h-4 w-4" />
            {t("search.empty.createIssue", { defaultValue: "Create issue from this query" })}
          </Button>
        ...
        <ul className="mt-2 space-y-0.5 text-xs text-muted-foreground">
          <li>{t("search.empty.tipTokens", { defaultValue: "Try fewer tokens or a single distinctive term." })}</li>
          <li>
            {t("search.empty.tipIdentifierBefore", { defaultValue: "Use an identifier shortcut like" })}{" "}
            <code className="rounded bg-muted px-1 py-0.5">PAP-123</code>.
          </li>
          <li>{t("search.empty.tipQuotes", { defaultValue: "Wrap multi-word phrases in quotes." })}</li>
        </ul>
      </div>
    );
  }
  ...
  return (
    <div className="flex w-full max-w-[960px] flex-col px-2 sm:px-4" data-testid="search-results">
      <div className="flex items-center justify-between py-2 text-[11px] uppercase tracking-wide text-muted-foreground">
        <span>
          {totalResults === 1
            ? t("search.results.countSingular", { defaultValue: "1 result" })
            : t("search.results.countPlural", {
                defaultValue: "{{total}} results",
                total: totalResults,
              })}{" "}
          {t("search.results.sortedByRelevance", { defaultValue: "· sorted by relevance" })}
        </span>
        {isFetching ? (
          <span aria-live="polite" className="normal-case tracking-normal">
            {t("search.results.updating", { defaultValue: "Updating…" })}
          </span>
        ) : null}
      </div>
      <div className="flex flex-col pb-10">
        {scope === "all" ? (
          subgroups.map((group, groupIndex) => (
            <section
              key={group.key}
              aria-label={t(`search.subgroup.${group.key}`, { defaultValue: SUBGROUP_LABELS[group.key] })}
              className={cn("flex flex-col", groupIndex > 0 && "mt-6")}
            >
              <IssueGroupHeader
                label={t(`search.subgroup.${group.key}`, { defaultValue: SUBGROUP_LABELS[group.key] })}
                trailing={
