...
import { PairRoundTimeline } from "../components/PairRoundTimeline";
...
import { IssueDesignReviewPanel } from "../components/IssueDesignReviewPanel";
import { IssuePlanRevisionPanel } from "../components/IssuePlanRevisionPanel";
import { IssueDesignComparePanel } from "../components/IssueDesignComparePanel";
import { IssueMonitorActivityCard } from "../components/IssueMonitorActivityCard";
import { IssueScheduledRetryCard } from "../components/IssueScheduledRetryCard";
import { IssueProperties } from "../components/IssueProperties";
import { IssueRunLedger } from "../components/IssueRunLedger";
import { LiveRunTail } from "../components/LiveRunTail";
...
function ActorIdentity({ evt, agentMap, userProfileMap }: { evt: ActivityEvent; agentMap: Map<string, Agent>; userProfileMap?: Map<string, import("../lib/company-members").CompanyUserProfile> }) {
  ...
  if (evt.actorType === "system") return <Identity name={t("issueDetail.actor.system", { defaultValue: "System" })} size="sm" />;
  if (evt.actorType === "user") {
    const profile = userProfileMap?.get(id);
    return <Identity name={profile?.label ?? t("issueDetail.actor.board", { defaultValue: "Board" })} avatarUrl={profile?.image} size="sm" />;
  }
  return <Identity name={id || t("issueDetail.actor.unknown", { defaultValue: "Unknown" })} size="sm" />;
}
            ...
            <>
              ...
              {headerSeed.originKind === "routine_execution" && headerSeed.originId ? (
                <span className="inline-flex items-center gap-1 rounded-full border border-violet-500/30 bg-violet-500/10 px-2 py-0.5 text-[10px] font-medium text-violet-600 dark:text-violet-400 shrink-0">
                  ...
                  {t("issueDetail.badge.routine", { defaultValue: "Routine" })}
                </span>
              ) : null}
  ...
  return (
    <div className="flex items-center w-full">
      <Button
        ...
        aria-label={t("issueDetail.toolbar.backToInbox", { defaultValue: "Back to inbox" })}
      >
      ...
      <div className="ml-auto flex items-center gap-0.5">
        {issueIdProp && !issueHidden && (
          <Button
            ...
            aria-label={t("issueDetail.toolbar.archiveFromInbox", { defaultValue: "Archive from inbox" })}
          >
        ...
        <Popover open={menuOpen} onOpenChange={setMenuOpen}>
          <PopoverTrigger asChild>
            <Button variant="ghost" size="icon-sm" aria-label={t("issueDetail.toolbar.moreActions", { defaultValue: "More actions" })}>
          ...
          <PopoverContent className="w-44 p-1" align="end">
            ...
            >
              ...
              {t("issueDetail.action.copyAsMarkdown", { defaultValue: "Copy as markdown" })}
            </button>
            <button
              className="flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50"
            ...
            >
              ...
              {t("issueDetail.action.properties", { defaultValue: "Properties" })}
            </button>
            {issueIdProp && (
              <button
              ...
              >
                ...
                {t("issueDetail.action.hideThisIssue", { defaultValue: "Hide this issue" })}
              </button>
            )}
          </PopoverContent>
        </Popover>
    ...
    <div className="space-y-3">
      {hasOlderComments ? (
        <div className="flex justify-center">
          ...
          >
            {commentsLoadingOlder ? t("issueDetail.comments.loadingEarlier", { defaultValue: "Loading earlier comments..." }) : t("issueDetail.comments.loadEarlier", { defaultValue: "Load earlier comments" })}
          </Button>
        </div>
      ) : null}
      <IssueChatThread
        ...
        stopRunLabel={t("issueDetail.run.pauseWork", { defaultValue: "Pause work" })}
        stoppingRunLabel={t("issueDetail.run.pausing", { defaultValue: "Pausing..." })}
...
function IssueDetailActivityTab({
  ...
  // An issue can be surfaced to the "To do" queue by a pending approval whose row
  // listApprovals(issueId) does not return (the To-do surfacing and the linked-
  // approvals query can diverge), leaving the issue in the queue with no approve
  // button. Fetch the attention's own approval by id and merge it in so the
  // approve/reject control is ALWAYS present for a board-actionable issue.
  const attentionApprovalId = issue.blockedInboxAttention?.approvalId ?? null;
  const { data: attentionApproval } = useQuery({
    queryKey: ["approval", attentionApprovalId],
    queryFn: () => approvalsApi.get(attentionApprovalId as string),
    enabled: Boolean(attentionApprovalId),
  });
  const effectiveApprovals = useMemo(() => {
    const list = [...(linkedApprovals ?? [])];
    if (attentionApproval && !list.some((a) => a.id === attentionApproval.id)) {
      list.push(attentionApproval);
    }
    return list;
  }, [linkedApprovals, attentionApproval]);
  ...
  // Resolve the currently running solo run so the live output tail below the
  // run ledger can attach to it. Query keys are shared with IssueRunLedger's
  // internal queries, so react-query dedupes the network traffic.
  const { data: activityTabLiveRuns } = useQuery({
    queryKey: queryKeys.issues.liveRuns(issueId),
    queryFn: () => heartbeatsApi.liveRunsForIssue(issueId),
    enabled: hasLiveRuns,
    refetchInterval: 3000,
  ...
  const { data: activityTabActiveRun = null } = useQuery({
    queryKey: queryKeys.issues.activeRun(issueId),
    queryFn: () => heartbeatsApi.activeRunForIssue(issueId),
  ...
  const runningActivityRun = useMemo(
    () => resolveRunningIssueRun(activityTabActiveRun, activityTabLiveRuns),
    [activityTabActiveRun, activityTabLiveRuns],
  );
  const { t } = useTranslation();
    ...
    <>
      {shouldShowCostSummary && (
        <div className="mb-3 px-3 py-2 rounded-lg border border-border">
          <div className="text-sm font-medium text-muted-foreground mb-1">{t("issueDetail.cost.summaryTitle", { defaultValue: "Cost Summary" })}</div>
          {!issueCostSummary.hasCost && !issueCostSummary.hasTokens && !hasIssueTreeCost ? (
            <div className="text-xs text-muted-foreground">{t("issueDetail.cost.noDataYet", { defaultValue: "No cost data yet." })}</div>
          ) : (
            <div className="space-y-1 text-xs text-muted-foreground tabular-nums">
              <div className="flex flex-wrap gap-3">
                <span className="font-medium text-foreground">{t("issueDetail.cost.thisIssue", { defaultValue: "This issue" })}</span>
                ...
                {!issueCostSummary.hasCost && !issueCostSummary.hasTokens && !issueCostSummary.hasRuntime ? (
                  <span>{t("issueDetail.cost.noDirectData", { defaultValue: "No direct cost data." })}</span>
                ) : null}
              </div>
      ...
      {/* Live tail of the currently running solo run, directly under the run
          ledger — shows real model output instead of just "running" so the
          user can tell a working run from a stuck one. The queries share keys
          with the ledger's own active-run/live-runs queries (deduped). */}
      {runningActivityRun ? (
        <div className="mb-3">
          <LiveRunTail runId={runningActivityRun.id} />
        </div>
      ) : null}
...
export function IssueDetail() {
  ...
  // C2: first-issue welcome coachmark — shown when arriving from onboarding
  // (?welcome=1) so a brand-new board understands the agent now works here.
  // Dismissed for the rest of the session.
  const [welcomeCoachmarkDismissed, setWelcomeCoachmarkDismissed] = useState(false);
  ...
  const sourceBreadcrumb = useMemo(
    () => readIssueDetailBreadcrumb(issueId, location.state, location.search) ?? { label: t("issueDetail.breadcrumb.issues", { defaultValue: "Issues" }), href: "/issues" },
  ...
  const commentReassignOptions = useMemo(() => {
    ...
    if (currentUserId) {
      options.push({ id: `user:${currentUserId}`, label: t("issueDetail.reassign.me", { defaultValue: "Me" }) });
    }
    return options;
  }, [t, agents, companyMembers?.users, currentUserId]);
  ...
  const suggestedAssigneeValue = useMemo(() => {
    // During an active review/approval stage the workflow machine owns the
    // assignee (it parked the issue on the reviewer). The "reply to the last
    // speaker" suggestion would silently turn every comment into a
    // reassignment PATCH that the stage guard 422s ("Only the active reviewer
    // or approver can advance..."). Suggest the CURRENT assignee instead so a
    // plain reply is just a comment; an explicit picker change still works.
  ...
  const updateIssue = useMutation({
    ...
    onError: (err, _variables, context) => {
      ...
      pushToast({
        title: t("issueDetail.toast.updateFailed.title", { defaultValue: "Issue update failed" }),
        body: err instanceof Error ? err.message : t("issueDetail.toast.updateFailed.body", { defaultValue: "Unable to save issue changes" }),
        tone: "error",
      });
    },
    onSettled: () => {
  ...
  const hasProof = (workProducts ?? []).some((wp) => wp.type === "proof");
  const addProof = useMutation({
    ...
    onError: (err) => {
      pushToast({
        title: t("issueDetail.toast.addProofFailed.title", { defaultValue: "Could not add proof bundle" }),
        body: err instanceof Error ? err.message : t("issueDetail.toast.addProofFailed.body", { defaultValue: "Unable to attach a proof bundle" }),
        tone: "error",
      });
    },
  });

  // WC-43 (§9 #9): manual context compaction. Refreshes the
  // continuation-summary document for the most recent run. Surfaced as
  // a small button on the detail header — the action is rare enough
  // that it doesn't deserve top-of-page placement, but visible enough
  // that users know it exists.
  const compactContext = useMutation({
    mutationFn: () => issuesApi.compactContext(issueId!),
    onSuccess: () => {
      pushToast({
        title: t("issueDetail.toast.contextCompacted.title", { defaultValue: "Context compacted" }),
        body: t("issueDetail.toast.contextCompacted.body", { defaultValue: "Continuation summary refreshed." }),
        tone: "success",
      });
    ...
    onError: (err) => {
      ...
      pushToast({
        title: t("issueDetail.toast.compactFailed.title", { defaultValue: "Could not compact context" }),
        body: msg.includes("no_run_available")
          ? t("issueDetail.toast.compactFailed.noRun", { defaultValue: "Start a run on this issue first." })
          : msg,
        tone: "error",
      });
    },
  ...
  const resolveRecoveryAction = useMutation({
    ...
    onError: (err) => {
      pushToast({
        title: t("issueDetail.toast.recoveryFailed.title", { defaultValue: "Recovery resolution failed" }),
        body: err instanceof Error ? err.message : t("issueDetail.toast.recoveryFailed.body", { defaultValue: "Unable to resolve recovery action" }),
        tone: "error",
      });
    },
    onSettled: () => {
  ...
  const executeTreeControl = useMutation({
    ...
    onError: (err) => {
      pushToast({
        title: t("issueDetail.toast.treeControlFailed.title", { defaultValue: "Unable to apply subtree control" }),
        body: err instanceof Error ? err.message : t("issueDetail.toast.treeControlFailed.body", { defaultValue: "Please try again." }),
        tone: "error",
      });
    },
  });
  const pauseIssueWorkRun = useMutation({
    ...
    onSuccess: async (result) => {
      const cancelCount = result.preview?.totals.activeRuns ?? 0;
      pushToast({
        title: t("issueDetail.toast.workPaused.title", { defaultValue: "Work paused" }),
        body: cancelCount > 0
          ? `Work paused. ${cancelCount} run${cancelCount === 1 ? "" : "s"} cancelled.`
          : t("issueDetail.toast.workPaused.held", { defaultValue: "Work paused. This issue is held until resume." }),
        tone: "success",
      });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.issues.detail(issueId!) }),
    ...
    onError: (err) => {
      pushToast({
        title: t("issueDetail.toast.pauseWorkFailed.title", { defaultValue: "Unable to pause work" }),
        body: err instanceof Error ? err.message : t("issueDetail.toast.pauseWorkFailed.body", { defaultValue: "Please try again." }),
        tone: "error",
      });
    },
  });
  ...
  const updateChildIssue = useMutation({
    ...
    onError: (err) => {
      pushToast({
        title: t("issueDetail.toast.updateFailed.title", { defaultValue: "Issue update failed" }),
        body: err instanceof Error ? err.message : t("issueDetail.toast.subIssueUpdateFailed.body", { defaultValue: "Unable to save sub-issue changes" }),
        tone: "error",
      });
    },
  });
  ...
  const checkIssueMonitorNow = useMutation({
    ...
    onSuccess: () => {
      ...
      invalidateIssueCollections();
      pushToast({
        title: t("issueDetail.toast.monitorQueued.title", { defaultValue: "Monitor check queued" }),
        tone: "success",
      });
    },
    onError: (err) => {
      pushToast({
        title: t("issueDetail.toast.monitorFailed.title", { defaultValue: "Monitor check failed" }),
        body: err instanceof Error ? err.message : t("issueDetail.toast.monitorFailed.body", { defaultValue: "Unable to trigger the monitor right now" }),
        tone: "error",
      });
    },
  });
  ...
  const approvalDecision = useMutation({
    ...
    onSuccess: (_approval, variables) => {
      ...
      pushToast({
        title: variables.action === "approve" ? t("issueDetail.toast.approvalApproved", { defaultValue: "Approval approved" }) : t("issueDetail.toast.approvalRejected", { defaultValue: "Approval rejected" }),
        tone: "success",
      });
    },
    onError: (err, variables) => {
      pushToast({
        title: variables.action === "approve" ? t("issueDetail.toast.approvalFailed", { defaultValue: "Approval failed" }) : t("issueDetail.toast.rejectionFailed", { defaultValue: "Rejection failed" }),
        body: err instanceof Error ? err.message : t("issueDetail.toast.approvalUpdateFailed.body", { defaultValue: "Unable to update approval" }),
        tone: "error",
      });
    },
    onSettled: () => {
  ...
  const addComment = useMutation({
    ...
    onSuccess: async (comment, _variables, context) => {
      ...
      if (context?.optimisticCommentId && cancelledQueuedOptimisticCommentIdsRef.current.has(context.optimisticCommentId)) {
        ...
        try {
          ...
          pushToast({
            title: t("issueDetail.toast.cancelFailed.title", { defaultValue: "Cancel failed" }),
            body: err instanceof Error ? err.message : t("issueDetail.toast.cancelQueuedFailed.body", { defaultValue: "Unable to cancel the queued comment" }),
            tone: "error",
          });
        }
      }
    ...
    onError: (err, _variables, context) => {
      ...
      pushToast({
        title: t("issueDetail.toast.commentFailed.title", { defaultValue: "Comment failed" }),
        body: err instanceof Error ? err.message : t("issueDetail.toast.commentFailed.body", { defaultValue: "Unable to post comment" }),
        tone: "error",
      });
    },
    ...
    onSuccess: (interaction) => {
      ...
      pushToast({
        title: interaction.kind === "request_confirmation"
          ? t("issueDetail.toast.requestConfirmed", { defaultValue: "Request confirmed" })
          : skippedCount > 0
          ? `Accepted ${createdCount} draft${createdCount === 1 ? "" : "s"} and skipped ${skippedCount}`
          : t("issueDetail.toast.suggestedTasksAccepted", { defaultValue: "Suggested tasks accepted" }),
        tone: "success",
      });
    },
    onError: (err) => {
      pushToast({
        title: t("issueDetail.toast.acceptFailed.title", { defaultValue: "Accept failed" }),
        body: err instanceof Error ? err.message : t("issueDetail.toast.acceptFailed.body", { defaultValue: "Unable to accept the suggested tasks" }),
        tone: "error",
      });
    },
  });
  const rejectInteraction = useMutation({
    ...
    onSuccess: (interaction) => {
      ...
      invalidateIssueCollections();
      pushToast({
        title: interaction.kind === "request_confirmation" ? t("issueDetail.toast.requestDeclined", { defaultValue: "Request declined" }) : t("issueDetail.toast.suggestionRejected", { defaultValue: "Suggestion rejected" }),
        tone: "success",
      });
    },
    onError: (err) => {
      pushToast({
        title: t("issueDetail.toast.rejectFailed.title", { defaultValue: "Reject failed" }),
        body: err instanceof Error ? err.message : t("issueDetail.toast.rejectFailed.body", { defaultValue: "Unable to reject the suggested tasks" }),
        tone: "error",
      });
    },
  });
  const answerInteraction = useMutation({
    ...
    onSuccess: (interaction) => {
      ...
      invalidateIssueCollections();
      pushToast({
        title: t("issueDetail.toast.answersSubmitted.title", { defaultValue: "Answers submitted" }),
        tone: "success",
      });
    },
    onError: (err) => {
      pushToast({
        title: t("issueDetail.toast.submitFailed.title", { defaultValue: "Submit failed" }),
        body: err instanceof Error ? err.message : t("issueDetail.toast.submitFailed.body", { defaultValue: "Unable to submit answers" }),
        tone: "error",
      });
    },
  });
  ...
  const cancelInteraction = useMutation({
    ...
    onSuccess: (interaction) => {
      ...
      invalidateIssueCollections();
      pushToast({
        title: t("issueDetail.toast.questionCancelled.title", { defaultValue: "Question cancelled" }),
        tone: "success",
      });
    },
    onError: (err) => {
      pushToast({
        title: t("issueDetail.toast.cancelFailed.title", { defaultValue: "Cancel failed" }),
        body: err instanceof Error ? err.message : t("issueDetail.toast.cancelQuestionFailed.body", { defaultValue: "Unable to cancel the question" }),
        tone: "error",
      });
    },
  });
  ...
  const interruptQueuedComment = useMutation({
    ...
    onSuccess: () => {
      ...
      invalidateIssueRunState();
      pushToast({
        title: t("issueDetail.toast.interruptRequested.title", { defaultValue: "Interrupt requested" }),
        body: t("issueDetail.toast.interruptRequested.body", { defaultValue: "The active run is stopping so queued comments can continue next." }),
        tone: "success",
      });
    },
    onError: (err, _runId, context) => {
      ...
      pushToast({
        title: t("issueDetail.toast.interruptFailed.title", { defaultValue: "Interrupt failed" }),
        body: err instanceof Error ? err.message : t("issueDetail.toast.interruptFailed.body", { defaultValue: "Unable to interrupt the active run" }),
        tone: "error",
      });
    },
  });
  ...
  const cancelQueuedComment = useMutation({
    ...
    onSuccess: (comment) => {
      ...
      invalidateIssueCollections();
      pushToast({
        title: t("issueDetail.toast.queuedCommentCanceled.title", { defaultValue: "Queued comment canceled" }),
        body: t("issueDetail.toast.queuedCommentCanceled.body", { defaultValue: "The queued message was restored to the composer." }),
        tone: "success",
      });
    },
    onError: (err) => {
        ...
        title:
          variables.sharingPreferenceAtSubmit === "prompt"
            ? variables.allowSharing
              ? t("issueDetail.toast.feedbackSavedShare", { defaultValue: "Feedback saved. Future votes will share" })
              : t("issueDetail.toast.feedbackSavedLocal", { defaultValue: "Feedback saved. Future votes will stay local" })
            : variables.allowSharing
              ? t("issueDetail.toast.feedbackSavedSharingEnabled", { defaultValue: "Feedback saved and sharing enabled" })
              : t("issueDetail.toast.feedbackSaved", { defaultValue: "Feedback saved" }),
        tone: "success",
      });
    },
    onError: (err, _variables, context) => {
      ...
      pushToast({
        title: t("issueDetail.toast.feedbackFailed.title", { defaultValue: "Failed to save feedback" }),
        body: err instanceof Error ? err.message : t("issueDetail.toast.feedbackFailed.body", { defaultValue: "Unknown error" }),
        tone: "error",
      });
    },
  });
  ...
  const archiveFromInbox = useMutation({
    ...
    onSuccess: () => {
      ...
      pushToast({ title: t("issueDetail.toast.archived.title", { defaultValue: "Issue archived from inbox" }), tone: "success" });
    },
    onError: (err) => {
      pushToast({
        title: t("issueDetail.toast.archiveFailed.title", { defaultValue: "Archive failed" }),
        body: err instanceof Error ? err.message : t("issueDetail.toast.archiveFailed.body", { defaultValue: "Unable to archive this issue from the inbox" }),
        tone: "error",
      });
    },
  });
  ...
  const copyIssueToClipboard = async () => {
    ...
    setCopied(true);
    pushToast({ title: t("issueDetail.toast.copiedToClipboard.title", { defaultValue: "Copied to clipboard" }), tone: "success" });
  ...
  const treeControlPrimaryButtonLabel =
    treeControlMode === "pause"
      ? treeControlScope === "leaf"
        ? t("issueDetail.treeControl.pauseWork", { defaultValue: "Pause work" })
        : t("issueDetail.treeControl.pauseAndStop", { defaultValue: "Pause and stop work" })
      : treeControlMode === "cancel"
        ? t("issueDetail.treeControl.cancelIssues", { defaultValue: "Cancel {{count}} issues", count: previewAffectedIssueCount })
      : treeControlMode === "restore"
          ? t("issueDetail.treeControl.restoreIssues", { defaultValue: "Restore {{count}} issues", count: previewAffectedIssueCount })
          : treeControlScope === "leaf"
            ? t("issueDetail.treeControl.resumeWork", { defaultValue: "Resume work" })
            : t("issueDetail.treeControl.resumeSubtree", { defaultValue: "Resume subtree" });
  ...
  const attachmentUploadButton = (
    <>
      ...
      >
        ...
        {uploadAttachment.isPending || importMarkdownDocument.isPending ? t("issueDetail.attachments.uploading", { defaultValue: "Uploading..." }) : (
          <>
            <span className="hidden sm:inline">{t("issueDetail.attachments.uploadAttachment", { defaultValue: "Upload attachment" })}</span>
            <span className="sm:hidden">{t("issueDetail.attachments.upload", { defaultValue: "Upload" })}</span>
          </>
        )}
      </Button>
    </>
  ...
  const showWelcomeCoachmark =
    !welcomeCoachmarkDismissed &&
    ...
    <div className="max-w-3xl space-y-6">
        ...
        <div className="flex items-start justify-between gap-3 rounded-lg border border-primary/30 bg-primary/5 p-3">
          <div className="min-w-0 space-y-1">
            <p className="text-sm font-medium">
              {issue.workOwnerKind === "pair"
                ? t("issueDetail.welcome.pairTitle", {
                    defaultValue: "Your two agents are paired on this task",
                  })
                : t("issueDetail.welcome.title", { defaultValue: "Your agent takes it from here" })}
            </p>
            <p className="text-xs text-muted-foreground">
              {issue.workOwnerKind === "pair"
                ? t("issueDetail.welcome.pairBody", {
                    defaultValue:
                      'Open the pair collaboration panel below and press "Run round" to start the two agents working together. Reply in the message box anytime to steer them.',
                  })
                : t("issueDetail.welcome.body", {
                    defaultValue:
                      "The assigned agent will start planning shortly. Watch progress here — and reply in the message box below to steer it anytime.",
                  })}
            </p>
          </div>
          <button
            type="button"
            onClick={() => setWelcomeCoachmarkDismissed(true)}
          ...
          >
            {t("issueDetail.welcome.dismiss", { defaultValue: "Got it" })}
          </button>
        </div>
      )}
      ...
      {issue.hiddenAt && (
        <div className="flex items-center gap-2 rounded-md border border-destructive/30 bg-destructive/10 px-3 py-2 text-sm text-destructive">
          ...
          {t("issueDetail.notice.hidden", { defaultValue: "This issue is hidden" })}
        </div>
      )}
      {activePauseHold && (
        <div className="rounded-md border border-amber-500/35 bg-amber-500/10 p-3 text-sm text-amber-800 dark:text-amber-200">
          {activePauseHold.isRoot ? (
            <div className="space-y-2">
              <div className="flex flex-wrap items-center gap-2">
                <span className="font-medium">
                  {childIssues.length === 0 ? t("issueDetail.pauseHold.leafTitle", { defaultValue: "Paused by board." }) : t("issueDetail.pauseHold.subtreeTitle", { defaultValue: "Subtree pause is active." })}
                </span>
                <span className="text-xs text-amber-900/80 dark:text-amber-100/80">
                  {childIssues.length === 0
                    ? t("issueDetail.pauseHold.leafDescription", { defaultValue: "Issue execution is held until resume. Human comments can still wake the assignee for triage." })
                    : t("issueDetail.pauseHold.subtreeDescription", { defaultValue: "Root and descendant execution is held until resume. Human comments can still wake assignees for triage." })}
                </span>
              </div>
              <div className="text-xs text-amber-900/80 dark:text-amber-100/80">
                {childIssues.length === 0
                  ? t("issueDetail.pauseHold.oneIssueHeld", { defaultValue: "1 issue held" })
              ...
              {canShowSubtreeControls || canResumeLeafWork ? (
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    {childIssues.length === 0 ? t("issueDetail.treeControl.resumeWork", { defaultValue: "Resume work" }) : t("issueDetail.treeControl.resumeSubtree", { defaultValue: "Resume subtree" })}
                  </Button>
                  <Button
                    variant="outline"
                    size="sm"
                  ...
                  >
                    {t("issueDetail.pauseHold.viewAffected", { defaultValue: "View affected ({{count}})", count: childIssues.length === 0 ? 1 : heldDescendantCount })}
                  </Button>
                  {canShowSubtreeControls ? (
                    ...
                    >
                      {t("issueDetail.treeControl.cancelSubtreeMenu", { defaultValue: "Cancel subtree..." })}
                    </Button>
                  ) : null}
                </div>
              ) : null}
            ...
            <div className="text-xs">
              {t("issueDetail.pauseHold.ancestorPausedPrefix", { defaultValue: "This issue is paused by ancestor" })}{" "}
              ...
              {t("issueDetail.pauseHold.ancestorPausedSuffix", { defaultValue: ". Resume from the root issue to deliver deferred work." })}
            </div>
          )}
        </div>
      )}
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2 min-w-0 flex-wrap">
          <StatusIcon
            ...
            disabledStatuses={hasProof ? {} : { done: t("issueDetail.status.attachProofFirst", { defaultValue: "Attach a proof bundle first" }) }}
            triggerTestId="issue-detail-status-trigger"
          />
          {!hasProof ? (
            <Button
              variant="outline"
              ...
              data-testid="issue-detail-add-proof"
            >
              {t("issueDetail.action.addProofBundle", { defaultValue: "Add proof bundle" })}
            </Button>
          ) : null}
          {/* WC-43 (§9 #9): compact-context button. Only shown when the
              issue has an execution run to compact from; the route would
              return 409 anyway, but hiding the button removes the
              dead-end-click footgun. */}
          {issue.executionRunId ? (
            <Button
              variant="outline"
              size="sm"
              className="h-6 px-2 text-xs shrink-0"
              disabled={compactContext.isPending}
              onClick={() => compactContext.mutate()}
              title={t("issueDetail.action.compactContextTooltip", { defaultValue: "Refresh the continuation summary from the most recent run" })}
              data-testid="issue-detail-compact-context"
            >
              {compactContext.isPending ? t("issueDetail.action.compacting", { defaultValue: "Compacting…" }) : t("issueDetail.action.compactContext", { defaultValue: "Compact context" })}
            </Button>
          ) : null}
          <PriorityIcon
          ...
          {hasLiveRuns && (
            <span className="inline-flex items-center gap-1.5 rounded-full bg-cyan-500/10 border border-cyan-500/30 px-2 py-0.5 text-[10px] font-medium text-cyan-600 dark:text-cyan-400 shrink-0">
              ...
              {t("issueDetail.badge.live", { defaultValue: "Live" })}
            </span>
          )}
          ...
          {issue.originKind === "issue_productivity_review" ? (
            <span
              ...
              title={t("issueDetail.badge.productivityReviewTooltip", { defaultValue: "This task is a productivity review." })}
            >
              <Eye className="h-3 w-3" />
              {t("issueDetail.badge.productivityReview", { defaultValue: "Productivity review" })}
            </span>
          ) : null}
          ...
          {issue.workMode === "planning" ? (
            <span
              ...
              title={t("issueDetail.badge.planningTooltip", { defaultValue: "This issue is in planning mode." })}
            >
              {t("issueDetail.badge.planning", { defaultValue: "Planning" })}
            </span>
          ) : null}
          ...
          {hasAssignedBacklogBlocker(issue.blockedBy) ? (
            <span
              ...
              title={t("issueDetail.badge.parkedBlockerTooltip", { defaultValue: "Blocked by parked work — at least one assigned blocker is in backlog and will not wake its assignee." })}
            >
              <Flag className="h-3 w-3" />
              {t("issueDetail.badge.parkedBlocker", { defaultValue: "Blocked by parked work" })}
            </span>
          ) : null}
          ...
          {!(isMobile && isFromInbox) && (
            <div className="ml-auto flex items-center gap-0.5 md:hidden shrink-0">
              <Button
                ...
                title={t("issueDetail.action.copyAsMarkdownTooltip", { defaultValue: "Copy issue as markdown" })}
              >
              ...
              <Button
                ...
                title={t("issueDetail.action.properties", { defaultValue: "Properties" })}
              >
          ...
          <div className="hidden md:flex items-center md:ml-auto shrink-0">
            {canArchiveFromInbox && (
              <Button
                ...
                disabled={archivePending}
                title={t("issueDetail.toolbar.archiveFromInbox", { defaultValue: "Archive from inbox" })}
                aria-label={t("issueDetail.toolbar.archiveFromInbox", { defaultValue: "Archive from inbox" })}
              >
            ...
            <Button
              ...
              title={t("issueDetail.action.showProperties", { defaultValue: "Show properties" })}
            >
              <SlidersHorizontal className="h-4 w-4" />
            </Button>
            ...
            <Popover open={moreOpen} onOpenChange={setMoreOpen}>
              <PopoverTrigger asChild>
                <Button
                  ...
                  size="icon-xs"
                  className="shrink-0"
                  aria-label={t("issueDetail.action.moreIssueActions", { defaultValue: "More issue actions" })}
                  title={t("issueDetail.action.moreIssueActions", { defaultValue: "More issue actions" })}
                  onKeyDown={(event) => {
            ...
            <PopoverContent className="w-52 p-1" align="end">
              {canPauseLeafWork ? (
                ...
                >
                  ...
                  {t("issueDetail.menu.pauseWork", { defaultValue: "Pause work..." })}
                </button>
              ) : null}
              {canResumeLeafWork ? (
                <button
                ...
                >
                  ...
                  {t("issueDetail.menu.resumeWork", { defaultValue: "Resume work" })}
                </button>
              ) : null}
              {canShowSubtreeControls ? (
                <>
                  ...
                  >
                    <PauseCircle className="h-3 w-3" />
                    {t("issueDetail.menu.pauseSubtree", { defaultValue: "Pause subtree..." })}
                  </button>
                  {canResumeSubtree ? (
                    <button
                      className="flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50"
                    ...
                    >
                      <PlayCircle className="h-3 w-3" />
                      {t("issueDetail.menu.resumeSubtree", { defaultValue: "Resume subtree" })}
                    </button>
                  ) : null}
                  <button
                    className="flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50 text-destructive"
                  ...
                  >
                    ...
                    {t("issueDetail.menu.cancelSubtree", { defaultValue: "Cancel subtree..." })}
                  </button>
                  {canRestoreSubtree ? (
                    <button
                      className="flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50"
                    ...
                    >
                      <Repeat className="h-3 w-3" />
                      {t("issueDetail.menu.restoreSubtree", { defaultValue: "Restore subtree..." })}
                    </button>
                  ) : null}
                </>
              ) : null}
              ...
              >
                <EyeOff className="h-3 w-3" />
                {t("issueDetail.menu.hideIssue", { defaultValue: "Hide this Issue" })}
              </button>
            </PopoverContent>
            </Popover>
          </div>
        ...
        <InlineEditor
          ...
          placeholder={t("issueDetail.description.placeholder", { defaultValue: "Add a description..." })}
          multiline
          foldable
      ...
      {showRichSubIssuesSection ? (
        <div className="space-y-3">
          <div className="flex items-center justify-between gap-2">
            <h3 className="text-sm font-medium text-muted-foreground">{t("issueDetail.section.subIssues", { defaultValue: "Sub-issues" })}</h3>
          </div>
          <IssuesList
            issues={childIssues}
            ...
            viewStateKey={`workcell:issue-detail:${issue.id}:subissues-view`}
            ...
            createIssueLabel={t("issueDetail.section.subIssueLabel", { defaultValue: "Sub-issue" })}
        ...
        <div className="flex flex-wrap items-center justify-end gap-2 min-w-0">
          <Button variant="outline" size="sm" onClick={openNewSubIssue} className="shrink-0 shadow-none">
            ...
            {t("issueDetail.action.newSubIssue", { defaultValue: "New Sub-issue" })}
          </Button>
        </div>
      )}

      {/* §9 #3 UI: pair collaboration round timeline (deliberation rounds). */}
      {issue.workOwnerKind === "pair" ? (
        <PairRoundTimeline issueId={issue.id} agents={agents ?? []} />
      ) : null}

      {/* D12 S5 (WC-123): the issue's knowledge-graph neighborhood — related
          issues, code, decisions, runs. Renders only when connections exist. */}
      <KnowledgeGraphPanel companyId={issue.companyId} issueId={issue.id} />
      ...
      {/* WC-188 / CP7: user feedback on the PLAN/기획 → the planner revises it.
          The PLAN-side mirror of the design request-changes→designer loop. Sits
          next to the plan/documents surface. Board-only (the server returns 403
          for agents), so render it only for board users. */}
      {session?.user?.id ? <IssuePlanRevisionPanel issueId={issue.id} /> : null}

      {/* WC-182 / D22: the design-review gate. The isPrimary design-type work
          product on this issue is its SOURCE OF TRUTH design; this panel surfaces
          it, its review state, a sandboxed preview, and the board's review
          actions. Sits in the deliverables stack alongside documents/attachments. */}
      <IssueDesignReviewPanel issueId={issue.id} />

      {/* WC-183c / (b)③: the VISIBLE 복각 verification. Renders only when the
          issue carries an extracted design-system artifact, showing the
          design-system reference next to the reproduced screen 시안. */}
      <IssueDesignComparePanel issueId={issue.id} />
      ...
      >
        <div className="flex items-center justify-between gap-2">
          <h3 className="text-sm font-medium text-muted-foreground">{t("issueDetail.section.attachments", { defaultValue: "Attachments" })}</h3>
            ...
            {imageAttachments.map((attachment) => (
              ...
              >
                ...
                {confirmDeleteId === attachment.id ? (
                  ...
                  >
                    <p className="text-xs text-white font-medium">{t("issueDetail.attachments.deleteConfirm", { defaultValue: "Delete?" })}</p>
                    <div className="flex gap-1.5">
                      ...
                      >
                        {t("issueDetail.attachments.deleteYes", { defaultValue: "Yes" })}
                      </button>
                      <button
                        type="button"
                      ...
                      >
                        {t("issueDetail.attachments.deleteNo", { defaultValue: "No" })}
                      </button>
                    </div>
                  </div>
                ) : (
                  <button
                    ...
                    title={t("issueDetail.attachments.deleteTooltip", { defaultValue: "Delete attachment" })}
                  >
      ...
      <Tabs value={detailTab} onValueChange={setDetailTab} className="space-y-3">
        <TabsList variant="line" className="w-full justify-start gap-1">
          <TabsTrigger value="chat" className="gap-1.5">
            ...
            {t("issueDetail.tab.chat", { defaultValue: "Chat" })}
          </TabsTrigger>
          <TabsTrigger value="activity" className="gap-1.5">
            <ActivityIcon className="h-3.5 w-3.5" />
            {t("issueDetail.tab.activity", { defaultValue: "Activity" })}
          </TabsTrigger>
          <TabsTrigger value="related-work" className="gap-1.5">
            <ListTree className="h-3.5 w-3.5" />
            {t("issueDetail.tab.relatedWork", { defaultValue: "Related work" })}
          </TabsTrigger>
        ...
        <TabsContent value="chat">
          {detailTab === "chat" ? (
            <IssueDetailChatTab
              ...
              userLabelMap={userLabelMap}
              userProfileMap={userProfileMap}
              draftKey={`workcell:issue-comment-draft:${issue.id}`}
        ...
        <DialogContent className="flex max-h-[calc(100dvh-2rem)] flex-col gap-0 overflow-hidden p-0 sm:max-w-[560px]">
          ...
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain px-6 py-4">
            {treeControlMode === "cancel" ? (
              <div className="rounded-md border border-destructive/30 bg-destructive/10 p-3 text-xs text-destructive">
                {t("issueDetail.dialog.cancelWarning", { defaultValue: "Cancelling a subtree is destructive. Non-terminal issues will be marked cancelled, and running or queued work will be interrupted where possible." })}
              </div>
            ) : null}

            <div className="space-y-1.5">
              <label className="text-xs text-muted-foreground">
                {t("issueDetail.dialog.reasonLabel", { defaultValue: "Reason (optional)" })}
              </label>
              <Textarea
                value={treeControlReason}
                onChange={(event) => setTreeControlReason(event.target.value)}
                placeholder={t("issueDetail.dialog.reasonPlaceholder", { defaultValue: "Explain why this subtree control is being applied..." })}
            ...
            {(treeControlMode === "resume" || treeControlMode === "restore") ? (
              <div className="space-y-2">
                <label className="flex items-start gap-2 text-sm">
                  ...
                  />
                  <span>
                    <span className="block font-medium">{t("issueDetail.dialog.wakeAgents", { defaultValue: "Wake affected agents ({{count}})", count: previewAffectedAgentCount })}</span>
                    <span className="text-xs text-muted-foreground">
                      {previewAffectedAgentCount === 0
                        ? t("issueDetail.dialog.wakeNone", { defaultValue: "No assigned agents are eligible to wake from this preview." })
                        : t("issueDetail.dialog.wakeDescription", { defaultValue: "Wake assigned agents after this operation completes." })}
                    </span>
                  </span>
                </label>
            ...
            {treeControlMode === "cancel" ? (
              <label className="flex items-start gap-2 rounded-md border border-destructive/30 bg-destructive/5 p-2 text-sm">
                ...
                />
                <span>{t("issueDetail.dialog.cancelConfirm", { defaultValue: "I understand this will cancel {{count}} issues.", count: previewAffectedIssueCount })}</span>
              </label>
            ) : null}
            ...
            <div className="space-y-2">
              {treeControlPreviewLoading ? (
                ...
                <div className="space-y-2">
                  ...
                  >
                    {t("issueDetail.dialog.retryPreview", { defaultValue: "Retry preview" })}
                  </Button>
                </div>
              ) : treeControlPreview ? (
                <div className="space-y-2">
                        ...
                        <div key={candidate.id} style={candidate.depth > 0 ? { paddingLeft: `${Math.min(candidate.depth, 6) * 14}px` } : undefined}>
                          ...
                          >
                            ...
                            {candidate.skipped && candidate.skipReason === "terminal_status" ? (
                              <span className="shrink-0 text-xs text-muted-foreground">{t("issueDetail.dialog.complete", { defaultValue: "Complete" })}</span>
                            ) : null}
                          </Link>
                        </div>
                      ))}
                ...
                <p className="text-xs text-muted-foreground">{t("issueDetail.dialog.previewUnavailable", { defaultValue: "Preview unavailable." })}</p>
              )}
            </div>
          </div>
          <DialogFooter className="border-t border-border/60 bg-background px-6 py-4">
            <Button variant="outline" onClick={() => setTreeControlOpen(false)} disabled={executeTreeControl.isPending}>
              {t("issueDetail.dialog.close", { defaultValue: "Close" })}
            </Button>
            <Button
            ...
            >
              {executeTreeControl.isPending ? t("issueDetail.dialog.applying", { defaultValue: "Applying..." }) : treeControlPrimaryButtonLabel}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Sheet open={mobilePropsOpen} onOpenChange={setMobilePropsOpen}>
        <SheetContent side="bottom" className="max-h-[85dvh] pb-[env(safe-area-inset-bottom)]">
          <SheetHeader>
            <SheetTitle className="text-sm">{t("issueDetail.action.properties", { defaultValue: "Properties" })}</SheetTitle>
          </SheetHeader>
