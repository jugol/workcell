...
import { PairSetupPanel } from "../components/PairSetupPanel";
...
export function IssueDetail() {
  ...
  // WC-185: open the issue-level pair setup from the assignee picker's "Pair"
  // choice. Reuses the existing PairSetupPanel deep-link (?pair=1) so the panel
  // expands and pre-fills the current assignee as owner, then scrolls it into
  // view. No-op when the issue is already a pair (the picker hides the option).
  const pairSetupRef = useRef<HTMLDivElement | null>(null);
  const openPairSetup = useCallback(() => {
    ...
    requestAnimationFrame(() => {
      pairSetupRef.current?.scrollIntoView({ behavior: "smooth", block: "center" });
    });
  ...
  return (
    <div className="max-w-3xl space-y-6">
      ...
      {/* §9 #3 UI: pair collaboration. When the issue is a pair, show the round
          timeline; otherwise offer to set one up (WC-78 makes it discoverable). */}
      {issue.workOwnerKind === "pair" ? (
        <PairRoundTimeline issueId={issue.id} agents={agents ?? []} />
      ) : (
        <div ref={pairSetupRef}>
          <PairSetupPanel
            issueId={issue.id}
            agents={agents ?? []}
            defaultOwnerAgentId={issue.assigneeAgentId}
            defaultOpen={new URLSearchParams(location.search).get("pair") === "1"}
            onCreated={() =>
