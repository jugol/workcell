...
export function InstanceAccess() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("instanceAccess.breadcrumbInstanceSettings", { defaultValue: "Instance Settings" }), href: "/instance/settings/general" },
  ...
  const updateCompanyAccessMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: t("instanceAccess.toastCompanyAccessUpdated", { defaultValue: "Team access updated" }), tone: "success" });
    },
  });

  const setAdminMutation = useMutation({
    mutationFn: async (makeAdmin: boolean) => {
      if (!selectedUserId) throw new Error(t("instanceAccess.errorNoUserSelected", { defaultValue: "No user selected" }));
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: t("instanceAccess.toastInstanceRoleUpdated", { defaultValue: "Instance role updated" }), tone: "success" });
    },
  });
  ...
  if (usersQuery.error) {
    const message =
      usersQuery.error instanceof ApiError && usersQuery.error.status === 403
        ? t("instanceAccess.errorAdminRequired", { defaultValue: "Instance admin access is required to manage users." })
    ...
    <div className="max-w-6xl space-y-6">
      <div className="space-y-3">
        ...
        <p className="max-w-3xl text-sm text-muted-foreground">
          {t("instanceAccess.subtitle", { defaultValue: "Search users, manage instance-admin status, and control which teams they can access." })}
        </p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[320px_minmax(0,1fr)]">
            ...
            {(usersQuery.data ?? []).map((user) => (
              ...
              >
                ...
                <div className="mt-2 text-xs text-muted-foreground">
                  {t("instanceAccess.activeMembershipsCount", { defaultValue: "{{count}} active team memberships", count: user.activeCompanyMembershipCount })}
                </div>
              </button>
            ))}
          </div>
        ...
        <section className="space-y-4 rounded-xl border border-border bg-card p-5">
          {!selectedUserId ? (
            <div className="text-sm text-muted-foreground">{t("instanceAccess.selectUserPrompt", { defaultValue: "Select a user to inspect instance access." })}</div>
            ...
            <>
              <div className="flex flex-wrap items-start justify-between gap-4">
                ...
                >
                  {selectedUser?.isInstanceAdmin ? t("instanceAccess.removeAdmin", { defaultValue: "Remove instance admin" }) : t("instanceAccess.promoteAdmin", { defaultValue: "Promote to instance admin" })}
                </Button>
              </div>

              <div className="space-y-3">
                <div>
                  <h2 className="text-sm font-semibold">{t("instanceAccess.companyAccessHeading", { defaultValue: "Team access" })}</h2>
                  <p className="text-sm text-muted-foreground">
                    {t("instanceAccess.companyAccessDescription", { defaultValue: "Toggle team membership for this user. New access defaults to an active operator membership." })}
                  </p>
                </div>
                ...
                <div className="flex justify-end">
                  ...
                  >
                    {updateCompanyAccessMutation.isPending ? t("instanceAccess.saving", { defaultValue: "Saving…" }) : t("instanceAccess.saveCompanyAccess", { defaultValue: "Save team access" })}
                  </Button>
                </div>
              </div>
