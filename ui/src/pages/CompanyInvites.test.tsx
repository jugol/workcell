...
vi.mock("@/context/CompanyContext", () => ({
  useCompany: () => ({
    ...
    selectedCompany: { id: "company-1", name: "Workcell", issuePrefix: "PAP" },
  }),
}));
...
describe("CompanyInvites", () => {
  ...
  const inviteHistory = Array.from({ length: 25 }, (_, index) => {
    ...
    return {
      ...
      invitedByUser: {
        ...
        email: `board${inviteNumber}@workcell.local`,
        image: null,
      },
  ...
  beforeEach(() => {
    ...
    createCompanyInviteMock.mockImplementation(() => {
      return Promise.resolve({
        token: "new-token",
        inviteUrl: "https://workcell.local/invite/new-token",
  ...
  it("renders a human-only invite flow and keeps invite history in a table", async () => {
    ...
    expect(container.textContent).toContain("Team Invites");
    ...
    expect(clipboardWriteTextMock).toHaveBeenCalledWith("https://workcell.local/invite/new-token");
    expect(container.textContent).toContain("Latest invite link");
    expect(container.textContent).toContain("This URL includes the current Workcell domain returned by the server.");
  ...
  it("falls back to selectable text when browser clipboard access is unavailable", async () => {
    ...
    expect(inviteInput?.value).toBe("https://workcell.local/invite/new-token");
