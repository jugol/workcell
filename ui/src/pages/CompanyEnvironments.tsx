...
function SupportMark({ supported }: { supported: boolean }) {
  ...
  return supported ? (
    <span className="inline-flex items-center gap-1 text-green-700 dark:text-green-400">
      ...
      {t("companyEnvironments.support.yes", { defaultValue: "Yes" })}
    </span>
  ) : (
    <span className="text-muted-foreground">{t("companyEnvironments.support.no", { defaultValue: "No" })}</span>
  );
}
...
export function CompanyEnvironments() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: selectedCompany?.name ?? t("companyEnvironments.breadcrumb.company", { defaultValue: "Company" }), href: "/dashboard" },
      { label: t("companyEnvironments.breadcrumb.settings", { defaultValue: "Settings" }), href: "/company/settings" },
      { label: t("companyEnvironments.breadcrumb.environments", { defaultValue: "Environments" }) },
    ]);
  ...
  const environmentMutation = useMutation({
    ...
    onSuccess: async (environment) => {
      ...
      pushToast({
        title: editingEnvironmentId ? t("companyEnvironments.toast.updated", { defaultValue: "Environment updated" }) : t("companyEnvironments.toast.created", { defaultValue: "Environment created" }),
        body: t("companyEnvironments.toast.ready", { defaultValue: "{{name}} is ready.", name: environment.name }),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("companyEnvironments.toast.saveFailed", { defaultValue: "Failed to save environment" }),
        body: error instanceof Error ? error.message : t("companyEnvironments.toast.saveFailedBody", { defaultValue: "Environment save failed." }),
        tone: "error",
      });
    },
  });
  ...
  const environmentProbeMutation = useMutation({
    ...
    onSuccess: (probe, environmentId) => {
      ...
      pushToast({
        title: probe.ok ? t("companyEnvironments.toast.probePassed", { defaultValue: "Environment probe passed" }) : t("companyEnvironments.toast.probeFailed", { defaultValue: "Environment probe failed" }),
        body: probe.summary,
    ...
    onError: (error, environmentId) => {
      ...
      setProbeResults((current) => ({
        ...
        [environmentId]: {
          ...
          summary: error instanceof Error ? error.message : t("companyEnvironments.toast.probeFailedBody", { defaultValue: "Environment probe failed." }),
          details: null,
        },
      }));
      pushToast({
        title: t("companyEnvironments.toast.probeFailed", { defaultValue: "Environment probe failed" }),
        body: error instanceof Error ? error.message : t("companyEnvironments.toast.probeFailedBody", { defaultValue: "Environment probe failed." }),
        tone: "error",
      });
    },
  });
  ...
  const draftEnvironmentProbeMutation = useMutation({
    ...
    onSuccess: (probe) => {
      pushToast({
        title: probe.ok ? t("companyEnvironments.toast.draftProbePassed", { defaultValue: "Draft probe passed" }) : t("companyEnvironments.toast.draftProbeFailed", { defaultValue: "Draft probe failed" }),
        body: probe.summary,
        tone: probe.ok ? "success" : "error",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("companyEnvironments.toast.draftProbeFailed", { defaultValue: "Draft probe failed" }),
        body: error instanceof Error ? error.message : t("companyEnvironments.toast.probeFailedBody", { defaultValue: "Environment probe failed." }),
        tone: "error",
      });
    },
  ...
  if (!selectedCompanyId) {
    return <div className="text-sm text-muted-foreground">{t("companyEnvironments.selectCompany", { defaultValue: "Select a company to manage environments." })}</div>;
  }
    ...
    return (
      <div className="max-w-3xl space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("companyEnvironments.title", { defaultValue: "Company Environments" })}</h1>
        </div>
        <div className="rounded-md border border-border px-4 py-4 text-sm text-muted-foreground">
          {t("companyEnvironments.disabledNote", { defaultValue: "Enable Environments in instance experimental settings to manage company execution targets." })}
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-5xl space-y-6" data-testid="company-settings-environments-section">
      <div className="space-y-2">
        ...
        <p className="max-w-3xl text-sm text-muted-foreground">
          {t("companyEnvironments.intro", { defaultValue: "Define reusable execution targets for projects, issue workspaces, and remote-capable adapters." })}
        </p>
      </div>

      <div className="space-y-4 rounded-md border border-border px-4 py-4">
        <div className="rounded-md border border-border/60 bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
          {t("companyEnvironments.matrixNote", { defaultValue: "Environment choices use the same adapter support matrix as agent defaults. SSH is always available for remote-managed adapters, and sandbox environments appear only when a run-capable sandbox provider plugin is installed." })}
        </div>
        {sandboxCreationEnabled ? (
          <div className="rounded-md border border-border/60 bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
            {t("companyEnvironments.installedProvidersLabel", { defaultValue: "Installed sandbox providers:" })}{" "}
            <span className="font-medium text-foreground">
              {discoveredPluginSandboxProviders.map((provider) => provider.displayName).join(", ")}
            </span>
            {t("companyEnvironments.installedProvidersSuffix", { defaultValue: ". These are not adapter types. They back the Sandbox driver for adapters that support sandbox execution." })}
          </div>
        ) : null}

        <div className="overflow-x-auto">
          <table className="w-full min-w-[34rem] text-left text-xs">
            <caption className="sr-only">{t("companyEnvironments.table.caption", { defaultValue: "Environment support by adapter" })}</caption>
            <thead className="border-b border-border text-muted-foreground">
              <tr>
                <th className="py-2 pr-3 font-medium">{t("companyEnvironments.table.adapter", { defaultValue: "Adapter" })}</th>
                <th className="px-3 py-2 font-medium">{t("companyEnvironments.table.local", { defaultValue: "Local" })}</th>
                <th className="px-3 py-2 font-medium">{t("companyEnvironments.table.ssh", { defaultValue: "SSH" })}</th>
                {sandboxSupportVisible ? (
                  <th className="px-3 py-2 font-medium">{t("companyEnvironments.table.sandboxViaPlugin", { defaultValue: "Sandbox via plugin" })}</th>
                ) : null}
              </tr>
            </thead>
        ...
        <div className="space-y-3">
          {(environments ?? []).length === 0 ? (
            <div className="text-sm text-muted-foreground">{t("companyEnvironments.list.empty", { defaultValue: "No environments saved for this company yet." })}</div>
          ) : (
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="space-y-1">
                      ...
                      {environment.driver === "ssh" ? (
                        <div className="text-xs text-muted-foreground">
                          {typeof environment.config.host === "string" ? environment.config.host : t("companyEnvironments.list.sshHostFallback", { defaultValue: "SSH host" })} ·{" "}
                          {typeof environment.config.username === "string" ? environment.config.username : t("companyEnvironments.list.sshUserFallback", { defaultValue: "user" })}
                        </div>
                        ...
                        <div className="text-xs text-muted-foreground">{t("companyEnvironments.list.localHost", { defaultValue: "Runs on this Workcell host." })}</div>
                      )}
                    </div>
                    <div className="flex flex-wrap items-center gap-2">
                      {environment.driver !== "local" ? (
                        ...
                        >
                          {environmentProbeMutation.isPending
                            ? t("companyEnvironments.list.testing", { defaultValue: "Testing..." })
                            : environment.driver === "ssh"
                              ? t("companyEnvironments.list.testConnection", { defaultValue: "Test connection" })
                              : t("companyEnvironments.list.testProvider", { defaultValue: "Test provider" })}
                        </Button>
                      ) : null}
                      <Button
                        size="sm"
                      ...
                      >
                        {isEditing ? t("companyEnvironments.list.editing", { defaultValue: "Editing" }) : t("companyEnvironments.list.edit", { defaultValue: "Edit" })}
                      </Button>
                    </div>
                  </div>
                  {probe ? (
        ...
        <div className="border-t border-border/60 pt-4">
          <div className="mb-3 text-sm font-medium">
            {editingEnvironmentId ? t("companyEnvironments.form.editTitle", { defaultValue: "Edit environment" }) : t("companyEnvironments.form.addTitle", { defaultValue: "Add environment" })}
          </div>
          <div className="space-y-3">
            <Field label={t("companyEnvironments.form.nameLabel", { defaultValue: "Name" })} hint={t("companyEnvironments.form.nameHint", { defaultValue: "Operator-facing name for this execution target." })}>
              <input
            ...
            <Field label={t("companyEnvironments.form.descriptionLabel", { defaultValue: "Description" })} hint={t("companyEnvironments.form.descriptionHint", { defaultValue: "Optional note about what this machine is for." })}>
              <input
                className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                type="text"
            ...
            <Field label={t("companyEnvironments.form.driverLabel", { defaultValue: "Driver" })} hint={t("companyEnvironments.form.driverHint", { defaultValue: "Local runs on this host. SSH stores a remote machine target. Sandbox stores plugin-backed provider config on the shared environment seam." })}>
              <select
                className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
              ...
              >
                <option value="ssh">{t("companyEnvironments.form.driverSsh", { defaultValue: "SSH" })}</option>
                {sandboxCreationEnabled || environmentForm.driver === "sandbox" ? (
                  <option value="sandbox">{t("companyEnvironments.form.driverSandbox", { defaultValue: "Sandbox" })}</option>
                ) : null}
                <option value="local">{t("companyEnvironments.form.driverLocal", { defaultValue: "Local" })}</option>
              </select>
            </Field>

            {environmentForm.driver === "ssh" ? (
              <div className="grid gap-3 md:grid-cols-2">
                <Field label={t("companyEnvironments.form.hostLabel", { defaultValue: "Host" })} hint={t("companyEnvironments.form.hostHint", { defaultValue: "DNS name or IP address for the remote machine." })}>
                  <input
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    type="text"
                ...
                <Field label={t("companyEnvironments.form.portLabel", { defaultValue: "Port" })} hint={t("companyEnvironments.form.portHint", { defaultValue: "Defaults to 22." })}>
                  <input
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    type="number"
                    min={1}
                ...
                <Field label={t("companyEnvironments.form.usernameLabel", { defaultValue: "Username" })} hint={t("companyEnvironments.form.usernameHint", { defaultValue: "SSH login user." })}>
                  <input
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    type="text"
                ...
                <Field label={t("companyEnvironments.form.remotePathLabel", { defaultValue: "Remote workspace path" })} hint={t("companyEnvironments.form.remotePathHint", { defaultValue: "Absolute path that Workcell will verify during SSH connection tests." })}>
                  <input
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    type="text"
                    placeholder="/Users/workcell/workspace"
                    value={environmentForm.sshRemoteWorkspacePath}
                    onChange={(e) =>
                      setEnvironmentForm((current) => ({ ...current, sshRemoteWorkspacePath: e.target.value }))}
                  />
                </Field>
                <Field label={t("companyEnvironments.form.privateKeyLabel", { defaultValue: "Private key" })} hint={t("companyEnvironments.form.privateKeyHint", { defaultValue: "Optional PEM private key. Leave blank to rely on the server's SSH agent or default keychain." })}>
                  <div className="space-y-2">
                    <select
                      className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    ...
                    >
                      <option value="">{t("companyEnvironments.form.noSavedSecret", { defaultValue: "No saved secret" })}</option>
                      {(secrets ?? []).map((secret) => (
                ...
                <Field label={t("companyEnvironments.form.knownHostsLabel", { defaultValue: "Known hosts" })} hint={t("companyEnvironments.form.knownHostsHint", { defaultValue: "Optional known_hosts block used when strict host key checking is enabled." })}>
                  <textarea
                    className="h-32 w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-xs font-mono outline-none"
                ...
                <div className="md:col-span-2">
                  <ToggleField
                    label={t("companyEnvironments.form.strictHostKeyLabel", { defaultValue: "Strict host key checking" })}
                    hint={t("companyEnvironments.form.strictHostKeyHint", { defaultValue: "Keep this on unless you deliberately want probe-time host key acceptance disabled." })}
            ...
            {environmentForm.driver === "sandbox" ? (
              <div className="grid gap-3 md:grid-cols-2">
                <Field label={t("companyEnvironments.form.providerLabel", { defaultValue: "Provider" })} hint={t("companyEnvironments.form.providerHint", { defaultValue: "Installed run-capable sandbox provider plugins appear here." })}>
                  <select
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                ...
                <div className="md:col-span-2 space-y-3">
                  ...
                  {selectedSandboxSchema ? (
                    ...
                    <div className="rounded-md border border-border/60 bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
                      {t("companyEnvironments.form.noProviderConfig", { defaultValue: "This provider does not declare additional configuration fields." })}
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                {environmentMutation.isPending
                  ? editingEnvironmentId
                    ? t("companyEnvironments.form.saving", { defaultValue: "Saving..." })
                    : t("companyEnvironments.form.creating", { defaultValue: "Creating..." })
                  : editingEnvironmentId
                    ? t("companyEnvironments.form.save", { defaultValue: "Save environment" })
                    : t("companyEnvironments.form.create", { defaultValue: "Create environment" })}
              </Button>
              {editingEnvironmentId ? (
                <Button
                  size="sm"
                ...
                >
                  {t("companyEnvironments.form.cancel", { defaultValue: "Cancel" })}
                </Button>
              ) : null}
              {environmentForm.driver !== "local" ? (
                ...
                >
                  {draftEnvironmentProbeMutation.isPending ? t("companyEnvironments.form.testingDraft", { defaultValue: "Testing..." }) : t("companyEnvironments.form.testDraft", { defaultValue: "Test draft" })}
                </Button>
              ) : null}
              {environmentMutation.isError ? (
                <span className="text-xs text-destructive">
                  {environmentMutation.error instanceof Error
                    ...
                    : t("companyEnvironments.form.saveErrorFallback", { defaultValue: "Failed to save environment" })}
                </span>
              ) : null}
