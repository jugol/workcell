...
export function CompanyEnvironments() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: selectedCompany?.name ?? t("companyEnvironments.breadcrumb.company", { defaultValue: "Team" }), href: "/dashboard" },
  ...
  if (!selectedCompanyId) {
    return <div className="text-sm text-muted-foreground">{t("companyEnvironments.selectCompany", { defaultValue: "Select a team to manage environments." })}</div>;
  }
    ...
    return (
      <div className="max-w-3xl space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("companyEnvironments.title", { defaultValue: "Team Environments" })}</h1>
        </div>
        <div className="rounded-md border border-border px-4 py-4 text-sm text-muted-foreground">
          {t("companyEnvironments.disabledNote", { defaultValue: "Enable Environments in instance experimental settings to manage team execution targets." })}
        </div>
      </div>
    );
  }
      ...
      <div className="space-y-4 rounded-md border border-border px-4 py-4">
        ...
        <div className="space-y-3">
          {(environments ?? []).length === 0 ? (
            <div className="text-sm text-muted-foreground">{t("companyEnvironments.list.empty", { defaultValue: "No environments saved for this team yet." })}</div>
          ) : (
