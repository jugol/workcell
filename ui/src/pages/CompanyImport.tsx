...
function ImportPreviewPane({
  ...
  if (!selectedFile || content === null) {
    return (
      <EmptyState icon={Package} message={t("companyImport.preview.selectFile", { defaultValue: "Select a file to preview its contents." })} />
    );
  }
      ...
      <div className="min-h-[560px] px-5 py-5">
        {parsed ? (
          ...
          <div className="rounded-lg border border-border bg-accent/10 px-4 py-3 text-sm text-muted-foreground">
            {t("companyImport.preview.binaryUnavailable", { defaultValue: "Binary asset preview is not available for this file type." })}
          </div>
        )}
      </div>
    </div>
...
/** Generate a prefix-based rename: e.g. "gstack" + "Orchestrator" → "gstack-Orchestrator" */
      ...
      <div className="rounded-md border border-border">
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          <h3 className="text-sm font-medium">
            {t("companyImport.conflicts.title", { defaultValue: "Renames" })}
          </h3>
          ...
          {conflicts.map((item) => {
            ...
            return (
              ...
              >
                ...
                >
                  {isSkipped ? t("companyImport.conflicts.skipped", { defaultValue: "skipped" }) : t("companyImport.conflicts.skip", { defaultValue: "skip" })}
                </button>
                  ...
                  >
                    {isConfirmed ? (
                      <>
                        ...
                        {t("companyImport.conflicts.confirmed", { defaultValue: "confirmed" })}
                      </>
                    ) : (
                      t("companyImport.conflicts.confirmRename", { defaultValue: "confirm rename" })
                    )}
                  </button>
                )}
              </div>
              ...
              <div key={agent.slug}>
                <div className="flex items-center gap-3 px-4 py-2.5 text-sm">
                  <span className={cn(
                    ...
                    {t("companyImport.adapters.agentBadge", { defaultValue: "agent" })}
                  </span>
                  ...
                  >
                    ...
                    {t("companyImport.adapters.configureAdapter", { defaultValue: "configure adapter" })}
                  </button>
                </div>
                {isExpanded && (
...
export function CompanyImport() {
  ...
  // Fetch current company agents to find the Orchestrator adapter type
  ...
  const ceoAdapterType = useMemo(() => {
    ...
    const ceo = companyAgents.find((a) => a.role === "orchestrator");
    return ceo?.adapterType ?? "claude_local";
  }, [companyAgents]);

  const localZipHelpText =
    t("companyImport.localZipHelpText", { defaultValue: "Upload a .zip exported directly from Workcell. Re-zipped archives created by Finder, Explorer, or other zip tools may not import correctly." });
  ...
  const previewMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      // Initialize adapter overrides — default all agents to the Orchestrator's adapter type
  ...
  const importMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("companyImport.toast.complete.title", { defaultValue: "Import complete" }),
        body: `${result.company.name}: ${result.agents.length} agent${result.agents.length === 1 ? "" : "s"} ${t("companyImport.toast.complete.processed", { defaultValue: "processed." })}`,
      });
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Download} message={t("companyImport.empty.selectCompany", { defaultValue: "Select a company to import into." })} />;
  }
      ...
      <div className="border-b border-border px-5 py-5 space-y-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            {t("companyImport.source.description", { defaultValue: "Choose a GitHub repo or upload a local Workcell zip package." })}
          </p>
        </div>
        ...
        {sourceMode === "local" ? (
          <div className="rounded-md border border-dashed border-border px-3 py-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                {t("companyImport.source.chooseZip", { defaultValue: "Choose zip" })}
              </Button>
              {localPackage && (
                <span className="text-xs text-muted-foreground">
          ...
          <Field
            label={t("companyImport.source.githubUrlLabel", { defaultValue: "GitHub URL" })}
            hint={t("companyImport.source.githubUrlHint", { defaultValue: "Repo tree path or blob URL to COMPANY.md (e.g. github.com/owner/repo/tree/main/company)." })}
          >
            <input
        ...
        <Field label={t("companyImport.target.label", { defaultValue: "Target" })} hint={t("companyImport.target.hint", { defaultValue: "Import into this company or create a new one." })}>
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
            value={targetMode}
            onChange={(e) => {
          ...
          >
            <option value="new">{t("companyImport.target.createNew", { defaultValue: "Create new company" })}</option>
            <option value="existing">
              {t("companyImport.target.existing", { defaultValue: "Existing company: {{name}}", name: selectedCompany?.name ?? "" })}
            </option>
          </select>
        </Field>
        ...
        {targetMode === "new" && (
          <Field
            label={t("companyImport.newCompanyName.label", { defaultValue: "New company name" })}
            hint={t("companyImport.newCompanyName.hint", { defaultValue: "Optional override. Leave blank to use the package name." })}
          >
            <input
              className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
              type="text"
              ...
              placeholder={t("companyImport.newCompanyName.placeholder", { defaultValue: "Imported Company" })}
            />
          </Field>
        )}

        <Field
          label={t("companyImport.collision.label", { defaultValue: "Collision strategy" })}
          hint={t("companyImport.collision.hint", { defaultValue: "Board imports can rename, skip, or replace matching company content." })}
        >
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            {previewMutation.isPending ? t("companyImport.previewButton.loading", { defaultValue: "Previewing..." }) : t("companyImport.previewButton.label", { defaultValue: "Preview import" })}
          </Button>
        </div>
      </div>
          ...
          <div className="sticky top-0 z-10 border-b border-border bg-background px-5 py-3">
            <div className="flex flex-wrap items-center gap-4 text-sm">
              <span className="font-medium">
                {t("companyImport.previewBar.title", { defaultValue: "Import preview" })}
              </span>
              <span className="text-muted-foreground">
                {selectedCount} / {totalFiles} file{totalFiles === 1 ? "" : "s"} {t("companyImport.previewBar.selected", { defaultValue: "selected" })}
              </span>
              {conflicts.length > 0 && (
          ...
          <div className="mx-5 mt-3 flex flex-wrap justify-end gap-2">
            ...
            >
              ...
              {importMutation.isPending
                ? t("companyImport.importButton.loading", { defaultValue: "Importing..." })
                : `${t("companyImport.importButton.label", { defaultValue: "Import" })} ${selectedCount} file${selectedCount === 1 ? "" : "s"}`}
            </Button>
          </div>
          ...
          <div className="grid gap-4 xl:h-[calc(100vh-16rem)] xl:grid-cols-[19rem_minmax(0,1fr)] xl:gap-0">
            <aside className="flex max-h-[24rem] flex-col overflow-hidden border-b border-border xl:max-h-none xl:border-b-0 xl:border-r">
              <div className="border-b border-border px-4 py-3 shrink-0">
                <h2 className="text-base font-semibold">{t("companyImport.packageFiles", { defaultValue: "Package files" })}</h2>
              </div>
