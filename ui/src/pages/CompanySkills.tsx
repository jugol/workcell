...
function sourceMeta(sourceBadge: CompanySkillSourceBadge, sourceLabel: string | null) {
  ...
  switch (sourceBadge) {
    ...
    case "workcell":
      return { icon: AttachmentIcon, label: sourceLabel ?? "Workcell", managedLabel: "Workcell managed" };
    default:
    ...
    <div className="border-b border-border px-4 py-4">
      <div className="space-y-3">
        <Input
          ...
          placeholder={t("companySkills.newForm.namePlaceholder", { defaultValue: "Skill name" })}
          className="h-9 rounded-none border-0 border-b border-border px-0 shadow-none focus-visible:ring-0"
        />
        <Input
          value={slug}
          onChange={(event) => setSlug(event.target.value)}
          placeholder={t("companySkills.newForm.slugPlaceholder", { defaultValue: "optional-shortname" })}
          className="h-9 rounded-none border-0 border-b border-border px-0 shadow-none focus-visible:ring-0"
        />
        <Textarea
          value={description}
          onChange={(event) => setDescription(event.target.value)}
          placeholder={t("companySkills.newForm.descriptionPlaceholder", { defaultValue: "Short description" })}
        ...
        <div className="flex items-center justify-end gap-2">
          <Button variant="ghost" size="sm" onClick={onCancel} disabled={isPending}>
            {t("companySkills.newForm.cancel", { defaultValue: "Cancel" })}
          </Button>
          <Button
            size="sm"
          ...
          >
            {isPending ? t("companySkills.newForm.creating", { defaultValue: "Creating..." }) : t("companySkills.newForm.create", { defaultValue: "Create skill" })}
          </Button>
        </div>
      </div>
    </div>
  ...
  if (filteredSkills.length === 0) {
    return (
      <div className="px-4 py-6 text-sm text-muted-foreground">
        {t("companySkills.list.noMatch", { defaultValue: "No skills match this filter." })}
      </div>
    );
  }
          ...
          <div key={skill.id} className="border-b border-border">
            ...
            >
              ...
              <button
                ...
                aria-label={expanded ? t("companySkills.list.collapse", { defaultValue: "Collapse {{name}}", name: skill.name }) : t("companySkills.list.expand", { defaultValue: "Expand {{name}}", name: skill.name })}
              >
                {expanded ? <ChevronDown className="h-3.5 w-3.5" /> : <ChevronRight className="h-3.5 w-3.5" />}
              </button>
            </div>
...
function SkillPane({
  ...
  const removeDisabledReason = removeBlocked
    ? t("companySkills.pane.removeBlockedReason", { defaultValue: "Detach this skill from all agents before removing it." })
    : null;
        ...
        <div className="mt-4 space-y-3 border-t border-border pt-4 text-sm">
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
            <div className="flex min-w-0 items-center gap-2">
              <span className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("companySkills.pane.sourceLabel", { defaultValue: "Source" })}</span>
            ...
            {detail.sourceType === "github" && (
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("companySkills.pane.pinLabel", { defaultValue: "Pin" })}</span>
                ...
                {updateStatus?.supported && updateStatus.hasUpdate && (
                  ...
                  >
                    ...
                    {t("companySkills.pane.installUpdate", { defaultValue: "Install update" })}{latestPin ? ` ${latestPin}` : ""}
                  </Button>
                )}
            ...
            <div className="flex items-center gap-2">
              <span className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("companySkills.pane.keyLabel", { defaultValue: "Key" })}</span>
              <span className="font-mono text-xs">{detail.key}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("companySkills.pane.modeLabel", { defaultValue: "Mode" })}</span>
              <span>{detail.editable ? t("companySkills.pane.modeEditable", { defaultValue: "Editable" }) : t("companySkills.pane.modeReadOnly", { defaultValue: "Read only" })}</span>
            </div>
          </div>
          <div className="flex flex-wrap items-start gap-x-3 gap-y-1">
            <span className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("companySkills.pane.usedByLabel", { defaultValue: "Used by" })}</span>
            {usedBy.length === 0 ? (
              <span className="text-muted-foreground">{t("companySkills.pane.noAgents", { defaultValue: "No agents attached" })}</span>
            ) : (
              ...
              <div className="flex items-center border border-border">
                ...
                >
                  <span className="flex items-center gap-1.5">
                    ...
                    {t("companySkills.pane.viewTab", { defaultValue: "View" })}
                  </span>
                </button>
                <button
                ...
                >
                  <span className="flex items-center gap-1.5">
                    ...
                    {t("companySkills.pane.codeTab", { defaultValue: "Code" })}
                  </span>
                </button>
              </div>
            )}
            {editMode && file?.editable && (
              <>
                <Button variant="ghost" size="sm" onClick={() => setEditMode(false)} disabled={savePending}>
                  {t("companySkills.pane.cancel", { defaultValue: "Cancel" })}
                </Button>
                <Button size="sm" onClick={onSave} disabled={savePending}>
                  <Save className="mr-1.5 h-3.5 w-3.5" />
                  {savePending ? t("companySkills.pane.saving", { defaultValue: "Saving..." }) : t("companySkills.pane.save", { defaultValue: "Save" })}
                </Button>
              </>
            )}
          </div>
...
export function CompanySkills() {
  ...
  const importSkill = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("companySkills.toast.imported", { defaultValue: "Skills imported" }),
    ...
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("companySkills.toast.importFailed", { defaultValue: "Skill import failed" }),
        body: error instanceof Error ? error.message : t("companySkills.toast.importFailedBody", { defaultValue: "Failed to import skill source." }),
      });
    },
  });
  ...
  const createSkill = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({
        tone: "success",
        title: t("companySkills.toast.created", { defaultValue: "Skill created" }),
        body: t("companySkills.toast.createdBody", { defaultValue: "{{name}} is now editable in the Workcell workspace.", name: skill.name }),
      });
    },
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("companySkills.toast.createFailed", { defaultValue: "Skill creation failed" }),
        body: error instanceof Error ? error.message : t("companySkills.toast.createFailedBody", { defaultValue: "Failed to create skill." }),
      });
    },
  });
  ...
  const scanProjects = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("companySkills.toast.scanComplete", { defaultValue: "Project skill scan complete" }),
        body: summary,
      });
      if (result.conflicts[0]) {
        pushToast({
          tone: "warn",
          title: t("companySkills.toast.conflictsFound", { defaultValue: "Skill conflicts found" }),
    ...
    onError: (error) => {
      ...
      pushToast({
        tone: "error",
        title: t("companySkills.toast.scanFailed", { defaultValue: "Project skill scan failed" }),
        body: error instanceof Error ? error.message : t("companySkills.toast.scanFailedBody", { defaultValue: "Failed to scan project workspaces." }),
      });
    },
  });
  ...
  const saveFile = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("companySkills.toast.saved", { defaultValue: "Skill saved" }),
        body: result.path,
      });
    },
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : t("companySkills.toast.saveFailedBody", { defaultValue: "Failed to save skill file." }),
      });
    },
  });
  ...
  const installUpdate = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({
        tone: "success",
        title: t("companySkills.toast.updated", { defaultValue: "Skill updated" }),
        body: skill.sourceRef ? t("companySkills.toast.pinnedTo", { defaultValue: "Pinned to {{ref}}", ref: shortRef(skill.sourceRef) }) : skill.name,
      });
    },
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : t("companySkills.toast.updateFailedBody", { defaultValue: "Failed to install skill update." }),
      });
    },
  });
  ...
  const deleteSkill = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({
        tone: "success",
        title: t("companySkills.toast.removed", { defaultValue: "Skill removed" }),
        body: t("companySkills.toast.removedBody", { defaultValue: "{{name}} was removed from the team skill library.", name: skill.name }),
      });
    },
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : t("companySkills.toast.removeFailedBody", { defaultValue: "Failed to remove skill." }),
      });
    },
  });

  if (!selectedCompanyId) {
    return <EmptyState icon={Boxes} message={t("companySkills.selectCompany", { defaultValue: "Select a team to manage skills." })} />;
  }
  ...
  return (
    <>
      <Dialog open={deleteOpen} onOpenChange={closeDeleteDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {t("companySkills.deleteDialog.description", { defaultValue: "Remove this skill from the team library. If any agents still use it, removal will be blocked until it is detached." })}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-sm">
            ...
            {deleteTargetDetail?.usedByAgents?.length ? (
              <div className="rounded-md border border-border px-3 py-3 text-muted-foreground">
                {t("companySkills.deleteDialog.currentlyUsedBy", { defaultValue: "Currently used by {{agents}}.", agents: deleteTargetDetail.usedByAgents.map((agent) => agent.name).join(", ") })}
              </div>
            ) : null}
            ...
            {(deleteTargetDetail?.usedByAgents.length ?? 0) > 0 ? (
              ...
              <>
                <Button variant="ghost" onClick={() => closeDeleteDialog(false)} disabled={deleteSkill.isPending}>
                  {t("companySkills.deleteDialog.cancel", { defaultValue: "Cancel" })}
                </Button>
                <Button
                  variant="destructive"
                ...
                >
                  {deleteSkill.isPending ? t("companySkills.deleteDialog.removing", { defaultValue: "Removing..." }) : t("companySkills.deleteDialog.remove", { defaultValue: "Remove skill" })}
                </Button>
              </>
            )}
          </DialogFooter>
      ...
      <Dialog open={emptySourceHelpOpen} onOpenChange={setEmptySourceHelpOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("companySkills.sourceDialog.title", { defaultValue: "Add a skill source" })}</DialogTitle>
            <DialogDescription>
              {t("companySkills.sourceDialog.description", { defaultValue: "Paste a local path, GitHub URL, or `skills.sh` command into the field first." })}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-sm">
            <a
            ...
            >
              <span>
                <span className="block font-medium">{t("companySkills.sourceDialog.browseTitle", { defaultValue: "Browse skills.sh" })}</span>
                <span className="mt-1 block text-muted-foreground">
                  {t("companySkills.sourceDialog.browseDescription", { defaultValue: "Find install commands and paste one here." })}
                </span>
              </span>
            ...
            >
              <span>
                <span className="block font-medium">{t("companySkills.sourceDialog.searchTitle", { defaultValue: "Search GitHub" })}</span>
                <span className="mt-1 block text-muted-foreground">
                  {t("companySkills.sourceDialog.searchDescription", { defaultValue: "Look for repositories with `SKILL.md`, then paste the repo URL here." })}
                </span>
              </span>
              <ExternalLink className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
            </a>
          ...
          <div className="border-b border-border px-4 py-3">
            <div className="flex items-center justify-between gap-2">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {skillsQuery.data?.length ?? 0} {t("companySkills.sidebar.available", { defaultValue: "available" })}
                </p>
              </div>
              <div className="flex items-center gap-1">
                ...
                <Button
                  variant="ghost"
                  size="icon-sm"
                  aria-label={t("companySkills.sidebar.newSkill", { defaultValue: "New skill" })}
            ...
            <div className="mt-3 flex items-center gap-2 border-b border-border pb-2">
              <input
                ...
                placeholder={t("companySkills.sidebar.sourcePlaceholder", { defaultValue: "Paste path, GitHub URL, or skills.sh command" })}
                className="w-full bg-transparent text-sm outline-none placeholder:text-muted-foreground"
              />
              <Button
                size="sm"
              ...
              >
                {importSkill.isPending ? <RefreshCw className="h-4 w-4 animate-spin" /> : t("companySkills.sidebar.add", { defaultValue: "Add" })}
              </Button>
            </div>
            {scanStatusMessage && (
