...
function StatusBadge({
  ...
  const label =
    status === "duplicate"
      ? t("importFromVaultDialog.statusImported", { defaultValue: "Imported" })
      : status === "conflict"
        ? t("importFromVaultDialog.statusConflict", { defaultValue: "Conflict" })
        : t("importFromVaultDialog.statusReady", { defaultValue: "Ready" });
  return (
  ...
  switch (status) {
    case "imported":
      return (
        ...
        >
          <CheckCircle2 className="h-3 w-3" /> {t("importFromVaultDialog.rowCreated", { defaultValue: "Created" })}
        </Badge>
      );
    case "skipped":
      return (
        ...
        >
          <Link2 className="h-3 w-3" /> {t("importFromVaultDialog.rowSkipped", { defaultValue: "Skipped" })}
        </Badge>
      );
    case "error":
    default:
      return (
        ...
        >
          <XCircle className="h-3 w-3" /> {t("importFromVaultDialog.rowFailed", { defaultValue: "Failed" })}
        </Badge>
      );
  }
}
...
function validateDraftRow(
  ...
  for (const existingSecret of existing) {
    if (existingSecret.name.trim().toLowerCase() === lowerName) {
      return "A Workcell secret already uses this name.";
    }
    if (existingSecret.key.trim().toLowerCase() === lowerKey) {
      return "A Workcell secret already uses this key.";
    }
  }
...
export function ImportFromVaultDialog({
  ...
  const importMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      if (result.errorCount === draftList.length && result.errorCount > 0) {
        toast.pushToast({
          title: t("importFromVaultDialog.importFailedTitle", { defaultValue: "Import failed" }),
          body: t("importFromVaultDialog.noSecretsImported", { defaultValue: "No secrets were imported from {{vaultName}}.", vaultName }),
          tone: "error",
        });
      } else {
        toast.pushToast({
          title: result.errorCount > 0
            ? t("importFromVaultDialog.importCompletedWithErrors", { defaultValue: "Import completed with errors" })
            : t("importFromVaultDialog.importComplete", { defaultValue: "Import complete" }),
          body: t("importFromVaultDialog.importSummaryBody", { defaultValue: "{{created}} created · {{skipped}} skipped · {{failed}} failed", created: result.importedCount, skipped: result.skippedCount, failed: result.errorCount }),
  ...
  function handleLoadMore() {
    ...
    secretsApi
      ...
      .catch((error) => {
        toast.pushToast({
          title: t("importFromVaultDialog.couldNotLoadMore", { defaultValue: "Could not load more results" }),
          body: readableErrorMessage(error),
          tone: "error",
        });
      })
  ...
  function handleClose(force = false) {
    ...
    if (!force && step !== "result" && selection.size > 0 && !importResult) {
      const ok = window.confirm(
        t("importFromVaultDialog.discardConfirm", {
          defaultValue: "Discard {{count}} pending import{{plural}}?",
          count: selection.size,
          plural: selection.size === 1 ? "" : "s",
        }),
      );
      if (!ok) return;
    }
        ...
        <header className="flex items-start justify-between gap-3 border-b border-border/60 px-5 py-4">
          <div className="flex flex-col gap-1">
            <DialogTitle className="text-base font-semibold">
              {t("importFromVaultDialog.title", { defaultValue: "Import from AWS Secrets Manager" })}
            </DialogTitle>
            <DialogDescription className="text-xs text-muted-foreground">
              {t("importFromVaultDialog.description", { defaultValue: "Bring AWS-managed secrets into Workcell as external references." })}
            </DialogDescription>
            <Stepper step={step} />
          </div>
          <button
            ...
            onClick={() => handleClose()}
            aria-label={t("importFromVaultDialog.closeAriaLabel", { defaultValue: "Close import dialog" })}
          >
            <X className="h-4 w-4" />
          </button>
        </header>
          ...
          <div className="flex items-center gap-2">
            {step !== "result" && (
              <Button variant="ghost" size="sm" onClick={() => handleClose()}>
                {t("importFromVaultDialog.cancel", { defaultValue: "Cancel" })}
              </Button>
            )}
            {step === "review" && (
              <Button
              ...
              >
                {t("importFromVaultDialog.back", { defaultValue: "Back" })}
              </Button>
            )}
            {step === "select" && (
              <Button
              ...
              >
                {t("importFromVaultDialog.continueToReview", { defaultValue: "Continue → Review" })}
              </Button>
            )}
            {step === "review" && (
              <Button
              ...
              >
                {importMutation.isPending ? (
                  <>
                    <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" /> {t("importFromVaultDialog.importing", { defaultValue: "Importing…" })}
                  </>
                ) : (
                  t("importFromVaultDialog.importCount", { defaultValue: "Import {{count}}", count: draftList.length })
                )}
              </Button>
            )}
            {step === "result" && (
              <Button size="sm" onClick={() => handleClose(true)}>
                {t("importFromVaultDialog.done", { defaultValue: "Done" })}
              </Button>
            )}
          </div>
        </footer>
...
function Stepper({ step }: { step: Step }) {
  ...
  const steps: { id: Step; label: string }[] = [
    { id: "select", label: t("importFromVaultDialog.stepSelect", { defaultValue: "Select" }) },
    { id: "review", label: t("importFromVaultDialog.stepReview", { defaultValue: "Review" }) },
    { id: "result", label: t("importFromVaultDialog.stepResult", { defaultValue: "Result" }) },
  ];
  ...
  if (noEligibleVaults) {
    return (
      <div className="flex min-h-0 flex-1 items-center justify-center p-6" data-testid="select-empty-vaults">
        <EmptyState
          icon={Cloud}
          message={t("importFromVaultDialog.noVaultConfigured", { defaultValue: "No AWS provider vault configured. Add one to import secrets." })}
          action={onManageVaults ? t("importFromVaultDialog.manageVaults", { defaultValue: "Manage vaults" }) : undefined}
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex flex-wrap items-center gap-2 border-b border-border/60 px-5 py-3">
        <label className="text-xs uppercase tracking-wide text-muted-foreground">{t("importFromVaultDialog.vaultLabel", { defaultValue: "Vault" })}</label>
        {awsVaults.length === 1 && eligible.length === 1 ? (
          ...
          >
            <SelectTrigger size="sm" className="text-xs" aria-label={t("importFromVaultDialog.selectVaultAriaLabel", { defaultValue: "Select AWS vault" })}>
              <SelectValue placeholder={t("importFromVaultDialog.selectVaultPlaceholder", { defaultValue: "Select an AWS vault" })} />
            </SelectTrigger>
            <SelectContent>
              {awsVaults.map((vault) => {
                ...
                return (
                  ...
                  >
                    <span className="flex items-center gap-2">
                      ...
                      {vault.isDefault && (
                        <Badge variant="outline" className="px-1 py-0 text-[10px]">{t("importFromVaultDialog.badgeDefault", { defaultValue: "default" })}</Badge>
                      )}
                      {vault.status === "warning" && (
                        <Badge variant="outline" className="px-1 py-0 text-[10px] text-amber-500 border-amber-500/40">{t("importFromVaultDialog.badgeWarning", { defaultValue: "warning" })}</Badge>
                      )}
                      {blocked && (
                        <Badge variant="outline" className="px-1 py-0 text-[10px] text-muted-foreground">
                          {vault.status === "coming_soon" ? t("importFromVaultDialog.badgeComingSoon", { defaultValue: "coming soon" }) : vault.status}
                        </Badge>
                      )}
                    </span>
                  </SelectItem>
        ...
        <div className="relative ml-auto w-64">
          ...
          <Input
            ...
            placeholder={t("importFromVaultDialog.searchPlaceholder", { defaultValue: "Search by name, ARN, tag" })}
            className="pl-7 pr-7 text-xs"
            aria-label={t("importFromVaultDialog.searchAriaLabel", { defaultValue: "Search remote secrets" })}
        ...
        <Button
          ...
          aria-label={t("importFromVaultDialog.refreshAriaLabel", { defaultValue: "Refresh remote secrets" })}
        >
      ...
      {selectedNotVisible > 0 && (
        <div className="flex items-center justify-between border-b border-border/60 bg-muted/20 px-5 py-1.5 text-xs text-muted-foreground">
          <span>
            {t("importFromVaultDialog.selectedNotVisible", { defaultValue: "{{selected}} selected · {{notVisible}} not visible with current search", selected: selection.size, notVisible: selectedNotVisible })}
          </span>
          <Button
            variant="ghost"
            size="sm"
          ...
          >
            {showOnlySelected ? t("importFromVaultDialog.showAll", { defaultValue: "Show all" }) : t("importFromVaultDialog.showSelected", { defaultValue: "Show selected" })}
          </Button>
        </div>
      )}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto" data-testid="vault-table-scroll">
              ...
              <tr>
                <th className="px-3 py-2 text-left">
                  <Checkbox
                    ...
                    aria-label={t("importFromVaultDialog.selectAllLoadedAriaLabel", { defaultValue: "Select all loaded ({{count}})", count: selectableInLoaded.length })}
                    disabled={selectableInLoaded.length === 0}
                  />
                </th>
                <th className="px-2 py-2 text-left font-medium">{t("importFromVaultDialog.columnRemoteName", { defaultValue: "Remote name" })}</th>
                <th className="px-2 py-2 text-left font-medium">{t("importFromVaultDialog.columnReference", { defaultValue: "Reference" })}</th>
                <th className="px-2 py-2 text-left font-medium">{t("importFromVaultDialog.columnLastChanged", { defaultValue: "Last changed" })}</th>
                <th className="px-2 py-2 text-left font-medium">{t("importFromVaultDialog.columnSuggestedName", { defaultValue: "Suggested name" })}</th>
                <th className="px-2 py-2 text-left font-medium">{t("importFromVaultDialog.columnState", { defaultValue: "State" })}</th>
              </tr>
            </thead>
                  ...
                  >
                    <td className="px-3 py-2.5" onClick={(e) => e.stopPropagation()}>
                      <Checkbox
                        ...
                        aria-label={t("importFromVaultDialog.selectRowAriaLabel", { defaultValue: "Select {{name}}", name: candidate.remoteName })}
                      />
                    </td>
                        ...
                        {candidate.status === "duplicate" &&
                          candidate.conflicts.find((c) => c.type === "exact_reference")?.existingSecretId && (
                            <span className="text-[11px] text-muted-foreground">
                              {t("importFromVaultDialog.alreadyImported", { defaultValue: "Already imported" })}
                            </span>
                          )}
                      </div>
        ...
        {hasNextPage && !previewError && (
          <div className="flex items-center justify-between border-t border-border/60 px-5 py-2 text-xs text-muted-foreground">
            <span>
              {t("importFromVaultDialog.loadedCount", { defaultValue: "{{count}} loaded", count: candidates.length })}
              {selectableInLoaded.length > 0 && (
                <span> {t("importFromVaultDialog.selectableSuffix", { defaultValue: "· {{count}} selectable", count: selectableInLoaded.length })}</span>
              )}
            </span>
            <Button
              variant="outline"
            ...
            >
              {pageLoading ? (
                <>
                  <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" /> {t("importFromVaultDialog.loading", { defaultValue: "Loading…" })}
                </>
              ) : (
                t("importFromVaultDialog.loadMore", { defaultValue: "Load {{count}} more", count: PAGE_SIZE })
              )}
            </Button>
          </div>
        )}
    ...
    >
      ...
      <div className="flex-1">
        <div className="font-medium">
          {isPermission
            ? t("importFromVaultDialog.errorDeniedListAccess", { defaultValue: "AWS denied list access" })
            : isThrottling
              ? t("importFromVaultDialog.errorThrottledListing", { defaultValue: "AWS throttled the listing request" })
              : t("importFromVaultDialog.errorCouldNotLoad", { defaultValue: "Could not load remote secrets" })}
        </div>
        <div className="mt-1 text-xs leading-relaxed text-destructive/80">
          {isPermission
            ? t("importFromVaultDialog.errorMissingListPermission", { defaultValue: "The AWS principal behind this vault is missing secretsmanager:ListSecrets. Update IAM and try again." })
            : message}
        </div>
        <div className="mt-2 flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={onRetry}>
            <RefreshCw className="mr-1.5 h-3.5 w-3.5" /> {t("importFromVaultDialog.retry", { defaultValue: "Retry" })}
          </Button>
          {isPermission && (
            <a
            ...
            >
              {t("importFromVaultDialog.iamReference", { defaultValue: "IAM reference" })} <ExternalLink className="h-3 w-3" />
            </a>
          )}
        </div>
      </div>
...
function EmptyCandidates({ query }: { query: string }) {
  ...
  if (query) {
    return (
      <EmptyState
        icon={Search}
        message={t("importFromVaultDialog.noSecretsMatch", { defaultValue: "No remote secrets match \"{{query}}\".", query })}
      />
    );
  }
  return (
    <EmptyState
      icon={Database}
      message={t("importFromVaultDialog.noSecretsVisible", { defaultValue: "No secrets visible to this vault." })}
    />
  );
}
  ...
  if (drafts.length === 0) {
    return (
      <div className="flex min-h-0 flex-1 items-center justify-center p-6">
        <EmptyState
          icon={Info}
          message={t("importFromVaultDialog.noSecretsSelected", { defaultValue: "No secrets selected. Go back to pick remote secrets to import." })}
        />
      </div>
    );
  }
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex flex-wrap items-center gap-3 border-b border-border/60 bg-muted/20 px-5 py-3 text-xs">
        <span className="font-medium">{t("importFromVaultDialog.secretsReadyToImport", { defaultValue: "{{count}} secrets ready to import", count: ready })}</span>
        {blocked > 0 && (
          <span className="text-amber-600 dark:text-amber-400">
            {t("importFromVaultDialog.needAttention", { defaultValue: "{{count}} need attention before import", count: blocked })}
          </span>
        )}
      </div>
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0 flex-1 space-y-2">
                  ...
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                    <label className="flex flex-col gap-1 text-xs">
                      <span className="text-muted-foreground">{t("importFromVaultDialog.workcellNameLabel", { defaultValue: "Workcell name" })}</span>
                      <Input
                        value={draft.name}
                        onChange={(e) =>
                    ...
                    <label className="flex flex-col gap-1 text-xs">
                      <span className="text-muted-foreground">{t("importFromVaultDialog.keyLabel", { defaultValue: "Key" })}</span>
                      <Input
                        value={draft.key}
                        onChange={(e) =>
                    ...
                    <label className="flex flex-col gap-1 text-xs">
                      <span className="text-muted-foreground">{t("importFromVaultDialog.descriptionOptionalLabel", { defaultValue: "Description (optional)" })}</span>
                      <Input
                ...
                <Button
                  ...
                  aria-label={t("importFromVaultDialog.removeRowAriaLabel", { defaultValue: "Remove {{name}}", name: draft.candidate.remoteName })}
                  className="h-7 w-7"
                  disabled={importing}
                >
...
function ResultStep({ result, draftList }: ResultStepProps) {
  ...
  const heading =
    result.errorCount === result.results.length && result.errorCount > 0
      ? t("importFromVaultDialog.importFailedTitle", { defaultValue: "Import failed" })
      : result.errorCount === 0 && result.skippedCount === 0
        ? t("importFromVaultDialog.allSecretsImported", { defaultValue: "All {{count}} secrets imported", count: result.importedCount })
        : t("importFromVaultDialog.importComplete", { defaultValue: "Import complete" });
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="border-b border-border/60 px-5 py-3" data-testid="result-summary">
        ...
        <div className="mt-1 flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
          <span className="text-emerald-600 dark:text-emerald-400">{t("importFromVaultDialog.resultCreated", { defaultValue: "✓ {{count}} created", count: result.importedCount })}</span>
          <span>{t("importFromVaultDialog.resultSkipped", { defaultValue: "⊘ {{count}} skipped", count: result.skippedCount })}</span>
          <span className="text-destructive">{t("importFromVaultDialog.resultFailed", { defaultValue: "⨯ {{count}} failed", count: result.errorCount })}</span>
        </div>
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto">
        {grouped.created.length > 0 && (
          <ResultGroup label={t("importFromVaultDialog.groupCreated", { defaultValue: "Created" })} rows={grouped.created} draftLookup={draftLookup} />
        )}
        {grouped.skipped.length > 0 && (
          <ResultGroup label={t("importFromVaultDialog.groupSkipped", { defaultValue: "Skipped" })} rows={grouped.skipped} draftLookup={draftLookup} />
        )}
        {grouped.failed.length > 0 && (
          <ResultGroup label={t("importFromVaultDialog.groupFailed", { defaultValue: "Failed" })} rows={grouped.failed} draftLookup={draftLookup} />
        )}
      </div>
    </div>
  );
  ...
  if (step === "select") {
    return (
      <div className="text-xs text-muted-foreground">
        {totalSelected === 0
          ? t("importFromVaultDialog.footerSelectPrompt", { defaultValue: "Select remote secrets to import" })
          : t("importFromVaultDialog.footerSelectedCount", { defaultValue: "{{count}} selected", count: totalSelected })}
      </div>
    );
  }
  if (step === "review") {
    return (
      <div className="text-xs text-muted-foreground">
        {t("importFromVaultDialog.footerReadyCount", { defaultValue: "{{count}} ready", count: readyReviewCount })}
        {blockedReviewCount > 0 && (
          <span className="ml-2 text-amber-600 dark:text-amber-400">
            {t("importFromVaultDialog.footerBlockedCount", { defaultValue: "· {{count}} blocked", count: blockedReviewCount })}
          </span>
        )}
      </div>
    );
  ...
  if (result) {
    return (
      <div className="flex items-center gap-3 text-xs text-muted-foreground">
        <span>{t("importFromVaultDialog.footerCreatedCount", { defaultValue: "{{count}} created", count: result.importedCount })}</span>
        <span>{t("importFromVaultDialog.footerSkippedCount", { defaultValue: "{{count}} skipped", count: result.skippedCount })}</span>
        <span>{t("importFromVaultDialog.footerFailedCount", { defaultValue: "{{count}} failed", count: result.errorCount })}</span>
      </div>
    );
  }
  return null;
