...
export function JoinRequestQueue() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: selectedCompany?.name ?? t("joinRequestQueue.breadcrumb.company", { defaultValue: "Company" }), href: "/dashboard" },
  ...
  if (requestsQuery.error) {
    const message =
      requestsQuery.error instanceof ApiError && requestsQuery.error.status === 403
        ? t("joinRequestQueue.error.forbidden", { defaultValue: "You do not have permission to review join requests for this company." })
