...
describe("InviteLandingPage", () => {
  ...
  it("defaults invite auth to account creation and guides existing users back to sign in", async () => {
    ...
    expect(container.textContent).toContain("You've been invited to join Workcell");
    ...
    expect(localStorage.getItem("workcell:pending-invite-token")).toBe("pcp_invite_test");
  ...
  it("auto-accepts the invite after account creation and redirects into the company", async () => {
    ...
    expect(localStorage.getItem("workcell:pending-invite-token")).toBeNull();
  ...
  it("shows the pending approval page with the company icon and linked access instructions", async () => {
    ...
    expect(container.textContent).toContain("A team admin must approve your request to join.");
    expect(container.textContent).toContain(
      "Ask them to visit Team Settings → Members to approve your request.",
    );
    ...
    const approvalLinks = Array.from(container.querySelectorAll("a")).filter(
      (link) => link.textContent === "Team Settings → Members",
    );
  ...
  it("waits for the membership check before showing invite acceptance to signed-in users", async () => {
    ...
    expect(container.textContent).not.toContain("Accept team invite");
    expect(acceptInviteMock).not.toHaveBeenCalled();
