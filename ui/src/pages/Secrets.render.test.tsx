    ...
    candidates: [
      {
        ...
        config: {
          ...
          kmsKeyId: "alias/workcell-secrets",
          ownerTag: "platform",
...
describe("Secrets page layout", () => {
  ...
  it("warns that removing a provider vault only removes Workcell config", async () => {
    ...
    expect(document.body.textContent).toContain("from Workcell only");
    ...
    const confirmButton = [...document.querySelectorAll("button")].find(
      (button) => button.textContent?.includes("Remove from Workcell"),
    ) as HTMLButtonElement | undefined;
    await act(async () => {
      confirmButton?.click();
    });
  ...
  it("discovers AWS provider vault candidates and applies selected values as prefill", async () => {
    ...
    await act(async () => {
      ...
      setInputValue(prefixInput!, "workcell");
    });
    await flushReact();
    ...
    expect((document.getElementById("provider-vault-secret-name-prefix") as HTMLInputElement).value).toBe("workcell");
    expect((document.getElementById("provider-vault-kms-key-id") as HTMLInputElement).value).toBe("alias/workcell-secrets");
