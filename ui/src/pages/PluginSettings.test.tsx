...
function basePlugin(overrides: Record<string, unknown> = {}) {
  return {
    id: "plugin-1",
    pluginKey: "workcell.e2b-sandbox-provider",
    ...
    manifestJson: {
      ...
      version: "0.1.0",
      description: "E2B environments for Workcell.",
      author: "Workcell",
...
describe("PluginSettings", () => {
  ...
  it("routes environment-provider plugins to company environments when they have no instance config", async () => {
    ...
    expect(container.textContent).toContain("Configure this plugin from Team Environments.");
    expect(container.textContent).toContain("team-scoped instead of instance-global");
    const link = container.querySelector('a[href="/company/settings/environments"]');
    expect(link?.textContent).toContain("Open Team Environments");
