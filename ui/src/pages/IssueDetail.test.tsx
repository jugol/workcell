...
import type { Agent, Issue, IssueTreeControlPreview, IssueTreeHold } from "@workcell/shared";
...
vi.mock("../context/CompanyContext", () => ({
  useCompany: () => ({
    ...
    selectedCompany: { id: "company-1", name: "Workcell", issuePrefix: "PAP", status: "active" },
...
describe("IssueDetail", () => {
  ...
  it("loads from the pending state into issue detail without changing hook order", async () => {
    ...
    // First call (the issue load) stays pending; later calls — e.g. the
    // design-requirement query in IssueDesignReviewPanel re-fetches the same
    // issue — must resolve, or react-query logs "Query data cannot be
    // undefined" and trips the console-error assertion below.
  ...
  it("forwards composer work mode changes to the issues API", async () => {
    ...
    localStorage.setItem("workcell:issue-comment-draft:issue-1", "Draft follow-up message");
    ...
    expect(localStorage.getItem("workcell:issue-comment-draft:issue-1")).toBe("Draft follow-up message");
    expect(container.textContent).toContain("planning-notes.txt");
    localStorage.removeItem("workcell:issue-comment-draft:issue-1");
  });
