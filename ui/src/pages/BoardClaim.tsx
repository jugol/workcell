        ...
        <div className="rounded-lg border border-border bg-card p-6">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {statusQuery.error instanceof Error
              ...
              : t("boardClaim.unavailable.invalidOrExpired", { defaultValue: "Challenge is invalid or expired." })}
          </p>
        </div>
      </div>
    );
    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          ...
          <Button asChild className="mt-4">
            <Link to={`/auth?next=${encodeURIComponent(currentPath)}`}>{t("boardClaim.signInRequired.cta", { defaultValue: "Sign in / Create account" })}</Link>
          </Button>
        </div>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          {t("boardClaim.claim.description", {
            defaultValue: "This will promote your user to instance admin and migrate team ownership access from local trusted mode.",
          })}
        </p>
        ...
        {claimMutation.error && (
          <p className="mt-3 text-sm text-destructive">
            {claimMutation.error instanceof Error
              ...
              : t("boardClaim.claim.failed", { defaultValue: "Failed to claim board ownership" })}
          </p>
        )}
        ...
        >
          {claimMutation.isPending
            ? t("boardClaim.claim.claiming", { defaultValue: "Claiming…" })
            : t("boardClaim.claim.claimButton", { defaultValue: "Claim ownership" })}
        </Button>
      </div>
    </div>
  );
