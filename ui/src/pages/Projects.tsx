...
export function Projects() {
  ...
  const sortFieldLabel = (field: ProjectSortField, fallback: string) =>
    t(`projects.sort.options.${field}`, { defaultValue: fallback });
  const sortLabel = (() => {
    const option = PROJECT_SORT_OPTIONS.find((opt) => opt.field === sortField);
    return option ? sortFieldLabel(option.field, option.label) : sortFieldLabel("name", "Name");
  })();

  if (!selectedCompanyId) {
    return <EmptyState icon={Hexagon} message={t("projects.empty.noCompany", { defaultValue: "Select a company to view projects." })} />;
  }
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="ghost" size="sm" className="w-fit text-xs" title={t("projects.sort.title", { defaultValue: "Sort" })}>
          ...
          <PopoverContent align="start" className="w-44 p-0">
            <div className="p-2 space-y-0.5">
              {PROJECT_SORT_OPTIONS.map((option) => (
                ...
                >
                  <span>{sortFieldLabel(option.field, option.label)}</span>
                  {sortField === option.field ? (
                    <span className="flex items-center gap-1 text-xs text-muted-foreground">
                      <Check className="h-3 w-3" />
                      {sortDir === "asc"
                        ? t("projects.sort.asc", { defaultValue: "Asc" })
                        : t("projects.sort.desc", { defaultValue: "Desc" })}
                    </span>
                  ) : null}
                </button>
              ))}
      ...
      {!isLoading && projects.length === 0 && (
        <EmptyState
          ...
          action={t("projects.actions.add", { defaultValue: "Add Project" })}
      ...
      {projects.length > 0 && (
        <div className="space-y-6">
          {([
            ["mine", "My Projects", groupedProjects.mine],
            ["other", "Other Projects", groupedProjects.other],
          ] as const).map(([groupKey, label, sectionProjects]) => {
            if (sectionProjects.length === 0) return null;

            return (
              <section key={groupKey} className="space-y-2">
                <div className="flex items-center justify-between">
                  <h2 className="text-sm font-medium">{t(`projects.groups.${groupKey}`, { defaultValue: label })}</h2>
