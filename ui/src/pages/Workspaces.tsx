...
import type { ExecutionWorkspace, Issue, Project } from "@workcell/shared";
...
export function Workspaces() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("workspaces.breadcrumb", { defaultValue: "Workspaces" }) }]);
  }, [setBreadcrumbs, t]);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold">{t("workspaces.heading", { defaultValue: "Workspaces" })}</h2>
      </div>

      {groups.length === 0 ? (
        <p className="text-sm text-muted-foreground">{t("workspaces.emptyNoActivity", { defaultValue: "No workspace activity yet." })}</p>
      ) : (
              ...
              <div className="flex flex-wrap items-end justify-between gap-2">
                ...
                <span className="text-xs text-muted-foreground">
                  {group.summaries.length === 1
                    ? t("workspaces.workspaceCount_one", {
                        defaultValue: "{{count}} workspace",
                        count: group.summaries.length,
                      })
                    : t("workspaces.workspaceCount_other", {
