...
export function GoalDetail() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("goalDetail.goals", { defaultValue: "Goals" }), href: "/goals" },
      { label: goal?.title ?? goalId ?? t("goalDetail.goalFallback", { defaultValue: "Goal" }) }
    ]);
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-3">
        ...
        {/* Surface mutation failures so goal edits don't revert silently. */}
        {updateGoal.isError && (
          <p className="text-xs text-destructive" role="alert">
            {updateGoal.error instanceof Error
              ? updateGoal.error.message
              : t("goalDetail.updateFailed", { defaultValue: "Couldn't save changes — try again." })}
          </p>
        )}
        {uploadImage.isError && (
          <p className="text-xs text-destructive" role="alert">
            {uploadImage.error instanceof Error
              ...
              : t("goalDetail.uploadFailed", { defaultValue: "Couldn't upload image — try again." })}
          </p>
        )}
      </div>
      ...
      <Tabs defaultValue="children">
        <TabsList>
          <TabsTrigger value="children">
            {t("goalDetail.subGoalsTab", { defaultValue: "Sub-Goals ({{count}})", count: childGoals.length })}
          </TabsTrigger>
          <TabsTrigger value="projects">
            {t("goalDetail.projectsTab", { defaultValue: "Projects ({{count}})", count: linkedProjects.length })}
          </TabsTrigger>
        </TabsList>
