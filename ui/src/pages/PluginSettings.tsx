...
import type { PluginLocalFolderDeclaration } from "@workcell/shared";
...
export function PluginSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: selectedCompany?.name ?? t("pluginSettings.breadcrumb.company", { defaultValue: "Team" }), href: "/dashboard" },
      { label: t("pluginSettings.breadcrumb.settings", { defaultValue: "Settings" }), href: "/instance/settings/heartbeats" },
      { label: t("pluginSettings.breadcrumb.plugins", { defaultValue: "Plugins" }), href: "/instance/settings/plugins" },
      { label: plugin?.manifestJson?.displayName ?? plugin?.packageName ?? t("pluginSettings.breadcrumb.pluginDetails", { defaultValue: "Plugin Details" }) },
    ]);
  ...
  if (pluginLoading) {
    return <div className="p-4 text-sm text-muted-foreground">{t("pluginSettings.loading", { defaultValue: "Loading plugin details..." })}</div>;
  }
  ...
  const statusVariant =
    plugin.status === "ready"
      ...
      : plugin.status === "error"
        ? "destructive"
        : "secondary";
  const pluginDescription = plugin.manifestJson.description || t("pluginSettings.noDescription", { defaultValue: "No description provided." });
      ...
      <div className="flex items-center gap-4">
        <Link to="/instance/settings/plugins">
          <Button
            ...
            size="icon"
            className="h-8 w-8"
            aria-label={t("pluginSettings.backAria", { defaultValue: "Back to plugins" })}
          >
        ...
        <TabsContent value="configuration" className="space-y-6">
          <div className="space-y-8">
            <section className="space-y-5">
              <h2 className="text-base font-semibold">{t("pluginSettings.about.title", { defaultValue: "About" })}</h2>
            ...
            <section className="space-y-4">
              <div className="space-y-1">
                <h2 className="text-base font-semibold">{t("pluginSettings.settings.title", { defaultValue: "Settings" })}</h2>
              </div>
              {hasLocalFolders ? (
              ...
              {hasCustomSettingsPage ? (
                ...
                <div className="rounded-md border border-border/60 bg-muted/20 px-4 py-3 text-sm">
                  <p className="font-medium text-foreground">{t("pluginSettings.environments.heading", { defaultValue: "Configure this plugin from Team Environments." })}</p>
                  <p className="mt-1 text-muted-foreground">
                    {t("pluginSettings.environments.body", {
                      defaultValue: "{{driver}} registers environment runtime settings there so credentials stay team-scoped instead of instance-global.",
                      driver: driverLabel || t("pluginSettings.environments.thisPlugin", { defaultValue: "This plugin" }),
                    })}
                  </p>
                  <div className="mt-3">
                    <Link to="/company/settings/environments">
                      <Button variant="outline" size="sm">{t("pluginSettings.environments.openButton", { defaultValue: "Open Team Environments" })}</Button>
                    </Link>
                  </div>
                </div>
              ) : !hasLocalFolders ? (
                  ...
                  {dashboardData ? (
                    <>
                              ...
                              <>
                                <div className="flex justify-between col-span-2">
                                  ...
                                  <span className="text-xs">
                                    {t("pluginSettings.dashboard.crashCounts", {
                      ...
                      <div className="flex items-center gap-1.5 border-t border-border/50 pt-2 text-xs text-muted-foreground">
                        ...
                        {t("pluginSettings.dashboard.lastChecked", { defaultValue: "Last checked:" })} {new Date(dashboardData.checkedAt).toLocaleTimeString()}
                      </div>
                    </>
                  ) : (
                    <p className="text-sm text-muted-foreground">
                      {t("pluginSettings.dashboard.unavailable", { defaultValue: "Runtime diagnostics are unavailable right now." })}
                    </p>
                  )}
                </CardContent>
              </Card>
              ...
              {recentLogs && recentLogs.length > 0 ? (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>{t("pluginSettings.logs.description", { defaultValue: "Last {{count}} log entries", count: recentLogs.length })}</CardDescription>
                  </CardHeader>
                  <CardContent>
  ...
  if (!companyId) {
    return (
      <div className="rounded-md border border-border/60 bg-muted/20 px-4 py-3 text-sm text-muted-foreground">
        {t("pluginSettings.localFolders.selectCompany", { defaultValue: "Select a team to configure this plugin's local folders." })}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-sm font-medium">{t("pluginSettings.localFolders.title", { defaultValue: "Local folders" })}</h3>
      </div>
      {error ? (
...
function PluginLocalFolderRow({ pluginId, companyId, declaration, status }: PluginLocalFolderRowProps) {
    ...
    onSuccess: (nextStatus) => {
      setMessage({
        ...
        text: nextStatus.healthy
          ? t("pluginSettings.localFolderRow.savedHealthy", { defaultValue: "Local folder saved." })
          : t("pluginSettings.localFolderRow.savedNeedsAttention", { defaultValue: "Local folder saved, but validation still needs attention." }),
      });
      queryClient.invalidateQueries({ queryKey: queryKeys.plugins.localFolders(pluginId, companyId) });
    },
    onError: (err: Error) => {
      setMessage({ type: "error", text: err.message || t("pluginSettings.localFolderRow.saveError", { defaultValue: "Failed to save local folder." }) });
    },
  });
  ...
  const handleSave = useCallback(() => {
    if (!trimmedPath) {
      setMessage({ type: "error", text: t("pluginSettings.localFolderRow.pathRequired", { defaultValue: "Local folder path is required." }) });
      return;
    }
    if (!isLikelyAbsolutePath(trimmedPath)) {
      setMessage({ type: "error", text: t("pluginSettings.localFolderRow.pathMustBeAbsolute", { defaultValue: "Local folder must be a full absolute path." }) });
      return;
    }
    setMessage(null);
    ...
    <div className="space-y-4 rounded-md border border-border/70 bg-background px-4 py-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 space-y-1">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge variant={status?.healthy ? "default" : "secondary"}>
              {status?.healthy ? t("pluginSettings.localFolderRow.healthy", { defaultValue: "Healthy" }) : t("pluginSettings.localFolderRow.needsAttention", { defaultValue: "Needs attention" })}
            </Badge>
          </div>
        ...
        <Badge variant={access === "readWrite" ? "default" : "outline"}>
          {access === "readWrite" ? t("pluginSettings.localFolderRow.readWrite", { defaultValue: "Read/write" }) : t("pluginSettings.localFolderRow.readOnly", { defaultValue: "Read only" })}
        </Badge>
      </div>

      <div className="grid gap-3 text-sm sm:grid-cols-3">
        <FolderStatusMetric label={t("pluginSettings.localFolderRow.metricConfigured", { defaultValue: "Configured" })} value={status?.configured ? t("pluginSettings.localFolderRow.yes", { defaultValue: "Yes" }) : t("pluginSettings.localFolderRow.no", { defaultValue: "No" })} ok={!!status?.configured} />
        <FolderStatusMetric label={t("pluginSettings.localFolderRow.metricReadable", { defaultValue: "Readable" })} value={status?.readable ? t("pluginSettings.localFolderRow.yes", { defaultValue: "Yes" }) : t("pluginSettings.localFolderRow.no", { defaultValue: "No" })} ok={!!status?.readable} />
        <FolderStatusMetric
          label={t("pluginSettings.localFolderRow.metricWritable", { defaultValue: "Writable" })}
          value={access === "read" ? t("pluginSettings.localFolderRow.notRequested", { defaultValue: "Not requested" }) : status?.writable ? t("pluginSettings.localFolderRow.yes", { defaultValue: "Yes" }) : t("pluginSettings.localFolderRow.no", { defaultValue: "No" })}
      ...
      {status?.path ? (
        <div className="space-y-1 text-sm">
          <div className="text-xs font-medium text-muted-foreground">{t("pluginSettings.localFolderRow.configuredPath", { defaultValue: "Configured path" })}</div>
      ...
      <div className="space-y-1.5">
        <label className="text-xs font-medium text-muted-foreground" htmlFor={`local-folder-${declaration.folderKey}`}>
          {t("pluginSettings.localFolderRow.pathLabel", { defaultValue: "Local folder path" })}
        </label>
        <div className="flex items-center gap-2">
          <input
          ...
          >
            ...
            {t("pluginSettings.localFolderRow.save", { defaultValue: "Save" })}
          </Button>
        </div>
      </div>
      ...
      {status?.problems?.length ? (
        <div className="space-y-2 rounded-md border border-destructive/20 bg-destructive/10 px-3 py-2 text-sm text-destructive">
          <div className="font-medium">{t("pluginSettings.localFolderRow.validationProblems", { defaultValue: "Validation problems" })}</div>
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        {inspectionUnavailable ? (
          ...
          <Badge variant="destructive" className="text-[10px]">
            {t("pluginSettings.requirements.missingBadge", { defaultValue: "{{count}} missing", count: missingItems.length })}
          </Badge>
        ) : (
          <Badge variant="outline" className="text-[10px]">{t("pluginSettings.requirements.present", { defaultValue: "Present" })}</Badge>
        )}
      </div>
      {items.length > 0 ? (
...
function PluginConfigForm({ pluginId, schema, initialValues, isLoading, pluginStatus, supportsConfigTest }: PluginConfigFormProps) {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      setSaveMessage({ type: "success", text: t("pluginSettings.configForm.saved", { defaultValue: "Configuration saved." }) });
      setTestResult(null);
    ...
    onError: (err: Error) => {
      setSaveMessage({ type: "error", text: err.message || t("pluginSettings.configForm.saveError", { defaultValue: "Failed to save configuration." }) });
    },
  });
  ...
  const testMutation = useMutation({
    ...
    onSuccess: (result) => {
      if (result.valid) {
        setTestResult({ type: "success", text: t("pluginSettings.configForm.testPassed", { defaultValue: "Configuration test passed." }) });
      } else {
        setTestResult({ type: "error", text: result.message || t("pluginSettings.configForm.testFailed", { defaultValue: "Configuration test failed." }) });
      }
    },
    onError: (err: Error) => {
      setTestResult({ type: "error", text: err.message || t("pluginSettings.configForm.testFailed", { defaultValue: "Configuration test failed." }) });
    },
  });
  ...
  if (isLoading) {
    return (
      <div className="flex items-center gap-2 text-sm text-muted-foreground py-4">
        <Loader2 className="h-4 w-4 animate-spin" />
        {t("pluginSettings.configForm.loading", { defaultValue: "Loading configuration..." })}
      </div>
    );
  }
      ...
      <div className="flex items-center gap-2 pt-2">
        ...
        >
          {saveMutation.isPending ? (
            <>
              <Loader2 className="h-3.5 w-3.5 animate-spin" />
              {t("pluginSettings.configForm.saving", { defaultValue: "Saving..." })}
            </>
          ) : (
            t("pluginSettings.configForm.saveConfiguration", { defaultValue: "Save Configuration" })
          )}
        </Button>
        {pluginStatus === "ready" && supportsConfigTest && (
          ...
          >
            {testMutation.isPending ? (
              <>
                <Loader2 className="h-3.5 w-3.5 animate-spin" />
                {t("pluginSettings.configForm.testing", { defaultValue: "Testing..." })}
              </>
            ) : (
              t("pluginSettings.configForm.testConfiguration", { defaultValue: "Test Configuration" })
            )}
          </Button>
        )}
      </div>
