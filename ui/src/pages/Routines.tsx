...
import type { RoutineListItem, RoutineVariable } from "@workcell/shared";
...
function concurrencyPolicyDescription(t: TFunction, policy: string): string {
  return t(`routines.concurrencyDescriptions.${policy}`, {
...
function catchUpPolicyDescription(t: TFunction, policy: string): string {
  return t(`routines.catchUpDescriptions.${policy}`, {
...
export function Routines() {
  ...
  const routineViewStateKey = selectedCompanyId
    ? `workcell:routines-view:${selectedCompanyId}`
    : "workcell:routines-view";
  const [routineViewState, setRoutineViewState] = useState<RoutineViewState>(() => getRoutineViewState(routineViewStateKey));
  const { t } = useTranslation();

  useEffect(() => {
    setBreadcrumbs([{ label: t("routines.breadcrumb", { defaultValue: "Routines" }) }]);
  }, [setBreadcrumbs, t]);
  ...
  const createRoutine = useMutation({
    ...
    onSuccess: async (routine) => {
      ...
      pushToast({
        title: t("routines.toast.created.title", { defaultValue: "Routine created" }),
        body: routine.assigneeAgentId
          ? t("routines.toast.created.bodyReady", { defaultValue: "Add the first trigger to turn it into a live workflow." })
          : t("routines.toast.created.bodyDraft", { defaultValue: "Draft saved. Add a default agent before enabling automation." }),
        tone: "success",
      });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("routines.toast.updateFailed.title", { defaultValue: "Failed to update routine" }),
        body: mutationError instanceof Error ? mutationError.message : t("routines.toast.updateFailed.body", { defaultValue: "Workcell could not update the routine." }),
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("routines.toast.runFailed.title", { defaultValue: "Routine run failed" }),
        body: mutationError instanceof Error ? mutationError.message : t("routines.toast.runFailed.body", { defaultValue: "Workcell could not start the routine run." }),
        tone: "error",
      });
    },
  });
  ...
  function handleToggleEnabled(routine: RoutineListItem, enabled: boolean) {
    if (!enabled && !routine.assigneeAgentId) {
      pushToast({
        title: t("routines.toast.defaultAgentRequired.title", { defaultValue: "Default agent required" }),
        body: t("routines.toast.defaultAgentRequired.body", { defaultValue: "Set a default agent before enabling routine automation." }),
        tone: "warn",
      });
      return;
    }
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Repeat} message={t("routines.empty.noCompany", { defaultValue: "Select a company to view routines." })} />;
  }
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div className="space-y-1">
          <h1 className="text-2xl font-semibold tracking-tight">
            {t("routines.heading", { defaultValue: "Routines" })}
          </h1>
          <p className="text-sm text-muted-foreground">
            {t("routines.subheading", { defaultValue: "Recurring work definitions that materialize into auditable execution issues." })}
          </p>
        </div>
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange}>
        ...
        <TabsContent value="routines" className="space-y-4">
          <div className="flex items-center justify-between gap-3">
            <p className="text-sm text-muted-foreground">
              {t("routines.count", {
                defaultValue_one: "{{count}} routine",
            ...
            <div className="flex items-center gap-1">
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="ghost" size="sm" className="text-xs" title={t("routines.sort.title", { defaultValue: "Sort" })}>
                    <ArrowUpDown className="h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1" />
                    <span className="hidden sm:inline">{t("routines.sort.label", { defaultValue: "Sort" })}</span>
                  </Button>
                </PopoverTrigger>
                <PopoverContent align="end" className="w-44 p-0">
                  <div className="p-2 space-y-0.5">
                    {([
                      ...
                      >
                        <span>{t(`routines.sort.options.${field}`, { defaultValue: label })}</span>
                        {routineViewState.sortField === field ? (
                          <span className="text-xs text-muted-foreground">
                            {routineViewState.sortDir === "asc"
                              ? t("routines.sort.asc", { defaultValue: "Asc" })
                              : t("routines.sort.desc", { defaultValue: "Desc" })}
                          </span>
                        ) : null}
                      </button>
                    ))}
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="ghost" size="sm" className="text-xs" title={t("routines.group.title", { defaultValue: "Group" })}>
                    <Layers className="h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1" />
                    <span className="hidden sm:inline">{t("routines.group.label", { defaultValue: "Group" })}</span>
                  </Button>
                </PopoverTrigger>
                <PopoverContent align="end" className="w-44 p-0">
                  <div className="p-2 space-y-0.5">
                    {([
                      ...
                      >
                        <span>{t(`routines.group.options.${value}`, { defaultValue: label })}</span>
        ...
        <TabsContent value="runs">
          <IssuesList
            ...
            viewStateKey="workcell:routine-recent-runs-view"
      ...
      >
        ...
        >
          <div className="shrink-0 flex flex-wrap items-center justify-between gap-3 border-b border-border/60 px-5 py-3">
            <div>
              <p className="text-xs font-medium uppercase tracking-[0.2em] text-muted-foreground">{t("routines.composer.eyebrow", { defaultValue: "New routine" })}</p>
              <p className="text-sm text-muted-foreground">
                {t("routines.composer.subtitle", { defaultValue: "Define the recurring work first. Default project and agent are optional for draft routines." })}
              </p>
            </div>
            <Button
              variant="ghost"
            ...
            >
              {t("routines.composer.cancel", { defaultValue: "Cancel" })}
            </Button>
          </div>
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            <div className="px-5 pt-5 pb-3">
              <textarea
                ...
                placeholder={t("routines.composer.titlePlaceholder", { defaultValue: "Routine title" })}
                rows={1}
                value={draft.title}
                onChange={(event) => {
            ...
            <div className="px-5 pb-3">
              <div className="overflow-x-auto overscroll-x-contain">
                <div className="inline-flex min-w-full flex-wrap items-center gap-2 text-sm text-muted-foreground sm:min-w-max sm:flex-nowrap">
                  <span>{t("routines.composer.for", { defaultValue: "For" })}</span>
                  <InlineEntitySelector
                    ref={assigneeSelectorRef}
                    value={draft.assigneeAgentId}
                    options={assigneeOptions}
                    recentOptionIds={recentAssigneeIds}
                    placeholder={t("routines.composer.assignee.placeholder", { defaultValue: "Assignee" })}
                    noneLabel={t("routines.composer.assignee.none", { defaultValue: "No assignee" })}
                    searchPlaceholder={t("routines.composer.assignee.search", { defaultValue: "Search assignees..." })}
                    emptyMessage={t("routines.composer.assignee.empty", { defaultValue: "No assignees found." })}
                    ...
                    renderTriggerValue={(option) =>
                      option ? (
                        ...
                        <span className="text-muted-foreground">{t("routines.composer.assignee.placeholder", { defaultValue: "Assignee" })}</span>
                      )
                    }
                    renderOption={(option) => {
                  ...
                  />
                  <span>{t("routines.composer.in", { defaultValue: "in" })}</span>
                  <InlineEntitySelector
                    ref={projectSelectorRef}
                    value={draft.projectId}
                    options={projectOptions}
                    recentOptionIds={recentProjectIds}
                    placeholder={t("routines.composer.project.placeholder", { defaultValue: "Project" })}
                    noneLabel={t("routines.composer.project.none", { defaultValue: "No project" })}
                    searchPlaceholder={t("routines.composer.project.search", { defaultValue: "Search projects..." })}
                    emptyMessage={t("routines.composer.project.empty", { defaultValue: "No projects found." })}
                    onChange={(projectId) => {
                    ...
                    renderTriggerValue={(option) =>
                      option && currentProject ? (
                        ...
                        <span className="text-muted-foreground">{t("routines.composer.project.placeholder", { defaultValue: "Project" })}</span>
                      )
                    }
                    renderOption={(option) => {
                      if (!option.id) return <span className="truncate">{option.label}</span>;
            ...
            <div className="border-t border-border/60 px-5 py-4">
              <MarkdownEditor
                ...
                placeholder={t("routines.composer.instructionsPlaceholder", { defaultValue: "Add instructions..." })}
                bordered={false}
            ...
            <div className="border-t border-border/60 px-5 py-3">
              <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
                <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
                  <div>
                    <p className="text-sm font-medium">{t("routines.composer.advanced.title", { defaultValue: "Advanced delivery settings" })}</p>
                    <p className="text-sm text-muted-foreground">{t("routines.composer.advanced.subtitle", { defaultValue: "Keep policy controls secondary to the work definition." })}</p>
                  </div>
                ...
                <CollapsibleContent className="pt-3">
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-2">
                      <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">{t("routines.composer.concurrency", { defaultValue: "Concurrency" })}</p>
                      <Select
                      ...
                      <p className="text-xs text-muted-foreground">{concurrencyPolicyDescription(t, draft.concurrencyPolicy)}</p>
                    </div>
                    <div className="space-y-2">
                      <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">{t("routines.composer.catchUp", { defaultValue: "Catch-up" })}</p>
                      <Select
                      ...
                      <p className="text-xs text-muted-foreground">{catchUpPolicyDescription(t, draft.catchUpPolicy)}</p>
                    </div>
                  </div>
                </CollapsibleContent>
              </Collapsible>
          ...
          <div className="shrink-0 flex flex-col gap-3 border-t border-border/60 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="text-sm text-muted-foreground">
              {t("routines.composer.footerHint", { defaultValue: "After creation, Workcell takes you straight to trigger setup. Draft routines stay paused until you add a default agent." })}
            </div>
            <div className="flex flex-col gap-2 sm:items-end">
              ...
              >
                <Plus className="mr-2 h-4 w-4" />
                {createRoutine.isPending
                  ? t("routines.composer.creating", { defaultValue: "Creating..." })
                  : t("routines.actions.create", { defaultValue: "Create routine" })}
              </Button>
              {createRoutine.isError ? (
                <p className="text-sm text-destructive">
                  {createRoutine.error instanceof Error ? createRoutine.error.message : t("routines.composer.createFailed", { defaultValue: "Failed to create routine" })}
                </p>
              ) : null}
            </div>
          </div>
        ...
        <div>
          {(routines ?? []).length === 0 ? (
            <div className="py-12">
              <EmptyState
                icon={Repeat}
                message={t("routines.empty.noRoutines", { defaultValue: "No routines yet. Use Create routine to define the first recurring workflow." })}
              />
            </div>
          ) : (
