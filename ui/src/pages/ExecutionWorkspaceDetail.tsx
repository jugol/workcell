...
import type { ExecutionWorkspace, Issue, Project, ProjectWorkspace, RoutineListItem } from "@workcell/shared";
...
const EXECUTION_WORKSPACE_BASE_TAB_ITEMS: Array<{
  value: ExecutionWorkspaceBaseTab;
  defaultLabel: string;
  order: number;
}> = [
  { value: "issues", defaultLabel: "Issues", order: 10 },
  { value: "services", defaultLabel: "Services", order: 20 },
  { value: "configuration", defaultLabel: "Configuration", order: 30 },
  { value: "runtime_logs", defaultLabel: "Runtime logs", order: 40 },
  { value: "routines", defaultLabel: "Routines", order: 60 },
];
...
function LegacyWorkspaceTabRedirect({ workspaceId }: { workspaceId: string }) {
  useEffect(() => {
    try {
      localStorage.removeItem(`workcell:execution-workspace-tab:${workspaceId}`);
    } catch {}
  }, [workspaceId]);
...
function ExecutionWorkspaceIssuesList({
  ...
  return (
    <IssuesList
      ...
      viewStateKey="workcell:execution-workspace-issues-view"
    ...
    <div className="flex flex-col gap-3 border-b border-border px-3 py-3 last:border-b-0 sm:flex-row sm:items-center">
      <div className="min-w-0 flex-1 space-y-1.5">
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
          <span>{routine.assigneeAgentId ? t("executionWorkspaceDetail.defaultAgentSet", { defaultValue: "Default agent set" }) : t("executionWorkspaceDetail.chooseAgentWhenRunning", { defaultValue: "Choose agent when running" })}</span>
          <span>{t("executionWorkspaceDetail.lastRun", { defaultValue: "Last run" })} {formatOptionalDateTime(routine.lastRun?.triggeredAt ?? routine.lastTriggeredAt)}</span>
      ...
      >
        ...
        {isRunning ? t("executionWorkspaceDetail.running", { defaultValue: "Running..." }) : t("executionWorkspaceDetail.runNow", { defaultValue: "Run now" })}
      </Button>
    </div>
  );
}
...
function ExecutionWorkspaceRoutinesList({
  ...
  const runRoutine = useMutation({
    ...
    onSuccess: async (_, { id }) => {
      ...
      pushToast({
        title: t("executionWorkspaceDetail.toastRoutineStartedTitle", { defaultValue: "Routine started" }),
        body: t("executionWorkspaceDetail.toastRoutineStartedBody", { defaultValue: "Workcell created a run using this execution workspace." }),
        tone: "success",
      });
    },
    onSettled: () => {
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("executionWorkspaceDetail.toastRoutineRunFailedTitle", { defaultValue: "Routine run failed" }),
        body: mutationError instanceof Error ? mutationError.message : t("executionWorkspaceDetail.toastRoutineRunFailedBody", { defaultValue: "Workcell could not start the routine run." }),
        tone: "error",
      });
    },
  });
        ...
        <CardContent>
          {isLoading ? (
            ...
            <p className="text-sm text-destructive">
              {error instanceof Error ? error.message : t("executionWorkspaceDetail.errorLoadRoutines", { defaultValue: "Failed to load routines." })}
            </p>
          ) : workspaceRoutines.length === 0 ? (
            <div className="flex flex-col items-center gap-2 py-10 text-center">
              <Repeat className="h-5 w-5 text-muted-foreground" />
              <p className="text-sm text-muted-foreground">
                {t("executionWorkspaceDetail.noWorkspaceRoutines", { defaultValue: "No routines use workspace-specific variables yet." })}
              </p>
            </div>
          ) : (
...
export function ExecutionWorkspaceDetail() {
  ...
  const baseTabItems = useMemo<OrderedExecutionWorkspaceTabItem[]>(
    () =>
      EXECUTION_WORKSPACE_BASE_TAB_ITEMS.map((item) => ({
        value: item.value,
        label: t(`executionWorkspaceDetail.tabs.${item.value}`, { defaultValue: item.defaultLabel }),
        order: item.order,
      })),
    [t],
  );
  const workspaceTabItems = useMemo(
    () => orderExecutionWorkspaceTabItems([...baseTabItems, ...workspacePluginTabItems]),
    [baseTabItems, workspacePluginTabItems],
  );
  ...
  useEffect(() => {
    if (!workspace) return;
    const crumbs = [
      { label: t("executionWorkspaceDetail.breadcrumbProjects", { defaultValue: "Projects" }), href: "/projects" },
      ...(project ? [{ label: project.name, href: `/projects/${projectRef}` }] : []),
      ...(project ? [{ label: t("executionWorkspaceDetail.breadcrumbWorkspaces", { defaultValue: "Workspaces" }), href: `/projects/${projectRef}/workspaces` }] : []),
  ...
  const updateWorkspace = useMutation({
    ...
    onError: (error) => {
      setErrorMessage(error instanceof Error ? error.message : t("executionWorkspaceDetail.errorSaveWorkspace", { defaultValue: "Failed to save execution workspace." }));
    },
  });
  ...
  const controlRuntimeServices = useMutation({
    ...
    onSuccess: (result, request) => {
      ...
      setRuntimeActionMessage(
        request.action === "run"
          ? t("executionWorkspaceDetail.runtimeJobCompleted", { defaultValue: "Workspace job completed." })
          : request.action === "stop"
            ? t("executionWorkspaceDetail.runtimeServiceStopped", { defaultValue: "Workspace service stopped." })
            : request.action === "restart"
              ? t("executionWorkspaceDetail.runtimeServiceRestarted", { defaultValue: "Workspace service restarted." })
              : t("executionWorkspaceDetail.runtimeServiceStarted", { defaultValue: "Workspace service started." }),
      );
    },
    onError: (error) => {
      setRuntimeActionMessage(null);
      setRuntimeActionErrorMessage(error instanceof Error ? error.message : t("executionWorkspaceDetail.errorControlCommands", { defaultValue: "Failed to control workspace commands." }));
    },
  });

  if (workspaceQuery.isLoading) return <p className="text-sm text-muted-foreground">{t("executionWorkspaceDetail.loading", { defaultValue: "Loading workspace…" })}</p>;
  if (workspaceQuery.error) {
    return (
      <p className="text-sm text-destructive">
        {workspaceQuery.error instanceof Error ? workspaceQuery.error.message : t("executionWorkspaceDetail.errorLoad", { defaultValue: "Failed to load workspace" })}
      </p>
    );
  }
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0 space-y-2">
            <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">
              {t("executionWorkspaceDetail.kicker", { defaultValue: "Execution workspace" })}
            </div>
        ...
        {activeTab === "services" ? (
          <WorkspaceRuntimeControls
            ...
            serviceEmptyMessage={
              effectiveRuntimeConfig
                ? t("executionWorkspaceDetail.serviceEmptyStarted", { defaultValue: "No services have been started for this execution workspace yet." })
                : t("executionWorkspaceDetail.serviceEmptyNoConfig", { defaultValue: "No workspace command config is defined for this execution workspace yet." })
            }
            jobEmptyMessage={t("executionWorkspaceDetail.jobEmpty", { defaultValue: "No one-shot jobs are configured for this execution workspace yet." })}
            disabledHint={
              canStartRuntimeServices
                ? null
                : t("executionWorkspaceDetail.disabledHint", { defaultValue: "Execution workspaces need a working directory before local commands can run, and services also need runtime config." })
            }
            onAction={(request) => controlRuntimeServices.mutate(request)}
          />
          ...
          <div className="space-y-4 sm:space-y-6">
            <Card className="rounded-none">
              <CardHeader>
                ...
                <CardAction>
                  ...
                  >
                    {workspace.status === "cleanup_failed" ? t("executionWorkspaceDetail.retryClose", { defaultValue: "Retry close" }) : t("executionWorkspaceDetail.closeWorkspace", { defaultValue: "Close workspace" })}
                  </Button>
                </CardAction>
              </CardHeader>
              ...
              <div className="space-y-6">
                <div className="space-y-4">
                  ...
                  <Field label={t("executionWorkspaceDetail.fieldWorkspaceName", { defaultValue: "Workspace name" })}>
                    <Input
                      value={form.name}
                      onChange={(event) => setForm((current) => current ? { ...current, name: event.target.value } : current)}
                      placeholder={t("executionWorkspaceDetail.placeholderWorkspaceName", { defaultValue: "Execution workspace name" })}
                    />
                  </Field>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <Field label={t("executionWorkspaceDetail.fieldBranchName", { defaultValue: "Branch name" })} hint={t("executionWorkspaceDetail.hintBranchName", { defaultValue: "Useful for isolated worktrees" })}>
                      <Input
                        className="font-mono"
                    ...
                    <Field label={t("executionWorkspaceDetail.fieldBaseRef", { defaultValue: "Base ref" })}>
                      <Input
                        className="font-mono"
                        value={form.baseRef}
                  ...
                  <Field label={t("executionWorkspaceDetail.fieldRepoUrl", { defaultValue: "Repo URL" })}>
                    <Input
                      value={form.repoUrl}
                ...
                <div className="space-y-4">
                  ...
                  <Field label={t("executionWorkspaceDetail.fieldWorkingDirectory", { defaultValue: "Working directory" })}>
                    <Input
                      className="font-mono"
                      value={form.cwd}
                  ...
                  <Field label={t("executionWorkspaceDetail.fieldProviderRef", { defaultValue: "Provider path / ref" })}>
                    <Input
                      className="font-mono"
                ...
                <div className="space-y-4">
                  ...
                  <Field label={t("executionWorkspaceDetail.fieldProvisionCommand", { defaultValue: "Provision command" })} hint={t("executionWorkspaceDetail.hintProvisionCommand", { defaultValue: "Runs when Workcell prepares this execution workspace" })}>
                    <Textarea
                  ...
                  <Field label={t("executionWorkspaceDetail.fieldTeardownCommand", { defaultValue: "Teardown command" })} hint={t("executionWorkspaceDetail.hintTeardownCommand", { defaultValue: "Runs when the execution workspace is archived or cleaned up" })}>
                    <Textarea
                      className="min-h-20 font-mono"
                  ...
                  <Field label={t("executionWorkspaceDetail.fieldCleanupCommand", { defaultValue: "Cleanup command" })} hint={t("executionWorkspaceDetail.hintCleanupCommand", { defaultValue: "Workspace-specific cleanup before teardown" })}>
                    <Textarea
                ...
                <div className="space-y-4">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-sm text-muted-foreground">
                          {runtimeConfigSource === "execution_workspace"
                            ? t("executionWorkspaceDetail.runtimeSourceOverride", { defaultValue: "This execution workspace currently overrides the project workspace runtime config." })
                            : runtimeConfigSource === "project_workspace"
                              ? t("executionWorkspaceDetail.runtimeSourceInherit", { defaultValue: "This execution workspace is inheriting the project workspace runtime config." })
                  ...
                  <details className="rounded-md border border-dashed border-border/70 bg-background px-4 py-3">
                    <summary className="cursor-pointer text-sm font-medium">{t("executionWorkspaceDetail.advancedRuntimeJsonSummary", { defaultValue: "Advanced runtime JSON" })}</summary>
                    <p className="mt-2 text-sm text-muted-foreground">
                      {t("executionWorkspaceDetail.advancedRuntimeJsonDescription", { defaultValue: "Override the inherited workspace command model only when this execution workspace truly needs different service or job behavior." })}
                    </p>
                    <div className="mt-3">
                      <Field label={t("executionWorkspaceDetail.fieldWorkspaceCommandsJson", { defaultValue: "Workspace commands JSON" })} hint={t("executionWorkspaceDetail.hintWorkspaceCommandsJson", { defaultValue: "Legacy `services` arrays still work, but `commands` supports both services and jobs." })}>
                        <div className="mb-2 flex items-center gap-2 text-sm text-muted-foreground">
                          ...
                          />
                          <label htmlFor="inherit-runtime-config">{t("executionWorkspaceDetail.inheritRuntimeConfigLabel", { defaultValue: "Inherit project workspace runtime config" })}</label>
                        </div>
                        <Textarea
            ...
            <Card className="rounded-none">
              ...
              <CardContent>
              <DetailRow label={t("executionWorkspaceDetail.detailProject", { defaultValue: "Project" })}>
                {project ? <Link to={`/projects/${projectRef}`} className="hover:underline">{project.name}</Link> : <MonoValue value={workspace.projectId} />}
              </DetailRow>
              <DetailRow label={t("executionWorkspaceDetail.detailProjectWorkspace", { defaultValue: "Project workspace" })}>
              ...
              <DetailRow label={t("executionWorkspaceDetail.detailSourceIssue", { defaultValue: "Source issue" })}>
                {sourceIssue ? (
              ...
              <DetailRow label={t("executionWorkspaceDetail.detailDerivedFrom", { defaultValue: "Derived from" })}>
                {derivedWorkspace ? (
              ...
              <DetailRow label={t("executionWorkspaceDetail.detailWorkspaceId", { defaultValue: "Workspace ID" })}>
            ...
            <Card className="rounded-none">
              ...
              <CardContent>
              <DetailRow label={t("executionWorkspaceDetail.detailWorkingDir", { defaultValue: "Working dir" })}>
                {workspace.cwd ? <MonoValue value={workspace.cwd} copy /> : t("executionWorkspaceDetail.none", { defaultValue: "None" })}
              </DetailRow>
              <DetailRow label={t("executionWorkspaceDetail.detailProviderRef", { defaultValue: "Provider ref" })}>
                {workspace.providerRef ? <MonoValue value={workspace.providerRef} copy /> : t("executionWorkspaceDetail.none", { defaultValue: "None" })}
              </DetailRow>
              <DetailRow label={t("executionWorkspaceDetail.detailRepoUrl", { defaultValue: "Repo URL" })}>
              ...
              <DetailRow label={t("executionWorkspaceDetail.detailBaseRef", { defaultValue: "Base ref" })}>
                {workspace.baseRef ? <MonoValue value={workspace.baseRef} copy /> : t("executionWorkspaceDetail.none", { defaultValue: "None" })}
              </DetailRow>
              <DetailRow label={t("executionWorkspaceDetail.detailBranch", { defaultValue: "Branch" })}>
                {workspace.branchName ? <MonoValue value={workspace.branchName} copy /> : t("executionWorkspaceDetail.none", { defaultValue: "None" })}
              </DetailRow>
              <DetailRow label={t("executionWorkspaceDetail.detailOpened", { defaultValue: "Opened" })}>{formatDateTime(workspace.openedAt)}</DetailRow>
              <DetailRow label={t("executionWorkspaceDetail.detailLastUsed", { defaultValue: "Last used" })}>{formatDateTime(workspace.lastUsedAt)}</DetailRow>
              <DetailRow label={t("executionWorkspaceDetail.detailCleanup", { defaultValue: "Cleanup" })}>
          ...
          <Card className="rounded-none">
            ...
            {workspaceOperationsQuery.isLoading ? (
              <p className="text-sm text-muted-foreground">{t("executionWorkspaceDetail.loadingOperations", { defaultValue: "Loading workspace operations…" })}</p>
            ) : workspaceOperationsQuery.error ? (
              <p className="text-sm text-destructive">
                {workspaceOperationsQuery.error instanceof Error
                  ? workspaceOperationsQuery.error.message
                  : t("executionWorkspaceDetail.errorLoadOperations", { defaultValue: "Failed to load workspace operations." })}
              </p>
              ...
              <p className="text-sm text-muted-foreground">{t("executionWorkspaceDetail.noOperations", { defaultValue: "No workspace operations have been recorded yet." })}</p>
            )}
            </CardContent>
          </Card>
        ) : activeTab === "issues" ? (
          ...
          <Card>
            <CardContent className="py-6 text-sm text-muted-foreground">{t("executionWorkspaceDetail.loadingPlugin", { defaultValue: "Loading workspace plugin..." })}</CardContent>
          </Card>
