...
import { WorkcellMark } from "@/components/WorkcellMark";
    ...
    <div className="space-y-10 max-w-4xl">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Every component, style, and pattern used across Workcell.
        </p>
      </div>

      {/* ============================================================ */}
      {/*  BRAND (WC-70 Orchestration direction)                       */}
      {/* ============================================================ */}
      <Section title="Brand">
        <SubSection title="Mark — orchestration hub">
          <div className="flex items-end gap-6">
            <div className="flex flex-col items-center gap-2">
              <WorkcellMark className="size-10 text-primary" />
              <span className="text-xs text-muted-foreground">40px</span>
            </div>
            <div className="flex flex-col items-center gap-2">
              <WorkcellMark className="size-6 text-primary" />
              <span className="text-xs text-muted-foreground">24px</span>
            </div>
            <div className="flex flex-col items-center gap-2">
              <WorkcellMark className="size-4 text-primary" />
              <span className="text-xs text-muted-foreground">16px</span>
            </div>
          </div>
          <p className="text-sm text-muted-foreground mt-3">
            A central orchestrator node driving connected agent nodes — a company
            of agents under one board. Uses <code className="font-mono text-xs">currentColor</code>,
            tinted with the brand token. Replaces the inherited paperclip mark.
          </p>
        </SubSection>
        <SubSection title="Wordmark">
          <div className="flex items-center gap-2">
            <WorkcellMark className="size-6 text-primary" />
            <span className="text-lg font-semibold tracking-tight">Workcell</span>
          </div>
        </SubSection>
        <SubSection title="Brand accent">
          <div className="flex items-center gap-3">
            ...
            <div className="text-sm">
              <div className="font-medium">--primary · --ring · --sidebar-primary</div>
              <div className="text-xs font-mono text-muted-foreground">
                oklch(0.48 0.17 262) light · oklch(0.62 0.18 262) dark
              </div>
              <div className="text-xs text-muted-foreground mt-0.5">
                Orchestration indigo-blue. Surfaces stay neutral; status/priority colors are unchanged.
              </div>
            </div>
            <Button className="ml-2">Primary action</Button>
          </div>
        </SubSection>
      </Section>

      {/* ============================================================ */}
      {/*  PRINCIPLES (WC-80 reality-check #4)                          */}
      {/* ============================================================ */}
      <Section title="Workcell Principles">
        <SubSection title="Operating model — card">
          <WorkcellPrinciples className="max-w-xl" />
        </SubSection>
        <SubSection title="Bare (no card chrome / custom heading)">
          <WorkcellPrinciples variant="bare" heading="Our principles" className="max-w-xl" />
        </SubSection>
      </Section>
      ...
      <Section title="Breadcrumb">
        <Breadcrumb>
          <BreadcrumbList>
            ...
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbLink href="#">Workcell App</BreadcrumbLink>
            </BreadcrumbItem>
            <BreadcrumbSeparator />
            <BreadcrumbItem>
      ...
      <Section title="Identity">
        ...
        <SubSection title="Initials derivation">
          <div className="flex flex-col gap-2">
            <Identity name="Orchestrator Agent" size="sm" />
      ...
      {/*  GRAPH CANVAS                                                 */}
      {/* ============================================================ */}
      <Section title="Graph Canvas">
        <SubSection title="Nodes + edges">
          <p className="mb-2 text-xs text-muted-foreground">
            Generic pannable / zoomable node-edge canvas. Pass plain{" "}
            <code className="font-mono">{`{ id, label, badge?, tint? }`}</code> nodes and{" "}
            <code className="font-mono">{`{ id, fromNodeId, toNodeId, label? }`}</code> edges;
            layout is computed internally. Drag to pan, wheel or the buttons to zoom. Used by the
            agent memory tab.
          </p>
          <GraphCanvas
            className="h-72"
            aria-label="Example graph"
            nodes={[
              { id: "a", label: "Prefers TypeScript", badge: "Fact", tint: "border-blue-500/40 bg-blue-50/60 text-blue-700 dark:bg-blue-950/30 dark:text-blue-300" },
              { id: "b", label: "Ship weekly", badge: "Preference", tint: "border-violet-500/40 bg-violet-50/60 text-violet-700 dark:bg-violet-950/30 dark:text-violet-300" },
              { id: "c", label: "Acme Corp", badge: "Entity", tint: "border-emerald-500/40 bg-emerald-50/60 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-300" },
              { id: "d", label: "Use feature flags", badge: "Decision", tint: "border-amber-500/40 bg-amber-50/60 text-amber-700 dark:bg-amber-950/30 dark:text-amber-300" },
            ]}
            edges={[
              { id: "e1", fromNodeId: "a", toNodeId: "d", label: "supports" },
              { id: "e2", fromNodeId: "c", toNodeId: "b", label: "about" },
            ]}
          />
        </SubSection>
      </Section>
