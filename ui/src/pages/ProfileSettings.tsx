...
import type { AuthSession, CurrentUserProfile, UpdateCurrentUserProfile } from "@workcell/shared";
...
export function ProfileSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("profileSettings.breadcrumbInstanceSettings", { defaultValue: "Instance Settings" }) },
      { label: t("profileSettings.breadcrumbProfile", { defaultValue: "Profile" }) },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("profileSettings.errorUpdate", { defaultValue: "Failed to update profile." }));
    },
  });
  ...
  const uploadAvatarMutation = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) {
        throw new Error(t("profileSettings.errorSelectCompany", { defaultValue: "Select a company before uploading a profile avatar." }));
      }
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("profileSettings.errorUploadAvatar", { defaultValue: "Failed to upload avatar." }));
    },
  });
  ...
  const removeAvatarMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("profileSettings.errorRemoveAvatar", { defaultValue: "Failed to remove avatar." }));
    },
  });
  ...
  if (sessionQuery.error || !sessionQuery.data) {
    return (
      <div className="text-sm text-destructive">
        {sessionQuery.error instanceof Error ? sessionQuery.error.message : t("profileSettings.errorLoad", { defaultValue: "Failed to load profile." })}
      </div>
    );
  }
  ...
  const uploadHint = selectedCompany
    ? t("profileSettings.uploadHintCompany", { defaultValue: "Stored in Workcell file storage for {{company}}.", company: selectedCompany.name })
    : t("profileSettings.uploadHintNoCompany", { defaultValue: "Select a company to upload an avatar into Workcell storage." });
    ...
    <div className="max-w-4xl space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          {t("profileSettings.subtitle", { defaultValue: "Control how your account appears in the sidebar and other board surfaces." })}
        </p>
      </div>
      ...
      <section className="space-y-8">
            ...
            <div className="flex flex-wrap items-end gap-5 rounded-[24px] border border-border/70 bg-background/92 p-5 shadow-[0_18px_44px_-28px_rgba(0,0,0,0.45)] backdrop-blur-sm">
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    {uploadAvatarMutation.isPending ? <LoaderCircle className="size-4 animate-spin" /> : <Camera className="size-4" />}
                    {currentImage ? t("profileSettings.changePhoto", { defaultValue: "Change photo" }) : t("profileSettings.uploadPhoto", { defaultValue: "Upload photo" })}
                  </Button>
                  {currentImage ? (
                    <Button
                      type="button"
                    ...
                    >
                      ...
                      {t("profileSettings.removePhoto", { defaultValue: "Remove" })}
                    </Button>
                  ) : null}
                </div>
              </div>
              ...
              <div className="min-w-0 flex-1 space-y-2 pb-1">
                <div>
                  ...
                  <p className="truncate text-sm text-muted-foreground">{sessionQuery.data.user.email ?? t("profileSettings.noEmail", { defaultValue: "No email" })}</p>
                </div>
                <p className="max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("profileSettings.avatarHint", { defaultValue: "Click the avatar to upload a new image. {{hint}}", hint: uploadHint })}
                </p>
              </div>
            </div>
          </div>
        ...
        >
          <div className="space-y-2">
            <Label htmlFor="profile-name">{t("profileSettings.displayNameLabel", { defaultValue: "Display name" })}</Label>
            <Input
              id="profile-name"
              value={name}
              onChange={(event) => setName(event.target.value)}
              maxLength={120}
              placeholder={t("profileSettings.displayNamePlaceholder", { defaultValue: "Board" })}
            />
            <p className="text-xs text-muted-foreground">
              {t("profileSettings.displayNameHint", { defaultValue: "Shown in the sidebar account footer and comment author surfaces." })}
            </p>
          </div>

          <div className="space-y-2">
            <Label htmlFor="profile-email">{t("profileSettings.emailLabel", { defaultValue: "Email" })}</Label>
            <Input
              id="profile-email"
            ...
            />
            <p className="text-xs text-muted-foreground">
              {t("profileSettings.emailHint", { defaultValue: "Email is managed by your auth session and is read-only here." })}
            </p>
          </div>
          ...
          <div className="md:col-span-2 flex justify-end">
            <Button type="submit" disabled={isSavingProfile || !name.trim()}>
              ...
              {updateMutation.isPending ? t("profileSettings.saving", { defaultValue: "Saving..." }) : t("profileSettings.saveProfile", { defaultValue: "Save profile" })}
            </Button>
          </div>
        </form>
      </section>
