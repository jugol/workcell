...
export function CompanySettingsPluginPage() {
  ...
  if (!resolvedCompanyId) {
    ...
    return <div className="text-sm text-muted-foreground">{t("pluginPage.selectCompany", { defaultValue: "Select a team to view this page." })}</div>;
  }
  ...
  if (pageSlots.length > 1) {
    return (
      <div className="rounded-md border border-destructive/30 bg-destructive/5 px-3 py-2 text-sm text-destructive">
        {t("pluginPage.conflictSettingsBefore", { defaultValue: "Multiple plugins declare the team settings route" })} <code>{settingsRoutePath}</code>{t("pluginPage.conflictSettingsAfter", { defaultValue: ". Disable one plugin or change its route." })}
      </div>
    );
  }
