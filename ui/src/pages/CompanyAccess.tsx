...
export function CompanyAccess() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: selectedCompany?.name ?? t("companyAccess.breadcrumb.company", { defaultValue: "Company" }), href: "/dashboard" },
  ...
  if (membersQuery.error) {
    const message =
      membersQuery.error instanceof ApiError && membersQuery.error.status === 403
        ? t("companyAccess.error.forbidden", { defaultValue: "You do not have permission to manage company members." })
        : membersQuery.error instanceof Error
          ? membersQuery.error.message
          : t("companyAccess.error.loadFailed", { defaultValue: "Failed to load company members." });
    ...
    <div className="max-w-6xl space-y-8">
      ...
      {access && !access.currentUserRole && (
        <div className="rounded-xl border border-amber-500/40 px-4 py-3 text-sm text-amber-200">
          {t("companyAccess.instanceAdminNotice", { defaultValue: "This account can manage access here through instance-admin privileges, but it does not currently hold an active company membership." })}
        </div>
      )}
      ...
      <section className="space-y-4">
        <div className="space-y-1">
          ...
          <p className="max-w-3xl text-sm text-muted-foreground">
            {t("companyAccess.humans.description", { defaultValue: "Manage human company memberships and status here." })}
          </p>
        </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {t("companyAccess.pendingJoins.description", { defaultValue: "Review pending join requests before they become active company members." })}
                </p>
              </div>
        ...
        <div className="overflow-hidden rounded-xl border border-border">
          ...
          {members.length === 0 ? (
            <div className="px-4 py-8 text-sm text-muted-foreground">{t("companyAccess.table.empty", { defaultValue: "No user memberships found for this company yet." })}</div>
          ) : (
            members.map((member) => {
            ...
            <div className="space-y-5">
              <div className="grid gap-4 md:grid-cols-2">
                <label className="space-y-2 text-sm">
                  <span className="font-medium">{t("companyAccess.editDialog.companyRole", { defaultValue: "Company role" })}</span>
                  <select
  ...
  return (
    <div className="max-w-2xl space-y-5">
      <div className="space-y-3">
        ...
        <p className="text-sm text-muted-foreground">
          {t("companyAccess.legacy.description", { defaultValue: "Advanced access, scoped assignment, and explicit grant controls are provided by installed company settings extensions." })}
        </p>
      </div>
      ...
      <div className="space-y-4 rounded-xl border border-border px-5 py-5">
        <div className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground">
            {t("companyAccess.legacy.unavailableBody", { defaultValue: "Core Workcell keeps enforcing company boundaries and any existing restrictive policy data, but editing advanced permissions requires an installed extension." })}
          </p>
          {errorMessage ? (
