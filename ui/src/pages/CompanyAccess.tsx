...
export function CompanyAccess() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: selectedCompany?.name ?? t("companyAccess.breadcrumb.company", { defaultValue: "Team" }), href: "/dashboard" },
      { label: t("companyAccess.breadcrumb.settings", { defaultValue: "Settings" }), href: "/company/settings" },
      { label: t("companyAccess.breadcrumb.members", { defaultValue: "Members" }) },
    ]);
  ...
  const updateMemberMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({
        title: t("companyAccess.toast.memberUpdated", { defaultValue: "Member updated" }),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("companyAccess.toast.memberUpdateFailed", { defaultValue: "Failed to update member" }),
        body: error instanceof Error ? error.message : t("companyAccess.toast.unknownError", { defaultValue: "Unknown error" }),
        tone: "error",
      });
    },
  });
  ...
  const approveJoinRequestMutation = useMutation({
    ...
    onSuccess: async () => {
      await refreshAccessData();
      pushToast({
        title: t("companyAccess.toast.joinRequestApproved", { defaultValue: "Join request approved" }),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("companyAccess.toast.joinRequestApproveFailed", { defaultValue: "Failed to approve join request" }),
        body: error instanceof Error ? error.message : t("companyAccess.toast.unknownError", { defaultValue: "Unknown error" }),
        tone: "error",
      });
    },
  ...
  const rejectJoinRequestMutation = useMutation({
    ...
    onSuccess: async () => {
      await refreshAccessData();
      pushToast({
        title: t("companyAccess.toast.joinRequestRejected", { defaultValue: "Join request rejected" }),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("companyAccess.toast.joinRequestRejectFailed", { defaultValue: "Failed to reject join request" }),
        body: error instanceof Error ? error.message : t("companyAccess.toast.unknownError", { defaultValue: "Unknown error" }),
        tone: "error",
      });
    },
  ...
  const archiveMemberMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        title: t("companyAccess.toast.memberRemoved", { defaultValue: "Member removed" }),
        body:
    ...
    onError: (error) => {
      pushToast({
        title: t("companyAccess.toast.memberRemoveFailed", { defaultValue: "Failed to remove member" }),
        body: error instanceof Error ? error.message : t("companyAccess.toast.unknownError", { defaultValue: "Unknown error" }),
        tone: "error",
      });
    },
  ...
  if (!selectedCompanyId) {
    return <div className="text-sm text-muted-foreground">{t("companyAccess.selectCompany", { defaultValue: "Select a team to manage access." })}</div>;
  }

  if (membersQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("companyAccess.loading", { defaultValue: "Loading team access…" })}</div>;
  }

  if (membersQuery.error) {
    const message =
      membersQuery.error instanceof ApiError && membersQuery.error.status === 403
        ? t("companyAccess.error.forbidden", { defaultValue: "You do not have permission to manage team members." })
        : membersQuery.error instanceof Error
          ? membersQuery.error.message
          : t("companyAccess.error.loadFailed", { defaultValue: "Failed to load team members." });
    ...
    <div className="max-w-6xl space-y-8">
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("companyAccess.title", { defaultValue: "Team Members" })}</h1>
        </div>
        <p className="max-w-3xl text-sm text-muted-foreground">
          {t("companyAccess.description", {
            defaultValue: "Manage the people who can work in {{company}}. Members can collaborate across the team by default.",
            company: selectedCompany?.name ?? "",
          })}
        </p>
        <div className="rounded-lg border border-border bg-muted/30 px-4 py-3 text-sm text-muted-foreground">
          {t("companyAccess.coreNote", { defaultValue: "Core keeps this page focused on membership, invite approvals, and safe member removal." })}
        </div>
      </div>

      {access && !access.currentUserRole && (
        <div className="rounded-xl border border-amber-500/40 px-4 py-3 text-sm text-amber-200">
          {t("companyAccess.instanceAdminNotice", { defaultValue: "This account can manage access here through instance-admin privileges, but it does not currently hold an active team membership." })}
        </div>
      )}
      ...
      <section className="space-y-4">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold">{t("companyAccess.humans.title", { defaultValue: "Humans" })}</h2>
          </div>
          <p className="max-w-3xl text-sm text-muted-foreground">
            {t("companyAccess.humans.description", { defaultValue: "Manage human team memberships and status here." })}
          </p>
        </div>
        ...
        {access?.canApproveJoinRequests && pendingHumanJoinRequests.length > 0 ? (
          <div className="space-y-3 rounded-xl border border-border px-4 py-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                <h3 className="text-sm font-semibold">{t("companyAccess.pendingJoins.title", { defaultValue: "Pending human joins" })}</h3>
                <p className="text-sm text-muted-foreground">
                  {t("companyAccess.pendingJoins.description", { defaultValue: "Review pending join requests before they become active team members." })}
                </p>
              </div>
              <Badge variant="outline">{pendingHumanJoinRequests.length} {t("companyAccess.pendingJoins.pending", { defaultValue: "pending" })}</Badge>
            </div>
            <div className="space-y-3">
              {pendingHumanJoinRequests.map((request) => (
                <PendingJoinRequestCard
                  ...
                  title={
                    ...
                    t("companyAccess.pendingJoins.unknownRequester", { defaultValue: "Unknown human requester" })
                  }
                  subtitle={
                    request.requesterUser?.email ||
                    request.requestEmailSnapshot ||
                    request.requestingUserId ||
                    t("companyAccess.pendingJoins.noEmail", { defaultValue: "No email available" })
                  }
                  context={
                    request.invite
                      ? `${request.invite.allowedJoinTypes} join invite${request.invite.humanRole ? ` • default role ${request.invite.humanRole}` : ""}`
                      : t("companyAccess.pendingJoins.inviteMetadataUnavailable", { defaultValue: "Invite metadata unavailable" })
                  }
                  detail={`${t("companyAccess.pendingJoins.submitted", { defaultValue: "Submitted" })} ${new Date(request.createdAt).toLocaleString()}`}
                  approveLabel={t("companyAccess.pendingJoins.approve", { defaultValue: "Approve human" })}
                  rejectLabel={t("companyAccess.pendingJoins.reject", { defaultValue: "Reject human" })}
        ...
        <div className="overflow-hidden rounded-xl border border-border">
          ...
          {members.length === 0 ? (
            <div className="px-4 py-8 text-sm text-muted-foreground">{t("companyAccess.table.empty", { defaultValue: "No user memberships found for this team yet." })}</div>
          ) : (
            members.map((member) => {
                ...
                >
                  ...
                  <div className="text-sm">
                    {member.membershipRole
                      ...
                      : t("companyAccess.table.roleUnset", { defaultValue: "Unset" })}
                  </div>
                  <div>
      ...
      <Dialog open={!!editingMember} onOpenChange={(open) => !open && setEditingMemberId(null)}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>{t("companyAccess.editDialog.title", { defaultValue: "Edit member" })}</DialogTitle>
            <DialogDescription>
              {t("companyAccess.editDialog.description", {
                defaultValue: "Update team role and membership status for {{member}}.",
            ...
            <div className="space-y-5">
              <div className="grid gap-4 md:grid-cols-2">
                <label className="space-y-2 text-sm">
                  <span className="font-medium">{t("companyAccess.editDialog.companyRole", { defaultValue: "Team role" })}</span>
                  <select
                  ...
                  >
                    <option value="">{t("companyAccess.editDialog.roleUnset", { defaultValue: "Unset" })}</option>
                ...
                <label className="space-y-2 text-sm">
                  <span className="font-medium">{t("companyAccess.editDialog.membershipStatus", { defaultValue: "Membership status" })}</span>
                  <select
                    className="w-full rounded-md border border-border bg-background px-3 py-2"
                    value={draftStatus}
                    onChange={(event) =>
                  ...
                  >
                    <option value="active">{t("companyAccess.editDialog.statusActive", { defaultValue: "Active" })}</option>
                    <option value="pending">{t("companyAccess.editDialog.statusPending", { defaultValue: "Pending" })}</option>
                    <option value="suspended">{t("companyAccess.editDialog.statusSuspended", { defaultValue: "Suspended" })}</option>
                  </select>
                </label>
              </div>
            </div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setEditingMemberId(null)}>
              {t("companyAccess.editDialog.cancel", { defaultValue: "Cancel" })}
            </Button>
            <Button
              onClick={() => {
                if (!editingMember) return;
            ...
            >
              {updateMemberMutation.isPending ? t("companyAccess.editDialog.saving", { defaultValue: "Saving…" }) : t("companyAccess.editDialog.save", { defaultValue: "Save member" })}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!removingMember} onOpenChange={(open) => !open && setRemovingMemberId(null)}>
        <DialogContent className="max-w-xl">
          <DialogHeader>
            <DialogTitle>{t("companyAccess.removeDialog.title", { defaultValue: "Remove member" })}</DialogTitle>
            <DialogDescription>
              {t("companyAccess.removeDialog.description", {
            ...
            <div className="space-y-5">
              <div className="rounded-lg border border-border px-3 py-3">
                ...
                <div className="mt-2 text-sm text-muted-foreground">
                  {assignedIssuesQuery.isLoading
                    ? t("companyAccess.removeDialog.checkingIssues", { defaultValue: "Checking assigned issues..." })
              ...
              {assignedIssues.length > 0 ? (
                <div className="space-y-2">
                  <div className="text-sm font-medium">{t("companyAccess.removeDialog.reassignmentLabel", { defaultValue: "Issue reassignment" })}</div>
                  <select
                    className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
                    value={reassignmentTarget}
                    onChange={(event) => setReassignmentTarget(event.target.value)}
                  >
                    <option value="__unassigned">{t("companyAccess.removeDialog.leaveUnassigned", { defaultValue: "Leave unassigned" })}</option>
                    {activeReassignmentUsers.length > 0 ? (
                      <optgroup label={t("companyAccess.removeDialog.optgroupHumans", { defaultValue: "Humans" })}>
                    ...
                    {activeReassignmentAgents.length > 0 ? (
                      <optgroup label={t("companyAccess.removeDialog.optgroupAgents", { defaultValue: "Agents" })}>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setRemovingMemberId(null)}>
              {t("companyAccess.removeDialog.cancel", { defaultValue: "Cancel" })}
            </Button>
            <Button
              variant="destructive"
              onClick={() => {
            ...
            >
              {archiveMemberMutation.isPending ? t("companyAccess.removeDialog.removing", { defaultValue: "Removing..." }) : t("companyAccess.removeDialog.confirm", { defaultValue: "Remove member" })}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
...
export function CompanyAccessLegacyRoute() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("companyAccess.legacy.breadcrumbSettings", { defaultValue: "Settings" }), href: "/company/settings" },
      { label: t("companyAccess.legacy.breadcrumbAccess", { defaultValue: "Access" }) },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  if (isLoading) {
    return <div className="text-sm text-muted-foreground">{t("companyAccess.legacy.checking", { defaultValue: "Checking for advanced permission extensions..." })}</div>;
  }
    ...
    <div className="max-w-2xl space-y-5">
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("companyAccess.legacy.title", { defaultValue: "Advanced Permissions" })}</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          {t("companyAccess.legacy.description", { defaultValue: "Advanced access, scoped assignment, and explicit grant controls are provided by installed team settings extensions." })}
        </p>
      </div>

      <div className="space-y-4 rounded-xl border border-border px-5 py-5">
        <div className="space-y-2">
          <h2 className="text-sm font-semibold">{t("companyAccess.legacy.unavailableTitle", { defaultValue: "Advanced permissions unavailable" })}</h2>
          <p className="text-sm text-muted-foreground">
            {t("companyAccess.legacy.unavailableBody", { defaultValue: "Core Workcell keeps enforcing team boundaries and any existing restrictive policy data, but editing advanced permissions requires an installed extension." })}
          </p>
          {errorMessage ? (
            <p className="text-sm text-destructive">{t("companyAccess.legacy.extensionsUnavailable", { defaultValue: "Plugin extensions unavailable:" })} {errorMessage}</p>
          ) : null}
        </div>
        <div className="flex flex-wrap gap-2">
          <Button asChild>
            <Link to="/company/settings/members">{t("companyAccess.legacy.openMembers", { defaultValue: "Open Members" })}</Link>
          </Button>
          <Button asChild variant="outline">
            <Link to="/company/settings/invites">{t("companyAccess.legacy.openInvites", { defaultValue: "Open Invites" })}</Link>
          </Button>
        </div>
      </div>
    </div>
