...
import { PROJECT_COLORS, isUuidLike, type BudgetPolicySummary } from "@workcell/shared";
...
type ProjectBaseTab = "overview" | "list" | "plugin-operations" | "workspaces" | "configuration" | "budget" | "design";
  ...
  return (
    <div className="space-y-6">
      <InlineEditor
        ...
        placeholder={t("projectDetail.overview.descriptionPlaceholder", { defaultValue: "Add a description..." })}
        multiline
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
        <div>
          <span className="text-muted-foreground">{t("projectDetail.overview.status", { defaultValue: "Status" })}</span>
          <div className="mt-1">
        ...
        {project.targetDate && (
          <div>
            <span className="text-muted-foreground">{t("projectDetail.overview.targetDate", { defaultValue: "Target Date" })}</span>
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        aria-label={t("projectDetail.colorPicker.change", { defaultValue: "Change project color" })}
      />
      {open && (
          ...
          <div className="grid grid-cols-5 gap-1.5">
            {PROJECT_COLORS.map((color) => (
              <button
                ...
                aria-label={t("projectDetail.colorPicker.select", { defaultValue: "Select color {{color}}", color })}
              />
            ))}
          </div>
        </div>
...
function ProjectIssuesList({ projectId, companyId }: { projectId: string; companyId: string }) {
  ...
  return (
    <IssuesList
      ...
      projectId={projectId}
      viewStateKey="workcell:project-issues-view"
...
function ProjectPluginOperationsList({
  ...
  return (
    <IssuesList
      ...
      liveIssueIds={liveIssueIds}
      projectId={projectId}
      viewStateKey={`workcell:project-plugin-operations-view:${pluginKey}`}
      onUpdateIssue={(id, data) => updateIssue.mutate({ id, data })}
    />
  );
}
...
export function ProjectDetail() {
  ...
  const archiveProject = useMutation({
    ...
    onSuccess: (updatedProject, archived) => {
      invalidateProject();
      const name = updatedProject?.name ?? project?.name ?? t("projectDetail.fallbackName", { defaultValue: "Project" });
      if (archived) {
        pushToast({ title: t("projectDetail.toast.archived", { defaultValue: "\"{{name}}\" has been archived", name }), tone: "success" });
        navigate("/dashboard");
      } else {
        pushToast({ title: t("projectDetail.toast.unarchived", { defaultValue: "\"{{name}}\" has been unarchived", name }), tone: "success" });
      }
    },
    onError: (_, archived) => {
      pushToast({
        title: archived
          ? t("projectDetail.toast.archiveFailed", { defaultValue: "Failed to archive project" })
          : t("projectDetail.toast.unarchiveFailed", { defaultValue: "Failed to unarchive project" }),
        tone: "error",
      });
    },
  });
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("projectDetail.breadcrumb.projects", { defaultValue: "Projects" }), href: "/projects" },
      { label: project?.name ?? routeProjectRef ?? t("projectDetail.fallbackName", { defaultValue: "Project" }) },
    ]);
  ...
  useEffect(() => {
    ...
    if (activeTab === "design") {
      navigate(`/projects/${canonicalProjectRef}/design`, { replace: true });
      return;
    }
  ...
  const projectBudgetSummary = useMemo(() => {
    ...
    return {
      ...
      scopeName: project?.name ?? t("projectDetail.fallbackName", { defaultValue: "Project" }),
      metric: "billed_cents",
      windowKind: "lifetime",
      amount: 0,
      observedAmount: 0,
  ...
  if (routeProjectRef && activeTab === null) {
    ...
    if (project?.id) {
      try { cachedTab = localStorage.getItem(`workcell:project-tab:${project.id}`); } catch {}
    }
    ...
    if (cachedTab === "design") {
      return <Navigate to={`/projects/${canonicalProjectRef}/design`} replace />;
    }
  ...
  const handleTabChange = (tab: ProjectTab) => {
    ...
    if (project?.id) {
      try { localStorage.setItem(`workcell:project-tab:${project.id}`, tab); } catch {}
    }
    ...
    if (tab === "overview") {
      ...
      navigate(`/projects/${canonicalProjectRef}/design`);
    ...
    <div className="space-y-6">
      {showLeftProjectNotice ? (
        <div className="flex items-center gap-3 border border-yellow-300/35 bg-yellow-300/10 px-3 py-2 text-sm text-yellow-100">
          <p className="min-w-0 flex-1">
            {t("projectDetail.leftNotice.message", { defaultValue: "You left this project. It no longer appears in your sidebar." })}
          </p>
          <MembershipAction
            compact
            state="left"
          ...
          <button
            ...
            aria-label={t("projectDetail.leftNotice.dismiss", { defaultValue: "Dismiss project membership notice" })}
        ...
        <div className="min-w-0 space-y-2">
          ...
          {project.managedByPlugin ? (
            <div className="inline-flex items-center gap-2 rounded-full border border-border bg-muted px-3 py-1 text-[11px] font-medium text-muted-foreground">
              ...
              {t("projectDetail.managedByPlugin", { defaultValue: "Managed by {{name}}", name: project.managedByPlugin.pluginDisplayName })}
            </div>
          ) : null}
        </div>
      </div>
      ...
      <Tabs value={activeTab ?? "list"} onValueChange={(value) => handleTabChange(value as ProjectTab)}>
        <PageTabBar
          items={[
            ...
            { value: "design", label: t("projectDetail.tabs.design", { defaultValue: "App Blueprint" }) },
      ...
      {activeTab === "workspaces" ? (
        workspaceTabDecisionLoaded ? (
          ...
          <p className="text-sm text-muted-foreground">{t("projectDetail.workspaces.loading", { defaultValue: "Loading workspaces..." })}</p>
        )
      ) : null}
      ...
      {activeTab === "design" && project?.id && (
        <DesignSystem scope={{ kind: "project", projectId: project.id }} />
      )}
