...
const PENDING_CONNECTION_KEY = "workcell-cloud-upstream-pending-connection";
const STEPS: Array<{ key: CloudUpstreamStep; defaultLabel: string }> = [
  { key: "connect", defaultLabel: "Connect" },
  { key: "scan", defaultLabel: "Scan" },
  { key: "preview", defaultLabel: "Preview" },
  { key: "push", defaultLabel: "Push" },
  { key: "verify", defaultLabel: "Verify" },
  { key: "activate", defaultLabel: "Activate" },
];
const ACTIVATION_CATEGORIES: Array<{
...
}> = [
  {
    key: "agents",
    defaultLabel: "Agents",
    defaultSingular: "agent",
    defaultDetail: "Confirm cloud secrets and adapter credentials before unpausing imported agents.",
  },
  {
    key: "routines",
    defaultLabel: "Routines",
    defaultSingular: "routine",
    defaultDetail: "Review schedules and trigger settings before enabling imported routines.",
  },
  {
    key: "monitors",
    defaultLabel: "Monitors",
    defaultSingular: "monitor",
    defaultDetail: "Activate after the target stack has been smoke tested.",
  },
];
...
export function CloudUpstream() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: selectedCompany?.name ?? t("cloudUpstream.breadcrumbCompanyFallback", { defaultValue: "Team" }), href: "/dashboard" },
      { label: t("cloudUpstream.breadcrumbSettings", { defaultValue: "Settings" }), href: "/company/settings" },
      { label: t("cloudUpstream.breadcrumbCloudUpstream", { defaultValue: "Cloud upstream" }) },
    ]);
  ...
  const finishMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      setNotice(t("cloudUpstream.noticeConnectionApproved", { defaultValue: "Cloud upstream connection approved." }));
      setActionError(null);
      await invalidateUpstreams();
      window.history.replaceState(null, "", settingsPath);
    },
    onError: (error) => setActionError(error instanceof Error ? error.message : t("cloudUpstream.errorFinishConnection", { defaultValue: "Failed to finish connection." })),
  });
  const {
  ...
  useEffect(() => {
    ...
    if (!pendingConnectionId) {
      setActionError(t("cloudUpstream.errorNoPendingConnection", { defaultValue: "No pending cloud upstream connection was found. Start the connection again." }));
      return;
    }
  ...
  useEffect(() => {
    if (callbackError) {
      setActionError(t("cloudUpstream.errorConnectionNotApproved", { defaultValue: "Cloud upstream connection was not approved: {{error}}", error: callbackError }));
    }
  }, [callbackError, t]);
  ...
  const startMutation = useMutation({
    ...
    onError: (error) => setActionError(error instanceof Error ? error.message : t("cloudUpstream.errorStartConnection", { defaultValue: "Failed to start connection." })),
  });
  ...
  const runMutation = useMutation({
    ...
    onSuccess: async (run) => {
      ...
      setNotice(run.status === "succeeded"
        ? t("cloudUpstream.noticePushCompleted", { defaultValue: "Push run completed. Review activation before unpausing automations." })
        : t("cloudUpstream.noticePushFailed", { defaultValue: "Push run failed. Review the run events and retry after correcting the issue." }));
      setActionError(null);
      await invalidateUpstreams();
    },
    onError: (error) => setActionError(error instanceof Error ? error.message : t("cloudUpstream.errorRunPush", { defaultValue: "Failed to run push." })),
  });
  const activationMutation = useMutation({
    ...
    onSuccess: async (run) => {
      setActiveRun(run);
      setNotice(t("cloudUpstream.noticeActivationUpdated", { defaultValue: "Activation checklist updated." }));
      setActionError(null);
      await invalidateUpstreams();
    },
    onError: (error) => setActionError(error instanceof Error ? error.message : t("cloudUpstream.errorActivateEntities", { defaultValue: "Failed to activate imported entities." })),
  });
  ...
  if (!selectedCompanyId || !selectedCompany) {
    return <div className="text-sm text-muted-foreground">{t("cloudUpstream.selectCompany", { defaultValue: "Select a team to configure cloud upstream." })}</div>;
  }
    ...
    return (
      <div className="max-w-2xl space-y-4">
        ...
        <div className="rounded-md border border-border px-4 py-4 text-sm text-muted-foreground">
          {t("cloudUpstream.disabledIntro", { defaultValue: "Cloud sync is disabled. Enable it in" })}{" "}
          <Link className="text-primary underline-offset-2 hover:underline" to="/instance/settings/experimental">
            {t("cloudUpstream.instanceSettingsLink", { defaultValue: "Instance Settings" })}
          </Link>{" "}
          {t("cloudUpstream.disabledOutro", { defaultValue: "to show upstream connection and push tools." })}
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-6xl space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="space-y-1">
          ...
          <p className="max-w-2xl text-sm text-muted-foreground">
            {t("cloudUpstream.intro", { defaultValue: "Push {{companyName}} into a Workcell Cloud stack. Automations stay paused until activation.", companyName: selectedCompany.name })}
          </p>
        </div>
        {connection?.target.origin ? (
          <Button variant="outline" size="sm" asChild>
            <a href={connection.target.origin} target="_blank" rel="noreferrer">
              ...
              {t("cloudUpstream.openCloud", { defaultValue: "Open cloud" })}
            </a>
          </Button>
        ) : null}
      </div>
      ...
      <section className="space-y-3">
        <div className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("cloudUpstream.connectionLabel", { defaultValue: "Connection" })}</div>
          ...
          {connection ? (
            <div className="grid gap-3 lg:grid-cols-[1fr_auto] lg:items-start">
              ...
              <div className="flex flex-col items-end gap-1">
                ...
                >
                  ...
                  {t("cloudUpstream.previewPush", { defaultValue: "Preview push" })}
                </Button>
            ...
            <div className="grid gap-3 md:grid-cols-[1fr_auto]">
              <Input
                ...
                placeholder="https://workcell.workcell.app/PC521D/dashboard"
                aria-label={t("cloudUpstream.stackUrlAriaLabel", { defaultValue: "Workcell Cloud stack URL" })}
              />
              <Button onClick={() => startMutation.mutate()} disabled={startMutation.isPending || !remoteUrl.trim()}>
                ...
                {t("cloudUpstream.connect", { defaultValue: "Connect" })}
              </Button>
            </div>
          )}
        </div>
      ...
      {preview ? (
        <section className="space-y-3">
          <div className="flex items-center justify-between gap-3">
            <div className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("cloudUpstream.previewLabel", { defaultValue: "Preview" })}</div>
            <Button
            ...
            >
              ...
              {t("cloudUpstream.pushToCloud", { defaultValue: "Push to cloud" })}
            </Button>
          </div>
      ...
      {latestRun ? (
        <section className="space-y-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("cloudUpstream.progressLabel", { defaultValue: "Progress and finish" })}</div>
            <div className="flex flex-wrap gap-2">
              <Button variant="outline" size="sm" onClick={() => downloadRunReport(latestRun)}>
                <FileJson className="h-4 w-4" />
                {t("cloudUpstream.downloadReport", { defaultValue: "Download report" })}
              </Button>
              {latestRun.status === "failed" || latestRun.status === "cancelled" ? (
                ...
                >
                  ...
                  {t("cloudUpstream.retry", { defaultValue: "Retry" })}
                </Button>
                ...
                >
                  <RefreshCcw className="h-4 w-4" />
                  {t("cloudUpstream.reRun", { defaultValue: "Re-run" })}
                </Button>
              ) : null}
            </div>
          </div>
          <div className="rounded-md border border-border px-4 py-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <div className="mt-1 text-xs text-muted-foreground">
                  Run {latestRun.id.slice(0, 8)} · {latestRun.completedAt ? `completed ${formatDate(latestRun.completedAt)}` : t("cloudUpstream.inProgress", { defaultValue: "in progress" })}
                </div>
              </div>
      ...
      {upstreamQuery.data?.runs.length ? (
        <section className="space-y-3">
          <div className="flex items-center gap-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">
            ...
            {t("cloudUpstream.historyLabel", { defaultValue: "History" })}
          </div>
          <div className="divide-y divide-border rounded-md border border-border">
            {upstreamQuery.data.runs.map((run) => (
              ...
              >
                <span>{t("cloudUpstream.runLabel", { defaultValue: "Run {{id}} · {{status}}", id: run.id.slice(0, 8), status: run.status })}</span>
        ...
        return (
          <div key={step.key} className="flex items-center gap-2 text-xs">
            ...
            <span className={active ? "font-medium text-foreground" : "text-muted-foreground"}>
              {t(`cloudUpstream.step.${step.key}`, { defaultValue: step.defaultLabel })}
            </span>
          </div>
        );
      })}
  ...
  return (
    <div className="rounded-md border border-border px-4 py-3">
      <div className="mb-2 flex items-center gap-2 text-sm font-medium">
        ...
        {t("cloudUpstream.warningsHeading", { defaultValue: "Warnings" })}
      </div>
...
function ConflictTable({ conflicts }: { conflicts: CloudUpstreamPreview["conflicts"] }) {
  ...
  return (
    <div className="rounded-md border border-border px-4 py-3">
      <div className="mb-2 text-sm font-medium">{t("cloudUpstream.conflictsHeading", { defaultValue: "Conflicts" })}</div>
      {conflicts.length === 0 ? (
        <div className="text-sm text-muted-foreground">{t("cloudUpstream.noConflicts", { defaultValue: "No target conflicts detected for this preview." })}</div>
      ) : (
        <div className="divide-y divide-border">
...
function ActivationChecklist({
  ...
  return (
    <div className="rounded-md border border-border px-4 py-3">
      <div className="mb-2 text-sm font-medium">{t("cloudUpstream.activationChecklistHeading", { defaultValue: "Activation checklist" })}</div>
      <div className="divide-y divide-border">
        {rows.map((row) => {
            ...
            <div key={row.key} className="grid gap-2 py-2 text-sm sm:grid-cols-[8rem_1fr_auto] sm:items-center">
              ...
              <div className="flex flex-wrap gap-2 sm:justify-end">
                ...
                >
                  ...
                  {activated ? t("cloudUpstream.activated", { defaultValue: "Activated" }) : t("cloudUpstream.activate", { defaultValue: "Activate" })}
                </Button>
                <Button variant="ghost" size="sm" disabled={activated || isPending}>
                  {t("cloudUpstream.keepPaused", { defaultValue: "Keep paused" })}
                </Button>
              </div>
            </div>
          );
...
export function buildActivationRows(run: CloudUpstreamRun, t?: TFunction) {
  ...
  return ACTIVATION_CATEGORIES.map((category) => {
    ...
    const englishPlural = `${category.defaultSingular}${count === 1 ? "" : "s"}`;
    const label = t
      ? t(`cloudUpstream.activationCategory.${category.key}`, { defaultValue: category.defaultLabel })
      : category.defaultLabel;
    const noun = t
      ? t(`cloudUpstream.activationNoun.${category.key}`, {
          count,
          defaultValue: englishPlural,
        })
      : englishPlural;
    const categoryDetail = t
      ? t(`cloudUpstream.activationCategoryDetail.${category.key}`, { defaultValue: category.defaultDetail })
      : category.defaultDetail;
    const detail = t
      ? t("cloudUpstream.activationDetail", {
          count,
          noun,
          suffix: categoryDetail,
          defaultValue: "{{count}} imported {{noun}} are paused by default. {{suffix}}",
        })
      : `${count} imported ${noun} are paused by default. ${categoryDetail}`;
    const emptyDetail = t
      ? t("cloudUpstream.activationEmptyDetail", {
          noun,
          defaultValue: "0 imported {{noun}} in this run.",
        })
      : `0 imported ${noun} in this run.`;
    const statusLabel =
      status === "activated"
        ? t
          ? t("cloudUpstream.activationStatusActivated", { count, defaultValue: "{{count}} activated" })
          : `${count} activated`
        : count === 0
          ? t
            ? t("cloudUpstream.activationStatusImportedZero", { defaultValue: "0 imported" })
            : "0 imported"
          : t
            ? t("cloudUpstream.activationStatusPaused", { count, defaultValue: "{{count}} paused" })
            : `${count} paused`;
    return {
      ...category,
      count,
