  ...
  if (!resolvedCompanyId) {
    ...
    return (
      <div className="space-y-4">
        <p className="text-sm text-muted-foreground">{t("pluginPage.selectCompany", { defaultValue: "Select a company to view this page." })}</p>
      </div>
    );
  }

  if (!contributions) {
    return <div className="text-sm text-muted-foreground">{t("pluginPage.loadingEllipsis", { defaultValue: "Loading…" })}</div>;
  }
    ...
    if (duplicateMatches.length > 1) {
      return (
        <div className="rounded-md border border-destructive/30 bg-destructive/5 px-3 py-2 text-sm text-destructive">
          {t("pluginPage.conflictRouteBefore", { defaultValue: "Multiple plugins declare the route" })} <code>{pluginRoutePath}</code>{t("pluginPage.conflictRouteAfter", { defaultValue: ". Use the plugin-id route until the conflict is resolved." })}
        </div>
      );
    }
  }
