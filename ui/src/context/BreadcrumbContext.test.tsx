...
describe("BreadcrumbContext", () => {
  ...
  it("builds page titles with the selected company name before Workcell", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "Anachronist Wiki")).toBe(
      "Inbox • Anachronist Wiki • Workcell",
    );
    expect(
      buildDocumentTitle(
    ...
    ).toBe("PAP-3515 • Issues • Anachronist Wiki • Workcell");
  });

  it("omits blank company names from page titles", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "  ")).toBe("Inbox • Workcell");
    expect(buildDocumentTitle([], null)).toBe("Workcell");
  });
});
