...
import type { Agent, Issue, IssueComment, LiveEvent } from "@workcell/shared";
...
function buildActivityToast(
  ...
  if (action === "issue.created") {
    return {
      ...
      tone: "success",
      action: { label: t("liveUpdates.action.viewRef", { defaultValue: "View {{ref}}", ref: issue.ref }), href: issue.href },
...
function buildJoinRequestToast(
  ...
  const title =
    requestType === "agent"
      ? t("liveUpdates.joinRequest.titleAgent", { defaultValue: "Agent wants to join" })
      : t("liveUpdates.joinRequest.titleSomeone", { defaultValue: "Someone wants to join" });

  return {
    title,
    body: t("liveUpdates.joinRequest.body", { defaultValue: "A new join request is waiting for approval." }),
    tone: "info",
    action: { label: t("liveUpdates.action.viewInbox", { defaultValue: "View inbox" }), href: "/inbox/mine" },
...
function buildAgentStatusToast(
  ...
  return {
    ...
    body,
    tone,
    action: { label: t("liveUpdates.action.viewAgent", { defaultValue: "View agent" }), href: `/agents/${agentId}` },
...
function buildRunStatusToast(
  ...
  return {
    ...
    action: { label: t("liveUpdates.action.viewRun", { defaultValue: "View run" }), href: `/agents/${agentId}/runs/${runId}` },
