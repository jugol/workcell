 ...
 *
 * Plugin bundles are built with `external: ["@workcell/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__workcellPluginBridge__`)
...
function createReactShimSource(reactModule: object): string {
  ...
  return `
        const R = globalThis.__workcellPluginBridge__?.react;
        if (!R) {
          throw new Error("Workcell plugin React runtime is not initialized.");
        }
        export default R;
${namedExports}
      `;
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__workcellPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__workcellPluginBridge__?.sdkUi ?? {};
        function missing(name) {
          return function MissingWorkcellSdkUiComponent() {
            throw new Error('Workcell plugin UI runtime is not initialized for "' + name + '". Ensure the host loaded the plugin bridge before rendering this UI module.');
          };
        }
 ...
 * - `import { ... } from "@workcell/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@workcell/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@workcell/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@workcell/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@workcell/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__workcellPluginBridge__) {
 ...
 * self-register via `window.workcellPlugins.registerReactComponent()`.
 ...
 * of `@workcell/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
