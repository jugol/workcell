...
describe("locale validation", () => {
  it("resolves English messages with key and default fallbacks", async () => {
    // i18n now initializes from the ambient browser/stored locale (WC-82), so
    // pin English explicitly — this test is about key + defaultValue resolution.
  ...
  it("allows missing keys (partial locales fall back to English) but rejects extra keys", () => {
    const errors = validateLocaleMessages({
      app: {
        noCompanies: {
          title: en.app.noCompanies.title,
          description: en.app.noCompanies.description,
          // `newCompany` intentionally omitted — partial locale, allowed.
    ...
    expect(errors).toEqual(
      expect.arrayContaining(["app.noCompanies.unexpected is not defined in English"]),
    );
  });
