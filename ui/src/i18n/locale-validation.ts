...
function validateNode(path: string[], candidate: unknown, englishReference: unknown, errors: string[]) {
  ...
  // WC-82: locales may be PARTIAL. A missing key falls back to English at
  // runtime (i18next fallbackLng), so missing keys are allowed — this lets the
  // translation surface grow incrementally without touching all 40 locales for
  // every new string. Extra keys (typos / stale) are still rejected, and any
  // key that IS present is fully validated below.
