...
function extractUiParserSource(
  ...
  const contractVersion = pkg.workcell?.adapterUiParser;
  if (contractVersion) {
    ...
    logger.info(
      { packageName },
      "Adapter has ./ui-parser export but no workcell.adapterUiParser version — loading anyway (future versions may require it)",
    );
  }
