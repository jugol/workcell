...
} from "@workcell/adapter-claude-local";
import {
...
const externalAdaptersReady: Promise<void> = (async () => {
  try {
    ...
    for (const externalAdapter of externalAdapters) {
      ...
      if (overriding) {
        console.log(
          `[workcell] External adapter "${externalAdapter.type}" overrides built-in adapter`,
        );
    ...
    console.error("[workcell] Failed to load external adapters:", err);
  }
})();
...
export function setOverridePaused(type: string, paused: boolean): boolean {
  ...
  if (paused && !wasPaused) {
    ...
    console.log(`[workcell] Override paused for "${type}" — builtin adapter restored`);
    return true;
  }
  if (!paused && wasPaused) {
    pausedOverrides.delete(type);
    console.log(`[workcell] Override resumed for "${type}" — external adapter active`);
    return true;
  }
  return false;
}
