/**
 * Server-side SVG renderer for Workcell org charts.
...
const ROLE_ICONS: Record<string, {
  ...
  orchestrator: {
    bg: "#fef3c7", roleLabel: "Orchestrator", accentColor: "#f0883e", iconColor: "#92400e",
  ...
  lead: {
    bg: "#dbeafe", roleLabel: "Lead", accentColor: "#58a6ff", iconColor: "#1e40af",
...
function guessRoleTag(node: OrgNode): string {
  ...
  if (role === "orchestrator" || name === "orchestrator") return "orchestrator";
  if (role === "lead" || name === "lead") return "lead";
...
const THEMES: Record<OrgChartStyle, StyleTheme> = {
  ...
  circuit: {
    ...
    renderCard: (ln: LayoutNode, theme: StyleTheme) => {
      ...
      const isOrchestrator = tag === "orchestrator";
      const borderColor = isOrchestrator ? "rgba(168,85,247,0.35)" : theme.cardBorder;
      const bgColor = isOrchestrator ? "rgba(168,85,247,0.06)" : theme.cardBg;
  ...
  schematic: {
    ...
    renderCard: (ln: LayoutNode, theme: StyleTheme) => {
      ...
      const schemaRoles: Record<string, string> = {
        orchestrator: "orchestrator", lead: "lead", engineer: "engineer",
        quality: "quality_assurance", design: "designer", default: "agent",
      };
...
// Workcell logo: scaled icon (~16px) + wordmark (13px), vertically centered
const WORKCELL_LOGO_SVG = `<g>
  ...
  <text x="22" y="11.5" font-family="system-ui, -apple-system, sans-serif" font-size="13" font-weight="600" fill="currentColor">Workcell</text>
</g>`;
