...
import { heartbeatRuns, instanceUserRoles, invites } from "@workcell/db";
...
function hasDevServerStatusToken(providedToken: string | undefined) {
  const expectedToken = process.env.WORKCELL_DEV_SERVER_STATUS_TOKEN?.trim();
...
export function healthRoutes(
  ...
  router.get("/", async (req, res) => {
    ...
    const exposeDevServerDetails =
      exposeFullDetails || hasDevServerStatusToken(req.get("x-workcell-dev-server-status-token"));
