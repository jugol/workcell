...
export function userProfileRoutes(db: Db) {
  ...
  router.get("/companies/:companyId/users/:userSlug/profile", async (req, res) => {
    ...
    const payload: UserProfileResponse = {
      ...
      recentActivity,
      // WC-137: agent_id is nullable at rest (preserved-but-detached billing of a
      // deleted agent). A per-user "top agents" breakdown has no agent to show for
      // orphaned costs, so drop those rows (and narrow agentId to non-null). The
      // company-level cost breakdown keeps them as an unattributed group.
      topAgents: topAgents
