 ...
 * toggle server-side adapter code for the whole Workcell instance.
 *
...
interface AdapterInstallRequest {
  /** npm package name (e.g., "droid-workcell-adapter") or local path */
  packageName: string;
...
export function adapterRoutes() {
  ...
  router.post("/adapters/install", async (req, res) => {
    ...
    // e.g. "@henkey/hermes-workcell-adapter@0.3.0" → packageName + version
  ...
  // without modifying Workcell's source code.
  //
