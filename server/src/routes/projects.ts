...
import {
  ...
  relocateProjectWorkspaceSchema,
  updateProjectSchema,
  updateProjectWorkspaceSchema,
  workspaceRuntimeControlTargetSchema,
} from "@workcell/shared";
import type { WorkspaceRuntimeDesiredState, WorkspaceRuntimeServiceStateMap } from "@workcell/shared";
import { trackProjectCreated } from "@workcell/shared/telemetry";
...
export function projectRoutes(db: Db) {
  ...
  // Move a local workspace's source folder to a new path + update its cwd in one
  // step (the intuitive "relocate" the board wants, instead of move-files-then-
  // edit-cwd). Host-filesystem mutation → agents are blocked; the service guards
  // against active runs and validates/moves safely.
  router.post(
    "/projects/:id/workspaces/:workspaceId/relocate",
    validate(relocateProjectWorkspaceSchema),
    async (req, res) => {
      const id = req.params.id as string;
      const workspaceId = req.params.workspaceId as string;
      const existing = await svc.getById(id);
      ...
      assertCompanyAccess(req, existing.companyId);
      assertNoAgentHostWorkspaceCommandMutation(req, [String(req.body.targetPath ?? "")]);

      const result = await svc.relocateWorkspace(id, workspaceId, {
      ...
      await logActivity(db, {
        ...
        actorId: actor.actorId,
        agentId: actor.agentId,
        action: "project.workspace_relocated",
        entityType: "project",
        entityId: id,
        details: { workspaceId: result.workspace.id, to: result.workspace.cwd },
      });
  ...
  async function handleProjectWorkspaceRuntimeCommand(req: Request, res: Response) {
    ...
    if (!workspaceCwd) {
      res.status(422).json({ error: "Project workspace needs a local path before Workcell can run workspace commands" });
      return;
    }
