...
import { agents as agentsTable, companies, heartbeatRuns, issues as issuesTable } from "@workcell/db";
import { and, desc, eq, inArray, not, sql } from "drizzle-orm";
import {
  agentSkillSyncSchema,
  agentMineInboxQuerySchema,
  AGENT_DEFAULT_MAX_CONCURRENT_RUNS,
  deliberateAgentSchema,
...
import { trackAgentCreated } from "@workcell/shared/telemetry";
...
import type { AdapterExecutionTarget } from "@workcell/adapter-utils/execution-target";
import type {
...
import { runClaudeLogin } from "@workcell/adapter-claude-local/server";
...
export function agentRoutes(
  ...
  options: {
    ...
    // WC-206 test seam: inject a deterministic stub DeliberationInvoke so the
    // /agents/:id/deliberate route can be driven end-to-end without a real LLM.
    // When provided it bypasses both the live invoke factory AND the
    // WORKCELL_PAIR_LIVE_LLM gate, so route tests stay hermetic. Production
    // never sets this — the route builds the live invoke under the flag instead.
    deliberationInvokeOverride?: DeliberationInvoke;
    // WC-215: injectable per-tenant limiter for the expensive LLM deliberation
    // start route. Tests pass a tiny-limit fake; production uses the shared default.
  ...
  // WC-209: persisted async deliberation runs (run + turn rows, pollable).
  const deliberationRuns = agentDeliberationRunService(db);
  // WC-215: per-tenant limiter guarding the live deliberation start route.
  const llmRateLimiter = options.llmRateLimiter ?? defaultLlmRouteRateLimiter;
   ...
   *   adapter probes the Workcell host (legacy behavior).
  ...
  async function buildAgentAccessState(agent: NonNullable<Awaited<ReturnType<typeof svc.getById>>>) {
    ...
    if (agent.role === "orchestrator") {
      return {
        canAssignTasks: true,
        taskAssignSource: "orchestrator_role" as const,
        membership,
        grants,
      };
    }
  ...
  function applyCreateDefaultsByAdapterType(
    ...
    if (adapterType === "codex_local") {
      // Deliberately NO default model stamping: an empty model means the codex
      // CLI's own default applies (codex-args.ts only passes --model when set).
      // Stamping a hardcoded id here silently pinned every "Default" agent to
      // a stale model long after the CLI default moved on.
      const hasBypassFlag =
  ...
  async function resolveDesiredSkillAssignment(
    ...
    return {
      adapterConfig: writeWorkcellSkillSyncPreference(adapterConfig, desiredSkills),
      desiredSkills,
      runtimeSkillEntries,
    };
  }
  ...
  function toLeanOrgNode(node: Record<string, unknown>): Record<string, unknown> {
    ...
    // Merged pair node: pass through the counterpart summary (lean fields
    // only) so the UI can render the "Owner ⇄ Counterpart" card.
    if (node.pair && typeof node.pair === "object") {
      const pair = node.pair as Record<string, unknown>;
      lean.pair = {
        id: String(pair.id),
  ...
  router.get("/agents/:id/skills", async (req, res) => {
    ...
    if (!adapter?.listSkills) {
      const preference = readWorkcellSkillSyncPreference(
  ...
  // WC-209 (deliberation async + persist): start a LIVE dual-brain deliberation
  // run ASYNCHRONOUSLY. Replaces the WC-206 synchronous contract.
  //
  // WHY ASYNC: a real run is brain A propose → A/B alternate review+revise until
  // one ACCEPTs (or maxRounds), each turn a real ~75s model call — so a run
  // blocked the HTTP request ~5 min and the transcript lived ONLY in the
  // response (lost on a client/proxy timeout, after spending real money). Now we
  // create a run row (status "running"), kick the engine off FIRE-AND-FORGET,
  // and return 202 { runId, status:"running" } immediately. The turns + final
  // result persist to agent_deliberation_runs / _turns; clients poll the GET
  // routes below. (NOTE v1: a server restart mid-run leaves the row stuck
  // "running" — the loop dies with the process. Acceptable for v1; a boot-time
  // reaper that fails orphaned "running" rows is a follow-up.)
  //
  // SAFE / STANDALONE: does NOT touch the heartbeat or default agent run path.
  // Spend is gated two ways: the agent must have deliberation.enabled, AND (in
  // production) WORKCELL_PAIR_LIVE_LLM must be set — else 503 so dev/CI never
  // spends by accident. Tests inject a deterministic stub via
  // options.deliberationInvokeOverride, which bypasses the live factory + flag
  // entirely; with a synchronous stub the run completes near-instantly so a test
  // can poll the GET route after a tick.
  router.post("/agents/:id/deliberate", validate(deliberateAgentSchema), async (req, res) => {
    const id = req.params.id as string;
    const agent = await svc.getById(id);
    if (!agent) {
      res.status(404).json({ error: "Agent not found" });
    ...
    // Board+company authz (cross-company agent → 403 here; a missing agent is
    // the 404 above, so a cross-company caller cannot distinguish existence).
    assertCompanyAccess(req, agent.companyId);
    // WC-215: cap expensive live dual-brain deliberation starts per tenant.
    if (!enforceLlmRouteRateLimit(req, res, llmRateLimiter, agent.companyId)) return;

    const deliberation = agent.deliberation ?? null;
    if (!deliberation?.enabled) {
      res.status(400).json({ error: "deliberation not enabled for this agent" });
      return;
    }

    const { task, maxRoundsOverride } = req.body as {
      task: string;
      maxRoundsOverride?: number;
    };

    // Resolve the invoke FACTORY: a test-injected stub takes precedence and
    // skips the live gate; otherwise we require the live flag before spending on
    // real models. The flag is the SAME one the pair-turn live path reuses; "0"
    // (or unset) keeps us off the live path. The factory receives the service's
    // per-turn `onCost` so each turn row is billed (the live invoke reports
    // cents through it; the stub ignores it → 0-cost turns).
    const override = options.deliberationInvokeOverride;
    let buildInvoke: (onCost: (costCents: number) => void) => DeliberationInvoke;
    if (override) {
      buildInvoke = () => override;
    } else {
      const liveEnv = process.env.WORKCELL_PAIR_LIVE_LLM;
      const liveLlmEnabled = Boolean(liveEnv) && liveEnv !== "0";
      if (!liveLlmEnabled) {
        res.status(503).json({ code: "deliberation_live_disabled" });
        return;
      }
      buildInvoke = (onCost) =>
        buildLiveDeliberationInvoke({
          ...
          runIdBase: `deliberate-${agent.id}-${randomUUID()}`,
          onCost,
        });
    }
    ...
    const startInput = {
      ...
      task,
      maxRoundsOverride,
      // WC-208: each brain carries its own adapter + model override (null =
      // inherit the agent's adapterType / configured model).
      brainA: {
        adapter: deliberation.brainA?.adapter ?? null,
        model: deliberation.brainA?.model ?? null,
      },
      brainB: {
        adapter: deliberation.brainB?.adapter ?? null,
        model: deliberation.brainB?.model ?? null,
      },
      maxRounds: deliberation.maxRounds,
      buildInvoke,
    };

    // Create the run row first so the 202 carries a runId the client can poll
    // immediately — even before the first turn lands.
    const runId = await deliberationRuns.create(startInput);

    // Activity event: the run was STARTED (terminal stats live on the run row,
    // which the GET routes expose).
    const actor = getActorInfo(req);
    await logActivity(db, {
      companyId: agent.companyId,
      actorType: actor.actorType,
      ...
      agentId: actor.agentId,
      runId: actor.runId,
      action: "agent.deliberation_run",
      entityType: "agent",
      entityId: agent.id,
      details: { runId, status: "running" },
    });

    // Kick the engine off FIRE-AND-FORGET — do NOT await the loop. runLoop never
    // throws (it records failures as status "failed" on the row); the .catch is
    // a belt-and-suspenders guard so an unexpected rejection can't crash the
    // process as an unhandled promise.
    void deliberationRuns.run(runId, startInput).catch((err) => {
      // eslint-disable-next-line no-console
      console.error(
        `[deliberation] run ${runId} loop rejected: ${err instanceof Error ? err.message : String(err)}`,
      );
    });
  ...
  // WC-209: list recent deliberation runs for an agent (company-scoped).
  router.get("/agents/:id/deliberations", async (req, res) => {
    const id = req.params.id as string;
    const agent = await svc.getById(id);
    if (!agent) {
      res.status(404).json({ error: "Agent not found" });
    ...
    assertCompanyAccess(req, agent.companyId);
    const limit = readLiveRunsQueryInt(req.query.limit, 100, 20);
    const runs = await deliberationRuns.listRuns(agent.companyId, agent.id, limit);
    res.json({ runs });
  });

  // WC-209: fetch one deliberation run + its ordered turns (company-scoped;
  // 404 when the run is missing OR belongs to another company so a cross-tenant
  // caller cannot probe existence).
  router.get("/agents/:id/deliberations/:runId", async (req, res) => {
    const id = req.params.id as string;
    ...
    assertCompanyAccess(req, agent.companyId);
    const found = await deliberationRuns.getRun(agent.companyId, runId);
    if (!found || found.run.agentId !== agent.id) {
      res.status(404).json({ error: "Deliberation run not found" });
      return;
    }
    res.json({ run: found.run, turns: found.turns });
  });
  ...
  router.post("/companies/:companyId/agent-hires", validate(createAgentHireSchema), async (req, res) => {
    ...
    // Autonomous (unattended) mode bypasses the board-approval hire gate so the
    // orchestrator can assemble its team without a human in the loop.
    const autonomousMode = (await instanceSettings.getExperimental()).autonomousMode;
    const requiresApproval = company.requireBoardApprovalForNewAgents && !autonomousMode;
  ...
  router.post("/companies/:companyId/agents", validate(createAgentSchema), async (req, res) => {
    ...
    if (company.requireBoardApprovalForNewAgents && !(await instanceSettings.getExperimental()).autonomousMode) {
      throw conflict(
  ...
  router.patch("/agents/:id/permissions", validate(updateAgentPermissionsSchema), async (req, res) => {
    ...
    if (req.actor.type === "agent") {
      ...
      if (actorAgent.role !== "orchestrator") {
        res.status(403).json({ error: "Only the Orchestrator can manage permissions" });
        return;
      }
    } else {
    ...
    const effectiveCanAssignTasks =
      agent.role === "orchestrator" || Boolean(agent.permissions?.canCreateAgents) || req.body.canAssignTasks;
