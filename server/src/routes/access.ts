...
import type { DeploymentExposure, DeploymentMode, HumanCompanyMembershipRole, PermissionKey } from "@workcell/shared";
import {
  forbidden,
  conflict,
  notFound,
...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (
    normalized !== "workcell" &&
    normalized !== "workcell-create-agent" &&
    normalized !== "workcell-create-plugin" &&
    normalized !== "workcell-converting-plans-to-tasks" &&
...
/** Resolve the Workcell repo skills directory (built-in / managed skills). */
function resolveWorkcellSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const workcellSkillsDir = resolveWorkcellSkillsDir();

  // Build set of Workcell-managed skill names
  const workcellSkillNames = new Set<string>();
  if (workcellSkillsDir) {
    try {
      for (const entry of fs.readdirSync(workcellSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) workcellSkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isWorkcellManaged: workcellSkillNames.has(entry.name),
      });
    }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.workcellApiUrl)) {
    const legacyWorkcellApiUrl = nonEmptyTrimmedString(input.workcellApiUrl);
    if (legacyWorkcellApiUrl) merged.workcellApiUrl = legacyWorkcellApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    workcellApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.workcellApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawWorkcellApiUrl =
    typeof defaults.workcellApiUrl === "string"
      ? defaults.workcellApiUrl.trim()
      : "";
  if (rawWorkcellApiUrl) {
    try {
      const parsedWorkcellApiUrl = new URL(rawWorkcellApiUrl);
      if (
        parsedWorkcellApiUrl.protocol !== "http:" &&
        parsedWorkcellApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_workcell_api_url_protocol",
          level: "warn",
          message: `workcellApiUrl must use http:// or https:// (got ${parsedWorkcellApiUrl.protocol}).`
        });
      } else {
        normalized.workcellApiUrl = parsedWorkcellApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_workcell_api_url_configured",
          level: "info",
          message: `workcellApiUrl set to ${parsedWorkcellApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_workcell_api_url_invalid",
        level: "warn",
        message: `Invalid workcellApiUrl: ${rawWorkcellApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your Workcell host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "Workcell is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm workcell allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = `/api/invites/${token}/skills/workcell`;
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an external Workcell agent, save your one-time claim secret, wait for board approval, then claim your API key. Use requestType='agent', include your agentName and capabilities, and set adapterType plus agentDefaultsPayload for your runtime when applicable. OpenClaw Gateway agents must use adapterType='openclaw_gateway', set agentDefaultsPayload.url to a ws:// or wss:// gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token.",
      inviteMessage: extractInviteMessage(invite),
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Workcell hostname is reachable and allowed via `pnpm workcell allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this Workcell API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "runtime-specific Workcell skill location"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    ...
    Decide which Workcell adapter type matches your runtime.

    Use adapterType only when there is a matching Workcell adapter. Put runtime-specific settings in agentDefaultsPayload.
    ...
    OpenClaw Gateway payload example:
    {
      ...
      "agentDefaultsPayload": {
        ...
        "workcellApiUrl": "https://workcell-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in Workcell before key claim is allowed.
    ...
    On successful claim, save the full JSON response somewhere private for your runtime and set WORKCELL_API_KEY and WORKCELL_API_URL for future Workcell API calls.
    ...
    ${
      ...
      "Ensure Workcell is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested Workcell base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.workcellApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm workcell allowed-hostname <host>
      - then restart Workcell and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@workcell.local";
...
export function resolveJoinRequestAgentManagerId(
  ...
  const ceoCandidates = candidates.filter(
    (candidate) => candidate.role === "orchestrator"
  );
...
export function accessRoutes(
  ...
  async function assertCanGenerateOpenClawInvitePrompt(
    ...
    if (req.actor.type === "agent") {
      ...
      if (actorAgent.role !== "orchestrator") {
        throw forbidden("Only the Orchestrator can generate OpenClaw invite prompts");
      }
      return;
    }
    if (req.actor.type !== "board") throw unauthorized();
  ...
  router.get("/skills/index", (req, res) => {
    ...
    res.json({
      skills: [
        { name: "workcell", path: "/api/skills/workcell" },
        {
          name: "para-memory-files",
          path: "/api/skills/para-memory-files"
        },
        {
          name: "workcell-create-agent",
          path: "/api/skills/workcell-create-agent"
        },
        {
          name: "workcell-converting-plans-to-tasks",
          path: "/api/skills/workcell-converting-plans-to-tasks"
        }
      ]
    });
  });
  ...
  router.get("/invites/:token/skills/index", async (req, res) => {
    ...
    res.json({
      skills: [
        {
          name: "workcell",
          path: `/api/invites/${token}/skills/workcell`,
        },
      ],
    });
  });
  ...
  router.get("/invites/:token/skills/:skillName", async (req, res) => {
    ...
    const skillName = (req.params.skillName as string).trim().toLowerCase();
    if (skillName !== "workcell") throw notFound("Skill not found");
    const markdown = readSkillMarkdown(skillName);
    if (!markdown) throw notFound("Skill not found");
    res.type("text/markdown").send(markdown);
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              workcellApiUrl: req.body.workcellApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.workcellApiUrl &&
          !persistedDefaults.workcellApiUrl
        ) {
          missingPersistedFields.push("workcellApiUrl");
        }
