...
import { createAssetImageMetadataSchema } from "@workcell/shared";
...
export function assetRoutes(db: Db, storage: StorageService) {
  ...
  router.get("/assets/:assetId/content", async (req, res, next) => {
    ...
    } else if (responseContentType === "text/html" || responseContentType.startsWith("text/html;")) {
      // Agent-authored 시안 (design mockup) HTML is UNTRUSTED. The bare `sandbox`
      // directive (NO allow-scripts, NO allow-same-origin) forces an opaque
      // origin and disables ALL JavaScript even though this is served from the
      // app origin — restoring exactly the security property the old
      // data:text/html URL had (no window.parent, no app cookies/localStorage).
      // It applies regardless of the embedding iframe's sandbox attribute, so it
      // is the real XSS control here (nosniff + inline disposition are not
      // sufficient alone). Inline CSS + inline/data images still render the
      // mockup. See ui/src/pages/DesignSystem.tsx (sandbox="") for defense-in-depth.
      res.setHeader(
        "Content-Security-Policy",
        "sandbox; default-src 'none'; img-src 'self' data: blob:; style-src 'unsafe-inline'; font-src 'self' data:; frame-ancestors 'self'",
      );
