...
export function companyRoutes(db: Db, storage?: StorageService) {
  ...
  async function assertCanUpdateBranding(req: Request, companyId: string) {
    ...
    if (actorAgent.role !== "orchestrator") {
      throw forbidden("Only the Orchestrator can update company branding");
    }
  }
  ...
  async function assertCanManagePortability(req: Request, companyId: string, capability: "imports" | "exports") {
    ...
    if (actorAgent.role !== "orchestrator") {
      throw forbidden(`Only the Orchestrator can manage company ${capability}`);
    }
  }
  ...
  router.post(COMPANY_IMPORT_ROUTE_PATH, async (req, res) => {
    ...
    if (req.header("x-workcell-cloud-async-import") === "1") {
      assertCloudTenantCaller(req);
      cleanupTerminalImportJobs(importJobs, importJobTerminalRetentionMs);
  ...
  router.patch("/:companyId", async (req, res) => {
    ...
    if (req.actor.type === "agent") {
      // Only the Orchestrator may update company branding fields
      const agentSvc = agentService(db);
      const actorAgent = req.actor.agentId ? await agentSvc.getById(req.actor.agentId) : null;
      if (!actorAgent || actorAgent.role !== "orchestrator") {
        throw forbidden("Only the Orchestrator or board users may update company settings");
      }
...
function cloudTenantRequestKey(req: Request) {
  return [
    req.actor.userId ?? "",
    req.header("x-workcell-cloud-stack-id")?.trim() ?? "",
    req.header("x-workcell-cloud-workcell-company-id")?.trim() ?? "",
  ].join(":");
}
