...
export function cloudUpstreamRoutes(db: Db, options: { instanceId?: string } = {}) {
  ...
  router.post("/cloud-upstreams/connect/finish", async (req, res) => {
    assertBoardOrgAccess(req);
    await assertEnabled();
    // No companyId is supplied here: the pending connection is bound to its company
    // server-side and guarded by the unguessable pendingConnectionId + state secret
    // issued only to the authorized startConnect caller (capability-token model).
