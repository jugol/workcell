...
import { companies, companySkills, createDb } from "@workcell/db";
import {
...
describeEmbeddedPostgres("companySkillService.list", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("workcell-company-skills-service-");
  ...
  it("lists skills without exposing markdown content", async () => {
    ...
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-heavy-skill-"));
