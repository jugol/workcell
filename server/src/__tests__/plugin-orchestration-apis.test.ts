...
describeEmbeddedPostgres("plugin orchestration APIs", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("workcell-plugin-orchestration-");
  ...
  async function makeLocalRoot() {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-plugin-host-folder-"));
    tempRoots.push(root);
    return root;
  }
  ...
  it("returns plugin-safe execution workspace metadata scoped to the company", async () => {
    ...
    await db.insert(executionWorkspaces).values({
      ...
      repoUrl: "https://example.com/workcell.git",
      baseRef: "main",
      branchName: "feature/workspace",
      providerType: "git_worktree",
      providerRef: "/tmp/workcell-feature",
      metadata: {
    ...
    const services = buildHostServices(db, "plugin-record-id", "workcell.workspace", createEventBusStub());
  ...
  it("creates plugin-origin issues with full orchestration fields and audit activity", async () => {
    ...
    const services = buildHostServices(db, "plugin-record-id", "workcell.missions", createEventBusStub());
    ...
    expect(stored?.originKind).toBe("plugin:workcell.missions");
      ...
      expect.arrayContaining([
        expect.objectContaining({
          ...
          details: expect.objectContaining({
            ...
            sourcePluginKey: "workcell.missions",
  ...
  it("enforces plugin origin namespaces", async () => {
    ...
    expect(featureIssue.originKind).toBe("plugin:workcell.missions:feature");
    ...
    ).rejects.toThrow("Plugin may only use originKind values under plugin:workcell.missions");
  ...
  it("creates plugin operation issues with the generic operation origin", async () => {
    ...
    expect(issue.originKind).toBe("plugin:workcell.missions:operation");
  ...
  it("lets bootstrap-style actions initialize required local folders from an empty root", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "workcell.plugin-llm-wiki",
      packageName: "@workcell/plugin-llm-wiki",
      version: "0.1.0",
      manifestJson: {
        id: "workcell.plugin-llm-wiki",
        apiVersion: 1,
        version: "0.1.0",
        ...
        localFolders: [
          {
            ...
            access: "readWrite",
            requiredDirectories: ["raw", "wiki", "wiki/concepts", ".workcell"],
  ...
  it("rejects worker local-folder access for undeclared manifest keys", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "workcell.local-folders",
      packageName: "@workcell/plugin-local-folders",
      version: "0.1.0",
      manifestJson: {
        id: "workcell.local-folders",
        apiVersion: 1,
        version: "0.1.0",
  ...
  it("resolves plugin-managed projects by stable key without overwriting user edits", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "workcell.missions",
      packageName: "@workcell/plugin-missions",
      version: "0.1.0",
      apiVersion: 1,
      categories: ["automation"],
      status: "ready",
      manifestJson: {
        id: "workcell.missions",
        apiVersion: 1,
        version: "0.1.0",
    ...
    const services = buildHostServices(db, pluginId, "workcell.missions", createEventBusStub());
