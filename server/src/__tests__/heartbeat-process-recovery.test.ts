...
describeEmbeddedPostgres("heartbeat orphaned process recovery", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("workcell-heartbeat-recovery-");
  ...
  it("resumes a restart-interrupted run at startup even after the periodic retry cap is spent", async () => {
    // The stuck-board bug: a dev-server restart marks the in-flight run as
    // process_lost. Once processLossRetryCount reaches 1 a PERIODIC reap gives up
    // (see the "retry is exhausted" test), so the failed run sits in the board 할
    // 일 forever across further restarts. A STARTUP reap treats it as
    // interrupted-by-restart and resumes it (up to RESTART_RESUME_LIMIT), so a
    // fresh queued run supersedes the failure in the inbox instead of stranding it.
    ...
    const result = await heartbeat.reapOrphanedRuns({ reason: "startup" });
    expect(result.reaped).toBe(1);
    expect(result.runIds).toEqual([runId]);
    ...
    const failedRun = runs.find((row) => row.id === runId);
    const resumeRun = runs.find((row) => row.id !== runId);
    // Honest, non-alarming disposition — an operational restart, not "Process lost".
    expect(failedRun?.status).toBe("failed");
    expect(failedRun?.error).toContain("Interrupted by server restart");
    expect(failedRun?.error).toContain("resuming");
    expect(failedRun?.error).not.toContain("Process lost");
    // Resumed despite the spent retry cap; the count carries onto the resumed run.
    expect(resumeRun?.status).toBe("queued");
    expect(resumeRun?.retryOfRunId).toBe(runId);
    expect(resumeRun?.processLossRetryCount).toBe(2);
  });
  ...
  it("dispatches assigned todo work with no prior run as a normal assignment wake", async () => {
    ...
    // A run was claimed for this assigned todo issue (runs above), and system
    // auto-start (feat 1fff12d "auto-start when work actually begins") flips a
    // scoped todo issue to in_progress the moment its run is claimed — the board
    // reflects that work has begun without waiting for the agent to self-checkout.
    // This assertion predated auto-start and long expected "todo"; migrated to the
    // intended behavior (see issue-auto-start.ts autoStartScopedIssue + its tests).
    const issue = await db.select().from(issues).where(eq(issues.id, issueId)).then((rows) => rows[0] ?? null);
    expect(issue?.status).toBe("in_progress");
  ...
  it("WC-201: a parent waiting behind an open design_request child is NOT stranded (suppression lifts when the child closes)", async () => {
    const { companyId, agentId, issueId } = await seedStrandedIssueFixture({
      status: "in_progress",
      runStatus: "cancelled",
      retryReason: "issue_continuation_needed",
      runErrorCode: "design_first_hold",
    });
    const issuePrefix = `T${companyId.replace(/-/g, "").slice(0, 6).toUpperCase()}`;
    const childId = randomUUID();
    // The design-first gate parks the parent behind this child (originKind
    // design_request, originId=parent). Unassigned, like the gate when no
    // designer exists — so the scan's assignee-scoped query never processes it.
    await db.insert(issues).values({
      id: childId,
      companyId,
      title: "Design 시안: held parent",
      status: "todo",
      priority: "high",
      parentId: issueId,
    ...
    // Phase 1 — OPEN design child ⇒ the parent is suppressed (skipped), not
    // stranded: no escalate, no re-wake, status stays in_progress.
    ...
    const stillHeld = await db.select().from(issues).where(eq(issues.id, issueId)).then((r) => r[0] ?? null);
    expect(stillHeld?.status).toBe("in_progress"); // never flipped to blocked
    const runsAfterHold = await db.select().from(heartbeatRuns).where(eq(heartbeatRuns.agentId, agentId));
    expect(runsAfterHold).toHaveLength(1); // no re-run / re-cancel of the held parent

    // Phase 2 — child cancelled ⇒ suppression lifts ⇒ the parent is processed
    // by normal recovery again (no longer skipped).
    await db.update(issues).set({ status: "cancelled" }).where(eq(issues.id, childId));
