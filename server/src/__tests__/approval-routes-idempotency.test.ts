...
describe("approval routes idempotent retries", () => {
  beforeEach(() => {
    ...
    mockWorkProductService.setDesignReviewState.mockReset();
  ...
  it("syncs board-approved design approval into the design review state", async () => {
    const workProduct = {
    ...
    mockApprovalService.getById.mockResolvedValue({
      ...
      payload: {
        ...
        summary: `승인 대상 design artifact work product ${workProduct.id.slice(0, 8)}`,
    ...
    mockWorkProductService.setDesignReviewState.mockResolvedValue({
      ...workProduct,
    ...
    expect(res.status).toBe(200);
    expect(mockWorkProductService.setDesignReviewState).toHaveBeenCalledWith(
      workProduct.id,
      "approved",
    );
