...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "workcell", LOGNAME: "workcell", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "workcell" }),
      },
    );

    expect(tokens).toEqual(["workcell", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "workcell" }, {
        userInfo: () => ({ username: "workcell" }),
      });

      expect(tokens).toEqual(["workcell", "custom-token"]);
    } finally {
