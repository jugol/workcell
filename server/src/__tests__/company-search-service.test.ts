...
import { companySearchQuerySchema, COMPANY_SEARCH_MAX_QUERY_LENGTH } from "@workcell/shared";
import {
...
describeEmbeddedPostgres("companySearchService", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("workcell-company-search-");
  ...
  async function createCompany(name = "Workcell") {
