...
describe("mergeJoinDefaultsPayloadForReplay", () => {
  it("merges replay payloads and allows gateway token override", () => {
    const merged = mergeJoinDefaultsPayloadForReplay(
      {
        ...
        workcellApiUrl: "http://host.docker.internal:3100",
        headers: {
      ...
      {
        workcellApiUrl: "https://workcell.example.com",
        headers: {
    ...
    expect(normalized.workcellApiUrl).toBe("https://workcell.example.com");
