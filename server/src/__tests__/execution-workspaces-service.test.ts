...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-execution-workspace-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.name", "Workcell Test"]);
  await runGit(repoRoot, ["config", "user.email", "test@workcell.local"]);
...
describeEmbeddedPostgres("executionWorkspaceService.getCloseReadiness", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("workcell-execution-workspaces-service-");
  ...
  it("warns about dirty and unmerged git worktrees and reports cleanup actions", async () => {
    ...
    const worktreePath = path.join(path.dirname(repoRoot), `workcell-worktree-${randomUUID()}`);
    tempDirs.add(worktreePath);

    await runGit(repoRoot, ["branch", "workcell-close-check"]);
    await runGit(repoRoot, ["worktree", "add", worktreePath, "workcell-close-check"]);
    ...
    await db.insert(executionWorkspaces).values({
      ...
      branchName: "workcell-close-check",
      baseRef: "main",
      metadata: {
