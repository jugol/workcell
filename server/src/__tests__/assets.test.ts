...
describe("GET /api/assets/:assetId/content", () => {
  ...
  function streamingStorage(body: string): ReturnType<typeof createStorageService> {
  ...
  it("serves an agent-authored text/html 시안 asset under a strict sandbox CSP", async () => {
    const html = "<html><body>sian-marker</body></html>";
    const app = await createApp(streamingStorage(html));
    getAssetByIdMock.mockResolvedValue({
      ...createAsset(),
      contentType: "text/html",
      objectKey: "assets/design-sian/sian.html",
      originalFilename: "sian.html",
    ...
    // The bare `sandbox` directive (NO allow-scripts) is the real XSS control —
    // it disables JS for this same-origin response, restoring the data: url's
    // opaque-origin protection.
    expect(csp).toContain("sandbox");
    expect(csp).toContain("default-src 'none'");
    expect(csp).not.toContain("allow-scripts");
    expect(res.headers["x-frame-options"]).toBe("SAMEORIGIN");
    expect(res.headers["x-content-type-options"]).toBe("nosniff");
    expect(res.text).toContain("sian-marker");
  });

  it("serves text/html with a charset parameter under the same sandbox CSP", async () => {
    const body = "<html></html>";
    const app = await createApp(streamingStorage(body));
    getAssetByIdMock.mockResolvedValue({
      ...createAsset(),
  ...
  it("does NOT set the html sandbox CSP for a normal image asset", async () => {
    const body = "binary";
    const app = await createApp(streamingStorage(body));
    getAssetByIdMock.mockResolvedValue({
      ...createAsset(),
  ...
  it("returns 404 when the asset does not exist", async () => {
    const app = await createApp(streamingStorage(""));
    getAssetByIdMock.mockResolvedValue(null);

    const res = await requestApp(app, (baseUrl) => request(baseUrl).get("/api/assets/missing/content"));
