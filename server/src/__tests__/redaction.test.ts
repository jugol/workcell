...
describe("redaction", () => {
  ...
  it("redacts common secret shapes from unstructured text", () => {
    ...
    const input = [
      ...
      `workcell {"WORKCELL_API_KEY":"workcell-json-secret"}`,
    ...
    expect(result).not.toContain("workcell-json-secret");
    expect(result).not.toContain("escaped-json-secret");
    expect(result).not.toContain("workcell-shell-secret");
  ...
  it("redacts inline secrets from command metadata without hiding safe command text", () => {
    const input = {
      ...
      env: {
        WORKCELL_RESOLVED_COMMAND: "env OPENAI_API_KEY=sk-live-example custom-acp --token ghp_example_secret",
        SAFE_VALUE: "visible",
      },
    };
