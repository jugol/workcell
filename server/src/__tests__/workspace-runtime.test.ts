...
import { resolveWorkcellConfigPath } from "../paths.ts";
import type { WorkspaceOperation } from "@workcell/shared";
...
async function createTempRepo(defaultBranch = "main") {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-worktree-repo-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.email", "workcell@example.com"]);
  await runGit(repoRoot, ["config", "user.name", "Workcell Test"]);
...
afterEach(async () => {
  ...
  delete process.env.WORKCELL_CONFIG;
  delete process.env.WORKCELL_HOME;
  delete process.env.WORKCELL_INSTANCE_ID;
  delete process.env.WORKCELL_WORKTREES_DIR;
  delete process.env.DATABASE_URL;
  await resetRuntimeServicesForTests();
});

describe("sanitizeRuntimeServiceBaseEnv", () => {
  it("removes inherited Workcell and pnpm auth flags before spawning runtime services", () => {
    const sanitized = sanitizeRuntimeServiceBaseEnv({
      PATH: process.env.PATH,
      DATABASE_URL: "postgres://example.test/workcell",
      WORKCELL_HOME: "/tmp/workcell-home",
      WORKCELL_INSTANCE_ID: "runtime-instance",
      npm_config_tailscale_auth: "true",
      npm_config_authenticated_private: "true",
      HOST: "0.0.0.0",
    });

    expect(sanitized.WORKCELL_HOME).toBeUndefined();
    expect(sanitized.WORKCELL_INSTANCE_ID).toBeUndefined();
...
describe("ensureServerWorkspaceLinksCurrent", () => {
  it("relinks stale server workspace dependencies inside the current repo root", async () => {
    const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-runtime-links-"));
    const staleRoot = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-runtime-links-stale-"));
    const serverNodeModulesScopeDir = path.join(repoRoot, "server", "node_modules", "@workcell");
    ...
    await fs.writeFile(path.join(repoRoot, ".git"), "gitdir: /tmp/workcell-main/.git/worktrees/runtime-links\n", "utf8");
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "@workcell/db" }),
      "utf8",
    );
    await fs.writeFile(
  ...
  it("skips relinking when server workspace dependencies already point at the repo", async () => {
    const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-runtime-links-current-"));
    const serverNodeModulesScopeDir = path.join(repoRoot, "server", "node_modules", "@workcell");
    const expectedPackageDir = path.join(repoRoot, "packages", "db");
    ...
    await fs.mkdir(expectedPackageDir, { recursive: true });
    await fs.mkdir(serverNodeModulesScopeDir, { recursive: true });
    await fs.writeFile(path.join(repoRoot, ".git"), "gitdir: /tmp/workcell-main/.git/worktrees/runtime-links-current\n", "utf8");
    await fs.writeFile(path.join(repoRoot, "pnpm-workspace.yaml"), "packages:\n  - packages/*\n  - server\n", "utf8");
    await fs.writeFile(
      path.join(repoRoot, "server", "package.json"),
      JSON.stringify({
  ...
  it("skips relinking outside linked git worktrees", async () => {
    const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-runtime-links-non-worktree-"));
    const staleRoot = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-runtime-links-non-worktree-stale-"));
    const serverNodeModulesScopeDir = path.join(repoRoot, "server", "node_modules", "@workcell");
    const expectedPackageDir = path.join(repoRoot, "packages", "db");
    const stalePackageDir = path.join(staleRoot, "db");
...
describe("realizeExecutionWorkspace", () => {
  it("creates and reuses a git worktree for an issue-scoped branch", async () => {
    ...
    expect(first.cwd).toContain(path.join(".workcell", "worktrees"));
  ...
  it("rejects reusing an empty directory that only looks like a worktree because it sits inside the repo", async () => {
    ...
    const poisonedPath = path.join(repoRoot, ".workcell", "worktrees", branchName);
  ...
  it("reuses the current linked worktree instead of nesting another worktree inside it", async () => {
    ...
    const currentWorktree = path.join(repoRoot, ".workcell", "worktrees", branchName);
  ...
  it("reuses an already checked out branch from git worktree metadata even when the target path differs", async () => {
    const repoRoot = await createTempRepo();
    const branchName = "PAP-1355-worktree-reuse";
    const existingWorktree = path.join(repoRoot, ".workcell", "worktrees", branchName);
    ...
    const realized = await realizeExecutionWorkspace({
      ...
      config: {
        workspaceStrategy: {
          type: "git_worktree",
          branchTemplate: "{{issue.identifier}}-{{slug}}",
          worktreeParentDir: ".workcell/other-worktrees",
        },
      },
      issue: {
        id: "issue-1",
  ...
  it("runs a configured provision command inside the derived worktree", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "printf '%s\\n' \"$WORKCELL_WORKSPACE_BRANCH\" > .workcell-provision-branch",
        "printf '%s\\n' \"$WORKCELL_WORKSPACE_BASE_CWD\" > .workcell-provision-base",
        "printf '%s\\n' \"$WORKCELL_WORKSPACE_CREATED\" > .workcell-provision-created",
      ].join("\n"),
      "utf8",
    );
    ...
    await expect(fs.readFile(path.join(workspace.cwd, ".workcell-provision-branch"), "utf8")).resolves.toBe(
      "PAP-448-run-provision-command\n",
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".workcell-provision-base"), "utf8")).resolves.toBe(
      `${repoRoot}\n`,
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".workcell-provision-created"), "utf8")).resolves.toBe(
      "true\n",
    );
    ...
    await expect(fs.readFile(path.join(reused.cwd, ".workcell-provision-created"), "utf8")).resolves.toBe("false\n");
  });
  ...
  it("uses the latest repo-managed provision script when reusing an existing worktree", async () => {
    ...
    await expect(fs.readFile(path.join(initial.cwd, ".workcell-provision-version"), "utf8")).resolves.toBe("v1\n");
    ...
    await expect(fs.readFile(path.join(reused.cwd, ".workcell-provision-version"), "utf8")).resolves.toBe("v2\n");
  }, 30_000);

  it("writes an isolated repo-local Workcell config and worktree branding when provisioning", async () => {
    const repoRoot = await createTempRepo();
    const previousCwd = process.cwd();
    const previousPath = process.env.PATH;
    const workcellHome = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-worktree-home-"));
    const isolatedWorktreeHome = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-worktrees-"));
    const isolatedBin = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-worktree-bin-"));
    const instanceId = "worktree-base";
    const sharedConfigDir = path.join(workcellHome, "instances", instanceId);
    const sharedConfigPath = path.join(sharedConfigDir, "config.json");
    const sharedEnvPath = path.join(sharedConfigDir, ".env");

    process.env.WORKCELL_HOME = workcellHome;
    process.env.WORKCELL_INSTANCE_ID = instanceId;
    process.env.WORKCELL_WORKTREES_DIR = isolatedWorktreeHome;
    ...
    await fs.writeFile(sharedEnvPath, 'DATABASE_URL="postgres://worktree:test@db.example.com:6543/workcell"\n', "utf8");
    ...
    try {
      ...
      const configPath = path.join(workspace.cwd, ".workcell", "config.json");
      const envPath = path.join(workspace.cwd, ".workcell", ".env");
      ...
      expect(envVars.WORKCELL_HOME).toBe(isolatedWorktreeHome);
      expect(envVars.WORKCELL_INSTANCE_ID).toBe(expectedInstanceId);
      expect(await fs.realpath(envVars.WORKCELL_CONFIG!)).toBe(await fs.realpath(configPath));
      expect(envVars.WORKCELL_IN_WORKTREE).toBe("true");
      expect(envVars.WORKCELL_WORKTREE_NAME).toBe("PAP-885-show-worktree-banner");

      process.chdir(workspace.cwd);
      expect(resolveWorkcellConfigPath()).toBe(configPath);
      ...
      await fs.writeFile(envPath, `${envContents}WORKCELL_WORKTREE_COLOR="#112233"\n`, "utf8");
      ...
      expect(reusedEnvContents).toContain('WORKCELL_WORKTREE_COLOR="#112233"');
    } finally {
  ...
  it("provisions successfully when install is needed but there are no symlinked node_modules to move", async () => {
    ...
    await expect(fs.readFile(path.join(workspace.cwd, ".workcell", "config.json"), "utf8")).resolves.toContain(
      "\"database\"",
    );
  }, 30_000);

  it("fails instead of writing an unseeded fallback config when worktree init errors after CLI detection succeeds", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-worktree-provision-fail-"));
    ...
    try {
      ...
      await fs.writeFile(
        ...
        [
          "#!/bin/sh",
          "if [ \"$1\" = \"workcell\" ] && [ \"$2\" = \"--help\" ]; then",
          "  exit 0",
          "fi",
          "if [ \"$1\" = \"workcell\" ] && [ \"$2\" = \"worktree\" ] && [ \"$3\" = \"init\" ]; then",
      ...
      try {
        await execFileAsync(scriptPath, [], {
          ...
          env: {
            ...
            WORKCELL_WORKSPACE_BASE_CWD: baseRoot,
            WORKCELL_WORKSPACE_CWD: worktreeRoot,
          },
        });
      } catch (error) {
      ...
      await expect(fs.stat(path.join(worktreeRoot, ".workcell", "config.json"))).rejects.toThrow();
      await expect(fs.stat(path.join(worktreeRoot, ".workcell", ".env"))).rejects.toThrow();
    } finally {
  ...
  it("retries worktree-local pnpm install without a frozen lockfile when the lockfile is outdated", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-worktree-outdated-lockfile-"));
    const baseRoot = path.join(tempRoot, "base");
    const worktreeRoot = path.join(tempRoot, "worktree");
    const fakeBin = path.join(tempRoot, "bin");
    const fakePnpmPath = path.join(fakeBin, "pnpm");
    ...
    try {
      ...
      await expect(fs.readFile(path.join(worktreeRoot, ".workcell", "config.json"), "utf8")).resolves.toContain(
        "\"database\"",
      );
    } finally {
      await fs.rm(tempRoot, { recursive: true, force: true });
  ...
  it("reattaches a missing persisted git worktree before manual control starts it", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "printf '%s\\n' \"$WORKCELL_WORKSPACE_BRANCH\" > .workcell-restored-branch",
      ].join("\n"),
      "utf8",
    );
    ...
    await expect(fs.readFile(path.join(initial.cwd, ".workcell-restored-branch"), "utf8")).resolves.toBe(`${branchName}\n`);
  ...
  it("reprovisions an existing persisted git worktree before manual control starts it", async () => {
    ...
    await fs.rm(path.join(initial.cwd, ".workcell-restored-state"), { force: true });
    ...
    await expect(fs.readFile(path.join(initial.cwd, ".workcell-restored-state"), "utf8")).resolves.toBe("reprovisioned\n");
  }, 15_000);
  ...
  it("auto-detects the default branch when baseRef is not configured", async () => {
    ...
    const bareRemote = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-worktree-bare-"));
  ...
  it("auto-detects the default branch via symbolic-ref when origin/HEAD is set", async () => {
    ...
    const bareRemote = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-worktree-bare-symref-"));
    await runGit(bareRemote, ["init", "--bare"]);
    await runGit(repoRoot, ["remote", "add", "origin", bareRemote]);
    await runGit(repoRoot, ["push", "-u", "origin", "master"]);
    await runGit(repoRoot, ["fetch", "origin"]);
...
describe("ensureRuntimeServicesForRun", () => {
  it("leaves manual runtime services untouched during agent runs", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-runtime-manual-"));
  ...
  it("reuses shared runtime services across runs and starts a new service after release", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-runtime-workspace-"));
    const workspace = buildWorkspace(workspaceRoot);
  ...
  it("does not reuse project-scoped shared services across different workspace launch contexts", async () => {
    const primaryWorkspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-runtime-primary-"));
    const worktreeWorkspaceRoot = path.join(primaryWorkspaceRoot, ".workcell", "worktrees", "PAP-874-chat-speed-issues");
    ...
    const serviceCommand =
      "node -e \"require('node:http').createServer((req,res)=>res.end(process.env.WORKCELL_HOME)).listen(Number(process.env.PORT), '127.0.0.1')\"";
    const config = {
      workspaceRuntime: {
        services: [
          {
            ...
            cwd: ".",
            env: {
              WORKCELL_HOME: "{{workspace.cwd}}/.workcell/runtime-services",
            },
            port: { type: "auto" },
            readiness: {
              type: "http",
    ...
    expect(await primaryResponse.text()).toBe(path.join(primaryWorkspaceRoot, ".workcell", "runtime-services"));

    const executionResponse = await fetch(executionServices[0]!.url!);
    expect(await executionResponse.text()).toBe(path.join(worktreeWorkspaceRoot, ".workcell", "runtime-services"));
  });

  it("does not leak parent Workcell instance env into runtime service commands", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-runtime-env-"));
    const workspace = buildWorkspace(workspaceRoot);
    ...
    const serviceCommand = [
      ...
      JSON.stringify(
        [
          ...
          "workcellConfig: process.env.WORKCELL_CONFIG ?? null,",
          "workcellHome: process.env.WORKCELL_HOME ?? null,",
          "workcellInstanceId: process.env.WORKCELL_INSTANCE_ID ?? null,",
    ...
    process.env.WORKCELL_CONFIG = "/tmp/base-workcell-config.json";
    process.env.WORKCELL_HOME = "/tmp/base-workcell-home";
    process.env.WORKCELL_INSTANCE_ID = "base-instance";
    process.env.DATABASE_URL = "postgres://shared-db.example.com/workcell";
    ...
    expect(captured.workcellConfig).toBeNull();
    expect(captured.workcellHome).toBeNull();
    expect(captured.workcellInstanceId).toBeNull();
  ...
  it("stops execution workspace runtime services by executionWorkspaceId", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-runtime-stop-"));
    const workspace = buildWorkspace(workspaceRoot);
  ...
  it("does not stop services in sibling directories when matching by workspace cwd", async () => {
    const workspaceParent = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-runtime-sibling-"));
  ...
  it("starts only the selected workspace-controlled runtime service", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-runtime-control-start-"));
    const workspace = buildWorkspace(workspaceRoot);
  ...
  it("stops only the selected execution workspace runtime service", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-runtime-control-stop-"));
    const workspace = buildWorkspace(workspaceRoot);
...
describeEmbeddedPostgres("workspace runtime startup reconciliation", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("workcell-workspace-runtime-");
  ...
  it("adopts a live auto-port shared service after runtime state is reset", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-runtime-reconcile-"));
    const workcellHome = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-runtime-home-"));
    process.env.WORKCELL_HOME = workcellHome;
    process.env.WORKCELL_INSTANCE_ID = `runtime-reconcile-${randomUUID()}`;
  ...
  it("marks persisted local services stopped when the registry pid is stale", async () => {
    ...
    await db.insert(workspaceRuntimeServices).values({
      ...
      serviceName: "workcell-dev",
      status: "running",
      lifecycle: "shared",
      reuseKey: `project_workspace:${projectWorkspaceId}:workcell-dev`,
      command: "pnpm dev",
      cwd: "/tmp/workcell-primary",
      port: 49195,
    ...
    await writeLocalServiceRegistryRecord({
      version: 1,
      serviceKey: "workspace-runtime-workcell-dev-stale",
  ...
  it("persists controlled execution workspace stops as stopped", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-runtime-stop-persisted-"));
    const companyId = randomUUID();
    const agentId = randomUUID();
    const projectId = randomUUID();
    const runId = randomUUID();
  ...
  it("restarts a stopped auto-port service on the same port when it is available", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-runtime-port-reuse-"));
    const companyId = randomUUID();
    const agentId = randomUUID();
    const projectId = randomUUID();
    const executionWorkspaceId = randomUUID();
