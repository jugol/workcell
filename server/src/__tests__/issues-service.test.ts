...
import { buildProjectMentionHref, MAX_ISSUE_REQUEST_DEPTH } from "@workcell/shared";
...
describeEmbeddedPostgres("issueService.list participantAgentId", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("workcell-issues-service-");
  ...
  afterEach(async () => {
    ...
    // WC-12 auto-creates a compound-checklist document on Done transitions;
    // tear those down (children-first) before the issues + companies parents.
  ...
  it("ranks comment matches ahead of description-only matches", async () => {
    ...
    await db.insert(issueComments).values({
      ...
      body: "Reference: https://github.com/jugol/workcell/pull/3303",
    });
  ...
  it("hides plugin operation issues from default lists and inbox-style filters while preserving explicit retrieval", async () => {
    ...
    await db.insert(issues).values([
      ...
      },
      {
        ...
        priority: "medium",
        assigneeAgentId: agentId,
        originKind: "plugin:workcell.missions:operation",
      ...
      },
      {
        ...
        priority: "medium",
        assigneeAgentId: agentId,
        originKind: "plugin:workcell.missions:operation:evaluation",
      ...
      },
      {
        ...
        priority: "medium",
        assigneeAgentId: agentId,
        originKind: "plugin:workcell.content-machine:evaluation",
    ...
    await expect(svc.list(companyId, { originKind: "plugin:workcell.missions:operation" }))
    ...
    await expect(svc.list(companyId, { originId: "mission-alpha:operation-1" }))
      .resolves.toEqual([expect.objectContaining({ id: operationIssueId })]);
    await expect(svc.list(companyId, { originKindPrefix: "plugin:workcell.missions:operation" }))
...
describeEmbeddedPostgres("issueService.create workspace inheritance", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("workcell-issues-create-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
    await ensureIssueRelationsTable(db);
  }, 20_000);
  ...
  it("inherits the parent's project when a child is created without an explicit projectId (project guard)", async () => {
    const companyId = randomUUID();
    const projectId = randomUUID();
    const parentIssueId = randomUUID();
    await db.insert(companies).values({
    ...
    await db.insert(issues).values({
      ...
      companyId,
      projectId,
      title: "Parent in a project",
      status: "in_progress",
      priority: "medium",
    });

    // Child WITHOUT projectId → inherits the parent's project (no company-scope leak).
    const inherited = await svc.create(companyId, { parentId: parentIssueId, title: "design 시안 child" });
    expect(inherited.projectId).toBe(projectId);

    // A top-level issue (no parent) stays project-less — the guard only inherits.
    const topLevel = await svc.create(companyId, { title: "Top-level issue" });
    expect(topLevel.projectId).toBeNull();
  });
...
describeEmbeddedPostgres("issueService blockers and dependency wake readiness", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("workcell-issues-blockers-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
    await ensureIssueRelationsTable(db);
  }, 20_000);
...
describeEmbeddedPostgres("issueService.findMentionedProjectIds", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("workcell-issues-mentioned-projects-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
    await ensureIssueRelationsTable(db);
  }, 20_000);
...
describeEmbeddedPostgres("issueService.clearExecutionRunIfTerminal", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("workcell-issues-execution-lock-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
  }, 20_000);
