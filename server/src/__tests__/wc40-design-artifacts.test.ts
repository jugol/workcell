...
import { companies, createDb, issueWorkProducts, issues, projects } from "@workcell/db";
import {
...
describeEmbeddedPostgres("WC-40 Open Design artifact listing route (PLAN §9 #4)", () => {
  ...
  afterEach(async () => {
    await db.execute(
      "truncate table companies, issues, issue_work_products, projects restart identity cascade" as any,
    );
  });
  ...
  it("returns work products matching the default design type set", async () => {
    ...
    // The linked issue's status rides along (the inbox uses it to hide design
    // reviews for terminal issues). The seed issue is in_progress.
    expect((res.body.items as any[]).every((i) => i.issueStatus === "in_progress")).toBe(true);
  });
  ...
  describe("project-scoped listing (GET /projects/:projectId/design-artifacts)", () => {
    it("returns only the design artifacts belonging to the project", async () => {
      const projectA = randomUUID();
      const projectB = randomUUID();
      await db.insert(projects).values([
        { id: projectA, companyId, name: "Project A" },
        { id: projectB, companyId, name: "Project B" },
      ]);
      await db.insert(issueWorkProducts).values([
        { companyId, issueId, projectId: projectA, type: "design", provider: "figma", title: "A design", status: "active" },
        { companyId, issueId, projectId: projectB, type: "design", provider: "figma", title: "B design", status: "active" },
        // No project: visible company-wide but NOT in either project view.
        { companyId, issueId, projectId: null, type: "mockup", provider: "workcell", title: "Unscoped mockup", status: "active" },
        // Wrong type inside project A: filtered out by the design type set.
        { companyId, issueId, projectId: projectA, type: "proof", provider: "workcell", title: "A proof", status: "active" },
      ]);

      const res = await request(app).get(`/api/projects/${projectA}/design-artifacts`);
      expect(res.status).toBe(200);
      expect(res.body.items).toHaveLength(1);
      expect(res.body.items[0].title).toBe("A design");

      // The company-wide overview still sees everything design-typed.
      const companyRes = await request(app).get(`/api/companies/${companyId}/design-artifacts`);
      expect(companyRes.body.items).toHaveLength(3);
    });

    it("returns 404 for an unknown or malformed project id", async () => {
      const missing = await request(app).get(`/api/projects/${randomUUID()}/design-artifacts`);
      expect(missing.status).toBe(404);
      expect(missing.body.error).toBe("Project not found");

      const malformed = await request(app).get("/api/projects/not-a-uuid/design-artifacts");
