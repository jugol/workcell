...
function manifest(): WorkcellPluginManifestV1 {
  return {
    id: "workcell.managed-agents-test",
    apiVersion: 1,
    version: "0.1.0",
...
describeEmbeddedPostgres("plugin-managed agents", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("workcell-plugin-managed-agents-");
  ...
  async function seedCompanyAndPlugin(options: { requireApproval?: boolean; manifest?: WorkcellPluginManifestV1 } = {}) {
    ...
    await db.insert(plugins).values({
      ...
      packageName: "@workcell/plugin-managed-agents-test",
  ...
  it("materializes declared managed agent instructions with local folder paths", async () => {
    const previousHome = process.env.WORKCELL_HOME;
    const previousInstance = process.env.WORKCELL_INSTANCE_ID;
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-managed-agent-home-"));
    const wikiRoot = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "workcell-managed-agent-wiki-")));
    process.env.WORKCELL_HOME = tempHome;
    process.env.WORKCELL_INSTANCE_ID = "test";
    try {
      const pluginManifest = manifest();
      ...
      if (previousHome === undefined) delete process.env.WORKCELL_HOME;
      else process.env.WORKCELL_HOME = previousHome;
      if (previousInstance === undefined) delete process.env.WORKCELL_INSTANCE_ID;
      else process.env.WORKCELL_INSTANCE_ID = previousInstance;
  ...
  it("respects board approval policy for new managed agents", async () => {
    ...
    expect(approval?.payload).toMatchObject({
      agentId: created.agentId,
      sourcePluginKey: "workcell.managed-agents-test",
