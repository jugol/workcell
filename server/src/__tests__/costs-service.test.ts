...
describeEmbeddedPostgres("cost and finance aggregate overflow handling", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("workcell-costs-service-");
  ...
  it("WC-162: createEvent rejects a nonexistent FK with a clean 404, not a 23503/500", async () => {
    ...
    // A well-formed but nonexistent issueId is a hard FK (cost_events_issue_id, no
    // onDelete): without the WC-162 guard the insert hits Postgres 23503 → 500.
    await expect(
    ...
    // The guard fires before the insert — no partial cost event leaked.
