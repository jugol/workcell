...
import type { CompanyPortabilityFileEntry } from "@workcell/shared";
...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        slug: "workcell",
        name: "workcell",
        description: "Workcell coordination skill",
        markdown: "---\nname: workcell\ndescription: Workcell coordination skill\n---\n\n# Workcell\n",
        sourceType: "github",
        sourceLocator: "https://github.com/workcell/workcell/tree/master/skills/workcell",
  ...
  it("parses canonical GitHub import URLs with explicit ref and package path", () => {
    expect(
      parseGitHubSourceUrl("https://github.com/workcell/companies?ref=feature%2Fdemo&path=gstack"),
    ).toEqual({
      hostname: "github.com",
      owner: "workcell",
      repo: "companies",
  ...
  it("parses canonical GitHub import URLs with explicit companyPath", () => {
    expect(
      parseGitHubSourceUrl(
        "https://github.com/workcell/companies?ref=abc123&companyPath=gstack%2FCOMPANY.md",
      ),
    ).toEqual({
      hostname: "github.com",
      owner: "workcell",
  ...
  it("exports referenced skills as stubs by default with sanitized Workcell extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "Workcell"');
    ...
    expect(asTextFile(exported.files["agents/claudecoder/AGENTS.md"])).toContain(`- "${workcellKey}"`);
    expect(asTextFile(exported.files["agents/cmo/AGENTS.md"])).not.toContain("skills:");
    expect(asTextFile(exported.files["skills/workcell/workcell/workcell/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/workcell/workcell/workcell/SKILL.md"])).toContain('kind: "github-dir"');
    expect(exported.files["skills/workcell/workcell/workcell/references/api.md"]).toBeUndefined();
    ...
    const extension = asTextFile(exported.files[".workcell.yaml"]);
    expect(extension).toContain('schema: "workcell/v1"');
    ...
    expect(extension).not.toContain("workcellSkillSync");
  ...
  it("exports hire approval policy only when approval is required", async () => {
    ...
    expect(asTextFile(exported.files[".workcell.yaml"])).toContain("requireBoardApprovalForNewAgents: true");
  });
  ...
  it("exports default sidebar order into the Workcell extension and manifest", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(asTextFile(exported.files[".workcell.yaml"])).toContain([
      "sidebar:",
      "  agents:",
      '    - "claudecoder"',
      '    - "cmo"',
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/workcell/workcell/workcell/SKILL.md"])).toContain("# Workcell");
    expect(asTextFile(exported.files["skills/workcell/workcell/workcell/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/workcell/workcell/workcell/references/api.md"])).toContain("# API");
  });
  ...
  it("exports only selected skills when skills filter is provided", async () => {
    ...
    expect(asTextFile(exported.files["skills/company/PAP/company-playbook/SKILL.md"])).toContain("# Company Playbook");
    expect(exported.files["skills/workcell/workcell/workcell/SKILL.md"]).toBeUndefined();
  });
  ...
  it("warns and exports all skills when skills filter matches nothing", async () => {
    ...
    expect(exported.files["skills/company/PAP/company-playbook/SKILL.md"]).toBeDefined();
    expect(exported.files["skills/workcell/workcell/workcell/SKILL.md"]).toBeDefined();
  });

  it("exports the company logo into images/ and references it from .workcell.yaml", async () => {
    const storage = {
    ...
    expect(exported.files[".workcell.yaml"]).toContain('logoPath: "images/company-logo.png"');
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    companySkillSvc.listFull.mockResolvedValue([
      ...
      },
      {
        id: "skill-workcell",
        companyId: "company-1",
        key: "workcell/workcell/release-changelog",
        slug: "release-changelog",
        name: "release-changelog",
        description: "Bundled release changelog skill",
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/workcell/workcell/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
        metadata: {
          sourceKind: "workcell_bundled",
          owner: "workcell",
          repo: "workcell",
          ref: "0123456789abcdef0123456789abcdef01234567",
          trackingRef: "master",
    ...
    expect(asTextFile(exported.files["skills/workcell/workcell/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/workcell/workcell/release-changelog/SKILL.md"])).toContain("workcell/workcell/release-changelog");
  });
  ...
  it("exports portable project workspace metadata and remaps it on import", async () => {
        ...
        workspaces: [
          {
            ...
            name: "Main Repo",
            sourceType: "git_repo",
            cwd: "/Users/dotta/workcell",
            repoUrl: "https://github.com/workcell/workcell.git",
            repoRef: "main",
            defaultRef: "main",
            visibility: "default",
            setupCommand: "pnpm install",
            cleanupCommand: "rm -rf .workcell-tmp",
            remoteProvider: null,
    ...
    expect(extension).toContain('repoUrl: "https://github.com/workcell/workcell.git"');
    expect(extension).toContain('defaultProjectWorkspaceKey: "main-repo"');
    expect(extension).toContain('projectWorkspaceKey: "main-repo"');
    expect(extension).not.toContain("/Users/dotta/workcell");
    ...
    await portability.importBundle({
      ...
      target: {
        mode: "new_company",
        newCompanyName: "Imported Workcell",
      },
  ...
  it("infers portable git metadata from a local checkout without task warning fan-out", async () => {
    const portability = companyPortabilityService({} as any);
    const repoDir = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-portability-git-"));
    execFileSync("git", ["init"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["checkout", "-b", "main"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["remote", "add", "origin", "https://github.com/workcell/workcell.git"], {
      cwd: repoDir,
      stdio: "ignore",
    });
    ...
    const extension = asTextFile(exported.files[".workcell.yaml"]);
    expect(extension).toContain('repoUrl: "https://github.com/workcell/workcell.git"');
    expect(extension).toContain('projectWorkspaceKey: "workcell"');
  ...
  it("reads env inputs back from .workcell.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("reads project env inputs back from .workcell.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("exports routines as recurring task packages with Workcell routine extensions", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("imports recurring task packages as routines instead of one-time issues", async () => {
    ...
    const preview = await portability.previewImport({
      source: { type: "inline", rootPath: "workcell-demo", files },
      include: { company: true, agents: true, projects: true, issues: true, skills: false },
      target: { mode: "new_company", newCompanyName: "Imported Workcell" },
      agents: "all",
      collisionStrategy: "rename",
    });
  ...
  it("migrates legacy schedule.recurrence imports into routine triggers", async () => {
    ...
    const files = {
      "COMPANY.md": ['---', 'schema: "agentcompanies/v1"', 'name: "Imported Workcell"', "---", ""].join("\n"),
  ...
  it("imports a vendor-neutral package without .workcell.yaml", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported Workcell");
  ...
  it("imports a packaged company logo and attaches it to the target company", async () => {
    ...
    exported.files[".workcell.yaml"] = `${exported.files[".workcell.yaml"]}`.replace(
      'brandColor: "#5c5fff"\n',
  ...
  it("preserves issue comment presentation fields through export and import", async () => {
    ...
    issueSvc.listComments.mockResolvedValue([
      {
        ...
        authorAgentId: null,
        authorUserId: null,
        body: "Workcell needs a disposition before this issue can continue.",
        presentation,
        metadata,
    ...
    expect(issueSvc.addComment).toHaveBeenCalledWith(
      "issue-imported",
      "Workcell needs a disposition before this issue can continue.",
