...
function registerModuleMocks() {
  vi.doMock("@workcell/shared/telemetry", () => ({
    trackAgentCreated: mockTrackAgentCreated,
    trackErrorHandlerCrash: vi.fn(),
  }));
...
describe.sequential("agent skill routes", () => {
  beforeEach(() => {
    ...
    mockAdapter.listSkills.mockResolvedValue({
      ...
      supported: true,
      mode: "ephemeral",
      desiredSkills: ["workcell/workcell/workcell"],
      entries: [],
      warnings: [],
    });
    ...
    expect(mockAdapter.listSkills).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        config: expect.objectContaining({
          workcellRuntimeSkills: expect.any(Array),
        }),
      }),
    );
  }, 10_000);
  ...
  it("skips runtime materialization when syncing Claude skills", async () => {
    ...
    const res = await requestApp(await createApp(), (baseUrl) => request(baseUrl)
      ...
      .send({ desiredSkills: ["workcell/workcell/workcell"] }));
    ...
    expect(mockAgentService.update).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        adapterConfig: expect.objectContaining({
          workcellSkillSync: expect.objectContaining({
            desiredSkills: ["workcell/workcell/workcell"],
          }),
        }),
      }),
  ...
  it("materializes the bundled Orchestrator instruction set for default Orchestrator agents", async () => {
    const res = await requestApp(await createApp(), (baseUrl) => request(baseUrl)
      .post("/api/companies/company-1/agents")
      .send({
        name: "Orchestrator",
    ...
    expect(mockAgentInstructionsService.materializeManagedBundle).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        "AGENTS.md": expect.stringContaining("You are the Orchestrator."),
        "HEARTBEAT.md": expect.stringContaining("Orchestrator Heartbeat Checklist"),
        "SOUL.md": expect.stringContaining("Orchestrator Persona"),
  ...
  it("materializes the bundled default instruction set for non-Orchestrator agents with no prompt template", async () => {
    const res = await requestApp(await createApp(), (baseUrl) => request(baseUrl)
      .post("/api/companies/company-1/agents")
      .send({
        name: "Engineer",
