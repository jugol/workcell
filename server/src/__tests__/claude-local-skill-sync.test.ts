...
import {
  listClaudeSkills,
  syncClaudeSkills,
} from "@workcell/adapter-claude-local/server";
...
describe("claude local skill sync", () => {
  ...
  it("defaults to mounting all built-in Workcell skills when no explicit selection exists", async () => {
    ...
    expect(snapshot.entries.find((entry) => entry.key === workcellKey)?.required).toBe(true);
  ...
  it("normalizes legacy flat Workcell skill refs to canonical keys", async () => {
    const snapshot = await listClaudeSkills({
      agentId: "agent-3",
      companyId: "company-1",
      adapterType: "claude_local",
  ...
  it("shows host-level user-installed Claude skills as read-only external entries", async () => {
    const home = await makeTempDir("workcell-claude-user-skills-");
