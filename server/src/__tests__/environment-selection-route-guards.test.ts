...
vi.mock("../services/index.js", () => ({
  ...
  agentService: () => ({
    getById: vi.fn(),
    // WC-5 default-QA-policy injection in the create/children routes calls
    // agentsSvc.list(companyId) to resolve an eligible reviewer; mock it so this
...
describe.sequential("execution environment route guards", () => {
  ...
  it("requires a project for a user/board-created top-level issue", async () => {
    const app = createIssueApp();
    const res = await request(app)
      .post("/api/companies/company-1/issues")
      .send({ title: "Projectless Issue" });
    expect(res.status).toBe(422);
    expect(res.body.error).toContain("project is required");
    expect(mockIssueService.create).not.toHaveBeenCalled();
  });

  it("accepts a top-level issue that has a project", async () => {
    mockIssueService.create.mockResolvedValue({
      id: "issue-1", companyId: "company-1", title: "Has Project", status: "todo", identifier: "PAPA-1",
    });
    const app = createIssueApp();
    const res = await request(app)
      .post("/api/companies/company-1/issues")
      .send({ title: "Has Project", projectId: "22222222-2222-4222-8222-222222222222" });
    expect(res.status).not.toBe(422);
    expect(mockIssueService.create).toHaveBeenCalled();
  });

  it("exempts a child issue without a project (it inherits the parent's project)", async () => {
    mockIssueService.create.mockResolvedValue({
      id: "issue-2", companyId: "company-1", title: "Child", status: "todo", identifier: "PAPA-2",
    });
    const app = createIssueApp();
    const res = await request(app)
      .post("/api/companies/company-1/issues")
      .send({ title: "Child", parentId: "33333333-3333-4333-8333-333333333333" });
    expect(res.status).not.toBe(422);
    expect(mockIssueService.create).toHaveBeenCalled();
  });
});
