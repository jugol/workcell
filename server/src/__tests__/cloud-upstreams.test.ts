...
import { companies, cloudUpstreamConnections, cloudUpstreamRuns, companySkills, createDb } from "@workcell/db";
...
describe("cloud upstream credential storage", () => {
  const previousMasterKey = process.env.WORKCELL_SECRETS_MASTER_KEY;

  afterEach(() => {
    if (previousMasterKey === undefined) {
      delete process.env.WORKCELL_SECRETS_MASTER_KEY;
    } else {
      process.env.WORKCELL_SECRETS_MASTER_KEY = previousMasterKey;
    }
  });

  it("stores new credentials as encrypted envelopes and preserves legacy plaintext reads", async () => {
    process.env.WORKCELL_SECRETS_MASTER_KEY = "12345678901234567890123456789012";
    const sealed = await sealCloudUpstreamCredential("cloud-access-token");

    expect(sealed).toMatch(/^workcell-cloud-credential:/);
...
describeEmbeddedPostgres("cloud upstream persistence", () => {
  ...
  beforeAll(async () => {
    process.env.WORKCELL_SECRETS_MASTER_KEY = "12345678901234567890123456789012";
    tempDb = await startEmbeddedPostgresTestDatabase("workcell-cloud-upstreams-");
  ...
  it("encrypts stored upstream credentials while keeping connection flows usable", async () => {
    ...
    vi.spyOn(globalThis, "fetch").mockImplementation(async (input, init) => {
      ...
      if (url.startsWith("https://cloud.example.test/.well-known/workcell-upstream")) {
        return jsonResponse({
          product: "Workcell Cloud",
          stack: {
            id: "stack-1",
      ...
      if (url === tokenUrl && init?.method === "POST") {
        ...
        expect(payload.codeVerifier).not.toContain("workcell-cloud-credential:");
        return jsonResponse({
    ...
    expect(row.privateKeyPem).toMatch(/^workcell-cloud-credential:/);
    expect(row.privateKeyPem).not.toContain("BEGIN PRIVATE KEY");
    expect(row.accessToken).toMatch(/^workcell-cloud-credential:/);
