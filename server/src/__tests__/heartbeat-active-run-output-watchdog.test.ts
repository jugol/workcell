...
describeEmbeddedPostgres("active-run output watchdog", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("workcell-active-run-output-watchdog-");
  ...
  async function seedRunningRun(opts: {
    ...
    withOutput?: boolean;
    logChunk?: string;
    sourceStatus?: "in_progress" | "in_review" | "done" | "cancelled";
    sourceOriginKind?: string;
    sameRunTerminalEvidence?: "activity" | "comment";
    outputAfterEvidence?: boolean;
  }) {
    ...
    const lastOutputAt = opts.outputAfterEvidence
      ? new Date(terminalEvidenceAt.getTime() + 5 * 60 * 1000)
      : opts.withOutput
  ...
  it("folds an in_review handoff with same-run durable evidence instead of creating watchdog work", async () => {
    // The run itself moved the source issue to in_review (work delivered, QA
    // owns the next step) and then never finalized (orphaned child after a
    // server restart / lingering CLI). The scan must fold the run instead of
    // opening an endless chain of "Review silent active run" evaluations.
    const now = new Date("2026-04-22T20:00:00.000Z");
    ...
    expect(run?.resultJson).toMatchObject({
      sourceResolvedWatchdogFold: { sourceIssueId: issueId, sourceIssueStatus: "in_review" },
    });
  });

  it("folds an in_review handoff even when the run kept emitting output after the handoff", async () => {
    // Real-world ordering (LOR-53 regression): the agent moves the source
    // issue to in_review and THEN prints its final summary / sanity checks,
    // so the handoff activity predates lastOutputAt. Gating the fold evidence
    // on the silence start rejected every real handoff and the watchdog
    // re-filed "Review silent active run" evaluations forever.
    const now = new Date("2026-04-22T20:00:00.000Z");
    const { companyId, issueId, runId } = await seedRunningRun({
      now,
      ageMs: ACTIVE_RUN_OUTPUT_CRITICAL_THRESHOLD_MS + 60_000,
  ...
  it("does not re-file an evaluation after a dismissed_false_positive until new output arrives", async () => {
    const now = new Date("2026-04-22T20:00:00.000Z");
    ...
    // The reviewer closes the evaluation as a false positive and records the
    // watchdog decision, but leaves the run untouched (no board cancellation).
    const [evaluation] = await db
      .select()
      .from(issues)
      .where(and(eq(issues.companyId, companyId), eq(issues.originKind, "stale_active_run_evaluation")));
    await db.update(issues).set({ status: "done" }).where(eq(issues.id, evaluation.id));
    ...
    await db.insert(heartbeatRunWatchdogDecisions).values({
      ...
      reason: "Reviewed; this silence is expected.",
      createdAt: decisionAt,
    });

    // Same silence, already judged: the scan must not open a new evaluation.
    const second = await heartbeat.scanSilentActiveRuns({
      now: new Date(decisionAt.getTime() + 10 * 60_000),
      companyId,
    });
    ...
    // New output after the dismissal re-arms the watchdog for the next silence.
    await db
      .update(heartbeatRuns)
      .set({
        lastOutputAt: new Date(decisionAt.getTime() + 5 * 60_000),
        lastOutputSeq: 9,
        lastOutputStream: "stdout",
      })
      .where(eq(heartbeatRuns.id, runId));
    const third = await heartbeat.scanSilentActiveRuns({
      now: new Date(decisionAt.getTime() + ACTIVE_RUN_OUTPUT_SUSPICION_THRESHOLD_MS + 10 * 60_000),
      companyId,
    });
    expect(third).toMatchObject({ created: 1, suppressed: 0 });
  });
