...
const multiMigrationPluginKey = "workcell.dbfixture";
const llmWikiPluginKey = "workcell.plugin-llm-wiki";
...
describe("buildPluginWorkerEnv", () => {
  ...
  it("passes only model provider keys through to environment driver plugins", () => {
    ...
    expect(env).toEqual({
      ...
      WORKCELL_DEPLOYMENT_EXPOSURE: "public",
      ANTHROPIC_API_KEY: "anthropic-token",
      OPENAI_API_KEY: "openai-token",
    });
  });
...
describeEmbeddedPostgres("plugin database namespaces", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("workcell-plugin-db-");
    db = createDb(tempDb.connectionString);
  }, 20_000);

  afterEach(async () => {
    for (const pluginKey of ["workcell.dbtest", "workcell.escape", "workcell.refresh", multiMigrationPluginKey, llmWikiPluginKey]) {
  ...
  async function createPluginPackage(manifest: WorkcellPluginManifestV1, migrationSql: string) {
    const packageRoot = await mkdtemp(path.join(os.tmpdir(), "workcell-plugin-package-"));
  ...
  function llmWikiManifest(): WorkcellPluginManifestV1 {
    return {
      id: llmWikiPluginKey,
      apiVersion: 1,
      version: "0.1.0",
  ...
  async function createInstallablePluginPackage(
    pluginManifest: WorkcellPluginManifestV1,
    migrationSql: string,
  ) {
    ...
    await writeFile(
      ...
      JSON.stringify({
        ...
        type: "module",
        workcellPlugin: { manifest: "./manifest.js" },
      }),
      "utf8",
    );
    await writeFile(
  ...
  async function installPluginRecord(manifest: WorkcellPluginManifestV1) {
  ...
  function manifest(pluginKey = "workcell.dbtest"): WorkcellPluginManifestV1 {
    return {
      id: pluginKey,
      apiVersion: 1,
      version: "1.0.0",
  ...
  it("applies the bundled LLM Wiki migrations through the production validator", async () => {
    ...
    expect(constraints).not.toContain("workcell_distillation_cursor_company_id_wiki_id_source_sco_key");
    expect(constraints).not.toContain("workcell_distillation_work_i_company_id_wiki_id_idempotenc_key");
    expect(constraints).not.toContain("workcell_page_bindings_company_id_wiki_id_page_path_key");
    expect(uniqueColumnSets).not.toContain("wiki_pages:company_id,wiki_id,path");
    expect(uniqueColumnSets).not.toContain("workcell_distillation_cursors:company_id,wiki_id,source_scope,scope_key,source_kind");
    expect(uniqueColumnSets).not.toContain("workcell_distillation_work_items:company_id,wiki_id,idempotency_key");
    expect(uniqueColumnSets).not.toContain("workcell_page_bindings:company_id,wiki_id,page_path");
  });
  ...
  it("records a failed migration when SQL escapes the plugin namespace", async () => {
    const pluginManifest = manifest("workcell.escape");
    const packageRoot = await createPluginPackage(
      pluginManifest,
  ...
  it("refreshes persisted manifests from disk before activation", async () => {
    const staleManifest = manifest("workcell.refresh");
    const refreshedManifest: WorkcellPluginManifestV1 = {
      ...staleManifest,
      database: {
