...
import { agents, companies, createDb, environmentLeases, environments, heartbeatRuns } from "@workcell/db";
import {
...
describeEmbeddedPostgres("environmentService leases", () => {
  ...
  it("WC-165: rejects a second concurrent active lease for the same provider sandbox", async () => {
    const { companyId, environmentId } = await seedEnvironment();
    const providerLeaseId = "sandbox-shared-1";

    // Two runs both try to reuse (resume) the same provider sandbox lease at once.
    const results = await Promise.allSettled([
      svc.acquireLease({ companyId, environmentId, provider: "fake", providerLeaseId, leasePolicy: "reuse_by_environment" }),
      svc.acquireLease({ companyId, environmentId, provider: "fake", providerLeaseId, leasePolicy: "reuse_by_environment" }),
    ]);
    ...
    // Exactly one ACTIVE lease exists for the shared sandbox — never two.
  ...
  it("WC-165: allows re-leasing the same provider sandbox after the prior lease is released", async () => {
    const { companyId, environmentId } = await seedEnvironment();
    const providerLeaseId = "sandbox-reuse-1";

    const lease1 = await svc.acquireLease({ companyId, environmentId, provider: "fake", providerLeaseId, leasePolicy: "reuse_by_environment" });
    await svc.releaseLease(lease1.id, "released");

    // Sequential reuse (the prior run freed the sandbox) is allowed — the unique index
    // only forbids two ACTIVE leases at once.
    const lease2 = await svc.acquireLease({ companyId, environmentId, provider: "fake", providerLeaseId, leasePolicy: "reuse_by_environment" });
