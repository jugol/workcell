...
describe("worktree config repair", () => {
  it("repairs legacy repo-local worktree config and env files into an isolated instance", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-worktree-repair-"));
    const worktreeRoot = path.join(tempRoot, "PAP-884-ai-commits-component");
    const workcellDir = path.join(worktreeRoot, ".workcell");
    const configPath = path.join(workcellDir, "config.json");
    const envPath = path.join(workcellDir, ".env");
    const sharedRoot = path.join(tempRoot, ".workcell", "instances", "default");
    const isolatedHome = path.join(tempRoot, ".workcell-worktrees");
    ...
    await fs.writeFile(
      envPath,
      [
        "# Workcell environment variables",
        "WORKCELL_IN_WORKTREE=true",
        "WORKCELL_WORKTREE_NAME=PAP-884-ai-commits-component",
        "WORKCELL_AGENT_JWT_SECRET=shared-secret",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.env.WORKCELL_WORKTREE_NAME = "PAP-884-ai-commits-component";
    process.env.WORKCELL_WORKTREES_DIR = isolatedHome;
    delete process.env.WORKCELL_HOME;
    delete process.env.WORKCELL_INSTANCE_ID;
    delete process.env.WORKCELL_CONFIG;
    delete process.env.WORKCELL_CONTEXT;
    ...
    expect(repairedEnv).toContain(`WORKCELL_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).toContain('WORKCELL_INSTANCE_ID="pap-884-ai-commits-component"');
    expect(repairedEnv).toContain(`WORKCELL_CONFIG=${JSON.stringify(await fs.realpath(configPath))}`);
    expect(repairedEnv).toContain(`WORKCELL_CONTEXT=${JSON.stringify(path.join(isolatedHome, "context.json"))}`);
    expect(repairedEnv).toContain('WORKCELL_AGENT_JWT_SECRET="shared-secret"');
    expect(process.env.WORKCELL_HOME).toBe(isolatedHome);
    expect(process.env.WORKCELL_INSTANCE_ID).toBe("pap-884-ai-commits-component");
  });

  it("avoids sibling worktree ports when repairing legacy configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-worktree-repair-ports-"));
    ...
    await fs.writeFile(
      ...
      [
        "# Workcell environment variables",
        "WORKCELL_IN_WORKTREE=true",
        "WORKCELL_WORKTREE_NAME=PAP-880-thumbs-capture-for-evals-feature",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.WORKCELL_IN_WORKTREE = "true";
    process.env.WORKCELL_WORKTREE_NAME = "PAP-880-thumbs-capture-for-evals-feature";
    process.env.WORKCELL_WORKTREES_DIR = isolatedHome;
    delete process.env.WORKCELL_HOME;
    delete process.env.WORKCELL_INSTANCE_ID;
    delete process.env.WORKCELL_CONFIG;
  ...
  it("ignores stale migrated env paths when the dev runner resolved the local config", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-worktree-migrated-env-"));
    const worktreeRoot = path.join(tempRoot, "PAP-9940-what-can-we-learn");
    const workcellDir = path.join(worktreeRoot, ".workcell");
    const configPath = path.join(workcellDir, "config.json");
    const envPath = path.join(workcellDir, ".env");
    const oldHome = "/old/home/.workcell-worktrees";
    const isolatedHome = path.join(tempRoot, ".workcell-worktrees");
    ...
    await fs.writeFile(
      ...
      [
        ...
        "WORKCELL_INSTANCE_ID=pap-9940-what-can-we-learn",
        "WORKCELL_CONFIG=/old/home/workcell/.workcell/worktrees/PAP-9940-what-can-we-learn/.workcell/config.json",
        "WORKCELL_CONTEXT=/old/home/.workcell-worktrees/context.json",
        "WORKCELL_IN_WORKTREE=true",
        "WORKCELL_WORKTREE_NAME=PAP-9940-what-can-we-learn",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.WORKCELL_IN_WORKTREE = "true";
    process.env.WORKCELL_CONFIG = configPath;
    process.env.WORKCELL_WORKTREES_DIR = isolatedHome;
    delete process.env.WORKCELL_HOME;
    delete process.env.WORKCELL_INSTANCE_ID;
    delete process.env.WORKCELL_CONTEXT;
    ...
    expect(repairedConfig.secrets.localEncrypted.keyFilePath).toBe(path.join(instanceRoot, "secrets", "master.key"));
    expect(repairedEnv).toContain(`WORKCELL_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).toContain(`WORKCELL_CONFIG=${JSON.stringify(configPath)}`);
  ...
  it("does not persist transient runtime home overrides over repo-local worktree env", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-worktree-runtime-override-"));
    const isolatedHome = path.join(tempRoot, ".workcell-worktrees");
    ...
    await fs.writeFile(
      ...
      [
        "# Workcell environment variables",
        `WORKCELL_HOME=${JSON.stringify(isolatedHome)}`,
        `WORKCELL_INSTANCE_ID=${JSON.stringify(instanceId)}`,
        `WORKCELL_CONFIG=${JSON.stringify(configPath)}`,
        `WORKCELL_CONTEXT=${JSON.stringify(path.join(isolatedHome, "context.json"))}`,
        'WORKCELL_IN_WORKTREE="true"',
        'WORKCELL_WORKTREE_NAME="PAP-989-multi-user-implementation-using-plan-from-pap-958"',
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.WORKCELL_IN_WORKTREE = "true";
    process.env.WORKCELL_WORKTREE_NAME = "PAP-989-multi-user-implementation-using-plan-from-pap-958";
    process.env.WORKCELL_HOME = transientHome;
    ...
    expect(repairedEnv).toContain(`WORKCELL_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).not.toContain(`WORKCELL_HOME=${JSON.stringify(transientHome)}`);
    expect(process.env.WORKCELL_HOME).toBe(isolatedHome);
  });

  it("rebalances duplicate ports for already isolated worktree configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-worktree-rebalance-"));
    const isolatedHome = path.join(tempRoot, ".workcell-worktrees");
    const repoWorktreesRoot = path.join(tempRoot, "repo", ".workcell", "worktrees");
    ...
    const workcellDir = path.join(currentWorktreeRoot, ".workcell");
    const configPath = path.join(workcellDir, "config.json");
    const envPath = path.join(workcellDir, ".env");
    const currentInstanceRoot = path.join(isolatedHome, "instances", "pap-884-ai-commits-component");
    const siblingConfigPath = path.join(siblingWorktreeRoot, ".workcell", "config.json");
  ...
  it("persists runtime-selected worktree ports back into explicit-port auth URLs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-worktree-ports-"));
    ...
    process.chdir(worktreeRoot);
    process.env.WORKCELL_IN_WORKTREE = "true";
    process.env.WORKCELL_WORKTREE_NAME = "PAP-878-create-a-mine-tab-in-inbox";
    process.env.WORKCELL_HOME = isolatedHome;
    process.env.WORKCELL_INSTANCE_ID = "pap-878-create-a-mine-tab-in-inbox";
    process.env.WORKCELL_CONFIG = configPath;
  ...
  it("does not rewrite no-port public auth URLs when persisting runtime-selected ports", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "workcell-worktree-public-ports-"));
    ...
    await fs.writeFile(
      ...
      JSON.stringify(
        {
          ...buildLegacyConfig(instanceRoot, "https://workcell.example"),
          database: {
            mode: "embedded-postgres",
            embeddedPostgresDataDir: path.join(instanceRoot, "db"),
            embeddedPostgresPort: 54331,
    ...
    process.chdir(worktreeRoot);
    process.env.WORKCELL_IN_WORKTREE = "true";
    process.env.WORKCELL_WORKTREE_NAME = "PAP-125-public-base-url";
    process.env.WORKCELL_HOME = isolatedHome;
    process.env.WORKCELL_INSTANCE_ID = "pap-125-public-base-url";
    process.env.WORKCELL_CONFIG = configPath;
    delete process.env.PORT;
    delete process.env.DATABASE_URL;
    ...
    expect(writtenConfig.server.port).toBe(3103);
    expect(writtenConfig.database.embeddedPostgresPort).toBe(54335);
    expect(writtenConfig.auth.publicBaseUrl).toBe("https://workcell.example");
  });
  ...
  it("does not rewrite the in-memory config when auth URL has no explicit port", () => {
    const { config, changed } = applyRuntimePortSelectionToConfig(
      buildLegacyConfig("/tmp/shared", "https://workcell.example"),
      {
        serverPort: 3104,
        databasePort: 54340,
        allowServerPortWrite: false,
    ...
    expect(config.server.port).toBe(3100);
    expect(config.database.embeddedPostgresPort).toBe(54340);
    expect(config.auth.publicBaseUrl).toBe("https://workcell.example");
  });
});
