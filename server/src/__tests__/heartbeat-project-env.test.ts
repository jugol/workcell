...
import { buildSkillMentionHref } from "@workcell/shared";
import {
...
describe("resolveExecutionRunAdapterConfig", () => {
  ...
  it("drops Workcell runtime-owned env before resolving agent, project, and routine overlays", async () => {
    ...
    const result = await resolveExecutionRunAdapterConfig({
      ...
      executionRunConfig: {
        env: {
          WORKCELL_API_KEY: { type: "secret_ref", secretId: "secret-api-key", version: "latest" },
          WORKCELL_AGENT_ID: "spoofed-agent",
          AGENT_ONLY: "agent-only",
        },
      },
      projectEnv: {
        WORKCELL_API_KEY: "project-api-key",
        WORKCELL_COMPANY_ID: "spoofed-company",
        PROJECT_ONLY: "project-only",
      },
      routineEnv: {
        WORKCELL_API_KEY: "routine-api-key",
        WORKCELL_RUN_ID: "spoofed-run",
        ROUTINE_ONLY: "routine-only",
      },
      routineId: "routine-1",
      secretsSvc: {
    ...
    expect(JSON.stringify(result.resolvedConfig.env)).not.toContain("WORKCELL_");
  });
