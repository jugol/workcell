...
const ORIGINAL_WORKCELL_RUNTIME_API_CANDIDATES_JSON = process.env.WORKCELL_RUNTIME_API_CANDIDATES_JSON;
...
function buildTestConfig(overrides: Record<string, unknown> = {}) {
  return {
    ...
    databaseUrl: "postgres://workcell:workcell@127.0.0.1:5432/workcell",
    embeddedPostgresDataDir: "/tmp/workcell-test-db",
    embeddedPostgresPort: 54329,
    databaseBackupEnabled: false,
    databaseBackupIntervalMinutes: 60,
    databaseBackupRetentionDays: 30,
    databaseBackupDir: "/tmp/workcell-test-backups",
    serveUi: false,
    uiDevMiddleware: false,
    secretsProvider: "local_encrypted",
    secretsStrictMode: false,
    secretsMasterKeyFilePath: "/tmp/workcell-master.key",
    storageProvider: "local_disk",
    storageLocalDiskBaseDir: "/tmp/workcell-storage",
    storageS3Bucket: "workcell-test",
...
vi.mock("../services/index.js", () => ({
  ...
  reapStaleDeliberationRuns: vi.fn(async () => ({ reaped: 0 })),
  sweepLeakedWorktrees: vi.fn(async () => ({ swept: 0 })),
  pairAutoRunTicker: vi.fn(() => ({ tick: vi.fn(async () => ({ groupsRun: 0, errors: [] })) })),
  pairRoundOrchestrator: vi.fn(() => ({
    runRound: vi.fn(async () => ({ skipped: true, group: null })),
    runUntilStop: vi.fn(async () => []),
  })),
  buildDefaultPairTurnExecutor: vi.fn(() => vi.fn()),
...
describe("startServer feedback export wiring", () => {
  ...
  it("refuses authenticated public startup when DATABASE_URL is not a postgres URL", async () => {
    loadConfigMock.mockReturnValue(buildTestConfig({
      ...
      authBaseUrlMode: "explicit",
      authPublicBaseUrl: "https://tenant.example.com",
      databaseUrl: "secret://workcell-cloud/stacks/alpha/database/runtime-url",
    }));
...
describe("startServer WORKCELL_API_URL handling", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    loadConfigMock.mockReturnValue(buildTestConfig());
    process.env.BETTER_AUTH_SECRET = "test-secret";
  ...
  afterEach(() => {
    ...
    if (ORIGINAL_WORKCELL_RUNTIME_API_CANDIDATES_JSON === undefined) {
      delete process.env.WORKCELL_RUNTIME_API_CANDIDATES_JSON;
    } else {
      process.env.WORKCELL_RUNTIME_API_CANDIDATES_JSON = ORIGINAL_WORKCELL_RUNTIME_API_CANDIDATES_JSON;
    }
  ...
  it("uses the externally set WORKCELL_API_URL when provided", async () => {
    process.env.WORKCELL_API_URL = "http://custom-api:3100";

    const started = await startServer();

    expect(started.apiUrl).toBe("http://custom-api:3100");
    expect(process.env.WORKCELL_API_URL).toBe("http://custom-api:3100");
    expect(JSON.parse(process.env.WORKCELL_RUNTIME_API_CANDIDATES_JSON ?? "[]")).toEqual(
      expect.arrayContaining(["http://custom-api:3100"]),
    );
    expect(JSON.parse(process.env.WORKCELL_RUNTIME_API_CANDIDATES_JSON ?? "[]")[0]).toBe("http://custom-api:3100");
  });

  it("falls back to host-based URL when WORKCELL_API_URL is not set", async () => {
    const started = await startServer();

    expect(started.apiUrl).toBe("http://127.0.0.1:3210");
    expect(process.env.WORKCELL_API_URL).toBe("http://127.0.0.1:3210");
  });
  ...
  it("rewrites explicit-port auth public URLs when detect-port selects a new port", async () => {
    ...
    expect(process.env.WORKCELL_RUNTIME_API_URL).toBe("http://my-host.ts.net:3110");
  });
  ...
  it("keeps no-port auth public URLs stable when detect-port selects a new port", async () => {
    loadConfigMock.mockReturnValueOnce(buildTestConfig({
      port: 3100,
      authBaseUrlMode: "explicit",
      authPublicBaseUrl: "https://workcell.example",
    }));
    detectPortMock.mockResolvedValueOnce(3110);
    ...
    expect(started.listenPort).toBe(3110);
    expect(started.apiUrl).toBe("https://workcell.example");
    expect(process.env.WORKCELL_RUNTIME_API_URL).toBe("https://workcell.example");
  });
});
