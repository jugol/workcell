...
describeEmbedded("PAP-9522 QA: routine secrets end-to-end", () => {
  ...
  const secretsTmpDir = path.join(os.tmpdir(), `workcell-qa-routine-secrets-${randomUUID()}`);
  ...
  beforeAll(async () => {
    ...
    tempDb = await startEmbeddedPostgresTestDatabase("workcell-qa-routine-secrets-");
  ...
  afterAll(async () => {
    ...
    if (previousKeyFile === undefined) delete process.env.WORKCELL_SECRETS_MASTER_KEY_FILE;
    else process.env.WORKCELL_SECRETS_MASTER_KEY_FILE = previousKeyFile;
