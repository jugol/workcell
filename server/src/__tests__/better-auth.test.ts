...
const ORIGINAL_INSTANCE_ID = process.env.WORKCELL_INSTANCE_ID;
const ORIGINAL_PUBLIC_URL = process.env.WORKCELL_PUBLIC_URL;

afterEach(() => {
  if (ORIGINAL_INSTANCE_ID === undefined) delete process.env.WORKCELL_INSTANCE_ID;
  else process.env.WORKCELL_INSTANCE_ID = ORIGINAL_INSTANCE_ID;
  if (ORIGINAL_PUBLIC_URL === undefined) delete process.env.WORKCELL_PUBLIC_URL;
  else process.env.WORKCELL_PUBLIC_URL = ORIGINAL_PUBLIC_URL;
});

describe("Better Auth cookie scoping", () => {
  it("derives an instance-scoped cookie prefix", () => {
    expect(deriveAuthCookiePrefix("default")).toBe("workcell-default");
    expect(deriveAuthCookiePrefix("PAP-1601-worktree")).toBe("workcell-PAP-1601-worktree");
  });

  it("uses WORKCELL_INSTANCE_ID for the Better Auth cookie prefix", () => {
    process.env.WORKCELL_INSTANCE_ID = "sat-worktree";

    const advanced = buildBetterAuthAdvancedOptions({ disableSecureCookies: false });

    expect(advanced).toEqual({
      cookiePrefix: "workcell-sat-worktree",
    });
    expect(getCookies({ advanced } as BetterAuthOptions).sessionToken.name).toMatch(
      /workcell-sat-worktree\.session_token$/,
    );
  });

  it("keeps local http auth cookies non-secure while preserving the scoped prefix", () => {
    process.env.WORKCELL_INSTANCE_ID = "pap-worktree";

    expect(buildBetterAuthAdvancedOptions({ disableSecureCookies: true })).toEqual({
      cookiePrefix: "workcell-pap-worktree",
      useSecureCookies: false,
    });
    expect(getCookies({
      advanced: buildBetterAuthAdvancedOptions({ disableSecureCookies: true }),
    } as BetterAuthOptions).sessionToken.name).toBe("workcell-pap-worktree.session_token");
  });
  ...
  it("uses an explicit public URL when deciding whether secure cookies are required", () => {
    expect(shouldDisableSecureAuthCookies({
      ...
      authBaseUrlMode: "auto",
      authPublicBaseUrl: undefined,
      publicUrl: "https://workcell.example.test",
    })).toBe(false);
    ...
    expect(shouldDisableSecureAuthCookies({
      ...
      authPublicBaseUrl: "http://workcell.local.test:3100",
      publicUrl: undefined,
    })).toBe(true);
  });

  it("disables secure cookies when no canonical public auth URL is configured", () => {
    delete process.env.WORKCELL_PUBLIC_URL;
  ...
  it("derives secure cookie behavior from the configured public auth URL", () => {
    ...
    expect(shouldDisableSecureAuthCookies({
      deploymentMode: "authenticated",
      authBaseUrlMode: "explicit",
      authPublicBaseUrl: "http://workcell-dev:46259",
    } as Parameters<typeof shouldDisableSecureAuthCookies>[0])).toBe(true);
    expect(shouldDisableSecureAuthCookies({
      deploymentMode: "authenticated",
      authBaseUrlMode: "explicit",
      authPublicBaseUrl: "https://workcell.example.test",
  ...
  it("uses the caller-resolved public URL for cookie security", () => {
    process.env.WORKCELL_PUBLIC_URL = "https://ignored.example.test";

    expect(shouldDisableSecureAuthCookies({
      deploymentMode: "authenticated",
      authBaseUrlMode: "explicit",
      authPublicBaseUrl: "https://workcell.example.test",
      publicUrl: "http://workcell-dev:46259",
    } as Parameters<typeof shouldDisableSecureAuthCookies>[0])).toBe(true);
  });
