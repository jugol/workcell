...
const mockInstanceSettings = vi.hoisted(() => ({
  getExperimental: vi.fn(async () => ({ autonomousMode: false })),
}));

// Team autonomy: the request_confirmation auto-accept now reads
// company.autoApproveConfirmations before falling back to the instance
// experimental autonomousMode flag.
...
vi.mock("@workcell/shared/telemetry", () => ({
...
function registerModuleMocks() {
  ...
  // routes/issues.ts imports instanceSettingsService directly from this module
  // (not via ../services/index.js); the WC-176 request_confirmation auto-accept
  // reads getExperimental().autonomousMode.
...
describe.sequential("issue thread interaction routes", () => {
  beforeEach(() => {
    ...
    registerModuleMocks();
    vi.clearAllMocks();
    mockInstanceSettings.getExperimental.mockResolvedValue({ autonomousMode: false });
  ...
  it("WC-176: auto-accepts a request_confirmation at create time when autonomous mode is on", async () => {
    mockInstanceSettings.getExperimental.mockResolvedValue({ autonomousMode: true });
    const rc = (status: string) => ({
      ...
      sourceCommentId: null,
      sourceRunId: "run-1",
      payload: { version: 1, prompt: "Confirm the plan" },
      result: status === "accepted" ? { version: 1, outcome: "accepted" } : null,
      createdAt: "2026-04-20T12:00:00.000Z",
      updatedAt: "2026-04-20T12:00:00.000Z",
    });
    mockInteractionService.create.mockResolvedValue(rc("pending"));
    mockInteractionService.acceptInteraction.mockResolvedValue({
      interaction: rc("accepted"),
    ...
    const res = await request(await createApp())
      .post("/api/issues/aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa/interactions")
      .send({ kind: "request_confirmation", payload: { version: 1, prompt: "Confirm the plan" } });

    expect(res.status).toBe(201);
    expect(res.body.status).toBe("accepted");
    expect(mockInteractionService.acceptInteraction).toHaveBeenCalledTimes(1);
    // continuation re-wakes the creator agent so the workflow keeps moving
    expect(mockHeartbeatService.wakeup).toHaveBeenCalledWith(ASSIGNEE_AGENT_ID, expect.anything());
    // the activity log records the auto-approval and its (back-compat) source
    expect(mockLogActivity).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({
        action: "issue.thread_interaction_created",
        details: expect.objectContaining({
          autoApproved: true,
          autoApproveSource: "instance_experimental",
        }),
      }),
    );
  });

  it("team autonomy: auto-accepts a request_confirmation when company.autoApproveConfirmations is on", async () => {
    // The instance experimental flag stays OFF — the company setting alone decides.
    mockInstanceSettings.getExperimental.mockResolvedValue({ autonomousMode: false });
    mockCompanyService.getById.mockResolvedValue({
      id: "company-1",
      attachmentMaxBytes: 10 * 1024 * 1024,
    ...
    expect(mockInteractionService.acceptInteraction).toHaveBeenCalledTimes(1);
    expect(mockHeartbeatService.wakeup).toHaveBeenCalledWith(ASSIGNEE_AGENT_ID, expect.anything());
    // the activity log marks the team-autonomy auto-approval for UI display
    expect(mockLogActivity).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({
        action: "issue.thread_interaction_created",
        details: expect.objectContaining({
          interactionStatus: "accepted",
          autoApproved: true,
          autoApproveSource: "team_autonomy",
        }),
      }),
    );
  });

  it("team autonomy: leaves a request_confirmation pending when both the company setting and the experimental flag are off", async () => {
    mockInstanceSettings.getExperimental.mockResolvedValue({ autonomousMode: false });
    mockCompanyService.getById.mockResolvedValue({
      id: "company-1",
      attachmentMaxBytes: 10 * 1024 * 1024,
    ...
    // no auto-approval marker in the created activity log
    expect(mockLogActivity).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({
        action: "issue.thread_interaction_created",
        details: expect.not.objectContaining({ autoApproved: true }),
      }),
    );
  });

  it("WC-176: leaves a request_confirmation pending when autonomous mode is off", async () => {
    mockInstanceSettings.getExperimental.mockResolvedValue({ autonomousMode: false });
    mockInteractionService.create.mockResolvedValue({
      id: "rc-2",
      companyId: "company-1",
  ...
  // Semantics change: an explicit board REJECTION is actionable feedback the
  // assignee must address, so it wakes the assignee even under
  // wake_assignee_on_accept (only expiry/supersede stays quiet). Previously a
  // rejected plan confirmation woke nobody and the board's feedback was lost.
  it("emits a continuation wake when request confirmations are rejected (feedback must land)", async () => {
  ...
  it("emits a continuation wake for rejected suggested tasks too (same feedback rule)", async () => {
    mockInteractionService.rejectInteraction.mockResolvedValueOnce({
      id: "interaction-5",
      companyId: "company-1",
      issueId: "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa",
