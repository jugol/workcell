...
import type { Issue, WorkcellPluginManifestV1 } from "@workcell/shared";
import { createTestHarness } from "../../../packages/plugins/sdk/src/testing.js";

function manifest(capabilities: WorkcellPluginManifestV1["capabilities"]): WorkcellPluginManifestV1 {
  return {
    id: "workcell.test-orchestration",
    apiVersion: 1,
    version: "0.1.0",
...
describe("plugin SDK orchestration contract", () => {
  it("supports expanded issue create fields and relation helpers", async () => {
    ...
    expect(created.originKind).toBe("plugin:workcell.test-orchestration");
  ...
  it("enforces plugin origin namespaces in the test harness", async () => {
    ...
    const created = await harness.ctx.issues.create({
      companyId,
      title: "Generated issue",
      originKind: "plugin:workcell.test-orchestration:feature",
    });

    expect(created.originKind).toBe("plugin:workcell.test-orchestration:feature");
    await expect(
    ...
    ).rejects.toThrow("Plugin may only use originKind values under plugin:workcell.test-orchestration");
    await expect(
  ...
  it("supports generic plugin operation issue visibility in the test harness", async () => {
    ...
    expect(created.originKind).toBe("plugin:workcell.test-orchestration:operation");
