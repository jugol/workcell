...
function createDevServerStatusFile(payload: unknown) {
  const dir = mkdtempSync(path.join(os.tmpdir(), "workcell-health-dev-server-"));
  tempDirs.push(dir);
...
describe("GET /health dev-server supervisor access", () => {
  it("exposes dev-server metadata to the supervising dev runner in authenticated mode", async () => {
    const previousFile = process.env.WORKCELL_DEV_SERVER_STATUS_FILE;
    const previousToken = process.env.WORKCELL_DEV_SERVER_STATUS_TOKEN;
    process.env.WORKCELL_DEV_SERVER_STATUS_FILE = createDevServerStatusFile({
      dirty: true,
    ...
    process.env.WORKCELL_DEV_SERVER_STATUS_TOKEN = "dev-runner-token";
    ...
    try {
      ...
      const res = await request(app)
        .get("/health")
        .set("X-Workcell-Dev-Server-Status-Token", "dev-runner-token");
      ...
      if (previousFile === undefined) {
        delete process.env.WORKCELL_DEV_SERVER_STATUS_FILE;
      } else {
        process.env.WORKCELL_DEV_SERVER_STATUS_FILE = previousFile;
      }
      if (previousToken === undefined) {
        delete process.env.WORKCELL_DEV_SERVER_STATUS_TOKEN;
      } else {
        process.env.WORKCELL_DEV_SERVER_STATUS_TOKEN = previousToken;
      }
    }
  });
});
  ...
  it("records a manual restart request for the dev runner", async () => {
    ...
    try {
      ...
      const requestPath = path.join(
        path.dirname(process.env.WORKCELL_DEV_SERVER_STATUS_FILE),
