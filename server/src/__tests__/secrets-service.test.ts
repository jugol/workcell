...
describeEmbeddedPostgres("secretService", () => {
  ...
  const previousKeyFile = process.env.WORKCELL_SECRETS_MASTER_KEY_FILE;
  const secretsTmpDir = path.join(os.tmpdir(), `workcell-secrets-service-${randomUUID()}`);

  beforeAll(async () => {
    mkdirSync(secretsTmpDir, { recursive: true });
    process.env.WORKCELL_SECRETS_MASTER_KEY_FILE = path.join(secretsTmpDir, "master.key");
  ...
  afterAll(async () => {
    ...
    if (previousKeyFile === undefined) {
      ...
      process.env.WORKCELL_SECRETS_MASTER_KEY_FILE = previousKeyFile;
    }
  ...
  it("blocks coming-soon provider vaults from secret selection", async () => {
    ...
    const draftVault = await svc.createProviderConfig(companyId, {
      ...
      config: { projectId: "workcell-prod1" },
    });
  ...
  it("passes selected provider vault config through create, rotate, and resolve", async () => {
    ...
    const createSpy = vi.spyOn(awsSecretsManagerProvider, "createSecret").mockResolvedValue({
      material: {
        ...
        secretId: "arn:aws:secretsmanager:us-east-1:123456789012:secret:workcell/prod-use1/company/openai-api-key",
        versionId: "aws-version-1",
        source: "managed",
      },
      valueSha256: "value-sha-1",
      fingerprintSha256: "fingerprint-sha-1",
      externalRef: "arn:aws:secretsmanager:us-east-1:123456789012:secret:workcell/prod-use1/company/openai-api-key",
  ...
  it("cleans up managed provider secrets when create persistence fails", async () => {
    ...
    const prepared = {
      material: {
        scheme: "aws_secrets_manager_v1",
        secretId:
          "arn:aws:secretsmanager:us-east-1:123456789012:secret:workcell/prod-use1/company/create-rollback",
        versionId: "aws-version-1",
        source: "managed",
      },
      valueSha256: "value-sha-1",
  ...
  it("keeps a local cleanup handle when create rollback cleanup fails", async () => {
    ...
    const prepared = {
      material: {
        scheme: "aws_secrets_manager_v1",
        secretId:
          "arn:aws:secretsmanager:us-east-1:123456789012:secret:workcell/prod-use1/company/create-cleanup-handle",
        versionId: "aws-version-1",
        source: "managed",
      },
      valueSha256: "value-sha-1",
  ...
  it("archives managed provider versions when rotate persistence fails", async () => {
    ...
    vi.spyOn(awsSecretsManagerProvider, "createSecret").mockResolvedValue({
      material: {
        scheme: "aws_secrets_manager_v1",
        secretId:
          "arn:aws:secretsmanager:us-east-1:123456789012:secret:workcell/prod-use1/company/rotate-rollback",
        versionId: "aws-version-1",
        source: "managed",
      },
      valueSha256: "value-sha-1",
  ...
  it("keeps a disabled version cleanup handle when rotate rollback cleanup fails", async () => {
    ...
    vi.spyOn(awsSecretsManagerProvider, "createSecret").mockResolvedValue({
      material: {
        scheme: "aws_secrets_manager_v1",
        secretId:
          "arn:aws:secretsmanager:us-east-1:123456789012:secret:workcell/prod-use1/company/rotate-cleanup-handle",
        versionId: "aws-version-1",
        source: "managed",
      },
      valueSha256: "value-sha-1",
  ...
  it("rejects generic provider vault reassignment for managed secrets", async () => {
    ...
    vi.spyOn(awsSecretsManagerProvider, "createSecret").mockResolvedValue({
      material: {
        scheme: "aws_secrets_manager_v1",
        secretId:
          "arn:aws:secretsmanager:us-east-1:123456789012:secret:workcell/prod-use1/company/vault-reassign",
        versionId: "aws-version-1",
        source: "managed",
      },
      valueSha256: "value-sha-1",
  ...
  it("sanitizes AWS remote import preview provider errors before crossing the service boundary", async () => {
    ...
    const rawProviderMessage =
      "AccessDeniedException: User: arn:aws:sts::123456789012:assumed-role/prod/Workcell is not authorized to perform secretsmanager:ListSecrets";
  ...
  it("previews AWS provider vault discovery from draft config without persisting a provider vault", async () => {
    ...
    const discoverSpy = vi.spyOn(awsSecretsManagerProvider, "discoverProviderConfigs").mockResolvedValue({
      ...
      skippedForeignWorkcellSampleCount: 0,
      candidates: [
        {
          provider: "aws_secrets_manager",
          displayName: "AWS production",
          ...
          sampleCount: 1,
          samples: [
            { name: "workcell/prod-use1/company-1/openai", hasKmsKey: false, tagKeys: ["workcell:environment"] },
          ],
          signals: {
            namespace: "prod-use1",
            secretNamePrefix: "workcell",
  ...
  it("sanitizes AWS remote import row provider errors", async () => {
    ...
    const rawProviderMessage =
      "AccessDeniedException: User: arn:aws:sts::123456789012:assumed-role/prod/Workcell is not authorized to perform secretsmanager:DescribeSecret on arn:aws:secretsmanager:us-east-1:123456789012:secret:prod/openai";
  ...
  it("rejects Workcell-managed AWS namespace refs during preview and import commit", async () => {
    const companyId = await seedCompany();
    const svc = secretService(db);
    const awsVault = await svc.createProviderConfig(companyId, {
      provider: "aws_secrets_manager",
    ...
    vi.spyOn(awsSecretsManagerProvider, "listRemoteSecrets").mockResolvedValue({
      secrets: [
        {
          externalRef:
            "arn:aws:secretsmanager:us-east-1:123456789012:secret:workcell/prod-use1/company-b/openai",
          name: "workcell/prod-use1/company-b/openai",
          providerVersionRef: null,
          metadata: {
            arn: "arn:aws:secretsmanager:us-east-1:123456789012:secret:workcell/prod-use1/company-b/openai",
            description: "must not leak",
            tags: [{ Key: "workcell:company-id", Value: "company-b" }],
          },
        },
      ],
    });
    ...
    expect(JSON.stringify(preview)).not.toContain("workcell:company-id");
    ...
    expect(result.results[0]?.reason).toMatch(/Workcell-managed namespace/i);
  ...
  it("rejects externalRef overrides on managed secrets", async () => {
    ...
    await expect(
      svc.update(secret.id, {
        externalRef: "arn:aws:secretsmanager:us-east-1:123456789012:secret:workcell/prod/company-b/openai-api-key",
      }),
  ...
  it("passes managed AWS secret context into provider delete during removal", async () => {
    ...
    const svc = secretService(db);
    const externalRef =
      "arn:aws:secretsmanager:us-east-1:123456789012:secret:workcell/prod-use1/company-1/openai-api-key";
  ...
  it("renames name and key during removal before provider deletion", async () => {
    ...
    const svc = secretService(db);
    const externalRef =
      "arn:aws:secretsmanager:us-east-1:123456789012:secret:workcell/prod-use1/company-1/remove-failure";
    const secret = await db
      .insert(companySecrets)
      .values({
        companyId,
  ...
  it("treats missing provider secrets as already removed during removal retry", async () => {
    ...
    const svc = secretService(db);
    const externalRef =
      "arn:aws:secretsmanager:us-east-1:123456789012:secret:workcell/prod-use1/company-1/retry-delete";
  ...
  it("removes DB rows even when the attached provider vault is disabled", async () => {
    ...
    const externalRef =
      "arn:aws:secretsmanager:us-east-1:123456789012:secret:workcell/prod/company-1/openai-api-key";
    const secret = await db
      .insert(companySecrets)
      .values({
        companyId,
