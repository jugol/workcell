...
describe("agent instructions service", () => {
  const originalWorkcellHome = process.env.WORKCELL_HOME;
  const originalWorkcellInstanceId = process.env.WORKCELL_INSTANCE_ID;
  const originalInstructionsRootAllowlist = process.env.WORKCELL_INSTRUCTIONS_ROOT_ALLOWLIST;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalWorkcellHome === undefined) delete process.env.WORKCELL_HOME;
    else process.env.WORKCELL_HOME = originalWorkcellHome;
    if (originalWorkcellInstanceId === undefined) delete process.env.WORKCELL_INSTANCE_ID;
    else process.env.WORKCELL_INSTANCE_ID = originalWorkcellInstanceId;
    if (originalInstructionsRootAllowlist === undefined) delete process.env.WORKCELL_INSTRUCTIONS_ROOT_ALLOWLIST;
    else process.env.WORKCELL_INSTRUCTIONS_ROOT_ALLOWLIST = originalInstructionsRootAllowlist;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const workcellHome = await makeTempDir("workcell-agent-instructions-home-");
    const externalRoot = await makeTempDir("workcell-agent-instructions-external-");
    cleanupDirs.add(workcellHome);
    cleanupDirs.add(externalRoot);
    process.env.WORKCELL_HOME = workcellHome;
    process.env.WORKCELL_INSTANCE_ID = "test-instance";
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("workcell-agent-instructions-new-external-");
    cleanupDirs.add(workcellHome);
    cleanupDirs.add(externalRoot);
    process.env.WORKCELL_HOME = workcellHome;
    process.env.WORKCELL_INSTANCE_ID = "test-instance";
    // An external root outside the instance dir now requires allowlisting.
    process.env.WORKCELL_INSTRUCTIONS_ROOT_ALLOWLIST = externalRoot;
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("workcell-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const workcellHome = await makeTempDir("workcell-agent-instructions-recover-");
    cleanupDirs.add(workcellHome);
    process.env.WORKCELL_HOME = workcellHome;
    process.env.WORKCELL_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const workcellHome = await makeTempDir("workcell-agent-instructions-stale-managed-");
    const staleRoot = await makeTempDir("workcell-agent-instructions-stale-root-");
    cleanupDirs.add(workcellHome);
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const workcellHome = await makeTempDir("workcell-agent-instructions-heal-write-");
    const staleRoot = await makeTempDir("workcell-agent-instructions-heal-write-stale-");
    cleanupDirs.add(workcellHome);
    cleanupDirs.add(staleRoot);
    process.env.WORKCELL_HOME = workcellHome;
    process.env.WORKCELL_INSTANCE_ID = "test-instance";
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const workcellHome = await makeTempDir("workcell-agent-instructions-heal-delete-");
    const staleRoot = await makeTempDir("workcell-agent-instructions-heal-delete-stale-");
    cleanupDirs.add(workcellHome);
    cleanupDirs.add(staleRoot);
    process.env.WORKCELL_HOME = workcellHome;
    process.env.WORKCELL_INSTANCE_ID = "test-instance";
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const workcellHome = await makeTempDir("workcell-agent-instructions-partial-managed-");
    const staleRoot = await makeTempDir("workcell-agent-instructions-partial-root-");
    cleanupDirs.add(workcellHome);
    cleanupDirs.add(staleRoot);
    process.env.WORKCELL_HOME = workcellHome;
    process.env.WORKCELL_INSTANCE_ID = "test-instance";
  ...
  it("rejects an external rootPath outside the instance dir and allowlist", async () => {
    const workcellHome = await makeTempDir("workcell-agent-instructions-reject-home-");
    const externalRoot = await makeTempDir("workcell-agent-instructions-reject-external-");
    cleanupDirs.add(workcellHome);
    cleanupDirs.add(externalRoot);
    process.env.WORKCELL_HOME = workcellHome;
    process.env.WORKCELL_INSTANCE_ID = "test-instance";
    delete process.env.WORKCELL_INSTRUCTIONS_ROOT_ALLOWLIST;

    const svc = agentInstructionsService();
    const agent = makeAgent({});

    await expect(
      svc.updateBundle(agent, { mode: "external", rootPath: externalRoot, entryFile: "AGENTS.md" }),
    ).rejects.toMatchObject({ status: 422 });
    // Nothing should have been written to the rejected root.
    await expect(fs.stat(path.join(externalRoot, "AGENTS.md"))).rejects.toThrow();
  });

  it("accepts an external rootPath when present in the allowlist", async () => {
    const workcellHome = await makeTempDir("workcell-agent-instructions-allow-home-");
    const otherRoot = await makeTempDir("workcell-agent-instructions-allow-other-");
    const externalRoot = await makeTempDir("workcell-agent-instructions-allow-external-");
    cleanupDirs.add(workcellHome);
    cleanupDirs.add(otherRoot);
    cleanupDirs.add(externalRoot);
    process.env.WORKCELL_HOME = workcellHome;
    process.env.WORKCELL_INSTANCE_ID = "test-instance";
    // Multi-entry, OS-path-delimited allowlist; the external root is the second entry.
    process.env.WORKCELL_INSTRUCTIONS_ROOT_ALLOWLIST = [otherRoot, externalRoot].join(path.delimiter);
    ...
    expect(result.bundle.mode).toBe("external");
    expect(result.bundle.rootPath).toBe(externalRoot);
    await expect(fs.stat(path.join(externalRoot, "AGENTS.md"))).resolves.toBeTruthy();
  });

  it("accepts an external rootPath nested under the instance dir without an allowlist", async () => {
    const workcellHome = await makeTempDir("workcell-agent-instructions-under-base-home-");
    cleanupDirs.add(workcellHome);
    process.env.WORKCELL_HOME = workcellHome;
    process.env.WORKCELL_INSTANCE_ID = "test-instance";
    delete process.env.WORKCELL_INSTRUCTIONS_ROOT_ALLOWLIST;

    const underBaseRoot = path.join(
      workcellHome,
      "instances",
      "test-instance",
      "shared",
    ...
    const result = await svc.updateBundle(agent, {
      mode: "external",
      rootPath: underBaseRoot,
      entryFile: "AGENTS.md",
    });

    expect(result.bundle.mode).toBe("external");
    expect(result.bundle.rootPath).toBe(underBaseRoot);
    await expect(fs.stat(path.join(underBaseRoot, "AGENTS.md"))).resolves.toBeTruthy();
  });
});
