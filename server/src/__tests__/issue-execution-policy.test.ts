...
import type { IssueExecutionPolicy, IssueExecutionState } from "@workcell/shared";
...
describe("issue execution policy transitions", () => {
  ...
  describe("autonomous mode (skipUserParticipation)", () => {
    const userReviewPolicy = () => makePolicy([
      { type: "review", participants: [{ type: "user", userId: boardUserId }] },
    ]);

    it("by default (flag off) parks a user-review stage on the human", () => {
      const policy = userReviewPolicy();
      const result = applyIssueExecutionPolicyTransition({
        issue: {
          status: "in_progress",
          assigneeAgentId: coderAgentId,
        ...
        actor: { agentId: coderAgentId },
        commentBody: "Done",
        // skipUserParticipation omitted → defaults to false
      });
    ...
    it("auto-completes a user-only stage at workflow start instead of parking on the human", () => {
      const policy = userReviewPolicy();
      const result = applyIssueExecutionPolicyTransition({
        issue: {
          status: "in_progress",
        ...
        actor: { agentId: coderAgentId },
        commentBody: "Done",
        skipUserParticipation: true,
      });

      // No parking on a user: workflow completes, leaving the caller's "done".
      expect(result.patch.status).toBeUndefined();
    ...
    it("skips a leading user stage and parks on the following agent (QA) stage", () => {
      const policy = makePolicy([
        { type: "review", participants: [{ type: "user", userId: boardUserId }] },
        { type: "review", participants: [{ type: "agent", agentId: qaAgentId }] },
      ]);
    ...
    it("auto-completes a trailing user stage after an agent approves (post-approval path)", () => {
      const policy = makePolicy([
        { type: "review", participants: [{ type: "agent", agentId: qaAgentId }] },
        { type: "approval", participants: [{ type: "user", userId: boardUserId }] },
      ]);
      const qaStageId = policy.stages[0].id;
      const userStageId = policy.stages[1].id;
      // QA agent is parked and approves; the trailing user-approval stage should
      // auto-complete under autonomous mode rather than parking on the human.
      const result = applyIssueExecutionPolicyTransition({
        issue: {
          status: "in_review",
          assigneeAgentId: qaAgentId,
      ...
      expect(result.patch.executionState).toMatchObject({
        status: "completed",
        completedStageIds: expect.arrayContaining([qaStageId, userStageId]),
      });
      expect(result.decision).toMatchObject({ stageId: qaStageId, stageType: "review", outcome: "approved" });
      expect(result.patch.status).toBeUndefined();
    });

    it("does NOT skip agent (QA) stages — the autonomous quality loop still runs", () => {
      const policy = makePolicy([
        { type: "review", participants: [{ type: "agent", agentId: qaAgentId }] },
      ]);
      const result = applyIssueExecutionPolicyTransition({
      ...
      // The QA agent stage is preserved (parked), not auto-approved.
      expect(result.patch.status).toBe("in_review");
      expect(result.patch.assigneeAgentId).toBe(qaAgentId);
      expect(result.patch.executionState).toMatchObject({
        status: "pending",
    ...
    it("prefers the agent for a mixed user+agent stage (never blocks on a human)", () => {
      // The user is listed FIRST, so without preferAgent the participant selector
      // would pick the user and the issue would park on a human even in autonomous
      // mode. A mixed stage is not user-only (so not auto-skipped), but autonomous
      // mode should still route it to the agent rather than a person.
      const policy = makePolicy([
        {
          type: "review",
          participants: [
            { type: "user", userId: boardUserId },
            { type: "agent", agentId: qaAgentId },
          ],
        },
      ]);
    ...
    it("with the flag OFF, a mixed user+agent stage parks on the first participant (the user)", () => {
      const policy = makePolicy([
        {
          type: "review",
          participants: [
      ...
      const result = applyIssueExecutionPolicyTransition({
        ...
        actor: { agentId: coderAgentId },
        commentBody: "Done",
        // skipUserParticipation omitted → preferAgent is off, behavior unchanged.
      });
