...
async function createGitRepo() {
  const repoRoot = await mkdtemp(path.join(os.tmpdir(), "workcell-accepted-plan-repo-"));
  await execFileAsync("git", ["init"], { cwd: repoRoot });
  await execFileAsync("git", ["config", "user.email", "workcell-test@example.com"], { cwd: repoRoot });
  await execFileAsync("git", ["config", "user.name", "Workcell Test"], { cwd: repoRoot });
...
describeEmbeddedPostgres("accepted plan workspace refresh", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("workcell-accepted-plan-workspace-");
  ...
  it("realizes an isolated workspace and drops stale shared task-session params before executing", async () => {
    ...
    expect(adapterInput.context.workcellWorkspace).toEqual(expect.objectContaining({
      mode: "isolated_workspace",
      strategy: "git_worktree",
    }));
    expect((adapterInput.context.workcellWorkspace as { cwd: string }).cwd).not.toBe(repoRoot);
