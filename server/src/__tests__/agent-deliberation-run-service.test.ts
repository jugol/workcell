...
describeEmbeddedPostgres("WC-209 agentDeliberationRunService", () => {
  ...
  it("persists turns in order, completes the run, and SUMS per-turn cost", async () => {
    ...
    // Fusion turns: A generate → B generate → A synthesize. total 15.
    ...
    const buildInvoke = (onCost: (c: number) => void): DeliberationInvoke => {
      return async ({ role }) => {
        ...
        // does (it bills after the adapter run, then returns the text). onCost
        // pushes onto the run service's per-turn cost queue.
        onCost(costByTurn[turnIndex] ?? 0);
        turnIndex += 1;
        return role === "synthesize" ? "Synthesized." : "Candidate.";
      };
    };
    ...
    expect(run!.acceptedBy).toBe("A"); // the synthesizer brain
    expect(run!.rounds).toBe(1);
    expect(run!.finalOutput).toBe("Synthesized.");
  ...
  it("getRun returns the run + ordered turns (company-scoped); cross-company → null", async () => {
    ...
    const buildInvoke = (): DeliberationInvoke => async ({ role }) =>
      role === "synthesize" ? "final" : "candidate";
    const runId = await svc.start({
      companyId,
      agentId,
      task: "Get me.",
    ...
    expect(found!.turns.length).toBe(3); // A generate + B generate + synthesize
  ...
  it("create() inserts a 'running' row WITHOUT running the loop; run() finishes it", async () => {
    ...
    expect(finished!.acceptedBy).toBe("A"); // the synthesizer brain
  });
