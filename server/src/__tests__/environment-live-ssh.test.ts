...
} from "@workcell/adapter-utils/ssh";
...
function resolveEnvLabStatePath(): string {
  const instanceRoot =
    process.env.WORKCELL_INSTANCE_ROOT?.trim() ||
    path.join(process.env.HOME ?? "/tmp", ".workcell-worktrees", "instances", "live-ssh-test");
  return path.join(instanceRoot, "env-lab", "ssh-fixture", "state.json");
}

/** Attempt to build config from explicit WORKCELL_ENV_LIVE_SSH_* env vars. */
function tryExplicitConfig(): {
  ...
  const host = process.env.WORKCELL_ENV_LIVE_SSH_HOST?.trim() ?? "";
  const username = process.env.WORKCELL_ENV_LIVE_SSH_USERNAME?.trim() ?? "";
  const remoteWorkspacePath =
    process.env.WORKCELL_ENV_LIVE_SSH_REMOTE_WORKSPACE_PATH?.trim() ?? "";
  const port = Number.parseInt(process.env.WORKCELL_ENV_LIVE_SSH_PORT ?? "22", 10);
 ...
 * 1. Explicit WORKCELL_ENV_LIVE_SSH_* env vars
...
async function resolveSshConfig(): Promise<SshConnectionConfig | null> {
  ...
  if (explicit) {
    return {
      ...
      privateKey: await readOptionalSecret(
        process.env.WORKCELL_ENV_LIVE_SSH_PRIVATE_KEY,
        process.env.WORKCELL_ENV_LIVE_SSH_PRIVATE_KEY_PATH,
      ),
      knownHosts: await readOptionalSecret(
        process.env.WORKCELL_ENV_LIVE_SSH_KNOWN_HOSTS,
        process.env.WORKCELL_ENV_LIVE_SSH_KNOWN_HOSTS_PATH,
      ),
      strictHostKeyChecking:
        (process.env.WORKCELL_ENV_LIVE_SSH_STRICT_HOST_KEY_CHECKING ?? "true").toLowerCase() !== "false",
    };
  }
  ...
  if (process.env.WORKCELL_ENV_LIVE_SSH_NO_AUTO_FIXTURE !== "true") {
...
const describeLiveSsh = (() => {
  ...
  if (process.env.WORKCELL_ENV_LIVE_SSH_NO_AUTO_FIXTURE === "true") {
    console.warn(
      "Skipping live SSH smoke test. Set WORKCELL_ENV_LIVE_SSH_HOST, WORKCELL_ENV_LIVE_SSH_USERNAME, and WORKCELL_ENV_LIVE_SSH_REMOTE_WORKSPACE_PATH to enable it, or remove WORKCELL_ENV_LIVE_SSH_NO_AUTO_FIXTURE to auto-start env-lab.",
    );
...
describeLiveSsh("live SSH environment smoke", () => {
  ...
  it("connects to the configured SSH environment and verifies basic runtime tools", async () => {
    ...
    if (!resolvedConfig) {
      console.warn(
        "Live SSH smoke test could not resolve SSH config from env vars or env-lab fixture. Set WORKCELL_ENV_LIVE_SSH_NO_AUTO_FIXTURE=true to mark this suite skipped intentionally.",
      );
      return;
    }
