...
const mockWorkProductService = vi.hoisted(() => ({
  ...
  deriveDesignGateForIssue: vi.fn(),
}));

// The heartbeat-context route uses workProductService.deriveDesignGateForIssue.
// Default the mock to the real pure derivation so older designGate route tests
// stay focused on HTTP projection rather than service internals.
const { deriveIssueDesignGate: realDeriveIssueDesignGate } = await vi.hoisted(
  async () => await import("../services/work-products.js"),
);
...
vi.mock("../services/index.js", () => ({
  ...
  deriveIssueDesignGate: realDeriveIssueDesignGate,
}));
...
describe.sequential("issue goal context routes", () => {
  beforeEach(() => {
    ...
    // Reset the work-products defaults each test so a per-test mockResolvedValue
    // (designGate cases) does not leak into the next test.
    mockWorkProductService.listForIssue.mockReset().mockResolvedValue([]);
    mockWorkProductService.deriveDesignGateForIssue.mockReset().mockImplementation(async (issueId: string) => {
      const workProducts = await mockWorkProductService.listForIssue(issueId);
      return realDeriveIssueDesignGate(workProducts as IssueWorkProduct[]);
    });
  ...
  // WC-182f / D22: the design gate is projected into the heartbeat-context bundle
  // so a dev/QA agent's wake context carries the build/hold signal.
  function designWorkProduct(overrides: Record<string, unknown> = {}) {
  ...
  it("includes a developmentHold designGate on GET /issues/:id/heartbeat-context for a pending source-of-truth design", async () => {
    mockWorkProductService.listForIssue.mockResolvedValue([
      designWorkProduct({ isPrimary: true, reviewState: "needs_board_review", title: "Pending design" }),
    ]);
    ...
    expect(res.status).toBe(200);
    expect(res.body.designGate).toEqual(
      expect.objectContaining({
        hasDesign: true,
        approved: false,
        developmentHold: true,
        authoritativeDesign: expect.objectContaining({
          title: "Pending design",
          reviewState: "needs_board_review",
        }),
      }),
    );
    expect(res.body.designGate.directive).toContain("HOLD development");
  });

  it("includes an approved (no-hold) designGate on GET /issues/:id/heartbeat-context for an approved source-of-truth design", async () => {
    mockWorkProductService.listForIssue.mockResolvedValue([
      designWorkProduct({
        isPrimary: true,
    ...
    expect(res.body.designGate.directive).toContain("Build and verify against it");
  });

  it("uses the authoritative work-product service designGate projection in GET /issues/:id/heartbeat-context", async () => {
    mockWorkProductService.deriveDesignGateForIssue.mockResolvedValue({
      hasDesign: true,
      approved: true,
      developmentHold: false,
      authoritativeDesign: {
        id: "design-approval-signal-1",
        title: "Approved by linked board approval",
        url: null,
        reviewState: "approved",
      },
      directive:
        'The approved source-of-truth design for this issue: "Approved by linked board approval". Build and verify against it; do not deviate from the design.',
      screens: [
        {
          screenKey: "tutorial-start-curriculum",
          screenName: "Tutorial start curriculum",
          workProductId: "design-approval-signal-1",
          url: null,
          reviewState: "approved",
          approved: true,
        },
    ...
    expect(res.status).toBe(200);
    expect(mockWorkProductService.deriveDesignGateForIssue).toHaveBeenCalledWith(
      legacyProjectLinkedIssue.id,
      legacyProjectLinkedIssue.companyId,
    );
  ...
  it("emits an empty designGate on GET /issues/:id/heartbeat-context when the issue has no designs", async () => {
    mockWorkProductService.listForIssue.mockResolvedValue([]);
