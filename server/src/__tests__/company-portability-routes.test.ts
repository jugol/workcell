...
const cloudHeaders = {
  "x-workcell-cloud-stack-id": "stack-alpha",
  "x-workcell-cloud-workcell-company-id": companyId,
};
...
describe.sequential("company portability routes", () => {
  beforeEach(() => {
    ...
    mockAgentService.getById.mockImplementation(async (id: string) => ({
      id,
      companyId,
      role: id === ceoAgentId ? "orchestrator" : "engineer",
    }));
  ...
  it.sequential("rejects non-CEO agents from CEO-safe export preview routes", async () => {
    ...
    expect(res.body.error).toContain("Only the Orchestrator");
  ...
  it.sequential("allows CEO agents to use company-scoped export preview routes", async () => {
    ...
    expect(res.body.rootPath).toBe("workcell");
  });
  ...
  it.sequential("accepts trusted Cloud async import jobs and reports success by job id", async () => {
    ...
    const accepted = await request(app)
      .post("/api/companies/import")
      .set("x-workcell-cloud-async-import", "1")
      .set(cloudHeaders)
      .send(importRequest);
