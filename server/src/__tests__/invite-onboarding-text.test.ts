...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Workcell Agent Onboarding");
    ...
    expect(text).toContain("/api/invites/token-123/skills/workcell");
    expect(text).toContain("Suggested Workcell base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("workcellApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.workcellApiUrl");
    expect(text).toContain("WORKCELL_API_KEY");
    expect(text).toContain("Use your runtime's normal skill or instruction installation path.");
    expect(text).toContain("Decide which Workcell adapter type matches your runtime.");
  });
  ...
  it("includes LAN candidates when the advertised host is tailnet-only", () => {
    ...
    try {
      const req = buildReq("workcell.example.test:3103");
      const invite = {
        id: "invite-4",
        companyId: "company-1",
        inviteType: "company_join",
      ...
      const text = buildInviteOnboardingTextDocument(req, "token-999", invite as any, {
        ...
        deploymentExposure: "private",
        bindHost: "0.0.0.0",
        allowedHostnames: ["workcell.example.test", "203.0.113.42"],
      });
