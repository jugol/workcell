...
describe("awsSecretsManagerProvider", () => {
  const previousEnv = {
    WORKCELL_SECRETS_AWS_REGION: process.env.WORKCELL_SECRETS_AWS_REGION,
    AWS_REGION: process.env.AWS_REGION,
    AWS_DEFAULT_REGION: process.env.AWS_DEFAULT_REGION,
    WORKCELL_SECRETS_AWS_DEPLOYMENT_ID: process.env.WORKCELL_SECRETS_AWS_DEPLOYMENT_ID,
    WORKCELL_SECRETS_AWS_KMS_KEY_ID: process.env.WORKCELL_SECRETS_AWS_KMS_KEY_ID,
  ...
  it("creates Workcell-managed AWS secrets without persisting plaintext in provider material", async () => {
    ...
    const provider = createAwsSecretsManagerProvider({
      ...
      gateway: {
        async createSecret(input) {
          ...
          return {
            ARN: "arn:aws:secretsmanager:us-east-1:123456789012:secret:workcell/prod-use1/company-1/openai-api-key",
    ...
    expect(calls).toEqual([
      expect.objectContaining({
        ...
        input: expect.objectContaining({
          Name: "workcell/prod-use1/company-1/openai-api-key",
          KmsKeyId: "arn:aws:kms:us-east-1:123456789012:key/test",
        }),
      }),
    ]);
    expect(JSON.stringify(prepared)).not.toContain("super-secret-value");
    expect(prepared.externalRef).toContain("workcell/prod-use1/company-1/openai-api-key");
  ...
  it("creates AWS secrets from selected provider vault config without deployment env fallback", async () => {
    delete process.env.WORKCELL_SECRETS_AWS_REGION;
    delete process.env.AWS_REGION;
    delete process.env.AWS_DEFAULT_REGION;
    delete process.env.WORKCELL_SECRETS_AWS_DEPLOYMENT_ID;
    delete process.env.WORKCELL_SECRETS_AWS_KMS_KEY_ID;
      ...
      expect.objectContaining({
        ...
        input: expect.objectContaining({
          ...
          Tags: expect.arrayContaining([
            { Key: "workcell:provider-owner", Value: "platform" },
            { Key: "workcell:environment", Value: "production" },
          ]),
        }),
      }),
    ]);
  ...
  it("signs AWS Secrets Manager JSON requests with default runtime credentials", async () => {
    ...
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValue(
      new Response(
        JSON.stringify({
          ARN: "arn:aws:secretsmanager:us-east-1:123456789012:secret:workcell/prod/company-1/openai-api-key",
          VersionId: "aws-version-1",
        }),
        { status: 200 },
      ),
    ...
    expect(calls).toEqual([
      {
        ...
        input: {
          ...
          VersionStages: ["WORKCELL_PENDING"],
        },
      },
    ]);
  ...
  it("rejects linked external references under the Workcell-managed namespace", async () => {
    const provider = createAwsSecretsManagerProvider({
      config: {
        region: "us-east-1",
        endpoint: "https://secretsmanager.us-east-1.amazonaws.com",
    ...
    await expect(
      provider.linkExternalSecret({
        externalRef:
          "arn:aws:secretsmanager:us-east-1:123456789012:secret:workcell/prod-use1/company-2/openai-api-key",
        providerVersionRef: "linked-version-7",
      }),
    ).rejects.toThrow(/Workcell-managed namespace/i);
  });
      ...
      gateway: {
        ...
        async listSecrets(input) {
          ...
          return {
            ...
            SecretList: [
              {
                ARN: "arn:aws:secretsmanager:us-east-1:123456789012:secret:workcell/prod-use1/company-1/openai",
                Name: "workcell/prod-use1/company-1/openai",
                KmsKeyId: "arn:aws:kms:us-east-1:123456789012:key/prod",
                Tags: [
                  { Key: "workcell:managed-by", Value: "workcell" },
                  { Key: "workcell:deployment-id", Value: "prod-use1" },
                  { Key: "workcell:company-id", Value: "company-1" },
                  { Key: "workcell:environment", Value: "production" },
                  { Key: "workcell:provider-owner", Value: "platform" },
                ],
              },
              {
                ARN: "arn:aws:secretsmanager:us-east-1:123456789012:secret:workcell/prod-use1/company-2/stripe",
                Name: "workcell/prod-use1/company-2/stripe",
                Tags: [
                  { Key: "workcell:managed-by", Value: "workcell" },
                  { Key: "workcell:company-id", Value: "company-2" },
                ],
              },
            ],
          };
    ...
    expect(calls).toEqual([
      {
        ...
        input: {
          ...
          NextToken: undefined,
          IncludePlannedDeletion: false,
          Filters: [{ Key: "all", Values: ["workcell"] }],
        },
      },
    ]);
  ...
  it("redacts AWS provider exception text when remote listing fails", async () => {
    const rawProviderMessage =
      "AccessDeniedException: User: arn:aws:sts::123456789012:assumed-role/prod/Workcell is not authorized to perform secretsmanager:ListSecrets on arn:aws:secretsmanager:us-east-1:123456789012:secret:prod/openai";
    const provider = createAwsSecretsManagerProvider({
      config: {
        region: "us-east-1",
        endpoint: "https://secretsmanager.us-east-1.amazonaws.com",
  ...
  it("resolves AWS secret values by provider version reference", async () => {
    ...
    const resolved = await provider.resolveVersion({
      material: {
        ...
        secretId: "arn:aws:secretsmanager:us-east-1:123456789012:secret:workcell/prod-use1/company-1/openai-api-key",
        versionId: "aws-version-2",
        source: "managed",
      },
      externalRef:
  ...
  it("warns when AWS provider configuration is incomplete and blocks managed writes", async () => {
    ...
    expect(health.message).toContain("missing WORKCELL_SECRETS_AWS_REGION");
    ...
    ).rejects.toThrow(/WORKCELL_SECRETS_AWS_REGION|AWS_REGION/i);
  });

  it("deletes only Workcell-managed AWS secrets", async () => {
    const calls: Array<{ op: string; input: Record<string, unknown> }> = [];
    const provider = createAwsSecretsManagerProvider({
      config: {
        region: "us-east-1",
  ...
  it("archives pending Workcell-managed AWS versions without deleting the secret", async () => {
    const calls: Array<{ op: string; input: Record<string, unknown> }> = [];
    const provider = createAwsSecretsManagerProvider({
      config: {
        region: "us-east-1",
