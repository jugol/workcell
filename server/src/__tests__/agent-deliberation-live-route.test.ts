...
// Deterministic Fusion stub: brain A and brain B each generate a candidate in
// parallel, then the synthesizer (A) merges them. `state.calls` counts the brain
// turns so tests can assert the stub (not a real model) was driven.
function makeConsensusStub(): { invoke: DeliberationInvoke; state: { calls: number } } {
  const state = { calls: 0 };
  const invoke: DeliberationInvoke = async ({ role, brain }) => {
    state.calls += 1;
    return role === "synthesize" ? "Synthesized final answer." : `Candidate from Brain ${brain}.`;
  };
  return { invoke, state };
}
...
describeEmbeddedPostgres("WC-209 agent deliberation async route", () => {
  ...
  it("POST returns 202 + runId; the run persists turns IN ORDER and completes (A generate ∥ B generate → A synthesize)", async () => {
    ...
    expect(done.body.run.acceptedBy).toBe("A"); // the synthesizer brain
    expect(done.body.run.rounds).toBe(1);
    expect(done.body.run.finalOutput).toBe("Synthesized final answer.");
    expect(done.body.run.completedAt).toBeTruthy();

    // Turns persisted in order: generate (A) ∥ generate (B) → synthesize (A).
    expect(done.body.turns).toHaveLength(3);
    expect(done.body.turns[0]).toMatchObject({ round: 0, brain: "A", action: "generate" });
    expect(done.body.turns[1]).toMatchObject({ round: 0, brain: "B", action: "generate" });
    expect(done.body.turns[2]).toMatchObject({ round: 1, brain: "A", action: "synthesize" });
    // 2 generate + 1 synthesize = 3 brain turns drove the stub.
  ...
  it("stores maxRoundsOverride on the run (fusion is single-pass; the override is recorded, not looped)", async () => {
    await insertAgent(agentId, companyId, enabledDeliberation);
    const stub = makeConsensusStub();
    const app = await makeApp(stub.invoke);
    ...
    expect(done.body.run.status).toBe("completed");
    expect(done.body.run.maxRounds).toBe(1); // the override is stored on the run row
    expect(done.body.run.acceptedBy).toBe("A"); // fusion always synthesizes
    // generate (A) ∥ generate (B) → synthesize (A) = 3 turns.
    expect(done.body.turns).toHaveLength(3);
  });
