...
function sendNestedHostRequest(originalRequest, invocationId) {
  ...
  if (mode === "echo") {
    nestedRequest.workcellInvocationId = invocationId;
  } else if (mode === "unknown") {
    nestedRequest.workcellInvocationId = "unknown-invocation";
  }
...
rl.on("line", (line) => {
  ...
  if (method === "getData" || method === "performAction") {
    sendNestedHostRequest(message, message.workcellInvocation?.id);
    return;
  }
