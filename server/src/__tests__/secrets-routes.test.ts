...
describe("secret routes", () => {
  ...
  it("previews provider vault discovery and logs only aggregate metadata", async () => {
      ...
      candidates: [
        {
          ...
          sampleCount: 2,
          samples: [
            { name: "workcell/prod-use1/company-1/openai", hasKmsKey: false, tagKeys: ["environment"] },
          ],
          signals: {
            namespace: "prod-use1",
            secretNamePrefix: "workcell",
    ...
    expect(JSON.stringify(mockLogActivity.mock.calls)).not.toContain("workcell/prod-use1/company-1/openai");
  });
        ...
        secrets: [
          {
            ...
            name: "OpenAI API key",
            key: "openai-api-key",
            description: "Operator-entered Workcell description",
          },
        ],
      });
