...
export async function startServer(): Promise<StartedServer> {
  ...
  if (config.databaseUrl) {
    ...
    const embeddedAdminConnectionString = `postgres://workcell:workcell@127.0.0.1:${port}/postgres`;
    const dbStatus = await ensurePostgresDatabase(embeddedAdminConnectionString, "workcell");
    if (dbStatus === "created") {
