...
export function resolveWorktreeEnvFilePath(rootDir: string): string {
  return path.resolve(rootDir, ".workcell", ".env");
}
...
function resolveDefaultWorktreeHome(env: NodeJS.ProcessEnv): string {
  return path.resolve(expandHomePrefix(env.WORKCELL_WORKTREES_DIR?.trim() || "~/.workcell-worktrees"));
}
...
function repairStaleMigratedWorktreeEnvEntries(
  ...
  const localConfigPath = path.resolve(rootDir, ".workcell", "config.json");
  const configuredPath = entries.WORKCELL_CONFIG?.trim();
  ...
  return {
    ...
    WORKCELL_CONFIG: localConfigPath,
    WORKCELL_CONTEXT: path.resolve(homeDir, "context.json"),
  };
}
