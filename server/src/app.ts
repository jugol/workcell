...
import { mojibakeRequestGuard } from "./services/text-encoding-guard.js";
...
import { buildDefaultPairTurnExecutor, mcpClientRegistry } from "./services/index.js";
...
import { designFlowRoutes } from "./routes/design-flow-routes.js";
...
export async function createApp(
  ...
  // Reject CP949-shell mojibake on every mutating JSON body before it can be
  // persisted anywhere (documents, comments, agent instructions, ...).
  api.use(mojibakeRequestGuard());
  api.use(
    "/health",
    healthRoutes(db, {
      deploymentMode: opts.deploymentMode,
  ...
  // WC-33 / WC-58 / WC-146 / WC-125 / WC-132: wire pair-group routes with the
  // default turn executor (live LLM by default, deterministic stub under test).
  // The construction logic lives in buildDefaultPairTurnExecutor so the
  // heartbeat auto-run ticker (index.ts) uses the exact same wiring.
  api.use(
    pairGroupRoutes(db, {
      pairTurnExecutor: buildDefaultPairTurnExecutor(db),
    }),
  );
  ...
  api.use(designFlowRoutes(db, opts.storageService));
