...
function jwtConfig() {
  const secret = process.env.WORKCELL_AGENT_JWT_SECRET?.trim() || process.env.BETTER_AUTH_SECRET?.trim();
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(process.env.WORKCELL_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.WORKCELL_AGENT_JWT_ISSUER ?? "workcell",
    audience: process.env.WORKCELL_AGENT_JWT_AUDIENCE ?? "workcell-api",
  };
}
...
export function verifyLocalAgentJwt(token: string): LocalAgentJwtClaims | null {
  ...
  if (issuer !== config.issuer) return null; // missing (null) or mismatched issuer → reject
  if (audience !== config.audience) return null; // missing (null) or mismatched audience → reject
