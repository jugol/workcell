...
import type { WorkcellConfig } from "@workcell/shared";
...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by Workcell worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.WORKCELL_IN_WORKTREE !== "true") return null;

  const configPath = resolveWorkcellConfigPath(overrideConfigPath);
  const envPath = resolveWorkcellEnvPath(configPath);
  const persistedEnv = readEnvEntries(envPath);
  const persistedConfigPath = nonEmpty(persistedEnv.WORKCELL_CONFIG);
  ...
  const worktreeName =
    nonEmpty(stablePersistedEnv.WORKCELL_WORKTREE_NAME) ??
    nonEmpty(env.WORKCELL_WORKTREE_NAME) ??
    path.basename(worktreeRoot);
  const instanceId =
    nonEmpty(stablePersistedEnv.WORKCELL_INSTANCE_ID) ??
    nonEmpty(env.WORKCELL_INSTANCE_ID) ??
    sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(stablePersistedEnv.WORKCELL_HOME) ??
      nonEmpty(env.WORKCELL_HOME) ??
      nonEmpty(env.WORKCELL_WORKTREES_DIR) ??
      "~/.workcell-worktrees",
  );
...
function writeConfigFile(configPath: string, config: WorkcellConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".workcell", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as WorkcellConfig;
...
function buildIsolatedWorktreeConfig(
  ...
  const databasePort =
    config.database.mode === "embedded-postgres"
      ...
      : undefined;
  const nextConfig: WorkcellConfig = {
    ...config,
    database: {
      ...config.database,
      ...(config.database.mode === "embedded-postgres"
...
): { config: WorkcellConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.WORKCELL_HOME = context.homeDir;
  process.env.WORKCELL_INSTANCE_ID = context.instanceId;
  process.env.WORKCELL_CONFIG = context.configPath;
  process.env.WORKCELL_CONTEXT = context.contextPath;
  process.env.WORKCELL_WORKTREE_NAME = context.worktreeName;

  let repairedConfig = false;
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as WorkcellConfig;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    WORKCELL_HOME: context.homeDir,
    WORKCELL_INSTANCE_ID: context.instanceId,
    WORKCELL_CONFIG: context.configPath,
    WORKCELL_CONTEXT: context.contextPath,
    WORKCELL_IN_WORKTREE: "true",
    WORKCELL_WORKTREE_NAME: context.worktreeName,
  };
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: WorkcellConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as WorkcellConfig;
  } catch {
    return;
  }
