...
import { resolveWorkcellInstanceId } from "../home-paths.js";
...
export function createBetterAuthInstance(db: Db, config: Config, trustedOrigins: string[]): BetterAuthInstance {
  const baseUrl = config.authBaseUrlMode === "explicit" ? config.authPublicBaseUrl : undefined;
  const publicUrl = process.env.WORKCELL_PUBLIC_URL?.trim() || baseUrl;
  const secret = process.env.BETTER_AUTH_SECRET ?? process.env.WORKCELL_AGENT_JWT_SECRET;
  if (!secret) {
    throw new Error(
      "BETTER_AUTH_SECRET (or WORKCELL_AGENT_JWT_SECRET) must be set. " +
      "For local development, set BETTER_AUTH_SECRET=workcell-dev-secret in your .env file.",
    );
  }
