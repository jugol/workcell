...
import { resolveWorkcellConfigPath, resolveWorkcellEnvPath } from "./paths.js";
import type { BindMode, DeploymentExposure, DeploymentMode } from "@workcell/shared";
...
function resolveAgentJwtSecretStatus(
  ...
  const envValue = process.env.WORKCELL_AGENT_JWT_SECRET?.trim();
  if (envValue) {
    return {
      status: "pass",
      message: "set",
  ...
  if (existsSync(envFilePath)) {
    ...
    const fileValue = typeof parsed.WORKCELL_AGENT_JWT_SECRET === "string" ? parsed.WORKCELL_AGENT_JWT_SECRET.trim() : "";
    if (fileValue) {
      return {
        status: "warn",
  ...
  return {
    status: "warn",
    message: "missing (run `pnpm workcell onboard`)",
  };
}
...
export function printStartupBanner(opts: StartupBannerOptions): void {
  ...
  const envFilePath = resolveWorkcellEnvPath();
  ...
  // Same WORKCELL lettering as cli/src/utils/banner.ts — the last visible
  // PAPERCLIP artwork (block-character art never matches a "paperclip" grep,
  // which is how it survived the rebrand).
  const art = [
    color("██╗    ██╗ ██████╗ ██████╗ ██╗  ██╗ ██████╗███████╗██╗     ██╗     ", "cyan"),
    color("██║    ██║██╔═══██╗██╔══██╗██║ ██╔╝██╔════╝██╔════╝██║     ██║     ", "cyan"),
    color("██║ █╗ ██║██║   ██║██████╔╝█████╔╝ ██║     █████╗  ██║     ██║     ", "cyan"),
