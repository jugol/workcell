...
function getRuntimeServicesDir() {
  return path.resolve(resolveWorkcellInstanceRoot(), "runtime-services");
}
...
// Windows has no POSIX process group: process.kill only ends the single pid,
// orphaning the service's descendants (workers/subprocesses). `taskkill /T /F`
// ends the whole tree. ALWAYS force (/F): taskkill WITHOUT /F cannot signal
// console processes ("can only be terminated forcefully") and silently kills
// nothing — a "graceful" first phase would burn the caller's forceAfterMs doing
// nothing, and under the 10s shutdown drain deadline the force phase would never
// be reached, leaving the entire run-child tree alive across a Ctrl+C restart.
// Pre-tree-kill behavior (process.kill => TerminateProcess) was equally abrupt.
// taskkill exits non-zero when the pid is already gone (fine); if taskkill
// itself is unavailable (ENOENT — sanitized PATH), fall back to a direct kill so
// at least the recorded pid dies.
async function killProcessTreeWindows(pid: number): Promise<void> {
  try {
    ...
    // non-zero exit: pid already gone — nothing to do.
  }
}
...
export async function terminateLocalService(
  ...
  const targetProcessGroup = !isWindows && record.processGroupId && record.processGroupId > 0;
  try {
    if (isWindows) {
      // Tree-kill, force-first (no graceful tier on win32 — see above).
      await killProcessTreeWindows(record.pid);
  ...
  try {
    if (isWindows) {
      await killProcessTreeWindows(record.pid); // retry the forced tree kill
    } else if (targetProcessGroup) {
