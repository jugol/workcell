 ...
 *    (`~/.workcell/plugins/`) and `node_modules` for packages matching
 *    the `workcell-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published Workcell plugins.
 * Packages matching this pattern are considered Workcell plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "workcell-plugin-";
...
export function buildPluginWorkerEnv(input: {
  manifest: Pick<WorkcellPluginManifestV1, "capabilities">;
  instanceInfo: { deploymentMode?: string | null; deploymentExposure?: string | null };
  processEnv?: NodeJS.ProcessEnv;
}): Record<string, string> {
  const processEnv = input.processEnv ?? process.env;
  const env: Record<string, string> = {
    WORKCELL_DEPLOYMENT_MODE: input.instanceInfo.deploymentMode ?? "",
    WORKCELL_DEPLOYMENT_EXPOSURE: input.instanceInfo.deploymentExposure ?? "",
  };
...
export interface DiscoveredPlugin {
  ...
  manifest: WorkcellPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.workcell/plugins/ local directory
  | "npm"               // npm packages matching workcell-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: WorkcellPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the workcell-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "workcell-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: WorkcellPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover Workcell plugins installed as npm packages in the current
   * Node.js environment matching the "workcell-plugin-*" naming convention.
   *
   ...
   * the "workcellPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a Workcell plugin.
   * Throws if the package is a Workcell plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<WorkcellPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: WorkcellPluginManifestV1;
    newManifest: WorkcellPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the Workcell plugin naming convention.
 * Accepts both the "workcell-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @workcell/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "workcellPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const workcellPlugin = pkgJson["workcellPlugin"];
  if (
    workcellPlugin !== null &&
    typeof workcellPlugin === "object" &&
    !Array.isArray(workcellPlugin)
  ) {
    const manifestRelPath = (workcellPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: WorkcellPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumWorkcellVersion;
}
 ...
 *   packageName: "workcell-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: WorkcellPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as WorkcellPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a Workcell plugin (no manifest found).`,
      );
    }
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<WorkcellPluginManifestV1> {
    let raw: unknown;
  ...
  async function loadManifestFromPackageRoot(
    packageRoot: string,
  ): Promise<WorkcellPluginManifestV1 | null> {
  ...
  async function refreshPluginManifestFromPackage(
    ...
    if (!manifest) {
      throw new Error(`Plugin package ${plugin.packageName} no longer exposes a Workcell manifest`);
    }
   ...
   * if the package is not a Workcell plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasWorkcellPlugin = "workcellPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasWorkcellPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<WorkcellPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: WorkcellPluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @workcell/shared exports). Run those workers through
