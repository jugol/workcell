...
// Build the round-0 "generate" prompt: a brain drafts its OWN candidate answer
// independently (it never sees the other brain's draft — divergence is the point).
function buildGeneratePrompt(brain: "A" | "B", task: string): string {
  return `You are Brain ${brain} — one of two minds inside a single agent. Independently produce YOUR best, complete, concrete answer to the task. Do not wait for or assume anything about the other brain.\n\nTask:\n${task}\n\nReturn your full answer.`;
}

// Build the "synthesize" prompt: the synthesizer brain reads BOTH candidate
// answers and merges them into one stronger final answer (OpenRouter Fusion's
// load-bearing step — keep what each got right, drop the rest, resolve conflicts).
function buildSynthesizePrompt(brain: "A" | "B", task: string, candidates: string[]): string {
  const blocks = candidates
    .map((c, i) => `Candidate ${i + 1}:\n---\n${c}\n---`)
    .join("\n\n");
  return `You are Brain ${brain}, acting as the SYNTHESIZER inside a single agent. Two minds independently answered the same task; reconcile their candidates into ONE best final answer — take what each got right, drop weak or wrong parts, and resolve any conflicts on the merits.\n\nTask:\n${task}\n\n${blocks}\n\nReturn the single best final answer (not a comparison or commentary).`;
}
...
export function buildLiveDeliberationInvoke(
  ...
  return async ({ brain, role, adapter, model, task, candidates, round }) => {
    const prompt =
      role === "synthesize"
        ? buildSynthesizePrompt(brain, task, candidates)
        : buildGeneratePrompt(brain, task);
