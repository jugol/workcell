...
import type { AdapterRuntimeServiceReport } from "@workcell/adapter-utils";
import type { Db } from "@workcell/db";
import { executionWorkspaces, projectWorkspaces, workspaceRuntimeServices } from "@workcell/db";
import {
...
function discoverWorkspacePackagePaths(rootDir: string): Map<string, string> {
  ...
  const ignoredDirNames = new Set([".git", ".workcell", "dist", "node_modules"]);
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.WORKCELL_WORKSPACE_CWD = input.worktreePath;
  env.WORKCELL_WORKSPACE_PATH = input.worktreePath;
  env.WORKCELL_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.WORKCELL_WORKSPACE_BRANCH = input.branchName;
  env.WORKCELL_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.WORKCELL_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.WORKCELL_WORKSPACE_SOURCE = input.base.source;
  env.WORKCELL_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.WORKCELL_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.WORKCELL_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.WORKCELL_PROJECT_ID = input.base.projectId ?? "";
  env.WORKCELL_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.WORKCELL_AGENT_ID = input.agent.id ?? "";
  env.WORKCELL_AGENT_NAME = input.agent.name;
  env.WORKCELL_COMPANY_ID = input.agent.companyId;
  env.WORKCELL_ISSUE_ID = input.issue?.id ?? "";
  env.WORKCELL_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.WORKCELL_ISSUE_TITLE = input.issue?.title ?? "";
  env.WORKCELL_ISSUE_WORK_MODE = input.issue?.workMode ?? "";
  return env;
}
...
function buildExecutionWorkspaceCleanupEnv(input: {
  ...
  env.WORKCELL_WORKSPACE_CWD = input.workspace.cwd ?? "";
  env.WORKCELL_WORKSPACE_PATH = input.workspace.cwd ?? "";
  env.WORKCELL_WORKSPACE_WORKTREE_PATH =
    input.workspace.providerRef ?? input.workspace.cwd ?? "";
  env.WORKCELL_WORKSPACE_BRANCH = input.workspace.branchName ?? "";
  env.WORKCELL_WORKSPACE_BASE_CWD = input.projectWorkspaceCwd ?? "";
  env.WORKCELL_WORKSPACE_REPO_ROOT = input.projectWorkspaceCwd ?? "";
  env.WORKCELL_WORKSPACE_REPO_URL = input.workspace.repoUrl ?? "";
  env.WORKCELL_WORKSPACE_REPO_REF = input.workspace.baseRef ?? "";
  env.WORKCELL_PROJECT_ID = input.workspace.projectId ?? "";
  env.WORKCELL_PROJECT_WORKSPACE_ID = input.workspace.projectWorkspaceId ?? "";
  env.WORKCELL_ISSUE_ID = input.workspace.sourceIssueId ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".workcell", "worktrees");
  ...
  for (const row of projectWorkspaceRows) {
    ...
    try {
      const refs = await startRuntimeServicesForWorkspaceControl({
        db,
        actor: { id: null, name: "Workcell", companyId: row.companyId },
        issue: null,
        workspace: {
          baseCwd: row.cwd,
