...
function buildOrgTreeFromManifest(agents: CompanyPortabilityManifest["agents"]): OrgNode[] {
  const ROLE_LABELS: Record<string, string> = {
    orchestrator: "Orchestrator", lead: "Lead", pm: "PM",
    engineer: "Engineer", designer: "Designer", researcher: "Researcher",
    writer: "Writer", qa: "QA", security: "Security", devops: "DevOps",
    general: "General",
    // Legacy / external roles — an imported company may still use C-suite titles.
    ceo: "Chief Executive", cto: "Technology", cmo: "Marketing",
    cfo: "Finance", coo: "Operations", vp: "VP", manager: "Manager", agent: "Agent",
  };
...
function classifyPortableFileKind(pathValue: string): CompanyPortabilityExportPreviewResult["fileInventory"][number]["kind"] {
  ...
  if (normalized === ".workcell.yaml" || normalized === ".workcell.yml") return "extension";
...
function readSkillKey(frontmatter: Record<string, unknown>) {
  ...
  const workcell = isPlainRecord(metadata?.workcell) ? metadata?.workcell as Record<string, unknown> : null;
  return normalizeSkillKey(
    asString(frontmatter.key)
    ?? asString(frontmatter.skillKey)
    ?? asString(metadata?.skillKey)
    ?? asString(metadata?.canonicalKey)
    ?? asString(metadata?.workcellSkillKey)
    ?? asString(workcell?.skillKey)
    ?? asString(workcell?.key),
  );
}
...
function deriveManifestSkillKey(
  ...
  if (sourceKind === "workcell_bundled") {
    return `workcell/workcell/${slug}`;
  }
...
const ADAPTER_DEFAULT_RULES_BY_TYPE: Record<string, Array<{ path: string[]; value: unknown }>> = {
  ...
  openclaw_gateway: [
    ...
    { path: ["sessionKey"], value: "workcell" },
...
function buildLegacyRoutineTriggerFromRecurrence(
  ...
  if (!frequency) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence without frequency; add .workcell.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  if (interval < 1) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid interval; add .workcell.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (startsAt && !zonedStartsAt) {
    errors.push(`Recurring task ${issue.slug} has an invalid legacy startsAt/timezone combination; add .workcell.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (hour < 0 || hour > 23 || minute < 0 || minute > 59) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid time; add .workcell.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }

  if (issue.legacyRecurrence.until != null || issue.legacyRecurrence.count != null) {
    warnings.push(`Recurring task ${issue.slug} uses legacy recurrence end bounds; Workcell will import the routine trigger without those limits.`);
  }
  ...
  if (frequency === "hourly") {
    ...
    if (Array.isArray(issue.legacyRecurrence.weekdays) || Array.isArray(issue.legacyRecurrence.monthDays) || Array.isArray(issue.legacyRecurrence.months)) {
      errors.push(`Recurring task ${issue.slug} uses unsupported legacy daily recurrence constraints; add .workcell.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence with interval > 1; add .workcell.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (cronWeekdays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence without weekdays; add .workcell.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} * * ${normalizeCronList(cronWeekdays)}`;
  } else if (frequency === "monthly") {
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence with interval > 1; add .workcell.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    if (Array.isArray(issue.legacyRecurrence.ordinalWeekdays) && issue.legacyRecurrence.ordinalWeekdays.length > 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy ordinal monthly recurrence; add .workcell.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence without monthDays; add .workcell.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence with interval > 1; add .workcell.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    const months = Array.isArray(issue.legacyRecurrence.months)
      ? issue.legacyRecurrence.months
    ...
    if (months.length === 0 || monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence without month/monthDay anchors; add .workcell.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} ${normalizeCronList(monthDays.map(String))} ${normalizeCronList(months.map(String))} *`;
  } else {
    errors.push(`Recurring task ${issue.slug} uses unsupported legacy recurrence frequency "${frequency}"; add .workcell.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
...
function filterExportFiles(
  ...
  const extensionEntry = filtered[workcellExtensionPath];
  if (selectedFiles.has(workcellExtensionPath) && typeof extensionEntry === "string") {
    filtered[workcellExtensionPath] = filterPortableExtensionYaml(extensionEntry, selectedFiles);
  }

  return filtered;
}

function findWorkcellExtensionPath(files: Record<string, CompanyPortabilityFileEntry>) {
  if (typeof files[".workcell.yaml"] === "string") return ".workcell.yaml";
  if (typeof files[".workcell.yml"] === "string") return ".workcell.yml";
  return Object.keys(files).find((entry) => entry.endsWith("/.workcell.yaml") || entry.endsWith("/.workcell.yml")) ?? null;
}
...
async function buildSkillSourceEntry(skill: CompanySkill) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "workcell_bundled") {
    const commit = await resolveBundledSkillsCommit();
    return {
      kind: "github-dir",
      repo: "workcell/workcell",
      path: `skills/${skill.slug}`,
      commit,
      trackingRef: "master",
      url: `https://github.com/workcell/workcell/tree/master/skills/${skill.slug}`,
    };
  }
...
function shouldReferenceSkillOnExport(skill: CompanySkill, expandReferencedSkills: boolean) {
  ...
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "workcell_bundled") return true;
...
async function withSkillSourceMetadata(skill: CompanySkill, markdown: string) {
  ...
  metadata.workcellSkillKey = skill.key;
  metadata.workcell = {
    ...(isPlainRecord(metadata.workcell) ? metadata.workcell : {}),
    skillKey: skill.key,
    slug: skill.slug,
  };
  const frontmatter = {
...
function buildManifestFromPackageFiles(
  ...
  const workcellExtensionPath = findWorkcellExtensionPath(normalizedFiles);
  const workcellExtension = workcellExtensionPath
    ? parseYamlFile(readPortableTextFile(normalizedFiles, workcellExtensionPath) ?? "")
    : {};
  const workcellCompany = isPlainRecord(workcellExtension.company) ? workcellExtension.company : {};
  const workcellSidebar = normalizePortableSidebarOrder(workcellExtension.sidebar);
  const workcellAgents = isPlainRecord(workcellExtension.agents) ? workcellExtension.agents : {};
  const workcellProjects = isPlainRecord(workcellExtension.projects) ? workcellExtension.projects : {};
  const workcellTasks = isPlainRecord(workcellExtension.tasks) ? workcellExtension.tasks : {};
  const workcellRoutines = isPlainRecord(workcellExtension.routines) ? workcellExtension.routines : {};
  const companyName =
  ...
  const manifest: CompanyPortabilityManifest = {
    ...
    company: {
      ...
      brandColor: asString(workcellCompany.brandColor),
      planReportLanguage: asString(workcellCompany.planReportLanguage) ?? undefined,
      logoPath: asString(workcellCompany.logoPath) ?? asString(workcellCompany.logo),
      attachmentMaxBytes:
        typeof workcellCompany.attachmentMaxBytes === "number" && Number.isFinite(workcellCompany.attachmentMaxBytes)
          ? Math.max(1, Math.floor(workcellCompany.attachmentMaxBytes))
          : null,
      requireBoardApprovalForNewAgents:
        typeof workcellCompany.requireBoardApprovalForNewAgents === "boolean"
          ? workcellCompany.requireBoardApprovalForNewAgents
          : readCompanyApprovalDefault(companyFrontmatter),
      feedbackDataSharingEnabled:
        typeof workcellCompany.feedbackDataSharingEnabled === "boolean"
          ? workcellCompany.feedbackDataSharingEnabled
          : false,
      feedbackDataSharingConsentAt:
        typeof workcellCompany.feedbackDataSharingConsentAt === "string"
          ? workcellCompany.feedbackDataSharingConsentAt
          : null,
      feedbackDataSharingConsentByUserId:
        asString(workcellCompany.feedbackDataSharingConsentByUserId),
      feedbackDataSharingTermsVersion:
        asString(workcellCompany.feedbackDataSharingTermsVersion),
    },
    sidebar: workcellSidebar,
    agents: [],
    skills: [],
    projects: [],
    issues: [],
  ...
  for (const agentPath of agentPaths) {
    ...
    const extension = isPlainRecord(workcellAgents[slug]) ? workcellAgents[slug] : {};
  ...
  for (const projectPath of projectPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(workcellProjects[slug]) ? workcellProjects[slug] : {};
  ...
  for (const taskPath of taskPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(workcellTasks[slug]) ? workcellTasks[slug] : {};
    const routineExtension = normalizeRoutineExtension(workcellRoutines[slug]);
    const routineExtensionRaw = isPlainRecord(workcellRoutines[slug]) ? workcellRoutines[slug] : {};
...
export function companyPortabilityService(db: Db, storage?: StorageService) {
  ...
  const strictSecretsMode = process.env.WORKCELL_SECRETS_STRICT_MODE === "true";
  ...
  async function assertImportAdapterConfigConstraints(
    ...
    // No vendor-specific adapter config constraints remain after adapter strip.
    void adapterType;
    void adapterConfig;
  }
  ...
  async function prepareImportedAgentAdapter(
    ...
    const nextAdapterConfig = writeWorkcellSkillSyncPreference(
    ...
    const candidatePaths = (tree.tree ?? [])
      ...
      .filter((entry) => {
        ...
        return (
          ...
          relative === ".workcell.yaml" ||
          relative === ".workcell.yml"
        );
      });
  ...
  async function exportBundle(
    ...
    const workcellAgentsOut: Record<string, Record<string, unknown>> = {};
    const workcellProjectsOut: Record<string, Record<string, unknown>> = {};
    const workcellTasksOut: Record<string, Record<string, unknown>> = {};
    const unportableTaskWorkspaceRefs = new Map<string, { workspaceId: string; taskSlugs: string[] }>();
    const workcellRoutinesOut: Record<string, Record<string, unknown>> = {};
    ...
    if (include.agents) {
      for (const agent of agentRows) {
        ...
        const desiredSkills = readWorkcellSkillSyncPreference(
        ...
        workcellAgentsOut[slug] = isPlainRecord(extension) ? extension : {};
      }
    }
    ...
    for (const project of selectedProjectRows) {
      ...
      workcellProjectsOut[slug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const issue of selectedIssueRows) {
      ...
      workcellTasksOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const routine of selectedRoutineRows) {
      ...
      workcellRoutinesOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }

    const workcellExtensionPath = ".workcell.yaml";
    const workcellAgents = Object.fromEntries(
      Object.entries(workcellAgentsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const workcellProjects = Object.fromEntries(
      Object.entries(workcellProjectsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const workcellTasks = Object.fromEntries(
      Object.entries(workcellTasksOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const workcellRoutines = Object.fromEntries(
      Object.entries(workcellRoutinesOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    files[workcellExtensionPath] = buildYamlFile(
      {
        schema: "workcell/v1",
        company: stripEmptyValues({
          brandColor: company.brandColor ?? null,
          planReportLanguage: company.planReportLanguage ?? undefined,
        ...
        agents: Object.keys(workcellAgents).length > 0 ? workcellAgents : undefined,
        projects: Object.keys(workcellProjects).length > 0 ? workcellProjects : undefined,
        tasks: Object.keys(workcellTasks).length > 0 ? workcellTasks : undefined,
        routines: Object.keys(workcellRoutines).length > 0 ? workcellRoutines : undefined,
      },
      { preserveEmptyStrings: true },
    );

    let finalFiles = filterExportFiles(files, input.selectedFiles, workcellExtensionPath);
  ...
  async function importBundle(
    ...
    if (input.target.mode === "new_company") {
      ...
      const created = await companies.create({
        ...
        planReportLanguage: include.company ? (sourceManifest.company?.planReportLanguage ?? undefined) : undefined,
      ...
      if (include.company && sourceManifest.company && mode === "board_full") {
        const updated = await companies.update(targetCompany.id, {
          ...
          planReportLanguage: sourceManifest.company.planReportLanguage,
