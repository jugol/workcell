 ...
 */
import { pluginManifestV1Schema } from "@workcell/shared";
import type { WorkcellPluginManifestV1 } from "@workcell/shared";
import { PLUGIN_API_VERSION } from "@workcell/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): WorkcellPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as WorkcellPluginManifestV1,
        };
      }
