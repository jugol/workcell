...
// WC-57: cost mapping extracted to a shared module so the pair-turn invoker
// (WC-58) bills identically to the heartbeat ledger.
import { normalizeBilledCostCents, normalizeLedgerBillingType } from "./cost-mapping.js";
import { getServerAdapter, listAdapterModelProfiles, runningProcesses } from "../adapters/index.js";
// WC-67: expose agent-declared MCP servers to the adapter (serialization is
// itself feature-gated in the adapter, so this is a no-op by default).
import { extractMcpServersFromConfig } from "@workcell/adapter-utils";
import type {
...
import { parseReviewVerdict } from "./agent-deliberation.js";
import {
  runDeliberationPasses,
  type DeliberationPassInput,
  type DeliberationPassOutput,
} from "./heartbeat-deliberation-pass.js";
import { parseObject, asBoolean, asNumber, appendWithByteCap, MAX_EXCERPT_BYTES } from "../adapters/utils.js";
import { costService } from "./costs.js";
import { autoStartScopedIssue } from "./issue-auto-start.js";
import { trackAgentFirstHeartbeat } from "@workcell/shared/telemetry";
...
import { issueRequiresProofForDone, issueService } from "./issues.js";
import { promptSafeDesignUrl, workProductService, type IssueDesignGate } from "./work-products.js";
import {
...
import {
  readWorkcellSkillSyncPreference,
  writeWorkcellSkillSyncPreference,
} from "@workcell/adapter-utils/server-utils";
import { extractSkillMentionIds, isUuidLike } from "@workcell/shared";
...
const DEFERRED_WAKE_CONTEXT_KEY = "_workcellWakeContext";
const WAKE_COMMENT_IDS_KEY = "wakeCommentIds";
const WORKCELL_WAKE_PAYLOAD_KEY = "workcellWake";
const WORKCELL_HARNESS_CHECKOUT_KEY = "workcellHarnessCheckedOut";
const DETACHED_PROCESS_ERROR_CODE = "process_detached";
// Execution-layer dual-brain skips recovery/evaluation issues: those are cheap,
// status-only runs (watchdog "Review silent active run", stranded-issue
// recovery, etc.) that must not pay for a 2x work->review loop.
const DELIBERATION_EXCLUDED_ORIGIN_KINDS = new Set<string>(Object.values(RECOVERY_ORIGIN_KINDS));
const REPO_ONLY_CWD_SENTINEL = "/__workcell_repo_only__";
const MANAGED_WORKSPACE_GIT_CLONE_TIMEOUT_MS = 10 * 60 * 1000;
// Production-readiness Wave 2 (REL — main-run timeout): the default max
// wall-clock (seconds) applied to a heartbeat run when the agent's resolved
// adapter config leaves `timeoutSec` unset or 0. The adapters treat 0 as "no
// kill timer", so a wedged local CLI would otherwise run forever (WC-211 fixed
// only the deliberation path). An explicit non-zero agent timeout is always
// preserved — this default ONLY fills in the unset/0 case.
//
// 3h, not 30min: real implementation runs routinely exceed 30 minutes (a
// verified healthy LOR run took 2h12m), so the old 1800s default killed
// legitimate work with "run timeout (1800)". Hung-but-silent runs are the
// silent-run watchdog's job (suspicious at 1h, critical at 4h of no output) —
// the wall-clock here only backstops a run that keeps producing output but
// never ends.
const DEFAULT_RUN_TIMEOUT_SEC = 10800;
const MAX_INLINE_WAKE_COMMENTS = 8;
const MAX_INLINE_WAKE_COMMENT_BODY_CHARS = 4_000;
const MAX_INLINE_WAKE_COMMENT_BODY_TOTAL_CHARS = 12_000;
// WC-217 (Item 2): heartbeat_runs is the highest-volume per-tenant table (one
// row per agent tick). heartbeatService.list runs `ORDER BY created_at DESC` and
// previously executed with NO LIMIT whenever the caller omitted one — and the
// GET /companies/:id/heartbeat-runs route passes `undefined` when the client
// sends no `limit` query param, so an unbounded full-table scan + serialization
// was one missing query param away on a hot endpoint. Apply a defensive default
// (newest N) when no limit is given, and clamp any explicit limit to the same
// ceiling. Override-able via WORKCELL_HEARTBEAT_RUN_LIST_MAX for the rare bulk
// caller. Mirrors the DEFAULT/MAX activity-list cap.
const HEARTBEAT_RUN_LIST_DEFAULT_LIMIT = 1000;
function heartbeatRunListMaxLimit(): number {
  const raw = Number.parseInt(process.env.WORKCELL_HEARTBEAT_RUN_LIST_MAX ?? "", 10);
  return Number.isFinite(raw) && raw > 0 ? raw : HEARTBEAT_RUN_LIST_DEFAULT_LIMIT;
}
function normalizeHeartbeatRunListLimit(limit: number | undefined): number {
  const max = heartbeatRunListMaxLimit();
  if (!Number.isFinite(limit) || limit === undefined) return max;
...
// Context-window exhaustion (codex "ran out of room"): the adapter already
// cleared the resume session (clearSession), so an immediate bounded retry
// starts a FRESH thread and the issue continuation summary carries progress
// forward — no manual "start a new thread", no permanent loop.
export const CONTEXT_EXHAUSTED_RETRY_REASON = "context_exhausted_fresh_session";
export const CONTEXT_EXHAUSTED_RETRY_WAKE_REASON = "context_exhausted_retry";
const CONTEXT_EXHAUSTED_RETRY_MAX_ATTEMPTS = 2;
const CONTEXT_EXHAUSTED_RETRY_DELAY_MS = 2_000;
...
function isWorkcellRuntimeEnvKey(key: string) {
  return key.startsWith("WORKCELL_");
}

function stripWorkcellRuntimeEnvBindings(envValue: unknown): Record<string, unknown> | null {
  const record = parseObject(envValue);
  const filtered = Object.fromEntries(
    Object.entries(record).filter(([key]) => !isWorkcellRuntimeEnvKey(key)),
  );
  return Object.keys(filtered).length > 0 ? filtered : null;
}

function stripWorkcellRuntimeEnvFromAdapterConfig(config: Record<string, unknown>): Record<string, unknown> {
  if (!Object.prototype.hasOwnProperty.call(config, "env")) return config;
  return {
    ...config,
    env: stripWorkcellRuntimeEnvBindings(config.env) ?? {},
  };
}
...
export async function resolveExecutionRunAdapterConfig(input: {
  ...
  const executionRunConfig = stripWorkcellRuntimeEnvFromAdapterConfig(input.executionRunConfig);
  const projectEnv = stripWorkcellRuntimeEnvBindings(input.projectEnv);
  const routineEnv = stripWorkcellRuntimeEnvBindings(input.routineEnv);
...
export function applyRunScopedMentionedSkillKeys(
  ...
  const existingPreference = readWorkcellSkillSyncPreference(config);
  return writeWorkcellSkillSyncPreference(config, [
...
/**
 * Production-readiness Wave 2 (REL — main-run timeout): resolve the wall-clock
 * timeout (seconds) for a heartbeat run from the agent's resolved
 * `adapterConfig.timeoutSec`. Returns the configured value when it is a positive
 * number; otherwise returns DEFAULT_RUN_TIMEOUT_SEC. The adapters treat 0/unset
 * as "no kill timer" (resolveAdapterExecutionTargetTimeoutSec keeps 0 for
 * local/SSH), so without this default a hung CLI runs forever. An explicit
 * positive agent timeout is ALWAYS preserved — this only fills the unset/0 case.
 * Pure + exported so the default-vs-preserve rule is unit-testable.
 */
export function resolveRunTimeoutSec(configuredTimeoutSec: unknown): number {
  const configured = asNumber(configuredTimeoutSec, 0);
  return configured > 0 ? Math.trunc(configured) : DEFAULT_RUN_TIMEOUT_SEC;
}
...
// A self-contained 시안 (design_attach html mode) is stored as a data:text/html
// URL holding the ENTIRE mockup — often hundreds of KB of URL-encoded HTML.
// When an agent fetches that work product and echoes it (e.g. promoting a 시안),
// the whole blob floods the run log. Elide the BODY of any oversized data: URL,
// keeping the media type + an omitted-size marker so the log stays readable.
// (data:image base64 in the assistant message JSON is handled separately above;
// this catches data: URLs anywhere else, including data:text/html.)
// Body excludes `;` too: a URL-encoded data: body never contains a raw `;`
// (encodeURIComponent emits %3B), so this lets a PowerShell-style `; field=`
// separator after the url survive instead of being swallowed into the blob.
const INLINE_DATA_URL_BODY_RE = /(data:[a-zA-Z][\w.+-]*\/[\w.+-]+(?:;[\w%=.+-]+)*,)([^\s"'<>\\;]{1024,})/g;
export function elideOversizedDataUrls(chunk: string) {
  return chunk.replace(INLINE_DATA_URL_BODY_RE, (_match, prefix: string, body: string) =>
    `${prefix}…[omitted data: url body: ${body.length} chars]`,
  );
}

export function compactRunLogChunk(chunk: string, maxChars = MAX_PERSISTED_LOG_CHUNK_CHARS) {
  const normalized = redactSensitiveText(elideOversizedDataUrls(redactInlineBase64ImageData(chunk)));
  ...
  const marker = `\n[workcell truncated run log chunk: omitted ${omittedChars} chars]\n`;
...
function enrichWakeContextSnapshot(input: {
  ...
  if (wakeCommentIds.length > 0) {
    ...
    delete contextSnapshot[WORKCELL_WAKE_PAYLOAD_KEY];
...
export function mergeCoalescedContextSnapshot(
  ...
  if (mergedCommentIds.length > 0) {
    ...
    delete merged[WORKCELL_WAKE_PAYLOAD_KEY];
  }
...
async function buildWorkcellWakePayload(input: {
  db: Db;
  companyId: string;
  contextSnapshot: Record<string, unknown>;
  continuationSummary?:
  ...
  return {
    ...
    checkedOutByHarness: input.contextSnapshot[WORKCELL_HARNESS_CHECKOUT_KEY] === true,
...
export function buildWorkcellTaskMarkdown(input: {
  issue: {
    id: string;
    identifier: string | null;
    title: string;
  ...
  // Company report language (resolvePlanReportLanguageLabel output; null =
  // default English, no directive). Injected so agents write user-facing
  // output in the team's language instead of defaulting to English.
  reportLanguageLabel?: string | null;
  // The issue's design gate (deriveDesignGateForIssue output). The gate is the
  // design-first contract: the directive must reach the agent IN THE TASK
  // PROMPT — the heartbeat-context API carrying it was not enough, because an
  // agent that never fetches that endpoint happily implemented screens with no
  // approved 시안. Null/absent = no design signal (planning issues, fetch
  // failure) → byte-identical prompt to before.
  designGate?: Pick<
    IssueDesignGate,
    "directive" | "approved" | "developmentHold" | "authoritativeDesign"
  > | null;
  // True only when the woken agent IS the current execution-stage participant
  // (the reviewer) during in_review. The review-stage QA-compare directive is
  // reviewer-only; a non-participant woken by a comment during in_review gets
  // the plain build-against directive instead. Defaults false → no flip.
  ...
  if (issue) {
    ...
    } else if (issueRequiresProofForDone(issue)) {
      // Execution mode (standard / non-planning): the work is not complete until a
      // proof bundle exists. Done is blocked server-side without a `proof` work
      // product (see proof-gated Done), so instruct the agent to produce one.
      // Proof-exempt issues (recovery-origin evaluations, e.g. watchdog
      // "Review silent active run") deliberately get NO directive: their Done
      // is not proof-gated, and telling a cheap status-only recovery run to
      // attach artifacts it is forbidden from writing made it block itself
      // and spawn a follow-up issue just to file paperwork.
      lines.push(
        "",
        "Proof-of-work directive:",
        "Before this issue can be completed (moved to Done), attach a proof bundle so a reviewer can verify the work.",
        "Run the project's verification (build, tests, and any relevant checks), then record the outcome by POST-ing to `POST /issues/:id/work-products` with `type` set to `proof` (include a `summary` of what you verified and the key results).",
        "An issue with no `proof`-type work product cannot be moved to Done.",
        ...(input.designGate?.approved
          ? [
              "This issue is design-gated: the proof must also include screenshot evidence that the implemented screen matches the approved source-of-truth design.",
            ]
          : []),
      );
    }
    // Design-first contract: the design gate's directive rides in the task
    // prompt itself (HOLD / build-against-the-design), not just in the
    // heartbeat-context API response. During the review stage the directive
    // flips from "build against it" to "verify the in-app screen against it" —
    // the design (시안) is the spec QA compares the implementation to.
    const designGate = issue.workMode === "planning" ? null : input.designGate ?? null;
    if (designGate?.directive) {
      if (
        issue.status === "in_review" &&
        input.isReviewParticipant === true &&
        designGate.approved &&
        designGate.authoritativeDesign
      ) {
        const design = designGate.authoritativeDesign;
        // Never inline a data:/oversized 시안 url — it would blow the prompt past
        // the model context window (see promptSafeDesignUrl).
        const safeUrl = promptSafeDesignUrl(design.url);
        const urlPart = safeUrl ? ` (${safeUrl})` : "";
        lines.push(
          "",
          "Design verification directive (review stage):",
          `This issue is design-gated. The approved source-of-truth design (시안) is "${design.title}"${urlPart} — it is the spec for this screen.`,
          "Before signing off, compare the implemented in-app screen against this design: layout, spacing, typography, colors, component usage, and interaction states.",
          "Attach comparison evidence (a screenshot of the implemented screen alongside the design) to your review comment or as a work product.",
          "When something is wrong, decide which of two distinct paths applies:",
          "(a) The implementation deviates from the approved 시안 (the design is right, the code is wrong) → do NOT approve. Request changes on the EXECUTION stage (move the issue back with a comment listing each mismatch) so the IMPLEMENTER fixes the code. The design wins; the implementation moves.",
          `(b) The approved 시안 ITSELF is inadequate (it does not meet the quality bar, conflicts with the design system, or fails to satisfy the issue) → the implementation cannot save it. Call POST /work-products/${design.id}/design-review/request-changes with a reason so the DESIGNER redoes the 시안. You (the current reviewer) are permitted to trigger this design-level request-changes while the issue is in_review.`,
          "Pick exactly one path per problem: code-fix (a) vs design-redo (b). Do not request code changes for a design that is fundamentally wrong, and do not bounce a correct design back to the designer over an implementation bug.",
        );
      } else {
        lines.push("", "Design directive:", designGate.directive);
      }
    }
  ...
  if (input.reportLanguageLabel) {
    lines.push(
      "",
      "Language directive:",
      `Write ALL user-facing output in ${input.reportLanguageLabel}: issue comments, completion/status reports, plan and report documents, confirmation prompts, and summaries.`,
      "Keep code, identifiers, commands, file paths, and API payload values as-is.",
    );
  }
  // Completion discipline — injected EVERY run for EVERY agent (the enforcement
  // layer that does not depend on a per-agent charter). Two recurring failure
  // modes: declaring work done without looking at the result, and never
  // capturing lessons/corrections to memory so the board keeps re-teaching.
  lines.push(
    "",
    "Before you mark this done:",
    "- VERIFY the actual result, not just that it compiles or tests pass. For any UI/screen/visual change, render it and LOOK (screenshot/preview/the 시안) as the final step; a green build is not proof the screen is right. If you truly cannot see it, say so — do not claim done.",
    "- If the board corrected you, gave a rule/preference, or you hit a reusable lesson or gotcha THIS run, `memory_remember` it now (with why + how-to-apply) so you never repeat it. Capture the lesson, not just the progress log.",
  );
...
// A run whose child died together with a previous server process (detected by the
// startup reap) was interrupted by the restart, not crashed. Present it honestly
// so the board doesn't read an operational restart as a task failure.
function buildRestartInterruptionMessage(run: {
  processPid: number | null;
  processGroupId: number | null;
}, options?: { descendantOnly?: boolean }) {
  if (options?.descendantOnly && run.processGroupId) {
    return `Interrupted by server restart -- parent pid ${run.processPid ?? "unknown"} exited; descendant group ${run.processGroupId} was terminated`;
  }
  return "Interrupted by server restart -- the run's child process ended with the previous server";
}
...
export function heartbeatService(db: Db, options: HeartbeatServiceOptions = {}) {
  ...
  function monitorRecoveryComment(input: {
    ...
    return [
      `Workcell cleared the scheduled external-service monitor for ${label} because ${reason}.`,
      "",
  ...
  async function applyIssueMonitorPatchWithOcc(
    issueId: string,
    now: Date,
    buildPatch: (issue: typeof issues.$inferSelect) => Partial<typeof issues.$inferInsert>,
  ): Promise<boolean> {
    // WC-166: the monitor's executionState write merges into the issue's CURRENT
    // state. A route PATCH committing between the monitor's claim and this write would
    // be clobbered by a stale merge. Re-read fresh and guard the write on
    // execution_state_version (bumped by the WC-164 trigger on any executionState
    // change), retrying on a concurrent change so the racing route edit survives.
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      const result = await db
        .update(issues)
        .set({ ...buildPatch(issue), updatedAt: now })
        .where(and(eq(issues.id, issueId), eq(issues.executionStateVersion, issue.executionStateVersion)))
  ...
  async function clearIssueMonitorAndRecover(input: {
    ...
    await applyIssueMonitorPatchWithOcc(input.claimed.id, input.now, (issue) =>
      buildIssueMonitorClearedPatch({
        issue,
        policy: input.policy,
        clearReason: input.clearReason,
  ...
  async function dispatchClaimedIssueMonitor(
    ...
    try {
      ...
      await applyIssueMonitorPatchWithOcc(claimed.id, new Date(), (issue) =>
        buildIssueMonitorTriggeredPatch({
          issue,
          policy,
          ...
          .where(
            and(
              ...
              sql`(
                ...
                or ${agentWakeupRequests.payload} -> '_workcellWakeContext' ->> 'issueId' = ${issue.id}
                or ${agentWakeupRequests.payload} -> '_workcellWakeContext' ->> 'taskId' = ${issue.id}
              )`,
            ),
          )
          .limit(1)
  ...
  async function promoteDueScheduledRetries(now = new Date()) {
    ...
    for (const dueRun of dueRuns) {
      try {
        ...
        // WC-156: isolate per-run failures so one bad scheduled-retry promotion
        // can't abort the batch and starve the remaining due retries (matches
        // WC-155 tickScheduledTriggers and the tickDueIssueMonitors guard).
        console.error(
          `[heartbeat] promoteDueScheduledRetries: run ${dueRun.id} failed; skipping`,
          err,
        );
      }
    }
  ...
  async function claimQueuedRun(run: typeof heartbeatRuns.$inferSelect) {
    ...
    const issueId = readNonEmptyString(context.issueId);
    // Tracked for the post-claim issue auto-start below: a verified tree-hold
    // interaction wake may claim THROUGH an active pause hold, but a held
    // issue must still not be auto-started.
    let issueUnderActivePauseHold = false;
    if (issueId) {
      const activePauseHold = await treeControlSvc.getActivePauseHoldGate(run.companyId, issueId);
      issueUnderActivePauseHold = Boolean(activePauseHold);
    ...
    // The run scoped to this issue is now actually running → auto-start a
    // "todo" issue (assigned to this same agent, no human co-assignee) to
    // in_progress so the board stops showing claimed work as "할일".
    // Best-effort + conditional inside the helper; skipped while the issue is
    // under an active pause hold (the board paused it on purpose).
    if (!issueUnderActivePauseHold) {
      await autoStartScopedIssue(db, claimed);
    }
  ...
  async function cancelQueuedRunForBlockedDependencies(
    ...
    const now = new Date();
    const reason =
      "Cancelled because issue dependencies are still blocked; Workcell will wake the assignee when blockers resolve";
  ...
  async function evaluateQueuedRunStaleness(
    ...
    if (
      ...
      const queuedWake = parseObject(context.workcellWake);
      const queuedContinuationSummary =
        readNonEmptyString(parseObject(context.workcellContinuationSummary).body) ??
      ...
      if (!resumeIntent && !wakeCommentId) {
        return {
          ...
          reason:
            ...
            `Approving a design does NOT complete the issue — it only marks the 시안 as the build target; the issue is completed separately. ` +
            `If this issue was finished before its implementation finished, reopen it (set the status back to in_progress) to re-engage the assignee.`,
          details: { issueId, currentStatus: issue.status },
        };
      }
    }
  ...
  async function reapOrphanedRuns(opts?: { staleThresholdMs?: number; reason?: "startup" | "periodic" }) {
    const staleThresholdMs = opts?.staleThresholdMs ?? 0;
    const now = new Date();
    // A startup reap runs while in-memory execution state is empty, so every
    // "running" row is a run the PREVIOUS server was executing when it exited — it
    // was INTERRUPTED by the restart, not crashed. Resume those (up to a bounded
    // count) instead of hard-failing them as "Process lost" — otherwise a run
    // stays stuck as a dead board 할 일 after each dev-server restart. A periodic
    // reap (live server) catching a vanished child is a genuine loss: retry once.
    const isRestartInterruption = (opts?.reason ?? "periodic") === "startup";
    const RESTART_RESUME_LIMIT = 5;
    ...
    for (const { run, adapterType, adapterConfig } of activeRuns) {
      ...
      const resumeAttempts = run.processLossRetryCount ?? 0;
      const hasTrackedHandle = tracksLocalChild && (!!run.processPid || !!run.processGroupId);
      // Restart-interruption resumes up to RESTART_RESUME_LIMIT (operational, not
      // the run's fault, so a higher cap than the single live-loss retry); a
      // live-server loss retries once. The count carried onto the requeued run
      // (enqueueProcessLossRetry sets processLossRetryCount+1) bounds both against
      // a crash loop across repeated restarts.
      const shouldRetry = hasTrackedHandle && (
        isRestartInterruption ? resumeAttempts < RESTART_RESUME_LIMIT : resumeAttempts < 1
      );
      const baseMessage = isRestartInterruption
        ? buildRestartInterruptionMessage(run, descendantOnlyCleanup ? { descendantOnly: true } : undefined)
        : buildProcessLossMessage(run, descendantOnlyCleanup ? { descendantOnly: true } : undefined);
      const finalMessage = shouldRetry
        ? `${baseMessage}${isRestartInterruption ? "; resuming" : "; retrying once"}`
        : baseMessage;
      ...
      await appendRunEvent(finalizedRun, await nextRunEventSeq(finalizedRun.id), {
        eventType: "lifecycle",
        stream: "system",
        level: isRestartInterruption ? "warn" : "error",
        message: shouldRetry
          ? `${baseMessage}; queued ${isRestartInterruption ? "resume" : "retry"} ${retriedRun?.id ?? ""}`.trim()
          : baseMessage,
        payload: {
  ...
  async function startNextQueuedRunForAgent(agentId: string) {
    return withAgentStartLock(agentId, async () => {
      ...
      for (const claimedRun of claimedRuns) {
        trackRunExecution(claimedRun.id);
      }
      return claimedRuns;
    });
  }

  function trackRunExecution(runId: string) {
    const execution = executeRun(runId)
      .catch((err) => {
        logger.error({ err, runId }, "queued heartbeat execution failed");
      })
      .finally(() => {
        activeRunExecutionPromises.delete(runId);
      });
    activeRunExecutionPromises.set(runId, execution);
    return execution;
  }

  async function waitForActiveRunExecutionsToSettle(opts?: { timeoutMs?: number }) {
    const timeoutMs = opts?.timeoutMs ?? 15_000;
    const deadline = Date.now() + timeoutMs;
    for (;;) {
      const executions = [...activeRunExecutionPromises.values()];
      ...
      const activeRuns = await db
        ...
        .limit(1);
      if (activeRuns.length === 0 && activeRunExecutionPromises.size === 0) return;
      if (Date.now() >= deadline) {
        throw new Error("Timed out waiting for active heartbeat run executions to settle");
      }
  ...
  // The design gate's HOLD directive in the task prompt was advisory: execution
  // runs kept dispatching (and implementing screens) while the issue sat at
  // "awaiting an approved 시안". This makes the hold REAL at the dispatch level:
  // an execution run on a design-held issue is cancelled before it starts, and
  // a design-request child routed to a designer is ensured instead. Board
  // design approval already wakes the issue assignee, which closes the loop:
  // hold → designer produces the 시안 on the parent → board approves → assignee
  // wakes → run proceeds.
  //
  // Deliberately NOT blocked: planning issues and recovery-origin evaluations
  // (same exemption as the proof gate), the review stage (QA verifies against
  // the design), designer agents (their job during the hold IS the 시안), and
  // the design-request child itself.
  async function enforceDesignFirstDispatchGate(input: {
    ...
    // The already-resolved gate (computed once per wake and reused by the task
    // prompt) — avoids re-querying deriveDesignGateForIssue here.
    gate: IssueDesignGate | null;
  }): Promise<boolean> {
    const issue = input.issueContext;
    if (issue.status === "in_review" || issue.status === "done" || issue.status === "cancelled") {
      return false;
    }
    if (!issueRequiresProofForDone({ workMode: issue.workMode, originKind: issue.originKind })) {
      return false;
    }
    if (issue.originKind === "design_request") return false;
    if (input.agent.role === "designer") return false;

    const gate = input.gate;
    if (!gate?.developmentHold) return false;

    // Ensure ONE open design-request child routed to a designer (idempotent).
    ...
    if (!existingRequest) {
      ...
      const designRequest = await issuesSvc.create(input.agent.companyId, {
        title: `Design 시안: ${issue.title}`,
        description: [
          `${issue.identifier ?? issue.id} ("${issue.title}") is screen work held by the design-first gate: no approved source-of-truth design exists yet.`,
          "",
          "Produce a HIGH-QUALITY 시안 for this screen and attach it to the PARENT issue (not this one).",
          "",
          "Before designing — ground yourself in the project's design language:",
          `- Review the project's existing APPROVED designs (GET /projects/:projectId/design-artifacts) and reuse their components, layout patterns, and visual language. Do not reinvent what already exists.`,
          `- Pull the project's design system tokens if present (the design_system work product) and build FROM them: the exact color palette, type scale, spacing scale, and component styles — do not invent new ones.`,
          "",
          "Produce the 시안 as a self-contained HTML/CSS mockup (use the design_attach tool with the `html` argument so it renders as an embedded preview — not a bare external url). Hold it to a real product bar:",
          "- SCREEN ↔ PLAN PAIR: keep the 시안 HTML a PURE rendered screen. Put the screen's SPEC — purpose, states, interactions, data — in design_attach's `planMarkdown` (the paired \"화면 기획\"), not baked into the mockup as annotations.",
          "- Clear visual hierarchy and consistent spacing/alignment on a grid; deliberate type scale; a coherent color system (not ad-hoc hexes).",
          "- Every state the screen needs: default, empty, loading, error, and key interaction/hover/focus states.",
          "- Responsive intent (how it reflows) and accessible contrast.",
          "- Match the project design system exactly; flag any new pattern you must introduce.",
          "",
          "Before submitting — RENDER AND SELF-REVIEW your own 시안; do not submit HTML you have not seen:",
          "- When you attach the 시안 (design_attach), Workcell automatically RENDERS it to a screenshot and re-wakes you with that PNG as image input — so on your next run you will SEE the actual rendered pixels of the design you attached. LOOK at it; do not trust the markup blind.",
          "- Review the rendered screenshot against the visual quality bar above AND the project's design-system tokens (palette, type scale, spacing). If it falls short, revise the 시안 and re-attach (you get a fresh render). If it clears the bar, submit it for board review. This visual self-review loop is capped at 3 rounds per 시안; after that you may submit anyway.",
          "- If no screenshot reaches you, at minimum self-critique the HTML line-by-line against the quality bar and the design-system tokens — confirm it actually USES real colors and font sizes, not an empty skeleton.",
          "- The submit gate also rejects an obviously-empty 시안 (no colors and no font sizes detected), so a blind skeleton will bounce back to you.",
          "",
          "Then submit it for board review (the design_submit_for_review tool, or POST /work-products/:id/design-review/submit).",
          "",
          "Once the board approves the design, the parent issue's implementation resumes automatically.",
          "If the parent is clearly NOT screen work, mark it design-exempt instead via POST /issues/" +
            issue.id +
            '/design-requirement { "required": false, "reason": "..." }.',
        ].join("\n"),
        status: "todo",
        priority: "high",
        parentId: issue.id,
        projectId: issue.projectId ?? null,
        assigneeAgentId: designer?.id ?? null,
        originKind: "design_request",
        originId: issue.id,
      });
      // The design task itself is design-exempt — it IS the design work; without
      // this the child would hold on its own missing 시안 and never finish.
      await db
        .update(issues)
        .set({
          designRequirement: { required: false, reason: "디자인 시안 제작 작업 (자동 생성)", setByKind: "auto" },
          updatedAt: new Date(),
        })
        .where(eq(issues.id, designRequest.id));

      await issuesSvc.addComment(
        issue.id,
        [
          "Design-first hold: 승인된 시안이 없어 구현 run을 시작하지 않았습니다.",
          "",
          `- 시안 작업 이슈: ${designRequest.identifier ?? designRequest.id}${designer ? ` (담당: ${designer.name})` : " (담당 디자이너 없음 — 디자이너를 고용하거나 직접 시안을 첨부해 주세요)"}`,
          "- 시안이 보드 승인되면 구현이 자동으로 재개됩니다.",
          "- 화면 작업이 아니라면 이 이슈를 design-exempt로 표시하세요.",
        ].join("\n"),
        { runId: input.run.id },
      );
      await logActivity(db, {
        ...
        runId: input.run.id,
        action: "heartbeat.design_first_hold",
        entityType: "issue",
        entityId: issue.id,
        details: {
          designRequestIssueId: designRequest.id,
          designerAgentId: designer?.id ?? null,
          gateDirective: gate.directive,
        },
      });
      if (designer) {
        await enqueueWakeup(designer.id, {
          source: "assignment",
          triggerDetail: "system",
          reason: "issue_assigned",
          requestedByActorType: "system",
          contextSnapshot: {
            issueId: designRequest.id,
            taskId: designRequest.id,
            wakeReason: "issue_assigned",
    ...
    await setRunStatus(input.run.id, "cancelled", {
      error: "Design-first hold: waiting for an approved source-of-truth design (시안)",
      errorCode: "design_first_hold",
      finishedAt: new Date(),
    });
    await setWakeupStatus(input.run.wakeupRequestId, "cancelled", { finishedAt: new Date() });
    const cancelledRun = await getRun(input.run.id);
  ...
  async function executeRun(runId: string) {
    ...
    // Design-first: resolve the gate ONCE per wake (skip for planning — it has
    // no design directive). Reused by both the dispatch gate and the task
    // prompt below, so deriveDesignGateForIssue runs at most once per wake.
    const designGateForWake =
      issueContext && issueContext.workMode !== "planning"
        ? await workProductService(db)
            .deriveDesignGateForIssue(issueContext.id, agent.companyId)
            .catch(() => null)
        : null;
    // Dispatch gate: cancel the run BEFORE checkout/execution when the issue is
    // screen work with no approved 시안 (and route a designer).
    if (
      issueContext &&
      (await enforceDesignFirstDispatchGate({ run, agent, issueContext, gate: designGateForWake }))
    ) {
      return;
    }
    ...
    if (
      ...
      try {
        ...
        context[WORKCELL_HARNESS_CHECKOUT_KEY] = true;
      } catch (error) {
        if (!isCheckoutConflictError(error)) throw error;
        context[WORKCELL_HARNESS_CHECKOUT_KEY] = false;
      }
    ...
    if (continuationSummary) {
      context.workcellContinuationSummary = {
        key: continuationSummary.key,
        title: continuationSummary.title,
      ...
      delete context.workcellContinuationSummary;
    }
    const workcellWakePayload = await buildWorkcellWakePayload({
      db,
      companyId: agent.companyId,
      contextSnapshot: context,
      continuationSummary,
    ...
    if (workcellWakePayload) {
      context[WORKCELL_WAKE_PAYLOAD_KEY] = workcellWakePayload;
    } else {
      delete context[WORKCELL_WAKE_PAYLOAD_KEY];
    }
    // Team report language: agents must write user-facing output (comments,
    // reports, plans) in the company's configured language — previously only
    // planner drafts honored it and everything else defaulted to English.
    const reportLanguageLabel = await db
      ...
      .catch(() => null);
    // Design-first contract: the HOLD / build-against / QA-verify directive
    // lands in the task prompt. Reuse the gate already resolved at dispatch
    // (designGateForWake) — no second deriveDesignGateForIssue round trip.
    const designGateForTask = designGateForWake;
    // The review-stage QA-compare directive ("compare the in-app screen against
    // the 시안, do NOT approve on mismatch") is for the REVIEWER only. During
    // in_review a comment/@-mention can wake the original IMPLEMENTER too; it
    // must NOT receive the reviewer's instructions. Gate the flip on the woken
    // agent actually being the current execution-stage participant.
    const issueExecutionParticipant = issueContext
      ? parseIssueExecutionState(issueContext.executionState)?.currentParticipant ?? null
      : null;
    const isReviewParticipant =
      issueExecutionParticipant?.type === "agent" &&
      issueExecutionParticipant.agentId === agent.id;
    const taskMarkdown = buildWorkcellTaskMarkdown({
      reportLanguageLabel,
      issue: issueRef
        ? {
            id: issueRef.id,
      ...
      designGate: designGateForTask,
      isReviewParticipant,
    });
    if (issueRef) {
      context.workcellIssue = {
        id: issueRef.id,
        identifier: issueRef.identifier,
        title: issueRef.title,
        description: issueRef.description,
      ...
      delete context.workcellIssue;
    }
    if (wakeCommentContext) {
      context.workcellWakeComment = wakeCommentContext;
    } else {
      delete context.workcellWakeComment;
    }
    if (taskMarkdown) {
      context.workcellTaskMarkdown = taskMarkdown;
    } else {
      delete context.workcellTaskMarkdown;
    }
    ...
    if (secretManifest.length > 0) {
      context.workcellSecrets = {
        manifest: secretManifest,
      };
    } else {
      delete context.workcellSecrets;
    }
    ...
    // Production-readiness Wave 2 (REL — main-run timeout): bound the run's
    // wall-clock. The adapters read `config.timeoutSec` and treat 0/unset as "no
    // kill timer" (resolveAdapterExecutionTargetTimeoutSec keeps 0 for local/SSH),
    // so a hung CLI would run forever. resolveRunTimeoutSec injects the default
    // ONLY when the resolved config leaves timeoutSec unset or <= 0; an explicit
    // positive agent timeout is preserved unchanged.
    let runtimeConfig = {
      ...effectiveResolvedConfig,
      timeoutSec: resolveRunTimeoutSec(effectiveResolvedConfig.timeoutSec),
      workcellRuntimeSkills: runtimeSkillEntries,
    };
    ...
    context.workcellWorkspaces = resolvedWorkspace.workspaceHints;
    ...
    if (runtimeServiceIntents.length > 0) {
      context.workcellRuntimeServiceIntents = runtimeServiceIntents;
    } else {
      delete context.workcellRuntimeServiceIntents;
    }
    ...
    if (sessionCompaction.rotate) {
      context.workcellSessionHandoffMarkdown = sessionCompaction.handoffMarkdown;
      context.workcellSessionRotationReason = sessionCompaction.reason;
      context.workcellPreviousSessionId = previousSessionDisplayId ?? runtimeSessionIdForAdapter;
      ...
      delete context.workcellSessionHandoffMarkdown;
      delete context.workcellSessionRotationReason;
      delete context.workcellPreviousSessionId;
    }
    ...
    try {
      ...
      if (runScopedMentionedSkillKeys.length > 0) {
        await onLog(
          "stdout",
          `[workcell] Enabled run-scoped skills from issue mentions: ${runScopedMentionedSkillKeys.join(", ")}\n`,
        );
      }
      ...
      if (runtimeServices.length > 0) {
        context.workcellRuntimeServices = runtimeServices;
        context.workcellRuntimePrimaryUrl =
      ...
      if (issueId && (executionWorkspace.created || runtimeServices.some((service) => !service.reused))) {
        try {
          ...
          await onLog(
            "stderr",
            `[workcell] Failed to post workspace-ready comment: ${err instanceof Error ? err.message : String(err)}\n`,
          );
        }
      }
      ...
      if (adapter.supportsLocalAgentJwt && !authToken) {
        logger.warn(
          ...
          "local agent jwt secret missing or invalid; running without injected WORKCELL_API_KEY",
        );
      }
      // WC-67: surface MCP servers the agent declared in adapterConfig so the
      // adapter can serialize them to a .mcp.json for its CLI. Gated on
      // presence; the adapter side is additionally behind a feature flag, so
      // the default run path is unchanged.
      const declaredMcpServers = extractMcpServersFromConfig(parseObject(agent.adapterConfig));
      if (declaredMcpServers.length > 0) {
        context.workcellMcpServers = declaredMcpServers;
      }
      // One adapter turn. Factored so the dual-brain loop can run several with
      // varied runtime(session)/config(model)/context(prompt)/auth while keeping
      // the rest IDENTICAL to the single call — onLog/onSpawn(process registry)/
      // onMeta/executionTarget are the same, so all run-record/watchdog/registry
      // invariants hold across passes.
      const executeAdapterPass = (
        passAdapter: typeof adapter,
        passAdapterType: string,
        passRuntime: typeof runtimeForAdapter,
        passConfig: typeof runtimeConfig,
        passContext: typeof context,
        passAuthToken: string | null,
      ) =>
        passAdapter.execute({
          runId: run.id,
          // A cross-adapter review pass (WC-XADAPT) advertises the review
          // adapter's type so that adapter reads the right agent fields.
          agent: passAdapterType === agent.adapterType ? agent : { ...agent, adapterType: passAdapterType },
          runtime: passRuntime,
          config: passConfig,
          context: passContext,
          runtimeCommandSpec: passAdapter.getRuntimeCommandSpec?.(passConfig) ?? null,
          executionTarget,
          ...
          authToken: passAuthToken ?? undefined,
        });

      // Execution-layer dual-brain: a deliberation-enabled agent FUSES two brains
      // INSIDE this one run (work -> independent candidate(s) -> synthesize),
      // OpenRouter-Fusion style. Pass 0 (work) is byte-for-byte today's single
      // call; the candidates are independent fresh passes that only PROPOSE (no
      // edits), and the synthesize pass merges them into the final. Returns one
      // aggregated result. Excluded: pair/deliberation child turns, recovery-origin
      // (cheap eval) issues, and a WORKCELL_DUAL_BRAIN=0 kill-switch. A brain B
      // adapter override runs the candidate cross-vendor.
      const dualBrain = agent.deliberation as
        | {
            enabled?: boolean;
            brainA?: { model?: string | null; adapter?: string | null } | null;
            brainB?: { model?: string | null; adapter?: string | null } | null;
            maxRounds?: number;
            reviewMode?: "single" | "panel";
            panel?: { members?: { model?: string | null; adapter?: string | null }[]; minAgree?: number } | null;
            track?: "auto" | "a" | "b";
          }
        | null
        | undefined;
      const useDeliberation =
        !!issueId &&
        dualBrain?.enabled === true &&
        !!dualBrain.brainB &&
        context.pairTurn !== true &&
        (context as Record<string, unknown>).deliberationTurn !== true &&
        !DELIBERATION_EXCLUDED_ORIGIN_KINDS.has(issueRef?.originKind ?? "") &&
        process.env.WORKCELL_DUAL_BRAIN !== "0";

      let adapterResult: AdapterExecutionResult;
      if (useDeliberation && dualBrain) {
        const workModel = readNonEmptyString(dualBrain.brainA?.model ?? null);
        const reviewModel = readNonEmptyString(dualBrain.brainB?.model ?? null);
        const maxRounds = Math.max(1, Math.min(8, Number(dualBrain.maxRounds) || 4));
        const aggregateBudgetMs =
          resolveRunTimeoutSec(runtimeConfig.timeoutSec) * 1000 * (maxRounds * 2 + 1);
        const baseTaskMarkdown =
          readNonEmptyString((context as Record<string, unknown>).workcellTaskMarkdown) ?? "";
        const reviewAdapter = readNonEmptyString(dualBrain.brainB?.adapter ?? null);
        // WC-PANEL: resolve the independent review panel (only meaningful when
        // reviewMode === "panel" and it has members).
        const reviewMode = dualBrain.reviewMode === "panel" ? "panel" : "single";
        const panelMembers = (dualBrain.panel?.members ?? []).map((m) => ({
          model: readNonEmptyString(m?.model ?? null),
          adapter: readNonEmptyString(m?.adapter ?? null),
        }));
        const panelMinAgree = dualBrain.panel?.minAgree;
        const runOnePass = async (
          input: DeliberationPassInput,
        ): Promise<DeliberationPassOutput> => {
          // WC-XADAPT: resolve the adapter this pass runs on. A review/panel pass
          // may name a DIFFERENT local adapter (cross-vendor review). Resolving an
          // unregistered adapter throws here — the deliberation loop catches a
          // review-pass throw and skips that reviewer (non-fatal), so a
          // misconfigured cross-adapter member never fails the run.
          const passAdapterType =
            input.adapter && input.adapter !== agent.adapterType ? input.adapter : agent.adapterType;
          const isCrossAdapter = passAdapterType !== agent.adapterType;
          const passAdapter = isCrossAdapter ? getServerAdapter(passAdapterType) : adapter;
          const passCodec = isCrossAdapter ? getAdapterSessionCodec(passAdapterType) : sessionCodec;
          // pass 0 = the run's original work call. A `fresh` pass (independent
          // panel member) or a cross-adapter pass starts a brand-new session
          // (can't resume another adapter's / another panelist's session); it
          // still sees the working tree (same cwd) + the work summary.
          const isPass0 = input.session === undefined && !input.fresh && !isCrossAdapter;
          const useFresh = !isPass0 && (input.fresh === true || isCrossAdapter || input.session === undefined);
          const passRuntime = isPass0
            ? runtimeForAdapter
            : useFresh
              ? { sessionId: null, sessionParams: null, sessionDisplayId: null, taskKey }
              : {
                  sessionId: input.session!.legacySessionId,
                  sessionParams: input.session!.params,
                  sessionDisplayId: input.session!.displayId,
                  taskKey,
                };
          const passConfig =
            !isPass0 && input.model && input.model !== (runtimeConfig as Record<string, unknown>).model
              ? { ...runtimeConfig, model: input.model }
              : runtimeConfig;
          const passContext = isPass0
            ? context
            : ({ ...context, workcellTaskMarkdown: input.taskMarkdown } as typeof context);
          const passAuthToken = isPass0
            ? authToken
            : passAdapter.supportsLocalAgentJwt
              ? createLocalAgentJwt(agent.id, agent.companyId, passAdapterType, run.id)
              : null;
          const result = await executeAdapterPass(passAdapter, passAdapterType, passRuntime, passConfig, passContext, passAuthToken);
          const next = resolveNextSessionState({
            codec: passCodec,
            adapterResult: result,
            previousParams: passRuntime.sessionParams ?? null,
            previousDisplayId: passRuntime.sessionDisplayId ?? null,
            previousLegacySessionId: passRuntime.sessionId ?? null,
          });
          return {
            result,
            nextSession: {
        ...
        adapterResult = await runDeliberationPasses({
          runPass: runOnePass,
          baseTaskMarkdown,
          workModel,
          reviewModel,
          workAdapter: agent.adapterType,
          reviewAdapter,
          reviewMode,
          panelMembers,
          panelMinAgree,
          track: dualBrain.track === "a" || dualBrain.track === "b" ? dualBrain.track : "auto",
          maxRounds,
          ...
          // Stop adding review/revise passes once the RUN is terminal
          // (cancel) OR the ISSUE itself reached done/cancelled — there is no
          // point self-reviewing work that is already finished or killed
          // (saves the extra LLM passes the board flagged). The work pass that
          // completed the issue still ran; only the would-be review is skipped.
          isTerminal: async () => {
            if (isHeartbeatRunTerminalStatus((await getRun(run.id))?.status)) return true;
            if (issueId) {
          ...
          onPassEvent: async (rec) => {
            // Build a human-readable timeline line: who ran which fusion pass
            // (work / candidate / synthesize). The full record rides along in
            // `payload` for any richer UI.
            const who =
              `brain ${rec.brain}` +
              (rec.model ? ` · ${rec.model}` : "") +
              (rec.adapter ? ` @${rec.adapter}` : "") +
              (rec.panelMember !== undefined ? ` · candidate #${rec.panelMember + 1}` : "");
            const message = `dual-brain ${rec.kind} (${who})`;
            await appendRunEvent(currentRun, seq++, {
              eventType: "deliberation.pass",
              stream: "system",
              level: "info",
              message,
              payload: { ...rec },
          ...
          onBoundaryLog: async (message) => {
            await onLog("stdout", `[workcell] ${message}\n`);
          },
          logWarn: (message, err) => logger.warn({ runId: run.id, agentId: agent.id, err }, message),
        });
      } else {
        adapterResult = await executeAdapterPass(adapter, agent.adapterType, runtimeForAdapter, runtimeConfig, context, authToken);
      }
      ...
      if (adapterManagedRuntimeServices.length > 0) {
        ...
        context.workcellRuntimeServices = combinedRuntimeServices;
        context.workcellRuntimePrimaryUrl =
        ...
        if (issueId) {
          try {
            ...
            await onLog(
              "stderr",
              `[workcell] Failed to post adapter-managed runtime comment: ${err instanceof Error ? err.message : String(err)}\n`,
            );
          }
        }
      }
      ...
      if (finalizedRun) {
        ...
        if (issueId && outcome === "succeeded" && !skipRunIssueComment) {
          try {
            ...
            await onLog(
              "stderr",
              `[workcell] Failed to post run summary comment: ${err instanceof Error ? err.message : String(err)}\n`,
            );
          }
        }
        if (outcome === "failed" && isMaxTurnExhaustionRun(livenessRun)) {
          ...
          await scheduleBoundedRetryForRun(livenessRun, agent);
        } else if (outcome === "failed" && readHeartbeatRunErrorFamily(livenessRun) === "context_exhausted") {
          // The resumed codex thread filled the context window. The adapter
          // cleared the session, so this bounded retry starts a fresh thread
          // (continuation summary re-injected) instead of looping on the full one.
          await scheduleBoundedRetryForRun(livenessRun, agent, {
            retryReason: CONTEXT_EXHAUSTED_RETRY_REASON,
            wakeReason: CONTEXT_EXHAUSTED_RETRY_WAKE_REASON,
            maxAttempts: CONTEXT_EXHAUSTED_RETRY_MAX_ATTEMPTS,
            delayMs: CONTEXT_EXHAUSTED_RETRY_DELAY_MS,
          });
        }
  ...
  function buildImmediateExecutionPathRecoveryComment(input: {
    ...
    if (input.status === "todo") {
      return (
        "Workcell automatically retried dispatch for this assigned `todo` issue during terminal run recovery, " +
    ...
    return (
      "Workcell automatically retried continuation for this assigned `in_progress` issue during terminal run " +
  ...
  async function releaseIssueExecutionAndPromote(run: typeof heartbeatRuns.$inferSelect) {
    ...
    const promotionResult = await db.transaction(async (tx) => {
      ...
      // WC-201: a design_first_hold cancel parks the parent behind an open
      // design_request child — do NOT synchronously flip it to blocked. (Same
      // predicate the design-first gate + the stranded-recovery scan run.)
      const openDesignChild = await tx
        .select({ id: issues.id })
        .from(issues)
        .where(
          and(
  ...
  return {
    list: async (companyId: string, agentId?: string, limit?: number) => {
      ...
      // WC-217: never run unbounded — default to the newest N when no limit is
      // given, and clamp any explicit limit to the ceiling (see constant above).
      const rows = await query.limit(normalizeHeartbeatRunListLimit(limit));
      return rows.map((row) => {
        const {
          contextIssueId,
          contextTaskId,
    ...
    triggerIssueMonitor,
    __testingDrainActiveRunExecutions: waitForActiveRunExecutionsToSettle,
    ...
    tickTimers: async (now = new Date()) => {
      // Production-readiness Wave 2 (DATA — tickTimers SQL filter): every 30s this
      // used to `SELECT * FROM agents` across ALL companies and then filter
      // per-row, an O(total agents)/tick full scan. Push the cleanly-SQL-able
      // predicate — the status exclusion — into the query so only schedulable
      // agents are materialized. The remaining eligibility ("heartbeat enabled +
      // positive interval" and the "interval elapsed" due check) lives in the
      // agent's runtime_config JSONB and is parsed/coerced in JS
      // (parseHeartbeatPolicy); replicating that coercion in SQL risks silently
      // dropping a due agent, so it stays in JS exactly as before. The status set
      // here MUST match the JS guard it replaces (paused/terminated/pending_approval).
