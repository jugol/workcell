 ...
 * ~/.workcell/adapter-plugins.json. This is the source of truth for which
...
import { resolveWorkcellHomeDir } from "../home-paths.js";
...
export interface AdapterPluginRecord {
  /** npm package name (e.g., "droid-workcell-adapter") */
  packageName: string;
...
function adapterPluginPaths() {
  const workcellDir = resolveWorkcellHomeDir();
  return {
    adapterPluginsDir: path.join(workcellDir, "adapter-plugins"),
    adapterPluginsStorePath: path.join(workcellDir, "adapter-plugins.json"),
    adapterSettingsPath: path.join(workcellDir, "adapter-settings.json"),
  };
}
...
function ensureDirs(): string {
  ...
  if (!fs.existsSync(pkgJsonPath)) {
    fs.writeFileSync(pkgJsonPath, JSON.stringify({
      name: "workcell-adapter-plugins",
      version: "0.0.0",
      private: true,
      description: "Managed directory for Workcell external adapter plugins. Do not edit manually.",
    }, null, 2) + "\n");
  }
