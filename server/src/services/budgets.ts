...
export function budgetService(db: Db, hooks: BudgetServiceHooks = {}) {
  ...
  async function createIncidentIfNeeded(
    ...
    // WC-154: ON CONFLICT DO NOTHING on the partial unique index
    // (policy_id, window_start, threshold_type) WHERE status <> 'dismissed'. Two
    // cost events crossing the same threshold near-simultaneously (normal parallel
    // runs) otherwise race the SELECT-then-INSERT above; the loser hit a raw 23505
    // that 500s and also failed the cost-event write. (Same class as WC-128/129/143.)
    ...
    // Lost the race — return the winner's incident. (In this rare double-crossing
    // race the approval just created is harmlessly orphaned; the board can dismiss it.)
    return db
      .select()
      .from(budgetIncidents)
      .where(
