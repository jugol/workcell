...
// System-generated recovery/evaluation issues (watchdog folds, liveness
// escalations, productivity reviews, stranded-issue recovery) are internal
// bookkeeping the platform auto-completes — they produce no deliverable work.
const PROOF_EXEMPT_ORIGIN_KINDS = new Set<string>(Object.values(RECOVERY_ORIGIN_KINDS));

// WC-12 (D19 first slice): templated body of the compound checklist that auto-
// appears on each non-exempt issue when it transitions to Done. The structure
// honors D11/D19 — learnings/prevention rules fold into existing docs (the
// `implemented/` snapshot, `QA_PLAYBOOK`, `DESIGN_DASHBOARD`) or the knowledge
// graph; this checklist is the WORKING SURFACE for capturing those notes, NOT
// a new doc type itself. Future slices replace the templated body with output
// drafted by the Planner Agent.
const COMPOUND_CHECKLIST_TEMPLATE = `# Compound checklist

Review and capture before archiving. Per D19 (Compound step), learnings and
prevention rules fold into **existing** docs — the \`implemented/\` snapshot,
\`QA_PLAYBOOK\`, \`DESIGN_DASHBOARD\`, or the knowledge graph node — not into a
new doc type. This checklist is the working surface for that capture.

## 1. What changed?

- (link the work products, commits/PR, runs that completed this issue)

## 2. Reusable learnings

- (patterns worth recording — fold into \`implemented/\` snapshot or graph node)

## 3. Prevention rules

- (what to NOT do next time — record in the relevant role artifact)

## 4. Failed approaches

- (anything attempted that didn't work — so future agents don't re-tread)

## 5. Follow-up issues

- (discovered debt, polish, next slices — each line should become a new issue)
`;

// Whether a `done` transition for this issue must be backed by a proof bundle.
// Planning issues produce a plan (a draft document), not a proof; recovery/
// evaluation issues are internal bookkeeping. Everything else is deliverable work
// that must show its proof before it can be marked done (see proof-gated Done).
// Exported so the PATCH route can early-fail at executor submit time (WC-7),
// not only when the late gate in `update` fires after the signoff machine.
export function issueRequiresProofForDone(issue: { workMode?: string | null; originKind?: string | null }): boolean {
  if (issue.workMode === "planning") return false;
  if (issue.originKind && PROOF_EXEMPT_ORIGIN_KINDS.has(issue.originKind)) return false;
  return true;
}
...
// WC-138 (WC-112 defense-in-depth): issues.parentId is a self-FK with NO
// same-company constraint, so a cross-company parentId is physically storable.
// WC-112 scoped the getAncestors READ path; this guards the WRITE path so a
// cross-tenant parentId can never be stored in the first place (belt-and-
// suspenders for any future reader that forgets to scope its traversal).
async function assertParentIssueInCompany(db: DbReader, companyId: string, parentId: string) {
...
// WC-140 (tenant-isolation invariant, sibling of WC-138): an issue must not
// reference another company's project or goal. The project/goal-derived lookups
// are already company-scoped, but the raw FK columns were storable cross-tenant.
async function assertProjectInCompany(db: DbReader, companyId: string, projectId: string) {
  const row = await db
    ...
    .then((rows) => rows[0] ?? null);
  if (!row) {
    throw unprocessable("Project not found in this company");
  }
}

async function assertGoalInCompany(db: DbReader, companyId: string, goalId: string) {
  const row = await db
    .select({ id: goals.id })
    .from(goals)
    ...
    .then((rows) => rows[0] ?? null);
  if (!row) {
    throw unprocessable("Goal not found in this company");
  }
}
...
const LEGACY_PLUGIN_OPERATION_ORIGIN_KINDS = [
  "plugin:workcell.content-machine:case",
  "plugin:workcell.content-machine:evaluation",
  "plugin:workcell.content-machine:source-sync",
] as const;
...
// WC-187 / CP6: the design-first hold is NOT surfaced through this map. This
// derivation only operates on issues already in `status === "blocked"` and walks
// the dependency graph; a design-hold issue is typically still todo/in_progress
// and is stopped at the Done boundary by the design gate (see issueService.update
// and the PATCH /issues/:id early gate, code: "design_review_pending"). Extending
// this heavy graph traversal to synthesize a design blocker for non-blocked
// statuses is out of scope (no heavy new blocked subsystem). The held state is
// surfaced via (a) the deriveIssueDesignGate `designGate` on the heartbeat-context
// route — IssueDesignReviewPanel renders its pending/HOLD state — and (b) the
// 409 the gate returns, which IssueDetail shows as an "Issue update failed" toast.
...
const issueListSelect = {
  ...
  // WC-195: design-first gate opt-out (null = required by default).
  designRequirement: issues.designRequirement,
  workOwnerKind: issues.workOwnerKind,
  pairGroupId: issues.pairGroupId,
  priority: issues.priority,
  assigneeAgentId: issues.assigneeAgentId,
  assigneeUserId: issues.assigneeUserId,
  ...
  executionStateVersion: issues.executionStateVersion,
...
export function issueService(db: Db) {
  ...
  return {
    ...
    list: async (companyId: string, filters?: IssueFilters) => {
      ...
      const issueIds = withRuns.map((row) => row.id);
      const [statsRows, readRows, lastActivityRows, blockedByMap, proofIssueIds, costCentsByIssueId] = await Promise.all([
        contextUserId
        ...
        // WC-9: batch-resolve which of these issues have a `type:"proof"` work
        // product so each card can render a proof-status chip without an N+1
        // round trip. Reuses workProductService's batch helper.
        workProductService(db).findIssueIdsWithProof(companyId, issueIds),
        // WC-11: batch-resolve total cost cents per issue so each card can
        // render a usage chip. Returns a Map; issues not present have zero.
        costService(db).sumCostCentsByIssueIds(companyId, issueIds),
      ]);
      const statsByIssueId = new Map(statsRows.map((row) => [row.issueId, row]));
      const lastActivityByIssueId = new Map(lastActivityRows.map((row) => [row.issueId, row]));
      const [
      ...
      if (!contextUserId) {
        return withRuns.map((row) => {
          ...
          return {
            ...
            lastActivityAt,
            hasProof: proofIssueIds.has(row.id),
            totalCostCents: costCentsByIssueId.get(row.id) ?? 0,
            ...(blockerAttentionByIssueId.has(row.id) ? { blockerAttention: blockerAttentionByIssueId.get(row.id) } : {}),
    ...
    create: async (
      ...
      return db.transaction(async (tx) => {
        // WC-138: validate parentId company-scope ALWAYS. The workspace-inheritance
        // block below only validates parentId when it is the inheritance source —
        // bypassed when inheritExecutionWorkspaceFromIssueId is supplied — so guard
        // it explicitly here to keep a cross-tenant parentId from ever being stored.
        if (issueData.parentId) {
          await assertParentIssueInCompany(tx, companyId, issueData.parentId);
          // Project guard: a child issue INHERITS its parent's project when none
          // is given, so spawned issues (design_request 시안 children, compound
          // follow-ups/auto-fills, delegated subtasks) stay in the parent's
          // project instead of leaking to company scope. Explicit projectId wins.
        ...
        // WC-140: same-company tenant-isolation for explicit projectId/goalId
        // (derived goal/workspace lookups are already scoped; the raw FKs were not).
        if (issueData.projectId) {
          await assertProjectInCompany(tx, companyId, issueData.projectId);
        }
        if (issueData.goalId) {
          await assertGoalInCompany(tx, companyId, issueData.goalId);
        }
    ...
    update: async (
      ...
      data: Partial<typeof issues.$inferInsert> & {
        ...
        actorAgentId?: string | null;
        actorUserId?: string | null;
        // Server-set only (never from request bodies): authorized governance paths
        // such as recovery-action resolution complete an issue as a disposition and
        // must not be blocked by the proof-of-work gate.
        bypassProofRequirement?: boolean;
        // WC-163: optimistic-concurrency version for executionState. When this patch
        // sets executionState, the caller passes the version it read; the write is
        // guarded to that version (and bumps it), so a concurrent executionState edit
        // is rejected (409) not silently clobbered. An integer token avoids the jsonb
        // serialization mismatch that broke the earlier OCC-on-raw-jsonb attempt.
      ...
      // WC-138: a parentId being set/changed must belong to the same company as
      // this issue (issues.parentId self-FK has no same-company constraint). The
      // update path was previously unguarded, so a cross-tenant parentId was
      // storable; reject it here (defense-in-depth over the WC-112 read scoping).
      if (issueData.parentId && issueData.parentId !== existing.parentId) {
        await assertParentIssueInCompany(dbOrTx, existing.companyId, issueData.parentId);
      }
      // WC-140: same-company tenant-isolation for projectId/goalId on update.
      if (issueData.projectId && issueData.projectId !== existing.projectId) {
        await assertProjectInCompany(dbOrTx, existing.companyId, issueData.projectId);
      }
      if (issueData.goalId && issueData.goalId !== existing.goalId) {
        await assertGoalInCompany(dbOrTx, existing.companyId, issueData.goalId);
      }
      ...
      if (
        ...
        const hasProof = await workProductService(dbOrTx).hasProofForIssue(id, existing.companyId, dbOrTx);
        if (!hasProof) throw conflict("done requires a proof bundle", { code: "proof_required", issueId: id });
      }

      // WC-187 / CP6: design-first gate. Beside the proof gate (same Done
      // transition, same dbOrTx), block completing development while the issue's
      // SOURCE-OF-TRUTH design exists but is NOT board-approved — i.e. the same
      // developmentHold the agent context already surfaces (deriveIssueDesignGate),
      // now ENFORCED at the Done boundary. Reuses the pure derivation via the
      // service helper (no re-derivation here). ADDITIVE + gated: when the issue
      // has NO authoritative design, or it IS approved, developmentHold is false
      // and this guard is a no-op → non-design issues are byte-identical.
      // bypassProofRequirement is the same governance override the proof gate
      // honors. The scheduler/run-start path and the execution-policy stage
      // machine are untouched — this fires only on a resolved status:done update.
      if (
        issueData.status === "done" &&
        existing.status !== "done" &&
        !bypassProofRequirement &&
        // WC-199: share the proof gate's exemption — a planning or system-recovery
        // issue is inherently non-screen, so it must never be design-gated (this is
        // what the comment above always intended; the clause was missing).
        issueRequiresProofForDone(existing)
      ) {
        const designGate = await workProductService(dbOrTx).deriveDesignGateForIssue(
          id,
          existing.companyId,
          dbOrTx,
        );
        if (designGate.developmentHold) {
          throw conflict(
            ...
            { code: "design_review_pending", issueId: id },
          );
        }
      }

      // WC-12 (D19 first slice): when a non-exempt issue actually transitions to
      // Done, drop a compound-checklist document so the next pass — Planner agent
      // or human reviewer — has a working surface for capturing learnings, prevention
      // rules, and follow-ups. Same exemption set as the proof gate above (planning
      // + recovery/governance overrides skip). Idempotent: if a prior transition or
      // a manual edit already created the doc, do nothing — never clobber user
      // content. The document is filed via the existing issue-documents machinery so
      // it shows up on the Issue detail page like any other doc.
      if (
        issueData.status === "done" &&
        existing.status !== "done" &&
        !bypassProofRequirement &&
        ...
        const docSvc = documentService(dbOrTx);
        const existingChecklist = await docSvc.getIssueDocumentByKey(id, "compound-checklist");
        if (!existingChecklist) {
          await docSvc.upsertIssueDocument({
            issueId: id,
            key: "compound-checklist",
            title: "Compound checklist",
            format: "markdown",
            body: COMPOUND_CHECKLIST_TEMPLATE,
            changeSummary: "Auto-created on Done transition (WC-12)",
      ...
      const runUpdate = async (tx: any) => {
        ...
        // WC-163: optimistic-concurrency guard for executionState. When the caller
        // passed the version it read and this patch writes executionState, only write
        // if execution_state_version is still that value, and bump it; a concurrent
        // executionState edit matches 0 rows → throw conflict so the caller reloads
        // and retries instead of silently losing one update. The integer version
        // (vs. comparing the raw jsonb) avoids the prior serialization-mismatch flaw.
        const guardVersion =
          patch.executionState !== undefined && expectedExecutionStateVersion !== undefined
            ? expectedExecutionStateVersion
            : null;
        if (guardVersion !== null) {
          patch.executionStateVersion = guardVersion + 1;
        }
        const updated = await tx
          .update(issues)
          .set(patch)
          .where(
            guardVersion !== null
              ? and(eq(issues.id, id), eq(issues.executionStateVersion, guardVersion))
              : eq(issues.id, id),
          )
          .returning()
          .then((rows: Array<typeof issues.$inferSelect>) => rows[0] ?? null);
        if (!updated) {
          if (guardVersion !== null) {
            ...
            if (stillExists) {
              throw conflict("Issue execution state changed concurrently; reload and retry", {
    ...
    remove: (id: string) =>
      db.transaction(async (tx) => {
        ...
        // WC-134: several issue FK children have NO onDelete, so delete(issues)
        // below would FK-violate (500) on any worked issue (comments, read
        // states, etc.). Purge the issue-CONTENT children here. cost_events /
        // finance_events are company billing records (not issue content), so
        // PRESERVE them by nulling the issue link instead of deleting. Mirrors
        // the company-removal cascade discipline (WC-117/118) at issue scope.
        ...
        // WC-159: issues.parentId is a self-FK with NO onDelete. Deleting a parent
        // issue that still has children would FK-violate (500). Detach children to
        // top-level (null parentId) before the delete — child issues are top-level
        // entities and must survive, mirroring the WC-134 content-purge discipline.
    ...
    createLabel: async (companyId: string, data: Pick<typeof labels.$inferInsert, "name" | "color">) => {
      ...
      // WC-143: (company_id, name) is unique; a duplicate name otherwise surfaces
      // as a raw 500. Idempotent: return the existing label on conflict.
      const [created] = await db
        .insert(labels)
        .values({ companyId, name, color: data.color })
        .onConflictDoNothing()
        .returning();
    ...
    getAncestors: async (issueId: string) => {
      ...
      // WC-112 (security): scope the ancestor walk to the start issue's company.
      // issues.parentId is a self-FK with NO same-company constraint, so a
      // (malicious or stale) cross-company parentId must never be traversed —
      // otherwise the parent's title/description/assignee/project/goal would
      // leak across tenants into the issue-context response. Mirrors the WC-54
      // fix for syncIssueAsNode's parent lookup. (Bonus: cross-company ancestors
      // never enter `raw`, so the downstream project/goal batch-fetch — which is
      // keyed only by id — can't leak cross-company artifacts either.)
      const companyId = start?.companyId ?? null;
      let currentId = start?.parentId ?? null;
      while (companyId && currentId && !visited.has(currentId) && raw.length < 50) {
        ...
        }).from(issues).where(and(eq(issues.companyId, companyId), eq(issues.id, currentId))).then(r => r[0] ?? null);
        if (!parent) break;
        raw.push({
