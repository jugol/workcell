...
export function agentDeliberationRunService(db: Db) {
  ...
  async function runLoop(runId: string, input: AgentDeliberationStartInput): Promise<void> {
    ...
    // Per-turn cost queue: each invoke reports its billed cents via onCost (push);
    // onTurn consumes them FIFO in turn order. A queue (not a single accumulator)
    // is required now that the fusion phase fires the two `generate` invokes in
    // PARALLEL before either turn is pushed — each generate turn still gets its own
    // cost. (If two live parallel turns' adapters finish out of order, the two
    // generate costs may swap between the two generate turns; the run TOTAL is
    // always exact.)
    const costQueue: number[] = [];
    const onCost = (costCents: number) => {
      costQueue.push(costCents);
    };
    const invoke = input.buildInvoke(onCost);

    const onTurn = async (entry: DeliberationTranscriptEntry): Promise<void> => {
      const costCents = costQueue.shift() ?? 0;
