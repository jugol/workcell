...
import { environmentLeases, environments } from "@workcell/db";
import {
  ENVIRONMENT_DRIVERS,
...
const DEFAULT_LOCAL_ENVIRONMENT_DESCRIPTION =
  "Default execution environment for Workcell runs on this machine.";
...
export function environmentService(db: Db) {
  return {
    ...
    create: async (companyId: string, input: CreateEnvironment): Promise<Environment> => {
      const now = new Date();
      // WC-143: a single local environment per company is enforced by a partial
      // unique index (driver='local'), and every company is auto-seeded one. A
      // second local create would surface the unique violation as a raw 500 —
      // reject it cleanly (409) instead.
    ...
    acquireLease: async (input: {
      ...
      const now = new Date();
      let row: typeof environmentLeases.$inferSelect | null = null;
      try {
        row = await db
        ...
        // WC-165: the unique partial index on (environment_id, provider_lease_id)
        // WHERE status='active' rejects a second concurrent run trying to reuse the
        // SAME provider sandbox lease (the reuse TOCTOU). Surface a clean, retryable
        // conflict instead of a raw 23505, so the caller retries / falls back rather
        // than silently sharing one sandbox between two runs.
        // Drizzle wraps the driver error, so walk the cause chain to find the pg
        // unique-violation code (23505) from the active-provider-lease index.
        const seen = new Set<unknown>();
        let current: unknown = err;
        while (current && typeof current === "object" && !seen.has(current)) {
          seen.add(current);
          if ((current as { code?: string }).code === "23505") {
            throw conflict("Reusable environment sandbox is already leased by a concurrent run", {
