...
import { plugins, pluginJobs, pluginJobRuns } from "@workcell/db";
import type {
...
/**
 * Default staleness threshold for the plugin-job orphan reaper: a
 * `plugin_job_runs` row still `status='running'` whose execution began (or, if
 * `started_at` was never written, whose row was created) more than this long ago
 * is considered abandoned and is failed. 20 min comfortably exceeds any healthy
 * scheduled run while still clearing crash debris quickly.
 */
const DEFAULT_STALE_PLUGIN_JOB_RUN_MS = 20 * 60 * 1000;

/**
 * Production-readiness Wave 2 (REL — plugin-job boot reaper):
 * fail orphaned `running` plugin job runs left behind by a crash/restart.
 *
 * WHY: `markRunning` is a plain UPDATE (no compare-and-set). If the host process
 * dies after a run is marked `running`, the row stays `running` forever — and the
 * scheduler's per-job overlap check (which refuses to start a job that already
 * has a `running` run) then permanently blocks that job. This boot reconciler
 * mirrors `reapStaleDeliberationRuns`: it marks every `plugin_job_runs` row that
 * is still `status='running'` AND whose `started_at` (falling back to
 * `created_at` when the run never recorded a start) is older than
 * `(now - olderThanMs)` as `status='failed'` with an "abandoned" error and a
 * `finished_at` of now. Called once on server startup (index.ts) while in-memory
 * scheduler state is empty, so a previous process cannot leave zombie `running`
 * rows that wedge their jobs. Returns the number of rows reaped.
 */
export async function reapStalePluginJobRuns(
  db: Db,
  opts?: { olderThanMs?: number },
): Promise<{ reaped: number }> {
  const olderThanMs = opts?.olderThanMs ?? DEFAULT_STALE_PLUGIN_JOB_RUN_MS;
  const cutoff = new Date(Date.now() - olderThanMs);
  const reaped = await db
    .update(pluginJobRuns)
    .set({
      status: "failed" as PluginJobRunStatus,
      error: "abandoned (server restart or stuck run)",
      finishedAt: new Date(),
    })
    .where(
      and(
        eq(pluginJobRuns.status, "running" as PluginJobRunStatus),
        // A reaped run is one that started before the cutoff. Runs that were
        // marked running always have started_at, but fall back to created_at so
        // a row missing started_at is still eligible once old enough.
        or(
          lt(pluginJobRuns.startedAt, cutoff),
          and(isNull(pluginJobRuns.startedAt), lt(pluginJobRuns.createdAt, cutoff)),
        ),
      ),
    )
