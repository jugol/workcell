...
const TRANSFER_SCHEMA = {
  family: "workcell-upstream-transfer",
  version: "1.0.0",
  major: 1,
  minor: 0,
} as const;
...
const CLOUD_CREDENTIAL_PREFIX = "workcell-cloud-credential:";
...
export function cloudUpstreamService(db: Db, options: { instanceId?: string } = {}) {
  const sourceInstanceId = `workcell-local-${options.instanceId ?? "default"}`;
  ...
  return {
    ...
    createRun: async (input: { connectionId: string; companyId: string; retryOfRunId?: string | null }): Promise<CloudUpstreamRun> => {
      ...
      if (!preview.schemaCompatible) {
        throw badRequest("Cloud stack schema is not compatible with this local Workcell version");
      }
      ...
      const now = new Date();
      const initialEvents = [
        event(now.toISOString(), "connect", "completed", "Connected to the target Workcell Cloud stack."),
  ...
  async function buildBundle(connection: ConnectionRow, mode: "preview" | "apply"): Promise<LocalUpstreamExportBundle> {
    ...
    const source = {
      ...
      exporterVersion: "workcell-local-cloud-ui-v1",
...
async function fetchDiscovery(remoteUrl: string): Promise<Record<string, unknown>> {
  ...
  const discoveryUrl = new URL("/.well-known/workcell-upstream", parsed.origin);
  if (stackId) {
      ...
      .set({
        ...
        report: {
          ...
          errorMessage: optionalString(report.errorMessage)
            ?? "The server restarted while this cloud upstream run was running, so Workcell marked it failed instead of leaving it stuck.",
...
function targetFromDiscovery(discovery: Record<string, unknown>): CloudUpstreamTarget {
  ...
  return {
    ...
    origin,
    product: optionalString(discovery.product) ?? "Workcell Cloud",
