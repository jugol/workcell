...
import { clampIssueRequestDepth } from "@workcell/shared";
import {
  agents,
  companies,
  costEvents,
...
export function productivityReviewService(db: Db, deps?: { enqueueWakeup?: EnqueueWakeup }) {
  ...
  async function resolveReviewOwnerAgentId(sourceIssue: IssueRow, sourceAgent: AgentRow) {
    ...
    const roleCandidates = await db
      .select({ id: agents.id })
      .from(agents)
      .where(and(eq(agents.companyId, sourceIssue.companyId), inArray(agents.role, ["lead", "orchestrator"])))
      .orderBy(sql`case when ${agents.role} = 'lead' then 0 else 1 end`, asc(agents.createdAt), asc(agents.id));
  ...
  function buildReviewMarkdown(evidence: ProductivityReviewEvidence, prefix: string) {
    ...
    return [
      "Workcell detected an unusual productivity/progression pattern on an assigned issue.",
      "",
      "## Source",
      "",
