...
function buildLivenessEscalationDescription(finding: IssueLivenessFinding) {
  ...
  return [
    "Workcell detected a harness-level issue graph liveness incident.",
    "",
    "## Source",
    "",
...
function buildLivenessOriginalIssueComment(finding: IssueLivenessFinding, escalation: typeof issues.$inferSelect) {
  return [
    "Workcell detected a harness-level liveness incident in this issue's dependency graph.",
    "",
...
export function recoveryService(db: Db, deps: { enqueueWakeup: RecoveryWakeup }) {
  ...
  // WC-201: a parent issue with an OPEN design_request child is intentionally
  // WAITING (its implementation run was cancelled by the design-first gate and
  // parked behind the 시안 task), NOT stranded. The child carries
  // originKind="design_request", originId=<parent.id> (heartbeat.ts design-first
  // gate). Excluding done/cancelled + hidden means the moment the design is
  // resolved the parent is no longer suppressed and normal recovery resumes. The
  // child itself is a separately-assigned issue this same scan still recovers.
  async function hasOpenDesignRequestChild(companyId: string, issueId: string) {
    ...
    for (const candidate of candidates) {
      ...
      await issuesSvc.addComment(
        ...
        [
          ...
          "",
          `Workcell found this issue is blocking ${blockingLinks} but had no assignee, so no heartbeat could pick it up.`,
          "",
  ...
  // A run whose SOURCE issue it itself moved to a handed-off state has
  // finished its mandate even when the issue is not terminal: in_review means
  // the work was delivered and the QA reviewer owns the next step. Without
  // in_review here, a finished-but-unfinalized run (orphaned child after a
  // server restart, lingering CLI) could never be folded - the watchdog kept
  // opening "Review silent active run" evaluations that concluded
  // false-positive without unsticking anything.
  function isHandedOffIssueStatus(status: string | null | undefined) {
    return isTerminalIssueStatus(status) || status === "in_review";
  }
  ...
  async function latestSameRunSourceTerminalEvidence(input: {
    ...
    if (!isHandedOffIssueStatus(input.sourceIssue.status)) return null;
    // The handoff evidence routinely lands BEFORE the run's last output: the
    // agent moves the source issue to in_review/done and then keeps talking
    // (final summary, sanity checks). Gating this window on the silence start
    // (lastOutputAt) therefore rejected every real handoff and the watchdog
    // re-filed "Review silent active run" evaluations forever. Same-run
    // authorship is already proven by the runId predicate below; only bound
    // the window to the run's own lifetime.
    const after = input.run.startedAt ?? input.run.createdAt ?? null;
  ...
  async function resolveStaleRunOwnerAgentId(input: {
    ...
    const roleCandidates = await db
      .select()
      .from(agents)
      .where(and(eq(agents.companyId, input.run.companyId), inArray(agents.role, ["lead", "orchestrator"])))
      .orderBy(sql`case when ${agents.role} = 'lead' then 0 else 1 end`, asc(agents.createdAt));
  ...
  function buildStaleRunEvaluationDescription(input: {
    ...
    const blockers = input.evidence.blockers.length > 0
      ...
      : "- none detected";
    return [
      `Workcell detected ${input.level} output silence on an active heartbeat run.`,
      "",
      "## Run",
      "",
  ...
  async function ensureSourceIssueBlockedByStaleEvaluation(input: {
    ...
    await issuesSvc.addComment(input.sourceIssue.id, [
      "Workcell detected critical output silence on this issue's active run.",
      "",
  ...
  async function createOrUpdateStaleRunEvaluation(input: {
    ...
    if (sourceIssue && isHandedOffIssueStatus(sourceIssue.status)) {
    ...
    // Backstop against evaluation churn: once a reviewer (board or assigned
    // recovery owner) has dismissed this run's silence as a false positive and
    // the run has produced NO output since that decision, the silence under
    // review is the same one already judged — re-filing it would restart the
    // dismiss/re-create loop. New output after the dismissal re-arms the scan.
    const dismissed = await latestDismissedFalsePositiveDecision(input.run.companyId, input.run.id);
    if (dismissed && (!silenceStartedAt || silenceStartedAt <= dismissed.createdAt)) {
  ...
  async function resolveStrandedIssueRecoveryOwnerAgentId(issue: typeof issues.$inferSelect) {
    ...
    const roleCandidates = await db
      .select()
      .from(agents)
      .where(and(eq(agents.companyId, issue.companyId), inArray(agents.role, ["lead", "orchestrator"])))
      .orderBy(sql`case when ${agents.role} = 'lead' then 0 else 1 end`, asc(agents.createdAt));
    candidateIds.push(...roleCandidates.map((agent) => agent.id));
  ...
  function buildStrandedIssueRecoveryDescription(input: {
    ...
    if (input.recoveryCause === SUCCESSFUL_RUN_MISSING_STATE_REASON) {
      ...
      return [
        "Workcell exhausted the bounded corrective handoff for a successful run that still has no valid issue disposition.",
        "",
    ...
    return [
      "Workcell exhausted automatic recovery for an assigned issue and created this explicit recovery task.",
      "",
      "## Source",
      "",
      `- Source issue: ${sourceIssue}`,
  ...
  function buildRecoveryIssueInPlaceEscalationComment(input: {
    ...
    return [
      "Workcell stopped automatic stranded-work recovery for this recovery issue.",
      "",
  ...
  async function escalateStrandedAssignedIssue(input: {
    ...
    // WC-201: never strand a parent waiting behind an open design_request child —
    // belt-and-suspenders for callers other than the scan loop (e.g. the
    // successful-run-handoff exhaustion path). null = "not escalated → skipped",
    // matching the existing `if (!updated) return null` contract below.
    if (await hasOpenDesignRequestChild(input.issue.companyId, input.issue.id)) {
      return null;
    }
    ...
    const recoveryLine = recoveryAction.ownerAgentId
      ...
      : [
        "",
        `- Recovery action: \`${recoveryAction.id}\``,
        "- Recovery owner: board escalation, because Workcell could not find an invokable manager, creator, or executive owner with budget available.",
  ...
  async function reconcileStrandedAssignedIssues() {
    ...
    for (const issue of candidates) {
      ...
      // WC-201: a design-held parent (waiting behind an open 시안 task) is
      // intentionally parked, not stranded — never re-wake/strand it. Resumes
      // automatically once the design_request child is done/cancelled.
      if (await hasOpenDesignRequestChild(issue.companyId, issue.id)) {
        result.skipped += 1;
        continue;
      }
      ...
      if (issue.status === "todo") {
        ...
        if (didAutomaticRecoveryFail(latestRun, "assignment_recovery")) {
          ...
          const updated = await escalateStrandedAssignedIssue({
            ...
            latestRun,
            comment:
              "Workcell automatically retried dispatch for this assigned `todo` issue after a lost wake/run, " +
      ...
      if (isSuccessfulInProgressContinuationRun(latestRun)) {
        ...
        if (isRepeatedProductiveContinuationRecovery(successfulRun)) {
          const updated = await escalateStrandedAssignedIssue({
            ...
            comment:
              "Workcell automatically retried continuation for this assigned `in_progress` issue and the retry " +
      ...
      if (didAutomaticRecoveryFail(latestRun, "issue_continuation_needed")) {
        ...
        const updated = await escalateStrandedAssignedIssue({
          ...
          latestRun,
          comment:
            "Workcell automatically retried continuation for this assigned `in_progress` issue after its live " +
