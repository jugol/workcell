...
import { agentWakeupRequests, agents, heartbeatRuns, issues } from "@workcell/db";
import type { IssueCommentMetadata, IssueCommentPresentation, RunLivenessState } from "@workcell/shared";
...
export const SUCCESSFUL_RUN_HANDOFF_EXHAUSTED_NOTICE_BODY =
  "Workcell could not resolve this issue's missing disposition automatically. The issue is blocked on a recovery owner.";
...
export function buildSuccessfulRunHandoffInstruction(input: {
  ...
  return [
    `Your previous run on ${issueLabel} succeeded, but the issue is still in \`in_progress\` and Workcell cannot identify a valid issue disposition.`,
    "",
    "Resolve the missing disposition before creating or revising any new artifacts. Choose **exactly one** outcome and perform the matching Workcell action:",
    "",
