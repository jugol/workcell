...
const INSTRUCTIONS_ROOT_ALLOWLIST_ENV = "WORKCELL_INSTRUCTIONS_ROOT_ALLOWLIST";

/** True when `candidate` is `base` itself or nests inside it (after `..` normalization). */
...
/** Allowed root prefixes from `WORKCELL_INSTRUCTIONS_ROOT_ALLOWLIST` (OS path-delimited). */
function instructionsRootAllowlist(): string[] {
  const raw = process.env[INSTRUCTIONS_ROOT_ALLOWLIST_ENV];
  if (!raw) return [];
  return raw
    .split(path.delimiter)
    .map((entry) => entry.trim())
    .filter((entry) => entry.length > 0)
    .map((entry) => path.resolve(resolveHomeAwarePath(entry)));
}

/**
 * Guard an absolute external instructions root. A board manager can otherwise point
 * `instructionsRootPath` at ANY absolute host path and have files written under it.
 * Paths inside the instance's own data root are always allowed (managed bundles,
 * workspaces, etc.); any other absolute root must be inside an allowlisted prefix.
 * Returns the (resolved) path on success; throws a 422 validation error otherwise.
 */
function assertExternalRootAllowed(resolvedRoot: string): string {
  const instanceRoot = resolveWorkcellInstanceRoot();
  if (isPathWithin(instanceRoot, resolvedRoot)) return resolvedRoot;

  const allowlist = instructionsRootAllowlist();
  if (allowlist.some((allowed) => isPathWithin(allowed, resolvedRoot))) return resolvedRoot;

  throw unprocessable(
    `External instructions rootPath "${resolvedRoot}" is outside the allowed roots. `
      + `Use a path under the instance data directory, or add an allowed prefix to ${INSTRUCTIONS_ROOT_ALLOWLIST_ENV}.`,
  );
}
...
export function agentInstructionsService() {
  ...
  async function updateBundle(
    ...
    if (nextMode === "managed") {
      ...
      nextRootPath = assertExternalRootAllowed(path.resolve(resolvedRoot));
    }
