...
import { issueExecutionPolicySchema, issueExecutionStateSchema } from "@workcell/shared";
...
type TransitionInput = {
  ...
  /**
   * Autonomous (unattended) mode. When true, execution stages whose participants
   * are ALL users auto-approve instead of parking the issue on a human reviewer/
   * approver — so the multi-agent workflow can run end-to-end without a person in
   * the loop. Agent (review/QA/compound) stages are unaffected.
   */
  skipUserParticipation?: boolean;
};
...
function selectStageParticipant(
  ...
  opts?: {
    ...
    // Autonomous mode: for a stage with both user and agent participants (which
    // is NOT auto-skipped because it's not user-only), prefer the agent so the
    // workflow parks on the agent rather than blocking on a human — fulfilling
    // "skip user participation" for mixed stages too. An explicit `preferred`
    // still wins; this only changes the otherwise-arbitrary first-participant pick.
    preferAgent?: boolean;
  },
  ...
  if (opts?.preferAgent) {
    const agent = participants.find((participant) => participant.type === "agent");
    if (agent) return { type: agent.type, agentId: agent.agentId ?? null, userId: agent.userId ?? null };
  }
...
/**
 * A stage whose participants are ALL users (no agent participant). Autonomous
 * mode auto-approves these so the workflow never parks on a human. Exported so
 * callers can cheaply check whether autonomous mode could possibly apply to a
 * policy before paying for an instance-settings read.
 */
export function stageIsUserOnly(stage: IssueExecutionStage): boolean {
  return stage.participants.length > 0 && stage.participants.every((participant) => participant.type === "user");
}
...
function applyIssueExecutionStageTransition(input: TransitionInput): TransitionResult {
  ...
  const skipUserParticipation = input.skipUserParticipation === true;
  ...
  if (activeStage) {
    ...
    if (principalsEqual(currentParticipant, actor)) {
      if (requestedStatus === "done") {
        ...
        let nextStage = nextPendingStage(input.policy, approvedState);
        // Autonomous mode: auto-approve any subsequent user-only stages instead of
        // parking the issue on a human reviewer/approver. (No-op when the flag is
        // off — the loop guard is false, leaving the original single-step behavior.)
        const postApprovalCompletedStageIds = [...approvedState.completedStageIds];
        while (
          nextStage &&
          skipUserParticipation &&
          requestedStatus === "done" &&
          stageIsUserOnly(nextStage)
        ) {
          postApprovalCompletedStageIds.push(nextStage.id);
          nextStage = nextPendingStage(
            input.policy,
            buildStateWithCompletedStages({
              previous: approvedState,
              completedStageIds: postApprovalCompletedStageIds,
              returnAssignee: existingState?.returnAssignee ?? null,
            }),
          );
        }
        const stagesAutoSkipped =
          postApprovalCompletedStageIds.length !== approvedState.completedStageIds.length;

        if (!nextStage) {
          patch.executionState = stagesAutoSkipped
            ? buildSkippedStageCompletedState({
                previous: approvedState,
                completedStageIds: postApprovalCompletedStageIds,
                returnAssignee: approvedState.returnAssignee,
              })
            : approvedState;
          return { patch, decision: approvalDecision };
        }

        const participant = selectStageParticipant(nextStage, {
          preferred: explicitAssignee,
          exclude: existingState?.returnAssignee ?? null,
          preferAgent: skipUserParticipation,
        });
        if (!participant) {
  ...
  while (
    (!participant && canAutoSkipPendingStage({ stage: pendingStage, returnAssignee, requestedStatus })) ||
    (skipUserParticipation && requestedStatus === "done" && stageIsUserOnly(pendingStage))
  ) {
