...
export interface AdapterSingleTurnInput {
  ...
  authToken?: string;
  // Optional live-output tap: called for every adapter log chunk IN ADDITION
  // to the internal stdout/stderr accumulation. Lets callers stream a turn's
  // output (e.g. persist it as run telemetry) without losing the collected
  // text in the returned result. Errors thrown here are the caller's problem
  // — pass a non-throwing callback.
...
export async function runAdapterSingleTurn(
  ...
  const result = await adapter.execute({
    ...
    onLog: async (stream, chunk) => {
      ...
      if (input.onLog) await input.onLog(stream, chunk);
    },
