...
// Seeded into a freshly-created EMPTY local workspace so every agent (Developer,
// Designer, QA) lays files out the same way instead of improvising per task —
// the gap that lets greenfield projects sprawl. Refined by the Planner per stack.
const GREENFIELD_PROJECT_AGENTS_MD = `# Project layout (Workcell convention)

This file was seeded by Workcell when this workspace was created empty. It defines
where things go so every agent — Developer, Designer, QA — lays files out the same
way instead of improvising per task. The Planner should refine the tree below to the
chosen stack and record the final topology here.

## Default layout

- \`src/\` — application source. Group by feature/domain (not by file type) as it grows.
- \`tests/\` — automated tests that mirror \`src/\` (or co-locate \`*.test.*\` next to source — pick one and stay consistent).
- \`docs/\` — documentation, specs, decisions.
- \`scripts/\` — dev / ops scripts.
- \`assets/\` — static assets (images, fonts, fixtures).
- \`public/\` — web static root (web apps only).
- Manifests and config (package.json, tsconfig, .env.example, README) live at the root.

## Conventions

- One concern per directory; do not dump unrelated files at the root.
- Tests mirror or co-locate with the code they test. Never scatter throwaway files — use a gitignored \`scratch/\` if you must.
- Keep build output, coverage, and other generated artifacts out of source and gitignored.
- Naming: follow the language/framework idiom (kebab-case files, PascalCase component files where expected).

## Per-stack quick reference (keep the one you chose, delete the rest)

- **Web app (React / Vite / Next):** \`src/{components,routes,lib,hooks,styles}\`, \`public/\`, \`tests/\`.
- **Node service / monorepo:** service code in \`src/\`; a monorepo uses \`packages/<name>/\` with shared code in \`packages/shared/\`.
- **Python:** package under \`src/<pkg>/\`, \`tests/\`, \`pyproject.toml\` at the root.
- **Generic / polyglot:** \`src/\`, \`tests/\`, \`docs/\`, \`scripts/\` as above.

> If a real convention already exists in the repo, that wins — update this file to match it.
`;

// Best-effort: write the layout convention into a brand-new empty local workspace.
// Never clobbers an existing project (non-empty dir or pre-existing AGENTS.md), and
// never blocks workspace creation on a filesystem error.
async function seedGreenfieldProjectLayout(
  cwd: string | null,
  sourceType: string,
): Promise<void> {
  if (!cwd || !path.isAbsolute(cwd)) return;
  if (sourceType !== "local_path" && sourceType !== "non_git_path") return;
  try {
    if (existsSync(cwd)) {
      const entries = await fs.readdir(cwd);
      const meaningful = entries.filter((entry) => entry !== ".git" && entry !== ".DS_Store");
      if (meaningful.length > 0) return; // existing project — leave it alone
    } else {
      await fs.mkdir(cwd, { recursive: true });
    }
    const agentsPath = path.join(cwd, "AGENTS.md");
    if (existsSync(agentsPath)) return; // never overwrite an existing AGENTS.md
    await fs.writeFile(agentsPath, GREENFIELD_PROJECT_AGENTS_MD, "utf8");
  } catch {
    // Best-effort only — a filesystem issue must not fail workspace creation.
  }
}
...
export function projectService(db: Db) {
  const createProject = async (
    ...
    // WC-160: insert the project row and sync its goal-link join rows atomically.
    // Previously both ran on the bare db handle, so a failed project_goals insert
    // (e.g. a concurrently-deleted goalId FK violation) left a committed project
    // with zero goal associations.
    const row = await db.transaction(async (tx) => {
      ...
      const inserted = await txDb
        ...
        .then((rows) => rows[0]);
      if (ids && ids.length > 0) {
        await syncGoalLinks(txDb, inserted.id, companyId, ids);
      }
      return inserted;
    });
  ...
  return {
      ...
      if (existingBinding) {
        ...
        if (existingProject) {
          ...
          return {
            ...
            project: project as import("@workcell/shared").Project | null,
        ...
        return {
          ...
          project: hydrated as import("@workcell/shared").Project | null,
          status: "relinked",
        };
      }
    ...
    update: async (
      ...
      // WC-160: update the project row and re-sync its goal-link join rows
      // atomically. syncGoalLinks DELETEs all existing links then re-inserts, so on
      // the bare db handle a failed re-insert would wipe the project's previously
      // good goal associations (a regression from a working state).
      const row = await db.transaction(async (tx) => {
        const txDb = tx as unknown as Db;
    ...
    remove: (id: string) =>
      db.transaction(async (tx) => {
        // WC-135: issues.project_id / cost_events.project_id /
        // finance_events.project_id reference projects.id with NO onDelete, so
        // delete(projects) FK-violates (500) on any project that has issues or
        // billing. Issues are top-level entities — DETACH them (don't delete);
        // cost/finance are billing records — PRESERVE them (null the link). The
        // remaining project children cascade / set-null at the FK level.
    ...
    createWorkspace: async (
      ...
      if (created) {
        // Seed a layout convention into greenfield local workspaces so agents
        // converge on one folder structure instead of sprawling per task.
        await seedGreenfieldProjectLayout(cwd, sourceType);
      }
    ...
    // Move a local on-disk workspace's source folder to a new path AND update its
    // cwd in one step. local_path workspaces are used IN PLACE (the source folder
    // IS the run cwd), so the folder is only moved when no run is using it — hence
    // the active-execution-workspace guard. The next run self-heals (re-resolves
    // cwd from the updated project workspace). Returns a result union the route
    // maps to HTTP status codes.
    relocateWorkspace: async (
      projectId: string,
      workspaceId: string,
      opts: { targetPath: string; force?: boolean },
    ): Promise<
      { ok: true; workspace: ProjectWorkspace } | { ok: false; status: number; error: string }
    > => {
      const existing = await db
        .select()
        .from(projectWorkspaces)
        ...
        .then((rows) => rows[0] ?? null);
      if (!existing) return { ok: false, status: 404, error: "Workspace not found" };

      const currentCwd = normalizeWorkspaceCwd(existing.cwd);
      const sourceType = readNonEmptyString(existing.sourceType);
      if (!currentCwd || (sourceType !== "local_path" && sourceType !== "non_git_path")) {
        return {
          ok: false,
          status: 422,
          error: "Relocate applies only to a local on-disk workspace (a local_path/non_git_path workspace with a folder).",
        };
      }

      const target = path.normalize(opts.targetPath.trim());
      if (!path.isAbsolute(target)) {
        return { ok: false, status: 422, error: "Target path must be an absolute path." };
      }
      if (path.resolve(target).toLowerCase() === path.resolve(currentCwd).toLowerCase()) {
        return { ok: false, status: 422, error: "Target is the same as the current location." };
      }
      const rel = path.relative(currentCwd, target);
      if (rel !== "" && !rel.startsWith("..") && !path.isAbsolute(rel)) {
        return { ok: false, status: 422, error: "Cannot move a folder into a subfolder of itself." };
      }
      if (!existsSync(currentCwd)) {
        return { ok: false, status: 422, error: `Current folder does not exist on disk: ${currentCwd}` };
      }
      if (existsSync(target)) {
        const entries = await fs.readdir(target).catch(() => null);
        if (entries === null || entries.length > 0) {
          return { ok: false, status: 422, error: `Target already exists and is not empty: ${target}` };
        }
      }

      // Guard: a non-terminal execution workspace bound to this project workspace
      // means a run could be using the folder in place — refuse (fail closed).
      const active = await db
        ...
        .where(
          and(
            ...
            eq(executionWorkspaces.projectWorkspaceId, workspaceId),
      ...
      if (active.length > 0) {
        return {
          ok: false,
          status: 409,
          error: "Active execution workspaces are using this folder. Close them (and stop any running dev servers) before relocating.",
        };
      }

      // Move: same-volume rename; cross-volume (EXDEV) copy → verify → remove.
      try {
        await fs.mkdir(path.dirname(target), { recursive: true });
        try {
          await fs.rename(currentCwd, target);
        } catch (err) {
          if ((err as NodeJS.ErrnoException)?.code === "EXDEV") {
            await fs.cp(currentCwd, target, { recursive: true });
            const before = await fs.readdir(currentCwd).catch(() => [] as string[]);
            const after = await fs.readdir(target).catch(() => [] as string[]);
            if (after.length < before.length) {
              throw new Error("copy verification failed (fewer entries at target)");
            }
            await fs.rm(currentCwd, { recursive: true, force: true });
          } else {
            throw err;
          }
        }
      } catch (err) {
        // Fail closed: the source folder is left intact on any move error.
        return {
          ok: false,
          status: 500,
          error: `Could not move the folder (it may be open/locked — close editors and dev servers, then retry): ${err instanceof Error ? err.message : String(err)}`,
        };
      }

      const updated = await db
        .update(projectWorkspaces)
        .set({ cwd: target, updatedAt: new Date() })
        .where(eq(projectWorkspaces.id, workspaceId))
        .returning()
        .then((rows) => rows[0] ?? null);
      if (!updated) {
        return {
          ok: false,
          status: 500,
          error: `Folder was moved to ${target} but the workspace path failed to update — set cwd to ${target} manually.`,
        };
      }
      return { ok: true, workspace: toWorkspace(updated) };
    },
