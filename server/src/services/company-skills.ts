...
import { companies, companySkills } from "@workcell/db";
import { readWorkcellSkillSyncPreference } from "@workcell/adapter-utils/server-utils";
import type { WorkcellSkillEntry } from "@workcell/adapter-utils/server-utils";
import type {
  CompanySkill,
...
import { normalizeAgentUrlKey } from "@workcell/shared";
...
function buildSkillRuntimeName(key: string, slug: string) {
  if (key.startsWith("workcell/workcell/")) return slug;
...
function resolveManagedSkillsRoot(companyId: string) {
  return path.resolve(resolveWorkcellInstanceRoot(), "skills", companyId);
}
...
function deriveSkillSourceInfo(skill: SkillSourceInfoTarget): {
  ...
  if (metadata.sourceKind === "workcell_bundled") {
    return {
      editable: false,
      editableReason: "Bundled Workcell skills are read-only.",
      sourceLabel: "Workcell bundled",
      sourceBadge: "workcell",
      sourcePath: null,
    };
  }
  ...
  if (skill.sourceType === "local_path") {
    ...
    if (localSkillDir && localSkillDir.startsWith(managedRoot)) {
      return {
        editable: true,
        editableReason: null,
        sourceLabel: "Workcell workspace",
        sourceBadge: "workcell",
...
export function companySkillService(db: Db) {
  ...
  async function listRuntimeSkillEntries(
    ...
    for (const skill of skills) {
      ...
      const required = sourceKind === "workcell_bundled";
      out.push({
        key: skill.key,
  ...
  async function upsertImportedSkills(companyId: string, imported: ImportedSkill[]): Promise<CompanySkill[]> {
    ...
    for (const skill of imported) {
      ...
      if (
        existing
        && existingMeta.sourceKind === "workcell_bundled"
        && incomingKind === "github"
        && incomingOwner === "workcell"
        && incomingRepo === "workcell"
      ) {
        out.push(existing);
        continue;
      }
