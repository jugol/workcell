...
import { readWorkcellSkillSyncPreference } from "@workcell/adapter-utils/server-utils";
import { claudeConfigDir, parseClaudeStreamJson } from "@workcell/adapter-claude-local/server";
import { codexHomeDir, parseCodexJsonl } from "@workcell/adapter-codex-local/server";
import {
...
import { resolveHomeAwarePath, resolveWorkcellInstanceRoot } from "../home-paths.js";
...
const FEEDBACK_SCHEMA_VERSION = "workcell-feedback-envelope-v2";
const FEEDBACK_BUNDLE_VERSION = "workcell-feedback-bundle-v2";
const FEEDBACK_PAYLOAD_VERSION = "workcell-feedback-v1";
const FEEDBACK_DESTINATION = "workcell_labs_feedback_v1";
...
function captureStatusFromFiles(files: FeedbackTraceBundleFile[]): FeedbackTraceBundleCaptureStatus {
  ...
  const hasAdapterFiles = files.some((file) =>
    file.source !== "workcell_run" &&
    file.source !== "workcell_run_events" &&
    file.source !== "workcell_run_log",
  );
...
async function buildOpenCodeTraceFiles(input: {
  ...
  const opencodeRoot = resolveHomeAwarePath(
    process.env.WORKCELL_OPENCODE_STORAGE_DIR ?? "~/.local/share/opencode",
  );
...
async function buildAgentContext(
  ...
  const desiredSkillRefs = uniqueNonEmpty(readWorkcellSkillSyncPreference(adapterConfig).desiredSkills).slice(0, MAX_SKILLS);
  ...
  // Order matched skills by the agent's declared `desiredSkillRefs` rather than
  // raw DB row order. The shared feedback envelope is content-addressed
  // (payloadDigest = sha256 over the payload, in which `skills.items` is an
  // ordered array), so a non-deterministic row order would yield unstable
  // digests for identical data. Resolving in desired-ref order also keeps
  // `items` aligned with the `desiredRefs` we already surface below.
  const matchedSkills: Array<typeof companySkills.$inferSelect> = [];
  const matchedSkillIds = new Set<string>();
  for (const reference of desiredSkillRefs) {
    const skill = availableSkills.find(
      (candidate) => !matchedSkillIds.has(candidate.id) && matchesSkillReference(candidate, reference),
    );
    if (skill) {
      matchedSkills.push(skill);
      matchedSkillIds.add(skill.id);
      if (matchedSkills.length >= MAX_SKILLS) break;
    }
  }
...
async function buildFeedbackTraceBundleFromRow(
  ...
  let workcellRun: Record<string, unknown> | null = null;
  ...
  if (!sourceRunId) {
    ...
    if (!run || run.companyId !== row.companyId) {
      ...
      workcellRun = sanitizeFeedbackValue(
        {
          id: run.id,
      ...
      files.push(makeBundleFile({
        path: "workcell/run.json",
        contentType: "application/json",
        source: "workcell_run",
        contents: `${JSON.stringify(workcellRun, null, 2)}\n`,
      }));

      const sanitizedEvents = sanitizeFeedbackValue(
        events,
        state,
        "bundle.workcellRun.events",
        MAX_TRACE_FILE_CHARS,
      );
      files.push(makeBundleFile({
        path: "workcell/run-events.json",
        contentType: "application/json",
        source: "workcell_run_events",
        contents: `${JSON.stringify(sanitizedEvents, null, 2)}\n`,
      }));

      if (logText) {
        files.push(makeBundleFile({
          path: "workcell/run-log.ndjson",
          contentType: "application/x-ndjson",
          source: "workcell_run_log",
          contents: `${sanitizeFeedbackText(logText, state, "bundle.workcellRun.log", MAX_TRACE_FILE_CHARS)}\n`,
        }));
      } else {
