...
import { activityLog, agents, companies, costEvents, goals, heartbeatRuns, issues, projects } from "@workcell/db";
...
export function costService(db: Db, budgetHooks: BudgetServiceHooks = {}) {
  ...
  return {
    createEvent: async (companyId: string, data: Omit<typeof costEvents.$inferInsert, "companyId">) => {
      // WC-137: agent_id is nullable at rest (billing is preserved-but-detached
      // after an agent is deleted), but creating a NEW cost event must always
      // reference a live agent.
      const agentId = data.agentId;
      if (!agentId) throw unprocessable("Cost events require an agentId");
      // WC-162: validate EVERY FK reference exists in this company BEFORE the insert.
      // cost_events.{issue,project,goal,heartbeatRun}_id are hard FKs with no onDelete,
      // so a well-formed but stale/wrong uuid (realistic from an agent reporting cost)
      // would otherwise hit a raw Postgres 23503 → 500 instead of a clean 404/422.
      // Mirrors financeService.createEvent, which validated all FKs; cost only checked
      // the agent.
      await assertBelongsToCompany(db, agents, agentId, companyId, "Agent");
    ...
    // WC-11: batch-resolve total cost cents per issue for an arbitrary subset.
    // Used by the issue list endpoint to populate Issue.totalCostCents so the
    // Kanban/list views can render a "usage" chip without an N+1 per card.
    // Empty input short-circuits without hitting the DB. Issues not present in
    // the returned Map have zero cost events (the caller treats `undefined` as 0).
    sumCostCentsByIssueIds: async (
      companyId: string,
      issueIds: string[],
      ...
      const rows = await db
        .select({
          ...
          totalCents: sumAsNumber(costEvents.costCents),
        })
        .from(costEvents)
        .where(
          and(
      ...
      for (const row of rows) {
        if (row.issueId) map.set(row.issueId, Math.round(Number(row.totalCents ?? 0)));
      }
      return map;
    },
