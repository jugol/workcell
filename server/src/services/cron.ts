 ...
 *
 * Day-of-month and day-of-week follow POSIX/Vixie crontab(5) semantics: when
 * BOTH fields are restricted (neither is `*`), a day matches if EITHER field
 * matches (e.g. `0 0 13 * 5` ⇒ the 13th OR any Friday); when at least one field
 * is `*`, both must match — which reduces to the single restricted field.
 *
 * @module
 */
...
export interface ParsedCron {
  ...
  daysOfWeek: number[];
  /**
   * Whether the day-of-month field was restricted (its token was not `*`).
   * Together with {@link ParsedCron.daysOfWeekRestricted} this drives POSIX
   * day-matching: when BOTH day fields are restricted they combine with OR,
   * otherwise with AND. See {@link cronMatchesDay}.
   */
  daysOfMonthRestricted: boolean;
  /** Whether the day-of-week field was restricted (its token was not `*`). */
...
export function parseCron(expression: string): ParsedCron {
  ...
  return {
    ...
    // POSIX day-matching needs to know whether each day field was a wildcard,
    // which is lost once `*` is expanded to a full value list above.
    daysOfMonthRestricted: tokens[2]!.trim() !== "*",
    daysOfWeekRestricted: tokens[4]!.trim() !== "*",
  };
}
...
/**
 * Determine whether a calendar day satisfies the cron's day-of-month and
 * day-of-week fields, following POSIX/Vixie crontab(5) semantics:
 *
 *  - If BOTH day fields are restricted (neither was `*`), the day matches when
 *    it satisfies EITHER field — e.g. `13 * 5` fires on the 13th **or** any
 *    Friday.
 *  - Otherwise (at least one field is `*`), the day matches only when both sets
 *    contain it, which reduces to the single restricted field because the `*`
 *    field always matches.
 *
 * @param cron       — Parsed cron schedule.
 * @param dayOfMonth — Day of month (1–31).
 * @param dayOfWeek  — Day of week (0–6, Sun=0).
 */
export function cronMatchesDay(
  ...
  if (cron.daysOfMonthRestricted && cron.daysOfWeekRestricted) {
...
export function nextCronTick(cron: ParsedCron, after: Date): Date | null {
  ...
  for (let i = 0; i < maxIterations; i++) {
    ...
    // Check day-of-month / day-of-week per POSIX semantics: OR when both fields
    // are restricted, AND otherwise (see cronMatchesDay).
