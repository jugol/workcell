...
import { trackRoutineRun } from "@workcell/shared/telemetry";
...
import { cronMatchesDay, parseCron, validateCron } from "./cron.js";
...
function matchesCronMinute(expression: string, timeZone: string, date: Date) {
  ...
  return (
    ...
    cronMatchesDay(cron, parts.day, parts.weekday)
  );
}
...
function resolveRoutineVariableValues(
  ...
  for (const variable of variables) {
    ...
    // Workcell manages from execution context, so callers cannot override them.
  ...
  return {
    ...
    create: async (companyId: string, input: CreateRoutine, actor: Actor): Promise<Routine> => {
      ...
      const env = input.env === undefined || input.env === null
        ...
        : await secretsSvc.normalizeEnvBindingsForPersistence(companyId, input.env, {
            strictMode: process.env.WORKCELL_SECRETS_STRICT_MODE === "true",
            fieldPath: "env",
          });
    ...
    createTrigger: async (
      ...
      if (input.kind === "webhook") {
        ...
        secretMaterial = {
          webhookUrl: `${process.env.WORKCELL_API_URL}/api/routine-triggers/public/${publicId}/fire`,
    ...
    rotateTriggerSecret: async (
      ...
      return {
        trigger: trigger as RoutineTrigger,
        secretMaterial: {
          webhookUrl: `${process.env.WORKCELL_API_URL}/api/routine-triggers/public/${existing.publicId}/fire`,
    ...
    firePublicTrigger: async (publicId: string, input: {
      ...
      if (trigger.signingMode === "none") {
        ...
        // generic X-Workcell-Signature header so operators can use github_hmac
    ...
    tickScheduledTriggers: async (now: Date = new Date()) => {
      ...
      for (const row of due) {
        // WC-155: isolate per-row failures. Without this, a single bad trigger —
        // a deleted assignee (→ dispatchRoutineRun throws "Default agent required")
        // or a stored-invalid cron/timezone (→ nextCronTickInTimeZone throws) —
        // aborts the entire tick and starves every other due routine. Mirrors the
        // per-row try/catch the sibling scheduler tickDueIssueMonitors already uses.
        try {
        ...
        for (let i = 0; i < runCount; i += 1) {
          ...
          console.error(
            `[routines] tickScheduledTriggers: trigger ${row.trigger.id} failed; skipping to keep other due routines running`,
            err,
          );
          continue;
        }
