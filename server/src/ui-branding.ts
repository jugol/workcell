const FAVICON_BLOCK_START = "<!-- WORKCELL_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- WORKCELL_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- WORKCELL_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- WORKCELL_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.WORKCELL_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.WORKCELL_WORKTREE_NAME) ?? nonEmpty(env.WORKCELL_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.WORKCELL_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="workcell-worktree-enabled" content="true" />',
    `<meta name="workcell-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="workcell-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="workcell-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
