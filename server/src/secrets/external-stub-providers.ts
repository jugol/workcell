...
function unavailableProvider(
  ...
  return {
    ...
    async createSecret() {
      throw unprocessable(`${id} provider is not configured for Workcell-managed values`);
    },
    async createVersion() {
      throw unprocessable(`${id} provider is not configured for Workcell-managed values`);
    },
    ...
    async deleteOrArchive() {
      // External references are metadata-only in Workcell for unconfigured providers.
    },
    async healthCheck() {
      return {
        provider: id,
