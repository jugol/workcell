...
import type { DeploymentMode, SecretProviderConfigDiscoveryPreviewResult } from "@workcell/shared";
...
const DEFAULT_PREFIX = "workcell";
const DEFAULT_OWNER_TAG = "workcell";
const DEFAULT_VERSION_STAGE = "AWSCURRENT";
const WORKCELL_PENDING_VERSION_STAGE = "WORKCELL_PENDING";
...
const AWS_RUNTIME_CREDENTIAL_WARNING =
  "AWS bootstrap credentials must be available to the Workcell server runtime through the AWS SDK default credential provider chain: IAM role/workload identity, AWS_PROFILE/SSO/shared credentials, web identity, container/instance metadata, or short-lived shell credentials.";
const AWS_CREDENTIAL_CUSTODY_WARNING =
  "Do not store AWS root credentials or long-lived IAM user access keys in Workcell company_secrets; the AWS provider bootstrap belongs in deployment infrastructure, the process environment, an AWS profile, or the orchestrator secret store.";
...
function readProviderVaultConfig(input: SecretProviderVaultRuntimeConfig): AwsSecretsManagerConfig {
  ...
  const recoveryWindowRaw = process.env.WORKCELL_SECRETS_AWS_DELETE_RECOVERY_DAYS?.trim();
  ...
  if (!Number.isFinite(recoveryWindow) || recoveryWindow < 7 || recoveryWindow > 30) {
    throw unprocessable(
      "WORKCELL_SECRETS_AWS_DELETE_RECOVERY_DAYS must be an integer between 7 and 30",
    );
  }
  ...
  return {
    region,
    endpoint:
      process.env.WORKCELL_SECRETS_AWS_ENDPOINT?.trim() ||
...
function getAwsConfigReadiness() {
  const region = (
    process.env.WORKCELL_SECRETS_AWS_REGION ??
    process.env.AWS_REGION ??
    process.env.AWS_DEFAULT_REGION
  )?.trim();
  const deploymentId = process.env.WORKCELL_SECRETS_AWS_DEPLOYMENT_ID?.trim();
  const kmsKeyId = process.env.WORKCELL_SECRETS_AWS_KMS_KEY_ID?.trim();
  const missingConfig: string[] = [];

  if (!region) {
    missingConfig.push("WORKCELL_SECRETS_AWS_REGION or AWS_REGION/AWS_DEFAULT_REGION");
  }
  if (!deploymentId) {
    missingConfig.push("WORKCELL_SECRETS_AWS_DEPLOYMENT_ID");
  }
  if (!kmsKeyId) {
    missingConfig.push("WORKCELL_SECRETS_AWS_KMS_KEY_ID");
  }
...
function loadAwsSecretsManagerConfig(): AwsSecretsManagerConfig {
  ...
  const region =
    process.env.WORKCELL_SECRETS_AWS_REGION?.trim() ||
  ...
  if (!region) {
    throw unprocessable(
      "AWS Secrets Manager provider requires WORKCELL_SECRETS_AWS_REGION or AWS_REGION",
    );
  }
  if (!deploymentId) {
    throw unprocessable(
      "AWS Secrets Manager provider requires WORKCELL_SECRETS_AWS_DEPLOYMENT_ID",
    );
  }
  if (!kmsKeyId) {
    throw unprocessable(
      "AWS Secrets Manager provider requires WORKCELL_SECRETS_AWS_KMS_KEY_ID",
    );
  }
  ...
  return {
    ...
    endpoint:
      ...
      `https://secretsmanager.${region}.amazonaws.com`,
    deploymentId,
    prefix: sanitizePathSegment(process.env.WORKCELL_SECRETS_AWS_PREFIX?.trim() || DEFAULT_PREFIX),
    kmsKeyId,
    environmentTag:
      process.env.WORKCELL_SECRETS_AWS_ENVIRONMENT?.trim() ||
      process.env.NODE_ENV?.trim() ||
      "unknown",
    providerOwnerTag:
      process.env.WORKCELL_SECRETS_AWS_PROVIDER_OWNER?.trim() || DEFAULT_OWNER_TAG,
    deleteRecoveryWindowDays: recoveryWindow,
  };
}
...
function assertNotManagedNamespaceExternalRef(
  ...
  throw unprocessable(
    "AWS Workcell-managed namespace secrets cannot be imported as external references",
  );
}
...
function buildManagedSecretTags(
  ...
  if (!context) return [];
  return [
    { Key: "workcell:managed-by", Value: "workcell" },
    { Key: "workcell:provider-owner", Value: config.providerOwnerTag },
    { Key: "workcell:deployment-id", Value: config.deploymentId },
    { Key: "workcell:company-id", Value: context.companyId },
    { Key: "workcell:secret-key", Value: context.secretKey },
    { Key: "workcell:environment", Value: config.environmentTag },
  ];
}
...
function inferPathSignals(entry: AwsSecretsManagerListSecretEntry, tags: Map<string, string>) {
  ...
  const workcellDeploymentId = tagValue(tags, ["workcell:deployment-id"]);
  const workcellManaged = tagValue(tags, ["workcell:managed-by"])?.toLowerCase() === "workcell";

  if (workcellDeploymentId || workcellManaged) {
    return {
      prefix: segments[0] ?? DEFAULT_PREFIX,
      namespace: workcellDeploymentId ?? segments[1] ?? null,
    };
  }
...
function discoverAwsProviderConfigCandidates(input: {
  ...
  type DiscoverySample = {
    ...
    ownerTag: string | null;
    kmsKeyId: string | null;
    workcellManaged: boolean;
    workcellCompanyId: string | null;
  };

  const skippedWarnings: string[] = [];
  let skippedForeignWorkcellSampleCount = 0;
  ...
  for (const entry of input.entries) {
    ...
    const workcellManaged = tagValue(tags, ["workcell:managed-by"])?.toLowerCase() === "workcell";
    const workcellCompanyId = tagValue(tags, ["workcell:company-id"]);
    if (workcellManaged && workcellCompanyId !== input.companyId) {
      skippedForeignWorkcellSampleCount += 1;
      continue;
    }
    ...
    samples.push({
      ...
      environmentTag: tagValue(tags, ["workcell:environment", "environment", "env", "stage"]),
      ownerTag: tagValue(tags, ["workcell:provider-owner", "owner", "team", "service", "application"]),
  ...
  if (skippedForeignWorkcellSampleCount > 0) {
    skippedWarnings.push(
      `Skipped ${skippedForeignWorkcellSampleCount} Workcell-managed AWS secret sample(s) that were not tagged for this company.`,
    );
  }
  ...
  const candidates = [...groups.values()]
    ...
    .map((group) => {
      ...
      if (group.some((sample) => sample.workcellManaged && sample.workcellCompanyId === input.companyId)) {
        candidateWarnings.push("Sample includes Workcell-managed secrets for this company; do not import them as external references.");
      }
      ...
      return {
        ...
        signals: {
          ...
          sampleCount: group.length,
          workcellManagedSampleCount: group.filter((sample) => sample.workcellManaged).length,
          skippedForeignWorkcellSampleCount,
        },
...
export function createAwsSecretsManagerProvider(
  ...
  async function validateConfig(
    ...
    if (!config.prefix) {
      warnings.push("WORKCELL_SECRETS_AWS_PREFIX should be set to a deployment-scoped prefix");
    }
  ...
  async function healthCheck(
    ...
    try {
      ...
      return {
        ...
        backupGuidance: [
          "Back up Workcell metadata separately from AWS-managed secrets.",
          "Restoring access requires the Workcell database plus the same AWS secret namespace and KMS permissions.",
        ],
      };
    } catch (error) {
      const readiness = getAwsConfigReadiness();
      ...
      return {
        ...
        details: {
          ...
          requiredProviderConfig: input?.providerConfig
            ? ["region"]
            : [
                "WORKCELL_SECRETS_AWS_REGION or AWS_REGION/AWS_DEFAULT_REGION",
                "WORKCELL_SECRETS_AWS_DEPLOYMENT_ID",
                "WORKCELL_SECRETS_AWS_KMS_KEY_ID",
              ],
          optionalProviderConfig: [
            "WORKCELL_SECRETS_AWS_PREFIX",
            "WORKCELL_SECRETS_AWS_ENVIRONMENT",
            "WORKCELL_SECRETS_AWS_PROVIDER_OWNER",
            "WORKCELL_SECRETS_AWS_ENDPOINT",
            "WORKCELL_SECRETS_AWS_DELETE_RECOVERY_DAYS",
          ],
          credentialSource: "AWS SDK default credential provider chain",
          detectedCredentialSources: readiness.credentialSources,
        },
    ...
    async createSecret(input) {
      ...
      try {
        const createInput = {
          ...
          Description: input.context ? `Workcell secret ${input.context.secretName}` : undefined,
    ...
    async createVersion(input) {
      ...
      try {
        const created = await gateway.putSecretValue({
          SecretId: secretId,
          SecretString: input.value,
          VersionStages: [WORKCELL_PENDING_VERSION_STAGE],
        });
        const normalizedSecretId = created.ARN ?? created.Name ?? secretId;
        return {
          material: createManagedMaterial(normalizedSecretId, created.VersionId ?? null),
        ...
        if (input.mode === "archive") {
          if (material.versionId && gateway.updateSecretVersionStage) {
            await gateway.updateSecretVersionStage({
              SecretId: secretId,
              VersionStage: WORKCELL_PENDING_VERSION_STAGE,
