...
const JWT_SECRET_ENV_KEY = "WORKCELL_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by Workcell CLI commands",
...
export function resolveWorkcellEnvFile(configPath?: string): string {
...
export function loadWorkcellEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeWorkcellEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readWorkcellEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeWorkcellEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeWorkcellEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readWorkcellEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeWorkcellEnvEntries(next, filePath);
  return next;
}
