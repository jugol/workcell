...
import type { SecretProvider } from "@workcell/shared";
import type { SecretsConfig } from "../config/schema.js";
import { resolveDefaultSecretsKeyFilePath, resolveWorkcellInstanceId } from "../config/home.js";
  ...
  if (provider !== "local_encrypted") {
    p.note(
      provider === "aws_secrets_manager"
        ? "AWS credentials must come from the Workcell server runtime (IAM role/workload identity, AWS_PROFILE/SSO/shared credentials, or short-lived shell env), not from Workcell company secrets."
