...
export class WorkcellApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-workcell-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "Could not reach the Workcell API.",
    "",
  ...
  lines.push(
    "",
    "This usually means the Workcell server is not running, the configured URL is wrong, or the request is being blocked before it reaches Workcell.",
    "",
    "Try:",
    "- Start Workcell with `pnpm dev` or `pnpm workcell run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If Workcell is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`WORKCELL_API_URL\`.`,
  );
  return lines.join("\n");
}
