...
type OnboardDefaults = Pick<WorkcellConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;

const TAILNET_BIND_WARNING =
  "No Tailscale address was detected during setup. The saved config will stay on loopback until Tailscale is available or WORKCELL_TAILNET_BIND_HOST is set.";

const ONBOARD_ENV_KEYS = [
  "WORKCELL_PUBLIC_URL",
  "DATABASE_URL",
  "WORKCELL_DB_BACKUP_ENABLED",
  "WORKCELL_DB_BACKUP_INTERVAL_MINUTES",
  "WORKCELL_DB_BACKUP_RETENTION_DAYS",
  "WORKCELL_DB_BACKUP_DIR",
  "WORKCELL_DEPLOYMENT_MODE",
  "WORKCELL_DEPLOYMENT_EXPOSURE",
  "WORKCELL_BIND",
  "WORKCELL_BIND_HOST",
  "WORKCELL_TAILNET_BIND_HOST",
  "HOST",
  "PORT",
  "SERVE_UI",
  "WORKCELL_ALLOWED_HOSTNAMES",
  "WORKCELL_AUTH_BASE_URL_MODE",
  "WORKCELL_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "WORKCELL_STORAGE_PROVIDER",
  "WORKCELL_STORAGE_LOCAL_DIR",
  "WORKCELL_STORAGE_S3_BUCKET",
  "WORKCELL_STORAGE_S3_REGION",
  "WORKCELL_STORAGE_S3_ENDPOINT",
  "WORKCELL_STORAGE_S3_PREFIX",
  "WORKCELL_STORAGE_S3_FORCE_PATH_STYLE",
  "WORKCELL_SECRETS_PROVIDER",
  "WORKCELL_SECRETS_STRICT_MODE",
  "WORKCELL_SECRETS_MASTER_KEY_FILE",
] as const;
...
function describeServerBinding(server: Pick<WorkcellConfig["server"], "bind" | "customBindHost" | "host" | "port">): string {
...
function quickstartDefaultsFromEnv(opts?: { preferTrustedLocal?: boolean }): {
  ...
  const instanceId = resolveWorkcellInstanceId();
  ...
  const publicUrl = preferTrustedLocal
    ? undefined
    : (
      process.env.WORKCELL_PUBLIC_URL?.trim() ||
      process.env.WORKCELL_AUTH_PUBLIC_BASE_URL?.trim() ||
  ...
  const deploymentMode = preferTrustedLocal
    ? "local_trusted"
    : (parseEnumFromEnv<DeploymentMode>(process.env.WORKCELL_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted");
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.WORKCELL_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  const deploymentExposure =
    deploymentMode === "local_trusted" ? "private" : (deploymentExposureFromEnv ?? "private");
  const bindFromEnv = parseEnumFromEnv<BindMode>(process.env.WORKCELL_BIND, BIND_MODES);
  const customBindHostFromEnv = process.env.WORKCELL_BIND_HOST?.trim() || undefined;
  ...
  const resolvedBind = resolveRuntimeBind({
    ...
    tailnetBindHost: process.env.WORKCELL_TAILNET_BIND_HOST?.trim(),
  });
  const authPublicBaseUrl = publicUrl;
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.WORKCELL_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.WORKCELL_ALLOWED_HOSTNAMES
    ? process.env.WORKCELL_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.WORKCELL_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.WORKCELL_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.WORKCELL_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.WORKCELL_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.WORKCELL_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.WORKCELL_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.WORKCELL_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.WORKCELL_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.WORKCELL_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.WORKCELL_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.WORKCELL_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.WORKCELL_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.WORKCELL_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.WORKCELL_SECRETS_MASTER_KEY_FILE) ??
  ...
  if (deploymentMode === "local_trusted" && process.env.WORKCELL_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "WORKCELL_DEPLOYMENT_EXPOSURE",
      reason: "Ignored because deployment mode local_trusted always forces private exposure",
    });
  }
  if (deploymentMode === "local_trusted" && process.env.WORKCELL_BIND !== undefined) {
    ignoredEnvKeys.push({
      key: "WORKCELL_BIND",
      reason: "Ignored because deployment mode local_trusted always uses loopback reachability",
    });
  }
  if (deploymentMode === "local_trusted" && process.env.WORKCELL_BIND_HOST !== undefined) {
    ignoredEnvKeys.push({
      key: "WORKCELL_BIND_HOST",
      reason: "Ignored because deployment mode local_trusted always uses loopback reachability",
    });
  }
...
function canCreateBootstrapInviteImmediately(config: Pick<WorkcellConfig, "database" | "server">): boolean {
...
export async function onboard(opts: OnboardOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" workcell onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolveWorkcellInstanceId());
  p.log.message(
    pc.dim(
  ...
  let existingConfig: WorkcellConfig | null = null;
  ...
  if (existingConfig) {
    p.log.message(
      pc.dim("Existing Workcell install detected; keeping the current configuration unchanged."),
    );
    p.log.message(pc.dim(`Use ${pc.cyan("workcell configure")} if you want to change settings.`));

    const jwtSecret = ensureAgentJwtSecret(configPath);
    const envFilePath = resolveAgentJwtEnvFile(configPath);
    if (jwtSecret.created) {
      p.log.success(`Created ${pc.cyan("WORKCELL_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    } else if (process.env.WORKCELL_AGENT_JWT_SECRET?.trim()) {
      p.log.info(`Using existing ${pc.cyan("WORKCELL_AGENT_JWT_SECRET")} from environment`);
    } else {
      p.log.info(`Using existing ${pc.cyan("WORKCELL_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    }
    ...
    p.note(
      [
        ...
        "Agent auth: WORKCELL_AGENT_JWT_SECRET configured",
      ].join("\n"),
      "Configuration ready",
    );
    ...
    p.note(
      [
        `Run: ${pc.cyan("workcell run")}`,
        `Reconfigure later: ${pc.cyan("workcell configure")}`,
        `Diagnose setup: ${pc.cyan("workcell doctor")}`,
      ].join("\n"),
      "Next commands",
    );
    ...
    if (shouldRunNow && !opts.invokedByRun) {
      process.env.WORKCELL_OPEN_ON_LISTEN = "true";
      const { runCommand } = await import("./run.js");
      await runCommand({ config: configPath, repair: true, yes: true });
      return;
    }

    p.outro("Existing Workcell setup is ready.");
    return;
  }
  ...
  let llm: WorkcellConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@workcell/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `workcell doctor`"));
      }
    }
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        "Bootstrap admin invite will be created after the server starts.",
        `Next: ${pc.cyan("workcell run")}`,
        `Then: ${pc.cyan("workcell auth bootstrap-admin")}`,
      ].join("\n"),
    );
  }
