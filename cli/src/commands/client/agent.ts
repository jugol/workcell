...
function buildAgentEnvExports(input: {
  ...
  return [
    `export WORKCELL_API_URL='${escaped(input.apiBase)}'`,
    `export WORKCELL_COMPANY_ID='${escaped(input.companyId)}'`,
    `export WORKCELL_AGENT_ID='${escaped(input.agentId)}'`,
    `export WORKCELL_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local Workcell skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing Workcell skills into ~/.codex/skills and ~/.claude/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            const skillsDir = await resolveWorkcellSkillsDir(__moduleDir, [path.resolve(process.cwd(), "skills")]);
            if (!skillsDir) {
              throw new Error(
                "Could not locate local Workcell skills directory. Expected ./skills in the repo checkout.",
              );
            }
