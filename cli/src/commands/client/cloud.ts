...
import { resolveWorkcellInstanceId } from "../../config/home.js";
import {
...
export function registerCloudCommands(program: Command): void {
  const cloud = program.command("cloud").description("Workcell Cloud upstream sync commands");

  addCommonClientOptions(
    cloud
      .command("connect")
      .description("Authorize this local instance to push into a Workcell Cloud stack")
      .argument("<remote-url>", "Workcell Cloud stack URL")
  ...
  addCommonClientOptions(
    cloud
      .command("push")
      .description("Preview or apply a local company push into the connected Workcell Cloud stack")
...
export async function connectCloud(remoteUrl: string, opts: CloudConnectOptions = {}): Promise<CloudConnection> {
  ...
  if (ctx.json) {
    ...
    console.log(pc.bold("Connected to Workcell Cloud"));
...
export async function pushCloud(opts: CloudPushOptions): Promise<unknown> {
  ...
  if (!connection) {
    throw new Error("No cloud connection found. Run `workcell cloud connect <remote-url>` first.");
  }
  ...
  const coordinator = new LocalUpstreamPushCoordinator({
    ...
    workcellCompanyId: connection.targetCompanyId,
...
export async function discoverUpstream(remoteUrl: string): Promise<UpstreamDiscovery> {
  ...
  const discoveryUrl = new URL("/.well-known/workcell-upstream", base);
...
export function assertDiscoveryCompatible(discovery: UpstreamDiscovery): void {
  if (discovery.schema !== "workcell-upstream-discovery-v1") {
    throw new Error("Remote URL is not a Workcell Cloud upstream target.");
  }
  ...
  if (!discovery.transfer.featureFlags?.includes("cloud_sync")) {
    throw new Error("Remote Workcell Cloud stack does not advertise the cloud_sync transfer flag.");
  }
}
...
export async function buildBundleFromLocalCompany(input: {
  ...
  const source: UpstreamTransferManifestSource = {
    ...
    exporterVersion: "workcell-cli-cloud-v1",
    sourceSchemaVersion: "workcell-local-portability-v1",
  };
...
async function authorizeConnection(
  ...
  if (!discovery.auth.deviceCode) {
    throw new Error("Remote Workcell Cloud stack does not support device-code authorization.");
  }
  ...
  const entities: LocalUpstreamExportEntityInput[] = [
    {
      key: companyKey,
      body: {
        kind: "workcell_company_portability_manifest",
        manifest: exported.manifest,
        rootPath: exported.rootPath,
        workcellExtensionPath: exported.workcellExtensionPath,
...
async function assertCloudSyncEnabled(settingsPromise: Promise<InstanceExperimentalSettings | null>): Promise<void> {
  ...
  if (settings?.enableCloudSync !== true) {
    throw new Error(
      "Cloud sync is disabled. Enable the cloud sync experimental setting before running `workcell cloud push`.",
    );
  }
}
...
function cloudProofHeaders(connection: CloudConnection, method: string, pathAndSearch: string): Record<string, string> {
  ...
  return {
    ...
    "X-Workcell-Upstream-Source-Instance-Id": connection.sourceInstanceId,
    "X-Workcell-Upstream-Proof-Timestamp": timestamp,
    "X-Workcell-Upstream-Proof-Nonce": nonce,
    "X-Workcell-Upstream-Proof-Signature": sign(
      null,
...
function createSourceIdentity() {
  ...
  const sourceInstanceFingerprint = `sha256:${createHash("sha256")
    ...
    .digest("hex")}`;
  return {
    sourceInstanceId: `workcell-local-${resolveWorkcellInstanceId()}`,
...
async function startPkceCallbackServer(): Promise<{
  ...
  const server = createServer((req, res) => {
    ...
    if (!code || state !== expectedState) {
      ...
      res.end("Workcell Cloud authorization failed. You can close this tab.");
      rejectCode?.(new Error("Authorization callback was missing a valid code or state."));
      return;
    }
    res.writeHead(200, { "Content-Type": "text/plain" });
    res.end("Workcell Cloud authorization complete. You can close this tab.");
    resolveCode?.(code);
  });
