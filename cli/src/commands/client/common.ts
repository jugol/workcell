...
import { ApiRequestError, WorkcellApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to Workcell config file")
    .option("-d, --data-dir <path>", "Workcell data directory root (isolates state from ~/.workcell)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the Workcell API")
...
export function resolveCommandContext(
  ...
  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.WORKCELL_API_KEY?.trim() ||
  ...
  const companyId =
    ...
    process.env.WORKCELL_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set WORKCELL_COMPANY_ID, or set context profile companyId via `workcell context set`.",
    );
  }

  const api = new WorkcellApiClient({
    apiBase,
    apiKey,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.WORKCELL_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.WORKCELL_SERVER_PORT || "");
