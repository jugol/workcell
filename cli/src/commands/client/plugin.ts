...
} from "../../../../packages/plugins/create-workcell-plugin/src/index.js";
...
export function renderLocalPluginInstallHint(packagePath: string): string {
  return [
    ...
    pc.dim(`Keep ${pc.cyan("pnpm dev")} running in ${packagePath}; Workcell watches rebuilt dist output and reloads the plugin worker.`),
  ].join("\n");
}
...
export function buildPluginInitNextCommands(outputDir: string): string[] {
  ...
  return [
    ...
    "pnpm install",
    "pnpm dev",
    `workcell plugin install ${quotedOutputDir}`,
  ];
}
...
export function registerPluginCommands(program: Command): void {
  ...
  addCommonClientOptions(
    plugin
      ...
      .description("Scaffold a local Workcell plugin project")
      ...
      .option("--sdk-path <path>", "Local @workcell/plugin-sdk package path")
    ...
    plugin
      ...
      .description(
        "Install a plugin from a local path or npm package.\n" +
          "  Examples:\n" +
          "    workcell plugin install ./my-plugin              # local path\n" +
          "    workcell plugin install @acme/plugin-linear      # npm package\n" +
          "    workcell plugin install @acme/plugin-linear@1.2  # pinned version",
      )
          ...
          for (const ex of rows) {
            console.log(
              `${pc.bold(ex.displayName)}  ${pc.dim(ex.pluginKey)}\n` +
                `  ${ex.description}\n` +
                `  ${pc.cyan(`workcell plugin install ${ex.localPath}`)}`,
            );
          }
        } catch (err) {
          handleCommandError(err);
