...
import { createDb, instanceUserRoles, invites } from "@workcell/db";
import { inferBindModeFromHost } from "@workcell/shared";
...
export async function bootstrapAdminInvite(opts: {
  ...
  const config = readConfig(configPath);
  if (!config) {
    p.log.error(`No config found at ${configPath}. Run ${pc.cyan("workcell onboard")} first.`);
    return;
  }

  if (config.server.deploymentMode !== "authenticated") {
    p.log.info("Deployment mode is local_trusted. Bootstrap admin invite is only required for authenticated mode.");
    return;
  }
  ...
  try {
    ...
    await db
      ...
      .where(
        and(
          // stable DB contract value (not user-facing)
    ...
    p.log.info("If using embedded-postgres, start the Workcell server and run this command again.");
  } finally {
