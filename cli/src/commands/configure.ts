...
function defaultConfig(): WorkcellConfig {
...
export async function configure(opts: {
  ...
  p.intro(pc.bgCyan(pc.black(" workcell configure ")));
  const configPath = resolveConfigPath(opts.config);

  if (!configExists(opts.config)) {
    p.log.error("No config file found. Run `workcell onboard` first.");
    p.outro("");
    return;
  }
      ...
      case "secrets":
        config.secrets = await promptSecrets(config.secrets);
        {
          ...
          if (keyResult.status === "created") {
            ...
            p.log.message(pc.dim("Skipping local key file management because WORKCELL_SECRETS_MASTER_KEY is set"));
          }
        }
        break;
    }
