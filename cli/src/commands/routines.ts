...
async function openConfiguredDb(configPath: string): Promise<{
  ...
  try {
    if (config.database.mode === "embedded-postgres") {
      ...
      const adminConnectionString = `postgres://workcell:workcell@127.0.0.1:${embeddedHandle.port}/postgres`;
      await ensurePostgresDatabase(adminConnectionString, "workcell");
      const connectionString = `postgres://workcell:workcell@127.0.0.1:${embeddedHandle.port}/workcell`;
...
export async function disableAllRoutinesInConfig(
  ...
  const companyId =
    ...
    ?? nonEmpty(process.env.WORKCELL_COMPANY_ID)
    ?? null;
  if (!companyId) {
    throw new Error("Company ID is required. Pass --company-id or set WORKCELL_COMPANY_ID.");
  }
