...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@workcell/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://workcell:workcell@127.0.0.1:${port}/workcell`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" workcell db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveWorkcellInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "workcell";
