...
import { ensureAgentJwtSecret, loadWorkcellEnvFile, mergeWorkcellEnvEntries, readWorkcellEnvEntries, resolveWorkcellEnvFile } from "../config/env.js";
import { expandHomePrefix } from "../config/home.js";
import type { WorkcellConfig } from "../config/schema.js";
import { readConfig, resolveConfigPath, writeConfig } from "../config/store.js";
import { printWorkcellCliBanner } from "../utils/banner.js";
...
function isCurrentSourceConfigPath(sourceConfigPath: string): boolean {
  const currentConfigPath = process.env.WORKCELL_CONFIG;
...
const WORKTREE_NAME_PREFIX = "workcell-";
...
function resolveWorktreeHome(explicit?: string): string {
  return explicit ?? process.env.WORKCELL_WORKTREES_DIR ?? DEFAULT_WORKTREE_HOME;
}

function resolveWorktreeStartPoint(explicit?: string): string | undefined {
  return explicit ?? nonEmpty(process.env.WORKCELL_WORKTREE_START_POINT) ?? undefined;
}
...
function createConfiguredStorageFromWorkcellConfig(config: WorkcellConfig): ConfiguredStorage {
...
function openConfiguredStorage(configPath: string): ConfiguredStorage {
  ...
  return createConfiguredStorageFromWorkcellConfig(config);
}
...
function resolveRepoManagedWorktreesRoot(cwd: string): string | null {
  ...
  const marker = `${path.sep}.workcell${path.sep}worktrees${path.sep}`;
  const index = normalized.indexOf(marker);
  if (index === -1) return null;
  const repoRoot = normalized.slice(0, index);
  return path.resolve(repoRoot, ".workcell", "worktrees");
}
...
function collectClaimedWorktreePorts(homeDir: string, currentInstanceId: string, cwd: string): {
  ...
  if (repoManagedWorktreesRoot && existsSync(repoManagedWorktreesRoot)) {
    for (const entry of readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const configPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".workcell", "config.json");
      if (existsSync(configPath)) {
        configPaths.add(configPath);
      }
    }
...
export function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.workcell"));
...
export function resolveWorktreeReseedTargetPaths(input: {
  ...
  const envEntries = readWorkcellEnvEntries(resolveWorkcellEnvFile(input.configPath));
  const homeDir = nonEmpty(envEntries.WORKCELL_HOME);
  const instanceId = nonEmpty(envEntries.WORKCELL_INSTANCE_ID);

  if (!homeDir || !instanceId) {
    throw new Error(
      `Target config ${input.configPath} does not look like a worktree-local Workcell instance. Expected WORKCELL_HOME and WORKCELL_INSTANCE_ID in the adjacent .env.`,
    );
  }
...
function resolveExistingGitWorktree(selector: string, cwd: string): MergeSourceChoice | null {
  ...
  if (existsSync(directPath)) {
    return {
      ...
      hasWorkcellConfig: existsSync(path.resolve(directPath, ".workcell", "config.json")),
...
async function ensureRepairTargetWorktree(input: {
  ...
  const currentConfigPath = path.resolve(currentRoot, ".workcell", "config.json");
  ...
  if (existing) {
    return {
      ...
      configPath: path.resolve(existing.worktree, ".workcell", "config.json"),
  ...
  return {
    rootPath: targetPath,
    configPath: path.resolve(targetPath, ".workcell", "config.json"),
    label: branchName,
    branchName,
    created: true,
  };
...
function resolveSourceConnectionString(config: WorkcellConfig, envEntries: Record<string, string>, portOverride?: number): string {
  ...
  return `postgres://workcell:workcell@127.0.0.1:${port}/workcell`;
}

export function copySeededSecretsKey(input: {
  sourceConfigPath: string;
  sourceConfig: WorkcellConfig;
  ...
  const sourceInlineMasterKey =
    nonEmpty(input.sourceEnvEntries.WORKCELL_SECRETS_MASTER_KEY) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.WORKCELL_SECRETS_MASTER_KEY) : null);
  ...
  const sourceKeyFileOverride =
    nonEmpty(input.sourceEnvEntries.WORKCELL_SECRETS_MASTER_KEY_FILE) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.WORKCELL_SECRETS_MASTER_KEY_FILE) : null);
...
async function seedWorktreeDatabase(input: {
  ...
  const sourceEnvFile = resolveWorkcellEnvFile(input.sourceConfigPath);
  const sourceEnvEntries = readWorkcellEnvEntries(sourceEnvFile);
  ...
  try {
    if (input.sourceConfig.database.mode === "embedded-postgres") {
      ...
      const sourceAdminConnectionString = `postgres://workcell:workcell@127.0.0.1:${sourceHandle.port}/postgres`;
      await ensurePostgresDatabase(sourceAdminConnectionString, "workcell");
    }
    ...
    const adminConnectionString = `postgres://workcell:workcell@127.0.0.1:${targetHandle.port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "workcell");
    const targetConnectionString = `postgres://workcell:workcell@127.0.0.1:${targetHandle.port}/workcell`;
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  if (opts.force) {
    ...
    // <repo>/.workcell/worktrees/ holding every repo-managed worktree
  ...
  const sourceEnvEntries = readWorkcellEnvEntries(resolveWorkcellEnvFile(sourceConfigPath));
  const existingAgentJwtSecret =
    nonEmpty(sourceEnvEntries.WORKCELL_AGENT_JWT_SECRET) ??
    nonEmpty(process.env.WORKCELL_AGENT_JWT_SECRET);
  mergeWorkcellEnvEntries(
    {
      ...buildWorktreeEnvEntries(paths, branding),
      ...(existingAgentJwtSecret ? { WORKCELL_AGENT_JWT_SECRET: existingAgentJwtSecret } : {}),
    },
    paths.envPath,
  );
  ensureAgentJwtSecret(paths.configPath);
  loadWorkcellEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run Workcell inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}

export async function worktreeInitCommand(opts: WorktreeInitOptions): Promise<void> {
  printWorkcellCliBanner();
  p.intro(pc.bgCyan(pc.black(" workcell worktree init ")));
  await runWorktreeInit(opts);
}

export async function worktreeMakeCommand(nameArg: string, opts: WorktreeMakeOptions): Promise<void> {
  printWorkcellCliBanner();
  p.intro(pc.bgCyan(pc.black(" workcell worktree:make ")));
...
function toMergeSourceChoices(cwd: string): MergeSourceChoice[] {
  ...
  return parseGitWorktreeList(cwd).map((entry) => {
    ...
    return {
      ...
      branch: entry.branch,
      branchLabel,
      hasWorkcellConfig: existsSync(path.resolve(worktreePath, ".workcell", "config.json")),
...
export async function worktreeCleanupCommand(nameArg: string, opts: WorktreeCleanupOptions): Promise<void> {
  printWorkcellCliBanner();
  p.intro(pc.bgCyan(pc.black(" workcell worktree:cleanup ")));
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  ...
  const envPath = resolveWorkcellEnvFile(configPath);
  const envEntries = readWorkcellEnvEntries(envPath);
  const out = {
    WORKCELL_CONFIG: configPath,
    ...(envEntries.WORKCELL_HOME ? { WORKCELL_HOME: envEntries.WORKCELL_HOME } : {}),
    ...(envEntries.WORKCELL_INSTANCE_ID ? { WORKCELL_INSTANCE_ID: envEntries.WORKCELL_INSTANCE_ID } : {}),
    ...(envEntries.WORKCELL_CONTEXT ? { WORKCELL_CONTEXT: envEntries.WORKCELL_CONTEXT } : {}),
    ...envEntries,
  };
...
function resolveAttachmentLookupStorages(input: {
  ...
  const orderedConfigPaths = [
    ...
    ...toMergeSourceChoices(process.cwd())
      .filter((choice) => choice.hasWorkcellConfig)
      .map((choice) => path.resolve(choice.worktree, ".workcell", "config.json")),
  ];
...
async function openConfiguredDb(configPath: string): Promise<OpenDbHandle> {
  ...
  const envEntries = readWorkcellEnvEntries(resolveWorkcellEnvFile(configPath));
    ...
    if (migrationState.status !== "upToDate") {
      const pending =
        migrationState.reason === "pending-migrations"
          ...
          : "";
      throw new Error(
        `Database for ${configPath} is not up to date.${pending} Run \`pnpm db:migrate\` (or start Workcell once) before using worktree merge history.`,
      );
    }
...
function resolveRunningEmbeddedPostgresPid(config: WorkcellConfig): number | null {
  if (config.database.mode !== "embedded-postgres") {
    return null;
  }
...
export async function worktreeListCommand(opts: WorktreeListOptions): Promise<void> {
  ...
  for (const choice of choices) {
    const flags = [
      ...
      choice.hasWorkcellConfig ? "workcell" : "no-workcell-config",
...
function resolveEndpointFromChoice(choice: MergeSourceChoice): ResolvedWorktreeEndpoint {
  ...
  return {
    ...
    configPath: path.resolve(choice.worktree, ".workcell", "config.json"),
...
function resolveWorktreeEndpointFromSelector(
  ...
  if (existsSync(directPath)) {
    ...
    const configPath = path.resolve(directPath, ".workcell", "config.json");
    if (!existsSync(configPath)) {
      throw new Error(`Resolved worktree path ${directPath} does not contain .workcell/config.json.`);
    }
    return {
      rootPath: directPath,
      configPath,
  ...
  if (!matched.hasWorkcellConfig && !matched.isCurrent) {
    throw new Error(`Resolved worktree "${selector}" does not look like a Workcell worktree.`);
  }
...
async function promptForSourceEndpoint(excludeWorktreePath?: string): Promise<ResolvedWorktreeEndpoint> {
  ...
  const currentEndpoint = resolveCurrentEndpoint();
  const choices = toMergeSourceChoices(process.cwd())
    .filter((choice) => choice.hasWorkcellConfig || choice.isCurrent)
  ...
  if (choices.length === 0) {
    throw new Error("No Workcell worktrees were found. Run `workcell worktree:list` to inspect the repo worktrees.");
  }
...
export async function worktreeMergeHistoryCommand(sourceArg: string | undefined, opts: WorktreeMergeHistoryOptions): Promise<void> {
  ...
  if (path.resolve(sourceEndpoint.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target Workcell configs are the same. Choose different --from/--to worktrees.");
  }
...
async function runWorktreeReseed(opts: WorktreeReseedOptions): Promise<void> {
  ...
  if (path.resolve(source.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target Workcell configs are the same. Choose different --from/--to values.");
  }
  ...
  if (runningTargetPid && !opts.allowLiveTarget) {
    throw new Error(
      `Target worktree database appears to be running (pid ${runningTargetPid}). Stop Workcell in ${targetEndpoint.rootPath} before reseeding, or re-run with --allow-live-target if you want to override this guard.`,
    );
  }
  ...
  const confirmed = opts.yes
    ? true
    : await p.confirm({
      message: `Overwrite the isolated Workcell DB for ${targetEndpoint.label} from ${source.label} using ${seedMode} seed mode?`,
      initialValue: false,
    });
  if (p.isCancel(confirmed) || !confirmed) {
...
export async function worktreeRepairCommand(opts: WorktreeRepairOptions): Promise<void> {
  ...
  if (path.resolve(source.configPath) === path.resolve(target.configPath)) {
    throw new Error("Source and target Workcell configs are the same. Use --from-config/--from-instance to point repair at a different source.");
  }

  const targetConfig = existsSync(target.configPath) ? readConfig(target.configPath) : null;
  const targetEnvEntries = readWorkcellEnvEntries(resolveWorkcellEnvFile(target.configPath));
  const targetHasWorktreeEnv = Boolean(
    nonEmpty(targetEnvEntries.WORKCELL_HOME) && nonEmpty(targetEnvEntries.WORKCELL_INSTANCE_ID),
  );
  ...
  if (runningTargetPid && !opts.allowLiveTarget) {
    throw new Error(
      `Target worktree database appears to be running (pid ${runningTargetPid}). Stop Workcell in ${target.rootPath} before repairing, or re-run with --allow-live-target if you want to override this guard.`,
    );
  }
  if (runningTargetPid && opts.allowLiveTarget) {
    p.log.warning(`Proceeding even though the target embedded PostgreSQL appears to be running (pid ${runningTargetPid}).`);
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local Workcell instance helpers");

  program
    .command("worktree:make")
    .description("Create ~/NAME as a git worktree, then initialize an isolated Workcell instance inside it")
    .argument("<name>", "Worktree name — auto-prefixed with workcell- if needed (created at ~/workcell-NAME)")
    .option("--start-point <ref>", "Remote ref to base the new branch on (env: WORKCELL_WORKTREE_START_POINT)")
    .option("--instance <id>", "Explicit isolated instance id")
    .option("--home <path>", `Home root for worktree instances (env: WORKCELL_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source WORKCELL_HOME used when deriving the source config")
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local Workcell instance")
  ...
  program
    ...
    .description("List git worktrees visible from this repo and whether they look like Workcell worktrees")
  ...
  worktree
    .command("reseed")
    .description("Re-seed an existing worktree-local instance from another Workcell instance or worktree")
    .option("--from <worktree>", "Source worktree path, directory name, branch name, or current")
    .option("--to <worktree>", "Target worktree path, directory name, branch name, or current (defaults to current)")
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source WORKCELL_HOME used when deriving the source config")
  ...
  worktree
    .command("repair")
    .description("Create or repair a linked worktree-local Workcell instance without touching the primary checkout")
    .option("--branch <name>", "Existing branch/worktree selector to repair, or a branch name to create under .workcell/worktrees")
    .option("--home <path>", `Home root for worktree instances (env: WORKCELL_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source WORKCELL_HOME used when deriving the source config")
  ...
  program
    ...
    .argument("<name>", "Worktree name — auto-prefixed with workcell- if needed")
