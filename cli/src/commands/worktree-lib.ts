...
export const DEFAULT_WORKTREE_HOME = "~/.workcell-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".workcell");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: WorkcellConfig | null;
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "workcell",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    WORKCELL_HOME: paths.homeDir,
    WORKCELL_INSTANCE_ID: paths.instanceId,
    WORKCELL_CONFIG: paths.configPath,
    WORKCELL_CONTEXT: paths.contextPath,
    WORKCELL_IN_WORKTREE: "true",
    ...(branding?.name ? { WORKCELL_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { WORKCELL_WORKTREE_COLOR: branding.color } : {}),
  };
}
