...
function createExistingConfigFixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "workcell-onboard-"));
  const runtimeRoot = path.join(root, "runtime");
  const configPath = path.join(root, ".workcell", "config.json");
...
function createFreshConfigPath() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "workcell-onboard-fresh-"));
  return path.join(root, ".workcell", "config.json");
}
...
describe("onboard", () => {
  beforeEach(() => {
    ...
    delete process.env.WORKCELL_BIND;
    delete process.env.WORKCELL_BIND_HOST;
    delete process.env.WORKCELL_TAILNET_BIND_HOST;
  ...
  it("keeps --yes onboarding on local trusted loopback defaults", async () => {
    ...
    process.env.WORKCELL_BIND = "lan";

    await onboard({ config: configPath, yes: true, invokedByRun: true });

    const raw = JSON.parse(fs.readFileSync(configPath, "utf8")) as WorkcellConfig;
  ...
  it("creates instance-root config and data paths for a fresh WORKCELL_HOME", async () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "workcell-onboard-home-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "workcell-onboard-cwd-"));
    process.chdir(cwd);
    process.env.WORKCELL_HOME = home;
  ...
  it("supports authenticated/private quickstart bind presets", async () => {
    const configPath = createFreshConfigPath();
    process.env.WORKCELL_TAILNET_BIND_HOST = "100.64.0.8";
