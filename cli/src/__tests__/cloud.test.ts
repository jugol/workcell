...
import type { CompanyPortabilityExportResult } from "@workcell/shared";
import {
...
describe("cloud CLI helpers", () => {
  ...
  beforeEach(() => {
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "workcell-cloud-cli-"));
    process.env = { ...originalEnv, WORKCELL_HOME: tempHome };
  });
  ...
  it("connects with the device-code flow and stores the resulting cloud connection", async () => {
    globalThis.fetch = vi.fn(async (url, init) => {
      ...
      if (requestUrl.endsWith("/.well-known/workcell-upstream")) {
      ...
      if (requestUrl.endsWith("/api/upstream-sync/token")) {
        return jsonResponse({
          ...
          token: {
            ...
            sourceInstanceId: "workcell-local-default",
  ...
  it("reuses the same manifest and chunk identity when an interrupted apply is retried", async () => {
    ...
    const coordinator = new LocalUpstreamPushCoordinator({
      targetOrigin: "https://cloud.example.test",
      workcellCompanyId: "target-company-1",
...
function discovery(overrides: Partial<{ supportedSchemaMajor: number }> = {}) {
  return {
    schema: "workcell-upstream-discovery-v1",
    stack: {
      id: "stack-1",
      slug: "cloud-test",
...
function portabilityExport(): CompanyPortabilityExportResult {
  return {
    rootPath: ".",
    workcellExtensionPath: ".workcell.yaml",
    manifest: {
      schemaVersion: 1,
