...
function createBaseWorkcellEnv(options: TestWorkcellEnv) {
  ...
  env.WORKCELL_CONFIG = options.configPath;
  env.WORKCELL_HOME = options.workcellHome;
  env.WORKCELL_INSTANCE_ID = options.instanceId;
  env.WORKCELL_CONTEXT = path.join(options.workcellHome, "context.json");
  env.WORKCELL_AUTH_STORE = path.join(options.workcellHome, "auth.json");
...
function createServerEnv(
  ...
  options: Omit<TestWorkcellEnv, "configPath">,
) {
  const env = createBaseWorkcellEnv({
    configPath,
    ...options,
  });
  ...
  env.SERVE_UI = "false";
  env.WORKCELL_DB_BACKUP_ENABLED = "false";
  env.HEARTBEAT_SCHEDULER_ENABLED = "false";
  env.WORKCELL_MIGRATION_AUTO_APPLY = "true";
  env.WORKCELL_UI_DEV_MIDDLEWARE = "false";

  return env;
}

function createCliEnv(options: TestWorkcellEnv) {
  const env = createBaseWorkcellEnv(options);
  delete env.DATABASE_URL;
  delete env.PORT;
  delete env.HOST;
  delete env.SERVE_UI;
  delete env.WORKCELL_DB_BACKUP_ENABLED;
  delete env.HEARTBEAT_SCHEDULER_ENABLED;
  delete env.WORKCELL_MIGRATION_AUTO_APPLY;
  delete env.WORKCELL_UI_DEV_MIDDLEWARE;
  return env;
}
...
async function runCliJson<T>(
  args: string[],
  opts: TestWorkcellEnv & { apiBase?: string; includeConfigArg?: boolean },
) {
  const repoRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../../..");
  const cliArgs = ["--silent", "workcell", ...args];
  if (opts.apiBase) {
...
async function waitForServer(
  ...
  while (Date.now() - startedAt < 30_000) {
    if (child.exitCode !== null) {
      throw new Error(
        `workcell run exited before healthcheck succeeded.\nstdout:\n${output.stdout.join("")}\nstderr:\n${output.stderr.join("")}`,
      );
    }
...
describeEmbeddedPostgres("workcell company import/export e2e", () => {
  let tempRoot = "";
  let configPath = "";
  let exportDir = "";
  let apiBase = "";
  let workcellHome = "";
  let cliShellHome = "";
  let workcellInstanceId = "";
  let serverProcess: ServerProcess | null = null;
  let tempDb: Awaited<ReturnType<typeof startEmbeddedPostgresTestDatabase>> | null = null;

  beforeAll(async () => {
    tempRoot = mkdtempSync(path.join(os.tmpdir(), "workcell-company-cli-e2e-"));
    configPath = path.join(tempRoot, "config", "config.json");
    exportDir = path.join(tempRoot, "exported-company");
    workcellHome = path.join(tempRoot, "workcell-home");
    cliShellHome = path.join(tempRoot, "shell-home");
    workcellInstanceId = "company-cli-e2e";
    mkdirSync(workcellHome, { recursive: true });
    mkdirSync(cliShellHome, { recursive: true });

    tempDb = await startEmbeddedPostgresTestDatabase("workcell-company-cli-db-");
    ...
    const child = spawn(
      "pnpm",
      ["workcell", "run", "--config", configPath],
      {
        cwd: repoRoot,
        env: createServerEnv(configPath, port, tempDb.connectionString, {
          workcellHome,
          instanceId: workcellInstanceId,
  ...
  it("exports a company package and imports it into new and existing companies", async () => {
    ...
    const expectedContextPath = path.join(workcellHome, "context.json");
    const leakedContextPath = path.join(cliShellHome, ".workcell", "context.json");
    ...
    expect(readFileSync(path.join(exportDir, ".workcell.yaml"), "utf8")).toContain('schema: "workcell/v1"');
    ...
    writeFileSync(zipPath, createStoredZipArchive(portableFiles, "workcell-demo"));
