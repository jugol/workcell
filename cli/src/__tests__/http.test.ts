...
import { ApiConnectionError, ApiRequestError, WorkcellApiClient } from "../client/http.js";

describe("WorkcellApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new WorkcellApiClient({
    ...
    expect(headers["x-workcell-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new WorkcellApiClient({ apiBase: "http://localhost:3100" });
