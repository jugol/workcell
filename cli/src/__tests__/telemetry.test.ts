...
describe("cli telemetry", () => {
  ...
  it("respects telemetry.enabled=false from the config file", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "workcell-cli-telemetry-"));
    const configPath = makeConfigPath(root, false);
    process.env.WORKCELL_HOME = path.join(root, "home");
    process.env.WORKCELL_INSTANCE_ID = "telemetry-test";
