...
vi.mock("../../../packages/plugins/create-workcell-plugin/src/index.js", async () => {
  const actual =
    await vi.importActual<typeof import("../../../packages/plugins/create-workcell-plugin/src/index.js")>(
      "../../../packages/plugins/create-workcell-plugin/src/index.js",
    );
  return {
    ...actual,
...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "workcell-cli-plugin-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("plugin init", () => {
  ...
  it("maps package name and flags to scaffolder options", () => {
    const cwd = path.resolve("/tmp/workcell-cli-test");
    const options = buildPluginInitScaffoldOptions(
      "@acme/plugin-linear",
      {
        ...
        author: "Acme",
        sdkPath: "../workcell/packages/plugins/sdk",
      },
      cwd,
    );
  ...
  it("builds exact next commands using the scaffold path", () => {
    expect(buildPluginInitNextCommands("/tmp/acme plugin")).toEqual([
      ...
      "pnpm install",
      "pnpm dev",
      "workcell plugin install '/tmp/acme plugin'",
    ]);
  });
  ...
  it("registers the CLI wrapper and invokes the existing scaffolder", async () => {
    ...
    expect(mocks.scaffoldPluginProject).toHaveBeenCalledWith({
      ...
      outputDir: path.resolve("/tmp/workcell-init-output", "demo-plugin"),
      template: "workspace",
      category: "workspace",
