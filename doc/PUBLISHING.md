...
Low-level reference for how Workcell packages are prepared and published to npm.
...
Workcell no longer uses release branches or Changesets for publishing.

## Why the CLI needs special packaging

The CLI package, `workcell`, imports code from workspace packages such as:
...
### Manual first publish for `@workcell/ui`
...
npm view @workcell/ui version

# make sure the dist payload is fresh
pnpm --filter @workcell/ui build
...
- If `npm view @workcell/ui version` already returns the same version that is in [`ui/package.json`](../ui/package.json), do not republish. Bump the version or use the normal repo-wide release flow in [`scripts/release.sh`](../scripts/release.sh).
...
- For an organization-scoped package like `@workcell/ui`, the `workcell` npm organization must exist and the publisher must be a member with permission to publish to that scope.
...
Workcell uses calendar versions:
...
- `workcell@2026.318.1-canary.2`

This keeps the default install path unchanged while allowing explicit installs with:

```bash
npx workcell@canary onboard
```
...
- every published internal `@workcell/*` dependency referenced by that manifest exists on npm
...
Workcell does not auto-publish every non-private workspace package anymore.
...
Example for `@workcell/adapter-codex-local` from the repo root:

```bash
# safe preview
pnpm run release:bootstrap-package -- @workcell/adapter-codex-local

# one-time first publish from an authenticated maintainer machine
pnpm run release:bootstrap-package -- @workcell/adapter-codex-local --publish --otp 123456
```
...
1. open `https://www.npmjs.com/package/@workcell/adapter-codex-local`
2. go to `Settings` → `Trusted publishing`
3. add repository `workcell/workcell`
