...
We were asked to find the hook surfaces exposed by `pi` and `pi-mono`, then decide which ideas transfer cleanly into Workcell.
...
- current Workcell plugin and adapter surfaces in this repo
...
## What Workcell already has

Workcell already has several hook-like surfaces, but they are much narrower and safer:
...
## What transfers well to Workcell

These ideas from `pi-mono` fit Workcell with little conceptual risk:

### 1. Read-only run lifecycle subscriptions

Workcell should continue exposing run and transcript events to plugins, for example:
...
This matches Workcell's control-plane posture: observe, react, automate.

### 2. Plugin-to-plugin events

Workcell already has this. It is worth keeping and extending.

This is the clean replacement for many ad hoc hook chains.

### 3. Explicit worker lifecycle hooks

Workcell already has `setup()` and `onHealth()`. That is the right shape.

If more lifecycle is needed, it should stay explicit and host-controlled.

### 4. Trusted adapter-level prompt/runtime middleware

Some `pi-mono` ideas do belong in Workcell, but only inside trusted adapter/runtime code:
...
These `pi-mono` capabilities are a bad fit for Workcell core:

### 1. Arbitrary mutation hooks on control-plane decisions

Workcell should not let general plugins rewrite:
...
Workcell should keep plugin tools namespaced and non-shadowing.

### 3. Project-local plugin loading

Workcell is an operator-controlled control plane. Repo-local plugin auto-loading would make behavior too implicit and too hard to govern.
...
They do not map directly to Workcell's board-and-issues model.

## Recommended Workcell direction
...
If the question is "what works for Workcell too?":
