...
Agents need API keys to authenticate with Workcell. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Workcell
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Workcell generates a token and passes it directly to the agent
...
`WORKCELL_API_URL`, `WORKCELL_AGENT_ID`, `WORKCELL_COMPANY_ID`. We need to
add a `WORKCELL_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Workcell CLI
...
```
workcell auth login
# Opens browser -> user confirms -> token stored at ~/.workcell/credentials
```
...
managed by the Workcell server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Workcell.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Workcell SKILL.md (or a link to it)
   - What information Workcell needs from the agent (e.g. webhook URL, adapter
...
5. Workcell stores the pending registration.
...
7. On approval, Workcell provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Workcell issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.workcell.ing/skills/workcell/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How Workcell should send you heartbeats",
...
1. Generate a Workcell invite link for the company.
...
4. A Workcell company member approves the new agent.
5. Workcell begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `WORKCELL_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `workcell auth login` for developer-managed remote agents.                                      |
