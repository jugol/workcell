...
Date: 2026-03-13  
Related discussion: https://github.com/workcell/workcell/discussions/449
...
- local runtime data from the default Workcell instance on 2026-03-13
...
3. **Repeated context reacquisition.** The `workcell` skill tells agents to re-fetch assignments, issue details, ancestors, and full comment threads on every heartbeat. The API does not currently offer efficient delta-oriented alternatives.
...
The `workcell` skill currently tells agents to do this on essentially every heartbeat:
...
- Workcell discovers repo skills from the current checkout, then symlinks them into `$CODEX_HOME/skills` or `~/.codex/skills`.
- If an existing Workcell skill symlink already points at another live checkout, the current implementation skips it instead of repointing it.
- This can leave Codex using stale skill content from a different worktree even after Workcell-side skill changes land.
...
- `skills/workcell/SKILL.md`: 17,441 bytes
- `.agents/skills/create-agent-adapter/SKILL.md`: 31,832 bytes
- `skills/workcell-create-agent/SKILL.md`: 4,718 bytes
...
- Store both:
  ...
  - Workcell-normalized per-run usage
...
For `codex_local`, this also requires isolating the Codex skill home per worktree or teaching Workcell to repoint its own skill symlinks when the source checkout changes. Otherwise prompt and skill improvements in the active worktree may not reach the running agent.
...
Update the `workcell` skill so the default pattern becomes:
...
- For `codex_local`, either:
  ...
  - treat Workcell-owned Codex skill symlinks as repairable when they point at a different checkout
...
4. heartbeat delta APIs + `workcell` skill rewrite
...
6. Rewrite `skills/workcell/SKILL.md` around delta-fetch behavior.
