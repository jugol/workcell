...
Today release smoke testing for published Workcell packages is manual and shell-driven:
...
- runs `workcell@${WORKCELLAI_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@workcell.local`
- password: `workcell-smoke-password`
...
- the suite passes locally against both:
  - `WORKCELLAI_VERSION=canary`
  - `WORKCELLAI_VERSION=latest`
...
1. A published `workcell@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `workcell@latest` Docker install can be smoke-tested by Playwright in CI.
