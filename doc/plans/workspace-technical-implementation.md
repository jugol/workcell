...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/workcell-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `WORKCELL_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `WORKCELL_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - Workcell may create host-local worktrees, processes, and paths
- `cloud`
  - Workcell should assume no durable host-local execution workspace management
...
- `provider text not null`
  - `workcell | github | vercel | s3 | custom`
...
- use `WORKCELL_EXECUTION_TOPOLOGY` as a defaulting guardrail
