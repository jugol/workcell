...
Workcell's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends Workcell's API, UI, and data model | Yes |
...
modules/
  observability/
    workcell.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@workcell/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@workcell/core";
...
```
1. Scan modules/ directory for workcell.module.json manifests
...
```jsonc
// workcell.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@workcell/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@workcell/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:workcell/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.workcell.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm workcell store list                    # browse available modules and templates
pnpm workcell store install <module-id>     # install a module
pnpm workcell store import <template-id>    # import a company template
pnpm workcell store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one Workcell instance | `server:started` |
...
4. **Module config** — `workcell.config.json` with per-module config, env var interpolation
...
10. **`@workcell/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
