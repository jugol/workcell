...
This document defines the next-stage plan for Workcell company import/export.

The core shift is:

- move from a Workcell-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside Workcell.
...
Workcell already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just Workcell
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `workcell.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.workcell.yaml` sidecar for high-fidelity Workcell-specific details
...
11. adapter-aware skill sync surfaces so Workcell can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point Workcell at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a Workcell sidecar.
...
Workcell must not redefine `SKILL.md`.
...
- Workcell-specific fidelity lives in `.workcell.yaml`
- Workcell may resolve and install `SKILL.md` packages, but it must not require a Workcell-only skill format
...
### 5.4 Base Package Vs Workcell Extension
...
- base package:
  ...
  - no Workcell-only runtime fields by default
- Workcell extension:
  - `.workcell.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by Workcell tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`workcell.manifest.json` is not part of the future package direction.
...
Workcell import/export should support these entity kinds:
...
- Workcell can treat “this set of agents came from team package X” as the imported-team model
...
Workcell should support source references in package metadata with:
...
Workcell should define a new adapter surface area around skills:
...
Planned Workcell behavior:

- if an adapter supports read, Workcell should show current skills in the UI
- if an adapter supports write, Workcell should let the user enable/disable imported skills
- if an adapter supports sync, Workcell should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.workcell.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.workcell.yaml` alongside the base package
...
## 10. Storage Model Inside Workcell
...
Workcell should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `workcell company export <company-id> --out <path>`
- `workcell company import <path-or-url> --dry-run`
- `workcell company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in Workcell storage, or only referenced at import time?
...
3. Should Workcell support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps Workcell aligned with:
