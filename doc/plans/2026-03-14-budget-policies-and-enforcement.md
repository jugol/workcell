...
Workcell already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model Workcell should implement next.

## Product Goals

Workcell should let operators:
...
For the next implementation pass, Workcell should enforce these defaults:
...
Workcell should separate subscription-included usage from billed spend:
...
Workcell should have two threshold classes:
...
Workcell must know that a project is budget-paused and enforce it.
...
Workcell must also block execution before new work starts.
