...
Workcell needs to support two very different but equally valid ways of working:
...
Today, Workcell already has the beginnings of this model:
...
- making Workcell feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing Workcell users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Workcell deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Workcell into a full code review product
...
This object must be recorded explicitly so that Workcell can:
...
Workcell should treat PRs as a type of work product linked back to:
...
Workcell already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Workcell must be a first-class deployment mode

Workcell cannot assume that it is running on the same machine as the code.

In cloud deployments, Workcell may:
...
- `Runtime service`: a process or service Workcell owns or tracks for a workspace
...
It also lets cloud-hosted Workcell deployments point at codebases and remotes without pretending the Workcell host has direct filesystem access.
...
Without an explicit `execution workspace` record, Workcell has nowhere stable to attach:
...
Workcell needs a single place to show:
...
- Workcell-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Workcell owning the execution host
...
## 1. Workcell host and execution host must be decoupled
...
without the Workcell host itself running local git or local preview processes.
...
Workcell should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for Workcell.
...
Workcell should keep workspace automation centered on a small set of hooks:
...
Workcell should provide the equivalent visibility in the project `Code` surface:
...
Workcell should treat execution workspaces as the natural home for:
...
Workcell should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
Workcell should continue treating cleanup policy as part of the core workspace model:
...
Workcell should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the Workcell host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

Workcell should borrow Worktrunk's good execution patterns while keeping the broader Workcell model:
...
That keeps Workcell focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Workcell deployments remain first-class

That is a better fit for Workcell than either extreme:
