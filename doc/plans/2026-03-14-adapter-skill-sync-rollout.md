...
This document defines the rollout plan for adapter-wide skill support in Workcell.
...
- Workcell stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

Workcell currently has these adapters:
...
1. Desired skills live in Workcell for every adapter.
...
These adapters have a stable local skills directory that Workcell can read and manage.
...
These adapters do not have a meaningful Workcell-owned persistent install state.
...
- desired skills stored in Workcell
...
- runtime already injects Workcell skills into `~/.cursor/skills`
...
- runtime already injects Workcell skills into `~/.gemini/skills`
...
- runtime already injects Workcell skills into `~/.pi/agent/skills`
...
- Workcell can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects Workcell skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- Workcell may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed Workcell skills that are clearly marked as Workcell-managed
...
- Workcell stores desired skills only
...
- avoid destructive wording unless Workcell can prove a skill is Workcell-managed
...
That gets Workcell from “skills work for Codex and Claude” to “skills work for the whole local-adapter family,” which is the meaningful V1 milestone.
