...
This document defines the product and UI plan for skill management in Workcell.
...
- Workcell company import/export can include skills as package content
...
3. The UI must always tell the truth about what Workcell knows:
   - desired state in Workcell
...
Workcell should treat skills at two scopes:
...
- created locally inside Workcell later
...
7. “Understand what Workcell knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a Workcell-specific package layout
...
This matters because Codex and similar adapters may already have local skills that Workcell did not install.

These should be clearly marked:

- external
- not managed by Workcell
...
- `desired_only`: Workcell wants it, adapter does not show it yet
- `external`: adapter has it but Workcell does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, Workcell only tracks desired state
...
- Workcell can still track desired skills
...
- Workcell can see adapter skills
...
- `.workcell.yaml` may add Workcell-specific fidelity, but should not replace the base shortname association model
...
- `.workcell.yaml` may add Workcell-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
Workcell should have a company-scoped skill model or managed package model representing:
...
Workcell should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too Workcell-specific.
...
That gives Workcell one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
