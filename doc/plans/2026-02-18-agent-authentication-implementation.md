...
- New config/env settings:
  - `WORKCELL_AGENT_JWT_SECRET`
  - `WORKCELL_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `WORKCELL_AGENT_JWT_ISSUER` (default: `workcell`)
  - `WORKCELL_AGENT_JWT_AUDIENCE` (default: `workcell-api`)
...
1. In:
   ...
   inject `WORKCELL_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `WORKCELL_API_KEY`, do not overwrite it.
- Continue injecting:
  - `WORKCELL_AGENT_ID`
  - `WORKCELL_COMPANY_ID`
  - `WORKCELL_API_URL`
...
- Local adapters authenticate without manual `WORKCELL_API_KEY` config.
