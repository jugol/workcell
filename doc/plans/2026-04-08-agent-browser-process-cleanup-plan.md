...
Yes, there is a likely root cause in Workcell's local execution model.
...
If Codex, Claude, Cursor, or a skill launched through them starts Chrome or Chromium helpers, Workcell can lose ownership of those descendants even when it still believes it handled the run correctly.
...
This is the clearest internal comparison point: Workcell already has one local-process subsystem that treats process-group ownership as the right abstraction.
...
- Workcell may think it cancelled the run because the parent process is gone
...
This work is successful when Workcell can:
...
- require every skill author to add bespoke browser teardown logic before Workcell can clean up correctly
...
- make the leak measurable from Workcell's side before changing execution semantics
...
- add a control-plane cleanup action or CLI utility for stale local run processes owned by Workcell
...
Treat this as a Workcell executor ownership bug, not an `agent-browser` bug.
