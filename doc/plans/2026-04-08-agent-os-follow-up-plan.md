...
Turn the `agent-os` research into a low-risk Workcell execution plan that preserves Workcell's control-plane model while testing the few runtime ideas that appear worth adopting.

## Decision summary

Workcell should not absorb `agent-os` as a product model or orchestration layer.

Workcell should evaluate `agent-os` in three narrow areas:
...
This work is successful when Workcell has:
...
- a proposed runtime capability model that fits current Workcell adapters
...
- replace Workcell heartbeats, issues, comments, approvals, or budgets with `agent-os` primitives
- introduce Rust/sidecar requirements for all local execution paths
- migrate all adapters at once
- add runtime workflow/queue abstractions to Workcell core

## Existing Workcell integration points
...
- prove that Workcell can drive one local agent through an `agent-os` runtime without breaking heartbeat semantics
...
- keep Workcell's existing heartbeat, issue, workspace, and comment flow authoritative
...
- introduce a Workcell-native capability vocabulary without coupling the product to `agent-os`
...
- determine whether a layered/snapshotted root model improves some Workcell workloads
...
- identify where Workcell should prefer explicit typed tools over ambient shell access
...
- one concrete proposal for typed-tool adoption in Workcell
...
- runtime experimentation could blur the boundary between Workcell as control plane and Workcell as execution platform
