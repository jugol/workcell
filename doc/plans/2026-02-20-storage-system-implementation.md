...
Add a single storage subsystem for Workcell that supports:
...
- API serves attachment bytes through authenticated Workcell endpoints.
...
- Default local path is under instance root: `~/.workcell/instances/<instanceId>/data/storage`.
...
- `workcell onboard` writes a valid `storage` config block by default.
- `workcell configure --section storage` can switch between local and s3 modes.
...
- `workcell doctor` reports actionable storage status.
