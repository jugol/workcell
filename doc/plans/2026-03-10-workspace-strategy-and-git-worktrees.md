...
`PAP-447` asks how Workcell should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Workcell instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Workcell:

- some operators use Workcell to manage Workcell and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Workcell
- Claude and Codex expose different built-in affordances, so Workcell should not overfit to one tool

## Core Product Decision

Workcell should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Workcell
...
- Workcell should not assume that every adapter can see or use a host filesystem path directly
...
If Workcell makes worktrees agent-first, it will blur:
...
- Workcell core resolves and records execution workspace state
...
That gives Workcell room to support:
...
- Workcell handles workspace runtime services under the hood using project/workspace policy
...
Once Workcell is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So Workcell should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Workcell cannot infer
...
- when issue moves to done, should Workcell auto-commit?
- should it auto-push?
...
Workcell already has the right foundation for a project-first model.
...
For server-spawned local adapters, Workcell already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Workcell should mirror directly

Implication:

- for `codex_local`, Workcell should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Workcell should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Workcell should separate:
...
Workcell should be able to express intentions such as:
...
These are not specific to Workcell. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".workcell/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a Workcell-specific `devServer` flag.
...
Workcell should distinguish between:
...
Workcell should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, Workcell should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Workcell
...
For runtime services specifically, Workcell should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Workcell-only dev-server flags
...
3. Inject resolved execution workspace details into `context.workcellWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / Workcell instance can be reused through env/config injection when appropriate
...
- Workcell can decide whether to reuse or start a fresh service deterministically
...
3. Test against Workcell-on-Workcell workflows first.
...
7. Workspace runtime services are modeled generically, not as Workcell-specific dev-server toggles.
