...
This document defines the V1 design for a Workcell adapter that integrates with
...
4. synthesized stdout events for Workcell UI/CLI
...
3. **Agent callback auth to Workcell** uses a bootstrap exchange flow (no long-lived Workcell key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Workcell endpoints, not full SKILL.md prompt injection.
...
- `workcellPublicUrl` (optional override; else `WORKCELL_PUBLIC_URL` env)
...
## Workcell Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `WORKCELL_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.workcellPublicUrl`
2. `process.env.WORKCELL_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Workcell credentials in prompt text.

Flow:

1. Before launch/follow-up, Workcell mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Workcell validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Workcell API calls.
...
1. Prompt includes a compact instruction to fetch skills from Workcell.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/workcell`
   - `GET /api/skills/workcell-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <workcellPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active Workcell run context.
...
5. `workcellPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from Workcell skill endpoints |
| Workcell auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Workcell context.
3. Add richer PR metadata surfacing in Workcell UI.
