...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented Workcell plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside Workcell.
...
- package: `@workcell/plugin-kitchen-sink-example`
- plugin id: `workcell.kitchen-sink-example` or `workcell-kitchen-sink-example`

Recommendation: use `workcell-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core Workcell event such as `issue.created`
...
Recommendation: default to a Workcell-local endpoint or a stable public echo endpoint to avoid flaky docs.
