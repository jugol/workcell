...
Operational contract for the hosted `aws_secrets_manager` secret provider used by Workcell Cloud.

## Scope

- Hosted provider for Workcell-managed secrets when Workcell Cloud runs on AWS.
- Source of truth for secret values is AWS Secrets Manager, not Postgres.
- Workcell stores only metadata needed for ownership, bindings, version selection, audit, and runtime resolution.
- AWS provider bootstrap credentials are deployment/runtime credentials, not Workcell-managed company secrets.
- Remote import for existing AWS secrets is metadata-only. Preview/import uses
  AWS inventory metadata and creates Workcell external references; it does not
  copy plaintext into Workcell.
...
The AWS provider has a chicken-and-egg boundary: Workcell cannot use
`company_secrets` to unlock the AWS provider that stores those secrets. The
initial AWS trust must exist before the Workcell server starts.

Allowed bootstrap locations:

- Infrastructure IAM or workload identity attached to the Workcell server
  runtime.
- Process environment or orchestrator secret store used to start the Workcell
  server.
...
keys into the Workcell board UI. Do not store those bootstrap keys in
`company_secrets`.

## Workcell Cloud Bootstrap

Workcell Cloud must provision the AWS backing resources before any board user
can create AWS-backed company secrets:

1. Create or select the deployment KMS key.
2. Create the Workcell server runtime role for the deployment.
...
4. Configure the server runtime with the non-secret provider environment
   variables below.
5. Run `workcell doctor` or the provider health endpoint from the deployed
...
Once this is in place, the board UI can create Workcell-managed AWS secrets and
Workcell will write them under the deployment/company namespace.
...
```sh
aws sso login --profile workcell-dev
AWS_PROFILE=workcell-dev \
WORKCELL_SECRETS_PROVIDER=aws_secrets_manager \
WORKCELL_SECRETS_AWS_REGION=us-east-1 \
WORKCELL_SECRETS_AWS_DEPLOYMENT_ID=dev-local \
WORKCELL_SECRETS_AWS_KMS_KEY_ID=arn:aws:kms:us-east-1:123456789012:key/abcd-... \
pnpm dev
```
...
should not be written to Workcell config, committed to `.env` files, stored in
`company_secrets`, or used as the default Workcell Cloud bootstrap path.
...
WORKCELL_SECRETS_PROVIDER=aws_secrets_manager
WORKCELL_SECRETS_AWS_REGION=us-east-1
WORKCELL_SECRETS_AWS_DEPLOYMENT_ID=prod-us-1
WORKCELL_SECRETS_AWS_KMS_KEY_ID=arn:aws:kms:us-east-1:123456789012:key/abcd-...
```
...
```sh
WORKCELL_SECRETS_AWS_PREFIX=workcell
WORKCELL_SECRETS_AWS_ENVIRONMENT=production
WORKCELL_SECRETS_AWS_PROVIDER_OWNER=workcell
WORKCELL_SECRETS_AWS_ENDPOINT=
WORKCELL_SECRETS_AWS_DELETE_RECOVERY_DAYS=30
```

Naming convention for Workcell-managed secrets:

```text
workcell/{deploymentId}/{companyId}/{secretKey}
```

Tag set for Workcell-managed secrets:

- `workcell:managed-by=workcell`
- `workcell:provider-owner=<owner tag>`
- `workcell:deployment-id=<deployment id>`
- `workcell:company-id=<company id>`
- `workcell:secret-key=<secret key>`
- `workcell:environment=<environment tag>`

## IAM And KMS Assumptions

Launch posture:

- One Workcell app role per deployment.
- One deployment-scoped KMS key per deployment at launch.
- Future per-company KMS keys remain compatible because Workcell stores provider refs and version metadata separately from values.
...
```text
arn:aws:secretsmanager:<region>:<account-id>:secret:workcell/<deployment-id>/*
```
...
- Prefer workload identity / role-based auth. Do not store AWS credentials inline in Workcell config.
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:secretsmanager:<region>:<account-id>:secret:workcell/<deployment-id>/*"
    },
    {
      "Sid": "WorkcellDeploymentKms",
      "Effect": "Allow",
      "Action": [
        "kms:Encrypt",
        "kms:Decrypt",
...
- Workcell-managed secrets may be deleted only by Workcell or an operator with equivalent break-glass access.
- External references may resolve through Workcell runtime, but Workcell should not delete the external secret resource.
...
```json
{
  "Sid": "WorkcellRemoteSecretInventory",
  "Effect": "Allow",
...
operator-approved external prefixes that Workcell is allowed to consume:

```json
{
  "Sid": "WorkcellResolveImportedExternalReferences",
  "Effect": "Allow",
...
permissions scoped to `workcell/<deployment-id>/*`; do not broaden them for
remote import.

Safe scoping guidance:

- Prefer one Workcell runtime role per environment/account.
...
Workcell also blocks importing refs under its own managed namespace as
external references. Use the Workcell-managed flow for
`workcell/{deploymentId}/{companyId}/{secretKey}` resources.

## Existing AWS Secrets

V1 keeps existing AWS Secrets Manager entries as **linked external references**, not adopted
Workcell-managed resources.

Use the Workcell-managed flow when Workcell should create and rotate the value. The AWS
...
```text
/workcell-bench/anthropic_api_key
```

In that mode Workcell stores only the path or ARN, resolves it at runtime, and records
redacted access events. Operators rotate the actual value in AWS. Update the Workcell
reference only when the AWS path, ARN, or pinned provider version changes.

Workcell does not currently offer an "adopt existing AWS secret" flow that takes over future
`PutSecretValue` writes for an arbitrary existing secret. Adding that later requires explicit
confirmation UX, scope validation, expected Workcell tags, and security/cloud-ops review.

## Data Custody

- Workcell stores `externalRef`, `providerVersionRef`, provider id, fingerprint hash, status, and binding metadata.
- Workcell does not store AWS secret plaintext in `company_secret_versions.material`.
- Runtime resolution fetches the value from AWS only when a bound consumer needs it.

## Rotation Runbook

Manual Workcell-managed rotation:

1. Write the new value through the Workcell secret rotate flow.
2. Workcell creates a new AWS secret version with `PutSecretValue`.
3. Workcell records the new `providerVersionRef` in `company_secret_versions`.
4. Re-run or restart affected workloads that consume `latest`, or pin consumers to a specific Workcell version before rollout when you need staged release safety.

Guidance:

- Prefer pinned Workcell secret versions for risky rollouts.
...
- Workcell database metadata for secret ownership, bindings, status, and provider version refs.
- AWS Secrets Manager namespace under the configured deployment prefix.
- The configured KMS key and its decrypt permissions.

Restore checklist:

1. Restore Workcell database metadata.
2. Confirm the same AWS Secrets Manager namespace still exists.
3. Confirm the Workcell runtime role can call `GetSecretValue` on the restored prefix.
4. Confirm the role still has decrypt access to the CMK referenced by `WORKCELL_SECRETS_AWS_KMS_KEY_ID`.
...
1. Stop or pause affected Workcell runs.
2. Audit recent Workcell secret access events for impacted secret ids and consumers.
...
4. Rotate impacted secrets in AWS through Workcell-managed versioning.
...
- `WORKCELL_SECRETS_PROVIDER=aws_secrets_manager`
- The required `WORKCELL_SECRETS_AWS_*` environment variables set.

Suggested smoke:

1. Create a test secret through the Workcell board or API under a throwaway company.
2. Confirm the resulting AWS secret name matches `workcell/{deploymentId}/{companyId}/{secretKey}`.
3. Rotate the secret once and confirm a new `providerVersionRef` appears in Workcell metadata.
