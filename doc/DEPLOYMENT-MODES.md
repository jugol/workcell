...
Workcell supports two runtime modes:
...
Workcell now treats **bind** as a separate concern from auth:
...
pnpm workcell onboard --yes --bind lan
pnpm workcell run --bind tailnet
```
...
When running `authenticated` mode, if the only instance admin is `local-board`, Workcell emits a startup warning with a one-time high-entropy claim URL.
