# Workcell V1 Implementation Spec
...
Workcell V1 must provide a full control-plane loop for autonomous agents:
...
- Primary: PostgreSQL
- Local default: embedded PostgreSQL at `~/.workcell/instances/default/db`
- Optional local prod-like: Docker Postgres
- Optional hosted: Supabase/Postgres-compatible
- File/object storage:
  - local default: `~/.workcell/instances/default/data/storage` (`local_disk`)
...
- `runtime_config` jsonb not null default `{}`; may include Workcell runtime policy such as `modelProfiles.cheap.adapterConfig` for an optional low-cost model lane that does not change the primary adapter config
...
- project env is merged into run environment for issues in that project and overrides conflicting agent env keys before Workcell runtime-owned keys are injected

Routine execution issues add a routine-scoped env overlay after project env and before Workcell runtime-owned keys. Routine env uses the same secret-aware binding format, is stored on `routines.env`, is snapshotted in routine revisions, and resolves secret refs against the routine binding target so routine-owned secrets do not require direct bindings on the executing agent.
...
- when Workcell cannot safely infer the next action, it surfaces the problem through visible blocked/recovery work instead of silently completing or reassigning work
...
Workcell V1 keeps a company-scoped visibility model as the default because centralized authorization and scoped work-object controls are not yet a core V1 control surface.
...
- if unset, auto-use embedded PostgreSQL under `~/.workcell/instances/default/db`
...
- `.workcell.yaml` sidecar for Workcell-specific fidelity
...
- export emits a clean vendor-neutral markdown package plus `.workcell.yaml`
- projects and starter tasks are opt-in export content rather than default package content
- recurring `TASK.md` entries use `recurring: true` in the base package and Workcell routine fidelity in `.workcell.yaml`
- Workcell imports recurring task packages as routines instead of downgrading them to one-time issues
- export strips environment-specific paths (`cwd`, local instruction file paths, inline prompt duplication) while preserving portable project repo/workspace metadata such as `repoUrl`, refs, and workspace-policy references keyed in `.workcell.yaml`
