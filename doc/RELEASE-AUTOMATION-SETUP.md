...
This document covers the GitHub and npm setup required for the current Workcell release model:
...
Do this for every public package that Workcell publishes.
...
3. add a trusted publisher for the GitHub repository `workcell/workcell`
...
2. open that package on npm and add the `workcell/workcell` trusted publisher for `.github/workflows/release.yml`
...
2. revoke any personal automation token that used to publish Workcell
