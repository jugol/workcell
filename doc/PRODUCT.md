# Workcell — Product Definition

## What It Is

Workcell is the control plane for autonomous AI companies. One instance of Workcell can run multiple companies. A **company** is a first-order object.
...
- **Adapter type + config** — how this agent runs and what defines its identity/behavior. This is adapter-specific (e.g., an OpenClaw agent might use SOUL.md and HEARTBEAT.md files; a Claude Code agent might use CLAUDE.md; a bare script might use CLI args). Workcell doesn't prescribe the format — the adapter does.
...
Workcell supports several ways to run an agent's heartbeat:

1. **Local CLI/session adapters** — Workcell starts or resumes local coding-tool sessions such as Claude Code, Codex, Gemini, OpenCode, Pi, and Cursor, then tracks the run.
2. **Run a command** — Workcell kicks off a process (shell command, Python script, etc.) and tracks it. The heartbeat is "execute this and monitor it."
3. **Fire and forget a request** — Workcell sends a webhook/API call to an externally running agent. The heartbeat is "notify this agent to wake up." OpenClaw-style hooks work this way.
4. **External adapter plugins** — Workcell loads adapter packages through the plugin/adapter flow so self-hosted installs can add runtimes without hardcoding them in core.

Agent runs can use project and execution workspaces, managed runtime services such as preview/dev servers, adapter-specific session state, and HTTP/webhook-style execution. We provide sensible defaults, but the adapter is still the boundary: if a runtime can be invoked, observed, and authorized, Workcell can coordinate it.
...
1. **Unopinionated about how you run your agents.** Your agents could be OpenClaw bots, Python scripts, Node scripts, Claude Code sessions, Codex instances — we don't care. Workcell defines the control plane for communication and provides utility infrastructure for heartbeats. It does not mandate an agent runtime.

2. **Company is the unit of organization.** Everything lives under a company. One Workcell instance, many companies.
...
5. **Control plane, not execution plane.** Workcell orchestrates. Agents run wherever they run and phone home.

## User Flow (Dream Scenario)

1. Open Workcell, create a new company
...
There are two runtime modes Workcell must support:
...
Workcell’s core identity is a **control plane for autonomous AI companies**, centered on **companies, org charts, goals, issues/comments, heartbeats, budgets, approvals, and board governance**. The public docs are also explicit about the current boundaries: **tasks/comments are the built-in communication model**, Workcell is **not a chatbot**, and it is **not a code review tool**. The roadmap already points toward **easier onboarding, cloud agents, easier agent configuration, plugins, better docs, and ClipMart/ClipHub-style reusable companies/templates**.

## What Workcell should do vs. not do
...
- Do not build a complete Jira/GitHub replacement. The repo/docs already position Workcell as organization orchestration, not focused on pull-request review.
- Do not build enterprise-grade RBAC first. Workcell now has authenticated mode, company memberships, instance roles, and permission grants, but fine-grained enterprise governance should remain secondary to the core company control plane.
