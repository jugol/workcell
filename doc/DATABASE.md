...
Workcell uses PostgreSQL via [Drizzle ORM](https://orm.drizzle.team/). There are three ways to run the database, from simplest to most production-ready.
...
1. Creates a `~/.workcell/instances/default/db/` directory for storage
2. Ensures the `workcell` database exists
3. Runs migrations automatically for empty databases
4. Starts serving requests

Data persists across restarts in `~/.workcell/instances/default/db/`. To reset local dev data, delete that directory.
...
When `DATABASE_URL` is unset, this command targets the current embedded PostgreSQL instance for your active Workcell config/instance.
...
Docker note: the Docker quickstart image also uses embedded PostgreSQL by default. Persist `/workcell` to keep DB state across container restarts (see `doc/DOCKER.md`).
...
# DATABASE_URL=postgres://workcell:workcell@localhost:5432/workcell
```
...
If you later run the app with a pooled runtime URL, set `DATABASE_MIGRATION_URL` to the direct connection URL. Workcell uses it for startup schema checks/migrations and plugin namespace migrations, while the app continues to use `DATABASE_URL` for runtime queries:
...
If your hosted database requires transaction-pooling-only connections, use a direct or session-pooled connection for Workcell until runtime pooling support is documented in this guide. Do not edit database client source files as part of deployment setup.
...
| `DATABASE_URL` | Mode |
|---|---|
| Not set | Embedded PostgreSQL (`~/.workcell/instances/default/db/`) |
...
Workcell stores current-user sidebar membership state in:
...
Workcell supports automatic and manual logical database backups. These dumps include
...
`workcell db:backup` / `pnpm db:backup` commands and backup retention
configuration.
...
Workcell stores secret metadata and versions in:
...
- Default key file: `~/.workcell/instances/default/secrets/master.key` (auto-created if missing).
- CLI config location: `~/.workcell/instances/default/config.json` under `secrets.localEncrypted.keyFilePath`.
...
- `WORKCELL_SECRETS_MASTER_KEY` (32-byte key as base64, hex, or raw 32-char string)
