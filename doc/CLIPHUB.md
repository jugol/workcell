...
ClipHub is the public registry where people share, discover, and download Workcell company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Workcell what a package registry is to a programming language. Workcell already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Workcell — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Workcell instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Workcell spec. It contains:
...
**Install (fresh start):**
```
workcell install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Workcell instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Workcell, export your company as a template, then publish:

```
workcell export --template my-company
workcell publish cliphub my-company
```
...
ClipHub is a **separate service** from Workcell itself. Workcell is self-hosted; ClipHub is a hosted registry that Workcell instances talk to.
...
| **Workcell CLI** | `workcell install`, `workcell publish`, `workcell cliphub sync` — built into Workcell |
| **Workcell UI** | "Browse ClipHub" panel in the Workcell web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Workcell) |
| Backend | TypeScript + Hono (consistent with Workcell) |
| Database | PostgreSQL |
...
4. Run `workcell install cliphub:acme/lean-saas-shop`
5. Workcell creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Workcell until it works well
2. Export: `workcell export --template my-agency`
3. Publish: `workcell publish cliphub my-agency`
...
3. Install just that agent: `workcell install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to Workcell

ClipHub is **not required** to use Workcell. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Workcell what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `workcell install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Workcell web UI
- [ ] `workcell cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Workcell's own philosophy)
