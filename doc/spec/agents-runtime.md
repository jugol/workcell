...
For `claude_local` and `codex_local`, Workcell assumes the CLI is already installed and authenticated on the host machine.
...
Workcell stores resumable session state per `(agent, taskKey, adapterType)`.
