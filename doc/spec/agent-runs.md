...
This spec defines how Workcell actually runs agents while staying runtime-agnostic.
...
1. Workcell is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Workcell manages lifecycle, and on exit Workcell parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `workcell.skill` (shared Workcell skill text block)
...
2. Preferred transport is env vars (`WORKCELL_*`) injected at runtime.
