# Workcell Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Workcell's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Workcell app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Workcell HTTP APIs directly.
...
- The current runtime ships a small host-provided plugin UI component kit through `@workcell/plugin-sdk/ui`, but does not support plugin asset uploads/reads yet. Treat plugin asset APIs as future-scope ideas, not current implementation promises.
...
Workcell plugin design is based on the following assumptions:

1. Workcell is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Workcell business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Workcell core.
...
2. Let plugins add major capabilities without editing Workcell core.
...
4. Depend on project-local plugin folders such as `.workcell/plugins`.
...
The single Workcell deployment an operator installs and controls.

### 5.2 Company

A first-class Workcell business object inside the instance.
...
A trusted in-process extension loaded directly by Workcell core.
...
An installable instance-wide extension package loaded through the Workcell plugin runtime.
...
Workcell has two extension classes.
...
Workcell already has a concrete workspace model:
...
Plugins live under the Workcell instance directory.

Suggested layout:

- `~/.workcell/instances/default/plugins/package.json`
- `~/.workcell/instances/default/plugins/node_modules/`
- `~/.workcell/instances/default/plugins/.cache/`
- `~/.workcell/instances/default/data/plugins/<plugin-id>/`
...
Workcell should add CLI commands:

- `pnpm workcell plugin list`
- `pnpm workcell plugin install <package[@version]>`
- `pnpm workcell plugin uninstall <plugin-id>`
- `pnpm workcell plugin upgrade <plugin-id> [version]`
- `pnpm workcell plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@workcell/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface WorkcellPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
- `minimumHostVersion` is preferred, with `minimumWorkcellVersion` retained for
...
Plugins may contribute tools that Workcell agents can use during runs.
...
- Workcell server starts one worker process per installed plugin
...
Receives one typed Workcell domain event.
...
Trusted orchestration plugins can create and update Workcell issues through `ctx.issues` instead of importing server internals. The public issue contract includes parent/project/goal links, board or agent assignees, blocker IDs, labels, billing code, request depth, execution workspace inheritance, and plugin origin metadata.

Plugins that perform durable work should declare managed Workcell resources rather than using private plugin state:
...
export function definePlugin(definition: PluginDefinition): WorkcellPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: WorkcellPluginManifestV1;
  config: {
...
6. For recurring business workflows that should create visible Workcell work, prefer managed routines and managed resources over jobs. Jobs remain useful for private plugin-runtime maintenance tasks.
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@workcell/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@workcell/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@workcell/plugin-sdk/ui`)
...
Workcell-internal pages. It exposes `resolveHref(to)`, `navigate(to,
...
Workcell navigation because those can force a full document reload.
...
- Plugin bundles must not import from host internals. They may only import from `@workcell/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@workcell/plugin-sdk/ui`
...
The `@workcell/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Workcell secret provider system rather than a plain text input
...
1. Core Workcell data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Workcell objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Workcell product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Workcell later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing Workcell secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm workcell plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Workcell server. This is a normative requirement, not optional.
...
### 27.1 `@workcell/plugin-test-harness`
...
```ts
import { createTestHarness } from "@workcell/plugin-test-harness";
...
For developing a plugin against a running Workcell instance:

- The operator installs the plugin from a local path: `pnpm workcell plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-workcell-plugin`) that scaffolds:

- `package.json` with correct `workcellPlugin` keys
...
- `@workcell/plugin-workspace-files`
- `@workcell/plugin-terminal`
- `@workcell/plugin-git`
- `@workcell/plugin-linear`
- `@workcell/plugin-github-issues`
- `@workcell/plugin-grafana`
- `@workcell/plugin-runtime-processes`
- `@workcell/plugin-stripe`
...
- `@workcell/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@workcell/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@workcell/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@workcell/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@workcell/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@workcell/plugin-sdk/ui`
...
- `@workcell/plugin-test-harness`
- `create-workcell-plugin` starter template
...
Workcell should not implement a generic in-process hook bag modeled directly after local coding tools.

Workcell should implement:
...
That is the complete target design for the Workcell plugin system.
