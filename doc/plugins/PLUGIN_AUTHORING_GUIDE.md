...
This guide describes the current, implemented way to create a Workcell plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main Workcell app.
...
- The host provides a small shared React component kit through
  `@workcell/plugin-sdk/ui`; use it for common Workcell controls before
...
```bash
workcell plugin init @yourscope/plugin-name --output /absolute/path/to/plugin-repos
```
...
Inside this monorepo, the scaffold uses `workspace:*` for `@workcell/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@workcell/plugin-sdk` from the local Workcell checkout into a `.workcell-sdk/` tarball so you can build and test a plugin without publishing anything to npm first. Pass `--sdk-path /absolute/path/to/workcell/packages/plugins/sdk` if you have more than one Workcell checkout.

## Local development workflow

See the short [Local Plugin Development guide](./LOCAL_PLUGIN_DEVELOPMENT.md) for the full happy path (`pnpm dev` → `workcell plugin install <absolute-path>` → `workcell plugin list`) and reload semantics.
...
## Managed Workcell resources

Plugins that provide durable Workcell business objects should declare them in
...
- Managed agents: declare top-level `agents[]` and require
  ...
  normal Workcell agents with plugin ownership metadata, not background plugin
  workers.
...
- Managed routines: declare top-level `routines[]` and require
  ...
  jobs that should create visible Workcell issues. Prefer managed routines over
...
- Managed skills: declare top-level `skills[]` and require `skills.managed`.
  ...
  synced into Workcell managed agents.
  ...
  agents: [
    {
      ...
      instructions: {
        content: "Follow the Workcell heartbeat and produce concise research briefs.",
      },
    },
  ],
  projects: [
...
```ts
import { definePlugin } from "@workcell/plugin-sdk";
...
- Use managed routines for recurring or externally triggered work that should
  ...
  triggers, and each run has the normal Workcell issue/audit trail.
...
- Keep defaults conservative. Managed declarations are suggestions owned by the
  plugin, but the resulting resources are normal Workcell records that the
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@workcell/plugin-sdk/ui`
...
Use shared components from `@workcell/plugin-sdk/ui` when the plugin needs a
Workcell-native control. The host owns the implementation, so plugins inherit
the board's current styling, ordering, recent selections, and dark-mode behavior
without importing `ui/src` internals.

Prefer shared components for common Workcell UX patterns to reduce drift and
...
Use `FileTree` from `@workcell/plugin-sdk/ui` whenever the plugin only needs
