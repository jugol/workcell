...
description: >
  Handle a GitHub Security Advisory response for Workcell, including
...
```
gh api repos/workcell/workcell/security-advisories/{{ghsaId}}
...
```
gh api --method POST \
  repos/workcell/workcell/security-advisories/{{ghsaId}}/forks
...
```
# Clone the private fork somewhere outside ~/workcell
...
**Do not edit `~/workcell`** — the dev server is running off the `~/workcell` master branch and we don't want to touch it. All work happens in the private fork clone.
...
```
gh api --method POST \
  repos/workcell/workcell/security-advisories/{{ghsaId}}/cve
...
```
gh api repos/workcell/workcell/security-advisories/{{ghsaId}} --jq '.credits'
...
```
gh api --method PATCH \
  repos/workcell/workcell/security-advisories/{{ghsaId}} \
  --input - << 'EOF'
{
  "credits": [
    {
...
### Advisory
https://github.com/workcell/workcell/security/advisories/{{ghsaId}}
...
gh release view v{{patchedVersion}} --repo workcell/workcell
...
* The published advisory URL: `https://github.com/workcell/workcell/security/advisories/{{ghsaId}}`
* The release URL
