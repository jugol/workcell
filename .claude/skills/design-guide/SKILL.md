...
name: design-guide
description: >
  Workcell UI design system guide for building consistent, reusable frontend
...
# Workcell Design Guide

Workcell's UI is a professional-grade control plane — dense, keyboard-driven, **dark-themed by default** — a dim charcoal-gray "operations board," easy on the eyes (not harsh white, not pure black); light remains available via the toggle (the "Orchestration" direction). Every pixel earns its place.

**Brand identity (WC-70 — Orchestration direction):** the product mark is the **orchestration hub** (`WorkcellMark`, `ui/src/components/WorkcellMark.tsx`) — a central orchestrator node driving connected agent nodes ("a company of agents under one board"). The signature **brand accent** is a corporate indigo-blue (`oklch(0.48 0.17 262)` light / `oklch(0.62 0.18 262)` dark) applied to `--primary` / `--ring` / `--sidebar-primary` ONLY; surfaces stay neutral and status/priority colors are owned by `StatusBadge`/`StatusIcon` (never repurpose the brand hue for status).
...
- **Dark theme default (Orchestration direction).** A dim charcoal-gray base (OKLCH) — easy on the eyes, not harsh white, not pure black. A single brand indigo-blue accent on primary actions / focus / active nav; status & priority keep their own colors; text is the primary visual element. Light theme stays fully supported via the toggle.
...
| Panel / section header | `text-base font-semibold` | De-facto tier for in-page panel and section headers (~14 pages); use this for sub-page headings between section title and card title |
...
2. **Custom composites** (`ui/src/components/`) — StatusBadge, EntityRow, MetricCard, etc. These capture Workcell-specific design language.
