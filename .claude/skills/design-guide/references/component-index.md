...
**Props:** `nodes: GraphCanvasNode[]`, `edges: GraphCanvasEdge[]`, `selectedNodeId?`, `onNodeClick?`, `onBackgroundClick?`, `className?`, `aria-label?`
**Types:** `GraphCanvasNode = { id; label; badge?; tint? }`, `GraphCanvasEdge = { id; fromNodeId; toNodeId; label? }`
**Usage:** Generic pannable/zoomable SVG node-edge canvas. Data-shape-agnostic — callers pass plain nodes/edges and presentation (per-node `tint` classes, optional `badge`/edge `label`, selection); a deterministic radial layout is computed internally, so no coordinates are required. Drag to pan, wheel or the zoom buttons to zoom. Pure presentation (no fetching). Used by the per-agent memory tab (`AgentMemoryTab`).

```tsx
<GraphCanvas
  nodes={[{ id: "a", label: "Prefers TypeScript", badge: "Fact", tint: kindTint }]}
  edges={[{ id: "e1", fromNodeId: "a", toNodeId: "b", label: "supports" }]}
...
<Identity name="Orchestrator Agent" />
...
**File:** `WorkcellPrinciples.tsx`
**Props:** `variant?: "card" | "bare"`, `heading?: string | null`, `className?: string`
**Usage:** States Workcell's operating model directly in the UI — the board sets direction and approves, the Orchestrator turns direction into issues and runs the work like a company, agents are staffed by function (not C-suite titles), and nothing is Done without proof. Mounted on the no-company Dashboard empty-state and onboarding step 1. Exports `WORKCELL_PRINCIPLES` (the data array) for reuse. `variant="card"` wraps in bordered card chrome; `variant="bare"` drops it. Pass `heading={null}` to hide the "How Workcell works" label.

```tsx
<WorkcellPrinciples />
<WorkcellPrinciples variant="bare" heading="Our principles" />
```

Keep the copy in sync with the Orchestrator agent's seeded instructions (`server/src/onboarding-assets/orchestrator/AGENTS.md`).
